...
SELECT count(*)
FROM
  (SELECT DISTINCT unions.communication_device_type_code,
                   unions.communication_device_num,
                   unions.patid,
                   unions.full_server_nm
   FROM
     (SELECT cardioaccess_demographics_stg.patid,
      ...
      UNION DISTINCT SELECT cardioaccess_demographics_stg.patid,
      ...
      UNION DISTINCT SELECT cardioaccess_demographics_stg.patid,
                            ...
                            cardioaccess_demographics_stg.full_server_nm
      FROM {{ params.param_clinical_cdm_stage_dataset_name }}.cardioaccess_demographics_stg) AS unions
   INNER JOIN {{ params.param_clinical_cdm_core_dataset_name }}.ca_communication_device AS cad
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(unions.communication_device_num) = upper(cad.communication_device_num)
   WHERE unions.communication_device_num IS NOT NULL ) AS a
);
...
SELECT count(*)
FROM
  (SELECT ca_junc_patient_communication_device.*
   FROM {{ params.param_clinical_cdm_core_dataset_name }}.ca_junc_patient_communication_device
   WHERE ca_junc_patient_communication_device.dw_last_update_date_time >=
