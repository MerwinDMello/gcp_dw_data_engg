  ...
  (SELECT DISTINCT wrk0.communication_device_type_code,
   ...
   FROM
     (SELECT DISTINCT unions.communication_device_type_code,
      ...
      FROM
        (SELECT cardioaccess_contacts_stg.contactid,
         ...
         FROM {{ params.param_clinical_cdm_stage_dataset_name }}.cardioaccess_contacts_stg
         UNION DISTINCT SELECT cardioaccess_contacts_stg.contactid,
         ...
         UNION DISTINCT SELECT cardioaccess_contacts_stg.contactid,
                               ...
                               cardioaccess_contacts_stg.full_server_nm
         FROM {{ params.param_clinical_cdm_stage_dataset_name }}.cardioaccess_contacts_stg) AS unions
      WHERE unions.communication_device_num IS NOT NULL ) AS wrk0
   INNER JOIN {{ params.param_clinical_cdm_core_dataset_name }}.ca_communication_device AS cad
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(wrk0.communication_device_num) = upper(cad.communication_device_num)) AS b
);
