...
FROM
  (SELECT DISTINCT unions.communication_device_num
   ...
   WHERE unions.communication_device_num IS NOT NULL
     AND NOT EXISTS
       (SELECT 1
        FROM {{ params.param_clinical_cdm_core_dataset_name }}.ca_communication_device AS tgt
...
SELECT count(*)
FROM
  (SELECT ca_communication_device.*
   FROM {{ params.param_clinical_cdm_core_dataset_name }}.ca_communication_device
   WHERE ca_communication_device.dw_last_update_date_time >=
