...
FROM (SELECT concat(coalesce(CAST(sum(x.eor_gross_reimbursement_amt) AS STRING), '0'), coalesce(CAST(sum(x.eor_contractual_allowance_amt) AS STRING), '0'), coalesce(CAST(sum(x.eor_insurance_payment_amt) AS STRING), '0')) AS source_string
FROM
...
(
SELECT SPLIT(SOURCE_STRING,',') values_list
FROM (SELECT concat(coalesce(CAST(sum(disc.exp_gross_rbmt_rate_amt) AS STRING), '0'), coalesce(CAST(sum(disc.exp_cont_alw_rate_amt) AS STRING), '0'), coalesce(CAST(sum(disc.exp_payment_rate_amt) AS STRING), '0')) AS source_string
