...
FROM (SELECT trim(CAST(coalesce(sum(du.unbilled_charge_amt), NUMERIC '0') AS STRING)) AS source_string
FROM {{ params.param_auth_base_views_dataset_name }}.discharged_unbilled AS du
INNER JOIN {{ params.param_auth_base_views_dataset_name }}.facility_dimension AS fd ON upper(fd.company_code) = upper(du.company_code)
...
(
SELECT SPLIT(SOURCE_STRING,',') values_list
FROM (SELECT trim(CAST(coalesce(sum(fact_rcom_ar_patient_level.unbilled_gross_med_rec_amt), NUMERIC '0') AS STRING)) AS source_string
FROM {{ params.param_pbs_base_views_dataset_name }}.fact_rcom_ar_patient_level
