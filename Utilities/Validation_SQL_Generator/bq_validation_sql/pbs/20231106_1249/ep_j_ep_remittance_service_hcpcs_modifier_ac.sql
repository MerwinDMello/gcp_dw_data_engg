...
FROM
  (SELECT remittance_service.service_guid AS service_guid,
          'A' AS hcpcs_type_ind,
   ...
   UNION DISTINCT SELECT remittance_service.service_guid AS service_guid,
                         'A' AS hcpcs_type_ind,
   ...
   UNION DISTINCT SELECT remittance_service.service_guid AS service_guid,
                         ...
                         timestamp_trunc(current_timestamp(), SECOND) AS dw_last_update_date_time
   FROM {{ params.param_pbs_stage_dataset_name }}.remittance_service
   WHERE upper(coalesce(remittance_service.procedure_modifier_6_code6, '')) NOT IN('') ) AS a ;)
);
...
SELECT SPLIT(SOURCE_STRING,',') values_list
FROM (SELECT format('%20d', count(*)) AS source_string
FROM {{ params.param_pbs_core_dataset_name }}.remittance_service_hcpcs_modifier ;)
);
