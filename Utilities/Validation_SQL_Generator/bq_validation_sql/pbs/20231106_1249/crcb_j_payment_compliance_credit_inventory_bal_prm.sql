...
FROM (SELECT concat(format('%20d', coalesce(count(*), 0)), CAST(coalesce(sum(stg.refund_amt), NUMERIC '0') AS STRING), CAST(coalesce(sum(stg.total_acct_bal_amt), NUMERIC '0') AS STRING)) AS source_string
...
(
SELECT SPLIT(SOURCE_STRING,',') values_list
FROM (SELECT concat(format('%20d', coalesce(count(*), 0)), CAST(coalesce(sum(cbd.refund_amt), NUMERIC '0') AS STRING), CAST(coalesce(sum(cbd.total_account_balance_amt), NUMERIC '0') AS STRING)) AS source_string
FROM {{ params.param_pbs_core_dataset_name }}.payment_compliance_credit_inventory AS cbd
