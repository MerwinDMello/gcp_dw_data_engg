...
FROM
  (SELECT a.claim_guid AS claim_guid,
          ROUND(CASE coalesce(format('%#20.0f', fp.patient_dw_id), '999999')
                    WHEN '' THEN NUMERIC '0'
          ...
          parse_date('%Y/%m/%d', CASE
                                     WHEN length(a.received_date) > 0 THEN CASE
                                                                               WHEN length(SPLIT(a.received_date, '/')[ORDINAL(1)]) = 1
                                                                                    AND length(SPLIT(a.received_date, '/')[ORDINAL(2)]) = 1 THEN concat(SPLIT(a.received_date, '/')[ORDINAL(3)], '/0', SPLIT(a.received_date, '/')[ORDINAL(1)], '/0', SPLIT(a.received_date, '/')[ORDINAL(2)])
                                                                               WHEN length(SPLIT(a.received_date, '/')[ORDINAL(1)]) = 1 THEN concat(SPLIT(a.received_date, '/')[ORDINAL(3)], '/0', SPLIT(a.received_date, '/')[ORDINAL(1)], '/', SPLIT(a.received_date, '/')[ORDINAL(2)])
                                                                               WHEN length(SPLIT(a.received_date, '/')[ORDINAL(2)]) = 1 THEN concat(SPLIT(a.received_date, '/')[ORDINAL(3)], '/', SPLIT(a.received_date, '/')[ORDINAL(1)], '/0', SPLIT(a.received_date, '/')[ORDINAL(2)])
                                                                               ELSE concat(SPLIT(a.received_date, '/')[ORDINAL(3)], '/', SPLIT(a.received_date, '/')[ORDINAL(1)], '/', SPLIT(a.received_date, '/')[ORDINAL(2)])
                                                                           END
   ...
   FROM {{ params.param_pbs_stage_dataset_name }}.remittance_claim AS a
   LEFT OUTER JOIN
     (SELECT fact_patient.*
      FROM {{ params.param_auth_base_views_dataset_name }}.fact_patient
      WHERE upper(fact_patient.source_system_code) = 'P' ) AS fp ON fp.pat_acct_num = a.remit_account_number
   AND upper(a.provider_split_coid) = upper(CASE
                                                WHEN upper(fp.coid) = '08158'
                                                     AND CASE fp.sub_unit_num
                                                             WHEN '' THEN 0.0
                                                ...
                                                ELSE fp.coid
                                            END)
   LEFT OUTER JOIN {{ params.param_pbs_core_dataset_name }}.ref_remittance_subscriber_info AS b
   FOR system_time AS OF timestamp(tableload_start_time, 'US/Central') ON upper(coalesce(b.patient_health_ins_num, '')) = upper(coalesce(a.ins_subscriber_id, ''))
   ...
   LEFT OUTER JOIN {{ params.param_pbs_core_dataset_name }}.ref_remittance_oth_subscriber_info AS c
   FOR system_time AS OF timestamp(tableload_start_time, 'US/Central') ON upper(coalesce(a.othr_subcbr_enty_typ_qulfr_cod, '')) = upper(coalesce(c.oth_subscriber_enty_type_qualifier_code, ''))
   ...
   LEFT OUTER JOIN {{ params.param_pbs_core_dataset_name }}.ref_remittance_rendering_provider AS d
   FOR system_time AS OF timestamp(tableload_start_time, 'US/Central') ON upper(coalesce(d.serv_provider_enty_type_qualifier_code, '')) = upper(coalesce(a.srvce_prvdr_enty_typ_qulfr_cod, ''))
   ...
   LEFT OUTER JOIN {{ params.param_pbs_core_dataset_name }}.ref_remittance_cob_carrier AS e
   FOR system_time AS OF timestamp(tableload_start_time, 'US/Central') ON upper(coalesce(e.cob_qualifier_code, '')) = upper(coalesce(a.crossover_carrier_qualifr_code, ''))
   ...
   LEFT OUTER JOIN {{ params.param_pbs_core_dataset_name }}.ref_remittance_corrected_priority_payor AS f
   FOR system_time AS OF timestamp(tableload_start_time, 'US/Central') ON upper(coalesce(f.corrected_priority_payor_qualifier_code, '')) = upper(coalesce(a.corretd_prior_payor_qulifr_cod, ''))
   ...
   WHERE DATE(a.dw_last_update_date_time) =
       (SELECT max(DATE(remittance_claim.dw_last_update_date_time))
        FROM {{ params.param_pbs_stage_dataset_name }}.remittance_claim) ) AS a ;)
);
...
SELECT SPLIT(SOURCE_STRING,',') values_list
FROM (SELECT format('%20d', count(*)) AS source_string
FROM {{ params.param_pbs_core_dataset_name }}.remittance_claim
WHERE DATE(remittance_claim.dw_last_update_date_time) =
    (SELECT DATE(max(remittance_claim_0.dw_last_update_date_time))
