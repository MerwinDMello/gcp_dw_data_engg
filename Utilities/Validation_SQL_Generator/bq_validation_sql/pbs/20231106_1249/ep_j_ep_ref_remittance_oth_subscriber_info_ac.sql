...
FROM
  (SELECT
     (SELECT coalesce(max(ref_remittance_oth_subscriber_info.remittance_oth_subscriber_sid), 0)
      FROM {{ params.param_pbs_core_dataset_name }}.ref_remittance_oth_subscriber_info
      FOR system_time AS OF timestamp(tableload_start_time, 'US/Central')) + row_number() OVER (
                                                                                                ORDER BY upper(f.oth_subscriber_id_qualifier_code),
                                                                                                         upper(f.oth_subscriber_id),
                                                                                                         upper(f.oth_subscriber_enty_type_qualifier_code),
                                                                                                         upper(f.oth_subscriber_last_name),
                                                                                                         upper(f.oth_subscriber_first_name),
                                                                                                         upper(f.oth_subscriber_middle_name),
                                                                                                         upper(f.oth_subscriber_name_suffix)) AS remittance_oth_subscriber_sid, f.oth_subscriber_id_qualifier_code,
                                                                                                                                                                                f.oth_subscriber_id,
   ...
   FROM
     (SELECT max(remittance_claim.othr_subcbr_enty_typ_qulfr_cod) AS oth_subscriber_enty_type_qualifier_code,
      ...
      GROUP BY upper(remittance_claim.othr_subcbr_enty_typ_qulfr_cod),
               upper(remittance_claim.other_subscriber_last_name),
               upper(remittance_claim.other_subscriber_first_name),
               upper(remittance_claim.other_subscriber_middle_name),
               upper(remittance_claim.other_subscriber_name_suffix),
               upper(remittance_claim.othr_subcbr_idntfctn_qulfr_cod),
               upper(remittance_claim.other_subscriber_id)) AS f
   WHERE (upper(f.oth_subscriber_id_qualifier_code),
          upper(f.oth_subscriber_id),
          upper(f.oth_subscriber_enty_type_qualifier_code),
          upper(f.oth_subscriber_last_name),
          upper(f.oth_subscriber_first_name),
          upper(f.oth_subscriber_middle_name),
          upper(f.oth_subscriber_name_suffix)) NOT IN
       (SELECT AS STRUCT upper(ref_remittance_oth_subscriber_info.oth_subscriber_id_qualifier_code) AS oth_subscriber_id_qualifier_code,
...
FROM {{ params.param_pbs_core_dataset_name }}.ref_remittance_oth_subscriber_info
WHERE ref_remittance_oth_subscriber_info.dw_last_update_date_time >= tableload_start_time - interval 1 MINUTE ;)
);
