...
FROM
  (SELECT
     (SELECT coalesce(max(ref_remittance_additional_payee.remittance_additional_payee_sid), 0)
      FROM {{ params.param_pbs_core_dataset_name }}.ref_remittance_additional_payee
      FOR system_time AS OF timestamp(tableload_start_time, 'US/Central')) + row_number() OVER (
                                                                                                ORDER BY upper(f.additional_payee_id_qualifier_code),
                                                                                                         upper(f.additional_payee_id)) AS remittance_additional_payee_sid, f.additional_payee_id_qualifier_code,
                                                                                                                                                                           f.additional_payee_id,
                                                                                                                                                                           'E' AS source_system_code,
                                                                                                                                                                           timestamp_trunc(current_timestamp(), SECOND) AS dw_last_update_date_time
   FROM
     (SELECT remittance_payment.reference_id_qualifier1_code AS additional_payee_id_qualifier_code,
      ...
      UNION DISTINCT SELECT remittance_payment.reference_id_qualifier2_code AS additional_payee_id_qualifier_code,
      ...
      UNION DISTINCT SELECT remittance_payment.reference_id_qualifier3_code AS additional_payee_id_qualifier_code,
      ...
      UNION DISTINCT SELECT remittance_payment.reference_id_qualifier4_code AS additional_payee_id_qualifier_code,
      ...
      WHERE upper(coalesce(remittance_payment.reference_id_qualifier4_code, '')) NOT IN('')
        OR upper(coalesce(remittance_payment.additional_payee4_id, '')) NOT IN('') ) AS f) AS a ;)
);
...
FROM (SELECT format('%20d', count(*)) AS source_string
FROM {{ params.param_pbs_core_dataset_name }}.ref_remittance_additional_payee
WHERE ref_remittance_additional_payee.dw_last_update_date_time >= tableload_start_time - interval 1 MINUTE ;)
);
