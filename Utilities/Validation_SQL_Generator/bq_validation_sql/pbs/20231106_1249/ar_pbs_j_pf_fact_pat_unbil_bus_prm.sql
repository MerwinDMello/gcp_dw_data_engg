...
FROM (SELECT coalesce(trim(CAST(coalesce(sum(du.unbilled_charge_amt), NUMERIC '0') AS STRING)), '0') AS source_string
...
(
SELECT SPLIT(SOURCE_STRING,',') values_list
FROM (SELECT coalesce(trim(CAST(coalesce(sum(fact_rcom_ar_patient_level.unbilled_gross_bus_ofc_amt), NUMERIC '0') AS STRING)), '0') AS source_string
