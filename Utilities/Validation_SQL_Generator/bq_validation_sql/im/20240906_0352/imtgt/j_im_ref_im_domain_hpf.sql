...
FROM
  (SELECT count(DISTINCT upper(document_work_flow_instance.instance_connection_string)) AS counts
   FROM {{ params.param_im_auth_base_views_dataset_name }}.document_work_flow_instance) AS a)
  );
...
FROM
  (SELECT count(*) AS counts
   ...
   WHERE ref_im_domain.application_system_id = 3 ) AS a)
  );
