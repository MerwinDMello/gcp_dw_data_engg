...
FROM
  (SELECT stg.cn_patient_complication_sid,
   ...
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_result AS rs
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(rtrim(coalesce(trim(stg.complicationoutcome), 'XX'))) = upper(rtrim(coalesce(trim(rs.nav_result_desc), 'XX')))
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_complication.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_base_views_dataset_name }}.cn_patient_complication) ) AS src)
  );
