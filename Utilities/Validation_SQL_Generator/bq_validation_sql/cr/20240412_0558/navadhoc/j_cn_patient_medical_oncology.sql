...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_medical_oncology_stg');
...
FROM
  (SELECT stg.cn_patient_medical_oncology_id,
   ...
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_facility AS REF
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(rtrim(stg.medical_oncology_facility_id)) = upper(rtrim(ref.facility_name))
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_medical_oncology.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_base_views_dataset_name }}.cn_patient_medical_oncology) ) AS a)
  );
...
FROM {{ params.param_cr_base_views_dataset_name }}.cn_patient_medical_oncology
WHERE cn_patient_medical_oncology.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
