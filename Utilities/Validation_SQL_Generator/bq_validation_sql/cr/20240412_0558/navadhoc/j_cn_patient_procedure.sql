...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_procedure_stg');
  ...
  (SELECT stg.cn_patient_procedure_sid,
   ...
   WHERE upper(trim(stg.hashbite_ssk)) NOT IN
       (SELECT upper(trim(cn_patient_procedure.hashbite_ssk))
        FROM {{ params.param_cr_base_views_dataset_name }}.cn_patient_procedure) ) AS a)
  );
...
WHERE cn_patient_procedure.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
