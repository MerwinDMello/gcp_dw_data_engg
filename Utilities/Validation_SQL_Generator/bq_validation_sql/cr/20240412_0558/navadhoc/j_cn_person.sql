...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_person_stg');
  ...
  (SELECT cn_person_stg.nav_patient_id,
   ...
   WHERE cn_person_stg.nav_patient_id NOT IN
       (SELECT cn_person.nav_patient_id
        FROM {{ params.param_cr_base_views_dataset_name }}.cn_person) ) AS src)
  );
...
FROM {{ params.param_cr_base_views_dataset_name }}.cn_person
WHERE cn_person.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
