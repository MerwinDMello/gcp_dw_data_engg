...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cr_patient_phone_num_stg');
...
FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_phone_num_stg)
  );
  ...
  SELECT SPLIT(SOURCE_STRING,',') values_list
  FROM (SELECT format('%20d', count(*)) AS source_string
FROM {{ params.param_cr_base_views_dataset_name }}.cr_patient_phone_num)
  );
