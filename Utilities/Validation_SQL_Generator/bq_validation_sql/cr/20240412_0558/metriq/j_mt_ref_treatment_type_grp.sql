...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_treatment_type_group_stg');
...
FROM {{ params.param_cr_stage_dataset_name }}.ref_treatment_type_group_stg)
  );
...
WHERE ref_treatment_type_group.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
