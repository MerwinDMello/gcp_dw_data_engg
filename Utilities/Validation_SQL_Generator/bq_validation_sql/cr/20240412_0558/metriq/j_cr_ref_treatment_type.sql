...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_cr_treatment_type_stg');
...
FROM {{ params.param_cr_stage_dataset_name }}.ref_cr_treatment_type_stg)
  );
...
WHERE ref_cr_treatment_type.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
