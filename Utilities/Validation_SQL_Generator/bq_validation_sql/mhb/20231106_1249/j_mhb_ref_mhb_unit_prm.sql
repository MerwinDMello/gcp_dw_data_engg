...
FROM
  (SELECT rdc.rdc_sid AS rdc_sid,
   ...
   FROM {{ params.param_clinical_ci_stage_dataset_name }}.vwunits AS vwunit
   INNER JOIN {{ params.param_clinical_ci_base_views_dataset_name }}.ref_mhb_regional_data_center AS rdc
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(substr(trim(vwunit.databasename), strpos(trim(vwunit.databasename), '_') + 1)) = upper(rdc.rdc_desc)
   ...
   LEFT OUTER JOIN {{ params.param_clinical_ci_base_views_dataset_name }}.ref_mhb_unit AS un
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON rdc.rdc_sid = un.rdc_sid
   AND vwunit.unit_id = un.mhb_unit_id
   WHERE un.rdc_sid IS NULL ) AS q)
  );
...
FROM
  (SELECT ref_mhb_unit.*
   FROM {{ params.param_clinical_ci_core_dataset_name }}.ref_mhb_unit
   WHERE ref_mhb_unit.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE ) AS q)
  );
