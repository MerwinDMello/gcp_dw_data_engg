...
FROM
  (SELECT rdc.rdc_sid,
   ...
   FROM {{ params.param_clinical_ci_stage_dataset_name }}.vwdynamiccaredetachment AS detachment
   ...
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(detachment.owner_user_name) = upper(mu.user_login_name)
   ...
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(trim(detachment.owner_user_role)) = upper(trim(rmur.mhb_user_role_desc))
  ...
  FROM (SELECT format('%20d', count(*)) AS source_string
FROM
  (SELECT mhb_care_detachment.*
   FROM {{ params.param_clinical_ci_core_dataset_name }}.mhb_care_detachment
   WHERE mhb_care_detachment.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE ) AS q)
  );
