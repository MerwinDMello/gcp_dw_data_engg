...
FROM
  (SELECT DISTINCT vwwctpoutboundmessages.destinationname
   FROM {{ params.param_clinical_ci_stage_dataset_name }}.vwwctpoutboundmessages
   WHERE upper(vwwctpoutboundmessages.destinationname) NOT IN
       (SELECT upper(ref_mhb_wctp_destination_name.wctp_destination_name) AS wctp_destination_name
        FROM {{ params.param_clinical_ci_core_dataset_name }}.ref_mhb_wctp_destination_name
...
FROM
  (SELECT ref_mhb_wctp_destination_name.*
   FROM {{ params.param_clinical_ci_core_dataset_name }}.ref_mhb_wctp_destination_name
   WHERE ref_mhb_wctp_destination_name.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE ) AS q)
  );
