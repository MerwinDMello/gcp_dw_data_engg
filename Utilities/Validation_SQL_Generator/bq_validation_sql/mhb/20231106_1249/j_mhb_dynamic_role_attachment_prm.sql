...
FROM
  (SELECT rdc.rdc_sid,
   ...
   FROM {{ params.param_clinical_ci_stage_dataset_name }}.vwdynamicroleattachment AS attachment
   ...
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(attachment.user_name) = upper(mu.user_login_name)
   ...
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(trim(attachment.user_actual_role)) = upper(trim(rmur.mhb_user_role_desc))
   ...
   LEFT OUTER JOIN {{ params.param_clinical_ci_base_views_dataset_name }}.mhb_dynamic_role_attachment AS dynamic_role_attachment
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON dynamic_role_attachment.rdc_sid = rdc.rdc_sid
   ...
   GROUP BY 1,
            ...
            15,
            upper(attachment.dynamicrole_phone),
            17,
            18,
            19) AS q)
  );
  ...
  FROM (SELECT format('%20d', count(*)) AS source_string
FROM
  (SELECT mhb_dynamic_role_attachment.*
   FROM {{ params.param_clinical_ci_core_dataset_name }}.mhb_dynamic_role_attachment
   WHERE mhb_dynamic_role_attachment.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE ) AS q)
  );
