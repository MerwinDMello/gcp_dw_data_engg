...
FROM
  (SELECT mhb_user_wrk.rdc_sid
   FROM {{ params.param_clinical_ci_stage_dataset_name }}.mhb_user_wrk
   WHERE (coalesce(mhb_user_wrk.rdc_sid, 0),
       ...
       (SELECT AS STRUCT coalesce(mhb_user.rdc_sid, 0),
                         upper(coalesce(mhb_user.user_login_name, '')),
                         coalesce(mhb_user.mhb_user_role_sid, 0), upper(coalesce(mhb_user.user_title_text, '')),
        ...
        FROM {{ params.param_clinical_ci_core_dataset_name }}.mhb_user
...
FROM
  (SELECT mhb_user.*
   FROM {{ params.param_clinical_ci_core_dataset_name }}.mhb_user
   WHERE mhb_user.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE ) AS q)
  );
