  ...
  (SELECT rdc.rdc_sid AS rdc_sid,
   ...
   FROM
     (SELECT vwtextmessages.messages_id,
      ...
      FROM {{ params.param_clinical_ci_stage_dataset_name }}.vwtextmessages
      GROUP BY 1,
   ...
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(rdc.rdc_desc) = upper(replace(x.databasename, 'heartbeatDW_', ''))
   LEFT OUTER JOIN {{ params.param_clinical_cl_base_views_dataset_name }}.clinical_facility AS cf
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON trim(cf.facility_mnemonic_cs) = trim(x.sender_facilitycode)) AS q)
  );
...
FROM
  (SELECT mhb_message.*
   FROM {{ params.param_clinical_ci_core_dataset_name }}.mhb_message
   WHERE mhb_message.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE ) AS q)
  );
