...
FROM (sel row_number() OVER (
 ORDER BY CR_Tumor_Primary_Site_Id,
 CN_Tumor_Type_Id,
 CN_General_Tumor_Type_Id,
 CN_Navque_Tumor_Type_Id,
 CP_ICD_Oncology_Code ASC) AS Cancer_Tumor_Driver_SK ,
 a.*
 FROM
 (SELECT DISTINCT COALESCE (t3.ICD_Oncology_Code,
 '-99') AS CP_ICD_Oncology_Code ,
 ...
 (--PT ID TumorTypes
 SELECT DISTINCT T4.*
 FROM EDWCR_VIEWS.Cancer_Patient_Id_Output t1
 INNER JOIN EDWCR_VIEWS.Ref_ICD_Oncology t4 ON t4.ICD_Oncology_Code = t1.Submitted_Primary_ICD_Oncology_Code
 AND t4.ICD_Oncology_Code NOT IN ('c421'))t3
 FULL JOIN --CR TumorTypes
 EDWCR_Views.Ref_Lookup_Code t1 ON SUBSTR(t1.Lookup_Code, 1, 3) = SUBSTR(t3.ICD_Oncology_Code, 1, 3)
 LEFT JOIN
 (--Navigation Tumor Module Lookup
 SELECT CASE
 WHEN t1.Tumor_Type_Desc IN ('Breast') THEN 'C509'
 ...
 WHERE t1.Tumor_Type_Group_Name NOT IN ('General',
 'NavQ',
 'NULL',
 'Other')
 AND t1.Tumor_Type_Group_Name IS NOT NULL )t4 ON SUBSTR(t1.Lookup_Code, 1, 3) = SUBSTR(t4.Nav_ICD_Xwalk, 1, 3)
 LEFT JOIN
 (--Navigation General Tumor Look Up
 SELECT CASE
 WHEN TRIM(t1.Tumor_Type_Desc) IN ('Adrenal') THEN 'C749'
 ...
 t1.*
 FROM EDWCR_Views.Ref_Tumor_Type t1
 WHERE t1.Tumor_Type_Group_Name IN ('General') )t5 ON SUBSTR(t1.Lookup_Code, 1, 3) = SUBSTR(t5.Nav_General_ICD_Xwalk, 1, 3)
 LEFT JOIN
 (--NavQue
 SELECT CASE
 WHEN TRIM(t1.Tumor_Type_Desc) IN ('GYN') THEN 'C579'
 WHEN TRIM(t1.Tumor_Type_Desc) IN ('Lung') THEN 'C349'
 ...
 t1.*
 FROM EDWCR_Views.Ref_Tumor_Type t1
 WHERE t1.Tumor_Type_Group_Name IN ('NavQ') )t6 ON SUBSTR(t1.Lookup_Code, 1, 3) = SUBSTR(t6.NavQ_ICD_Xwalk, 1, 3)
 LEFT JOIN
 (--Tumor Grouping
 SELECT CASE
 WHEN t2.ICD_Oncology_Code IN ('C23',
 'C21',
 'C24',
 'C15',
 'C22',
 ...
 'C26',
 'C17') THEN 'Complex GI'
 WHEN t2.ICD_Oncology_Code IN ('C51',
 'C52',
 'C53',
 'C54',
 'C55',
 ...
 source_system_code
 FROM
 (SELECT DISTINCT SUBSTR (t1.ICD_Oncology_Code,
 1,
 3) AS ICD_Oncology_Code ,
 t1.ICD_Oncology_Site_Desc ,
 t1.source_system_code
 FROM EDWCR_VIEWS.Ref_ICD_Oncology t1)t2)t7 ON SUBSTR(t1.Lookup_Code, 1, 3) = SUBSTR(t7.ICD_Oncology_Code, 1, 3)
 WHERE t1.Lookup_Id IN ('18') ) a)A