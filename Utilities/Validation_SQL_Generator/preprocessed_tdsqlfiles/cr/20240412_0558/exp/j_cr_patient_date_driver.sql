WITH SC AS
 (SELECT EMR_Date.*,
 (EDWCR_Date_Driver - INTERVAL '90' DAY) AS HospEMR_Date_Driver
 FROM
 ...
 MIN(User_Action_Date_Time) AS date_driver
 FROM EDWCR_Base_Views.Cancer_Patient_Id_Output t1
 JOIN Cancer_Patient_Driver t2 ON t1.Patient_DW_ID = t2.CP_Patient_ID
 ...
 4,
 5
 UNION SELECT t1.Cancer_Patient_Driver_SK,
 t1.Network_Mnemonic_CS,
 t1.Patient_Market_URN_Text,
 t1.Medical_Record_Num,
 t1.EMPI_Text,
 'Nav_Diagnosis' AS DateSource,
 2 AS DateSourceOrder,
 MIN(diagnosis_date) AS date_driver
 FROM EDWCR_Base_Views.Cancer_Patient_Driver t1
 JOIN CN_Patient t2 ON t1.CN_Patient_Id = t2.Nav_Patient_Id
 JOIN CN_Patient_Diagnosis t3 ON t2.Nav_Patient_Id = t3.Nav_Patient_ID
 ...
 t1.Medical_Record_Num,
 t1.EMPI_Text,
 'CR_Diagnosis' AS DateSource,
 1 AS DateSourceOrder,
 MIN(diagnosis_date) AS date_driver
 FROM EDWCR_Base_Views.Cancer_Patient_Driver t1
 JOIN CR_Patient_Diagnosis_Detail t2 ON t1.CR_Patient_ID = t2.CR_Patient_Id
 ...
 t1.Medical_Record_Num,
 t1.EMPI_Text,
 'CR_Admission' AS DateSource,
 5 AS DateSourceOrder,
 MIN(admission_date) AS date_driver
 FROM EDWCR_Base_Views.Cancer_Patient_Driver t1
 JOIN CR_Patient_Tumor t2 ON t1.CR_Patient_ID = t2.CR_Patient_Id
 ...
 4,
 5,
 6) info ON date_source.Cancer_Patient_Driver_SK = info.Cancer_Patient_Driver_SK
...
FROM
 ...
 FROM EDWCR_Base_Views.Fact_Encounter FE
 INNER JOIN SC ON FE.Medical_Record_Num = SC.Medical_Record_Num
 AND FE.Coid = SC.Coid
 WHERE Admission_Date_Time >= HospEMR_Date_Driver )a
 UNION ALL SELECT Cancer_Patient_Driver_SK,
 Patient_DW_Id,
 EDWCR_Date_Driver AS Cancer_Diagnosis_Date,
 HospEMR_Date_Driver AS Cancer_Diagnosis_90_Day_Prior_Date,
 CURRENT_TIMESTAMP(0) AS DW_Last_Update_Date_Time
 ...
 SC.HospEMR_Date_Driver
 FROM EDWCR_Base_Views.Fact_Encounter FE
 INNER JOIN EDW_PUB_VIEWS.Clinical_Facility CF ON FE.Coid = CF.Coid
 AND CF.Facility_Active_Ind = 'Y'
 INNER JOIN SC ON FE.Patient_Market_URN = SC.Patient_Market_URN_Text
 AND CF.Network_Mnemonic_CS = SC.Network_Mnemonic_CS
 WHERE Admission_Date_Time >= HospEMR_Date_Driver )b
 ...
 INNER JOIN SC ON FE.Medical_Record_Num = SC.Medical_Record_Num
 AND FE.Coid = SC.Coid
 WHERE Admission_Date_Time >= HospEMR_Date_Driver )a) )A;