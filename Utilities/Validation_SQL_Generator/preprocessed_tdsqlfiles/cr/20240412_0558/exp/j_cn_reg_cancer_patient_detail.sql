...
FROM
 ...
 ORDER BY MET.PATIENT_MARKET_URN_TEXT, MET.NETWORK_MNEMONIC_CS ASC) AS CANCER_PATIENT_DETAIL_SK,
 ...
 CASE
 WHEN GN.LOOKUP_DESC='MALE' THEN 'M'
 WHEN GN.LOOKUP_DESC='FEMALE' THEN 'F'
 ELSE 'U'
 END AS GENDER_CODE ,
 ...
 ORDER BY FACILITY_ACTIVE_IND DESC)=1) DF ON CP.COID =DF.COID
 LEFT OUTER JOIN EDWCR_base_views.CR_PATIENT_ADDRESS CPA ON CP.CR_PATIENT_ID = CPA.CR_PATIENT_ID
 LEFT OUTER JOIN EDWCR_base_views.CR_PATIENT_CONTACT CPC ON CP.CR_PATIENT_ID = CPC.CR_PATIENT_ID
 LEFT OUTER JOIN EDWCR_base_views.CR_PATIENT_PHONE_NUM CPPN ON CP.CR_PATIENT_ID = CPPN.CR_PATIENT_ID
 AND CPC.PATIENT_CONTACT_ID=CPPN.PATIENT_CONTACT_ID
 LEFT OUTER JOIN EDWCR_base_views.CR_PATIENT_INSURANCE CPI ON CP.CR_PATIENT_ID = CPI.CR_PATIENT_ID
 LEFT OUTER JOIN (SEL MASTER_LOOKUP_SID,
 LOOKUP_DESC
 FROM EDWCR_base_views.REF_LOOKUP_CODE
 WHERE LOOKUP_ID=1) VS ON CP.VITAL_STATUS_ID=VS.MASTER_LOOKUP_SID
 LEFT OUTER JOIN (SEL MASTER_LOOKUP_SID,
 LOOKUP_DESC
 FROM EDWCR_base_views.REF_LOOKUP_CODE
 WHERE LOOKUP_ID=2) RC ON CP.PATIENT_RACE_ID=RC.MASTER_LOOKUP_SID
 LEFT OUTER JOIN (SEL MASTER_LOOKUP_SID,
 LOOKUP_DESC
 FROM EDWCR_base_views.REF_LOOKUP_CODE
 WHERE LOOKUP_ID=3) GN ON CP.PATIENT_GENDER_ID=GN.MASTER_LOOKUP_SID
 LEFT OUTER JOIN (SEL MASTER_LOOKUP_SID,
 LOOKUP_DESC
 FROM EDWCR_base_views.REF_LOOKUP_CODE
 WHERE LOOKUP_ID=4) REL ON CPC.CONTACT_RELATION_ID=REL.MASTER_LOOKUP_SID
 LEFT OUTER JOIN (SEL MASTER_LOOKUP_SID,
 LOOKUP_DESC
 FROM EDWCR_base_views.REF_LOOKUP_CODE
 WHERE LOOKUP_ID=13) ITI ON CPI.INSURANCE_TYPE_ID=ITI.MASTER_LOOKUP_SID
 LEFT OUTER JOIN (SEL MASTER_LOOKUP_SID,
 LOOKUP_DESC
 FROM EDWCR_base_views.REF_LOOKUP_CODE
 WHERE LOOKUP_ID=14) ICI ON CPI.INSURANCE_COMPANY_ID=ICI.MASTER_LOOKUP_SID
 LEFT OUTER JOIN (SEL MASTER_LOOKUP_SID,
 LOOKUP_CODE,
 LOOKUP_DESC
 FROM EDWCR_base_views.REF_LOOKUP_CODE
 WHERE LOOKUP_ID=23) ST ON CPA.STATE_ID=ST.MASTER_LOOKUP_SID) MET
 FULL OUTER JOIN
 ...
 FROM EDWCR_BASE_VIEWS.CANCER_PATIENT_DRIVER QUALIFY ROW_NUMBER() OVER(PARTITION BY CR_PATIENT_ID
 ORDER BY CANCER_PATIENT_DRIVER_SK ASC)=1) CP ON MET.CR_PATIENT_ID=CP.CR_PATIENT_ID
 LEFT OUTER JOIN
 (SELECT CN_PATIENT_ID,
 CANCER_PATIENT_DRIVER_SK
 FROM EDWCR_BASE_VIEWS.CANCER_PATIENT_DRIVER QUALIFY ROW_NUMBER() OVER(PARTITION BY CN_PATIENT_ID
 ORDER BY CANCER_PATIENT_DRIVER_SK ASC)=1) CPN ON INAV.NAV_PATIENT_ID=CPN.CN_PATIENT_ID
 LEFT OUTER JOIN
 (SELECT CP_PATIENT_ID,
 CANCER_PATIENT_DRIVER_SK
 FROM EDWCR_BASE_VIEWS.CANCER_PATIENT_DRIVER QUALIFY ROW_NUMBER() OVER(PARTITION BY CP_PATIENT_ID
 ORDER BY CANCER_PATIENT_DRIVER_SK ASC)=1) CPP ON CPID.PATIENT_DW_ID=CPP.CP_PATIENT_ID)A