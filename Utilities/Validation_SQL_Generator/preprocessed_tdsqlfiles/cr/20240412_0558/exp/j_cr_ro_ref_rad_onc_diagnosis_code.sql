...
FROM
 ...
 DECODE(dp.DiagnosisCode, '', NULL, dp.DiagnosisCode) AS Diagnosis_Code,
 sc.DiagnosisSites AS Diagnosis_Site_Text,
 dp.DiagnosisCodeClsSchemeId AS Diagnosis_Code_Class_Schema_Id,
 DECODE(dp.DiagnosisClinicalDescriptionEN, '', NULL, dp.DiagnosisClinicalDescriptionEN) AS Diagnosis_Clinical_Desc,
 DECODE(dp.DiagnosisFullTitleENU, '', NULL, dp.DiagnosisFullTitleENU) AS Diagnosis_Long_Desc,
 DECODE(dp.DiagnosisTableENU, '', NULL, dp.DiagnosisTableENU) AS Diagnosis_Type_Code,
 dp.LogID AS Log_Id,
 dp.RunID AS Run_Id,
 ...
 Site_SK
 FROM EDWCR.Ref_Rad_Onc_Site) rr ON rr.Source_Site_Id = dp.DimSiteID
 LEFT OUTER JOIN
 (SELECT DISTINCT TRIM(DiagnosisCode) DiagnosisCode,
 DiagnosisSites
 FROM EDWCR_Staging.CR_SC_DIAGNOSISSITES_STG) sc ON sc.DiagnosisCode=dp.DiagnosisCode)STG