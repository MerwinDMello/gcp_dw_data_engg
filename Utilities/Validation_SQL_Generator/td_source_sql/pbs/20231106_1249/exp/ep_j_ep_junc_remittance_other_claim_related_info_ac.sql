SELECT 'J_EP_Junc_Remittance_Other_Claim_Related_Info' || ',' || CAST(COUNT(*) AS VARCHAR(20)) || ',' as Source_String from
(
SEL     
Claim_GUID,
Payment_GUID,
Reference_Id_Line_Num,
Ref_Sid,
...
WHERE Coalesce(Othr_Clm_Rel_Ref_Idn_Qul10_Cod,'') IS NOT IN ('') OR Coalesce(Other_Claim_Related_Id10  ,'') IS NOT IN ('') 
 ...
 LEFT OUTER  JOIN EDWPBS.Ref_Remittance_Other_Claim_Related_Info  RRAP ON 
F.Reference_Id =RRAP.Reference_Id AND
F.Reference_Id_Qualifier_Code=RRAP.Reference_Id_Qualifier_Code
)a ;
