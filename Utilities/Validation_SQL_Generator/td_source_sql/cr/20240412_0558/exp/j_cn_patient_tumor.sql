Select 'J_CN_PATIENT_TUMOR'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING from 
(
Select 
...
from
edwcr_staging.CN_Patient_Tumor_STG STG
Left outer join edwcr.Ref_Facility RF1
on
STG.TumorReferralSource=RF1.Facility_Name
Left outer join edwcr.Ref_Status RS
on STG.NavigationStatus = RS.Status_Desc
Left outer join edwcr.CN_Physician_Detail PD
on
STG.EndTreatmentPhysician=PD.Physician_Name
And PD.Physician_Phone_Num is null
Left Outer Join edwcr.Ref_Facility RF2
On
...
qualify row_number() over(partition by CN_Patient_Tumor_SID order by PD.Physician_ID Desc)=1
 where STG.Hashbite_SSK not in ( Select Hashbite_SSK from edwcr.CN_Patient_Tumor )
) SRC