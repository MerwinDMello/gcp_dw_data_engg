Select 'J_CN_PATIENT_DIAGNOSIS'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING from 
(
Select 
...
from 
edwcr_staging.CN_Patient_Diagnosis_STG STG
Left Outer Join edwcr.Ref_Side RS
on
STG.DiagnosisSide=RS.Side_Desc
Left Outer Join edwcr.Ref_Diagnosis_Detail RDD
on
...
where STG.Hashbite_SSK not in ( Select Hashbite_SSK from edwcr.CN_Patient_Diagnosis)) SRC