...
-- Translated to: BigQuery
 -- Locking table edwpf_base_views.FACT_PATIENT for for system_time as of timestamp(tableload_start_time,'US/Central') access
-- Locking table edwpf_base_views.Registration_Service for for system_time as of timestamp(tableload_start_time,'US/Central') access
-- Locking table edwpf_base_views.Ref_Service for for system_time as of timestamp(tableload_start_time,'US/Central') access
...
FROM
 ...
 1 AS billed_patient_cnt
   ...
   LEFT OUTER JOIN {{ params.param_auth_base_views_dataset_name }}.fact_rcom_ar_patient_level AS fp ON a.pat_acct_num = fp.patient_sid
