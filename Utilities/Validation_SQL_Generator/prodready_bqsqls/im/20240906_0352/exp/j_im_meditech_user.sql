...
FROM
  (SELECT DISTINCT t6.im_domain_id AS im_domain_id,
                   ...
                   CASE
                       WHEN upper(rtrim(t1.actv_ind)) = 'Y' THEN CASE
                                                                     WHEN rtrim(t2.prctnr_fclt_role_cd) IN('1hcaResdnt',
                                                                                                           '1hcaFellow',
                                                                                                           '1hcaStuden',
                                                                                                           '1hcaPharm',
                                                                                                           '2hcaActive',
   ...
   INNER JOIN
     (SELECT s1.role_plyr_sk,
      ...
      WHERE upper(rtrim(s1.company_code)) = 'H'
        AND s1.vld_to_ts = '9999-12-31 00:00:00' ) AS t2 ON t1.role_plyr_sk = t2.role_plyr_sk
 ...
 -- User_ID (3/4)
 {{ params.param_im_base_views_dataset_name }}.prctnr_role_idfn AS t3 ON t1.role_plyr_sk = t3.role_plyr_sk
   AND upper(rtrim(t3.registn_type_ref_cd)) = 'NPI'
   AND t3.vld_to_ts <> DATETIME '9999-12-31 00:00:00'
   LEFT OUTER JOIN -- NPI (CMS)
 {{ params.param_im_base_views_dataset_name }}.prctnr_role_idfn AS t5 ON t1.role_plyr_sk = t5.role_plyr_sk
 ...
 {{ params.param_im_base_views_dataset_name }}.prctnr_role_idfn AS t7 ON t1.role_plyr_sk = t7.role_plyr_sk
 ...
 {{ params.param_im_base_views_dataset_name }}.prctnr_actvt_dtl AS t8 ON t2.role_plyr_sk = t8.role_plyr_sk
   ...
   WHERE upper(rtrim(t1.actv_ind)) = 'Y'
     AND t1.vld_to_ts = '9999-12-31 00:00:00' QUALIFY row_number() OVER (PARTITION BY im_domain_id,
                                                                                      upper(t4.mt_user_id)
