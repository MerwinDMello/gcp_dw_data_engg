   ...
   INNER JOIN
     (SELECT prctnr_role_idfn.role_plyr_sk,
      ...
      WHERE upper(rtrim(prctnr_role_idfn.registn_type_ref_cd)) = 'NPI'
        ...
        AND rtrim(prctnr_role_idfn.id_txt) <> '' ) AS t2 ON t1.national_provider_id = CAST({{ params.param_im_bqutil_fns_dataset_name }}.cw_td_normalize_number(t2.npi) AS FLOAT64)
   INNER JOIN
     (SELECT prctnr_role_idfn.role_plyr_sk,
