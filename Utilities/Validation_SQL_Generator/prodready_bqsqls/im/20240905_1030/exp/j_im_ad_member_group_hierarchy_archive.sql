...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240905_1030/input/exp/j_im_ad_member_group_hierarchy_archive.sql
     ...
     (SELECT current_date('US/Central') AS ad_archive_date,
      ...
      FROM
        (SELECT d.im_domain_name AS ad_domain_name,
         ...
         FROM {{ params.param_im_core_dataset_name }}.ad_member_group_hierarchy AS h
         FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central')
         INNER JOIN {{ params.param_im_core_dataset_name }}.ad_member_user AS m
         FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON h.ad_member_id = m.ad_member_id
         INNER JOIN {{ params.param_im_core_dataset_name }}.ad_group_user AS g
         FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON h.ad_group_id = g.ad_group_id
         LEFT OUTER JOIN {{ params.param_im_core_dataset_name }}.ad_group_user AS p
         FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON h.ad_group_parent_id = p.ad_group_id
         INNER JOIN {{ params.param_im_core_dataset_name }}.ref_im_domain AS d
         FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON m.im_domain_id = d.im_domain_id
