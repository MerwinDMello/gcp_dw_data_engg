...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240904_0246/input/exp/j_im_mt_cl_user_last_activity.sql
...
FROM
  (SELECT t1.company_code AS company_code,
   ...
   FROM
     (SELECT mt_cl_user_activity.company_code AS company_code,
      ...
      FROM {{ params.param_im_stage_dataset_name }}.mt_cl_user_activity
      UNION ALL SELECT mt_cl_user_activity_hist.company_code AS company_code,
      ...
      FROM {{ params.param_im_stage_dataset_name }}.mt_cl_user_activity_hist) AS t1 QUALIFY row_number() OVER (PARTITION BY network_mnemonic_cs,
                                                                                                                            mt_user_mnemonic_cs
                                                                                                               ORDER BY mt_user_last_activity_date DESC) = 1) AS a