...
FROM
     ...
     (SELECT DISTINCT cpd.coid,
      ...
      INNER JOIN
        (SELECT cancer_tumor_driver.*
         FROM {{ params.param_cr_core_dataset_name }}.cancer_tumor_driver QUALIFY
         FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') row_number() OVER (PARTITION BY cancer_tumor_driver.cr_tumor_primary_site_id
      ...
      FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(rtrim(cpnt.submitted_primary_icd_oncology_code)) = upper(rtrim(t2.cp_icd_oncology_code))
      ...
      FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON CASE upper(rtrim(rtt.tumor_type_group_name))
      ...
      FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON cpt.tumor_primary_site_id = t.cr_tumor_primary_site_id
      INNER JOIN
        (SELECT DISTINCT cancer_patient_id_output.patient_dw_id,
                         cancer_patient_id_output.submitted_primary_icd_oncology_code
         FROM {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_output) AS cpnt ON cpd.cp_patient_id = cpnt.patient_dw_id
      INNER JOIN {{ params.param_cr_core_dataset_name }}.cancer_tumor_driver AS t2
      FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(rtrim(t2.cp_icd_oncology_code)) = upper(rtrim(cpnt.submitted_primary_icd_oncology_code))
      ...
      WHERE cpd.cr_patient_id IS NOT NULL
        ...
        AND (cpnt.patient_dw_id,
          ...
          (SELECT DISTINCT AS STRUCT /*AND
           ...
           FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(rtrim(t2_0.cp_icd_oncology_code)) = upper(rtrim(cpnt_0.submitted_primary_icd_oncology_code))
      ...
      WHERE cpd.cr_patient_id IS NOT NULL
        ...
        AND (cpt.cr_patient_id,
          ...
          (SELECT DISTINCT AS STRUCT /*AND
           ...
           FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON cpt_0.tumor_primary_site_id = t_0.cr_tumor_primary_site_id
           INNER JOIN
             (SELECT DISTINCT cancer_patient_id_output.patient_dw_id,
                              cancer_patient_id_output.submitted_primary_icd_oncology_code
      ...
      WHERE cpd.cr_patient_id IS NOT NULL
        ...
        AND (cpnt.nav_patient_id,
          ...
          (SELECT DISTINCT AS STRUCT /*AND
           ...
           FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON CASE upper(rtrim(rtt_0.tumor_type_group_name))
 ...
 /* Both CN and CP have data and no data in CR*/ cpd.coid,
      ...
      INNER JOIN {{ params.param_cr_core_dataset_name }}.cancer_tumor_driver AS t2
      FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(rtrim(t2.cp_icd_oncology_code)) = upper(rtrim(cpnt2.submitted_primary_icd_oncology_code))
      WHERE cpd.cr_patient_id IS NULL
        AND cpd.cp_patient_id IS NOT NULL
        AND cpd.cn_patient_id IS NOT NULL
      ...
      WHERE cpd.cr_patient_id IS NULL
        ...
        AND (cpnt2.patient_dw_id,
          ...
          (SELECT DISTINCT AS STRUCT cpnt2_0.patient_dw_id,
           ...
           INNER JOIN {{ params.param_cr_core_dataset_name }}.cancer_tumor_driver AS t2_0
           FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(rtrim(t2_0.cp_icd_oncology_code)) = upper(rtrim(cpnt2_0.submitted_primary_icd_oncology_code))
           WHERE cpd_0.cr_patient_id IS NULL
             AND cpd_0.cp_patient_id IS NOT NULL
             AND cpd_0.cn_patient_id IS NOT NULL
          ...
          (SELECT DISTINCT AS STRUCT /* exclude CP data if CR and CP match*/ cpnt2.patient_dw_id,
           ...
           INNER JOIN
             (SELECT DISTINCT cancer_patient_id_output.patient_dw_id,
              ...
              FROM {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_output) AS cpnt2 ON cpd_0.cp_patient_id = cpnt2.patient_dw_id
           INNER JOIN {{ params.param_cr_core_dataset_name }}.cancer_tumor_driver AS t2_0
           FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(rtrim(t2_0.cp_icd_oncology_code)) = upper(rtrim(cpnt2.submitted_primary_icd_oncology_code))
           WHERE cpd_0.cr_patient_id IS NOT NULL
             AND cpd_0.cp_patient_id IS NOT NULL
             AND cpd_0.cn_patient_id IS NOT NULL
