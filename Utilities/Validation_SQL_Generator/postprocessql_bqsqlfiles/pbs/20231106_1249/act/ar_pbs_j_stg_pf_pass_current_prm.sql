...
FROM `hca-hin-dev-cur-parallon`.edwpbs_staging.pass_current_pf AS pc
WHERE upper(pc.rptg_period) = upper(format_date('%Y%m', date_add(current_date('US/Central'), interval -1 MONTH)))
  AND pc.dw_last_update_date_time IN
    (SELECT max(pass_current_pf.dw_last_update_date_time)
     FROM `hca-hin-dev-cur-parallon`.edwpbs_staging.pass_current_pf) 