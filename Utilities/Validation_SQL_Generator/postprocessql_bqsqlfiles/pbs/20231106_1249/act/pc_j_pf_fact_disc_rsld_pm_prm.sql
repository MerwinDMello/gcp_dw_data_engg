...
SELECT concat(format('%20d', coalesce(a.counts, 0)), CAST(coalesce(a.exp_gross_rbmt_crnt_amt, NUMERIC '0') AS STRING), CAST(coalesce(a.exp_cont_alw_crnt_amt, NUMERIC '0') AS STRING), CAST(coalesce(a.exp_payment_crnt_amt, NUMERIC '0') AS STRING), CAST(coalesce(a.exp_charge_crnt_amt, NUMERIC '0') AS STRING), CAST(coalesce(a.var_gross_rbmt_resolved_amt, NUMERIC '0') AS STRING), CAST(coalesce(a.var_cont_alw_resolved_amt, NUMERIC '0') AS STRING), CAST(coalesce(a.var_payment_resolved_amt, NUMERIC '0') AS STRING), CAST(coalesce(a.var_charge_resolved_amt, NUMERIC '0') AS STRING)) AS source_string
FROM
