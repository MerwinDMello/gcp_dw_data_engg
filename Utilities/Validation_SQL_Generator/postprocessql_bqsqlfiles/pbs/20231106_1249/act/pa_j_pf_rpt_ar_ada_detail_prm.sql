...
SELECT concat(coalesce(trim(CAST(sum(src.sum1) AS STRING)), '0'), coalesce(trim(CAST(sum(src.sum2) AS STRING)), '0'), coalesce(trim(CAST(sum(src.sum3) AS STRING)), '0')) AS source_string
FROM
  (SELECT sum(x.bad_debt_writeoff_amt) AS sum1,
          sum(x.non_secn_self_pay_ar_amt) AS sum2,
          sum(x.non_secn_unins_disc_amt) AS sum3
   FROM `hca-hin-dev-cur-parallon`.edwpbs.rpt_ar_ada_detail AS x
