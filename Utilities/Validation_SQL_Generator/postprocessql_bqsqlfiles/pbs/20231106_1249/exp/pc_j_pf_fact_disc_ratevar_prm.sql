...
FROM
  (SELECT max(c.company_code) AS company_code,
   ...
   FROM `hca-hin-dev-cur-parallon`.auth_base_views.explanation_of_reimbursement AS d
   INNER JOIN
     (SELECT max(k.company_code) AS company_code,
      ...
      FROM `hca-hin-dev-cur-parallon`.auth_base_views.explanation_of_reimbursement AS k
      INNER JOIN
        (SELECT max(h.company_code) AS company_code,
         ...
         FROM `hca-hin-dev-cur-parallon`.auth_base_views.explanation_of_reimbursement AS h
         INNER JOIN
           (SELECT max(a.company_code) AS company_code,
            ...
            FROM `hca-hin-dev-cur-parallon`.auth_base_views.explanation_of_reimbursement AS a
            INNER JOIN
              (SELECT max(b.company_code) AS company_code,
               ...
               FROM `hca-hin-dev-cur-parallon`.auth_base_views.explanation_of_reimbursement AS b
   ...
   LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.admission_discharge AS ad ON ad.patient_dw_id = d.patient_dw_id
   LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.facility AS ff ON upper(ff.coid) = upper(d.coid)
   AND upper(ff.company_code) = upper(d.company_code)
   INNER JOIN
     (SELECT pt.patient_dw_id,
             max(pt.eff_from_date) AS eff_from_date
      FROM `hca-hin-dev-cur-parallon`.auth_base_views.admission_patient_type AS pt
      GROUP BY 1) AS atp ON atp.patient_dw_id = d.patient_dw_id
   INNER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.admission_patient_type AS apt ON apt.patient_dw_id = d.patient_dw_id
   AND apt.eff_from_date = atp.eff_from_date
   LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.facility_reimbursement_log AS frl ON upper(d.company_code) = upper(frl.company_code)
