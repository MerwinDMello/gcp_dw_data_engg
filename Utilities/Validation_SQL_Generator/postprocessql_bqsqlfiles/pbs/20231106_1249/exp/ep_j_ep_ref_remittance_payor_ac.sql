...
FROM
  (SELECT
     (SELECT coalesce(max(ref_remittance_payor.remittance_payor_sid), 0)
      FROM `hca-hin-dev-cur-parallon`.edwpbs.ref_remittance_payor) + row_number() OVER (
                                                                                       ...
                                                                                       max(stg.payment_carrier_num) AS payment_carrier_num, -- EP_Payor_Num ,
 ...
 max(stg.payor_short_name) AS payor_short_name, /*Payor_Technical_Contact_Name ,
          ...
          Payor_Tertiary_Contact_Comm_Num,*/ 'E' AS source_system_code,
   ...
   WHERE DATE(stg.dw_last_update_date_time) =
     ...
     AND (upper(stg.payment_carrier_num),
       ...
       (SELECT AS STRUCT upper(ref_remittance_payor.payment_carrier_num) AS payment_carrier_num,
        ...
        FROM `hca-hin-dev-cur-parallon`.edwpbs.ref_remittance_payor)
