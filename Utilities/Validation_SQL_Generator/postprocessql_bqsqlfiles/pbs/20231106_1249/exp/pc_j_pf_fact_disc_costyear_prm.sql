...
FROM
  (SELECT count(*) AS ROW_COUNT
   FROM `hca-hin-dev-cur-parallon`.auth_base_views.dim_organization AS dorg
   LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.facility AS fac ON CASE fac.unit_num
                                                                                     WHEN '' THEN 0.0
