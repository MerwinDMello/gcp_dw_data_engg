...
FROM
 ...
 END AS payor_financial_class_sid
         FROM `hca-hin-dev-cur-parallon`.auth_base_views.denial_eom AS deom
         LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.admission AS adm ON adm.patient_dw_id = deom.patient_dw_id
         INNER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.eis_patient_type_dim AS ptd ON upper(concat('PT_', deom.patient_type_code)) = upper(ptd.patient_type_member)
         ...
         LEFT OUTER JOIN
           (SELECT rcom_payor_dimension_eom.payor_dw_id,
            ...
            FROM `hca-hin-dev-cur-parallon`.edwpbs_base_views.rcom_payor_dimension_eom) AS rpdeb ON deom.payor_dw_id = rpdeb.payor_dw_id
         AND adm.admission_date BETWEEN rpdeb.eff_from_date AND rpdeb.eff_to_date
         LEFT OUTER JOIN
           (SELECT rcom_payor_dimension_eom.payor_dw_id,
                   rcom_payor_dimension_eom.payor_id,
                   rcom_payor_dimension_eom.eff_from_date
            FROM `hca-hin-dev-cur-parallon`.edwpbs_base_views.rcom_payor_dimension_eom) AS rpde ON deom.payor_dw_id = rpde.payor_dw_id
