...
FROM
  (SELECT date_sub(date_add(CAST(trim(concat(format_date('%Y-%m', psc.charge_posted_date), '-01')) AS DATE), interval 1 MONTH), interval 6 DAY) AS date1,
   ...
   FROM `hca-hin-dev-cur-parallon`.auth_base_views.patient_suspended_charge AS psc
 ...
 `hca-hin-dev-cur-parallon`.auth_base_views.fact_facility AS ff ON upper(ff.coid) = upper(psc.coid)
