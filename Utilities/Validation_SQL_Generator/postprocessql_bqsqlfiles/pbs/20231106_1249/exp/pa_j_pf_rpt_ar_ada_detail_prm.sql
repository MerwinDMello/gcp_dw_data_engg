...
FROM
 ...
 sum(NUMERIC '0.000') AS transaction_amt
      FROM `hca-hin-dev-cur-parallon`.auth_base_views.pass_eom AS pem
      INNER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.facility_dimension AS fd ON upper(fd.unit_num) = upper(pem.unit_num)
      LEFT OUTER JOIN
        (SELECT max(k_0.coid) AS coid,
         ...
         FROM
           (SELECT max(a.coid) AS coid,
            ...
            FROM `hca-hin-dev-cur-parallon`.auth_base_views.ar_transaction_mv AS a
            INNER JOIN
              (SELECT pass_eom.*
   ...
   FROM `hca-hin-dev-cur-parallon`.auth_base_views.ar_transaction_mv AS ar
   INNER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.fact_patient AS fp ON ar.patient_dw_id = fp.patient_dw_id
   INNER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.facility_dimension AS fd ON upper(fd.unit_num) = upper(fp.unit_num)
   CROSS JOIN UNNEST(ARRAY[ date_sub(CAST(trim(concat(format_date('%Y-%m', current_date('US/Central')), '-01')) AS DATE), interval 1 DAY) ]) AS eom_end
