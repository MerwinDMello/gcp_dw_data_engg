...
FROM
  (SELECT
     (SELECT coalesce(max(ref_remittance_additional_payee.remittance_additional_payee_sid), 0)
      FROM `hca-hin-dev-cur-parallon`.edwpbs.ref_remittance_additional_payee) + row_number() OVER (
                                                                                                   ORDER BY upper(f.additional_payee_id_qualifier_code),
                                                                                                            upper(f.additional_payee_id)) AS remittance_additional_payee_sid, --  SID
