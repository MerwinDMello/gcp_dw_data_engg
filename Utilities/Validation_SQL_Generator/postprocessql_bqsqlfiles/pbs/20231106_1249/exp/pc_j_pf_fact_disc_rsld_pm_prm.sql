...
FROM
  (SELECT count(rsvd.patient_dw_id) AS rowcount,
   ...
   FROM `hca-hin-dev-cur-parallon`.auth_base_views.resolved_discrepancy AS rsvd
   LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.explanation_of_reimbursement AS eor ON upper(rsvd.company_code) = upper(eor.company_code)
   ...
   LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_rcom_pars_discrepancy AS disc ON disc.date_sid = CASE format_date('%Y%m', date_add(current_date('US/Central'), interval -1 MONTH))
                                                                                                                          WHEN '' THEN 0.0
   ...
   LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.admission_patient_type AS apt ON apt.patient_dw_id = rsvd.patient_dw_id
   AND apt.eff_from_date = atp.eff_from_date
   LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.admission_discharge AS ad ON ad.patient_dw_id = rsvd.patient_dw_id
   LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.auth_base_views.facility AS ff ON upper(ff.coid) = upper(rsvd.coid)
