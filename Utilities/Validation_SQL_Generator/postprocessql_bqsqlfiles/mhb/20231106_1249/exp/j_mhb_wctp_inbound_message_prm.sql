...
FROM
  (SELECT rdc.rdc_sid,
   ...
   FROM `hca-hin-dev-cur-clinical`.edwci_staging.vwwctpinboundmessages AS stg
 ...
 `hca-hin-dev-cur-clinical`.edwcl_base_views.clinical_facility AS cf ON trim(cf.facility_mnemonic_cs) = trim(stg.recipient_facilitycode)
   ...
   LEFT OUTER JOIN /*Left Join Edwcl_Base_Views.Clinical_Facility CF2
          on trim(CF2.Facility_Mnemonic_CS) = trim(stg.Recipient_FacilityCode)*/ `hca-hin-dev-cur-clinical`.edwcl_base_views.clinical_facility_location AS cfl ON trim(cfl.location_mnemonic_cs) = trim(substr(stg.recipient_unit_code, strpos(stg.recipient_unit_code, '_') + 1))
