...
FROM
  (SELECT rdc.rdc_sid,
          ...
          ROUND(CASE trim(coalesce(`hca-hin-dev-cur-clinical`.bqutil_fns.cw_regexp_extract(stg.launch_patient_visitnumber, '[0-9]+'), '999999999999'))
                    WHEN '' THEN NUMERIC '0'
                    ELSE CAST(trim(coalesce(`hca-hin-dev-cur-clinical`.bqutil_fns.cw_regexp_extract(stg.launch_patient_visitnumber, '[0-9]+'), '999999999999')) AS NUMERIC)
   ...
   FROM `hca-hin-dev-cur-clinical`.edwci_staging.vwinterappoutbound AS stg
 ...
 `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_user_role AS r10 ON upper(coalesce(trim(stg.user_role), 'Unknown')) = upper(trim(r10.mhb_user_role_desc))
   ...
   AND ck.pat_acct_num = ROUND(CASE trim(coalesce(`hca-hin-dev-cur-clinical`.bqutil_fns.cw_regexp_extract(stg.launch_patient_visitnumber, '[0-9]+'), '999999999999'))
                                   WHEN '' THEN NUMERIC '0'
                                   ELSE CAST(trim(coalesce(`hca-hin-dev-cur-clinical`.bqutil_fns.cw_regexp_extract(stg.launch_patient_visitnumber, '[0-9]+'), '999999999999')) AS NUMERIC)
   ...
   LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwcl_base_views.clinical_facility AS cf31 ON trim(cf31.facility_mnemonic_cs) = trim(stg.launch_patient_facility_code)
   AND upper(cf31.facility_active_ind) = 'Y'
   LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwcl_base_views.clinical_facility_location AS cfl2 ON trim(cfl2.location_mnemonic_cs) = trim(substr(stg.unitcode, strpos(stg.unitcode, '_') + 1))
   AND upper(cf31.coid) = upper(cfl2.coid)
   INNER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_unit AS rmu32 ON rmu32.rdc_sid = rdc.rdc_sid
   AND stg.launch_unit_id = rmu32.mhb_unit_id
   LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwcl_base_views.clinical_facility AS cf34 ON trim(cf34.facility_mnemonic_cs) = trim(stg.launch_facilitycode)
   AND upper(cf34.facility_active_ind) = 'Y'
   LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwcl_base_views.clinical_facility_location AS cfl33 ON trim(cfl33.location_mnemonic_cs) = trim(substr(stg.launch_unitcode, strpos(stg.launch_unitcode, '_') + 1))
