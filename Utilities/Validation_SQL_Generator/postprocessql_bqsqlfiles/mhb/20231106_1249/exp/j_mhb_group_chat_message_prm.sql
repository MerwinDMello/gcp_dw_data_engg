...
FROM
  (SELECT rdc.rdc_sid,
          ...
          ROUND(CASE trim(coalesce(`hca-hin-dev-cur-clinical`.bqutil_fns.cw_regexp_extract(stg.patient_visitnumber, '[0-9]+'), '999999999999'))
                    WHEN '' THEN NUMERIC '0'
                    ELSE CAST(trim(coalesce(`hca-hin-dev-cur-clinical`.bqutil_fns.cw_regexp_extract(stg.patient_visitnumber, '[0-9]+'), '999999999999')) AS NUMERIC)
          ...
          stg.lasttimestamp AS mhb_last_enter_date_time, -- TRIM(REGEXP_REPLACE(STG.MESSAGE_CONTENT, '[^a-z A-Z 0-9 ! ?]','',1,0,'i')) AS Message_Content_Text,
 ...
 timestamp_trunc(current_timestamp(), SECOND) AS dw_last_update_date_time
   FROM `hca-hin-dev-cur-clinical`.edwci_staging.vwgroupchatmessages AS stg
 ...
 `hca-hin-dev-cur-clinical`.edw_pub_views.clinical_facility AS cf2 ON trim(cf2.facility_mnemonic_cs) = stg.patient_facility_code
   ...
   AND ck.pat_acct_num = ROUND(CASE trim(coalesce(`hca-hin-dev-cur-clinical`.bqutil_fns.cw_regexp_extract(stg.patient_visitnumber, '[0-9]+'), '999999999999'))
                                   WHEN '' THEN NUMERIC '0'
                                   ELSE CAST(trim(coalesce(`hca-hin-dev-cur-clinical`.bqutil_fns.cw_regexp_extract(stg.patient_visitnumber, '[0-9]+'), '999999999999')) AS NUMERIC)
