...
FROM
  (SELECT mhb_alerting_audit_detail.*
   FROM `hca-hin-dev-cur-clinical`.edwci.mhb_alerting_audit_detail
   WHERE DATE(mhb_alerting_audit_detail.dw_last_update_date_time) = current_date('US/Central') ) AS q