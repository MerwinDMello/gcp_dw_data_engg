...
FROM
  (SELECT ref_mhb_message_delivery_priority.*
   FROM `hca-hin-dev-cur-clinical`.edwci.ref_mhb_message_delivery_priority
   WHERE DATE(ref_mhb_message_delivery_priority.dw_last_update_date_time) = current_date('US/Central') ) AS q