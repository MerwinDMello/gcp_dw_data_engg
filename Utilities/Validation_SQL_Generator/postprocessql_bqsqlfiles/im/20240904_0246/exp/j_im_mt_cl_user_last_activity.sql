  ...
  (SELECT t1.company_code AS company_code,
   ...
   FROM
     (SELECT mt_cl_user_activity.company_code AS company_code,
      ...
      FROM `hca-hin-dev-cur-comp`.edwim_staging.mt_cl_user_activity
      ...
      FROM `hca-hin-dev-cur-comp`.edwim_staging.mt_cl_user_activity_hist) AS t1 QUALIFY row_number() OVER (PARTITION BY network_mnemonic_cs,
                                                                                                                        mt_user_mnemonic_cs
