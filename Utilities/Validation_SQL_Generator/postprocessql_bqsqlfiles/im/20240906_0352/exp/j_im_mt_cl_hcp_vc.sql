...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240906_0352/input/exp/j_im_mt_cl_hcp_vc.sql
  ...
  (SELECT DISTINCT t1.hcp_dw_id AS hcp_dw_id,
   ...
   INNER JOIN
     (SELECT DISTINCT t2_0.hcp_other_id AS hcp_user_id_3_4,
                      t1_0.hcp_npi
      FROM `hca-hin-dev-cur-comp`.edwim_base_views.hcp AS t1_0
      ...
      AND `hca-hin-dev-cur-pub`.bqutil_fns.cw_regexp_instr_2(substr(trim(t2_0.hcp_other_id), 4, 4), '[A-Za-z_]') = 0
      AND `hca-hin-dev-cur-pub`.bqutil_fns.cw_regexp_instr_2(substr(trim(t2_0.hcp_other_id), 1, 3), '[0-9_]') = 0
   ...
   WHERE NOT t1.national_provider_id IS NULL ) AS a