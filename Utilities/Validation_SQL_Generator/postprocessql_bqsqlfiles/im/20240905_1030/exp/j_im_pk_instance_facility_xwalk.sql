...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240905_1030/input/exp/j_im_pk_instance_facility_xwalk.sql
  ...
  (SELECT count(*) AS counts
   FROM
     (SELECT DISTINCT t3.im_domain_id,
                      t2.company_code,
                      t2.coid
      FROM `hca-hin-dev-cur-comp`.auth_base_views.ref_pk_data_base_instance AS t1
      INNER JOIN `hca-hin-dev-cur-comp`.auth_base_views.pk_encounter AS t2 ON t1.pk_database_instance_sid = t2.pk_database_instance_sid
