...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240905_1030/input/exp/j_im_lawson_person_idmart.sql
      ...
      FROM
        (SELECT '21' AS im_domain_id,
                ...
                CASE
                    ...
                    ELSE 11
                END AS status_rank
         FROM `hca-hin-dev-cur-comp`.edwim_staging.lawsn_esaf_employee AS t1
         ...
         LEFT OUTER JOIN
           (SELECT s1.cmpy,
                   s1.supv_cd,
                   s2.cmpy_num
            FROM `hca-hin-dev-cur-comp`.edwim_staging.lawsn_esaf_supervisor AS s1
            INNER JOIN `hca-hin-dev-cur-comp`.edwim_staging.lawsn_esaf_personnel_employee AS s2 ON s2.empl = s1.empl) AS t3 ON t1.cmpy = t3.cmpy
           ...
           (SELECT s3.actn_cd,
            ...
            FROM
              (SELECT is3.actn_cd,
               ...
               FROM `hca-hin-dev-cur-comp`.edwim_staging.lawsn_esaf_personnel_action AS is3) AS s3
           ...
           (SELECT s4.cmpy,
            ...
            FROM
              (SELECT DISTINCT is4.cmpy,
               ...
               FROM `hca-hin-dev-cur-comp`.edwim_staging.lawsn_esaf_personnel_position AS is4
