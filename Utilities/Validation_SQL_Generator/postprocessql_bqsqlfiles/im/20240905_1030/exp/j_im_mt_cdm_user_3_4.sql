...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240905_1030/input/exp/j_im_mt_cdm_user_3_4.sql
...
FROM
  (SELECT prctnr_role_idfn.role_plyr_sk AS role_plyr_sk,
   ...
   WHERE upper(rtrim(prctnr_role_idfn.registn_type_ref_cd)) = 'LOGON_ID'
     ...
     AND `hca-hin-dev-cur-pub`.bqutil_fns.cw_regexp_instr_2(substr(mt_user_id, 4, 4), '[A-Za-z_]') = 0
     AND `hca-hin-dev-cur-pub`.bqutil_fns.cw_regexp_instr_2(substr(mt_user_id, 1, 3), '[0-9_]') = 0 QUALIFY row_number() OVER (PARTITION BY role_plyr_sk
