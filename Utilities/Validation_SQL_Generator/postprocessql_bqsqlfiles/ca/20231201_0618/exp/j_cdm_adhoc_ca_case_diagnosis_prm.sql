  ...
  (SELECT DISTINCT a.*
   FROM
     (SELECT NULL AS case_diagnosis_sk,
      ...
      FROM `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_diagnosis_stg AS stg
      LEFT OUTER JOIN
        (SELECT c.source_patient_case_num,
                s.server_name,
                c.patient_case_sk
         FROM `hca-hin-dev-cur-clinical`.edwcdm.ca_patient_case AS c
         INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS s ON c.server_sk = s.server_sk) AS a_0 ON stg.casenumber = a_0.source_patient_case_num
      ...
      LEFT OUTER JOIN
        (SELECT c.source_diagnosis_list_id,
                s.server_name,
                c.diagnosis_list_sk
         FROM `hca-hin-dev-cur-clinical`.edwcdm.ca_diagnosis_list AS c
         INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS s ON c.server_sk = s.server_sk) AS d ON stg.diagid = d.source_diagnosis_list_id
