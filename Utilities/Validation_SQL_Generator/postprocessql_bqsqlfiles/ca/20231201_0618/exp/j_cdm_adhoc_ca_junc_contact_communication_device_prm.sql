      ...
      FROM
        (SELECT cardioaccess_contacts_stg.contactid,
         ...
         UNION DISTINCT SELECT cardioaccess_contacts_stg.contactid,
                               ...
                               cardioaccess_contacts_stg.full_server_nm
         FROM `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_contacts_stg) AS unions
      WHERE unions.communication_device_num IS NOT NULL ) AS wrk0
   INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_communication_device AS cad ON upper(wrk0.communication_device_num) = upper(cad.communication_device_num)) AS b