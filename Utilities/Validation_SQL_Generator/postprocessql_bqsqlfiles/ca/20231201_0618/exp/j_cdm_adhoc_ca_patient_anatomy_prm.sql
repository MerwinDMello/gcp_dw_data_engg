  ...
  (SELECT DISTINCT a.*
   FROM
     (SELECT NULL AS patient_anatomy_sk,
      ...
      FROM `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_patanatomy_stg AS stg
      LEFT OUTER JOIN
        (SELECT c.source_patient_id,
                s.server_name,
                c.patient_sk
         FROM `hca-hin-dev-cur-clinical`.edwcdm.ca_patient AS c
         INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS s ON c.server_sk = s.server_sk) AS a_0 ON stg.patid = a_0.source_patient_id
