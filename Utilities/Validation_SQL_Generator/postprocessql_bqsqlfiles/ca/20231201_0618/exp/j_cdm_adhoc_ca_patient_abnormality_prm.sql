  ...
  (SELECT DISTINCT b.*
   FROM
     (SELECT CAST(NULL AS INT64) AS patient_abnormality_sk,
      ...
      FROM `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_syndrome_stg AS css
      LEFT OUTER JOIN
        (SELECT c.source_patient_id,
                s.server_name,
                c.patient_sk
         FROM `hca-hin-dev-cur-clinical`.edwcdm.ca_patient AS c
         INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS s ON c.server_sk = s.server_sk) AS a ON css.patid = a.source_patient_id
      AND upper(css.full_server_nm) = upper(a.server_name)
      INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS csrv ON upper(css.full_server_nm) = upper(csrv.server_name)
      LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_patient_abnormality AS ch ON ch.server_sk = csrv.server_sk
      ...
      UNION DISTINCT SELECT CAST(NULL AS INT64) AS patient_abnormality_sk,
                            ...
                            'C' AS source_system_code,
                            timestamp_trunc(current_timestamp(), SECOND) AS dw_last_update_date_time
      FROM `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_chromabnormalities_stg AS ccab
      LEFT OUTER JOIN
        (SELECT c.source_patient_id,
                s.server_name,
                c.patient_sk
         FROM `hca-hin-dev-cur-clinical`.edwcdm.ca_patient AS c
         INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS s ON c.server_sk = s.server_sk) AS a ON ccab.patid = a.source_patient_id
      AND upper(ccab.full_server_nm) = upper(a.server_name)
      INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS csrv ON upper(ccab.full_server_nm) = upper(csrv.server_name)
      LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_patient_abnormality AS ch ON ch.server_sk = csrv.server_sk
      ...
      UNION DISTINCT SELECT CAST(NULL AS INT64) AS patient_abnormality_sk,
                            ...
                            'C' AS source_system_code,
                            timestamp_trunc(current_timestamp(), SECOND) AS dw_last_update_date_time
      FROM `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_ncaa_stg AS ns
      LEFT OUTER JOIN
        (SELECT c.source_patient_id,
                s.server_name,
                c.patient_sk
         FROM `hca-hin-dev-cur-clinical`.edwcdm.ca_patient AS c
         INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS s ON c.server_sk = s.server_sk) AS a ON ns.patid = a.source_patient_id
      AND upper(ns.full_server_nm) = upper(a.server_name)
      INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS csrv ON upper(ns.full_server_nm) = upper(csrv.server_name)
      LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_patient_abnormality AS ch ON ch.server_sk = csrv.server_sk
