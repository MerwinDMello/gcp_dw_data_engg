  ...
  (SELECT DISTINCT a.*
   FROM
     (SELECT DISTINCT NULL AS contact_sk,
      ...
      FROM `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_contacts_stg AS stg
      LEFT OUTER JOIN
        (SELECT c.contact_type_sk,
                c.source_contact_type_id,
                s.server_name
         FROM `hca-hin-dev-cur-clinical`.edwcdm.ca_contact_type AS c
         INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS s ON c.server_sk = s.server_sk) AS a_0 ON stg.contacttype = a_0.source_contact_type_id
