  ...
  (SELECT DISTINCT a.*
   FROM
     (SELECT NULL AS case_perfusion_sk,
      ...
      FROM `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_perfusion_stg AS stg
      LEFT OUTER JOIN
      ...
      LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_case_perfusion AS ch ON ch.server_sk = csrv.server_sk
