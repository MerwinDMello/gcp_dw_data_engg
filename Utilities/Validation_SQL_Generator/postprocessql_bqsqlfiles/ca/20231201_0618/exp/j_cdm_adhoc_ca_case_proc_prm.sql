  ...
  (SELECT p.*
   FROM
     (SELECT 1 AS case_proc_sk,
      ...
      FROM `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_procedures_stg AS stg
      LEFT OUTER JOIN
        (SELECT c.patient_case_sk,
                c.source_patient_case_num,
                s_0.server_name
         FROM `hca-hin-dev-cur-clinical`.edwcdm.ca_patient_case AS c
         INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS s_0 ON trim(format('%11d', c.server_sk)) = trim(format('%11d', s_0.server_sk))) AS a ON upper(trim(stg.full_server_nm)) = upper(trim(a.server_name))
      ...
      LEFT OUTER JOIN
        (SELECT c.source_proc_list_id,
                p_0.server_name
         FROM `hca-hin-dev-cur-clinical`.edwcdm.ca_proc_list AS c
         INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS p_0 ON trim(format('%11d', c.server_sk)) = trim(format('%11d', p_0.server_sk))) AS b ON trim(format('%11d', stg.procid)) = trim(format('%11d', b.source_proc_list_id))
