...
FROM
  (SELECT wrk1.*
   FROM
     (SELECT DISTINCT CASE
                      ...
                      CASE
                          ...
                          ELSE CAST(`hca-hin-dev-cur-clinical`.bqutil_fns.cw_td_normalize_number(trim(format('%11d', wrk1_0.country_id))) AS INT64)
                      END AS country_id
      FROM
        (SELECT DISTINCT cardioaccess_demographics_stg.pataddr AS address_line_1_text,
         ...
         UNION ALL SELECT DISTINCT substr(cacs.address, 1, 150) AS address_line_1_text,
                                   ...
                                   CAST(`hca-hin-dev-cur-clinical`.bqutil_fns.cw_td_normalize_number(cacs.address3) AS INT64) AS address_line_3_text,
           ...
           (SELECT rcgl.lookup_id
            FROM `hca-hin-dev-cur-clinical`.edwcdm.ref_ca_global_lookup AS rcgl
         ...
         UNION ALL SELECT DISTINCT substr(cahs.hospaddress, 1, 150) AS address_line_1_text,
           ...
           (SELECT rcgl.lookup_id
            ...
            WHERE upper(rcgl.sts_code_text) = upper(cahs.hospcountry)
              AND upper(rcgl.short_name) = 'ISOCOUNTRY' ) AS country_id
         FROM `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_hospital_stg AS cahs) AS wrk1_0
      WHERE CASE
                WHEN upper(trim(wrk1_0.address_line_1_text)) = '' THEN CAST(NULL AS STRING)
                ELSE trim(wrk1_0.address_line_1_text)
            END IS NOT NULL
   ...
   WHERE NOT EXISTS
       (SELECT 1
        FROM `hca-hin-dev-cur-clinical`.edwcdm.ca_address AS caa
