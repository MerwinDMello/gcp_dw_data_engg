...
FROM
  (SELECT row_number() OVER (
                             ORDER BY upper(met.patient_market_urn_text),
                                      met.network_mnemonic_cs) AS cancer_patient_detail_sk,
   ...
   LEFT OUTER JOIN
     (SELECT cancer_patient_driver.cp_patient_id,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.cancer_patient_driver QUALIFY row_number() OVER (PARTITION BY cancer_patient_driver.cp_patient_id
                                                                                                   ORDER BY cancer_patient_driver.cancer_patient_driver_sk) = 1) AS cpp ON cpid.patient_dw_id = cpp.cp_patient_id) AS a