  ...
  (SELECT row_number() OVER (
   ...
   INNER JOIN
     (SELECT ref_rad_onc_site.source_site_id,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_site) AS rr ON CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dp.dimsiteid) AS FLOAT64) = rr.source_site_id) AS stg