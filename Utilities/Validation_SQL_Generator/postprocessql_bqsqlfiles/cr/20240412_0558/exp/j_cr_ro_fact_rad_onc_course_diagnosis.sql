  ...
  (SELECT row_number() OVER (
   ...
   INNER JOIN
     (SELECT ref_rad_onc_site.source_site_id,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_site) AS rr ON rr.source_site_id = dp.dimsiteid
   LEFT OUTER JOIN
     (SELECT ra_0.source_fact_patient_diagnosis_id,
      ...
      FROM
        (SELECT fact_rad_onc_patient_diagnosis.source_fact_patient_diagnosis_id,
         ...
         FROM `hca-hin-dev-cur-ops`.edwcr.fact_rad_onc_patient_diagnosis) AS ra_0
      INNER JOIN
        (SELECT ref_rad_onc_site.source_site_id,
                ref_rad_onc_site.site_sk
         FROM `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_site) AS rs ON rs.site_sk = ra_0.site_sk) AS ra ON dp.factpatientdiagnosisid = ra.source_fact_patient_diagnosis_id
     ...
     (SELECT ra1_0.source_patient_course_id,
      ...
      FROM
        (SELECT rad_onc_patient_course.source_patient_course_id,
         ...
         FROM `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient_course) AS ra1_0
      INNER JOIN
        (SELECT ref_rad_onc_site.source_site_id,
                ref_rad_onc_site.site_sk
         FROM `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_site) AS rs1 ON rs1.site_sk = ra1_0.site_sk) AS ra1 ON dp.dimcourseid = ra1.source_patient_course_id
     ...
     (SELECT dc_0.source_diagnosis_code_id,
      ...
      FROM
        (SELECT ref_rad_onc_diagnosis_code.source_diagnosis_code_id,
         ...
         FROM `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_diagnosis_code) AS dc_0
      INNER JOIN
        (SELECT ref_rad_onc_site.source_site_id,
                ref_rad_onc_site.site_sk
         FROM `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_site) AS rs2 ON rs2.site_sk = dc_0.site_sk) AS dc ON dp.dimdiagnosiscodeid = dc.source_diagnosis_code_id
