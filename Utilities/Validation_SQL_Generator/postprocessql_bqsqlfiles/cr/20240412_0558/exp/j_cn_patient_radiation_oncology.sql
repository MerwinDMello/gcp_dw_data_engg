...
FROM
  (SELECT stg.cn_patient_rad_oncology_sid,
   ...
   LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_site_location AS loc ON upper(rtrim(stg.treatment_site_location_id)) = upper(rtrim(loc.site_location_desc))
   LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_lung_lobe_location AS lob_loc ON upper(rtrim(stg.lung_lobe_location_id)) = upper(rtrim(lob_loc.lung_lobe_location_desc))
   LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_facility AS fac ON upper(rtrim(stg.radiation_oncology_facility_id)) = upper(rtrim(fac.facility_name))
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_radiation_oncology.hashbite_ssk) AS hashbite_ssk
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_radiation_oncology) ) AS a