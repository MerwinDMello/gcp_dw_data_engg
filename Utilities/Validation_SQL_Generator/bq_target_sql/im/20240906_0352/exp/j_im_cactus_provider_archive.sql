...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240906_0352/input/exp/j_im_cactus_provider_archive.sql
-- Translated from: Teradata
-- Translated to: BigQuery

SELECT
    format('%20d', coalesce(f.counts, 0)) AS source_string
  FROM
    (
      SELECT
          count(*) AS counts
        FROM
          (
            SELECT
              ...
              FROM
                `hca-hin-dev-cur-comp`.edwim_base_views.cactus_provider AS t1
              ...
              QUALIFY row_number() OVER (PARTITION BY t2.im_domain_id, upper(t1.cactus_provider_user_id) ORDER BY t1.fac_asgn_stts_sid DESC) = 1
          ) AS a
    ) AS f
;
