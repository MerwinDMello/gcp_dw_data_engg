...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240905_1030/input/exp/j_im_mt_cl_hcp_cdm.sql
    ...
    (
      SELECT DISTINCT
        ...
        FROM
          ...
          INNER JOIN (
            SELECT
              ...
              WHERE upper(rtrim(prctnr_role_idfn.registn_type_ref_cd)) = 'NPI'
               ...
               AND rtrim(prctnr_role_idfn.id_txt) <> ''
          ) AS t2 ON t1.national_provider_id = CAST(bqutil.fn.cw_td_normalize_number(t2.npi) as FLOAT64)
          INNER JOIN (
            SELECT
                prctnr_role_idfn.role_plyr_sk,
          ...
          ) AS t3 ON t2.role_plyr_sk = t3.role_plyr_sk
        WHERE t1.national_provider_id IS NOT NULL
         AND t1.national_provider_id > 0
        QUALIFY row_number() OVER (PARTITION BY hcp_dw_id ORDER BY upper(mt_user_id) DESC) = 1
    ) AS a
;
