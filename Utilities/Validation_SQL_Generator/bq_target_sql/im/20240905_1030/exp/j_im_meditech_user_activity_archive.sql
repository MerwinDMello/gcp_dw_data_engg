...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240905_1030/input/exp/j_im_meditech_user_activity_archive.sql
  ...
  FROM
    (
      SELECT
        ...
        FROM
          `hca-hin-dev-cur-comp`.edwim_base_views.meditech_user_activity AS t1
        ...
        QUALIFY row_number() OVER (PARTITION BY t2.im_domain_id, upper(t1.mt_user_id) ORDER BY t1.mt_user_last_activity_date DESC) = 1
    ) AS a
;
