...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240905_1030/input/exp/j_im_meditech_user.sql
      ...
      SELECT DISTINCT
          ...
          CASE
            WHEN upper(rtrim(t1.actv_ind)) = 'Y' THEN CASE
              WHEN upper(rtrim(t2.prctnr_fclt_role_cd)) IN(
                '1HCARESDNT', '1HCAFELLOW', '1HCASTUDEN', '1HCAPHARM', '2HCAACTIVE', '2HCAASOCAF', '2HCAAMBUL', '2HCACOURTS', '2HCACONSUL', '2HCAPRVNOM', '2HCANONPRV', '2HCAHOLD'
              ) THEN 'Y'
              ELSE 'N'
            END
            ELSE 'N'
        ...
        FROM
          `hca-hin-dev-cur-comp`.edwim_base_views.prctnr_dtl AS t1
          INNER JOIN (
            SELECT
                s1.role_plyr_sk,
                s1.vld_fr_ts,
              ...
              FROM
                `hca-hin-dev-cur-comp`.edwim_base_views.prctnr_fclt_dtl AS s1
                INNER JOIN (
                  SELECT
                    ...
                    FROM
                      `hca-hin-dev-cur-comp`.edw_pub_views.clinical_facility
                    WHERE upper(rtrim(clinical_facility.company_code)) = 'H'
                     ...
                     AND NOT rtrim(substr(trim(clinical_facility.facility_mnemonic_cs), 7, 1)) = '_'
                ) AS s2 ON upper(rtrim(s1.company_code)) = upper(rtrim(s2.company_code))
                 AND upper(rtrim(s1.coid)) = upper(rtrim(s2.coid))
              WHERE upper(rtrim(s1.company_code)) = 'H'
               AND s1.vld_to_ts = DATETIME '9999-12-31 00:00:00'
          ) AS t2 ON t1.role_plyr_sk = t2.role_plyr_sk
          INNER JOIN `hca-hin-dev-cur-comp`.edwim_base_views.ref_im_domain AS t6 ON rtrim(t2.network_mnemonic_cs) = rtrim(t6.im_domain_name)
           AND t6.application_system_id IN(
            -- Network
            5, 6
          )
          INNER JOIN (
            SELECT
              ...
              FROM
                `hca-hin-dev-cur-comp`.edwcdm_base_views.prctnr_role_idfn
              ...
              WHERE upper(rtrim(prctnr_role_idfn.registn_type_ref_cd)) = 'LOGON_ID'
               ...
               AND bqutil.fn.cw_regexp_instr_2(mt_user_id, '[*.()_]') = 0
               AND bqutil.fn.cw_regexp_instr_2(substr(trim(mt_user_id), 4, 4), '[A-Za-z_]') = 0
               AND bqutil.fn.cw_regexp_instr_2(substr(trim(mt_user_id), 1, 3), '[0-9_]') = 0
               AND prctnr_role_idfn.vld_to_ts = DATETIME '9999-12-31 00:00:00'
          ) AS t4 ON t1.role_plyr_sk = t4.role_plyr_sk
        ...
        WHERE upper(rtrim(t1.actv_ind)) = 'Y'
         AND t1.vld_to_ts = DATETIME '9999-12-31 00:00:00'
        QUALIFY row_number() OVER (PARTITION BY im_domain_id, upper(t4.mt_user_id) ORDER BY upper(mt_user_exempt_sw) DESC, mt_user_last_activity_date DESC) = 1
    ) AS a
;
-- Last Activity Date
-- Remove Duplicates
