    ...
    (
      SELECT
        ...
        FROM
          (
            SELECT
                current_date('US/Central') AS ad_archive_date,
                row_number() OVER (ORDER BY upper(t2.ad_user_name), t2.ad_hierarchy_level_id) AS ad_archive_id,
              ...
              FROM
                (
                  SELECT
                    ...
                    FROM
                      `hca-hin-dev-cur-comp`.edwim.ad_member_group_hierarchy AS h
                    ...
                    WHERE rtrim(coalesce(m.ad_user_name, '')) <> ''
                ) AS t2
              WHERE upper(rtrim(t2.ad_user_name)) IN(
                SELECT DISTINCT
                    upper(rtrim(im_person_activity.im_person_user_id)) AS im_person_user_id
                  FROM
                    `hca-hin-dev-cur-comp`.edwim.im_person_activity
                  WHERE im_person_activity.esaf_activity_date = current_date('US/Central')
                   AND upper(rtrim(im_person_activity.source_system_code)) = 'M'
              )
          ) AS t1
    ) AS src
;
