...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240905_1030/input/exp/j_im_mt_cl_hcp.sql
      ...
      SELECT
        ...
        FROM
          ...
          CROSS JOIN UNNEST(ARRAY[
            coalesce(t2.hcp_user_id, t3.hcp_user_id)
          ]) AS hcp_user_id_3_4
        WHERE NOT upper(rtrim(t1.hcp_mnemonic_cs)) = 'UNDEFINED'
         AND NOT hcp_user_id_3_4 IS NULL
         AND NOT rtrim(hcp_user_id_3_4) = ''
        QUALIFY row_number() OVER (PARTITION BY t1.hcp_dw_id, upper(hcp_mnemonic_cs) ORDER BY upper(t1.hcp_mis_user_mnemonic) DESC, upper(hcp_user_id_3_4) DESC) = 1
    ) AS a
;
