...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240904_0246/input/exp/j_im_meditech_user_archive.sql
  ...
  FROM
    (
      SELECT
        ...
        FROM
          `hca-hin-dev-cur-comp`.edwim_base_views.meditech_user AS t1
      ...
      SELECT
        ...
        QUALIFY row_number() OVER (PARTITION BY t3.im_domain_id, upper(t1.mt_user_id), t1.facility_mnemonic_cs ORDER BY t1.mt_user_exempt_sw, t1.mt_user_last_activity_date DESC) = 1
    ) AS a
;
