...
-- Source: gs://eim-comp-cs-datamig-dev-0002/im_bulk_conversion_validation/20240904_0246/input/exp/j_im_cactus_provider.sql
  ...
  FROM
    (
      SELECT
          ...
          t1.hcp_middle_name,
          CASE
            WHEN upper(rtrim(t6.fac_asgn_stts_desc)) IN(
              'CURRENT', 'TEMPORARY'
            ) THEN 1
            ELSE 0
        ...
        FROM
          `hca-hin-dev-cur-comp`.edwim_base_views.hcp AS t1
          INNER JOIN `hca-hin-dev-cur-comp`.edwim_base_views.ref_provider_category AS t2 ON t1.hcp_cat_sid = t2.prov_cat_sid
           AND upper(rtrim(t2.prov_cat_src_sys_key)) IN(
            'D2G019AL2J', 'D2G019AKNU'
          )
          ...
          CROSS JOIN UNNEST(ARRAY[
            substr(t3.hcp_other_id, 1, 7)
          ]) AS cactus_provider_user_id
        WHERE upper(rtrim(t1.hcp_active_ind)) = 'Y'
         AND cactus_provider_user_id IS NOT NULL
         AND NOT trim(cactus_provider_user_id) = ''
         AND bqutil.fn.cw_regexp_instr_2(cactus_provider_user_id, '[.]') = 0
         AND bqutil.fn.cw_regexp_instr_2(substr(trim(cactus_provider_user_id), 4, 4), '[A-Za-z_]') = 0
         AND bqutil.fn.cw_regexp_instr_2(substr(trim(cactus_provider_user_id), 1, 3), '[0-9_]') = 0
        QUALIFY row_number() OVER (PARTITION BY t7.im_domain_id, cactus_provider_user_id ORDER BY t5.hcp_fac_asgn_eff_to_date DESC) = 1
    ) AS a
;
