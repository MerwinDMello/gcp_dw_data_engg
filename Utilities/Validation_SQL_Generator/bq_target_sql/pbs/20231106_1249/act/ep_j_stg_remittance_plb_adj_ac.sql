...
SELECT
  ...
  FROM
    `hca-hin-dev-cur-parallon`.edwpbs_staging.remittance_plb_adj
  WHERE DATE(remittance_plb_adj.dw_last_update_date_time) = (
    SELECT
        DATE(max(remittance_plb_adj_0.dw_last_update_date_time))
      FROM
        `hca-hin-dev-cur-parallon`.edwpbs_staging.remittance_plb_adj AS remittance_plb_adj_0
  )
;
