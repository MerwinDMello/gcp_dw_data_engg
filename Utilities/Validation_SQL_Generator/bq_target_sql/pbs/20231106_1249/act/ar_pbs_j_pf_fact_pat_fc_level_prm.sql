...
SELECT
    concat(coalesce(trim(format('%20d', count(fact_rcom_ar_pat_fc_level.unit_num_sid))), '0'), coalesce(trim(CAST(sum(fact_rcom_ar_pat_fc_level.cash_receipt_amt) as STRING)), '0'), coalesce(trim(CAST(sum(fact_rcom_ar_pat_fc_level.gross_revenue_amt) as STRING)), '0')) AS source_string
  FROM
    `hca-hin-dev-cur-parallon`.edwpbs.fact_rcom_ar_pat_fc_level
  WHERE fact_rcom_ar_pat_fc_level.time_sid = CASE
