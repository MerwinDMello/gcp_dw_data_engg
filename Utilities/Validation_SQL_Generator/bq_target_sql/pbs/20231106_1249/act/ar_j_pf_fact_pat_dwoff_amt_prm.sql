...
-- Source: eim-parallon-cs-datamig-dev-0002/pbs_bulk_conversion_validation/20231106_1249/input/act/ar_j_pf_fact_pat_dwoff_amt_prm.sql
-- Translated from: Teradata
-- Translated to: BigQuery

-- Locking table edwpbs.Fact_RCOM_AR_patient_Level for access
SELECT
    concat(coalesce(CAST(sum(fact_rcom_ar_patient_level.payor_denial_amt) as STRING), '0'), coalesce(format('%11d', sum(fact_rcom_ar_patient_level.payor_denial_cnt)), '0')) AS source_string
  FROM
