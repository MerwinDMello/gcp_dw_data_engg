...
SELECT
  ...
  FROM
    (
      SELECT
          max(remittance_service_rarc.service_guid) AS service_guid,
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_staging.remittance_service_rarc
        ...
        GROUP BY upper(remittance_service_rarc.service_guid), upper(remittance_service_rarc.rarc_qualifier_code), upper(remittance_service_rarc.rarc_code), 4, upper(remittance_service_rarc.delete_ind), 6, 7, 8
    ) AS a
;
