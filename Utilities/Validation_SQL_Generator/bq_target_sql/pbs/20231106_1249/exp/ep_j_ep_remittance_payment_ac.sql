...
SELECT
  ...
  FROM
    (
      SELECT
          a.alternate_sn_format_ind AS alternate_sn_format_ind,
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_staging.remittance_payment AS a
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs.ref_remittance_payee AS b ON upper(coalesce(a.payee_name, '')) = upper(coalesce(b.payee_name, ''))
           ...
           AND CASE
            WHEN syslib.length(trim(format('%11d', a.provider_tax_id))) = 10 THEN trim(format('%11d', a.provider_tax_id))
            ELSE ''
          END = coalesce(b.provider_npi, '')
           AND CASE
             syslib.length(trim(format('%11d', a.provider_tax_id)))
