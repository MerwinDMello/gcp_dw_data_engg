...
SELECT
    concat(coalesce(CAST(sum(x.eor_gross_reimbursement_amt) as STRING), '0'), coalesce(CAST(sum(x.eor_contractual_allowance_amt) as STRING), '0'), coalesce(CAST(sum(x.eor_insurance_payment_amt) as STRING), '0')) AS source_string
  FROM
    (
      SELECT
          ...
          max(coalesce(date_diff(date_sub(current_date('US/Central'), interval extract(DAY from current_date('US/Central')) DAY), ad.discharge_date, DAY), 0)) AS disch_days
        FROM
          `hca-hin-dev-cur-parallon`.edwpf_base_views.explanation_of_reimbursement AS d
          INNER JOIN (
            SELECT
                max(k.company_code) AS company_code,
                max(k.coid) AS coid,
                k.patient_dw_id,
                k.payor_dw_id,
              ...
              FROM
                `hca-hin-dev-cur-parallon`.edwpf_base_views.explanation_of_reimbursement AS k
                INNER JOIN (
                  SELECT
                      max(h.company_code) AS company_code,
                      max(h.coid) AS coid,
                      h.patient_dw_id,
                      h.payor_dw_id,
                    ...
                    FROM
                      `hca-hin-dev-cur-parallon`.edwpf_base_views.explanation_of_reimbursement AS h
                      INNER JOIN (
                        SELECT
                          ...
                          FROM
                            `hca-hin-dev-cur-parallon`.edwpf_base_views.explanation_of_reimbursement AS a
                            INNER JOIN (
                              SELECT
                                ...
                                FROM
                                  `hca-hin-dev-cur-parallon`.edwpf_base_views.explanation_of_reimbursement AS b
                                WHERE upper(b.eor_reversal_code) = ' '
                                 AND b.iplan_insurance_order_num = 1
                                GROUP BY upper(b.company_code), upper(b.coid), 3, 4, upper(b.log_id), 6, 7, 8
                            ) AS z ON upper(a.company_code) = upper(z.company_code)
                          ...
                          GROUP BY upper(a.company_code), upper(a.coid), 3, 4, upper(a.log_id), 6, 7, 8
                      ) AS g ON upper(h.company_code) = upper(g.company_code)
                    ...
                    GROUP BY upper(h.company_code), upper(h.coid), 3, 4, upper(h.log_id), 6, 7, 8, 9
                ) AS j ON upper(k.company_code) = upper(j.company_code)
              ...
              GROUP BY upper(k.company_code), upper(k.coid), 3, 4, upper(k.log_id), 6, 7, 8, 9, 10
          ) AS c ON upper(d.company_code) = upper(c.company_code)
          ...
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.admission_discharge AS ad ON ad.patient_dw_id = d.patient_dw_id
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwfs_base_views.facility AS ff ON upper(ff.coid) = upper(d.coid)
          ...
          INNER JOIN (
            SELECT
              ...
              GROUP BY 1
          ) AS atp ON atp.patient_dw_id = d.patient_dw_id
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.admission_patient_type AS apt ON apt.patient_dw_id = d.patient_dw_id
           AND apt.eff_from_date = atp.eff_from_date
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.facility_reimbursement_log AS frl ON upper(d.company_code) = upper(frl.company_code)
        ...
        GROUP BY upper(c.company_code), upper(c.coid), 3, 4, upper(c.log_id), 6, 7, 8, 9, 10, upper(ff.unit_num), upper(CASE
          WHEN upper(substr(apt.admission_patient_type_code, 1, 1)) = 'I'
           AND upper(frl.ip_log_format_code) = 'O' THEN '0124'
          WHEN upper(substr(apt.admission_patient_type_code, 1, 1)) = 'I'
           AND upper(frl.ip_log_format_code) = 'M' THEN '0125'
          ...
          WHEN upper(d.source_system_code) = 'N' THEN '0124'
          ELSE '0000'
        END), upper(apt.admission_patient_type_code), 17, upper(d.source_system_code)
    ) AS x
;
