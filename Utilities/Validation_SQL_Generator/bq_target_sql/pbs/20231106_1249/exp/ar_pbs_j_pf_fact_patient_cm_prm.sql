...
-- Source: eim-parallon-cs-datamig-dev-0002/pbs_bulk_conversion_validation/20231106_1249/input/exp/ar_pbs_j_pf_fact_patient_cm_prm.sql
-- Translated from: Teradata
-- Translated to: BigQuery

SELECT
    trim(format('%20d', coalesce(count(*), 0))) AS source_string
  FROM
    (
      SELECT
          fp.patient_sid,
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_rcom_ar_patient_level AS fp
        WHERE fp.date_sid = (
          SELECT
              max(eis_date_dim.time_id)
            FROM
              `hca-hin-dev-cur-parallon`.edwpf.eis_date_dim
