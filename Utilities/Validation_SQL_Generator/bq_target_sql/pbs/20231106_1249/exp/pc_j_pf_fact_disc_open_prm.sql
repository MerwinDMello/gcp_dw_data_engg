...
SELECT
    concat('0', trim(format('%4d', coalesce(sum(0), 0))), trim(format('%4d', coalesce(sum(0), 0))), trim(format('%4d', coalesce(sum(0), 0))), trim(format('%4d', coalesce(sum(0), 0))), trim(CAST(coalesce(sum(abs(rd.var_gross_reimbursement_amt)), CAST(0 as BIGNUMERIC)) as STRING)), trim(CAST(coalesce(sum(abs(rd.var_contractual_allowance_amt)), CAST(0 as BIGNUMERIC)) as STRING)), trim(CAST(coalesce(sum(abs(rd.var_insurance_payment_amt)), CAST(0 as BIGNUMERIC)) as STRING)), trim(CAST(coalesce(sum(abs(rd.var_net_billed_charge_amt)), CAST(0 as BIGNUMERIC)) as STRING))) AS source_string
  FROM
    `hca-hin-dev-cur-parallon`.edwpf_base_views.reimbursement_discrepancy_eom AS rd
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.explanation_of_reimbursement AS eor ON upper(rd.company_code) = upper(eor.company_code)
