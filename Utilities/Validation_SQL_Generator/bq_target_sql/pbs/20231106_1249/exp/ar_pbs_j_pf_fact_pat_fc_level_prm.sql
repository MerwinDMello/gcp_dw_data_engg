...
SELECT
    concat(trim(format('%20d', count(x.unit_num_sid))), trim(CAST(sum(x.cash_receipt_amt) as STRING)), trim(regexp_replace(format('%#17.2f', CAST(ROUND(sum(x.gross_revenue_amt), 2, 'ROUND_HALF_EVEN') as NUMERIC)), r'^(.*?)(-)?0(\..*)', r'\1 \2\3'))) AS source_string
  FROM
    (
      SELECT
          max(z.unit_num_sid) AS unit_num_sid,
        ...
        FROM
          (
            SELECT
                max(sar.unit_num) AS unit_num_sid,
                CASE
              ...
              FROM
                `hca-hin-dev-cur-parallon`.edwpf_base_views.smry_ar_cash_collections AS sar
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.eis_patient_fin_class_dim AS epf ON CASE
                   substr(epf.patient_financial_class_member, 5, 2)
                  WHEN '' THEN 0.0
                ...
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.eis_patient_type_dim AS pt ON upper(pt.patient_type_member) = upper(concat('PT_', CASE
              ...
              GROUP BY upper(sar.unit_num), 2, 3, upper(sar.rptg_period), 5, 6, upper(sar.company_code), upper(sar.coid)
            UNION DISTINCT
            SELECT
                max(fd.unit_num) AS unit_num_sid,
                epf.patient_financial_class_sid,
                1 AS scenario_sid,
                max(format_date('%Y%m', date_add(current_date('US/Central'), interval -1 MONTH))) AS time_sid,
                1 AS source_sid,
                CASE
                   upper(rd.data_type_text)
              ...
              FROM
                `hca-hin-dev-cur-parallon`.edwpf_base_views.revstats_department AS rd
                INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.facility_dimension AS fd ON upper(fd.coid) = upper(rd.coid)
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.eis_patient_fin_class_dim AS epf ON CASE
                   substr(epf.patient_financial_class_member, 5, 2)
                  WHEN '' THEN 0.0
                  ELSE CAST(substr(epf.patient_financial_class_member, 5, 2) as FLOAT64)
                END = rd.financial_class_code
                 AND upper(epf.patient_financial_class_member) <> 'NO_FC'
              WHERE upper(rd.data_type_text) IN(
                'IP AMOUNT', 'OP AMOUNT'
              )
               AND upper(rd.sub_unit_num) <> '00000'
               AND (upper(fd.company_code) = 'H'
               OR upper(substr(trim(fd.company_code_operations), 1, 1)) = 'Y')
              GROUP BY upper(fd.unit_num), 2, 3, upper(format_date('%Y%m', date_add(current_date('US/Central'), interval -1 MONTH))), 5, 6, upper(fd.company_code), upper(rd.coid)
          ) AS z
        GROUP BY upper(z.unit_num_sid), 2, 3, 4, upper(z.time_sid), 6, upper(z.company_code), upper(z.coid)
    ) AS x
;
