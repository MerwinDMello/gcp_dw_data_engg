...
-- Source: eim-parallon-cs-datamig-dev-0002/pbs_bulk_conversion_validation/20231106_1249/input/exp/ep_j_ep_junc_remittance_other_claim_related_info_ac.sql
          ...
          (
            ...
            SELECT
              ...
              WHERE upper(coalesce(remittance_claim.othr_clm_rel_ref_idn_qul10_cod, '')) NOT IN(
               ...
               OR upper(coalesce(remittance_claim.other_claim_related_id10, '')) NOT IN(
                ''
              )
          ) AS f
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs.ref_remittance_other_claim_related_info AS rrap ON upper(f.reference_id) = upper(rrap.reference_id)
           AND upper(f.reference_id_qualifier_code) = upper(rrap.reference_id_qualifier_code)
    ) AS a
;
