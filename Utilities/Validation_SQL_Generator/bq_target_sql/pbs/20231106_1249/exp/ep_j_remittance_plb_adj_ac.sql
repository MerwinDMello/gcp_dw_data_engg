  ...
  FROM
    (
      SELECT
          ...
          row_number() OVER (PARTITION BY upper(a.payment_guid), upper(a.adj_code), a.audit_date, upper(a.adj_id) ORDER BY upper(a.payment_guid)) AS adj_record_num,
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_staging.remittance_plb_adj AS a
    ) AS a
;
