...
SELECT
    concat(format('%20d', coalesce(count(*), 0)), CAST(coalesce(sum(stg.refund_amt), NUMERIC '0') as STRING), CAST(coalesce(sum(stg.total_acct_bal_amt), NUMERIC '0') as STRING)) AS source_string
  FROM
