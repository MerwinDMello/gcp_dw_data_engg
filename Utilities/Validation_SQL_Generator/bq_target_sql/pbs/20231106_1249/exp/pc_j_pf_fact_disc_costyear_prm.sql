...
SELECT
    format('%20d', a.row_count) AS source_string
  FROM
    (
      SELECT
          count(*) AS row_count
        FROM
          `hca-hin-dev-cur-parallon`.edwpf_base_views.dim_organization AS dorg
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwfs_base_views.facility AS fac ON CASE
             fac.unit_num
            WHEN '' THEN 0.0
