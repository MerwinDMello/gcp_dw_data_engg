  ...
  FROM
    (
      SELECT
          (
            SELECT
                coalesce(max(ref_remittance_cob_carrier.cob_carrier_sid), 0)
              FROM
                `hca-hin-dev-cur-parallon`.edwpbs.ref_remittance_cob_carrier
          ) + row_number() OVER (ORDER BY upper(f.cob_qualifier_code), upper(f.cob_carrier_num), upper(f.cob_carrier_name)) AS cob_carrier_sid,
          --  SID
        ...
        FROM
          (
            SELECT
                max(remittance_claim.crossover_carrier_qualifr_code) AS cob_qualifier_code,
              ...
              GROUP BY upper(remittance_claim.crossover_carrier_qualifr_code), upper(remittance_claim.cordintn_of_benefit_carrier_nm), upper(remittance_claim.coordintn_of_beneft_carrier_nm)
          ) AS f
        WHERE (upper(f.cob_qualifier_code), upper(f.cob_carrier_num), upper(f.cob_carrier_name)) NOT IN(
          SELECT AS STRUCT
              upper(ref_remittance_cob_carrier.cob_qualifier_code) AS cob_qualifier_code,
