...
-- Source: eim-parallon-cs-datamig-dev-0002/pbs_bulk_conversion_validation/20231106_1249/input/exp/ar_j_pf_fact_patient_level_legacyar_prm.sql
-- Translated from: Teradata
-- Translated to: BigQuery

-- locking table edwpbs_staging.pass_current for access
SELECT
    concat('PBMAR100', ',', trim(CAST(sum(coalesce(subq.eom_pymt_rcvd, NUMERIC '0')) as STRING)), ',', trim(CAST(sum(CASE
  ...
  FROM
    (
      SELECT
          ...
          1 AS ins_order_num
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_staging.pass_current AS pc
          CROSS JOIN `hca-hin-dev-cur-parallon`.edwpf_views.facility_dimension AS fd
