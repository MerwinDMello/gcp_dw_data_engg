  ...
  FROM
    (
      SELECT
          ...
          END) AS dnfbch,
          -- Discharged_Not_Final_Bill_Charity_Amt
          sum(CASE
          ...
          sum(CASE
            WHEN upper(format_date('%Y%m', peom.discharge_date)) <= upper(peom.rptg_period)
             AND upper(peom.account_status_code) NOT IN(
              'UB', 'BD'
            )
          ...
          END) + sum(CASE
            ...
            ELSE CAST(0 as NUMERIC)
          END) AS insfc99,
          -- Insured_Self_Pay_Amt
          -- -ADDED 9943 AND 9948
          -- ---ADDED 9943 AND 9948
          -- AND FINANCIAL_CLASS_CODE='99'
          sum(CASE
            WHEN upper(peom.account_status_code) IN(
              'AR', 'AX', 'CA', 'UB'
            )
             AND upper(format_date('%Y%m', peom.discharge_date)) <= upper(peom.rptg_period)
             AND peom.total_account_balance_amt > 0
             AND (CAST(peom.iplan_id_ins1 as FLOAT64) IN(
              9940.0, 9941.0, 9942.0, 9943.0, 9944.0, 9945.0, 9946.0, 9947.0, 9948.0, 9949.0
            )
          ...
          END) + sum(CASE
            WHEN upper(peom.account_status_code) IN(
             ...
             AND upper(format_date('%Y%m', peom.discharge_date)) <= upper(peom.rptg_period)
             AND peom.total_account_balance_amt > 0
             AND (CAST(peom.iplan_id_ins2 as FLOAT64) IN(
              9940.0, 9941.0, 9942.0, 9943.0, 9944.0, 9945.0, 9946.0, 9947.0, 9948.0, 9949.0
            )
          ...
          END) + sum(CASE
            ...
            ELSE CAST(0 as NUMERIC)
          END) AS totalcharityexpansiondiscount,
          -- Charity_Discount_Amt
          sum(ar_plp.ar_transaction_amt) AS plp,
          -- Patient_Liab_Prot_Discount_Amt
          sum(CASE
            WHEN upper(peom.account_status_code) IN(
              'AR', 'AX', 'CA', 'UB'
            )
          ...
          END) + sum(CASE
            WHEN upper(peom.account_status_code) IN(
             ...
             AND peom.financial_class_code_ins3 = 15.0) THEN peom.payor_contract_allow_amt_ins3 + peom.payor_adjustment_amt_ins3
            ELSE CAST(0 as NUMERIC)
          END)) + sum(ar_plp.ar_transaction_amt) AS total_discounts,
          -- Total_Discount_Amt
          coalesce(sum(CASE
            WHEN upper(peom.account_status_code) IN(
              'AR', 'AX', 'CA', 'UB'
          ...
          END) + sum(CASE
            WHEN upper(peom.account_status_code) IN(
             ...
             AND peom.financial_class_code_ins3 = 99.0 THEN peom.payor_contract_allow_amt_ins3 + peom.payor_adjustment_amt_ins3
            ELSE CAST(0 as NUMERIC)
          END), NUMERIC '0.00') AS secondaryagyuninsureddisc,
          -- AND PEOM.COLLECTOR_ORG_CODE IN (858,859,860)
          ...
          END) + sum(CASE
            ...
            ELSE CAST(0 as NUMERIC)
          END), CAST(0 as BIGNUMERIC)) AS secondaryagencycharityexpansiondiscount_ins,
          -- Secn_Agcy_Charity_Discount_Amt
          sum(ar_plp_sec.ar_transaction_amt) AS sec_plp,
          -- Secn_Agcy_Pat_Liab_Prot_Discount_Amt
          coalesce(sum(CASE
            WHEN upper(peom.account_status_code) IN(
              'AR', 'AX', 'CA', 'UB'
            )
          ...
          END) + sum(CASE
            WHEN upper(peom.account_status_code) IN(
             ...
             AND upper(peom.collector_org_type_code) = 'S' THEN peom.payor_contract_allow_amt_ins3 + peom.payor_adjustment_amt_ins3
            ELSE CAST(0 as NUMERIC)
          END), CAST(0 as BIGNUMERIC)) + sum(ar_plp_sec.ar_transaction_amt) AS secondary_agency_discounts,
          -- Total_Secn_Agcy_Discount_Amt
          any_value(totalnonsecondaryuninsureddiscount) AS totalnonsecondaryuninsureddiscount,
          -- Non_Secn_Unins_Disc_Amt
          any_value(totalnonsecondarycharityexpansiondiscount) AS totalnonsecondarycharityexpansiondiscount,
          -- Non_Secn_Charity_Discount_Amt
          any_value(total_nonsecondary_pat_liability_protection_discount) AS total_nonsecondary_pat_liability_protection_discount,
          -- Non_Secn_Patient_Liab_Prot_Discount_Amt
          any_value(totalnonsecondarydiscounts) AS totalnonsecondarydiscounts,
          --  Total_Non_Secn_Discount_Amt --ADA SPCA
          coalesce(sum(CASE
            WHEN upper(peom.account_status_code) IN(
              'AR', 'AX', 'CA'
            )
            ...
            ELSE CAST(0 as NUMERIC)
          END), NUMERIC '0.00') AS secondarytotacctbal,
          -- AND PEOM.COLLECTOR_ORG_CODE IN (858,859,860)
          ...
          sum(CASE
            ...
            ELSE CAST(0 as NUMERIC)
          END) AS totpatientdue,
          -- Total_Patient_Due_Amt
          sum(CASE
            WHEN (upper(format_date('%Y%m', peom.discharge_date)) > upper(peom.rptg_period)
             OR upper(peom.account_status_code) = 'UB'
             AND peom.discharge_date IS NOT NULL
            ...
            ELSE CAST(0 as NUMERIC)
          END) AS dnfbsp,
          -- Discharged_Not_Final_Bill_Self_Pay_Amt
          sum(CASE
            ...
            ELSE CAST(0 as NUMERIC)
          END) AS ihsp,
          -- Inhouse_Self_Pay_Amt
          any_value(totalselfpayar) AS totalselfpayar,
          --  Self_Pay_AR_Amt
          any_value(totalnonsecondaryselfpayar) AS totalnonsecondaryselfpayar,
          --  Non_Secn_Self_Pay_AR_Amt
          any_value(totalnonsecondaryselfpayar) + any_value(totalnonsecondarydiscounts) AS totalgrossnonsecondaryselfpayar
        FROM
          --  Gross_Non_Secn_Self_Pay_AR_Amt
          `hca-hin-dev-cur-parallon`.edwpf_base_views.pass_eom AS peom
          INNER JOIN (
            SELECT
                fd.*
              FROM
                `hca-hin-dev-cur-parallon`.edwpbs_base_views.facility_dimension_eom AS fd
              WHERE upper(fd.summary_7_member_ind) = 'Y'
               AND upper(fd.company_code) = 'H'
            UNION ALL
            SELECT
                facility_dimension_eom.*
              FROM
                `hca-hin-dev-cur-parallon`.edwpbs_base_views.facility_dimension_eom
              WHERE upper(facility_dimension_eom.coid) IN(
                SELECT DISTINCT
                    upper(dim_esb_organization_homehealth_dnd.coid) AS coid
                  FROM
                    `hca-hin-dev-cur-parallon`.edwpbs_staging.dim_esb_organization_homehealth_dnd
                  WHERE dim_esb_organization_homehealth_dnd.coid IS NOT NULL
              )
          ) AS f ON upper(f.unit_num) = upper(peom.unit_num)
          LEFT OUTER JOIN (
            SELECT
                max(k.coid) AS coid,
                k.pat_acct_num,
                ...
                (
                  SELECT
                    ...
                    FROM
                      ...
                      `hca-hin-dev-cur-parallon`.edwpf_base_views.ar_transaction_mv AS a
                      INNER JOIN (
                        SELECT
                            pass_eom.*
                          FROM
                      ...
                      ) AS pa ON upper(pa.coid) = upper(a.coid)
                       AND pa.pat_acct_num = a.pat_acct_num
                    WHERE upper(a.transaction_type_code) IN(
                      '4', '5'
                    )
                     AND CAST(a.transaction_code as FLOAT64) IN(
                      -- AR_TRANSACTION_ENTER_DATE < ( current_date - EXTRACT(DAY FROM ADD_MONTHS(CURRENT_DATE,-1)) + 5) AND
                      784984.0, 784985.0
                    )
                     AND a.iplan_id IN(
                      9929, 0
                    )
                    GROUP BY upper(a.coid), 2
                  UNION ALL
                  SELECT
                      --  AND A.COID=34632
                      max(a.coid) AS coid,
                  ...
                  SELECT
                    ...
                    WHERE upper(a.transaction_type_code) IN(
                     ...
                     AND a.iplan_id = 9929
                     AND CAST(a.transaction_code as FLOAT64) IN(
                      -- AND AR_TRANSACTION_ENTER_DATE < ( current_date - EXTRACT(DAY FROM ADD_MONTHS(CURRENT_DATE,-1)) + 5)
                      920972.0, 920970.0, 920980.0, 920971.0, 920981.0, 920982.0
                    )
                    GROUP BY upper(a.coid), 2
                ) AS k
              GROUP BY upper(k.coid), 2
          ) AS ar_plp ON upper(ar_plp.coid) = upper(peom.coid)
          ...
          (
            SELECT
                ...
                (
                  SELECT
                    ...
                    FROM
                      ...
                      ) AS pe ON upper(pe.coid) = upper(a.coid)
              ...
              GROUP BY upper(k1.coid), 2
          ) AS ar_plp_sec ON upper(ar_plp_sec.coid) = upper(peom.coid)
          ...
          CROSS JOIN UNNEST(ARRAY[
            coalesce(totalselfpayar - coalesce(sum(CASE
              WHEN upper(peom.account_status_code) IN(
                'AR', 'AX', 'CA'
              )
               AND upper(format_date('%Y%m', peom.discharge_date)) <= upper(peom.rptg_period)
               ...
               AND upper(peom.collector_org_type_code) = 'S' THEN peom.total_account_balance_amt
              ELSE CAST(0 as NUMERIC)
            END), NUMERIC '0.00'), NUMERIC '0.00')
          ]) AS totalnonsecondaryselfpayar
          CROSS JOIN UNNEST(ARRAY[
            coalesce(sum(ar_plp.ar_transaction_amt) - sum(ar_plp_sec.ar_transaction_amt), NUMERIC '0.00')
          ...
          CROSS JOIN UNNEST(ARRAY[
            ...
            END) + sum(CASE
              WHEN upper(peom.account_status_code) IN(
               ...
               AND upper(peom.collector_org_type_code) = 'S' THEN peom.payor_contract_allow_amt_ins3 + peom.payor_adjustment_amt_ins3
              ELSE CAST(0 as NUMERIC)
            END), CAST(0 as BIGNUMERIC)), NUMERIC '0.00')
          ]) AS totalnonsecondarycharityexpansiondiscount
          CROSS JOIN UNNEST(ARRAY[
            coalesce(sum(CASE
              WHEN upper(peom.account_status_code) IN(
                'AR', 'AX', 'CA', 'UB'
            ...
            END) + sum(CASE
              ...
              ELSE CAST(0 as NUMERIC)
            END), NUMERIC '0.00'), NUMERIC '0.00')
          ]) AS totalnonsecondaryuninsureddiscount
          CROSS JOIN UNNEST(ARRAY[
            totalnonsecondaryuninsureddiscount + totalnonsecondarycharityexpansiondiscount + total_nonsecondary_pat_liability_protection_discount
          ]) AS totalnonsecondarydiscounts
        WHERE upper(peom.rptg_period) = upper(format_date('%Y%m', date_add(current_date('US/Central'), interval -1 MONTH)))
         AND left(peom.patient_type_code, 1) IN(
          'O', 'E', 'S', 'I'
        )
    ) AS src
;
