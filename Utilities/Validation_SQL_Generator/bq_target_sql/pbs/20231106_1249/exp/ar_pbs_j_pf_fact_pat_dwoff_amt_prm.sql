...
-- Source: eim-parallon-cs-datamig-dev-0002/pbs_bulk_conversion_validation/20231106_1249/input/exp/ar_pbs_j_pf_fact_pat_dwoff_amt_prm.sql
-- Translated from: Teradata
-- Translated to: BigQuery

SELECT
    concat(coalesce(regexp_replace(format('%#20.3f', exp1.sum1), r'^(.*?)(-)?0(\..*)', r'\1 \2\3'), '0'), coalesce(format('%4d', exp1.cnt1), '0')) AS source_string
  FROM
    (
      SELECT
          k.pe_date,
          coalesce(CAST(ROUND(CAST(sum(CASE
              ...
              FROM
                (
                  SELECT
                      ...
                      deom.company_code,
                      deom.coid,
                      -- DEOM.Write_Off_Denial_Account_Amt , --modified on 08/27
                    ...
                    FROM
                      `hca-hin-dev-cur-parallon`.edwpf_base_views.denial_eom AS deom
                      LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.admission AS adm ON adm.patient_dw_id = deom.patient_dw_id
                      INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.eis_patient_type_dim AS ptd ON upper(concat('PT_', deom.patient_type_code)) = upper(ptd.patient_type_member)
                      ...
                      ) AS rpdeb ON deom.payor_dw_id = rpdeb.payor_dw_id
                      ...
                      LEFT OUTER JOIN (
                        SELECT
                          ...
                          FROM
                            `hca-hin-dev-cur-parallon`.edwpbs_base_views.rcom_payor_dimension_eom
                      ) AS rpde ON deom.payor_dw_id = rpde.payor_dw_id
                       AND rpde.eff_from_date IS NULL
                      CROSS JOIN UNNEST(ARRAY[
                        substr(ltrim(regexp_replace(format('%#20.3f', CASE
                          WHEN upper(deom.source_system_code) = 'N' THEN deom.cc_cash_adjustment_amt
                          ELSE deom.write_off_denial_account_amt
                        END), r'^( *?)(-)?0(\..*)', r'\2\3')), 1, 22)
                      ]) AS w_off_denial_account_amt
                    WHERE CASE
                       w_off_denial_account_amt
                      WHEN '' THEN 0.0
                      ELSE CAST(w_off_denial_account_amt as FLOAT64)
                    END <> 0
                     AND upper(format_date('%Y%m', deom.pe_date)) = upper(format_date('%Y%m', date_add(current_date('US/Central'), interval -1 MONTH)))
                ) AS k_0
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.eis_payor_dim AS pd ON upper(pd.payor_member) = upper(k_0.payor_id)
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_rcom_ar_patient_level AS fp ON fp.patient_sid = k_0.pat_acct_num
                 AND upper(fp.company_code) = upper(k_0.company_code)
                 AND upper(fp.coid) = upper(k_0.coid)
                 AND CASE
                   format_date('%Y%m', k_0.pe_date)
                  WHEN '' THEN 0
                ...
                END
                 ...
                 AND fp.payor_financial_class_sid = k_0.payor_financial_class_sid
          ) AS k
        GROUP BY 1
    ) AS exp1
;
