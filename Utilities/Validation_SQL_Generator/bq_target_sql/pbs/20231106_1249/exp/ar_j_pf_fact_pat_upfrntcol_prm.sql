...
-- Source: eim-parallon-cs-datamig-dev-0002/pbs_bulk_conversion_validation/20231106_1249/input/exp/ar_j_pf_fact_pat_upfrntcol_prm.sql
...
SELECT
    coalesce(CAST(k.sum1 as STRING), '0') AS source_string
  FROM
    (
      SELECT
          date_sub(date_add(CAST(trim(concat(format_date('%Y-%m', art.ar_transaction_effective_date), '-01')) as DATE), interval 1 MONTH), interval 1 DAY) AS effective_date,
          sum(art.ar_transaction_amt) AS sum1
        FROM
          `hca-hin-dev-cur-parallon`.edwpf.ar_transaction AS art
          INNER JOIN `hca-hin-dev-cur-parallon`.edwfs.fact_facility AS ff ON upper(ff.coid) = upper(art.coid)
           AND upper(ff.company_code) = upper(art.company_code)
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpf.registration_account_status AS sts ON art.patient_dw_id = sts.patient_dw_id
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf.admission_discharge AS ad ON sts.patient_dw_id = ad.patient_dw_id
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs.fact_rcom_ar_patient_level AS fp ON art.pat_acct_num = fp.patient_sid
           ...
           AND CASE
             format_date('%Y%m', art.ar_transaction_effective_date)
            WHEN '' THEN 0
            ELSE CAST(format_date('%Y%m', art.ar_transaction_effective_date) as INT64)
          END = fp.date_sid
           AND fp.iplan_insurance_order_num = 0
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf.admission AS adm ON adm.patient_dw_id = art.patient_dw_id
          LEFT OUTER JOIN (
            SELECT
          ...
          ) AS rpdeb ON art.payor_dw_id = rpdeb.payor_dw_id
          ...
          LEFT OUTER JOIN (
            SELECT
              ...
              FROM
                `hca-hin-dev-cur-parallon`.edwpbs_base_views.rcom_payor_dimension_eom
          ) AS rpde ON art.payor_dw_id = rpde.payor_dw_id
           AND rpde.eff_from_date IS NULL
        WHERE sts.account_status_date = (
          SELECT
              max(stsm.account_status_date)
            FROM
              `hca-hin-dev-cur-parallon`.edwpf.registration_account_status AS stsm
         ...
         AND upper(sts.account_status_code) IN(
          'AR', 'UB', 'CA'
        )
