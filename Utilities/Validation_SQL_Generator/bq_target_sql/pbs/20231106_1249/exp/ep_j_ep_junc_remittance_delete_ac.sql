  ...
  FROM
    (
      SELECT DISTINCT
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_base_views.remittance_claim AS clm
          ...
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.remittance_service AS svc ON upper(svc.claim_guid) = upper(clm.claim_guid)
    ) AS a
;
