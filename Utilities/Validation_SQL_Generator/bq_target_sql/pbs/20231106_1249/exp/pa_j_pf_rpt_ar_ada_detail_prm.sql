...
SELECT
    concat(trim(regexp_replace(format('%#17.2f', CAST(ROUND(sum(k.transaction_amt), 2, 'ROUND_HALF_EVEN') as NUMERIC)), r'^(.*?)(-)?0(\..*)', r'\1 \2\3')), trim(CAST(sum(k.non_sec_self_pay_ar_amt) as STRING)), trim(CAST(sum(k.non_sec_uninsured_discount) as STRING))) AS source_string
  FROM
    (
      SELECT
          in_query.eom_end,
        ...
        FROM
          (
            SELECT
                eom_end AS eom_end,
                sum(CASE
                  WHEN upper(pem.account_status_code) IN(
                    'AR', 'AX', 'CA'
                  )
                ...
                END) + sum(CASE
                  WHEN upper(format_date('%Y%m', pem.discharge_date)) <= upper(pem.rptg_period)
                   AND upper(pem.account_status_code) NOT IN(
                    'UB', 'BD'
                  )
                ...
                sum(CASE
                  WHEN upper(pem.account_status_code) IN(
                   ...
                   AND upper(format_date('%Y%m', pem.discharge_date)) <= upper(pem.rptg_period)
                   AND pem.total_account_balance_amt > 0
                   AND (CAST(pem.iplan_id_ins1 as FLOAT64) IN(
                ...
                END) + sum(CASE
                  WHEN upper(pem.account_status_code) IN(
                   ...
                   AND upper(format_date('%Y%m', pem.discharge_date)) <= upper(pem.rptg_period)
                   AND pem.total_account_balance_amt > 0
                   AND (CAST(pem.iplan_id_ins2 as FLOAT64) IN(
                    9940.0, 9941.0, 9942.0, 9943.0, 9944.0, 9945.0, 9946.0, 9947.0, 9948.0, 9949.0
                  )
                ...
                END) + sum(CASE
                  WHEN upper(pem.account_status_code) IN(
                   ...
                   AND upper(format_date('%Y%m', pem.discharge_date)) <= upper(pem.rptg_period)
                   AND pem.total_account_balance_amt > 0
                   AND (CAST(pem.iplan_id_ins3 as FLOAT64) IN(
                    9940.0, 9941.0, 9942.0, 9943.0, 9944.0, 9945.0, 9946.0, 9947.0, 9948.0, 9949.0
                  )
                ...
                END) + sum(CASE
                  ...
                  ELSE CAST(0 as NUMERIC)
                END) AS secondaryagycharityexpdiscount,
                ...
                END) + sum(CASE
                  WHEN upper(pem.account_status_code) IN(
                   ...
                   AND upper(trim(pem.collector_org_type_code)) = 'S' THEN pem.payor_contract_allow_amt_ins3 + pem.payor_adjustment_amt_ins3
                  ELSE CAST(0 as NUMERIC)
                END) AS secondaryagyuninsureddisc,
                -- Added 9943 and 9948
                sum(CASE
              ...
              FROM
                `hca-hin-dev-cur-parallon`.edwpf_base_views.pass_eom AS pem
                INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.facility_dimension AS fd ON upper(fd.unit_num) = upper(pem.unit_num)
                LEFT OUTER JOIN (
                  SELECT
                      max(k_0.coid) AS coid,
                      k_0.pat_acct_num,
                      ...
                      (
                            ...
                            INNER JOIN (
                              SELECT
                                ...
                                WHERE pass_eom.total_account_balance_amt > 0
                                 AND upper(pass_eom.rptg_period) = upper(format_date('%Y%m', date_add(current_date('US/Central'), interval -1 MONTH)))
                        ...
                        SELECT
                          ...
                          END IN(
                           ...
                           AND a.iplan_id = 9929
                           AND a.transaction_code IN(
                            920972, 920970, 920980, 920971, 920981, 920982
                          )
                          GROUP BY upper(a.coid), 2
                      ) AS k_0
                    GROUP BY upper(k_0.coid), 2
                ) AS ar_plp ON upper(ar_plp.coid) = upper(pem.coid)
                      ...
                      (
                        SELECT
                          ...
                          FROM
                            ...
                            ) AS p ON upper(p.coid) = upper(a.coid)
                ...
                ) AS ar_plp_sec ON upper(ar_plp_sec.coid) = upper(pem.coid)
                 AND ar_plp_sec.pat_acct_num = pem.pat_acct_num
                CROSS JOIN UNNEST(ARRAY[
                  date_sub(CAST(trim(concat(format_date('%Y-%m', current_date('US/Central')), '-01')) as DATE), interval 1 DAY)
                ]) AS eom_end
                CROSS JOIN UNNEST(ARRAY[
                  coalesce(sum(CASE
                    WHEN pem.total_account_balance_amt > 0 THEN coalesce(ar_plp.ar_transaction_amt, CAST(0 as BIGNUMERIC))
                  ...
                  END), CAST(0 as BIGNUMERIC)) - coalesce(sum(CASE
                    WHEN pem.total_account_balance_amt > 0 THEN coalesce(ar_plp_sec.ar_transaction_amt, CAST(0 as BIGNUMERIC))
                    ELSE CAST(0 as BIGNUMERIC)
                  END), CAST(0 as BIGNUMERIC))
                ]) AS pat_liability_protection_discount
              WHERE upper(pem.rptg_period) = upper(format_date('%Y%m', eom_end))
               AND left(pem.patient_type_code, 1) IN(
                'O', 'E', 'S', 'I'
              )
      ...
      SELECT
          ...
          sum(CASE
            ...
            WHEN upper(ar.transaction_type_code) IN(
              '4', '5'
            )
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpf_base_views.ar_transaction_mv AS ar
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.fact_patient AS fp ON ar.patient_dw_id = fp.patient_dw_id
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.facility_dimension AS fd ON upper(fd.unit_num) = upper(fp.unit_num)
          CROSS JOIN UNNEST(ARRAY[
            date_sub(CAST(trim(concat(format_date('%Y-%m', current_date('US/Central')), '-01')) as DATE), interval 1 DAY)
          ]) AS eom_end
        WHERE (ar.ar_transaction_effective_date BETWEEN CAST(trim(concat(format_date('%Y-%m', eom_end), '-01')) as DATE) AND eom_end
         ...
         AND upper(ar.transaction_type_code) IN(
          '3', '4', '5'
        )
         AND (upper(fp.patient_type_code_pos1) IN(
          'O', 'E', 'S', 'I', 'N'
        )
