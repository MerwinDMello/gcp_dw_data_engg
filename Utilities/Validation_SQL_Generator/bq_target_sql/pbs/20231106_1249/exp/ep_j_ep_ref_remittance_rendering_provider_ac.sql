  ...
  FROM
    (
      SELECT
          (
            SELECT
                coalesce(max(ref_remittance_rendering_provider.remittance_rendering_provider_sid), 0)
              FROM
                `hca-hin-dev-cur-parallon`.edwpbs.ref_remittance_rendering_provider
          ) + row_number() OVER (ORDER BY upper(f.serv_provider_enty_type_qualifier_code), upper(f.rendering_provider_last_org_name), upper(f.rendering_provider_first_name), upper(f.rendering_provider_middle_name), upper(f.rendering_provider_name_suffix), upper(f.serv_provider_id_qualifier_code), upper(f.rendering_provider_id)) AS remittance_rendering_provider_sid,
          --  SID
        ...
        FROM
          (
            SELECT
                max(remittance_claim.srvce_prvdr_enty_typ_qulfr_cod) AS serv_provider_enty_type_qualifier_code,
              ...
              GROUP BY upper(remittance_claim.srvce_prvdr_enty_typ_qulfr_cod), upper(remittance_claim.rendering_provider_last_org_nm), upper(remittance_claim.rendering_provider_first_name), upper(remittance_claim.rendering_provider_middle_name), upper(remittance_claim.rendering_provider_name_suffix), upper(remittance_claim.srvce_prvdr_idntfctn_qulfr_cod), upper(remittance_claim.rendering_provider_id)
          ) AS f
        WHERE (upper(coalesce(f.serv_provider_enty_type_qualifier_code, '')), upper(coalesce(f.rendering_provider_last_org_name, '')), upper(coalesce(f.rendering_provider_first_name, '')), upper(coalesce(f.rendering_provider_middle_name, '')), upper(coalesce(f.rendering_provider_name_suffix, '')), upper(coalesce(f.serv_provider_id_qualifier_code, '')), upper(coalesce(f.rendering_provider_id, ''))) NOT IN(
          SELECT AS STRUCT
              upper(coalesce(ref_remittance_rendering_provider.serv_provider_enty_type_qualifier_code, '')),
