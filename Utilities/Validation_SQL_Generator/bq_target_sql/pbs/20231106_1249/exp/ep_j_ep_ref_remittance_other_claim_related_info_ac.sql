  ...
  FROM
    (
      SELECT
          (
            SELECT
                coalesce(max(ref_remittance_other_claim_related_info.ref_sid), CAST(0 as BIGNUMERIC))
              FROM
                `hca-hin-dev-cur-parallon`.edwpbs.ref_remittance_other_claim_related_info
          ) + CAST(row_number() OVER (ORDER BY upper(f.reference_id_qualifier_code), upper(f.reference_id)) as BIGNUMERIC) AS ref_sid,
          --  SID
