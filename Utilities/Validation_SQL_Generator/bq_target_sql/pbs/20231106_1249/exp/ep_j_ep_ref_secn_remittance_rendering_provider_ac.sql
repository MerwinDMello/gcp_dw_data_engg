  ...
  FROM
    (
      SELECT
          (
            SELECT
                coalesce(max(ref_secn_remittance_rendering_provider.remittance_secn_rendering_provider_sid), CAST(0 as BIGNUMERIC))
              FROM
                `hca-hin-dev-cur-parallon`.edwpbs.ref_secn_remittance_rendering_provider
          ) + CAST(row_number() OVER (ORDER BY upper(f.secn_rendering_provider_id_qlfr_code), upper(f.secn_rendering_provider_id)) as BIGNUMERIC) AS remittance_secn_rendering_provider_sid,
          --  SID
          ...
          (
            ...
            SELECT
              ...
              WHERE upper(coalesce(remittance_claim.rndrng_prvdr_rf_idn_qul_10_cod, '')) NOT IN(
               ...
               OR upper(coalesce(remittance_claim.rendering_provdr_secndary_id10, '')) NOT IN(
                ''
              )
          ) AS f
    ) AS a
