    ...
    (
      ...
      SELECT
        ...
        WHERE upper(coalesce(remittance_service.rendrng_provdr_ref_idn_qual10, '')) NOT IN(
         ...
         OR upper(coalesce(remittance_service.rendering_providr_identifier10, '')) NOT IN(
          ''
        )
    ) AS f
  WHERE (upper(coalesce(f.secn_rendering_provider_id_qlfr_code, '')), upper(coalesce(f.secn_rendering_provider_id, ''))) NOT IN(
    SELECT AS STRUCT
        upper(coalesce(ref_secn_remittance_rendering_provider.secn_rendering_provider_id_qlfr_code, '')),
