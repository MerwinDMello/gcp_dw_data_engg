  ...
  FROM
    (
      SELECT
          (
            SELECT
                coalesce(max(ref_remittance_corrected_priority_payor.corrected_priority_payor_sid), 0)
              FROM
                `hca-hin-dev-cur-parallon`.edwpbs.ref_remittance_corrected_priority_payor
          ) + row_number() OVER (ORDER BY upper(f.corrected_priority_payor_qualifier_code), upper(f.corrected_priority_payor_id), upper(f.corrected_priority_payor_name)) AS corrected_priority_payor_sid,
          --  SID
        ...
        FROM
          (
            SELECT
                max(remittance_claim.corretd_prior_payor_qulifr_cod) AS corrected_priority_payor_qualifier_code,
              ...
              GROUP BY upper(remittance_claim.corretd_prior_payor_qulifr_cod), upper(remittance_claim.corrected_priority_payor_num), upper(remittance_claim.nm103_corretd_priority_payr_nm)
          ) AS f
        WHERE (upper(f.corrected_priority_payor_qualifier_code), upper(f.corrected_priority_payor_id), upper(f.corrected_priority_payor_name)) NOT IN(
          SELECT AS STRUCT
              upper(ref_remittance_corrected_priority_payor.corrected_priority_payor_qualifier_code) AS corrected_priority_payor_qualifier_code,
