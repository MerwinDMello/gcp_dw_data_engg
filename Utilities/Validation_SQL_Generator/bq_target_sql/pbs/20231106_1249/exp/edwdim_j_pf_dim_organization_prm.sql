...
SELECT
    trim(format('%20d', count(*))) AS source_string
  FROM
    (
      SELECT
          dim_org_stg_mthy.coid,
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_staging.dim_org_stg_mthy
      UNION DISTINCT
      SELECT
          dim_org_stg_mthy_pf_t3185_dnd.coid,
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_staging.dim_org_stg_mthy_pf_t3185_dnd
      UNION DISTINCT
      SELECT
          dim_esb_organization_homehealth_dnd.coid,
