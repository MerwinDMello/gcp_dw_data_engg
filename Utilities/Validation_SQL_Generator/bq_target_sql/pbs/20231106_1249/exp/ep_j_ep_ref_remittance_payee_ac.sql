  ...
  FROM
    (
      SELECT
          (
            SELECT
                coalesce(max(ref_remittance_payee.remittance_payee_sid), 0)
              FROM
                `hca-hin-dev-cur-parallon`.edwpbs.ref_remittance_payee
          ) + row_number() OVER (ORDER BY provider_npi, stg.provider_tax_id, upper(max(stg.payee_name)), upper(max(stg.payee_identification_qual_code)), upper(max(stg.payee_city_name)), upper(max(stg.payee_state_code)), upper(max(stg.payee_postal_zone_code)), upper(max(stg.provider_tax_id_lookup_code))) AS remittance_payee_sid,
          /*
          ...
          provider_npi,
          CASE
             syslib.length(trim(format('%11d', stg.provider_tax_id)))
        ...
        FROM
          ...
          CROSS JOIN UNNEST(ARRAY[
            CASE
              WHEN syslib.length(trim(format('%11d', stg.provider_tax_id))) = 10 THEN trim(format('%11d', stg.provider_tax_id))
              ELSE CAST(NULL as STRING)
            END
          ]) AS provider_npi
        WHERE DATE(stg.dw_last_update_date_time) = (
         ...
         AND (upper(stg.payee_name), stg.payee_identification_qual_code, upper(stg.payee_city_name), upper(stg.payee_state_code), upper(stg.payee_postal_zone_code), upper(stg.provider_tax_id_lookup_code), CASE
          WHEN syslib.length(trim(format('%11d', stg.provider_tax_id))) = 10 THEN trim(format('%11d', stg.provider_tax_id))
          ELSE ''
        END, CASE
           syslib.length(trim(format('%11d', stg.provider_tax_id)))
        ...
        END) NOT IN(
          SELECT DISTINCT AS STRUCT
              upper(a.payee_name) AS payee_name,
            ...
            FROM
              `hca-hin-dev-cur-parallon`.edwpbs.ref_remittance_payee AS a
        )
        GROUP BY 2, stg.provider_tax_id, upper(stg.payee_name), upper(stg.payee_identification_qual_code), upper(stg.payee_city_name), upper(stg.payee_state_code), upper(stg.payee_postal_zone_code), upper(stg.provider_tax_id_lookup_code)
    ) AS a
;
