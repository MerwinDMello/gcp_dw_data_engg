...
-- Source: eim-parallon-cs-datamig-dev-0002/pbs_bulk_conversion_validation/20231106_1249/input/exp/ar_pbs_j_pf_fact_patient_ltchg_prm.sql
...
-- locking table Edwfs.Fact_Facility for access
SELECT
    concat(exp1.late_charge_credit_amt, exp1.late_charge_debit_amt) AS source_string
  FROM
    (
      SELECT
          date_sub(date_add(CAST(trim(concat(format_date('%Y-%m', psc.charge_posted_date), '-01')) as DATE), interval 1 MONTH), interval 6 DAY) AS date1,
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpf_base_views.patient_suspended_charge AS psc
          ...
          `hca-hin-dev-cur-parallon`.edwfs_base_views.fact_facility AS ff ON upper(ff.coid) = upper(psc.coid)
           AND upper(ff.company_code) = upper(psc.company_code)
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_rcom_ar_patient_level AS fp ON fp.patient_sid = psc.pat_acct_num
           AND upper(fp.company_code) = upper(psc.company_code)
           AND upper(fp.coid) = upper(psc.coid)
           AND CASE
             format_date('%Y%m', psc.charge_posted_date)
            WHEN '' THEN 0
