...
SELECT
  ...
  FROM
    (
      SELECT DISTINCT
          vwwctpoutboundmessages.destinationname
        FROM
          `hca-hin-dev-cur-clinical`.edwci_staging.vwwctpoutboundmessages
        WHERE upper(vwwctpoutboundmessages.destinationname) NOT IN(
          SELECT
              upper(ref_mhb_wctp_destination_name.wctp_destination_name) AS wctp_destination_name
            FROM
              `hca-hin-dev-cur-clinical`.edwci.ref_mhb_wctp_destination_name
        )
    ) AS a
;
