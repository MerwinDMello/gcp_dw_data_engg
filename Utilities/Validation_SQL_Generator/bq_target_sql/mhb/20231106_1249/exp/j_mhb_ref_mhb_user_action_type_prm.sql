    ...
    (
      SELECT
          CASE
            ...
            ELSE (
              SELECT
                  coalesce(max(ref_mhb_user_action_type.user_action_type_sid), 0)
                FROM
                  `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_user_action_type
            ) + row_number() OVER (ORDER BY upper(y.user_action_type_desc))
          ...
          (
            SELECT
              ...
              FROM
                (
                  SELECT
                      max(trim(vwwctpinboundmessages.user_action)) AS user_action_type_desc
                    FROM
                      `hca-hin-dev-cur-clinical`.edwci_staging.vwwctpinboundmessages
                    GROUP BY upper(trim(vwwctpinboundmessages.user_action))
                ) AS x
              WHERE upper(x.user_action_type_desc) NOT IN(
                SELECT
                    ...
                    upper(ref_mhb_user_action_type.user_action_type_desc) AS user_action_type_desc
                  FROM
                    `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_user_action_type
              )
          ) AS y
