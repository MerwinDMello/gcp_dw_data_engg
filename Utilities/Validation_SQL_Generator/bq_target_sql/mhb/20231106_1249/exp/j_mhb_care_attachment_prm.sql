  ...
  FROM
    (
      SELECT
        ...
        FROM
          `hca-hin-dev-cur-clinical`.edwci_staging.vwdynamiccareattachment AS attachment
          ...
          INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm_base_views.clinical_acctkeys AS ck ON upper(ck.coid) = upper(cf2.coid)
           AND ck.pat_acct_num = ROUND(CASE
             translate(attachment.patient_visitnumber, translate(attachment.patient_visitnumber, '0123456789', ''), '')
            WHEN '' THEN NUMERIC '0'
