        ...
        FROM
          (
            SELECT
              ...
              FROM
                `hca-hin-dev-cur-clinical`.edwci_staging.vwtextmessages
              GROUP BY 1, upper(vwtextmessages.sender_facilitycode), upper(vwtextmessages.sent_date_time), upper(vwtextmessages.message_content), upper(vwtextmessages.urgent), upper(vwtextmessages.quickpick), upper(vwtextmessages.databasename)
          ) AS x
          ...
          LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwcl_base_views.clinical_facility AS cf ON trim(cf.facility_mnemonic_cs) = trim(x.sender_facilitycode)
    ) AS q
;
