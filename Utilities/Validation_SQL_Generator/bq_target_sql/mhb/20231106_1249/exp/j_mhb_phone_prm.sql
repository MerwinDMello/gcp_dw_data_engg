...
SELECT
  ...
  FROM
    (
      SELECT
          mhb_phone_wrk.rdc_sid
        FROM
          `hca-hin-dev-cur-clinical`.edwci_staging.mhb_phone_wrk
        WHERE (coalesce(mhb_phone_wrk.rdc_sid, 0), coalesce(format('%#20.0f', mhb_phone_wrk.mhb_phone_id), ''), upper(coalesce(mhb_phone_wrk.phone_id_text, '')), upper(coalesce(mhb_phone_wrk.phone_name, '')), upper(coalesce(mhb_phone_wrk.phone_os_code, '')), upper(coalesce(mhb_phone_wrk.os_version_text, '')), upper(coalesce(mhb_phone_wrk.device_pooling_config_code, '')), upper(coalesce(mhb_phone_wrk.active_dw_ind, ''))) NOT IN(
          SELECT AS STRUCT
              coalesce(mhb_phone.rdc_sid, 0),
            ...
            FROM
              `hca-hin-dev-cur-clinical`.edwci.mhb_phone
        )
    ) AS q
;
