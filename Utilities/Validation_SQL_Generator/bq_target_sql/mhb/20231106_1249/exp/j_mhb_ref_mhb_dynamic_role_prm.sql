  ...
  FROM
    (
      SELECT
          (
            SELECT
                coalesce(max(ref_mhb_dynamic_role.dynamic_role_sid), 0)
              FROM
                `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_dynamic_role
          ) + row_number() OVER (ORDER BY y.rdc_sid, upper(y.dynamicrole_name), upper(y.dynamicrole_label)) AS dynamic_role_sid,
        ...
        FROM
          (
            SELECT
                vwdynamicrole.dynamicrole_name,
                vwdynamicrole.dynamicrole_label,
                rdc.rdc_sid
              FROM
                (
                  SELECT
                      max(vwdynamicroledetachment.dynamicrole_name) AS dynamicrole_name,
                    ...
                    FROM
                      `hca-hin-dev-cur-clinical`.edwci_staging.vwdynamicroledetachment
                    GROUP BY upper(vwdynamicroledetachment.dynamicrole_name), upper(vwdynamicroledetachment.dynamicrole_label), upper(vwdynamicroledetachment.databasename)
                  UNION DISTINCT
                  SELECT
                      substr(max(vwdynamicroleattachment.dynamicrole_name), 1, 30) AS dynamicrole_name,
                    ...
                    FROM
                      `hca-hin-dev-cur-clinical`.edwci_staging.vwdynamicroleattachment
                    GROUP BY upper(substr(vwdynamicroleattachment.dynamicrole_name, 1, 30)), upper(substr(vwdynamicroleattachment.dynamicrole_label, 1, 40)), upper(vwdynamicroleattachment.databasename)
                ) AS vwdynamicrole
