  ...
  FROM
    (
      SELECT
        ...
        FROM
          `hca-hin-dev-cur-clinical`.edwci_staging.vwwctpoutboundmessages AS stg
          ...
          INNER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.mhb_user AS u ON upper(stg.sender_username) = upper(u.user_login_name)
           AND rdc.rdc_sid = u.rdc_sid
          LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_user_role AS r ON upper(coalesce(trim(stg.sender_role), 'Unknown')) = upper(trim(r.mhb_user_role_desc))
           AND r.mhb_user_role_sid = u.mhb_user_role_sid
          INNER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.mhb_user AS usr ON upper(stg.recipient_username) = upper(usr.user_login_name)
           AND rdc.rdc_sid = usr.rdc_sid
          LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_user_role AS ur ON upper(coalesce(trim(stg.recipient_role), 'Unknown')) = upper(trim(ur.mhb_user_role_desc))
           AND ur.mhb_user_role_sid = usr.mhb_user_role_sid
          LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwcl_base_views.clinical_facility AS cf2 ON trim(cf2.facility_mnemonic_cs) = trim(stg.sender_facilitycode)
          LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwcl_base_views.clinical_facility_location AS cfl ON trim(cfl.location_mnemonic_cs) = trim(substr(stg.sender_unit_code, strpos(stg.sender_unit_code, '_') + 1))
          INNER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_unit AS rmu ON rmu.rdc_sid = rdc.rdc_sid
          ...
          LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_wctp_destination_name AS ref ON upper(coalesce(stg.destinationname, 'Unknown')) = upper(ref.wctp_destination_name)
        ...
        GROUP BY 1, 2, upper(coalesce(cf.company_code, 'H')), upper(coalesce(cf.coid, '99999')), upper(u.user_login_name), upper(coalesce(cf2.coid, '99999')), 7, 8, 9, upper(usr.user_login_name), upper(coalesce(cf.coid, '99999')), 12, 13, 14, 15, 16, 17, 18, upper(stg.messagepayload), 20, 21, 22, 23
    ) AS a
;
