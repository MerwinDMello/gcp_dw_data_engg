  ...
  FROM
    (
      SELECT
        ...
        FROM
          `hca-hin-dev-cur-clinical`.edwci_staging.vwwctpinboundmessages AS stg
          ...
          `hca-hin-dev-cur-clinical`.edwcl_base_views.clinical_facility AS cf ON trim(cf.facility_mnemonic_cs) = trim(stg.recipient_facilitycode)
          INNER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.mhb_user AS u ON upper(stg.recipient_username) = upper(u.user_login_name)
           AND rdc.rdc_sid = u.rdc_sid
          LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_user_role AS r ON upper(coalesce(trim(stg.recipient_role), 'Unknown')) = upper(trim(r.mhb_user_role_desc))
           AND r.mhb_user_role_sid = u.mhb_user_role_sid
          LEFT OUTER JOIN /*Left Join Edwcl_Base_Views.Clinical_Facility CF2
          on trim(CF2.Facility_Mnemonic_CS) = trim(stg.Recipient_FacilityCode)*/
          `hca-hin-dev-cur-clinical`.edwcl_base_views.clinical_facility_location AS cfl ON trim(cfl.location_mnemonic_cs) = trim(substr(stg.recipient_unit_code, strpos(stg.recipient_unit_code, '_') + 1))
          ...
          LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_message_delivery_priority AS mdp ON upper(coalesce(stg.message_delivery_priority, 'Unknown')) = upper(mdp.message_delivery_priority_desc)
          LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_wctp_source_name AS sn ON upper(coalesce(stg.source_name, 'Unknown')) = upper(sn.wctp_source_name)
          LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_wctp_source_type AS st ON upper(coalesce(stg.source_type, 'Unknown')) = upper(st.wctp_source_type_desc)
          LEFT OUTER JOIN `hca-hin-dev-cur-clinical`.edwci_base_views.ref_mhb_user_action_type AS uat ON upper(coalesce(stg.user_action, 'Unknown')) = upper(uat.user_action_type_desc)
        WHERE CAST(trim(substr(stg.received_date_time, 1, 10)) as DATE) BETWEEN u.eff_from_date AND u.eff_to_date
        GROUP BY 1, 2, upper(coalesce(cf.company_code, 'H')), upper(coalesce(cf.coid, '99999')), upper(u.user_login_name), upper(coalesce(cf.coid, '99999')), 7, 8, 9, upper(stg.sender_information), upper(stg.sender_message_id), upper(stg.sender_transaction_id), 13, 14, 15, 16, 17, 18, 19, upper(stg.message_payload), 21, 22, 23, 24, 25, 26, 27, upper(stg.lasttimestamp), 29, 30
    ) AS a
;
