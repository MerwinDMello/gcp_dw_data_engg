  ...
  FROM
    (
      SELECT
        ...
        FROM
          `hca-hin-dev-cur-clinical`.edwci_staging.vwinterappoutbound AS stg
        ...
        GROUP BY 1, upper(u10.user_login_name), 3, 4, upper(coalesce(ck.company_code, 'H')), upper(coalesce(ck.coid, '99999')), 7, 8, 9, upper(coalesce(cf22.coid, '99999')), 11, 12, 13, upper(stg.launch_app_name), upper(stg.launch_action), upper(stg.launch_status), upper(CASE
           upper(stg.launch_sso_enabled)
          WHEN 'TRUE' THEN 'Y'
          WHEN 'FALSE' THEN 'N'
        END), 18, upper(coalesce(cf31.coid, '99999')), 20, 21, upper(coalesce(cf34.coid, '99999')), 23, 24, 25, 26
    ) AS a
;
