...
SELECT
  ...
  FROM
    (
      SELECT
          ref_mhb_dynamic_role.*
        FROM
          `hca-hin-dev-cur-clinical`.edwci.ref_mhb_dynamic_role
        WHERE DATE(ref_mhb_dynamic_role.dw_last_update_date_time) = current_date('US/Central')
    ) AS q
;
