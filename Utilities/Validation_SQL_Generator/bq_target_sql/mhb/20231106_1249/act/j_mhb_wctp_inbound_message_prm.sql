...
SELECT
  ...
  FROM
    (
      SELECT
          mhb_wctp_inbound_message.*
        FROM
          `hca-hin-dev-cur-clinical`.edwci.mhb_wctp_inbound_message
        WHERE DATE(mhb_wctp_inbound_message.dw_last_update_date_time) = current_date('US/Central')
    ) AS q
;
