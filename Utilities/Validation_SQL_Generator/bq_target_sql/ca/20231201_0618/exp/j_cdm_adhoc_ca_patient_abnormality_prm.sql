    ...
    (
      SELECT DISTINCT
        ...
        FROM
          (
            SELECT
                CAST(NULL as INT64) AS patient_abnormality_sk,
              ...
              FROM
                `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_syndrome_stg AS css
                LEFT OUTER JOIN (
                  SELECT
                      c.source_patient_id,
                      s.server_name,
                ...
                ) AS a ON css.patid = a.source_patient_id
            ...
            SELECT
                ...
                timestamp_trunc(current_timestamp(), SECOND) AS dw_last_update_date_time
              FROM
                `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_chromabnormalities_stg AS ccab
                LEFT OUTER JOIN (
                  SELECT
                      c.source_patient_id,
                      s.server_name,
                    ...
                    FROM
                      `hca-hin-dev-cur-clinical`.edwcdm.ca_patient AS c
                      INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS s ON c.server_sk = s.server_sk
                ) AS a ON ccab.patid = a.source_patient_id
            ...
            SELECT
                ...
                timestamp_trunc(current_timestamp(), SECOND) AS dw_last_update_date_time
              FROM
                `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_ncaa_stg AS ns
                LEFT OUTER JOIN (
                  SELECT
                      c.source_patient_id,
                      s.server_name,
                    ...
                    FROM
                      `hca-hin-dev-cur-clinical`.edwcdm.ca_patient AS c
                      INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS s ON c.server_sk = s.server_sk
                ) AS a ON ns.patid = a.source_patient_id
