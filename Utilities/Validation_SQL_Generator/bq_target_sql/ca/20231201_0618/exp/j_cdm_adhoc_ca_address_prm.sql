      ...
      SELECT
          ...
          (
            SELECT DISTINCT
                ...
                CASE
                  ...
                  ELSE CAST(bqutil.fn.cw_td_normalize_number(trim(format('%11d', wrk1_0.country_id))) as INT64)
                END AS country_id
              FROM
                (
                  SELECT DISTINCT
                      cardioaccess_demographics_stg.pataddr AS address_line_1_text,
                  ...
                  UNION ALL
                  SELECT DISTINCT
                      substr(cacs.address, 1, 150) AS address_line_1_text,
                      substr(cacs.address2, 1, 60) AS address_line_2_text,
                      CAST(bqutil.fn.cw_td_normalize_number(cacs.address3) as INT64) AS address_line_3_text,
                      ...
                      (
                        SELECT
                            rcgl.lookup_id
                          FROM
                            `hca-hin-dev-cur-clinical`.edwcdm.ref_ca_global_lookup AS rcgl
                          WHERE upper(rcgl.sts_code_text) = upper(cacs.country)
                           AND upper(rcgl.short_name) = 'ISOCOUNTRY'
                      ) AS country_id
                    FROM
                      `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_contacts_stg AS cacs
                  UNION ALL
                  SELECT DISTINCT
                      substr(cahs.hospaddress, 1, 150) AS address_line_1_text,
                    ...
                    FROM
                      `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_hospital_stg AS cahs
                ) AS wrk1_0
              WHERE CASE
                WHEN upper(trim(wrk1_0.address_line_1_text)) = '' THEN CAST(NULL as STRING)
                ELSE trim(wrk1_0.address_line_1_text)
        ...
        WHERE NOT EXISTS (
          SELECT
              1
            FROM
              `hca-hin-dev-cur-clinical`.edwcdm.ca_address AS caa
            WHERE upper(coalesce(wrk1.address_line_1_text, 'NULL')) = upper(coalesce(caa.address_line_1_text, 'NULL'))
             ...
             AND coalesce(format('%11d', wrk1.country_id), 'NULL') = coalesce(format('%11d', caa.country_id), 'NULL')
        )
    ) AS a
;
