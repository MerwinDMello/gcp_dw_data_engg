        ...
        FROM
          (
            SELECT
              ...
              FROM
                `hca-hin-dev-cur-clinical`.edwcdm_staging.cardioaccess_referringphysicians_stg AS stg
                LEFT OUTER JOIN (
                  SELECT
                      c.contact_type_sk,
                ...
                ) AS a ON trim(format('%11d', stg.contacttypeid)) = trim(format('%11d', a.source_contact_type_id))
                ...
                LEFT OUTER JOIN (
                  SELECT
                      ...
                      s_0.server_name
                    FROM
                      `hca-hin-dev-cur-clinical`.edwcdm.ca_contact AS c
                      INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS s_0 ON trim(format('%11d', c.server_sk)) = trim(format('%11d', s_0.server_sk))
                ) AS b ON trim(format('%11d', stg.contactid)) = trim(format('%11d', b.source_contact_id))
                ...
                LEFT OUTER JOIN (
                  SELECT
                      ...
                      s_0.server_name
                    FROM
                      `hca-hin-dev-cur-clinical`.edwcdm.ca_patient_hosp AS c
                      INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS s_0 ON trim(format('%11d', c.server_sk)) = trim(format('%11d', s_0.server_sk))
                ) AS p_0 ON trim(format('%11d', stg.hospitalizationid)) = trim(format('%11d', p_0.source_patient_hosp_id))
