    ...
    (
      SELECT DISTINCT
        ...
        FROM
          (
            SELECT
                cah.hospital_sk AS hospital_sk,
              ...
              FROM
                ...
                INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_server AS cas ON upper(cas.server_name) = upper(cahs.full_server_nm)
                INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_hospital AS cah ON cas.server_sk = cah.server_sk
            ...
            SELECT
              ...
              FROM
                ...
                CROSS JOIN UNNEST(ARRAY[
                  substr(cahs.hosppfi, 1, 50)
                ]) AS hospital_detail_measure_value_text
            ...
            SELECT
              ...
              FROM
                ...
                INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_hospital AS cah ON cas.server_sk = cah.server_sk
                 AND cahs.hospitalid = cah.source_hospital_id
                CROSS JOIN UNNEST(ARRAY[
                  substr(cahs.encryptionkey, 1, 50)
                ]) AS hospital_detail_measure_value_text
              WHERE hospital_detail_measure_value_text IS NOT NULL
            UNION DISTINCT
            SELECT
            ...
            SELECT
              ...
              FROM
                ...
                INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_hospital AS cah ON cas.server_sk = cah.server_sk
                 AND cahs.hospitalid = cah.source_hospital_id
                CROSS JOIN UNNEST(ARRAY[
                  substr(cahs.pc4encryptionkey, 1, 50)
                ]) AS hospital_detail_measure_value_text
              WHERE hospital_detail_measure_value_text IS NOT NULL
            UNION DISTINCT
            SELECT
              ...
              FROM
                ...
                INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_hospital AS cah ON cas.server_sk = cah.server_sk
                 AND cahs.hospitalid = cah.source_hospital_id
                CROSS JOIN UNNEST(ARRAY[
                  substr(cahs.cparticipantid, 1, 50)
                ]) AS hospital_detail_measure_value_text
              WHERE hospital_detail_measure_value_text IS NOT NULL
            UNION DISTINCT
            SELECT
            ...
            SELECT
              ...
              FROM
                ...
                INNER JOIN `hca-hin-dev-cur-clinical`.edwcdm.ca_hospital AS cah ON cas.server_sk = cah.server_sk
                 AND cahs.hospitalid = cah.source_hospital_id
              WHERE cahs.faxnumber IS NOT NULL
          ) AS a
        WHERE upper(trim(a.hospital_detail_measure_value_text)) <> ''
         OR a.hospital_detail_measure_value_num IS NOT NULL
    ) AS b
;
