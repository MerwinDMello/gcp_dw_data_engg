...
SELECT
  ...
  FROM
    (
      SELECT
          trim(ref_physician_specialty_stg.physician_speciality_desc) AS physician_speciality_desc,
        ...
        WHERE upper(trim(ref_physician_specialty_stg.physician_speciality_desc)) NOT IN(
         ...
         AND ref_physician_specialty_stg.dw_last_update_date_time < (
          SELECT
              max(etl_job_run.job_start_date_time) AS job_start_date_time
            FROM
              `hca-hin-dev-cur-ops`.edwcr_dmx_ac.etl_job_run
            WHERE upper(rtrim(etl_job_run.job_name)) = 'J_REF_PHYSICIAN_SPECIALTY'
        )
    ) AS a
;
