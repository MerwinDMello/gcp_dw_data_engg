...
SELECT
  ...
  FROM
    (
      SELECT
          trim(ref_test_type_stg.test_type_desc) AS test_type_desc,
        ...
        FROM
          `hca-hin-dev-cur-ops`.edwcr_staging.ref_test_type_stg
        WHERE (upper(trim(ref_test_type_stg.test_type_desc)), upper(trim(ref_test_type_stg.test_sub_type_desc))) NOT IN(
          SELECT AS STRUCT
              upper(trim(ref_test_type.test_type_desc)),
              upper(trim(ref_test_type.test_sub_type_desc))
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_test_type
        )
         AND ref_test_type_stg.dw_last_update_date_time < (
          SELECT
              max(etl_job_run.job_start_date_time) AS job_start_date_time
            FROM
              `hca-hin-dev-cur-ops`.edwcr_dmx_ac.etl_job_run
            WHERE upper(rtrim(etl_job_run.job_name)) = 'J_CR_REF_TEST_TYPE'
        )
    ) AS a
;
