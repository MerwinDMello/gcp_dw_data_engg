      ...
      (
        SELECT
          ...
          FROM
            (
              SELECT
                  dates.cancer_patient_driver_sk,
                ...
                FROM
                  (
                    SELECT
                      ...
                      FROM
                        `hca-hin-dev-cur-ops`.edwcr_base_views.cancer_patient_id_output AS t1
                      ...
                      GROUP BY 1, 2, upper(t2.patient_market_urn_text), upper(t2.medical_record_num), upper(t2.empi_text)
                    UNION DISTINCT
                    SELECT
                        t1.cancer_patient_driver_sk,
                      ...
                      FROM
                        `hca-hin-dev-cur-ops`.edwcr_base_views.cancer_patient_driver AS t1
                      ...
                      GROUP BY 1, 2, upper(t1.patient_market_urn_text), upper(t1.medical_record_num), upper(t1.empi_text)
                    UNION DISTINCT
                    SELECT
                        t1.cancer_patient_driver_sk,
                        t1.network_mnemonic_cs,
                ...
                GROUP BY 1, 2, upper(dates.patient_market_urn_text)
            ) AS date_source
            ...
            (
              SELECT
                ...
                WHERE upper(rtrim(t1.user_action_desc)) = 'CONFIRM'
                 AND upper(rtrim(t1.message_flag_code)) <> 'RAD'
                GROUP BY 1, 2, upper(t2.patient_market_urn_text), upper(t2.medical_record_num), upper(t2.empi_text), upper(t2.coid)
              UNION DISTINCT
              SELECT
                  t1.cancer_patient_driver_sk,
                  t1.network_mnemonic_cs,
                ...
                FROM
                  ...
                  INNER JOIN `hca-hin-dev-cur-ops`.edwcr.cn_patient_diagnosis AS t3 ON t2.nav_patient_id = t3.nav_patient_id
                WHERE t3.diagnosis_date > DATE '1900-01-01'
                GROUP BY 1, 2, upper(t1.patient_market_urn_text), upper(t1.medical_record_num), upper(t1.empi_text), upper(t1.coid)
              UNION DISTINCT
              SELECT
                  t1.cancer_patient_driver_sk,
                  t1.network_mnemonic_cs,
              ...
              SELECT
                ...
                WHERE t2.admission_date > DATE '1900-01-01'
                GROUP BY 1, 2, upper(t1.patient_market_urn_text), upper(t1.medical_record_num), upper(t1.empi_text), upper(t1.coid)
            ) AS info ON date_source.cancer_patient_driver_sk = info.cancer_patient_driver_sk
             AND date_source.mindatesource = info.datesourceorder
          GROUP BY 1, 2, upper(info.patient_market_urn_text), upper(info.medical_record_num), upper(info.empi_text), upper(info.coid), upper(info.datesource)
      ) AS emr_date
)
SELECT
        ...
        FROM
          (
            SELECT
              ...
              FROM
                `hca-hin-dev-cur-ops`.edwcr_base_views.fact_encounter AS fe
              ...
              WHERE fe.admission_date_time >= sc.hospemr_date_driver
          ) AS a
      UNION ALL
      SELECT
          b.cancer_patient_driver_sk,
          b.patient_dw_id,
        ...
        WHERE b.patient_dw_id + b.cancer_patient_driver_sk NOT IN(
          SELECT
              a.patient_dw_id + a.cancer_patient_driver_sk
            FROM
              (
                SELECT
                    fe.patient_dw_id,
