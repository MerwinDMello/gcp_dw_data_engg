...
SELECT
  ...
  FROM
    (
      SELECT
          row_number() OVER (ORDER BY met.cr_patient_id, inav.nav_patient_id, cpid.patient_dw_id) AS cancer_patient_driver_sk,
                ...
                (
                  SELECT
                    ...
                    WHERE upper(rtrim(e.encounter_id_type_name)) = 'PATIENT_DW_ID'
                     AND CAST(bqutil.fn.cw_td_normalize_number(e.empi_text) as FLOAT64) > 0
                ) AS a
