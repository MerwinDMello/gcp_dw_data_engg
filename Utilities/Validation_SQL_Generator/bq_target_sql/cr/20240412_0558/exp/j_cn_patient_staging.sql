  ...
  FROM
    (
      SELECT
        ...
        WHERE (upper(cn_patient_staging_stg.hashbite_ssk), upper(cn_patient_staging_stg.cancer_stage_class_method_code)) IN(
          SELECT AS STRUCT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.cn_patient_staging
        )
         AND cn_patient_staging_stg.dw_last_update_date_time < (
          SELECT
            ...
            WHERE upper(rtrim(etl_job_run.job_name)) = 'J_CN_PATIENT_STAGING'
        )
    ) AS a
;
