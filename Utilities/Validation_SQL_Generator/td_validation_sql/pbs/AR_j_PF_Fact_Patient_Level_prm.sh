...
AND ARP.Patient_SID <>0 and Source_Sid=10"

export AC_EXP_SQL_STATEMENT="locking table edwpf_staging.pass_current for access
SELECT 
'PBMAR240'  || ',' ||
...
Trim(SUM(CASE WHEN Ins_Order_Num = 0 AND CAST(CAST( CAST (DISCH_DT AS DATE FORMAT 'YYYYMM') AS CHAR(6))  AS INTEGER) = CAST(RPTG_Period AS INTEGER)
                   THEN CAST(ZEROIFNULL(EOM_Total_Chgs) AS DECIMAL (18,3)) * .01
                   ELSE 0.000
            END)   )  || ',' ||
...
CASE WHEN CAST(PC.INS_PLAN1 AS INTEGER)  <> 0
           THEN CAST(PC.EOM_INS_BAL1 AS DECIMAL (18,3)) * .01
            ELSE 0.000
    END,
CAST(PC.EOM_INS_PYMT_RCVD1 AS DECIMAL (18,3)) * .01,
CAST(PC.EOM_INS_ALL_RCVD1 AS DECIMAL (18,3)) * .01, 
CAST(PC.EOM_PAT_BAL AS DECIMAL (18,3)) * .01,
PC.EOM_TOTAL_CHGS, 
CAST(PC.EOM_TOT_ACCT_BAL AS DECIMAL (18,3)) * .01,
1 

FROM
edwpf_staging.PASS_Current PC,
...
and  cast(PC.DISCH_DT as date) <= (add_months(cast((substr(PC.RPTG_PERIOD, 1, 4) || '-' || substr(PC.RPTG_PERIOD, 5, 2) || '-01') as date) ,1) -1) 
...
CASE WHEN CAST(PC.INS_PLAN2 AS INTEGER)  <> 0
           THEN CAST(PC.EOM_INS_BAL2 AS DECIMAL (18,3)) * .01
            ELSE 0.000
    END,
CAST(PC.EOM_INS_PYMT_RCVD2 AS DECIMAL (18,3)) * .01,
CAST(PC.EOM_INS_ALL_RCVD2 AS DECIMAL (18,3)) * .01, 
CAST(PC.EOM_PAT_BAL AS DECIMAL (18,3)) * .01,
PC.EOM_TOTAL_CHGS, 
CAST(PC.EOM_TOT_ACCT_BAL AS DECIMAL (18,3)) * .01,
2
...
CASE WHEN CAST(PC.INS_PLAN3 AS INTEGER)  <> 0
           THEN CAST(PC.EOM_INS_BAL3 AS DECIMAL (18,3)) * .01
            ELSE 0.000
    END,
CAST(PC.EOM_INS_PYMT_RCVD3 AS DECIMAL (18,3)) * .01,
CAST(PC.EOM_INS_ALL_RCVD3 AS DECIMAL (18,3)) * .01, 
CAST(PC.EOM_PAT_BAL AS DECIMAL (18,3)) * .01,
PC.EOM_TOTAL_CHGS, 
CAST(PC.EOM_TOT_ACCT_BAL AS DECIMAL (18,3)) * .01,
3
...
CAST(PC.DISCH_DT AS DATE FORMAT 'YYYY-MM-DD'), 
CASE WHEN CAST(PC.INS_PLAN1 AS INTEGER)  = 0
           THEN CAST(PC.EOM_PAT_BAL AS DECIMAL (18,3)) * .01
            ELSE 0.000
    END,
CAST(PC.EOM_PAT_PYMT_RCVD AS DECIMAL (18,3)) * .01, 
CAST(PC.EOM_PAT_ALL_RCVD AS DECIMAL (18,3)) * .01, 
CAST(PC.EOM_PAT_BAL AS DECIMAL (18,3)) * .01,
PC.EOM_TOTAL_CHGS, 
CAST(PC.EOM_TOT_ACCT_BAL AS DECIMAL (18,3)) * .01,
0
