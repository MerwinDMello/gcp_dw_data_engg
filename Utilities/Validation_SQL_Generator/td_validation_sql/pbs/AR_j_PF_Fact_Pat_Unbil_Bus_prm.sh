...
export JOBNAME='PBMAR390-40'

export AC_EXP_SQL_STATEMENT="select  
	'PBMAR390-40'||','||coalesce(trim(cast(zeroifnull(sum(du.unbilled_charge_amt)) as varchar(20))),'0')  ||',' as Source_string
from 
...
du.unbilled_responsibility_ind  = 'B'
AND (du.Company_Code = 'H' OR substr(Trim(FD.Company_Code_Operations),1,1) = 'Y')
AND (du.Account_Process_Ind = 'S')"

export AC_ACT_SQL_STATEMENT="Select 'PBMAR390-40'||','||coalesce(trim(Cast(zeroifnull(Sum(Unbilled_Gross_Bus_Ofc_Amt)) As VARCHAR(20))),'0') ||',' as source_string
