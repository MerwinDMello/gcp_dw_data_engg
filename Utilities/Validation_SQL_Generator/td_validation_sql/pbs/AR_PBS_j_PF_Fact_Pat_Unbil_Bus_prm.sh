...
export JOBNAME='PBMAR900-40'

export AC_EXP_SQL_STATEMENT="select  
	'PBMAR900-40'||','||coalesce(trim(cast(zeroifnull(sum(du.unbilled_charge_amt)) as varchar(20))),'0')  ||',' as Source_string
from 
...
export AC_ACT_SQL_STATEMENT="Select 'PBMAR900-40'||','||coalesce(trim(Cast(zeroifnull(Sum(Unbilled_Gross_Bus_Ofc_Amt)) As VARCHAR(20))),'0') ||',' as source_string
