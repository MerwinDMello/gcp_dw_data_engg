...
export JOBNAME='PBMAR390-60'

export AC_EXP_SQL_STATEMENT="Locking table edwpf_base_views.FACT_PATIENT for access
...
Select 'PBMAR390-60' ||','|| trim(zeroifnull(Count(Pat_Acct_Num))) ||','||  trim(sum(cast(Discharge_To_Billing_Day_Cnt as decimal(20,2)))) ||',' as source_string
From(
Select 
a.Pat_Acct_Num,
...
left outer join edwpf_base_views.fact_rcom_ar_patient_level fp
...
select 'PBMAR390-60' ||','|| trim(zeroifnull(sum(Billed_Patient_cnt))) ||','|| 
