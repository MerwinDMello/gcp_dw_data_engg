...
export JOBNAME='PBMAR390-20'
export AC_ACT_SQL_STATEMENT="Locking table edwpf.Fact_RCOM_AR_Patient_Level for access
Select 'PBMAR390-20' || ',' || coalesce(Cast(Sum(Late_Charge_Credit_Amt) As VARCHAR(20)), '0') || ',' ||
...
locking table Edwfs.Fact_Facility for access
locking table edwpf.fact_rcom_ar_patient_level  for access
Select 'PBMAR390-20' || ',' || EXP1.Late_Charge_Credit_Amt || ','  ||   EXP1.Late_Charge_Debit_Amt || ',' AS SOURCE_STRING from (
SELECT 
