export JOBNAME='J_CN_PATIENT_PROCEDURE_PATHOLOGY_RESULT'
export AC_EXP_SQL_STATEMENT="Select 'J_CN_PATIENT_PROCEDURE_PATHOLOGY_RESULT'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING from 
(
Select 
...
from $NCR_STG_SCHEMA.CN_PATIENT_PROCEDURE_PATHOLOGY_RESULT_STG STG
...
where (Hashbite_SSK,Navigation_Procedure_Type_Code)  not in (Select Hashbite_SSK,Navigation_Procedure_Type_Code from $NCR_TGT_SCHEMA.CN_PATIENT_PROCEDURE_PATHOLOGY_RESULT )
and STG.DW_Last_Update_Date_Time < (SELECT MAX(Job_Start_Date_Time) as Job_Start_Date_Time FROM $NCR_AC_VIEW.ETL_JOB_RUN where Job_Name = 'J_CN_PATIENT_PROCEDURE_PATHOLOGY_RESULT')

) SRC"


export AC_ACT_SQL_STATEMENT="Select 'J_CN_PATIENT_PROCEDURE_PATHOLOGY_RESULT'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING from ${NCR_TGT_SCHEMA}.CN_PATIENT_PROCEDURE_PATHOLOGY_RESULT 
where DW_LAST_UPDATE_DATE_TIME >= (Select max(Job_Start_Date_time) from $NCR_AC_VIEW.ETL_JOB_RUN where Job_Name='J_CN_PATIENT_PROCEDURE_PATHOLOGY_RESULT');"