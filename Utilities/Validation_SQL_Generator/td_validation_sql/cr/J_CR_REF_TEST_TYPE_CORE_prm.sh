export JOBNAME='J_CR_REF_TEST_TYPE'
...
export AC_ACT_SQL_STATEMENT="select 'J_CR_REF_TEST_TYPE'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING 
FROM EDWCR.REF_TEST_TYPE WHERE 
DW_Last_Update_Date_Time >= (SELECT MAX(Job_Start_Date_Time) as Job_Start_Date_Time FROM EDWCR_DMX_AC.ETL_JOB_RUN where Job_Name = 'J_CR_REF_TEST_TYPE');"
