export JOBNAME='J_REF_BREAST_CANCER_TYPE'
...
export AC_ACT_SQL_STATEMENT="select 'J_REF_BREAST_CANCER_TYPE'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING 
FROM EdwCR.Ref_Breast_Cancer_Type WHERE 
DW_Last_Update_Date_Time >= (SELECT MAX(Job_Start_Date_Time) as Job_Start_Date_Time FROM EDWCR_DMX_AC.ETL_JOB_RUN where Job_Name = 'J_REF_BREAST_CANCER_TYPE');"
