export JOBNAME='J_CN_REG_CANCER_PATIENT_DRIVER'

export AC_EXP_SQL_STATEMENT="Select 'J_CN_REG_CANCER_PATIENT_DRIVER'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING from
(
SELECT
ROW_NUMBER() OVER(ORDER BY MET.CR_PATIENT_ID,INAV.NAV_PATIENT_ID,CPID.PATIENT_DW_ID ASC) AS CANCER_PATIENT_DRIVER_SK,
...
MEDICAL_RECORD_NUM,
PATIENT_DW_ID , CRIO.COID       ,COMPANY_CODE        
...
CPT.MEDICAL_RECORD_NUM,CPT.COMPANY_CODE,
CPT.EMPI_TEXT,CPT.COID,
...
QUALIFY ROW_NUMBER () OVER(PARTITION BY COALESCE(CPT.PATIENT_MARKET_URN,CPT.MEDICAL_RECORD_NUM||DF2.FACILITY_MNEMONIC_CS,CP.Nav_Patient_ID) ORDER BY CP.NAV_PATIENT_ID DESC)=1
  ...
  LEFT OUTER JOIN
  (SEL  PATIENT_MARKET_URN,NETWORK_MNEMONIC_CS,EMPI_TEXT FROM (
SEL  PATIENT_MARKET_URN,NETWORK_MNEMONIC_CS, E.EMPI_TEXT FROM EDWCR_VIEWS.CLINICAL_REGISTRATION CR
INNER JOIN
EDWCR_VIEWS.EMPI_ENCOUNTER_ID_XWALK E
ON CR.PATIENT_DW_ID=E.ENCOUNTER_ID
WHERE ENCOUNTER_ID_TYPE_NAME='PATIENT_DW_ID'
AND EMPI_TEXT>0
) A
QUALIFY ROW_NUMBER() OVER(PARTITION BY PATIENT_MARKET_URN,NETWORK_MNEMONIC_CS ORDER BY EMPI_TEXT ASC)=1) EMPI
ON 
COALESCE(MET.NETWORK_MNEMONIC_CS,CPID.NETWORK_MNEMONIC_CS,INAV.NETWORK_MNEMONIC_CS) = EMPI.NETWORK_MNEMONIC_CS
AND COALESCE(MET.PATIENT_MARKET_URN_TEXT,CPID.PATIENT_MARKET_URN,INAV.EMPI_TEXT) =EMPI.PATIENT_MARKET_URN
)B"


export AC_ACT_SQL_STATEMENT="Select 'J_CN_REG_CANCER_PATIENT_DRIVER'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING from ${NCR_TGT_SCHEMA}.CANCER_PATIENT_DRIVER"