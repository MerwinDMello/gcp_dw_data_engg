...
export JOBNAME='J_CN_CANCER_PAT_ID_OUTPUT_DRIVER'

export AC_EXP_SQL_STATEMENT="SELECT 'J_CN_CANCER_PAT_ID_OUTPUT_DRIVER'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING 
FROM 
(SELECT  
row_number() over (order by A1.Cancer_Patient_Driver_SK,A1.Cancer_Patient_Id_Output_SK  asc) as Cancer_Patient_Id_Output_Driver_SK,
...
inner join 
 (Sel  Cancer_Patient_Driver_SK, COID,cp_patient_id,Network_Mnemonic_CS from EDWCR_BASE_VIEWS.Cancer_Patient_Driver

qualify row_number() over(partition by cp_patient_id,Network_Mnemonic_CS order by Cancer_Patient_Driver_SK asc)=1
) Cpd
on A.patient_dw_id=cpd.cp_patient_id
...
A.DW_Last_Update_Date_Time
FROM EDWCR_BASE_VIEWS.Cancer_Patient_Id_Output A
Left outer JOIN (SELECT  FACILITY_MNEMONIC_CS,COID,FACILITY_ACTIVE_IND FROM EDWCR_BASE_VIEWS.CLINICAL_FACILITY 
  QUALIFY ROW_NUMBER() OVER(PARTITION BY FACILITY_MNEMONIC_CS ORDER BY FACILITY_ACTIVE_IND DESC )=1 ) DF2
  ON A.Facility_menmonic_cs =DF2.FACILITY_MNEMONIC_CS
Left Outer JOIN (
Sel  Cancer_Patient_Driver_SK, coid,COALESCE(
patient_market_urn_text||Network_Mnemonic_CS,
MEDICAL_RECORD_NUM||coid) as PID,Network_Mnemonic_CS from EDWCR_BASE_VIEWS.Cancer_Patient_Driver
qualify row_number() over(partition by COALESCE(patient_market_urn_text||Network_Mnemonic_CS,MEDICAL_RECORD_NUM||COID) order by Cancer_Patient_Driver_SK asc)=1
) Cpd
ON COALESCE(A.patient_market_urn||A.Network_Mnemonic_CS,A.MEDICAL_RECORD_NUM||coalesce(A.coid,DF2.coid))  = Cpd.PID
and coalesce(A.Network_Mnemonic_CS,'-1')=coalesce(cpd.Network_Mnemonic_CS,'-1')
Where
(Zeroifnull(A.Patient_DW_ID),A.Network_Mnemonic_CS) not in (
SELECT  
Zeroifnull(A.Patient_DW_ID),A.Network_Mnemonic_CS
FROM EDWCR_BASE_VIEWS.Cancer_Patient_Id_Output A
inner join EDWCR_BASE_VIEWS.Cancer_Patient_Driver cpd
on A.patient_dw_id=cpd.cp_patient_id
and A.Network_Mnemonic_CS=cpd.Network_Mnemonic_CS
...
export AC_ACT_SQL_STATEMENT="select 'J_CN_CANCER_PAT_ID_OUTPUT_DRIVER'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING 
