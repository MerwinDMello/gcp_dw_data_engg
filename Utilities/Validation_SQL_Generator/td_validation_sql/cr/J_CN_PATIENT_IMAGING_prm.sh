export JOBNAME='J_CN_PATIENT_IMAGING'
export AC_EXP_SQL_STATEMENT="Select 'J_CN_PATIENT_IMAGING'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING from 
...
export AC_ACT_SQL_STATEMENT="Select 'J_CN_PATIENT_IMAGING'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING from $NCR_TGT_SCHEMA.CN_PATIENT_IMAGING
where DW_LAST_UPDATE_DATE_TIME >= (Select max(Job_Start_Date_time) from $NCR_AC_VIEW.ETL_JOB_RUN where Job_Name='J_CN_PATIENT_IMAGING');"
