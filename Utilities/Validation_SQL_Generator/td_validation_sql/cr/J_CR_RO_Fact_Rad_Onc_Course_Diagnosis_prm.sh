...
export JOBNAME='J_CR_RO_Fact_Rad_Onc_Course_Diagnosis'

export AC_EXP_SQL_STATEMENT="SELECT 'J_CR_RO_Fact_Rad_Onc_Course_Diagnosis'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING 
FROM 
(SELECT
ROW_NUMBER() OVER (
ORDER BY DimSiteID,
...
export AC_ACT_SQL_STATEMENT="select 'J_CR_RO_Fact_Rad_Onc_Course_Diagnosis'||','|| cast(count(*) as varchar(20))||',' as SOURCE_STRING 
FROM $NCR_TGT_SCHEMA.Fact_Rad_Onc_Course_Diagnosis
where 
