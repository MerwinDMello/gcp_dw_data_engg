...
export JOBNAME='J_MHB_Dynamic_Role_Attachment'

export AC_ACT_SQL_STATEMENT="SELECT 'J_MHB_Dynamic_Role_Attachment'||','||CAST(Count(*) AS VARCHAR (20))||','
...
export AC_EXP_SQL_STATEMENT="SELECT 'J_MHB_Dynamic_Role_Attachment'||','||CAST(Count(*) AS VARCHAR (20))||','
AS SOURCE_STRING FROM
(
SELECT 
RDC.RDC_SID AS RDC_SID
,MU.User_Login_Name AS User_Login_Name
,cast(ATTACHMENT.DynamicRole_DateTime as timestamp(6)) AS Attach_Date_Time
...
,ATTACHMENT.DynamicRole_Phone AS Dynamic_Role_Phone_Num
,ATTACHMENT.Trail_Id AS MHB_Audit_Trail_Num
,'B' AS SOURCE_SYSTEM_CODE,
CURRENT_TIMESTAMP(0) AS DW_LAST_UPDATE_DATE_TIME

FROM EDWCI_STAGING.VWDYNAMICROLEATTACHMENT ATTACHMENT
...
ON ATTACHMENT.USER_NAME = MU.USER_LOGIN_NAME 
...
and Dynamic_Role.MHB_Dynamic_Role_Parent_Name = ATTACHMENT.DynamicRole_Name 
and Dynamic_Role.MHB_Dynamic_Role_Child_Name = ATTACHMENT.DynamicRole_Label

LEFT JOIN EDWCI_BASE_VIEWS.REF_MHB_USER_ROLE RMUR
ON TRIM(ATTACHMENT.USER_ACTUAL_ROLE)  = TRIM(RMUR.MHB_USER_ROLE_DESC)
...
LEFT JOIN EDWCI_BASE_VIEWS.MHB_Dynamic_Role_Attachment  Dynamic_Role_Attachment
ON Dynamic_Role_Attachment.RDC_SID = RDC.RDC_SID
AND Dynamic_Role_Attachment.User_Login_Name = MU.User_Login_Name
AND Dynamic_Role_Attachment.Attach_Date_Time = ATTACHMENT.DynamicRole_DateTime 

WHERE  CAST(ATTACHMENT.DYNAMICROLE_DATETIME AS TIMESTAMP(6)) BETWEEN MU.EFF_FROM_DATE  AND MU.EFF_TO_DATE
