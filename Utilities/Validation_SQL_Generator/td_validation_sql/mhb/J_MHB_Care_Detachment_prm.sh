...
export JOBNAME='J_MHB_Care_Detachment'



export AC_ACT_SQL_STATEMENT="SELECT 'J_MHB_Care_Detachment'||','||CAST(Count(*) AS VARCHAR (20))||','
...
export AC_EXP_SQL_STATEMENT="SELECT 'J_MHB_Care_Detachment'||','||CAST(Count(*) AS VARCHAR (20))||','
AS SOURCE_STRING FROM
(
SELECT 
...
,cast(Detachment.Detach_DateTime as timestamp(6)) AS Detach_Date_Time
...
,DETACHMENT.Detach_Source AS Detach_Source_Text
,'H' AS SOURCE_SYSTEM_CODE,
CURRENT_TIMESTAMP(0) AS DW_LAST_UPDATE_DATE_TIME

FROM EDWCI_STAGING.vwDynamicCareDetachment DETACHMENT
...
ON DETACHMENT.Owner_User_Name = MU.USER_LOGIN_NAME 
AND RDC.RDC_SID=MU.RDC_SID
inner join Edw_Pub_Views.Clinical_Facility CF2
on  trim(CF2.Facility_Mnemonic_CS) = trim(DETACHMENT.Patient_FacilityCode)
inner join EDWCDM_BASE_VIEWS.Clinical_Acctkeys CK 
on CK.coid = CF2.coid and  CK.Pat_Acct_Num = Cast (oTranslate(DETACHMENT.Patient_VisitNumber, oTranslate(DETACHMENT.Patient_VisitNumber, '0123456789',''), '') as DECIMAL(18,0))
...
ON TRIM(DETACHMENT.Owner_User_Role)  = TRIM(RMUR.MHB_USER_ROLE_DESC)
...
WHERE  CAST(DETACHMENT.Detach_DateTime AS TIMESTAMP(6)) BETWEEN MU.EFF_FROM_DATE  AND MU.EFF_TO_DATE
AND CAST(Detach_DateTime AS TIMESTAMP(6)) BETWEEN MU2.EFF_FROM_DATE  AND MU2.EFF_TO_DATE
