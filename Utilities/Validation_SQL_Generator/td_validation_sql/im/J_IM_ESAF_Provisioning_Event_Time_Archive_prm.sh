...
export Job_Name='J_IM_ESAF_Provisioning_Event_Time_Archive'
export JOBNAME='J_IM_ESAF_Provisioning_Event_Time_Archive'

export AC_EXP_SQL_STATEMENT="select 'J_IM_ESAF_Provisioning_Event_Time_Archive' ||','||cast(zeroifnull(count(*)) as varchar(20)) ||',' as source_string from
 (
SELECT 
T2.IM_Domain_Id
,T1.eSAF_User_Id
...
export AC_ACT_SQL_STATEMENT="select 'J_IM_ESAF_Provisioning_Event_Time_Archive' ||','||cast(zeroifnull(A.counts) as varchar(20)) ||',' as source_string from 
(
SELECT
	COUNT(*) as counts
