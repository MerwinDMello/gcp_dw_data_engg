export Job_Name='J_IM_Cactus_Provider_Archive'
export JOBNAME='J_IM_Cactus_Provider_Archive'


export AC_EXP_SQL_STATEMENT="select 'J_IM_Cactus_Provider_Archive' ||','||cast(zeroifnull(F.counts) as varchar(20)) ||',' AS SOURCE_STRING from 
(
...
export AC_ACT_SQL_STATEMENT="select 'J_IM_Cactus_Provider_Archive' ||','||cast(zeroifnull(A.counts) as varchar(20)) ||',' AS SOURCE_STRING from 
(
 SELECT COUNT(*) as counts FROM
