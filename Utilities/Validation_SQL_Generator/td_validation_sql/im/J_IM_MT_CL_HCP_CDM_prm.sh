...
export Job_Name='J_IM_MT_CL_HCP_CDM'
export JOBNAME='J_IM_MT_CL_HCP_CDM'

export AC_EXP_SQL_STATEMENT="select 'J_IM_MT_CL_HCP_CDM' ||','||cast(zeroifnull(count(*)) as varchar(20)) ||',' as source_string from 
(
...
export AC_ACT_SQL_STATEMENT="select 'J_IM_MT_CL_HCP_CDM' ||','||cast(zeroifnull(A.counts) as varchar(20)) ||',' as source_string from 
(
SELECT
	COUNT(*) as counts
 FROM Edwim_Staging.MT_CL_HCP_CDM        
