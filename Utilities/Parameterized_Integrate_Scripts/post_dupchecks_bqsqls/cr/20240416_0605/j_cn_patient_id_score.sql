...
FROM `hca-hin-dev-cur-ops`.edwcr_base_views.cancer_patient_id_output AS res
  ...
  (SELECT count(*)
   FROM
     (SELECT cancer_patient_id_output_sk,
             score_sequence_num
      FROM `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_score
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_score');
