...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_status AS mt USING
  (SELECT DISTINCT row_number() OVER (
     ...
     (SELECT coalesce(max(ref_status.status_id), 0) AS id1
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_status) AS status_id,
 ...
 upper(trim(ref_status.status_type_desc))
           FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_status) ) AS type_stg) AS ms ON mt.status_id = ms.status_id
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT status_id
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_status
      GROUP BY status_id
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_status');
