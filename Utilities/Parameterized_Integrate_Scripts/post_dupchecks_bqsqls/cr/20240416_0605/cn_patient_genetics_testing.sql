...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_genetics_testing
WHERE upper(cn_patient_genetics_testing.hashbite_ssk) NOT IN
    (SELECT upper(cn_patient_genetics_testing_stg.hbsource) AS hbsource
     FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_genetics_testing_stg);
...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_genetics_testing AS mt USING
  (SELECT DISTINCT pgt.geneticstestingfactid,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_genetics_testing_stg AS pgt
   ...
   WHERE upper(pgt.hbsource) NOT IN
       (SELECT upper(cn_patient_genetics_testing.hashbite_ssk) AS hashbite_ssk
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_genetics_testing) ) AS ms ON mt.cn_patient_genetics_testing_sid = ms.geneticstestingfactid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_genetics_testing');
