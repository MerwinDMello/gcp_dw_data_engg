  ...
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM
     (SELECT dimdiagnosiscode_stg.dimdiagnosiscodeid,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.dimdiagnosiscode_stg) AS dp
   INNER JOIN
   ...
   LEFT OUTER JOIN
     (SELECT DISTINCT trim(cr_sc_diagnosissites_stg.diagnosiscode) AS diagnosiscode,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.cr_sc_diagnosissites_stg) AS sc ON rtrim(sc.diagnosiscode) = rtrim(dp.diagnosiscode)) AS ms ON mt.diagnosis_code_sk = ms.diagnosis_code_sk
  ...
  (SELECT count(*)
   FROM
     (SELECT diagnosis_code_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_diagnosis_code
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_diagnosis_code');
