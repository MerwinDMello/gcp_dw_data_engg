  ...
  (SELECT count(*)
   FROM
     (SELECT fact_patient_diagnosis_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.fact_rad_onc_patient_diagnosis
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.fact_rad_onc_patient_diagnosis');
