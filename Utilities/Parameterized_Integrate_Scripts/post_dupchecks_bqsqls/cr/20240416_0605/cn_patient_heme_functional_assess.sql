...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_func_assess
WHERE upper(rtrim(cn_patient_heme_func_assess.hashbite_ssk)) NOT IN
    (SELECT upper(rtrim(cn_patient_heme_functional_assess_stg.hbsource)) AS hbsource
     FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_heme_functional_assess_stg);
...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_func_assess AS mt USING
  (SELECT DISTINCT stg.patienthemefuncassessfactid AS cn_patient_heme_func_assess_sid,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_heme_functional_assess_stg AS stg
   ...
   WHERE upper(rtrim(stg.hbsource)) NOT IN
       (SELECT upper(rtrim(cn_patient_heme_func_assess.hashbite_ssk)) AS hashbite_ssk
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_func_assess) ) AS ms ON mt.cn_patient_heme_func_assess_sid = ms.cn_patient_heme_func_assess_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_func_assess');
