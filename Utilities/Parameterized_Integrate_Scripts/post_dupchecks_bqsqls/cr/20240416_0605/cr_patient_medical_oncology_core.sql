...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_medical_oncology AS mt USING
  (SELECT DISTINCT cr_patient_medical_oncology_wrk.cycle_id,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_medical_oncology_wrk) AS ms ON coalesce(mt.cycle_id, 0) = coalesce(ms.cycle_id, 0)
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT drug_id
      FROM `hca-hin-dev-cur-ops`.edwcr.cr_patient_medical_oncology
      GROUP BY drug_id
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cr_patient_medical_oncology');
