  ...
  (SELECT count(*)
   FROM
     (SELECT patient_contact_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient_contact
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient_contact');
