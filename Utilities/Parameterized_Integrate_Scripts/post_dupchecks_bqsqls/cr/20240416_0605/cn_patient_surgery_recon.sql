...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_surgery_reconstruction
WHERE upper(cn_patient_surgery_reconstruction.hashbite_ssk) NOT IN
    (SELECT upper(cn_patient_surgery_reconstruction_stg.hashbite_ssk) AS hashbite_ssk
     FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_surgery_reconstruction_stg);
  ...
  (SELECT DISTINCT stg.cn_patient_surgery_recstr_sid,
   ...
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_surgery_reconstruction.hashbite_ssk) AS hashbite_ssk
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_surgery_reconstruction) ) AS ms ON mt.cn_patient_surgery_recstr_sid = ms.cn_patient_surgery_recstr_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_surgery_reconstruction');
