...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_therapy_type AS mt USING
  (SELECT DISTINCT row_number() OVER (
     ...
     (SELECT coalesce(max(ref_therapy_type.therapy_type_id), 0) AS id1
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_therapy_type) AS therapy_type_id,
   ...
   FROM
     (SELECT therapy_type_stg.therapy_type_desc,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.therapy_type_stg
      WHERE upper(trim(therapy_type_stg.therapy_type_desc)) NOT IN
          (SELECT upper(trim(ref_therapy_type.therapy_type_desc))
           FROM `hca-hin-dev-cur-ops`.edwcr.ref_therapy_type) ) AS type_stg) AS ms ON mt.therapy_type_id = ms.therapy_type_id
  ...
  (SELECT count(*)
   FROM
     (SELECT therapy_type_id
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_therapy_type
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_therapy_type');
