...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_contact AS mt USING
  (SELECT DISTINCT cr_patient_contact_wrk.patient_contact_id,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_contact_wrk) AS ms ON mt.patient_contact_id = ms.patient_contact_id
  ...
  (SELECT count(*)
   FROM
     (SELECT patient_contact_id,
             cr_patient_id
      FROM `hca-hin-dev-cur-ops`.edwcr.cr_patient_contact
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cr_patient_contact');
