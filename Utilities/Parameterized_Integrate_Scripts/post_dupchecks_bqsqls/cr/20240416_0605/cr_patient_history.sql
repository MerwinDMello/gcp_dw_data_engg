...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_history AS mt USING
  (SELECT DISTINCT stg.patientid AS cr_patient_id,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_history_stg AS stg
   LEFT OUTER JOIN
  ...
  (SELECT count(*)
   FROM
     (SELECT cr_patient_id,
             tumor_id
      FROM `hca-hin-dev-cur-ops`.edwcr.cr_patient_history
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cr_patient_history');
