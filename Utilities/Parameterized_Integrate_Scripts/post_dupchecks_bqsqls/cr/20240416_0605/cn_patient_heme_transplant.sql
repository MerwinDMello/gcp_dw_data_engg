  ...
  (SELECT DISTINCT iq.cn_patient_heme_sid AS cn_patient_heme_sid,
   ...
   FROM
     (SELECT phd.patienthemefactid AS cn_patient_heme_sid,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.stg_patienthemetransplant AS phd
  ...
  (SELECT count(*)
   FROM
     (SELECT cn_patient_heme_sid
      FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_transplant
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_transplant');
...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_transplant AS cpht
WHERE NOT EXISTS
    (SELECT 'X'
     FROM `hca-hin-dev-cur-ops`.edwcr_staging.stg_patienthemetransplant AS sphd
