  ...
  (SELECT count(*)
   FROM
     (SELECT ajcc_stage_id
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_ajcc_stage
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_ajcc_stage');
