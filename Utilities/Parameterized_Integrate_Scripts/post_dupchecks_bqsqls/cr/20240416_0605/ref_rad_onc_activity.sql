...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_activity AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimactivity AS dhd
   INNER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.ref_rad_onc_site AS rr ON rr.source_site_id = CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dhd.dimsiteid) AS INT64)) AS ms ON mt.activity_sk = ms.activity_sk
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT activity_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_activity
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_activity');
