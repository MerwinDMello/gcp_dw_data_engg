...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_national_service_center AS mt USING
  (SELECT DISTINCT ref_national_svc_center_stg.id,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.ref_national_svc_center_stg) AS ms ON mt.nsc_id = ms.id
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT nsc_id
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_national_service_center
      GROUP BY nsc_id
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_national_service_center');
