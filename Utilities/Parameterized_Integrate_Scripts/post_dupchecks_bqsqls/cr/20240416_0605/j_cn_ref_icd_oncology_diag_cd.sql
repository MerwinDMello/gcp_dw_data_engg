...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.icd_oncology_diagnosis_code_xwalk
WHERE upper(icd_oncology_diagnosis_code_xwalk.icd_oncology_code) NOT IN
    (SELECT upper(ref_icd_oncology_diag_cd_stg.icd_oncology_code) AS icd_oncology_code
     FROM `hca-hin-dev-cur-ops`.edwcr_staging.ref_icd_oncology_diag_cd_stg);
...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.icd_oncology_diagnosis_code_xwalk AS mt USING
  (SELECT DISTINCT stg.icd_oncology_code AS icd_oncology_code,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.ref_icd_oncology_diag_cd_stg AS stg
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.icd_oncology_diagnosis_code_xwalk');
