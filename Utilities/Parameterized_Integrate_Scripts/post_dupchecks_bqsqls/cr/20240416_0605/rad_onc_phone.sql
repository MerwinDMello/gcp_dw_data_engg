...
FROM
  (SELECT trim(stg_dimpatient.patienthomephone) AS phone_num_text
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimdoctor) AS src
...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.rad_onc_phone AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.rad_onc_phone_wrk AS ds) AS ms ON mt.phone_num_sk = ms.phone_num_sk
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT phone_num_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.rad_onc_phone
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.rad_onc_phone');
