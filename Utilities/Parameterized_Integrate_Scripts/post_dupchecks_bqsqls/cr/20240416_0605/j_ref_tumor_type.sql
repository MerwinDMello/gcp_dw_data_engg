...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.ref_tumor_type
...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_tumor_type AS mt USING
  (SELECT DISTINCT ref_tumor_type_stg.tumor_type_id,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.ref_tumor_type_stg) AS ms ON mt.tumor_type_id = ms.tumor_type_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_tumor_type');
