...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_hospital AS mt USING
  (SELECT DISTINCT ref_hospital_stg.hospital_id,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.ref_hospital_stg) AS ms ON mt.hospital_id = ms.hospital_id
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT hospital_id
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_hospital
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_hospital');
