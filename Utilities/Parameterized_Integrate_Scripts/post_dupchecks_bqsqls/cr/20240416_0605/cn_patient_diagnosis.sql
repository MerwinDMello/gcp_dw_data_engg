...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_diagnosis AS mt USING
  (SELECT DISTINCT stg.cn_patient_diagnosis_sid,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_diagnosis_stg AS stg
   ...
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_diagnosis.hashbite_ssk) AS hashbite_ssk
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_diagnosis) ) AS ms ON mt.cn_patient_diagnosis_sid = ms.cn_patient_diagnosis_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_diagnosis');
