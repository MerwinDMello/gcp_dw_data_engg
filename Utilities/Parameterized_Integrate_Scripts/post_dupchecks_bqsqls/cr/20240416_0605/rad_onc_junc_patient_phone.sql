...
TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr.rad_onc_junc_patient_phone;
...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.rad_onc_junc_patient_phone AS mt USING
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT patient_sk,
             phone_num_type_code
      FROM `hca-hin-dev-cur-ops`.edwcr.rad_onc_junc_patient_phone
      GROUP BY patient_sk,
               phone_num_type_code
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.rad_onc_junc_patient_phone');
