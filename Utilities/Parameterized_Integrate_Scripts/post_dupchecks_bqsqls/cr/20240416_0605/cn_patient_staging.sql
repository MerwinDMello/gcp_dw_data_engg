...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_staging
  ...
  (SELECT DISTINCT cn_patient_staging_stg.cn_patient_staging_sid,
   ...
   WHERE (upper(cn_patient_staging_stg.hashbite_ssk),
       ...
       (SELECT AS STRUCT upper(cn_patient_staging.hashbite_ssk) AS hashbite_ssk,
        ...
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_staging) ) AS ms ON mt.cn_patient_staging_sid = ms.cn_patient_staging_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_staging');
