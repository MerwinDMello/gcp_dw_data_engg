...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_sample_type AS mt USING
  (SELECT DISTINCT
     (SELECT coalesce(max(ref_sample_type.sample_type_id), 0) AS maxid
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_sample_type) + row_number() OVER (
  ...
  (SELECT count(*)
   FROM
     (SELECT sample_type_id
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_sample_type
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_sample_type');
