...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_test_type AS mt USING
  (SELECT DISTINCT
     (SELECT coalesce(max(ref_test_type.test_type_id), 0)
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_test_type) + row_number() OVER (
   ...
   FROM
     (SELECT r.test_type_desc,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.ref_test_type_stg AS r
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT test_type_id
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_test_type
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_test_type');
