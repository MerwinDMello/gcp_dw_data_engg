...
FROM `hca-hin-dev-cur-ops`.edwcr_staging.cancer_abstraction_values_stg AS stg
...
INNER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.ref_abstraction_measure AS measure ON upper(trim(stg.abstraction_field)) = upper(trim(measure.abstraction_measure_name)) QUALIFY row_number() OVER (PARTITION BY cancer_abstraction_sk,
...
UPDATE `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_abstraction_detail
...
FROM `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_detail_wrk0 AS src
...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_abstraction_detail AS mt USING
  (SELECT DISTINCT stg.cancer_abstraction_sk,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_detail_wrk0 AS stg
  ...
  (SELECT count(*)
   FROM
     (SELECT cancer_abstraction_sk,
             abstraction_measure_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_abstraction_detail
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_abstraction_detail');
