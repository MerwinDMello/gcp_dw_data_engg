  ...
  (SELECT DISTINCT ab.physician_id,
   ...
   FROM
     (SELECT cn_physician_role_stg.physician_id,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr.cn_physician_detail
      INNER JOIN `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme ON cn_physician_detail.physician_id = cn_patient_heme.hematologist_physician_id) AS ab
   WHERE (ab.physician_id,
       ...
       (SELECT AS STRUCT cn_physician_role.physician_id,
        ...
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_physician_role) ) AS ms ON mt.physician_id = ms.physician_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_physician_role');
