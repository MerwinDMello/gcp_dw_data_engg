...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr_staging.scri_patient_id_history
     ...
     (SELECT max(stg.msh_msg_control_id) AS message_control_id_text,
             coalesce(pt.patient_type_status_sk,
                        (SELECT ref_patient_type_status.patient_type_status_sk
                         FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_patient_type_status
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.scri_patient_id_history AS stg
  ...
  (SELECT count(*)
   FROM
     (SELECT message_control_id_text
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_input_wrk
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_input_wrk');
...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_input AS mt USING
  (SELECT DISTINCT cancer_patient_id_input_wrk.message_control_id_text,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_input_wrk) AS ms ON mt.message_control_id_text = ms.message_control_id_text
AND (coalesce(mt.patient_type_status_sk, 0) = coalesce(ms.patient_type_status_sk, 0)
     AND coalesce(mt.patient_type_status_sk, 1) = coalesce(ms.patient_type_status_sk, 1))
AND (upper(coalesce(mt.coid, '0')) = upper(coalesce(ms.coid, '0'))
     AND upper(coalesce(mt.coid, '1')) = upper(coalesce(ms.coid, '1')))
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT message_control_id_text
      FROM `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_input
      GROUP BY message_control_id_text
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_input');
