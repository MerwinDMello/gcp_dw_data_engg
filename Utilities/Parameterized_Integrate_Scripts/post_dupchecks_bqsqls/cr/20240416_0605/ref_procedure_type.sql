...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_procedure_type AS mt USING
  (SELECT DISTINCT row_number() OVER (
     ...
     (SELECT coalesce(max(ref_procedure_type.procedure_type_id), 0) AS id1
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_procedure_type) AS procedure_type_id,
   ...
   FROM
     (SELECT procedure_type_stg.procedure_type_desc,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.procedure_type_stg
      WHERE (upper(trim(procedure_type_stg.procedure_type_desc)),
          ...
          (SELECT AS STRUCT upper(trim(ref_procedure_type.procedure_type_desc)),
           ...
           FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_procedure_type
  ...
  (SELECT count(*)
   FROM
     (SELECT procedure_type_id
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_procedure_type
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_procedure_type');
