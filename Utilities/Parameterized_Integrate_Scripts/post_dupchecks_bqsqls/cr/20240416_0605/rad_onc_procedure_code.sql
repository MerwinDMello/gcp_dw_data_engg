...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_procedure_code AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   INNER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.ref_rad_onc_site AS rr ON dp.dimsiteid = rr.source_site_id) AS ms ON mt.procedure_code_sk = ms.procedure_code_sk
  ...
  (SELECT count(*)
   FROM
     (SELECT procedure_code_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_procedure_code
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_procedure_code');
