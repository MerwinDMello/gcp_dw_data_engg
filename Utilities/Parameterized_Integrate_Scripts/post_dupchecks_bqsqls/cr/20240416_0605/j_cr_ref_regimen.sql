  ...
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(dis.regimen_name)) + coalesce(
                                                                                     (SELECT max(coalesce(a.regimen_id, 0)) AS max_key
                                                                                      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_regimen AS a), 0) AS regimen_id,
   ...
   WHERE NOT EXISTS
       (SELECT 1
        FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_regimen AS rcdc
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT regimen_id
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_regimen
      GROUP BY regimen_id
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_regimen');
