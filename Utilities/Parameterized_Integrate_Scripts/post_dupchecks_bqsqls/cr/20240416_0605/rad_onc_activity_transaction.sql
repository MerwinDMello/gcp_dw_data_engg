  ...
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM
     (SELECT stg_dimactivitytransaction.dimsiteid,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimactivitytransaction) AS dp
   INNER JOIN
     (SELECT ref_rad_onc_site.source_site_id,
             ref_rad_onc_site.site_sk
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_rad_onc_site) AS rr ON CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dp.dimsiteid) AS FLOAT64) = rr.source_site_id
   LEFT OUTER JOIN
     (SELECT ref_rad_onc_activity_priority.activity_priority_sk,
             ref_rad_onc_activity_priority.activity_priority_desc
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_rad_onc_activity_priority) AS ap ON upper(rtrim(dp.activitypriority)) = upper(rtrim(ap.activity_priority_desc))
   LEFT OUTER JOIN
     (SELECT ref_rad_onc_hospital.source_hospital_id,
             ref_rad_onc_hospital.hospital_sk,
             ref_rad_onc_hospital.site_sk
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_rad_onc_hospital) AS rh ON CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dp.dimhospitaldepartmentid) AS FLOAT64) = rh.source_hospital_id
   AND rr.site_sk = rh.site_sk
   LEFT OUTER JOIN
     (SELECT rad_onc_patient.source_patient_id,
             rad_onc_patient.patient_sk,
             rad_onc_patient.site_sk
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.rad_onc_patient) AS rp ON CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dp.dimpatientid) AS FLOAT64) = rp.source_patient_id
   ...
   LEFT OUTER JOIN
     (SELECT ref_rad_onc_activity.source_activity_id,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_rad_onc_activity) AS ra ON CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dp.dimactivityid) AS FLOAT64) = ra.source_activity_id
  ...
  (SELECT count(*)
   FROM
     (SELECT activity_transaction_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.rad_onc_activity_transaction
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.rad_onc_activity_transaction');
