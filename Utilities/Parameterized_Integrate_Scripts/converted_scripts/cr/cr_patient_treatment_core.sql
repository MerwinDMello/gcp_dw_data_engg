...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_treatment AS mt USING (
    SELECT DISTINCT
        cr_patient_treatment_wrk.treatment_id,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_treatment_wrk
  ) AS ms
  ON mt.treatment_id = ms.treatment_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (treatment_id, tumor_id, treatment_hospital_id, treatment_type_id, surgical_site_id, surgical_margin_result_id, treatment_type_group_id, clinical_trial_start_date, treatment_start_date, clinical_trial_text, comment_text, treatment_performing_physician_code, source_system_code, dw_last_update_date_time)
