...
-- ####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_SYSTEM_PRODUCTIVITY_LOG;' FOR SESSION;
  ...
  INSERT INTO `hca-hin-dev-cur-ops`.edwcr_staging.cr_system_productivity_log_wrk (system_productivity_log_id, cr_patient_id, tumor_id, system_user_id_code, system_change_status_date, source_system_code, dw_last_update_date_time)
    SELECT
      ...
      FROM
        (
          SELECT
              stg.system_productivity_log_id,
              stg.cr_patient_id,
              stg.tumor_id,
              stg.system_user_id_code,
              CASE
                WHEN rtrim(substr(trim(format('%11d', stg.system_change_status_date)), 5, 4)) IN(
                  '9999', '0000'
                ) THEN concat(substr(trim(format('%11d', stg.system_change_status_date)), 1, 4), '01', '01')
                WHEN rtrim(substr(trim(format('%11d', stg.system_change_status_date)), 7, 2)) IN(
                  '99', '00'
                ) THEN concat(substr(trim(format('%11d', stg.system_change_status_date)), 1, 6), '01')
                WHEN rtrim(substr(trim(format('%11d', stg.system_change_status_date)), 5, 2)) IN(
                  '99', '00'
                ) THEN concat(substr(trim(format('%11d', stg.system_change_status_date)), 1, 4), '01', substr(trim(format('%11d', stg.system_change_status_date)), 7, 2))
                ELSE trim(format('%11d', stg.system_change_status_date))
              END AS system_change_status_date,
              'M' AS source_system_code,
              datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.cr_system_productivity_log_stg AS stg
        ) AS x
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.sys_calendar AS b ON (CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(substr(x.system_change_status_date, 1, 4)) as FLOAT64) - 1900) * 10000 + CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(substr(x.system_change_status_date, 5, 2)) as FLOAT64) * 100 + CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(substr(x.system_change_status_date, 7, 2)) as FLOAT64) = (extract(YEAR from b.calendar_date) - 1900) * 10000 + extract(MONTH from b.calendar_date) * 100 + extract(DAY from b.calendar_date)
  ;
EXCEPTION WHEN ERROR THEN
  SET _ERROR_CODE = 1;
  SET _ERROR_MSG = @@error.message;
