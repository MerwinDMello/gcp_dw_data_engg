...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_REG_CANCER_PATIENT_DRIVER;' FOR SESSION;
/* Truncate Core Table */
BEGIN
  SET _ERROR_CODE = 0;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cancer_patient_driver AS mt USING (
    SELECT DISTINCT
        CAST(row_number() OVER (ORDER BY met.cr_patient_id, inav.nav_patient_id, cpid.patient_dw_id) as NUMERIC) AS cancer_patient_driver_sk,
      ...
      FROM
        (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.cr_patient AS cp
              LEFT OUTER JOIN (
                SELECT
              ...
              ) AS df ON upper(rtrim(cp.coid)) = upper(rtrim(df.coid))
        ) AS met
        FULL OUTER JOIN (
          SELECT
              ...
              crio.patient_dw_id,
              /* Handled null coid in CRIO */ coalesce(crio.coid, df2.coid) AS coid,
              crio.company_code,
            ...
            FROM
              ...
              (
                SELECT
                    ...
                    clinical_facility.facility_active_ind
                  FROM
                    `hca-hin-dev-cur-ops`.edwcr_views.clinical_facility
              ...
              ) AS df2 ON rtrim(crio.facility_menmonic_cs) = rtrim(df2.facility_mnemonic_cs)
            QUALIFY row_number() OVER (PARTITION BY coalesce(concat(crio.patient_market_urn, crio.network_mnemonic_cs), concat(crio.medical_record_num, facility_mnemonic_cs), format('%#20.0f', crio.patient_dw_id)) ORDER BY crio.report_assigned_date_time DESC) = 1
        ...
        FULL OUTER JOIN (
          SELECT
              ...
              cp.source_system_code
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.cn_person AS cp
              ...
              LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.cn_patient_phone_num AS cpp ON cp.nav_patient_id = cpp.nav_patient_id
            QUALIFY row_number() OVER (PARTITION BY coalesce(cpt.patient_market_urn, concat(cpt.medical_record_num, df2.facility_mnemonic_cs), format('%#20.0f', cp.nav_patient_id)) ORDER BY cp.nav_patient_id DESC) = 1
        ...
        LEFT OUTER JOIN (
          SELECT
            ...
            FROM
              (
                SELECT
                    cr.patient_market_urn,
                    cr.network_mnemonic_cs,
                    e.empi_text
                  FROM
                    `hca-hin-dev-cur-ops`.edwcr_views.clinical_registration AS cr
                  ...
                  WHERE upper(rtrim(e.encounter_id_type_name)) = 'PATIENT_DW_ID'
                   AND CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(e.empi_text) as FLOAT64) > 0
              ) AS a
            QUALIFY row_number() OVER (PARTITION BY upper(a.patient_market_urn), upper(a.network_mnemonic_cs) ORDER BY upper(a.empi_text)) = 1
        ) AS empi ON rtrim(coalesce(met.network_mnemonic_cs, cpid.network_mnemonic_cs, inav.network_mnemonic_cs)) = rtrim(empi.network_mnemonic_cs)
        ...
        CROSS JOIN UNNEST(ARRAY[
          coalesce(met.coid, cpid.coid, inav.coid)
        ]) AS coid1
      WHERE coid1 IS NOT NULL
  ) AS ms
  ON mt.cancer_patient_driver_sk = ms.cancer_patient_driver_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cancer_patient_driver_sk, cr_patient_id, cn_patient_id, cp_patient_id, coid, company_code, network_mnemonic_cs, patient_market_urn_text, medical_record_num, empi_text, patient_first_name, patient_middle_name, patient_last_name, preferred_name, source_system_code, dw_last_update_date_time)
