...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_Patient_Lab_Result;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_lab_result WHERE upper(cn_patient_lab_result.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_lab_result_stg.hashbite_ssk) AS hashbite_ssk
      FROM
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_lab_result AS mt USING (
    SELECT DISTINCT
  ...
  ON mt.nav_patient_lab_result_sid = ms.nav_patient_lab_result_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (nav_patient_lab_result_sid, lab_type_id, core_record_type_id, nav_patient_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, navigator_id, coid, company_code, lab_date, lab_result_text, comment_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
