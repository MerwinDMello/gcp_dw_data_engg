...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_PATIENT_STAGING;' FOR SESSION;
...
BEGIN
  ...
  INSERT INTO `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_staging_wrk (cr_patient_staging_sid, cr_patient_id, tumor_id, ajcc_stage_id, cancer_stage_classification_method_code, cancer_stage_type_code, cancer_stage_result_text, source_system_code, dw_last_update_date_time)
    SELECT
        row_number() OVER (ORDER BY stg.tumorid, stg.patientid DESC) AS cr_patient_staging_sid,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_staging_sample_stage AS stg
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_staging AS mt USING (
    SELECT DISTINCT
        cr_patient_staging_wrk.cr_patient_staging_sid,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_staging_wrk
      GROUP BY 1, 2, 3, 4, upper(cr_patient_staging_wrk.cancer_stage_classification_method_code), upper(cr_patient_staging_wrk.cancer_stage_type_code), upper(cr_patient_staging_wrk.cancer_stage_result_text), upper(cr_patient_staging_wrk.source_system_code), 9
  ) AS ms
  ON mt.cr_patient_staging_sid = ms.cr_patient_staging_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cr_patient_staging_sid, cr_patient_id, tumor_id, ajcc_stage_id, cancer_stage_classification_method_code, cancer_stage_type_code, cancer_stage_result_text, source_system_code, dw_last_update_date_time)
