...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_Ref_Site_Location;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_site_location AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(trim(src.site_location_desc))) + (
          SELECT
              coalesce(max(ref_site_location.site_location_id), 0) AS id1
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_site_location
        ) AS site_location_id,
      ...
      FROM
        (
          SELECT
              ref_site_location_stg.site_location_desc,
              ref_site_location_stg.source_system_code
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.ref_site_location_stg
            WHERE upper(trim(ref_site_location_stg.site_location_desc)) NOT IN(
              SELECT
                  upper(trim(ref_site_location.site_location_desc))
                FROM
                  `hca-hin-dev-cur-ops`.edwcr.ref_site_location
      ...
      WHERE src.site_location_desc IS NOT NULL
       AND upper(rtrim(src.site_location_desc)) <> ''
  ) AS ms
  ON mt.site_location_id = ms.site_location_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (site_location_id, site_location_desc, source_system_code, dw_last_update_date_time)
