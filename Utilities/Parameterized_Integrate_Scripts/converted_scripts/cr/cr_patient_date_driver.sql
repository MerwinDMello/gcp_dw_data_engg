...
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_Patient_Date_Driver;' FOR SESSION;
BEGIN
  SET _ERROR_CODE = 0;
        ...
        FROM
          (
            SELECT
                  ...
                  SELECT
                    ...
                    FROM
                      (
                        ...
                        UNION DISTINCT
                        SELECT
                            t1.cancer_patient_driver_sk AS cancer_patient_driver_sk,
              ...
              GROUP BY 1, 2, 3, 4, 5, 6, upper(info.datesource)
          ) AS emr_date
  ;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_date_driver AS mt USING (
    SELECT DISTINCT
        a.cancer_patient_driver_sk AS cancer_patient_driver_sk,
        a.patient_dw_id,
  ...
  ON mt.cancer_patient_driver_sk = ms.cancer_patient_driver_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cancer_patient_driver_sk, patient_dw_id, cancer_diagnosis_date, cancer_diagnosis_90_day_prior_date, dw_last_update_date_time)
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_date_driver AS mt USING (
    SELECT DISTINCT
        b.cancer_patient_driver_sk AS cancer_patient_driver_sk,
        b.patient_dw_id,
      ...
      FROM
        (
          SELECT
            ...
            WHERE fe.admission_date_time >= sc.hospemr_date_driver
            QUALIFY row_number() OVER (PARTITION BY sc.cancer_patient_driver_sk, fe.patient_dw_id ORDER BY upper(cf.facility_active_ind) DESC) = 1
        ) AS b
      WHERE (b.patient_dw_id, b.cancer_patient_driver_sk) NOT IN(
        SELECT AS STRUCT
          ...
          FROM
            `hca-hin-dev-cur-ops`.edwcr_base_views.cr_patient_date_driver
      )
  ) AS ms
  ON mt.cancer_patient_driver_sk = ms.cancer_patient_driver_sk
   AND mt.patient_dw_id = ms.patient_dw_id
