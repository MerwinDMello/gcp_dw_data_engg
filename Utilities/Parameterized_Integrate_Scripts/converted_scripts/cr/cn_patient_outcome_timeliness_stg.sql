...
-- ####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_Patient_Outcome_Timeliness_STG;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_outcome_timeliness_stg;
  ...
  INSERT INTO `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_outcome_timeliness_stg (cancer_patient_tumor_driver_sk, cancer_patient_driver_sk, cancer_tumor_driver_sk, coid, company_code, length_to_chemo_day_num, length_to_hormone_day_num, length_to_immuno_day_num, length_to_surgery_day_num, length_to_radiation_day_num, length_to_transplant_day_num, length_to_first_treatment_day_num, first_surgery_chemo_elapsed_day_num, first_chemo_surgery_elapsed_day_num, radiation_elapsed_day_num, length_to_surgery_last_contact_day_num, length_to_diagnosis_last_contact_day_num, length_to_diagnosis_last_contact_mth_num, last_contact_date, admission_date, rln10_concordant_ind, rln12_concordant_ind, act_concordant_ind, bcs_concordant_ind, bcsrt_concordant_ind, cbrrt_concordant_ind, cerct_concrodant_ind, cerrt_concrodant_ind, endctrt_concordant_ind, endlrc_concordant_ind, g15rlnc_concordant_ind, lct_concordant_ind, ht_concordant_ind, lnosurg_concordant_ind, mac_concordant_ind, mastrt_concordant_ind, nb_concordant_ind, ovsal_concordant_ind, recrtct_concordance_ind, source_system_code, dw_last_update_date_time)
    SELECT
      ...
      FROM
        ...
        INNER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.cr_patient AS crp ON cpt.cr_patient_id = crp.cr_patient_id
        INNER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.cancer_patient_tumor_driver AS cptd ON cptd.cr_patient_id = cpt.cr_patient_id
         AND cptd.cr_tumor_primary_site_id = cpt.tumor_primary_site_id
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.cr_patient_diagnosis_detail AS cpdd ON cpt.tumor_id = cpdd.tumor_id
         AND cpt.cr_patient_id = cpdd.cr_patient_id
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.rln10_concordant_ind AS rln10 ON cpt.cr_patient_id = rln10.patientid
         AND cpt.tumor_id = rln10.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.rln12_concordant_ind AS rln12 ON cpt.cr_patient_id = rln12.patientid
         AND cpt.tumor_id = rln12.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.act_concordant_ind AS act ON cpt.cr_patient_id = act.patientid
         AND cpt.tumor_id = act.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.bcs_concordant_ind AS bcs ON cpt.cr_patient_id = bcs.patientid
         AND cpt.tumor_id = bcs.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.bcsrt_concordant_ind AS bcsrt ON cpt.cr_patient_id = bcsrt.patientid
         AND cpt.tumor_id = bcsrt.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.cbrrt_concordant_ind AS cbrrt ON cpt.cr_patient_id = cbrrt.patientid
         AND cpt.tumor_id = cbrrt.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.cerct_concrodant_ind AS cerct ON cpt.cr_patient_id = cerct.patientid
         AND cpt.tumor_id = cerct.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.cerrt_concrodant_ind AS cerrt ON cpt.cr_patient_id = cerrt.patientid
         AND cpt.tumor_id = cerrt.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.endctrt_concordant_ind AS endctrt ON cpt.cr_patient_id = endctrt.patientid
         AND cpt.tumor_id = endctrt.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.endlrc_concordant_ind AS endlrc ON cpt.cr_patient_id = endlrc.patientid
         AND cpt.tumor_id = endlrc.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.g15rln_concordant_ind AS g15rln ON cpt.cr_patient_id = g15rln.patientid
         AND cpt.tumor_id = g15rln.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.lct_concordant_ind AS lct ON cpt.cr_patient_id = lct.patientid
         AND cpt.tumor_id = lct.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.ht_concordant_ind AS ht ON cpt.cr_patient_id = ht.patientid
         AND cpt.tumor_id = ht.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.lnosurg_concordant_ind AS lnosurg ON cpt.cr_patient_id = lnosurg.patientid
         AND cpt.tumor_id = lnosurg.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.mac_concordant_ind AS mac ON cpt.cr_patient_id = mac.patientid
         AND cpt.tumor_id = mac.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.mastrt_concordant_ind AS mastrt ON cpt.cr_patient_id = mastrt.patientid
         AND cpt.tumor_id = mastrt.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.nb_concordant_ind AS nb ON cpt.cr_patient_id = nb.patientid
         AND cpt.tumor_id = nb.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.ovsal_concordant_ind AS ovsal ON cpt.cr_patient_id = ovsal.patientid
         AND cpt.tumor_id = ovsal.tumorid
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.recrtct_concordant_ind AS recrtct ON cpt.cr_patient_id = recrtct.patientid
         AND cpt.tumor_id = recrtct.tumorid
      QUALIFY row_number() OVER (PARTITION BY cpt.cr_patient_id, cpt.tumor_primary_site_id ORDER BY cpt.tumor_id DESC) = 1
  ;
EXCEPTION WHEN ERROR THEN
  SET _ERROR_CODE = 1;
  SET _ERROR_MSG = @@error.message;
