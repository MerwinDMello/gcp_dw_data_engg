...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_REF_Rad_Onc_Physician;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.rad_onc_physician AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dhd.dimsiteid) as INT64), CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dhd.dimdoctorid) as INT64)) AS physician_sk,
        ...
        CASE
           upper(trim(dhd.resourceobjectstatus))
          WHEN 'ACTIVE' THEN 'Y'
          WHEN 'DELETED' THEN 'N'
          ELSE 'U'
        END AS resource_active_ind,
        CASE
           upper(trim(dhd.schedulable))
          WHEN 'YES' THEN 'Y'
          WHEN 'NO' THEN 'N'
          ELSE 'U'
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimdoctor AS dhd
        INNER JOIN (
          SELECT
        ...
        ) AS rr ON rr.source_site_id = CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dhd.dimsiteid) as FLOAT64)
  ...
  ON mt.physician_sk = ms.physician_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (physician_sk, site_sk, source_physician_id, location_sk, resource_type_id, physician_first_name, physician_last_name, physician_suffix_name, physician_alias_name, physician_title_name, physician_email_address_text, physician_specialty_text, physician_id_text, resource_active_ind, appointment_schedule_ind, physician_start_date_time, physician_termination_date_time, physician_institution_text, physician_comment_text, resource_id, log_id, run_id, source_system_code, dw_last_update_date_time)
