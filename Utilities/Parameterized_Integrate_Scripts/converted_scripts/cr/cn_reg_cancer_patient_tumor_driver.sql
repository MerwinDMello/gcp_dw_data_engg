...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cancer_patient_tumor_driver AS mt USING (
    SELECT DISTINCT
        CAST(row_number() OVER (ORDER BY a.cancer_patient_driver_sk, a.t_sk, a.cr_patient_id, a.cn_patient_id, a.cp_patient_id, a.cr_tumor_primary_site_id, a.cn_tumor_type_id, upper(a.cp_icd_oncology_code)) as NUMERIC) AS cancer_patient_tumor_driver_sk,
        a.coid AS coid,
  ...
  ON mt.cancer_patient_tumor_driver_sk = ms.cancer_patient_tumor_driver_sk
   AND mt.coid = ms.coid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cancer_patient_tumor_driver_sk, coid, company_code, cancer_patient_driver_sk, cancer_tumor_driver_sk, cr_patient_id, cr_tumor_primary_site_id, cn_patient_id, cn_tumor_type_id, cp_patient_id, cp_icd_oncology_code, source_system_code, dw_last_update_date_time)
