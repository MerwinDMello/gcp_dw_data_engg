...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_Cancer_Patient_Id_Abstraction;' FOR SESSION;
...
BEGIN
  ...
  INSERT INTO `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_wrk0 (cancer_abstraction_sk, cancer_patient_id_output_sk, message_control_id_text, coid, company_code, abstraction_action_date_time, patient_dw_id, pat_acct_num, abstraction_report_assigned_date_time, abstraction_date_time, abstraction_action_user_3_4_id, abstraction_action_desc, primary_icd_oncology_code, primary_icd_site_desc, primary_icd_site_and_model_score_desc, changed_primary_icd_oncology_code, changed_primary_icd_site_desc, topography_icd_oncology_code, topography_icd_site_desc, laterality_icd_oncology_code, laterality_icd_site_desc, secondary_icd_oncology_code, secondary_icd_site_desc, source_system_code, dw_last_update_date_time)
    SELECT
        NULL AS cancer_abstraction_sk,
        /* nvl(ROW_NUMBER() OVER ( ORDER BY unique_message_id || ' - ' ||COALESCE(site_and_associated_model_output_score,'' )||COALESCE(second_primary_site,'') ),0)as Cancer_Abstraction_SK ------Added for CES 17570 */
        coalesce(o.cancer_patient_id_output_sk, -1) AS cancer_patient_id_output_sk,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cancer_abstraction_stg AS a
        ...
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.clinical_acctkeys AS cact ON ROUND(CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(translate(a.patient_account_number, translate(a.patient_account_number, '0123456789', ''), '')) as NUMERIC), 0, 'ROUND_HALF_EVEN') = cact.pat_acct_num
         ...
         AND upper(rtrim(cact.company_code)) = 'H'
      QUALIFY /* QUALIFY ROW_NUMBER() OVER(partition by unique_message_id ORDER BY CAST(CAST(OREPLACE(A.abstraction_action_date_time, 'T', ' ') AS VARCHAR(19)) AS TIMESTAMP(0) FORMAT 'YYYY-MM-DDBHH:MI:SS') DESC) = 1; */ row_number() OVER (PARTITION BY upper(a.unique_message_id), upper(a.site_and_associated_model_output_score), upper(a.second_primary_site) ORDER BY abstraction_action_date_time DESC) = 1
  ;
EXCEPTION WHEN ERROR THEN
  SET _ERROR_CODE = 1;
  SET _ERROR_MSG = @@error.message;
...
EXCEPTION WHEN ERROR THEN
  ...
  SET _ERROR_MSG = @@error.message;
END;
/*Generate SK for new record*/ IF _ERROR_CODE <> 0 THEN
  RAISE USING MESSAGE = concat('Script terminated with return code: ', _ERROR_CODE, ' ', _ERROR_MSG);
END IF;
BEGIN
  SET _ERROR_CODE = 0;
  INSERT INTO `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_wrk1 (cancer_abstraction_sk, cancer_patient_id_output_sk, message_control_id_text, coid, company_code, abstraction_action_date_time, patient_dw_id, pat_acct_num, abstraction_report_assigned_date_time, abstraction_date_time, abstraction_action_user_3_4_id, abstraction_action_desc, primary_icd_oncology_code, primary_icd_site_desc, primary_icd_site_and_model_score_desc, changed_primary_icd_oncology_code, changed_primary_icd_site_desc, topography_icd_oncology_code, topography_icd_site_desc, laterality_icd_oncology_code, laterality_icd_site_desc, secondary_icd_oncology_code, secondary_icd_site_desc, source_system_code, dw_last_update_date_time)
    SELECT
        -- -nvl(ROW_NUMBER() OVER ( ORDER BY wr.Message_Control_Id_Text ASC) +
        coalesce(row_number() OVER (ORDER BY upper(wr.message_control_id_text), upper(wr.secondary_icd_site_desc), upper(wr.primary_icd_site_and_model_score_desc)) + (
          SELECT
              coalesce(max(cancer_patient_id_abstraction.cancer_abstraction_sk), 0) AS max_val
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.cancer_patient_id_abstraction
        ), 0) AS cancer_abstraction_sk,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_wrk0 AS wr
...
BEGIN
  ...
  INSERT INTO `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_wrk2 (cancer_abstraction_sk, cancer_patient_id_output_sk, message_control_id_text, coid, company_code, abstraction_action_date_time, patient_dw_id, pat_acct_num, abstraction_report_assigned_date_time, abstraction_date_time, abstraction_action_user_3_4_id, abstraction_action_desc, primary_icd_oncology_code, primary_icd_site_desc, primary_icd_site_and_model_score_desc, changed_primary_icd_oncology_code, changed_primary_icd_site_desc, topography_icd_oncology_code, topography_icd_site_desc, laterality_icd_oncology_code, laterality_icd_site_desc, secondary_icd_oncology_code, secondary_icd_site_desc, source_system_code, dw_last_update_date_time)
    SELECT
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_wrk1 AS stg
      QUALIFY row_number() OVER (PARTITION BY stg.cancer_abstraction_sk ORDER BY stg.abstraction_action_date_time DESC) = 1
  ;
EXCEPTION WHEN ERROR THEN
  SET _ERROR_CODE = 1;
  SET _ERROR_MSG = @@error.message;
...
BEGIN
  SET _ERROR_CODE = 0;
  UPDATE `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_abstraction SET cancer_patient_id_output_sk = src.cancer_patient_id_output_sk, message_control_id_text = src.message_control_id_text, coid = src.coid, company_code = src.company_code, abstraction_action_date_time = src.abstraction_action_date_time, patient_dw_id = src.patient_dw_id, pat_acct_num = src.pat_acct_num, abstraction_report_assigned_date_time = src.abstraction_report_assigned_date_time, abstraction_date_time = src.abstraction_date_time, abstraction_action_user_3_4_id = src.abstraction_action_user_3_4_id, abstraction_action_desc = src.abstraction_action_desc, primary_icd_oncology_code = src.primary_icd_oncology_code, primary_icd_site_desc = src.primary_icd_site_desc, primary_icd_site_and_model_score_desc = src.primary_icd_site_and_model_score_desc, changed_primary_icd_oncology_code = src.changed_primary_icd_oncology_code, changed_primary_icd_site_desc = src.changed_primary_icd_site_desc, topography_icd_oncology_code = src.topography_icd_oncology_code, topography_icd_site_desc = src.topography_icd_site_desc, laterality_icd_oncology_code = src.laterality_icd_oncology_code, laterality_icd_site_desc = src.laterality_icd_site_desc, secondary_icd_oncology_code = src.secondary_icd_oncology_code, secondary_icd_site_desc = src.secondary_icd_site_desc, source_system_code = src.source_system_code, dw_last_update_date_time = src.dw_last_update_date_time FROM `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_wrk2 AS src WHERE cancer_patient_id_abstraction.cancer_abstraction_sk = src.cancer_abstraction_sk;
EXCEPTION WHEN ERROR THEN
  SET _ERROR_CODE = 1;
  SET _ERROR_MSG = @@error.message;
END;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_abstraction AS mt USING (
    SELECT DISTINCT
        ...
        stg.dw_last_update_date_time
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_wrk2 AS stg
      ...
      WHERE tgt.cancer_abstraction_sk IS NULL
  ) AS ms
  ON mt.cancer_abstraction_sk = ms.cancer_abstraction_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cancer_abstraction_sk, cancer_patient_id_output_sk, message_control_id_text, coid, company_code, abstraction_action_date_time, patient_dw_id, pat_acct_num, abstraction_report_assigned_date_time, abstraction_date_time, abstraction_action_user_3_4_id, abstraction_action_desc, primary_icd_oncology_code, primary_icd_site_desc, primary_icd_site_and_model_score_desc, changed_primary_icd_oncology_code, changed_primary_icd_site_desc, topography_icd_oncology_code, topography_icd_site_desc, laterality_icd_oncology_code, laterality_icd_site_desc, secondary_icd_oncology_code, secondary_icd_site_desc, source_system_code, dw_last_update_date_time)
