...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_Rad_Onc_Junc_Physician_Address;' FOR SESSION;
      ...
      FROM
        (
          SELECT
              ...
              CASE
                 CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dhd.isprimarydoctoraddress) as INT64)
                WHEN 1 THEN 'Y'
                WHEN 0 THEN 'N'
            ...
            FROM
              ...
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.rad_onc_physician AS pt ON rr.site_sk = pt.site_sk
               AND CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dhd.dimdoctorid) as INT64) = pt.source_physician_id
        ) AS stg
  ) AS ms
  ON mt.physician_sk = ms.physician_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (physician_sk, address_type_code, address_sk, primary_address_ind, source_system_code, dw_last_update_date_time)
