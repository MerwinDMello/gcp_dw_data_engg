...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_REF_TREATMENT_PHASE;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_treatment_phase AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(ref_treatment_phase)) + coalesce((
          SELECT
              max(coalesce(a.treatment_phase_id, 0)) AS max_key
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.ref_treatment_phase AS a
        ), 0) AS treatment_phase_id,
      ...
      FROM
        (
          SELECT DISTINCT
              trim(st.treatmentphase) AS treatment_phase_desc
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.patient_heme_treatment_regimen_stg AS st
            WHERE st.treatmentphase IS NOT NULL
        ) AS dis
        CROSS JOIN UNNEST(ARRAY[
          substr(trim(dis.treatment_phase_desc), 1, 50)
        ]) AS ref_treatment_phase
      WHERE NOT EXISTS (
        SELECT
            1
          FROM
            `hca-hin-dev-cur-ops`.edwcr_base_views.ref_treatment_phase AS rcdc
          WHERE upper(trim(rcdc.treatment_phase_desc)) = upper(trim(dis.treatment_phase_desc))
      )
  ) AS ms
  ON mt.treatment_phase_id = ms.treatment_phase_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (treatment_phase_id, treatment_phase_desc, source_system_code, dw_last_update_date_time)
