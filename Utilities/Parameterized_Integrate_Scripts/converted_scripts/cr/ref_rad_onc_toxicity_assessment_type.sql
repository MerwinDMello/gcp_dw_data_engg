...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_RAD_ONC_PLAN_PURPOSE;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_toxicity_assessment_type;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_toxicity_assessment_type AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(src.toxicitycomponentname)) AS toxicity_assessment_type_sk,
      ...
      FROM
        (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.stg_factpatienttoxicity AS dhd
            QUALIFY row_number() OVER (PARTITION BY upper(toxicity_assessment_type_desc) ORDER BY upper(toxicity_assessment_type_desc)) = 1
        ) AS src
  ) AS ms
  ON mt.toxicity_assessment_type_sk = ms.toxicity_assessment_type_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (toxicity_assessment_type_sk, toxicity_assessment_type_desc, source_system_code, dw_last_update_date_time)
