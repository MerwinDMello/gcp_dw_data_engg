...
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr.cr_patient_contact;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_contact AS mt USING (
    SELECT DISTINCT
        cr_patient_contact_wrk.patient_contact_id,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_contact_wrk
  ) AS ms
  ON mt.patient_contact_id = ms.patient_contact_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (patient_contact_id, cr_patient_id, contact_relation_id, contact_type_id, contact_num_code, contact_first_name, contact_last_name, contact_middle_name, preferred_contact_method_text, source_system_code, dw_last_update_date_time)
