...
-- #########################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_PATIENT_MEDICAL_ONCOLOGY;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr.cr_patient_medical_oncology;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_medical_oncology AS mt USING (
    SELECT DISTINCT
        cr_patient_medical_oncology_wrk.cycle_id,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_medical_oncology_wrk
  ) AS ms
  ON coalesce(mt.cycle_id, 0) = coalesce(ms.cycle_id, 0)
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cycle_id, treatment_id, drug_route_id, drug_dose_unit_id, drug_id, drug_days_given_num_text, drug_frequency_num, total_drug_dose_amt, drug_hospital_id, nsc_id, treatment_start_date, treatment_end_date, cycle_num_text, source_system_code, dw_last_update_date_time)
