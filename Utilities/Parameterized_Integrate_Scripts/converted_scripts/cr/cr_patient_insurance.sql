...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_PATIENT_INSURANCE;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_insurance AS mt USING (
    SELECT DISTINCT
        ...
        )) AS insurance_type_id,
        lkp1.master_lookup_sid AS insurance_company_id,
        'M' AS source_system_code,
        datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_insurance_stg AS stg
        LEFT OUTER JOIN (
          SELECT
              rln.lookup_sid,
              rlc.lookup_code,
            ...
            FROM
              ...
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
            WHERE upper(rtrim(rln.lookup_name)) = 'INSURANCE TYPE'
        ) AS lkp ON upper(rtrim(stg.primpayerdx)) = upper(rtrim(lkp.lookup_code))
        LEFT OUTER JOIN (
          SELECT
              rln.lookup_sid,
              rlc.lookup_desc,
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_name AS rln
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
            WHERE upper(rtrim(rln.lookup_name)) = 'INSURANCE COMPANY'
        ) AS lkp1 ON upper(rtrim(CASE
          WHEN stg.txtpaysource IS NULL
           OR upper(rtrim(stg.txtpaysource)) = '' THEN 'Unknown Description'
          ELSE stg.txtpaysource
        END)) = upper(rtrim(lkp1.lookup_desc))
  ) AS ms
  ON mt.tumor_id = ms.tumor_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (tumor_id, cr_patient_id, insurance_type_id, insurance_company_id, source_system_code, dw_last_update_date_time)
