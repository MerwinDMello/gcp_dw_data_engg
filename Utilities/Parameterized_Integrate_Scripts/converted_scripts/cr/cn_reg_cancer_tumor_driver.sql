...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_REG_CANCER_TUMOR_DRIVER;' FOR SESSION;
BEGIN
  SET _ERROR_CODE = 0;
  CREATE TEMPORARY TABLE cancer_tumor_driver
...
BEGIN
  ...
  ON mt.cancer_tumor_driver_sk = ms.cancer_tumor_driver_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cancer_tumor_driver_sk, cp_icd_oncology_code, cp_icd_oncology_site_desc, cp_icd_oncology_group_name, cr_tumor_primary_site_id, cn_tumor_type_id, cn_general_tumor_type_id, cn_navque_tumor_type_id, cr_icd_oncology_code, cr_icd_oncology_site_desc, cn_tumor_group_name, cn_tumor_type_desc, cn_general_tumor_group_name, cn_general_tumor_type_desc, cn_navque_tumor_type_desc, source_system_code, dw_last_update_date_time)
