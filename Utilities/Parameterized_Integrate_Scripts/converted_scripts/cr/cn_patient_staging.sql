...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_STAGING;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_staging WHERE (upper(cn_patient_staging.hashbite_ssk), upper(cn_patient_staging.cancer_stage_class_method_code)) NOT IN(
    SELECT AS STRUCT
        upper(cn_patient_staging_stg.hashbite_ssk) AS hashbite_ssk,
        upper(cn_patient_staging_stg.cancer_stage_class_method_code) AS cancer_stage_class_method_code
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_staging_stg
  );
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_staging AS mt USING (
    SELECT DISTINCT
        cn_patient_staging_stg.cn_patient_staging_sid,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_staging_stg
      WHERE (upper(cn_patient_staging_stg.hashbite_ssk), upper(cn_patient_staging_stg.cancer_stage_class_method_code)) NOT IN(
        SELECT AS STRUCT
            upper(cn_patient_staging.hashbite_ssk) AS hashbite_ssk,
            upper(cn_patient_staging.cancer_stage_class_method_code) AS cancer_stage_class_method_code
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_staging
      )
  ) AS ms
  ON mt.cn_patient_staging_sid = ms.cn_patient_staging_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_staging_sid, diagnosis_result_id, nav_patient_id, tumor_type_id, nav_diagnosis_id, navigator_id, coid, company_code, cancer_stage_class_method_code, cancer_staging_type_code, cancer_staging_result_code, cancer_stage_code, hashbite_ssk, source_system_code, dw_last_update_date_time)
