...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_Patient_Clinical_Trial;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_clinical_trial WHERE upper(cn_patient_clinical_trial.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_clinical_trial_stg.hashbite_ssk) AS hashbite_ssk
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_clinical_trial_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_clinical_trial AS mt USING (
    SELECT DISTINCT
        ...
        cn_patient_clinical_trial_stg.cn_patient_clinical_trial_sid,
        ...
        /*,case when Clinical_Trial_Evaluated_Ind='True' then 'Y'
      ...
      WHERE upper(cn_patient_clinical_trial_stg.hashbite_ssk) NOT IN(
        SELECT
            upper(cn_patient_clinical_trial.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_clinical_trial
      )
  ) AS ms
  ON mt.cn_patient_clinical_trial_sid = ms.cn_patient_clinical_trial_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_clinical_trial_sid, nav_patient_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, navigator_id, coid, company_code, clinical_trial_name, clinical_trial_enrolled_ind, clinical_trial_enrolled_date, clinical_trial_offered_ind, clinical_trial_offered_date, hashbite_ssk, source_system_code, dw_last_update_date_time)
