...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_Patient_Contact;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_contact WHERE upper(cn_patient_contact.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_contact_stg.hashbite_ssk) AS hashbite_ssk
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_contact_stg
  );
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_contact AS mt USING (
    SELECT DISTINCT
        cpstg.cn_patient_contact_sid,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_contact_stg AS cpstg
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_contact_purpose AS rcp ON upper(rtrim(cpstg.purpose_of_contact)) = upper(rtrim(rcp.contact_purpose_desc))
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_contact_person AS cpe ON upper(rtrim(cpstg.person_of_contact)) = upper(rtrim(cpe.contact_person_desc))
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_contact_method AS rcm ON upper(rtrim(cpstg.method_of_contact)) = upper(rtrim(rcm.contact_method_desc))
      WHERE upper(cpstg.hashbite_ssk) NOT IN(
        SELECT
            upper(cn_patient_contact.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_contact
      )
  ) AS ms
  ON mt.cn_patient_contact_sid = ms.cn_patient_contact_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_contact_sid, contact_purpose_id, contact_method_id, contact_person_id, nav_patient_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, navigator_id, coid, company_code, contact_date, other_purpose_detail_text, other_person_contacted_text, time_spent_amount_text, comment_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
