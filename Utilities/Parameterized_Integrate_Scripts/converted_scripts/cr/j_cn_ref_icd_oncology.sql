...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_Ref_ICD_Oncology;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.ref_icd_oncology WHERE upper(ref_icd_oncology.icd_oncology_code) NOT IN(
    SELECT
        upper(ref_icd_oncology_stg.icd_oncology_code) AS icd_oncology_code
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.ref_icd_oncology_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_icd_oncology AS mt USING (
    SELECT DISTINCT
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.ref_icd_oncology_stg AS stg
  ...
  ON mt.icd_oncology_code = ms.icd_oncology_code
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (icd_oncology_code, icd_oncology_type_code, icd_oncology_category_type_code, icd_oncology_site_desc, source_system_code, dw_last_update_date_time)
