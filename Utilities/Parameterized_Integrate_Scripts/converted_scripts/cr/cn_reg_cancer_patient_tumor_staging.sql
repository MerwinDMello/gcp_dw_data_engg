...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_REG_CANCER_PATIENT_TUMOR_DRIVER;' FOR SESSION;
/* Truncate Core Table */
BEGIN
  SET _ERROR_CODE = 0;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cancer_patient_tumor_staging AS mt USING (
    SELECT DISTINCT
        CAST(row_number() OVER (ORDER BY a.cancer_patient_tumor_driver_sk, a.cancer_patient_driver_sk, a.cancer_tumor_driver_sk, upper(a.cancer_stage_class_method_code), upper(cancer_stage_type_code)) as NUMERIC) AS cancer_patient_tumor_staging_sk,
  ...
  ON mt.cancer_patient_tumor_staging_sk = ms.cancer_patient_tumor_staging_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cancer_patient_tumor_staging_sk, cancer_patient_tumor_driver_sk, cancer_patient_driver_sk, cancer_tumor_driver_sk, coid, company_code, best_cs_summary_desc, best_cs_tnm_desc, tumor_size_num_text, cancer_stage_code, cancer_stage_class_method_code, cancer_stage_type_code, cancer_stage_result_text, ajcc_stage_desc, tumor_size_summary_desc, source_system_code, dw_last_update_date_time)
