...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_phone_num WHERE cn_patient_phone_num.nav_patient_id NOT IN(
    SELECT
  ...
  ON mt.nav_patient_id = ms.nav_patient_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (nav_patient_id, phone_num_type_code, phone_num, source_system_code, dw_last_update_date_time)
