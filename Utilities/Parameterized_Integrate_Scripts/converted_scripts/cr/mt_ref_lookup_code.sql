...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_REF_LOOKUP_CODE;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY p.lookup_id) AS master_lookup_sid,
      ...
      FROM
        (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.ref_lookup_code_stg
            GROUP BY 1, upper(ref_lookup_code_stg.lookup_code), upper(ref_lookup_code_stg.lookup_sub_code), upper(ref_lookup_code_stg.lookup_desc), upper(ref_lookup_code_stg.source_system_code), 6
        ) AS p
      WHERE p.lookup_id IS NOT NULL
  ) AS ms
  ON mt.master_lookup_sid = ms.master_lookup_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (master_lookup_sid, lookup_id, lookup_code, lookup_sub_code, lookup_desc, source_system_code, dw_last_update_date_time)
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(max(ref_lookup_code_stg.lookup_code))) + (
          SELECT
              coalesce(max(ref_lookup_code.master_lookup_sid), 0)
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code
        ) AS master_lookup_sid,
        41 AS lookup_id,
      ...
      GROUP BY 2, upper(substr(ref_lookup_code_stg.lookup_code, 1, 100)), upper(ref_lookup_code_stg.lookup_sub_code), upper(substr(ref_lookup_code_stg.lookup_desc, 1, 500)), upper(ref_lookup_code_stg.source_system_code), 7
  ) AS ms
  ON mt.master_lookup_sid = ms.master_lookup_sid
   AND mt.lookup_id = ms.lookup_id
   AND mt.lookup_code = ms.lookup_code
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(o.lookup_code)) + (
          SELECT
              coalesce(max(ref_lookup_code.master_lookup_sid), 0)
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code
      ...
      FROM
        (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.ref_lookup_groups_stg AS lg
            ...
            GROUP BY 1, upper(lc.lookup_code), upper(CAST(lg.group_id as STRING)), upper(lc.lookup_desc)
          UNION ALL
          SELECT
              35 AS lookup_id,
              max(lc.lookup_code) AS lookup_code,
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY stg.lookup_sid) + (
          SELECT
              coalesce(max(ref_lookup_code.master_lookup_sid), 0)
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code
      ...
      FROM
        (
          SELECT DISTINCT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_name
        ) AS stg
        INNER JOIN (
          SELECT
              '-99' AS lookup_code,
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY a.lookup_code) + (
          SELECT
              coalesce(max(ref_lookup_code.master_lookup_sid), 0)
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code
      ...
      FROM
        (
          SELECT DISTINCT
              ...
              END AS lookup_desc
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.ref_lookup_code_stg AS t1
            WHERE t1.lookup_id = 4005.0
             AND t1.group_id = 290
             AND t1.lookup_desc IS NOT NULL
        ) AS a
  ) AS ms
  ON mt.master_lookup_sid = ms.master_lookup_sid
   AND mt.lookup_id = ms.lookup_id
