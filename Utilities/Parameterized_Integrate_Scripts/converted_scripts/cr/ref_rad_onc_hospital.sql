...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_REF_RAD_ONC_HOSPITAL;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_hospital AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dhd.dimsiteid) as INT64), CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dhd.dimhospitaldepartmentid) as INT64)) AS hospital_sk,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimhospitaldepartment AS dhd
        ...
        LEFT OUTER JOIN (
          SELECT DISTINCT
              stg_sc_factfacility.hospitalname,
              stg_sc_factfacility.coid
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.stg_sc_factfacility
            WHERE upper(trim(stg_sc_factfacility.hospitalname)) <> ''
        ) AS ff ON upper(trim(dhd.hospitalname)) = upper(trim(ff.hospitalname))
  ) AS ms
  ON mt.hospital_sk = ms.hospital_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (hospital_sk, hospital_address_sk, site_sk, source_hospital_id, coid, company_code, hospital_name, hospital_email_address_text, hospital_history_user_name, hospital_history_date_time, log_id, run_id, source_system_code, dw_last_update_date_time)
