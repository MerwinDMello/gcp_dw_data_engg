...
IF _ERROR_CODE <> 0 THEN
  RAISE USING MESSAGE = concat('Script terminated with return code: ', _ERROR_CODE, ' ', _ERROR_MSG);
END IF;
/* Populate the WRK table with redcords for which Physician_ID is NOT NULL (This loads the data from DimMedicalSpecialist Table) */
BEGIN
  SET _ERROR_CODE = 0;
...
IF _ERROR_CODE <> 0 THEN
  RAISE USING MESSAGE = concat('Script terminated with return code: ', _ERROR_CODE, ' ', _ERROR_MSG);
END IF;
/* Populate the WRK table with unique redcords for which Physician_ID is NULL */ /* This will insert the unique records. eg. If there are two records with phone Number NULL then it will insert only one record. */
BEGIN
  SET _ERROR_CODE = 0;
  INSERT INTO `hca-hin-dev-cur-ops`.edwcr_staging.cn_physician_detail_stg_wrk (physician_id, physician_name, physician_phone_num, dw_last_update_date_time)
    SELECT
      ...
      WHERE cn_physician_detail_stg.physician_id IS NULL
       ...
       OR cn_physician_detail_stg.physician_phone_num IS NOT NULL)
      QUALIFY row_number() OVER (PARTITION BY upper(trim(cn_physician_detail_stg.physician_name)), upper(trim(cn_physician_detail_stg.physician_phone_num)) ORDER BY upper(cn_physician_detail_stg.physician_phone_num) DESC) = 1
  ;
EXCEPTION WHEN ERROR THEN
  SET _ERROR_CODE = 1;
  SET _ERROR_MSG = @@error.message;
