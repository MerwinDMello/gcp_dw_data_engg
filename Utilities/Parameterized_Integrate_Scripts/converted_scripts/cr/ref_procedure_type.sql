...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_REF_PROCEDURE_TYPE;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_procedure_type AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(trim(type_stg.procedure_type_desc))) + (
          SELECT
              coalesce(max(ref_procedure_type.procedure_type_id), 0) AS id1
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.ref_procedure_type
        ) AS procedure_type_id,
      ...
      WHERE type_stg.procedure_type_desc IS NOT NULL
       AND upper(trim(type_stg.procedure_type_desc)) <> ''
  ) AS ms
  ON mt.procedure_type_id = ms.procedure_type_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (procedure_type_id, procedure_type_desc, procedure_sub_type_desc, source_system_code, dw_last_update_date_time)
