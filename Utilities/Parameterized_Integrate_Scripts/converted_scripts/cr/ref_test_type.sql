...
-- #####################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_REF_TEST_TYPE;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_test_type AS mt USING (
    SELECT DISTINCT
        (
          SELECT
              coalesce(max(ref_test_type.test_type_id), 0)
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_test_type
        ) + row_number() OVER (ORDER BY upper(a.test_type_desc), upper(a.test_sub_type_desc)) AS test_type_id,
        a.test_type_desc,
      ...
      FROM
        (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.ref_test_type_stg AS r
              LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_test_type AS rtt ON upper(rtrim(coalesce(trim(r.test_sub_type_desc), '##'))) = upper(rtrim(coalesce(trim(rtt.test_sub_type_desc), '##')))
            ...
            WHERE rtt.test_type_id IS NULL
             AND r.test_sub_type_desc IS NOT NULL
        ) AS a
  ) AS ms
  ON mt.test_type_id = ms.test_type_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (test_type_id, test_type_desc, test_sub_type_desc, source_system_code, dw_last_update_date_time)
