...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_TUMOR;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_tumor WHERE upper(cn_patient_tumor.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_tumor_stg.hashbite_ssk) AS hashbite_ssk
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_tumor_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_tumor AS mt USING (
    SELECT DISTINCT
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_tumor_stg AS stg
      ...
      WHERE upper(stg.hashbite_ssk) NOT IN(
        SELECT
            upper(cn_patient_tumor.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_tumor
      )
      QUALIFY row_number() OVER (PARTITION BY stg.cn_patient_tumor_sid ORDER BY treatment_end_physician_id DESC) = 1
  ) AS ms
  ON mt.cn_patient_tumor_sid = ms.cn_patient_tumor_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_tumor_sid, nav_patient_id, tumor_type_id, navigator_id, coid, company_code, electronic_folder_id_text, referral_source_facility_id, nav_status_id, identification_period_text, referral_date, referring_physician_id, nav_end_reason_text, treatment_end_reason_text, treatment_end_physician_id, treatment_end_facility_id, hashbite_ssk, source_system_code, dw_last_update_date_time)
