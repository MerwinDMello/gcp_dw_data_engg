...
BEGIN
  ...
  INSERT INTO `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_medical_oncology_wrk (cycle_id, treatment_id, drug_route_id, drug_dose_unit_id, drug_id, drug_days_given_num_text, drug_frequency_num, total_drug_dose_amt, drug_hospital_id, nsc_id, treatment_start_date, treatment_end_date, cycle_num_text, source_system_code, dw_last_update_date_time)
    SELECT DISTINCT
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_medical_oncology_stg AS stg
        LEFT OUTER JOIN (
          SELECT
              refc.master_lookup_sid,
              refc.lookup_code
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS refc
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_lookup_name AS refn ON refc.lookup_id = refn.lookup_sid
               AND upper(rtrim(refn.lookup_name)) = 'DRUG ROUTE'
        ) AS route ON upper(rtrim(route.lookup_code)) = upper(rtrim(stg.drug_route_id))
        LEFT OUTER JOIN (
          SELECT
              refc.master_lookup_sid,
              refc.lookup_code
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS refc
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_lookup_name AS refn ON refc.lookup_id = refn.lookup_sid
               AND upper(rtrim(refn.lookup_name)) = 'DOSE UNITS'
        ) AS dose ON upper(rtrim(dose.lookup_code)) = upper(rtrim(stg.drug_dose_unit_id))
