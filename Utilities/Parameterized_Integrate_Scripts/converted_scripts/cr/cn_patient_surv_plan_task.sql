...
-- #####################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_SURV_PLAN_TASK;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_survivorship_plan_task WHERE upper(cn_patient_survivorship_plan_task.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_survivorship_plan_task_stg.hashbite_ssk) AS hashbite_ssk
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_survivorship_plan_task_stg
  );
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_survivorship_plan_task AS mt USING (
    SELECT DISTINCT
  ...
  ON mt.nav_survivorship_plan_task_sid = ms.nav_survivorship_plan_task_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (nav_survivorship_plan_task_sid, task_status_id, contact_method_id, nav_patient_id, navigator_id, coid, company_code, task_desc_text, task_resolution_date, task_closed_date, contact_result_text, contact_date, comment_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
