...
-- #####################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_CONTACT_DETAIL;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_contact_detail WHERE upper(cn_patient_contact_detail.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_contact_detail_stg.hashbite_ssk) AS hashbite_ssk
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_contact_detail_stg
  );
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_contact_detail AS mt USING (
    SELECT DISTINCT
        stg.cn_patient_contact_sid,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_contact_detail_stg AS stg
      WHERE upper(stg.hashbite_ssk) NOT IN(
        SELECT
            upper(cn_patient_contact_detail.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_contact_detail
      )
  ) AS ms
  ON mt.cn_patient_contact_sid = ms.cn_patient_contact_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_contact_sid, contact_detail_measure_type_id, contact_detail_measure_value_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
