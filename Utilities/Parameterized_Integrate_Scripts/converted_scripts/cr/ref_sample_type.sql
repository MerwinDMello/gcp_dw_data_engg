...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_REF_SAMPLE_TYPE ;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_sample_type AS mt USING (
    SELECT DISTINCT
        (
          SELECT
              coalesce(max(ref_sample_type.sample_type_id), 0) AS maxid
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.ref_sample_type
        ) + row_number() OVER (ORDER BY upper(trim(ssc.sample_type_name))) AS sample_type_id,
      ...
      FROM
        (
          SELECT DISTINCT
              trim(patient_heme_disease_assessment_stg.samplesourcetype) AS sample_type_name
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.patient_heme_disease_assessment_stg
            WHERE trim(patient_heme_disease_assessment_stg.samplesourcetype) IS NOT NULL
        ) AS ssc
  ...
  ON mt.sample_type_id = ms.sample_type_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (sample_type_id, sample_type_name, source_system_code, dw_last_update_date_time)
