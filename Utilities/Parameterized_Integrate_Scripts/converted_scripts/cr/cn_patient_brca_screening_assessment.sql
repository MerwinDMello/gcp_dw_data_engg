...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_BRCA_SCREENING_ASSESSMENT;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_brca_screening_assessment WHERE upper(trim(cn_patient_brca_screening_assessment.hashbite_ssk)) NOT IN(
    SELECT
        upper(trim(cn_patient_brca_screening_assessment_stg.hashbite_ssk))
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_brca_screening_assessment_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_brca_screening_assessment AS mt USING (
    SELECT DISTINCT
        cn_patient_brca_screening_assessment_stg.brca_screening_assessment_sid,
      ...
      WHERE upper(cn_patient_brca_screening_assessment_stg.hashbite_ssk) NOT IN(
        SELECT
            upper(cn_patient_brca_screening_assessment.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_brca_screening_assessment
      )
  ) AS ms
  ON mt.brca_screening_assessment_sid = ms.brca_screening_assessment_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (brca_screening_assessment_sid, nav_patient_id, tumor_type_id, navigator_id, coid, company_code, early_onset_breast_cancer_ind, ovarian_cancer_history_ind, two_primary_breast_cancer_ind, male_breast_cancer_ind, triple_negative_ind, ashkenazi_jewish_ind, two_plus_relative_cancer_ind, meeting_assessment_critieria_ind, hashbite_ssk, source_system_code, dw_last_update_date_time)
