...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_FACT_RAD_ONC_PATIENT;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.fact_rad_onc_patient AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY dhd.dimsiteid, dhd.factpatientid) AS fact_patient_sk,
      ...
      FROM
        (
          SELECT DISTINCT
              stg_factpatient.dimsiteid AS dimsiteid,
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.stg_factpatient
        ) AS dhd
        ...
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.rad_onc_patient AS rpa ON dhd.dimpatientid = rpa.source_patient_id
         AND rr.site_sk = rpa.site_sk
  ) AS ms
  ON mt.fact_patient_sk = ms.fact_patient_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (fact_patient_sk, hospital_sk, patient_sk, patient_status_id, location_sk, race_id, gender_id, site_sk, source_fact_patient_id, creation_date_time, admission_date_time, discharge_date_time, log_id, run_id, source_system_code, dw_last_update_date_time)
