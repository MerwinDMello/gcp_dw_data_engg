...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_Ref_Abstraction_Measure;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_abstraction_measure AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(src.abstraction_field)) + (
          SELECT
              coalesce(max(ref_abstraction_measure.abstraction_measure_sk), 0)
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.ref_abstraction_measure
        ) AS abstraction_measure_sk,
      ...
      FROM
        (
          SELECT DISTINCT
              cancer_abstraction_values_stg.abstraction_field
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.cancer_abstraction_values_stg
        ) AS src
      ...
      WHERE tgt.abstraction_measure_name IS NULL
  ) AS ms
  ON mt.abstraction_measure_sk = ms.abstraction_measure_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (abstraction_measure_sk, abstraction_measure_name, source_system_code, dw_last_update_date_time)
