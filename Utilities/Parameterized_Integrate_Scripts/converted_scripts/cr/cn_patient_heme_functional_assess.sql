...
-- #################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_HEME_FUNCTIONAL_ASSESS;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_func_assess WHERE upper(rtrim(cn_patient_heme_func_assess.hashbite_ssk)) NOT IN(
    SELECT
        upper(rtrim(cn_patient_heme_functional_assess_stg.hbsource)) AS hbsource
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_heme_functional_assess_stg
  );
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_func_assess AS mt USING (
    SELECT DISTINCT
        stg.patienthemefuncassessfactid AS cn_patient_heme_func_assess_sid,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_heme_functional_assess_stg AS stg
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.ref_test_type AS ref ON upper(rtrim(ref.test_sub_type_desc)) = upper(rtrim(stg.testtype))
         AND upper(rtrim(ref.test_type_desc)) = 'FUNCTIONAL ASSESSMENT'
      WHERE upper(rtrim(stg.hbsource)) NOT IN(
        SELECT
            upper(rtrim(cn_patient_heme_func_assess.hashbite_ssk)) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_func_assess
      )
  ) AS ms
  ON mt.cn_patient_heme_func_assess_sid = ms.cn_patient_heme_func_assess_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_heme_func_assess_sid, nav_patient_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, coid, company_code, navigator_id, func_assess_date, test_type_id, test_type_result_amt, hashbite_ssk, source_system_code, dw_last_update_date_time)
