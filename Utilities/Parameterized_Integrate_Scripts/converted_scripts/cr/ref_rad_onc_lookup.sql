...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_REF_RAD_ONC_LOOKUP;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_lookup;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_lookup AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY ds.dimsiteid, CASE
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimlookup AS ds
        CROSS JOIN `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_site AS rr
      WHERE rr.source_site_id = ds.dimsiteid
  ) AS ms
  ON mt.lookup_sk = ms.lookup_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (lookup_sk, site_sk, source_lookup_id, table_name, lookup_code_text, lookup_desc, log_id, run_id, source_system_code, dw_last_update_date_time)
