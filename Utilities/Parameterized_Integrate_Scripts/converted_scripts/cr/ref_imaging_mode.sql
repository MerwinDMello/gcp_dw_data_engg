...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_REF_IMAGING_MODE;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_imaging_mode AS mt USING (
    SELECT DISTINCT
        CAST(CAST(row_number() OVER (ORDER BY upper(trim(type_stg.imaging_mode_desc))) as FLOAT64) + CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number((
          SELECT
              coalesce(max(ref_imaging_mode.imaging_mode_desc), format('%4d', 0)) AS id1
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_imaging_mode
        )) as FLOAT64) as INT64) AS imaging_mode_id,
      ...
      FROM
        (
          SELECT
              ref_imaging_mode_stg.imaging_mode_desc,
              ref_imaging_mode_stg.source_system_code
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.ref_imaging_mode_stg
            WHERE upper(trim(ref_imaging_mode_stg.imaging_mode_desc)) NOT IN(
              SELECT
                  upper(trim(ref_imaging_mode.imaging_mode_desc))
                FROM
                  `hca-hin-dev-cur-ops`.edwcr.ref_imaging_mode
            )
        ) AS type_stg
  ) AS ms
  ON mt.imaging_mode_id = ms.imaging_mode_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (imaging_mode_id, imaging_mode_desc, source_system_code, dw_last_update_date_time)
