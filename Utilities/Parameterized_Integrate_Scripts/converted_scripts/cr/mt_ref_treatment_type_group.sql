...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_MT_REF_TREATMENT_TYPE_GRP;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_treatment_type_group AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(ref_treatment_type_group_stg.treatment_type_group_code)) AS treatment_type_group_id,
        ref_treatment_type_group_stg.treatment_type_group_code AS treatment_type_group_code,
        substr(CASE
           upper(rtrim(ref_treatment_type_group_stg.treatment_type_group_code))
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.ref_treatment_type_group_stg
  ) AS ms
  ON mt.treatment_type_group_id = ms.treatment_type_group_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (treatment_type_group_id, treatment_type_group_code, treatment_type_group_desc, nav_treatment_type_group_desc, source_system_code, dw_last_update_date_time)
