...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_plan_purpose AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(src.plan_purpose_name)) AS plan_purpose_sk,
      ...
      FROM
        (
          SELECT DISTINCT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimplan AS dhd
        ) AS src
      WHERE src.plan_purpose_name IS NOT NULL
  ) AS ms
  ON mt.plan_purpose_sk = ms.plan_purpose_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (plan_purpose_sk, plan_purpose_name, source_system_code, dw_last_update_date_time)
