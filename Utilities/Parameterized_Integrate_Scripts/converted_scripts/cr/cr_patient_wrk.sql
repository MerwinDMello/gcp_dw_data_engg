...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_PATIENT_STG;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_wrk AS mt USING (
    SELECT DISTINCT
        CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(x.patientid) as INT64) AS cr_patient_id,
        coalesce(rlc.master_lookup_sid, (
          SELECT
              rcl.master_lookup_sid
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS rcl
            ...
            WHERE upper(rtrim(nm.lookup_name)) = 'GENDER'
             AND upper(rtrim(rcl.lookup_code)) = '-99'
        )) AS patient_gender_id,
        coalesce(rlc1.master_lookup_sid, (
          SELECT
              rcl1.master_lookup_sid
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS rcl1
            ...
            WHERE upper(rtrim(nm1.lookup_name)) = 'RACE'
             AND upper(rtrim(rcl1.lookup_code)) = '-99'
        )) AS patient_race_id,
        coalesce(rlc2.master_lookup_sid, (
          SELECT
              rcl2.master_lookup_sid
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS rcl2
            ...
            WHERE upper(rtrim(nm2.lookup_name)) = 'VITAL STATUS'
             AND upper(rtrim(rcl2.lookup_code)) = '-99'
        )) AS vital_status_id,
        -- Coalesce(RLC.Master_Lookup_Sid,(Select Master_Lookup_Sid from EDWCR.Ref_Lookup_Code where Lookup_Code=-99)) as Patient_Gender_Id,
      ...
      FROM
        (
          SELECT
              trim(format('%11d', a.patientid)) AS patientid,
              trim(a.sex) AS sex,
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_stg AS a
        ) AS x
        ...
        SUBSTR(X.NEW_DATELASTCONTACT,7,2 )) =CAST(B1.CALENDAR_DATE AS INTEGER)*/
        /*LEFT JOIN
        ...
        (
          SELECT
            ...
            WHERE upper(rtrim(rln.lookup_name)) = 'GENDER'
        ) AS rlc ON upper(rtrim(x.sex)) = upper(rtrim(rlc.lookup_code))
        LEFT OUTER JOIN (
          SELECT
              rln.lookup_sid,
              rlc_0.lookup_code,
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_name AS rln
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS rlc_0 ON rlc_0.lookup_id = rln.lookup_sid
               AND upper(rtrim(rln.lookup_name)) = 'RACE'
        ) AS rlc1 ON upper(rtrim(x.race1)) = upper(rtrim(rlc1.lookup_code))
        LEFT OUTER JOIN (
          SELECT
              rln.lookup_sid,
              rlc_0.lookup_code,
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_name AS rln
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS rlc_0 ON rlc_0.lookup_id = rln.lookup_sid
            WHERE upper(rtrim(rln.lookup_name)) = 'VITAL STATUS'
        ) AS rlc2 ON upper(rtrim(x.vitalstatus)) = upper(rtrim(rlc2.lookup_code))
  ) AS ms
  ON mt.cr_patient_id = ms.cr_patient_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cr_patient_id, patient_gender_id, patient_race_id, vital_status_id, patient_system_id, coid, company_code, patient_birth_date, last_contact_date, patient_first_name, patient_middle_name, patient_last_name, patient_email_address_text, accession_num_code, patient_market_urn_text, medical_record_num, source_system_code, dw_last_update_date_time)
