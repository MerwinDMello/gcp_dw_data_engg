...
-- #####################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_Patient_Mltp_Disciplinary_Meeting;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_mltp_disciplinary_meeting WHERE upper(cn_patient_mltp_disciplinary_meeting.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_mltp_disciplinary_meeting_stg.hashbite_ssk) AS hashbite_ssk
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_mltp_disciplinary_meeting_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_mltp_disciplinary_meeting AS mt USING (
    SELECT DISTINCT
        stg.cn_patient_mltp_disc_meet_sid,
        stg.nav_patient_id,
        stg.tumor_type_id,
        stg.navigator_id,
        stg.coid AS coid,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_mltp_disciplinary_meeting_stg AS stg
      WHERE upper(stg.hashbite_ssk) NOT IN(
        SELECT
            upper(cn_patient_mltp_disciplinary_meeting.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_mltp_disciplinary_meeting
      )
  ) AS ms
  ON mt.cn_patient_mltp_disciplinary_meet_sid = ms.cn_patient_mltp_disc_meet_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_mltp_disciplinary_meet_sid, nav_patient_id, tumor_type_id, navigator_id, coid, company_code, meeting_date, patient_discussed_ind, treatment_change_ind, meeting_notes_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
