...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_Rad_Onc_Procedure_code;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_procedure_code;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_procedure_code AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY dp.dimsiteid, dp.dimprocedurecodeid) AS procedure_code_sk,
      ...
      FROM
        (
          SELECT
              CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(stg_dimprocedurecode.dimsiteid) as INT64) AS dimsiteid,
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimprocedurecode
        ) AS dp
        LEFT OUTER JOIN (
          SELECT DISTINCT
              stg_sc_modalities.treatment_type,
              stg_sc_modalities.treatment_category,
              stg_sc_modalities.procedure_code
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.stg_sc_modalities
        ) AS sm ON upper(rtrim(sm.procedure_code)) = upper(rtrim(dp.procedurecode))
  ...
  ON mt.procedure_code_sk = ms.procedure_code_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (procedure_code_sk, treatment_type_sk, site_sk, source_procedure_code_id, procedure_code, procedure_short_desc, procedure_long_desc, active_ind, log_id, run_id, source_system_code, dw_last_update_date_time)
