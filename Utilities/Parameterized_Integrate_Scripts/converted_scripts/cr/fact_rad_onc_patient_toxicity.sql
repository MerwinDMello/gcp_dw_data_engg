...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_Fact_Rad_Onc_Patient_Toxicity;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.fact_rad_onc_patient_toxicity AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY dp.dimsiteid, dp.factpatienttoxicityid DESC) AS fact_patient_toxicity_sk,
      ...
      FROM
        (
          SELECT
              stg_factpatienttoxicity.toxicitycomponentname,
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.stg_factpatienttoxicity
        ) AS dp
        INNER JOIN (
          SELECT
        ...
        LEFT OUTER JOIN (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.ref_rad_onc_toxicity_component
        ) AS tc ON upper(rtrim(dp.toxicitycomponentname)) = upper(rtrim(tc.toxicity_component_name))
        LEFT OUTER JOIN (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.ref_rad_onc_toxicity_assessment_type
        ) AS ta ON upper(rtrim(dp.toxicityassessmenttype)) = upper(rtrim(ta.toxicity_assessment_type_desc))
        LEFT OUTER JOIN (
          SELECT
  ...
  ON mt.fact_patient_toxicity_sk = ms.fact_patient_toxicity_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (fact_patient_toxicity_sk, toxicity_component_sk, toxicity_assessment_type_sk, activity_transaction_sk, patient_sk, scheme_id, toxicity_cause_certainty_type_id, toxicity_cause_type_id, diagnosis_code_sk, site_sk, source_fact_patient_toxicity_id, assessment_date_time, toxicity_effective_date, toxicity_grade_num, valid_entry_ind, toxicity_approved_date_time, assessment_performed_date_time, toxicity_reason_text, toxicity_approved_ind, toxicity_header_valid_entry_ind, revision_num, log_id, run_id, source_system_code, dw_last_update_date_time)
