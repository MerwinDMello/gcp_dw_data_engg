...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_CANCER_PAT_ID_OUTPUT_DRIVER;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_output_driver AS mt USING (
    SELECT DISTINCT
        CAST(row_number() OVER (ORDER BY a1.cancer_patient_driver_sk, a1.cancer_patient_id_output_sk) as NUMERIC) AS cancer_patient_id_output_driver_sk,
  ...
  ON mt.cancer_patient_id_output_driver_sk = ms.cancer_patient_id_output_driver_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cancer_patient_id_output_driver_sk, cancer_patient_driver_sk, cancer_patient_id_output_sk, message_control_id_text, user_action_date_time, coid, company_code, patient_dw_id, pat_acct_num, medical_record_num, patient_market_urn, last_name, first_name, birth_date, gender_code, social_security_num, address_line_1_text, address_line_2_text, city_name, state_code, zip_code, patient_type_code, message_type_code, message_event_type_code, message_flag_code, message_origin_requested_date_time, message_signed_observation_date_time, message_ingestion_date_time, message_created_date_time, document_type_id_text, document_type_text, document_type_coding_system_code, first_insert_date_time, icd_oncology_type_code, predicted_primary_icd_oncology_code, predicted_primary_icd_site_desc, suggested_primary_icd_oncology_code, suggested_primary_icd_site_desc, submitted_primary_icd_oncology_code, submitted_primary_icd_site_desc, transition_of_care_num, user_action_desc, user_action_criticality_text, user_3_4_id, report_assigned_date_time, attending_physician_full_name, pcp_full_name, pcp_phone_num, facility_menmonic_cs, network_mnemonic_cs, model_predicted_sarcoma_ind, submitted_sarcoma_ind, suggested_sarcoma_ind, benign_brain_tumor_type_ind, met_to_brain_tumor_type_ind, other_tumor_type_ind, source_system_code, dw_last_update_date_time)
