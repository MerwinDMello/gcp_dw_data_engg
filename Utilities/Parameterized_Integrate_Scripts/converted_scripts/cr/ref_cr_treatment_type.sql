...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_REF_TREATMENT_TYPE_GRP;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_cr_treatment_type AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(ref_cr_treatment_type_stg.treatment_type_code)) AS treatment_type_group_id,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.ref_cr_treatment_type_stg
  ) AS ms
  ON mt.treatment_type_id = ms.treatment_type_group_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (treatment_type_id, treatment_type_code, treatment_type_desc, treatment_group_id, source_system_code, dw_last_update_date_time)
