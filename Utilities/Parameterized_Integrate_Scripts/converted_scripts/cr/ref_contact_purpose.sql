...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_REF_CONTACT_PURPOSE;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_contact_purpose AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(trim(type_stg.contact_purpose_desc))) + (
          SELECT
              coalesce(max(ref_contact_purpose.contact_purpose_id), 0) AS id1
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_contact_purpose
        ) AS contact_purpose_id,
      ...
      FROM
        (
          SELECT
              contact_purpose_stg.contact_purpose_desc,
            ...
            WHERE upper(trim(contact_purpose_stg.contact_purpose_desc)) NOT IN(
              SELECT
                  upper(trim(ref_contact_purpose.contact_purpose_desc))
                FROM
                  `hca-hin-dev-cur-ops`.edwcr.ref_contact_purpose
                WHERE ref_contact_purpose.contact_purpose_desc IS NOT NULL
            )
        ) AS type_stg
      WHERE type_stg.contact_purpose_desc IS NOT NULL
       AND upper(trim(type_stg.contact_purpose_desc)) <> ''
  ) AS ms
  ON mt.contact_purpose_id = ms.contact_purpose_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (contact_purpose_id, contact_purpose_desc, source_system_code, dw_last_update_date_time)
