...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_REF_RAD_ONC_TOXICITY_COMPONENT;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_toxicity_component AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(src.toxicity_component_name)) AS toxicity_component_sk,
      ...
      FROM
        (
          SELECT DISTINCT
              stg_factpatienttoxicity.toxicitycomponentname AS toxicity_component_name,
              'R' AS source_system_code,
              datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.stg_factpatienttoxicity
        ) AS src
      WHERE src.toxicity_component_name IS NOT NULL
  ) AS ms
  ON mt.toxicity_component_sk = ms.toxicity_component_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (toxicity_component_sk, toxicity_component_name, source_system_code, dw_last_update_date_time)
