...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_PATIENT_DIAGNOSIS_DETAIL;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_diagnosis_detail AS mt USING (
    SELECT DISTINCT
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_diagnosis_detail_stg AS stg
        LEFT OUTER JOIN (
          SELECT
              rln.lookup_sid,
              rlc.lookup_code,
            ...
            FROM
              ...
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
            WHERE upper(rtrim(rln.lookup_name)) = 'LATERALITY'
        ) AS lkp ON upper(rtrim(stg.laterality)) = upper(rtrim(lkp.lookup_code))
        ...
        (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_name AS rln
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
            WHERE upper(rtrim(rln.lookup_name)) = 'HISTOLOGY'
        ) AS lkp1 ON upper(rtrim(stg.histology)) = upper(rtrim(lkp1.lookup_code))
         AND upper(rtrim(stg.histosubcode)) = upper(rtrim(lkp1.lookup_sub_code))
      QUALIFY row_number() OVER (PARTITION BY stg.tumor_id, stg.cr_patient_id ORDER BY stg.tumor_id, stg.cr_patient_id DESC) = 1
  ) AS ms
  ON mt.tumor_id = ms.tumor_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (tumor_id, cr_patient_id, tumor_site_id, diagnosis_name_id, diagnosis_date, diagnose_age_num, first_diagnose_year_num, source_system_code, dw_last_update_date_time)
