...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_Ref_Housing_Type;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_housing_type AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(dis.housing_type_name)) + coalesce((
          SELECT
              max(coalesce(a.housing_type_id, 0)) AS max_key
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.ref_housing_type AS a
        ), 0) AS housing_type_id,
      ...
      FROM
        (
          SELECT DISTINCT
              trim(st.housing_type_name) AS housing_type_name
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.ref_housing_type_stg AS st
      ...
      WHERE NOT EXISTS (
        SELECT
            1
          FROM
            `hca-hin-dev-cur-ops`.edwcr_base_views.ref_housing_type AS rcdc
          WHERE upper(trim(rcdc.housing_type_name)) = upper(trim(dis.housing_type_name))
      )
  ) AS ms
  ON mt.housing_type_id = ms.housing_type_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (housing_type_id, housing_type_name, source_system_code, dw_last_update_date_time)
