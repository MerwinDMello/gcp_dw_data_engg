...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_PATHOLOGY_RESULT;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_pathology_result WHERE upper(trim(cn_patient_pathology_result.hashbite_ssk)) NOT IN(
    SELECT
        upper(trim(cn_patient_pathology_result_stg.hashbite_ssk))
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_pathology_result_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_pathology_result AS mt USING (
    SELECT DISTINCT
        cn_patient_pathology_result_stg.cn_patient_pathology_res_sid,
      ...
      WHERE upper(cn_patient_pathology_result_stg.hashbite_ssk) NOT IN(
        SELECT
            upper(cn_patient_pathology_result.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_pathology_result
      )
  ) AS ms
  ON mt.cn_patient_pathology_result_sid = ms.cn_patient_pathology_res_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_pathology_result_sid, pathology_result_type_id, core_record_type_id, nav_patient_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, navigator_id, coid, company_code, result_value_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
