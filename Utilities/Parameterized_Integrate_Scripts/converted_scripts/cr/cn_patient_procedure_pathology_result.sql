...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_PROCEDURE_PATHOLOGY_RESULT;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_procedure_pathology_result WHERE (upper(cn_patient_procedure_pathology_result.hashbite_ssk), upper(cn_patient_procedure_pathology_result.navigation_procedure_type_code)) NOT IN(
    SELECT AS STRUCT
        upper(cn_patient_procedure_pathology_result_stg.hashbite_ssk) AS hashbite_ssk,
        upper(cn_patient_procedure_pathology_result_stg.navigation_procedure_type_code) AS navigation_procedure_type_code
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_procedure_pathology_result_stg
  );
...
BEGIN
  ...
  ON mt.cn_patient_proc_pathology_result_sid = ms.cn_patient_proc_pathology_result_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_proc_pathology_result_sid, cn_patient_procedure_sid, margin_result_id, nav_result_id, oncotype_diagnosis_result_id, navigation_procedure_type_code, pathology_result_date, pathology_result_name, pathology_grade_available_ind, pathology_grade_num, pathology_tumor_size_available_ind, tumor_size_num_text, margin_result_detail_text, sentinel_node_result_code, estrogen_receptor_sw, estrogen_receptor_strength_code, estrogen_receptor_pct_text, progesterone_receptor_sw, progesterone_receptor_strength_code, progesterone_receptor_pct_text, oncotype_diagnosis_score_num, oncotype_diagnosis_risk_text, comment_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
