...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_MT_REF_AJCC_STAGE;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr.ref_ajcc_stage;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_ajcc_stage AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(ref_lookup_code_stg.lookup_code)) AS ajcc_stage_id,
  ...
  ON mt.ajcc_stage_id = ms.ajcc_stage_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (ajcc_stage_id, ajcc_stage_code, ajcc_stage_sub_code, ajcc_stage_desc, ajcc_stage_group_id, source_system_code, dw_last_update_date_time)
