...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_REF_PATHWAY_VAR_REASON;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_pathway_var_reason AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(dis.pathway_var_reason_type_desc), upper(dis.pathway_var_reason_sub_type_desc)) + coalesce((
          SELECT
              max(coalesce(a.pathway_var_reason_id, 0)) AS max_key
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.ref_pathway_var_reason AS a
        ), 0) AS pathway_var_reason_id,
      ...
      FROM
        (
          SELECT DISTINCT
              trim(st.pathwayvariancereason) AS pathway_var_reason_type_desc,
            ...
            WHERE st.pathwayvariancereason IS NOT NULL
             OR st.otherpathwayreason IS NOT NULL
        ) AS dis
      WHERE (
        SELECT
          ...
          FROM
            `hca-hin-dev-cur-ops`.edwcr_base_views.ref_pathway_var_reason AS rcdc
      )
  ) AS ms
  ON mt.pathway_var_reason_id = ms.pathway_var_reason_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (pathway_var_reason_id, pathway_var_reason_type_desc, pathway_var_reason_sub_type_desc, source_system_code, dw_last_update_date_time)
