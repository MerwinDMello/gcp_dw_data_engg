...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_REF_RAD_ONC_SITE;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_site AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY ds.dimsiteid) AS site_sk,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_ro_dimsite_stg AS ds
  ) AS ms
  ON mt.site_sk = ms.site_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (site_sk, source_site_id, source_site_guid_text, site_code_text, site_name, server_name, site_desc, server_ip_address_text, aura_version_text, aura_last_installed_date_time, registration_date_time, history_user_name, history_date_time, source_system_code, dw_last_update_date_time)
