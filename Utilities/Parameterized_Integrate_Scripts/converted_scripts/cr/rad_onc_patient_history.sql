...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_Rad_Onc_Patient_History;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient_history AS mt USING (
    SELECT DISTINCT
      ...
      WHERE dhd.history_value_text IS NOT NULL
  ) AS ms
  ON mt.patient_sk = ms.patient_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (patient_sk, history_query_id, history_value_text, source_system_code, dw_last_update_date_time)
