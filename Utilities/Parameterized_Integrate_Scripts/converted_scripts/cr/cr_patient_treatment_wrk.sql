...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_PATIENT_TREATMENT;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_treatment_wrk1;
...
BEGIN
  SET _ERROR_CODE = 0;
  INSERT INTO `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_treatment_wrk1 (treatmentid, tumorid, treatmenthospid, rxcode, description, groupid, othercode, surgmarg, rxtype, dtprotocol, dtrxstart, prottitle, rxtxt, rxmd1, source_system_code, dw_last_update_date_time)
    SELECT
        trt.treatmentid,
        trt.tumorid,
        trt.treatmenthospid,
        ...
        trt.surgmarg,
        trt.rxtype,
        (extract(YEAR from trt.dtprotocol) - 1900) * 10000 + extract(MONTH from trt.dtprotocol) * 100 + extract(DAY from trt.dtprotocol) AS dtprotocol,
        (extract(YEAR from trt.dtrxstart) - 1900) * 10000 + extract(MONTH from trt.dtrxstart) * 100 + extract(DAY from trt.dtrxstart) AS dtrxstart,
        trt.prottitle,
        trt.rxtxt,
        trt.rxmd1,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_treatment_stg AS trt
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_treatment_stg1 AS trt1 ON trt.treatmentid = trt1.treatmentid
...
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_treatment_wrk;
EXCEPTION WHEN ERROR THEN
  SET _ERROR_CODE = 1;
  SET _ERROR_MSG = @@error.message;
END;
...
BEGIN
  ...
  INSERT INTO `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_treatment_wrk (treatment_id, tumor_id, treatment_hospital_id, treatment_type_id, surgical_site_id, surgical_margin_result_id, treatment_type_group_id, clinical_trial_start_date, treatment_start_date, clinical_trial_text, comment_text, treatment_performing_physician_code, source_system_code, dw_last_update_date_time)
    SELECT
        ...
        -- COALESCE(RLC1.MASTER_LOOKUP_SID,(SELECT MASTER_LOOKUP_SID FROM EDWCR.REF_LOOKUP_CODE WHERE LOOKUP_CODE=-99))  AS SURGICAL_MARGIN_RESULT_ID,
      ...
      FROM
        (
          SELECT
              stg.treatmentid AS treatment_id,
              ...
              datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_treatment_wrk1 AS stg
        ) AS src
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_hospital AS rh ON upper(rtrim(src.treatmenthospid)) = upper(rtrim(rh.hospital_code))
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_cr_treatment_type AS tt ON upper(rtrim(src.rxcode)) = upper(rtrim(tt.treatment_type_code))
         AND CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(src.groupid) as FLOAT64) = tt.treatment_group_id
        ...
        AND NM.LOOKUP_NAME='SURGERY SITE'*/
        /*LEFT JOIN  EDWCR.REF_LOOKUP_CODE RLC1
        ON SRC.SURGMARG=RLC1.LOOKUP_CODE
        LEFT JOIN EDWCR.REF_LOOKUP_NAME NM1
        ON RLC1.LOOKUP_ID=NM1.LOOKUP_SID
        AND NM1.LOOKUP_NAME='SURGERY MARGIN RESULT'*/
        `hca-hin-dev-cur-ops`.edwcr.ref_treatment_type_group AS tgtp ON upper(rtrim(src.rxtype)) = upper(rtrim(tgtp.treatment_type_group_code))
        LEFT OUTER JOIN (
          SELECT
              rln.lookup_sid,
              rlc.lookup_code,
            ...
            FROM
              ...
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
            WHERE upper(rtrim(rln.lookup_name)) = 'SURGERY SITE'
        ) AS lkp ON upper(rtrim(src.othercode)) = upper(rtrim(lkp.lookup_code))
        LEFT OUTER JOIN (
          SELECT
              rln.lookup_sid,
              rlc.lookup_code,
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_lookup_name AS rln
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
               AND upper(rtrim(rln.lookup_name)) = 'SURGERY MARGIN RESULT'
        ) AS lkp1 ON upper(rtrim(src.surgmarg)) = upper(rtrim(lkp1.lookup_code))
  ;
EXCEPTION WHEN ERROR THEN
  SET _ERROR_CODE = 1;
  SET _ERROR_MSG = @@error.message;
