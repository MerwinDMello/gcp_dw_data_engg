...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_Patient_Address;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_address WHERE cn_patient_address.nav_patient_id NOT IN(
    SELECT
        cn_patient_address_stg.nav_patient_id
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_address_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_address AS mt USING (
    SELECT DISTINCT
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_address_stg AS stg
  ...
  ON mt.nav_patient_id = ms.nav_patient_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (nav_patient_id, housing_type_id, address_line_1_text, address_line_2_text, city_name, state_code, zip_code, local_housing_address_text, source_system_code, dw_last_update_date_time)
