...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_Fact_Cancer_Patient_Financial;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.fact_cancer_patient_financial AS mt USING (
    SELECT DISTINCT
        cpe.encounter_id AS encounter_id,
      ...
      FROM
        (
          SELECT
              consolidated_patient_encounter.*
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.consolidated_patient_encounter
            WHERE consolidated_patient_encounter.encounter_start_date BETWEEN DATE '2018-01-01' AND date_sub(current_date('US/Central'), interval extract(DAY from current_date('US/Central')) DAY)
             AND upper(rtrim(consolidated_patient_encounter.encounter_database_code)) = 'EDWPF'
        ) AS cpe
        INNER JOIN (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.consolidated_patient_service
            WHERE upper(rtrim(consolidated_patient_service.oncology_ind)) = 'Y'
             AND upper(rtrim(consolidated_patient_service.normal_newborn_bed_type_ind)) = 'N'
        ) AS cps ON cps.encounter_id = cpe.encounter_id
        ...
        INNER JOIN (
          SELECT
              fact_facility.coid
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.fact_facility
            WHERE upper(rtrim(fact_facility.lob_code)) IN(
              'HOS'
            )
             AND upper(rtrim(fact_facility.sub_lob_code)) IN(
              'MED'
            )
             AND upper(rtrim(fact_facility.coid_status_code)) = 'F'
        ) AS ff ON upper(rtrim(cpe.coid)) = upper(rtrim(ff.coid))
        LEFT OUTER JOIN (
          SELECT
              pe.company_code,
              pe.coid,
              ...
              pe.esl_level_5_desc
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.patient_esl AS pe
              ...
              LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.diagnosis_related_group AS drg ON upper(rtrim(pe.esl_level_5_code)) = upper(rtrim(drg.reimbursement_group_code))
               ...
               AND drg.reimbursement_group_end_date >= current_date('US/Central')
        ) AS pesl ON pesl.encounter_id = cpe.encounter_id
  ) AS ms
  ON mt.encounter_id = ms.encounter_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (encounter_id, encounter_source_system_code, empi_text, pat_acct_num, medical_record_num, patient_market_urn, encounter_start_date, encounter_end_date, coid, company_code, sub_unit_num, diagnosis_type_code, principal_diagnosis_code, procedure_type_code, principal_procedure_code, principal_procedure_date, total_billed_charge_amt, patient_age, patient_person_dw_id, financial_class_code, patient_type_code_pos1, emergency_ind, oncology_tumor_site_id, oncology_detail_tumor_site_id, robotic_ind3, op_product_line_desc, drg_code_hcfa, drg_medical_surgical_ind, calculated_length_of_stay_num, estimated_net_revenue_amt, direct_contribution_margin_amt, ebdita_amt, esl_level_1_desc, esl_level_2_desc, esl_level_3_desc, esl_level_4_desc, dw_last_update_date_time)
