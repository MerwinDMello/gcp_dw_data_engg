...
BEGIN
  ...
  INSERT INTO `hca-hin-dev-cur-ops`.edwcr_staging.rad_onc_address_wrk (address_sk, address_line_1_text, address_line_2_text, full_address_text, address_comment_text)
    SELECT
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimhospitaldepartment
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.rad_onc_address AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(CASE
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.rad_onc_address_wrk AS ds
  ) AS ms
  ON mt.address_sk = ms.address_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (address_sk, address_line_1_text, address_line_2_text, full_address_text, address_comment_text, source_system_code, dw_last_update_date_time)
