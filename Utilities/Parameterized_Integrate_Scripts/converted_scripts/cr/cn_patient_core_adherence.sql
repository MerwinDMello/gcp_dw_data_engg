...
-- #####################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_CORE_ADHERENCE;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_core_adherence WHERE upper(cn_patient_core_adherence.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_core_adherence_stg.hashbite_ssk) AS hashbite_ssk
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_core_adherence_stg
  );
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_core_adherence AS mt USING (
    SELECT DISTINCT
        stg.cn_patient_core_adherence_sid,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_core_adherence_stg AS stg
      WHERE upper(stg.hashbite_ssk) NOT IN(
        SELECT
            upper(cn_patient_core_adherence.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_core_adherence
      )
  ) AS ms
  ON mt.cn_patient_core_adherence_sid = ms.cn_patient_core_adherence_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_core_adherence_sid, core_adherence_measure_id, nav_patient_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, navigator_id, coid, company_code, core_adherence_measure_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
