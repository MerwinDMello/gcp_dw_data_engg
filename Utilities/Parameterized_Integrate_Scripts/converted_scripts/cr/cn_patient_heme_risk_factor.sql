...
-- #####################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_HEME_RISK_FACTOR_STG;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_risk_factor WHERE upper(cn_patient_heme_risk_factor.hashbite_ssk) NOT IN(
    SELECT
        upper(patient_heme_risk_factor_stg.hashbite_ssk) AS hashbite_ssk
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.patient_heme_risk_factor_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_risk_factor AS mt USING (
    SELECT DISTINCT
        t1.cn_patient_heme_diagnosis_sid,
        t1.nav_patient_id,
        t1.tumor_type_id,
      ...
      FROM
        (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.patient_heme_risk_factor_stg AS stg
  ...
  ON mt.cn_patient_heme_diagnosis_sid = ms.cn_patient_heme_diagnosis_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_heme_diagnosis_sid, nav_patient_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, coid, company_code, navigator_id, risk_factor_text, other_risk_factor_text, previous_tumor_site_id, other_previous_tumor_site_id, hashbite_ssk, source_system_code, dw_last_update_date_time)
