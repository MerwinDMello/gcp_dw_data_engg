...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_Ref_Breast_Cancer_Type;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_breast_cancer_type AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(trim(type_stg.breast_cancer_type_desc))) + (
          SELECT
              coalesce(max(ref_breast_cancer_type.breast_cancer_type_id), 0) AS id1
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_breast_cancer_type
        ) AS breast_cancer_type_id,
  ...
  ON mt.breast_cancer_type_id = ms.breast_cancer_type_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (breast_cancer_type_id, breast_cancer_type_desc, source_system_code, dw_last_update_date_time)
