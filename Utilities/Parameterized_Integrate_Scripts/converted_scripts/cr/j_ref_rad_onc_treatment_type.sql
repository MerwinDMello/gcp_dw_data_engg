...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_REF_RAD_ONC_ADDRESS;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_treatment_type AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(max(trim(sc.treatment_category_desc))), upper(max(trim(sc.treatment_type_desc)))) AS treatment_type_sk,
        ...
        datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.stg_sc_modalities_wrk AS sc
      ...
      GROUP BY upper(substr(trim(sc.treatment_category_desc), 1, 50)), upper(substr(trim(sc.treatment_type_desc), 1, 50))
  ) AS ms
  ON mt.treatment_type_sk = ms.treatment_type_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (treatment_type_sk, treatment_category_desc, treatment_type_desc, source_system_code, dw_last_update_date_time)
