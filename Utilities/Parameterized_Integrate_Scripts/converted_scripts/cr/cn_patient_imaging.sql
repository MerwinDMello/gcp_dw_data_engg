...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_IMAGING;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_imaging WHERE upper(cn_patient_imaging.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_imaging_stg.hashbite_ssk) AS hashbite_ssk
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_imaging_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_imaging AS mt USING (
    SELECT DISTINCT
        ...
        CASE
           upper(rtrim(stg.initial_diagnosis_ind))
          WHEN 'YES' THEN 'Y'
          WHEN 'NO' THEN 'N'
          ELSE 'U'
        END AS initial_diagnosis_ind,
        CASE
           upper(rtrim(stg.disease_monitoring_ind))
          WHEN 'YES' THEN 'Y'
          WHEN 'NO' THEN 'N'
          ELSE 'U'
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_imaging_stg AS stg
      ...
      WHERE upper(stg.hashbite_ssk) NOT IN(
        SELECT
            upper(cn_patient_imaging.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr_base_views.cn_patient_imaging
      )
  ) AS ms
  ON mt.cn_patient_imaging_sid = ms.cn_patient_imaging_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_imaging_sid, core_record_type_id, nav_patient_id, med_spcl_physician_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, navigator_id, coid, company_code, imaging_type_id, imaging_date, imaging_mode_id, imaging_area_side_id, imaging_location_text, imaging_facility_id, birad_scale_code, comment_text, disease_status_id, treatment_status_id, other_image_type_text, initial_diagnosis_ind, disease_monitoring_ind, radiology_result_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
