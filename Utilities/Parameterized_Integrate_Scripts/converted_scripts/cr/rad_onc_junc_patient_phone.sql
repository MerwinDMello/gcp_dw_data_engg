  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.rad_onc_junc_patient_phone AS mt USING (
    SELECT DISTINCT
      ...
      FROM
        (
          SELECT
              CAST(NULL as INT64) AS patient_sk,
        ...
        INNER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.rad_onc_patient AS pt ON rr.site_sk = pt.site_sk
         AND dim_sub.dimpatientid = pt.source_patient_id
  ) AS ms
  ON mt.patient_sk = ms.patient_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (patient_sk, phone_num_type_code, phone_num_sk, source_system_code, dw_last_update_date_time)
