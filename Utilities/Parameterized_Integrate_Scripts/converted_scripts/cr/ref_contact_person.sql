...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_REF_CONTACT_PERSON;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_contact_person AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(trim(type_stg.contact_person_desc))) + (
          SELECT
              coalesce(max(ref_contact_person.contact_person_id), 0) AS id1
            FROM
          ...
          SELECT
            ...
            WHERE upper(trim(contact_person_stg.contact_person_desc)) NOT IN(
              SELECT
                ...
                FROM
                  `hca-hin-dev-cur-ops`.edwcr.ref_contact_person
                WHERE ref_contact_person.contact_person_desc IS NOT NULL
            )
        ) AS type_stg
      WHERE type_stg.contact_person_desc IS NOT NULL
       AND upper(trim(type_stg.contact_person_desc)) <> ''
  ) AS ms
  ON mt.contact_person_id = ms.contact_person_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (contact_person_id, contact_person_desc, source_system_code, dw_last_update_date_time)
