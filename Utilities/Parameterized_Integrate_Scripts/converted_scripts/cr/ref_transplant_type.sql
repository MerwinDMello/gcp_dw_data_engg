...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_REF_TRANSPLANT_TYPE;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_transplant_type AS mt USING (
    SELECT DISTINCT
        (
          SELECT
              coalesce(max(ref_transplant_type.transplant_type_id), 0) AS maxid
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.ref_transplant_type
        ) + row_number() OVER (ORDER BY upper(ssc.transplant_type_name)) AS transplant_type_id,
      ...
      FROM
        (
          SELECT DISTINCT
              stg_patienthemetransplant.type_type AS transplant_type_name
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.stg_patienthemetransplant
            WHERE stg_patienthemetransplant.type_type IS NOT NULL
        ) AS ssc
  ...
  ON mt.transplant_type_id = ms.transplant_type_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (transplant_type_id, transplant_type_name, source_system_code, dw_last_update_date_time)
