...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_Fact_Rad_Onc_Course_Diagnosis;' FOR SESSION;
      ...
      FROM
        ...
        LEFT OUTER JOIN (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.fact_rad_onc_patient_diagnosis
        ...
        LEFT OUTER JOIN (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.ref_rad_onc_diagnosis_code
        ) AS dc ON dp.dimdiagnosiscodeid = dc.source_diagnosis_code_id
         AND rr.site_sk = dc.site_sk
  ) AS ms
  ON mt.fact_course_diagnosis_sk = ms.fact_course_diagnosis_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (fact_course_diagnosis_sk, fact_patient_diagnosis_sk, patient_course_sk, diagnosis_code_sk, site_sk, source_fact_course_diagnosis_id, primary_course_ind, log_id, run_id, source_system_code, dw_last_update_date_time)
