...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_Rad_Onc_Patient_Physician;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient_physician AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY dhd.dimsiteid, dhd.dimpatientdoctorid) AS patient_physician_sk,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimpatientdoctor AS dhd
  ...
  ON mt.patient_physician_sk = ms.patient_physician_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (patient_physician_sk, site_sk, source_patient_physician_id, patient_sk, primary_physician_ind, oncologist_ind, resource_id, log_id, run_id, source_system_code, dw_last_update_date_time)
