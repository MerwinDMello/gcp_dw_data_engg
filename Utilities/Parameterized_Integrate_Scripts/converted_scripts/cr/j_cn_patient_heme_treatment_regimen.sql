...
-- #####################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_HEME_TREATMENT_REGIMEN;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_treatment_regimen WHERE upper(rtrim(cn_patient_heme_treatment_regimen.hashbite_ssk)) NOT IN(
    SELECT
        upper(rtrim(patient_heme_treatment_regimen_stg.hbsource)) AS hbsource
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.patient_heme_treatment_regimen_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_treatment_regimen AS mt USING (
    SELECT DISTINCT
        CAST(stg.patienthemediagnosisfactid as INT64) AS cn_patient_heme_diagnosis_sid,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.patient_heme_treatment_regimen_stg AS stg
      ...
      WHERE upper(trim(stg.hbsource)) NOT IN(
        SELECT
            upper(trim(cn_patient_heme_treatment_regimen.hashbite_ssk))
          FROM
            `hca-hin-dev-cur-ops`.edwcr_base_views.cn_patient_heme_treatment_regimen
      )
  ) AS ms
  ON mt.cn_patient_heme_diagnosis_sid = ms.cn_patient_heme_diagnosis_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_heme_diagnosis_sid, nav_patient_id, regimen_id, treatment_phase_id, pathway_var_reason_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, navigator_id, coid, company_code, planned_start_date, actual_start_date, drug_text, cycle_num, cycle_length_num, cycle_frequency_text, ordinal_cycle_num, pathway_ind, pathway_text, pathway_compliant_ind, treatment_plan_document_date, prior_plan_document_timeframe_ind, treatment_regimen_comment_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
