...
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient_contact;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient_contact AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY stg.patient_sk, stg.contact_address_sk, upper(stg.contact_full_name), upper(stg.contact_relation_text), upper(stg.contact_entrusted_ind), upper(stg.contact_comment_text)) AS patient_contact_sk,
        stg.patient_sk,
      ...
      FROM
        (
          SELECT
              ...
              CASE
                 dhd.patemergencycontactentrusted
                WHEN 1 THEN 'Y'
                WHEN 0 THEN 'N'
                ELSE 'U'
            ...
            FROM
              ...
              LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.rad_onc_address AS ra ON upper(trim(dhd.patemergencycontactaddress)) = upper(trim(ra.full_address_text))
      ...
      WHERE stg.contact_address_sk IS NOT NULL
       ...
       OR stg.contact_comment_text IS NOT NULL
  ) AS ms
  ON mt.patient_contact_sk = ms.patient_contact_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (patient_contact_sk, patient_sk, contact_address_sk, contact_full_name, contact_relation_text, contact_entrusted_ind, contact_comment_text, source_system_code, dw_last_update_date_time)
