...
-- #################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_Patient_Heme;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme WHERE upper(rtrim(cn_patient_heme.hashbite_ssk)) NOT IN(
    SELECT
        upper(rtrim(cn_patient_heme_stg.hbsource)) AS hbsource
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_heme_stg
  );
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme AS mt USING (
    SELECT DISTINCT
        stg.patienthemefactid,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_heme_stg AS stg
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.cn_physician_detail AS dtl ON upper(rtrim(dtl.physician_name)) = upper(rtrim(stg.hematologist))
      WHERE upper(rtrim(stg.hbsource)) NOT IN(
        SELECT
            upper(rtrim(cn_patient_heme.hashbite_ssk)) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme
      )
      GROUP BY 1, 2, 3, 4, 5, upper(stg.coid), 8, upper(stg.transportation), upper(stg.drugusehistory), 11, upper(substr(stg.hbsource, 1, 60)), 14
      QUALIFY row_number() OVER (PARTITION BY stg.patienthemefactid ORDER BY dtl.physician_id DESC) = 1
  ) AS ms
  ON mt.cn_patient_heme_sid = ms.patienthemefactid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_heme_sid, nav_patient_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, coid, company_code, navigator_id, transportation_text, drug_use_history_text, hematologist_physician_id, hashbite_ssk, source_system_code, dw_last_update_date_time)
