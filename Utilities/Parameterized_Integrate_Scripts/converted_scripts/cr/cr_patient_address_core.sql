...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_address AS mt USING (
    SELECT DISTINCT
        cr_patient_address_wrk.cr_patient_id,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_address_wrk
  ) AS ms
  ON mt.cr_patient_id = ms.cr_patient_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cr_patient_id, state_id, address_line_1_text, address_line_2_text, city_name, zip_code, source_system_code, dw_last_update_date_time)
