...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_Cancer_Patient_Id_Abstraction_Detail;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_detail_wrk0;
...
BEGIN
  SET _ERROR_CODE = 0;
  INSERT INTO `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_detail_wrk0 (cancer_abstraction_sk, abstraction_measure_sk, cancer_patient_id_output_sk, message_control_id_text, coid, company_code, patient_dw_id, pat_acct_num, predicted_value_text, submitted_value_text, suggested_value_text, source_system_code, dw_last_update_date_time)
    SELECT
        abstrct.cancer_abstraction_sk AS cancer_abstraction_sk,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cancer_abstraction_values_stg AS stg
        INNER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.cancer_patient_id_abstraction AS abstrct ON upper(trim(stg.unique_message_id)) = upper(trim(abstrct.message_control_id_text))
        ...
        INNER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.ref_abstraction_measure AS measure ON upper(trim(stg.abstraction_field)) = upper(trim(measure.abstraction_measure_name))
      QUALIFY row_number() OVER (PARTITION BY cancer_abstraction_sk, abstraction_measure_sk ORDER BY upper(stg.unique_message_id) DESC) = 1
  ;
EXCEPTION WHEN ERROR THEN
  SET _ERROR_CODE = 1;
  SET _ERROR_MSG = @@error.message;
...
BEGIN
  SET _ERROR_CODE = 0;
  UPDATE `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_abstraction_detail SET cancer_patient_id_output_sk = src.cancer_patient_id_output_sk, message_control_id_text = src.message_control_id_text, coid = src.coid, company_code = src.company_code, patient_dw_id = src.patient_dw_id, pat_acct_num = src.pat_acct_num, predicted_value_text = src.predicted_value_text, submitted_value_text = src.submitted_value_text, suggested_value_text = src.suggested_value_text, source_system_code = src.source_system_code, dw_last_update_date_time = src.dw_last_update_date_time FROM `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_detail_wrk0 AS src WHERE cancer_patient_id_abstraction_detail.cancer_abstraction_sk = src.cancer_abstraction_sk
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_abstraction_detail AS mt USING (
    SELECT DISTINCT
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_abstraction_detail_wrk0 AS stg
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.cancer_patient_id_abstraction_detail AS tgt ON stg.cancer_abstraction_sk = tgt.cancer_abstraction_sk
  ...
  ON mt.cancer_abstraction_sk = ms.cancer_abstraction_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cancer_abstraction_sk, abstraction_measure_sk, cancer_patient_id_output_sk, message_control_id_text, coid, company_code, patient_dw_id, pat_acct_num, predicted_value_text, submitted_value_text, suggested_value_text, source_system_code, dw_last_update_date_time)
