...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_FAMILY_HISTORY;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_family_history WHERE upper(cn_patient_family_history.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_family_history_stg.hbsource) AS hbsource
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_family_history_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_family_history AS mt USING (
    SELECT DISTINCT
        cn_patient_family_history_stg.patienthistoryfactid AS cn_patient_family_history_sid,
      ...
      WHERE upper(cn_patient_family_history_stg.hbsource) NOT IN(
        SELECT
            upper(cn_patient_family_history.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr_base_views.cn_patient_family_history
      )
  ) AS ms
  ON mt.cn_patient_family_history_sid = ms.cn_patient_family_history_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_family_history_sid, family_history_query_id, nav_patient_id, coid, company_code, family_history_value_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
