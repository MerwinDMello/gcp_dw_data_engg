...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_NAVQUE_PATIENT_TUMOR_DRIVER;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.navque_patient_tumor_driver AS mt USING (
    SELECT DISTINCT
        CAST(row_number() OVER (ORDER BY CAST(sk.cancer_patient_driver_sk as INT64), ctd.cancer_tumor_driver_sk, a.navque_history_id) as NUMERIC) AS navque_patient_tumor_driver_sk,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_base_views.navque_history AS a
        LEFT OUTER JOIN (
          SELECT
              cpio.*
            FROM
              (
                SELECT DISTINCT
                    a_0.cancer_patient_driver_sk,
                    a_0.message_control_id_text,
                    a_0.user_action_date_time
                  FROM
                    `hca-hin-dev-cur-ops`.edwcr_base_views.cancer_patient_id_output_driver AS a_0
              ) AS cpio
            QUALIFY row_number() OVER (PARTITION BY upper(cpio.message_control_id_text) ORDER BY cpio.user_action_date_time DESC) = 1
        ...
        LEFT OUTER JOIN (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.cancer_tumor_driver
            QUALIFY row_number() OVER (PARTITION BY cancer_tumor_driver.cn_navque_tumor_type_id ORDER BY cancer_tumor_driver.cancer_tumor_driver_sk DESC) = 1
        ) AS ctd ON a.tumor_type_id = ctd.cn_navque_tumor_type_id
  ) AS ms
  ON mt.navque_patient_tumor_driver_sk = ms.navque_patient_tumor_driver_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (navque_patient_tumor_driver_sk, cancer_patient_driver_sk, cancer_tumor_driver_sk, navque_history_id, navque_action_id, navque_reason_id, tumor_type_id, navigator_id, coid, company_code, message_control_id_text, message_date, navque_insert_date, navque_action_date, medical_record_num, patient_market_urn, network_mnemonic_cs, transition_of_care_score_num, navigated_patient_ind, message_source_flag, hashbite_ssk, source_system_code, dw_last_update_date_time)
