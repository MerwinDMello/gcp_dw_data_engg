...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_REG_CANCER_PATIENT_DETAIL;' FOR SESSION;
/* Truncate Core Table */
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr.cancer_patient_detail;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cancer_patient_detail AS mt USING (
    SELECT DISTINCT
        CAST(row_number() OVER (ORDER BY upper(met.patient_market_urn_text), met.network_mnemonic_cs) as NUMERIC) AS cancer_patient_detail_sk,
  ...
  ON mt.cancer_patient_detail_sk = ms.cancer_patient_detail_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cancer_patient_detail_sk, cancer_patient_driver_sk, cr_patient_id, cn_patient_id, cp_patient_id, coid, company_code, network_mnemonic_cs, patient_market_urn_text, medical_record_num, relationship_name, address_line_1_text, address_line_2_text, city_name, death_date, patient_birth_date, patient_email_address_text, patient_gender_code, patient_state_code, zip_code, phone_num_type_code, phone_num, preferred_language_text, insurance_type_name, preferred_contact_method_text, insurance_company_name, race_name, patient_system_id, vital_status_name, source_system_code, dw_last_update_date_time)
...
BEGIN
  SET _ERROR_CODE = 0;
  UPDATE `hca-hin-dev-cur-ops`.edwcr.cancer_patient_detail AS cd1 SET cancer_patient_driver_sk = cp.cancer_patient_driver_sk FROM `hca-hin-dev-cur-ops`.edwcr.cancer_patient_driver AS cp WHERE upper(rtrim(cd1.medical_record_num)) = upper(rtrim(cp.medical_record_num))
...
BEGIN
  SET _ERROR_CODE = 0;
  UPDATE `hca-hin-dev-cur-ops`.edwcr.cancer_patient_detail AS cd1 SET cancer_patient_driver_sk = cp.cancer_patient_driver_sk FROM (
    SELECT
        cpd.cp_patient_id,
      ...
      FROM
        (
          SELECT
              ...
              count(*) AS cnt
            FROM
              `hca-hin-dev-cur-ops`.edwcr.cancer_patient_driver
            WHERE cancer_patient_driver.cn_patient_id IS NULL
             AND cancer_patient_driver.cr_patient_id IS NULL
            GROUP BY 1
            HAVING count(*) > 1
        ) AS cpd
        INNER JOIN `hca-hin-dev-cur-ops`.edwcr.cancer_patient_driver AS cp1 ON cpd.cp_patient_id = cp1.cp_patient_id
  ) AS cp WHERE upper(rtrim(cd1.medical_record_num)) = upper(rtrim(cp.medical_record_num))
   AND upper(rtrim(cd1.patient_market_urn_text)) = upper(rtrim(cp.patient_market_urn_text));
EXCEPTION WHEN ERROR THEN
  SET _ERROR_CODE = 1;
  SET _ERROR_MSG = @@error.message;
