...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_REF_TUMOR_TYPE;' FOR SESSION;
...
END IF;
/* Delete the records from Core table which delete logic to purge data only for Source_System_Cd='N' for Ref_Tumor_Type */
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.ref_tumor_type WHERE upper(rtrim(ref_tumor_type.source_system_code)) = 'N';
...
IF _ERROR_CODE <> 0 THEN
  RAISE USING MESSAGE = concat('Script terminated with return code: ', _ERROR_CODE, ' ', _ERROR_MSG);
END IF;
/* Insert the records into the Core Table */
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_tumor_type AS mt USING (
    SELECT DISTINCT
        ref_tumor_type_stg.tumor_type_id,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.ref_tumor_type_stg
  ) AS ms
  ON mt.tumor_type_id = ms.tumor_type_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (tumor_type_id, tumor_type_desc, navigation_tumor_code_id, tumor_type_group_name, source_system_code, dw_last_update_date_time)
