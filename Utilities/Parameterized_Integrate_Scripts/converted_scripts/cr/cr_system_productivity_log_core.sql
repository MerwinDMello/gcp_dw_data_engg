...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_system_productivity_log AS mt USING (
    SELECT DISTINCT
        cr_system_productivity_log_wrk.system_productivity_log_id,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_system_productivity_log_wrk
  ) AS ms
  ON mt.system_productivity_log_id = ms.system_productivity_log_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (system_productivity_log_id, cr_patient_id, tumor_id, system_user_id_code, system_change_status_date, source_system_code, dw_last_update_date_time)
