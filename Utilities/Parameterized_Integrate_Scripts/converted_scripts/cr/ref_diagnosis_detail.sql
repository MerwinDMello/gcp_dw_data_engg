...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_Ref_Diagnosis_Detail;' FOR SESSION;
-- CALL DBADMIN_PROCS.collect_stats_table('edwcr_staging','Ref_Diagnosis_Detail_Stg');
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_diagnosis_detail AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(trim(type_stg.diagnosis_detail_desc)), upper(trim(type_stg.diagnosis_indicator_text))) + (
          SELECT
              coalesce(max(ref_diagnosis_detail.diagnosis_detail_id), 0) AS id1
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_diagnosis_detail
        ) AS diagnosis_detail_id,
  ...
  ON mt.diagnosis_detail_id = ms.diagnosis_detail_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (diagnosis_detail_id, diagnosis_detail_desc, diagnosis_indicator_text, source_system_code, dw_last_update_date_time)
