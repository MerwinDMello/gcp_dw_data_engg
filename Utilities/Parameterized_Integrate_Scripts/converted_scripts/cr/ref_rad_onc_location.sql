...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_REF_RAD_ONC_LOCATION;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_location AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY rr.site_sk, CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(ds.dimlocationid) as INT64)) AS location_sk,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimlocation AS ds
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.ref_rad_onc_site AS rr ON rr.source_site_id = CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(ds.dimsiteid) as INT64)
  ) AS ms
  ON mt.location_sk = ms.location_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (location_sk, site_sk, source_location_id, country_name, state_name, city_name, county_name, zip_code, log_id, run_id, source_system_code, dw_last_update_date_time)
