...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_REF_HOSPITAL;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr.ref_hospital;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_hospital AS mt USING (
    SELECT DISTINCT
        ref_hospital_stg.hospital_id,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.ref_hospital_stg
  ) AS ms
  ON mt.hospital_id = ms.hospital_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (hospital_id, hospital_code, hospital_name, source_system_code, dw_last_update_date_time)
