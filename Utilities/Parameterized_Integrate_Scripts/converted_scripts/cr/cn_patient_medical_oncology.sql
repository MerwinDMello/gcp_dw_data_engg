...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_MEDICAL_ONCOLOGY;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_medical_oncology WHERE upper(trim(cn_patient_medical_oncology.hashbite_ssk)) NOT IN(
    SELECT
        upper(trim(cn_patient_medical_oncology_stg.hashbite_ssk))
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_medical_oncology_stg
  );
    ...
    SELECT DISTINCT
      ...
      WHERE upper(stg.hashbite_ssk) NOT IN(
        SELECT
          ...
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_medical_oncology
      )
  ) AS ms
  ON mt.cn_patient_medical_oncology_sid = ms.cn_patient_medical_oncology_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_medical_oncology_sid, treatment_type_id, medical_oncology_facility_id, core_record_type_id, med_spcl_physician_id, nav_patient_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, navigator_id, coid, company_code, core_record_date, treatment_start_date, treatment_end_date, estimated_end_date, drug_name, dose_dense_chemo_ind, drug_dose_amt_text, drug_dose_measurement_text, drug_available_ind, drug_qty, cycle_num, cycle_frequency_text, medical_oncology_reason_text, terminated_ind, treatment_therapy_schedule_code, comment_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
