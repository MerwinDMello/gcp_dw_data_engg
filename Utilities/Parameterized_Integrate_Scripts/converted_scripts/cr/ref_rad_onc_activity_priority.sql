...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_activity_priority AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(src.activity_priority_desc)) AS activity_priority_sk,
      ...
      FROM
        (
          SELECT DISTINCT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimactivitytransaction AS dhd
        ) AS src
      WHERE src.activity_priority_desc IS NOT NULL
  ) AS ms
  ON mt.activity_priority_sk = ms.activity_priority_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (activity_priority_sk, activity_priority_desc, source_system_code, dw_last_update_date_time)
