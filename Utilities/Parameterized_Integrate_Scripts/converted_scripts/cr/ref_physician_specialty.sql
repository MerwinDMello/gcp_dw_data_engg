...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_Ref_Physician_Speciality;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_physician_specialty AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(trim(type_stg.physician_specialty_desc))) + (
          SELECT
              coalesce(max(ref_physician_specialty.physician_specialty_id), 0) AS id1
            FROM
              `hca-hin-dev-cur-ops`.edwcr.ref_physician_specialty
        ) AS physician_specialty_id,
      ...
      FROM
        (
          SELECT
              physician_specialty_desc AS physician_specialty_desc,
              ref_physician_specialty_stg.source_system_code AS source_system_code
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.ref_physician_specialty_stg
              CROSS JOIN UNNEST(ARRAY[
                trim(ref_physician_specialty_stg.physician_speciality_desc)
              ]) AS physician_specialty_desc
            WHERE upper(trim(physician_specialty_desc)) NOT IN(
              SELECT
                  upper(trim(ref_physician_specialty.physician_specialty_desc))
                FROM
                  `hca-hin-dev-cur-ops`.edwcr.ref_physician_specialty
            )
        ) AS type_stg
  ) AS ms
  ON mt.physician_specialty_id = ms.physician_specialty_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (physician_specialty_id, physician_specialty_desc, source_system_code, dw_last_update_date_time)
