...
-- ##################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=$Job_Name;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient_plan AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(dp.dimsiteid), upper(dp.dimplanid)) AS patient_plan_sk,
        rpp.plan_purpose_sk,
      ...
      FROM
        ...
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient_course AS rpc ON rpc.source_patient_course_id = CAST(`hca-hin-dev-cur-ops`.bqutil_fns.cw_td_normalize_number(dp.dimcourseid) as FLOAT64)
         AND rpc.site_sk = rr.site_sk
  ) AS ms
  ON mt.patient_plan_sk = ms.patient_plan_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (patient_plan_sk, plan_purpose_sk, site_sk, source_patient_plan_id, patient_course_sk, plan_status_id, plan_unique_id_text, plan_creation_date_time, treatment_start_date_time, treatment_end_date_time, history_user_name, history_date_time, log_id, run_id, source_system_code, dw_last_update_date_time)
