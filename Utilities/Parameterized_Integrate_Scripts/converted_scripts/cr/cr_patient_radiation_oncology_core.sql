...
-- #########################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_PATIENT_RADIATION_ONCOLOGY;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_radiation_oncology AS mt USING (
    SELECT DISTINCT
        cr_patient_radiation_oncology_wrk.radiation_id,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_radiation_oncology_wrk
  ) AS ms
  ON mt.radiation_id = ms.radiation_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (radiation_id, treatment_id, radiation_type_id, radiation_hospital_id, radiation_treatment_start_date, radiation_treatment_end_date, source_system_code, dw_last_update_date_time)
