...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PHYSICIAN_ROLE;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_physician_role AS mt USING (
    SELECT DISTINCT
        ab.physician_id,
      ...
      FROM
        (
          SELECT
              cn_physician_role_stg.physician_id,
              cn_physician_role_stg.physician_role_code
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.cn_physician_role_stg
          UNION DISTINCT
          SELECT
          ...
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr.cn_physician_detail
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr.cn_patient_tumor ON cn_physician_detail.physician_id = cn_patient_tumor.treatment_end_physician_id
          UNION DISTINCT
          SELECT
              cn_physician_detail.physician_id,
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr.cn_physician_detail
              INNER JOIN `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme ON cn_physician_detail.physician_id = cn_patient_heme.hematologist_physician_id
        ) AS ab
      WHERE (ab.physician_id, upper(ab.physician_role_code)) NOT IN(
        SELECT AS STRUCT
            cn_physician_role.physician_id,
            upper(cn_physician_role.physician_role_code) AS physician_role_code
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_physician_role
      )
  ) AS ms
  ON mt.physician_id = ms.physician_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (physician_id, physician_role_code, source_system_code, dw_last_update_date_time)
