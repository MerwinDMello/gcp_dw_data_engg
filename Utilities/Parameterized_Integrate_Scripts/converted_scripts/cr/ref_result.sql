...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_REF_RESULT;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_result AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY upper(trim(type_stg.nav_result_desc))) + (
          SELECT
              coalesce(max(ref_result.nav_result_id), 0) AS id1
            FROM
  ...
  ON mt.nav_result_id = ms.nav_result_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (nav_result_id, nav_result_desc, source_system_code, dw_last_update_date_time)
