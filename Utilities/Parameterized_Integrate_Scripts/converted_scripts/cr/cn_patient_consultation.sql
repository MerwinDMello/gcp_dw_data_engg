...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_CONSULTATION;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_consultation WHERE upper(cn_patient_consultation.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_consultation_stg.hashbite_ssk) AS hashbite_ssk
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_consultation_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_consultation AS mt USING (
    SELECT DISTINCT
        cn_patient_consultation_stg.cn_patient_consultation_sid,
      ...
      WHERE upper(cn_patient_consultation_stg.hashbite_ssk) NOT IN(
        SELECT
            upper(cn_patient_consultation.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_consultation
      )
  ) AS ms
  ON mt.cn_patient_consultation_sid = ms.cn_patient_consultation_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_consultation_sid, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, navigator_id, consult_type_id, nav_patient_id, coid, company_code, med_spcl_physician_id, consult_other_type_text, consult_date, consult_phone_num, consult_notes_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
