...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_REF_DISEASE_ASSESS_SOURCE ;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_disease_assess_source AS mt USING (
    SELECT DISTINCT
        (
          SELECT
              coalesce(max(ref_disease_assess_source.disease_assess_source_id), 0) AS maxid
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.ref_disease_assess_source
        ) + row_number() OVER (ORDER BY upper(trim(ssc.disease_assess_source_name))) AS disease_assess_source_id,
      ...
      FROM
        (
          SELECT DISTINCT
              trim(patient_heme_disease_assessment_stg.source) AS disease_assess_source_name
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.patient_heme_disease_assessment_stg
            WHERE trim(patient_heme_disease_assessment_stg.source) IS NOT NULL
        ) AS ssc
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.ref_disease_assess_source AS rtt ON upper(trim(ssc.disease_assess_source_name)) = upper(trim(rtt.disease_assess_source_name))
      WHERE trim(rtt.disease_assess_source_name) IS NULL
  ) AS ms
  ON mt.disease_assess_source_id = ms.disease_assess_source_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (disease_assess_source_id, disease_assess_source_name, source_system_code, dw_last_update_date_time)
