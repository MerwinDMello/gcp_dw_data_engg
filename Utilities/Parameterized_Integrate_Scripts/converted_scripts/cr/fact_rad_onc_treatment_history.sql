...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_Fact_rad_Onc_Treatment_History;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.fact_rad_onc_treatment_history AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY CAST(dp.dimsiteid as INT64), CAST(dp.facttreatmenthistoryid as INT64)) AS fact_treatment_history_sk,
      ...
      FROM
        (
          SELECT
              facttreatmenthist_stg.dimsiteid,
              ...
              CASE
                 facttreatmenthist_stg.isactive
                WHEN 1 THEN 'Y'
                WHEN 0 THEN 'N'
              END AS active_ind,
              ...
              CASE
                 facttreatmenthist_stg.primaryflag
                WHEN 1 THEN 'Y'
                WHEN 0 THEN 'N'
              END AS primary_ind,
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.facttreatmenthist_stg
        ) AS dp
        INNER JOIN (
          SELECT
        ...
        LEFT OUTER JOIN (
          SELECT
              rad_onc_patient_course.patient_course_sk AS patient_course_sk,
        ...
        ) AS ra ON dp.dimcourseid = ra.source_patient_course_id
        ...
        LEFT OUTER JOIN (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.rad_onc_patient_plan
        ) AS pp ON dp.dimplanid = pp.source_patient_plan_id
        ...
        ) AS ra1 ON dp.dimpatientid = ra1.source_patient_id
         AND rr.site_sk = ra1.site_sk
  ) AS ms
  ON mt.fact_treatment_history_sk = ms.fact_treatment_history_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (fact_treatment_history_sk, patient_course_sk, patient_plan_sk, patient_sk, treatment_intent_type_id, clinical_status_id, plan_status_id, field_technique_id, technique_id, technique_label_id, technique_delivery_type_id, site_sk, source_fact_treatment_history_id, completion_date_time, first_treatment_date_time, last_treatment_date_time, status_date_time, active_ind, planned_dose_rate_num, course_dose_delivered_amt, course_dose_planned_amt, course_dose_remaining_amt, other_course_dose_delivered_amt, dose_correction_amt, total_dose_limit_amt, daily_dose_limit_amt, session_dose_limit_amt, primary_ind, log_id, run_id, source_system_code, dw_last_update_date_time)
