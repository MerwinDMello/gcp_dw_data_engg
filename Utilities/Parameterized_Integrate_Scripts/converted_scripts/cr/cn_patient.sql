...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient WHERE cn_patient.nav_patient_id NOT IN(
    SELECT
        cn_patient_stg.nav_patient_id
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient AS mt USING (
    SELECT DISTINCT
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_stg AS stg
        ...
        ) AS cf ON upper(rtrim(stg.coid)) = upper(rtrim(cf.coid))
      WHERE stg.nav_patient_id NOT IN(
        SELECT
            -- where STG.COID is NOT NULL
            cn_patient.nav_patient_id
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient
      )
  ...
  ON mt.nav_patient_id = ms.nav_patient_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (nav_patient_id, navigator_id, coid, company_code, patient_market_urn, medical_record_num, empi_text, gynecologist_physician_id, primary_care_physician_id, facility_mnemonic_cs, network_mnemonic_cs, nav_create_date, source_system_code, dw_last_update_date_time)
