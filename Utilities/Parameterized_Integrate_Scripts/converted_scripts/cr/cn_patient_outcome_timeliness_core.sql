...
-- ####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_Patient_Outcome_Timeliness_CORE;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cancer_patient_outcome_timeliness AS mt USING (
    SELECT DISTINCT
        stg.cancer_patient_tumor_driver_sk,
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_outcome_timeliness_stg AS stg
  ) AS ms
