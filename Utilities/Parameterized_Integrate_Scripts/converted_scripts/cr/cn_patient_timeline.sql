...
-- #####################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_TIMELINE;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_timeline WHERE upper(cn_patient_timeline.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_timeline_stg.hashbite_ssk) AS hashbite_ssk
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_timeline_stg
  );
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_timeline AS mt USING (
    SELECT DISTINCT
        cn_patient_timeline_stg.cn_patient_timeline_id,
      ...
      WHERE upper(cn_patient_timeline_stg.hashbite_ssk) NOT IN(
        SELECT
            upper(cn_patient_timeline.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_timeline
      )
  ) AS ms
  ON mt.cn_patient_timeline_id = ms.cn_patient_timeline_id
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_timeline_id, nav_patient_id, tumor_type_id, navigator_id, coid, company_code, nav_referred_date, first_treatment_date, first_consult_date, first_imaging_date, first_medical_oncology_date, first_radiation_oncology_date, first_diagnosis_date, first_biopsy_date, first_surgery_consult_date, first_surgery_date, survivorship_care_plan_close_date, survivorship_care_plan_resolve_date, end_treatment_date, death_date, diagnosis_first_treatment_day_num, diagnosis_first_treatment_available_ind, hashbite_ssk, source_system_code, dw_last_update_date_time)
