...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient_course AS mt USING (
    SELECT DISTINCT
        row_number() OVER (ORDER BY dp.dimsiteid, dp.dimcourseid) AS patient_course_sk,
      ...
      WHERE ra.site_sk IS NOT NULL
  ) AS ms
  ON mt.patient_course_sk = ms.patient_course_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (patient_course_sk, patient_sk, clinical_status_id, treatment_intent_type_id, site_sk, source_patient_course_id, course_id_text, course_start_date_time, course_session_planned_num, course_session_delivered_num, course_session_remaining_num, dose_delivered_amt, course_duration_num, comment_text, log_id, run_id, source_system_code, dw_last_update_date_time)
