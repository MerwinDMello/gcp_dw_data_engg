...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_COMPLICATION;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_complication WHERE upper(cn_patient_complication.hashbite_ssk) NOT IN(
    SELECT
        upper(cn_patient_complication_stg.hashbite_ssk) AS hashbite_ssk
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_complication_stg
  );
...
BEGIN
  SET _ERROR_CODE = 0;
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_complication AS mt USING (
    SELECT DISTINCT
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_complication_stg AS stg
        LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr.ref_therapy_type AS rtt ON upper(rtrim(coalesce(trim(stg.associatetherapytype), 'X'))) = upper(rtrim(coalesce(trim(rtt.therapy_type_desc), 'X')))
      ...
      WHERE upper(stg.hashbite_ssk) NOT IN(
        SELECT
            upper(cn_patient_complication.hashbite_ssk) AS hashbite_ssk
          FROM
            `hca-hin-dev-cur-ops`.edwcr.cn_patient_complication
      )
  ) AS ms
  ON mt.cn_patient_complication_sid = ms.cn_patient_complication_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_complication_sid, nav_patient_id, core_record_type_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, navigator_id, coid, company_code, complication_date, therapy_type_id, treatment_stopped_ind, outcome_result_id, complication_text, specific_complication_text, comment_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
