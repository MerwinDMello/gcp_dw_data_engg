...
-- #####################################################################################
-- bteq << EOF > $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_PATIENT_PROCEDURE;' FOR SESSION;
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_procedure WHERE upper(trim(cn_patient_procedure.hashbite_ssk)) NOT IN(
    SELECT
        upper(trim(cn_patient_procedure_stg.hashbite_ssk))
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_procedure_stg
  );
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_procedure AS mt USING (
    SELECT DISTINCT
      ...
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_procedure_stg AS stg
      ...
      WHERE upper(trim(stg.hashbite_ssk)) NOT IN(
        SELECT
            upper(trim(cn_patient_procedure.hashbite_ssk))
          FROM
            `hca-hin-dev-cur-ops`.edwcr_base_views.cn_patient_procedure
      )
  ) AS ms
  ON mt.cn_patient_procedure_sid = ms.cn_patient_procedure_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_procedure_sid, core_record_type_id, procedure_type_id, med_spcl_physician_id, nav_patient_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, navigator_id, coid, company_code, procedure_date, palliative_ind, hashbite_ssk, source_system_code, dw_last_update_date_time)
