...
*==========================================================================*/
  BEGIN
    TRUNCATE TABLE edwim.epms_provider_activity;
            ...
            SELECT
                ...
                CASE
                  WHEN upper(rtrim(ep.source_system_code)) = 'R'
                   AND rtrim(cfs.fac_category) IN(
                    -- WHEN  (MT3.PROVIDER_TYPE_FCLT = '1hcaResdnt' AND CFS.FAC_Status = 'Applicant')
                    -- 		THEN 1
                    -- WHEN  (MT3.PROVIDER_TYPE_FCLT = '1hcaFellow'  AND CFS.FAC_Status = 'Applicant')
                    -- 		THEN 1
                    'Active', 'Associate/Affiliate', 'Consulting', 'Courtesy', 'Privileges without Membership'
                  )
                   AND rtrim(cfs.fac_status) IN(
                    'Applicant', 'Suspend - Controlled Drugs'
                  )
                ...
                CASE
                  WHEN upper(rtrim(ep.source_system_code)) = 'E'
                   AND CAST(bqutil.fn.cw_td_normalize_number(dea.cactus_active_dea_license_sw) as FLOAT64) = 1 THEN 1
                  ELSE 0
                END AS cactus_active_dea_license_sw,
                CASE
                   upper(rtrim(mt3.provider_type_fclt))
                ...
                CASE
                  ...
                  ELSE NULL
                END AS mt_active_txf_provider_sw,
                -- WHEN (MT3.NETWORK_MNEMONIC = 'TXF' AND MT3.PROVIDER_TYPE_FCLT = '2hcaAmbul')
                -- 	THEN 1
                datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
              FROM
                (
                  SELECT
                      im.im_domain_name AS epms_prac_code,
                      epms.im_domain_id,
                      st.state_code,
                      epms.epms_prov_npi,
                    ...
                    FROM
                      edwim_base_views.epms_provider_status AS epms
                      INNER JOIN edwim_base_views.ref_im_domain AS im ON epms.im_domain_id = im.im_domain_id
                      INNER JOIN (
                        SELECT DISTINCT
                            company_master.unit_num,
                            company_master.state_code
                          FROM
                            edwim_views.company_master
                          WHERE rtrim(coalesce(company_master.state_code, '')) <> ''
                          QUALIFY row_number() OVER (PARTITION BY company_master.unit_num, company_master.state_code ORDER BY 2) = 1
                      ) AS st ON im.im_domain_name = st.unit_num
                    WHERE rtrim(coalesce(epms.epms_user_id, '')) <> ''
                     AND upper(rtrim(epms.epms_prov_active_flag)) = 'Y'
                ) AS ep
                LEFT OUTER JOIN /*LEFT JOIN ( --CACTUS FACILITY STATUS
                										...
                										SELECT	
                													...
                													(
                																	...
                																	LEFT OUTER JOIN  EDW_PUB_VIEWS.Fact_Facility fac ON e1.Coid = fac.Coid AND fac.Company_Code = e1.Company_Code 
                							...
                							AND EP.EPMS_Prac_Code =  CFS.Unit_Number*/
                -- CACTUS FACILITY STATUS
                (
                  SELECT
                      CASE
                        WHEN cmap.coid_shared IS NULL
                         AND rtrim(cp.coid) IN(
                          '08158', '08165'
                        ) THEN '08158'
                        ELSE coalesce(cmap.coid_shared, cp.coid)
                      END AS coid,
                      ...
                      cp.facility_effective_to_date,
                      CASE
                        WHEN rtrim(cp.fac_category) IN(
                          'Active', 'Associate/Affiliate', 'Consulting', 'Courtesy', 'Privileges without Membership'
                        )
                         AND rtrim(cp.fac_status) IN(
                          'Applicant', 'Current', 'Temporary', 'Suspend - Select', 'Suspend - Admitting'
                        )
                    ...
                    FROM
                      (
                        SELECT
                          ...
                          FROM
                            ...
                            LEFT OUTER JOIN edw_pub_views.fact_facility AS fac ON e1.coid = fac.coid
                      ...
                      LEFT OUTER JOIN (
                        SELECT
                            ...
                            com.unit_num AS shared_unit_number
                          FROM
                            edwim_base_views.ref_attribute_value AS rav
                            INNER JOIN edwim_views.company_master AS com ON rtrim(substr(attribute_text, 7, 5)) = rtrim(com.coid)
                          WHERE rav.attribute_id = 14
                           AND upper(rtrim(rav.attribute_active_ind)) = 'Y'
                      ) AS cmap ON rtrim(cp.coid) = rtrim(cmap.coid)
                    QUALIFY row_number() OVER (PARTITION BY coid, cp.npi_number ORDER BY cactus_privileged_provider_sw DESC, CASE
                      WHEN rtrim(cp.fac_category) IN(
                        'Active', 'Associate/Affiliate', 'Consulting', 'Courtesy', 'Privileges without Membership'
                      )
                       AND rtrim(cp.fac_status) IN(
                      ...
                      ELSE 0
                    END DESC) = 1
                ) AS cfs ON SAFE_CAST(ep.epms_prov_npi AS NUMERIC) = cfs.npi_number
                    ...
                    FROM
                      (
                        SELECT
                            hcp.hcp_npi AS npi_number,
                            -- d1.HCP_DW_Id,
                            d1.hcp_licn_persistent_vfy AS dea_license_persistent_verification,
                            -- e2.Coid,
                            ...
                            END AS dea_license_expiration_date,
                            -- Coalesce(d1.HCP_Licn_Expiration_Date,  d1.HCP_Licn_Verbal_Expr_Date) AS Expiration_Date,
                          ...
                          FROM
                            edwim_base_views.hcp_license AS d1
                            ...
                            INNER JOIN edw_pub_views.facility_address AS ad ON e2.coid = ad.coid
                          ...
                          WHERE upper(rtrim(d1.active_dw_ind)) = 'Y'
                           ...
                           AND upper(trim(coalesce(s1.licn_stts_desc, ''))) IN(
                            'ACTIVE', 'TEMPORARY PERMIT', 'ACTIVE MILITARY', 'CPC - UNDER REVIEW', 'EXEMPT', 'EXPIRED', 'LIMITED', 'PENDING', 'RESTRICTED', 'REVOKED', 'SUSPENDED'
                          )
                           AND coalesce(hcp.hcp_npi, 0) <> 0
                          QUALIFY row_number() OVER (PARTITION BY npi_number, d1.state_code ORDER BY CASE
                            WHEN upper(trim(coalesce(dea_license_persistent_verification, ''))) = 'Y' THEN 1
                            ELSE 0
                          END DESC, dea_license_expiration_date DESC, coalesce(license_award_date, parse_date('%m/%d/%Y', '01/01/1950')) DESC) = 1
                      ) AS dea_lic
                    WHERE dea_lic.dea_license_expiration_date >= date_sub(current_date(), interval 3 DAY)
                ) AS dea ON SAFE_CAST(ep.epms_prov_npi AS NUMERIC) = dea.npi_number
                ...
                (
                  SELECT
                      CAST(/* expression of unknown or erroneous type */ prc.prctnr_ssk_cs as STRING) AS provider_mnemonic,
                      ...
                      fclt.unit_number
                    FROM
                      edwim_base_views.prctnr_ent AS prc
                      INNER JOIN (
                        SELECT
                            CAST(/* expression of unknown or erroneous type */ fac.ntwk_ssk_cs as STRING) AS ntwk_ssk_cs,
                          ...
                          FROM
                            edwim_base_views.prctnr_fclt_ent AS fac
                            INNER JOIN edwim_views.company_master AS comp ON fac.company_code = comp.company_code
                             AND fac.coid = comp.coid
                          WHERE fac.vld_to_ts = '9999-12-31 00:00:00'
                          QUALIFY row_number() OVER (PARTITION BY upper(fac.ntwk_ssk_cs), upper(fac.fclt_ssk_cs), upper(fac.prctnr_ssk_cs) ORDER BY fac.src_lst_upd_ts DESC, fac.vld_to_ts DESC, fac.vld_fr_ts DESC) = 1
                      ) AS fclt ON upper(rtrim(CAST(/* expression of unknown or erroneous type */ prc.ntwk_ssk_cs as STRING))) = upper(rtrim(fclt.ntwk_ssk_cs))
                      ...
                      (
                        SELECT
                            prc_0.role_plyr_sk,
                            fclt_0.coid,
                            ROUND(CAST(bqutil.fn.cw_td_normalize_number(trim(npi.id_txt)) as NUMERIC), 0, 'ROUND_HALF_EVEN') AS npi_number
                          FROM
                            edwim_base_views.prctnr_dtl AS prc_0
                            INNER JOIN edwim_base_views.prctnr_fclt_dtl AS fclt_0 ON prc_0.role_plyr_sk = fclt_0.role_plyr_sk
                             ...
                             AND rtrim(fclt_0.src_sys_ref_cd) IN(
                              'MIS_PROVIDER'
                            )
                            ...
                            (
                              SELECT
                                ...
                                FROM
                                  edwim_base_views.prctnr_role_idfn AS hcp
                                  INNER JOIN edw_pub_views.nppes_provider AS nppes ON trim(hcp.id_txt) = rtrim(CAST(/* expression of unknown or erroneous type */ nppes.npi as STRING))
                                WHERE rtrim(hcp.src_sys_ref_cd) IN(
                                  'MIS_PROVIDER', 'MIS'
                                )
                                 AND hcp.vld_to_ts = '9999-12-31 00:00:00'
                                 AND upper(rtrim(hcp.registn_type_ref_cd)) = 'NPI'
                                QUALIFY row_number() OVER (PARTITION BY hcp.role_plyr_sk ORDER BY hcp.vld_to_ts DESC, hcp.vld_fr_ts DESC) = 1
                            ) AS npi ON npi.role_plyr_sk = prc_0.role_plyr_sk
                          QUALIFY row_number() OVER (PARTITION BY fclt_0.coid, trim(npi.id_txt) ORDER BY fclt_0.vld_to_ts DESC, fclt_0.vld_fr_ts DESC) = 1
                      ) AS bynpi ON upper(rtrim(fclt.coid)) = upper(rtrim(bynpi.coid))
                      ...
                      (
                        SELECT
                            prc_0.role_plyr_sk,
                            -- cl.NETWORK_MNEMONIC_CS AS Network_Mnemonic,
                            fclt_0.coid,
                          ...
                          FROM
                            ...
                            (
                              SELECT
                                  prctnr_role_idfn.role_plyr_sk,
                                  prctnr_role_idfn.id_txt AS provider_mnemonic,
                                  prctnr_role_idfn.registn_type_ref_cd
                                FROM
                                  edwim_base_views.prctnr_role_idfn
                                WHERE rtrim(prctnr_role_idfn.src_sys_ref_cd) IN(
                                  'MIS_PROVIDER', 'MIS'
                                )
                                 AND prctnr_role_idfn.vld_to_ts = '9999-12-31 00:00:00'
                                 AND upper(rtrim(prctnr_role_idfn.registn_type_ref_cd)) = 'MNEMONIC'
                                QUALIFY row_number() OVER (PARTITION BY role_plyr_sk ORDER BY prctnr_role_idfn.vld_to_ts DESC, prctnr_role_idfn.vld_fr_ts DESC) = 1
                            ) AS mnem ON mnem.role_plyr_sk = fclt_0.role_plyr_sk
                          QUALIFY row_number() OVER (PARTITION BY fclt_0.coid, provider_mnemonic ORDER BY fclt_0.vld_to_ts DESC, fclt_0.vld_fr_ts DESC) = 1
                      ) AS bymnemonic ON upper(rtrim(fclt.coid)) = upper(rtrim(bymnemonic.coid))
                    ...
                    WHERE prc.vld_to_ts = '9999-12-31 00:00:00'
                     ...
                     AND rtrim(CAST(/* expression of unknown or erroneous type */ prc.natnl_prvdr_id as STRING)) <> ''
                    QUALIFY row_number() OVER (PARTITION BY upper(fclt.coid), upper(fclt.fclt_ssk_cs), bynpi.npi_number ORDER BY prc.vld_to_ts DESC, CASE
                      WHEN upper(rtrim(fclt.provider_type_fclt)) IN(
                        -- QUALIFY Row_Number() Over(PARTITION BY fclt.COID, fclt.FCLT_SSK_CS, Coalesce(byMnemonic.Role_Plyr_Sk, byNPI.Role_Plyr_Sk) ORDER BY prc.SRC_LST_UPD_TS DESC, prc.VLD_TO_TS DESC, prc.VLD_FR_TS DESC) = 1
                        '1HCARESDNT', '1HCAFELLOW'
                      ) THEN 1
                      WHEN upper(rtrim(network_mnemonic)) = 'TXF'
                       AND upper(rtrim(fclt.provider_type_fclt)) IN(
                        '2HCAACTIVE', '2HCAPRVNOM', '2HCAASOCAF', '2HCACOURTS', '2HCACONSUL'
                      ) THEN 2
                      ELSE 3
                    END, prc.vld_fr_ts DESC) = 1
                ) AS mt3 ON upper(rtrim(ep.epms_prov_npi)) = upper(trim(coalesce(mt3.npi_number, '')))
                 AND upper(rtrim(ep.epms_prac_code)) = upper(rtrim(CASE
                  WHEN mt3.unit_number = ep.epms_prac_code THEN mt3.unit_number
                  ELSE mt3.coid
                END))
          ) AS ep2
        QUALIFY row_number() OVER (PARTITION BY ep2.im_domain_id, ep2.epms_user_id, ep2.esaf_activity_date, ep2.source_system_code ORDER BY CASE
           1
