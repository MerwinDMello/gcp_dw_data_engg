...
*==========================================================================*/
  BEGIN
    TRUNCATE TABLE edwim.epms_provider_status;
    INSERT INTO edwim.epms_provider_status (im_domain_id, epms_prov_npi, source_system_code, epms_user_id, epms_prov_last_name, epms_prov_first_name, epms_prov_enabled_date, epms_prov_active_flag, dw_last_update_date_time)
          ...
          (
            SELECT DISTINCT
              ...
              FROM
                ...
                (
                  SELECT
                      rc1.provider_npi AS epms_prov_npi,
                    ...
                    FROM
                      edwim_staging.rcopia_provider_status_stg AS rc1
                      INNER JOIN (
                        SELECT
                            bqutil.fn.cw_td_strtok(av_0.attribute_text, ':', 1) AS unit_number,
                            bqutil.fn.cw_td_strtok(av_0.attribute_text, ':', 2) AS facility_name
                          FROM
                            edwim_base_views.ref_attribute_value AS av_0
                          WHERE av_0.attribute_id = 15
                      ) AS av ON rtrim(rc1.practice_name) = rtrim(av.facility_name)
                    WHERE rc1.provider_npi IS NOT NULL
                ) AS rc2
                ...
                LEFT OUTER JOIN edwim_base_views.ad_account AS ad ON SAFE_CAST(rc2.epms_prov_npi AS NUMERIC) = ad.ad_account_npi
                 AND ad.im_domain_id = 2
          ) AS rc3
        QUALIFY row_number() OVER (PARTITION BY rc3.im_domain_id, rc3.epms_prov_npi, upper(rc3.source_system_code) ORDER BY CASE
      ...
      SELECT DISTINCT
          ...
          (
            SELECT DISTINCT
              ...
              FROM
                ...
                (
                  SELECT
                      epcs.provider_npi AS epms_prov_npi,
                      ...
                      av.unit_number AS epcs_practice_code
                    FROM
                      edwim_staging.epcs_provider_status_stg AS epcs
                      INNER JOIN (
                        SELECT
                            bqutil.fn.cw_td_strtok(av_0.attribute_text, ':', 1) AS unit_number,
                            bqutil.fn.cw_td_strtok(av_0.attribute_text, ':', 2) AS facility_name
                          FROM
                            edwim_base_views.ref_attribute_value AS av_0
                          WHERE av_0.attribute_id = 15
                      ) AS av ON rtrim(epcs.source_name) = rtrim(av.facility_name)
                    WHERE epcs.provider_npi IS NOT NULL
                ) AS epcs2
        ...
        QUALIFY row_number() OVER (PARTITION BY epcs3.im_domain_id, epcs3.epms_prov_npi, upper(epcs3.source_system_code) ORDER BY CASE
