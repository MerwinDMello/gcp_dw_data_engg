...
*==========================================================================*/
  BEGIN
    DELETE FROM edwim.im_person_activity WHERE rtrim(im_person_activity.source_system_code) IN(
      'R', 'E', 'D'
    );
              ...
              FROM
                (
                  SELECT
                      ...
                      ep.im_domain_id,
                      ep.epms_user_id,
                      ep.esaf_activity_date,
                      -- BEGIN RULES--
                      CASE
                        WHEN trim(coalesce(imd.im_domain_name, '')) IN(
                          '25901', '00322'
                        ) THEN 0
                    ...
                    FROM
                      edwim_base_views.epms_provider_activity AS ep
                          ...
                          FROM
                            (
                              SELECT
                                ...
                                FROM
                                  edwim_base_views.hr_job_code AS jbcode
                                ...
                                WHERE rtrim(jclcode.job_class_code) = '103'
                                 ...
                                 AND upper(rtrim(jbcode.active_dw_ind)) = 'Y'
                            ) AS jcd
                            INNER JOIN (
                              SELECT
                                  hr_job_position.position_sid,
                                  hr_job_position.lawson_company_num,
                                  hr_job_position.process_level_code,
                                  hr_job_position.job_code_sid
                                FROM
                                  edwim_base_views.hr_job_position
                                WHERE CAST(/* expression of unknown or erroneous type */ hr_job_position.eff_to_date as DATE) = DATE '9999-12-31'
                                 ...
                                 AND upper(rtrim(hr_job_position.active_dw_ind)) = 'Y'
                            ) AS jpos ON jcd.job_code_sid = jpos.job_code_sid
                            INNER JOIN (
                              SELECT
                                  hr_employee_position.employee_sid,
                                  hr_employee_position.lawson_company_num,
                                  hr_employee_position.process_level_code,
                                  hr_employee_position.position_sid
                                FROM
                                  edwim_base_views.hr_employee_position
                            ...
                            INNER JOIN (
                              SELECT
                                ...
                                WHERE CAST(/* expression of unknown or erroneous type */ hr_employee.valid_to_date as DATE) = DATE '9999-12-31'
                                 ...
                                 AND upper(rtrim(hr_employee.source_system_code)) = 'L'
                            ) AS hre ON hre.employee_sid = pos.employee_sid
                             AND hre.lawson_company_num = pos.lawson_company_num
                             AND hre.process_level_code = pos.process_level_code
                          GROUP BY 1
                      ) AS ljc ON ep.epms_user_id = ljc.employee_34_login_code
                  ...
                  SELECT
                    ...
                    FROM
                      ...
                      LEFT OUTER JOIN (
                        SELECT
                          ...
                          FROM
                            edwim_base_views.provider_license AS pl
                            ...
                            INNER JOIN (
                              SELECT
                                ...
                                FROM
                                  edwim_base_views.ref_im_domain AS imd_0
                                ...
                                GROUP BY 1, 2, upper(trim(coalesce(fac_0.facility_state_code, '')))
                            ) AS fac ON upper(rtrim(fac.facility_state_code)) = upper(trim(coalesce(pl.licn_state_name, '')))
                          ...
                          GROUP BY 1, upper(trim(coalesce(pd.provider_npi, '')))
                      ) AS mds ON mds.im_domain_id = ep.im_domain_id
                    ...
                    WHERE upper(rtrim(ep.source_system_code)) = 'E'
                ) AS ep2
          ) AS ep3
