...
-- ##################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CR_RO_Fact_Rad_Onc_Activity_Billing;' FOR SESSION;
      ...
      FROM
        ...
        LEFT OUTER JOIN (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.rad_onc_physician
        ) AS rop ON dp.dimdoctorid = rop.source_physician_id
        ...
        LEFT OUTER JOIN (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.rad_onc_physician
        ) AS rop1 ON dp.dimdoctid_attdoncologist = rop1.source_physician_id
        ...
        LEFT OUTER JOIN (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.rad_onc_patient_course
        ) AS pc ON dp.dimcourseid = pc.source_patient_course_id
        ...
        ) AS rh ON dp.dimhospitaldepartmentid = rh.source_hospital_id
        ...
        ) AS ra ON dp.dimactivityid = ra.source_activity_id
        ...
        LEFT OUTER JOIN (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.rad_onc_activity_transaction
        ) AS rat ON dp.dimactivitytransactionid = rat.source_activity_transaction_id
        ...
        LEFT OUTER JOIN (
          SELECT
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_base_views.ref_rad_onc_procedure_code
        ) AS rpp ON dp.dimprocedurecodeid = rpp.source_procedure_code_id
        ...
        ) AS rp ON dp.dimpatientid = rp.source_patient_id
         AND rr.site_sk = rp.site_sk
  ) AS ms
  ON mt.fact_activity_billing_sk = ms.fact_activity_billing_sk
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (fact_activity_billing_sk, physician_sk, attending_oncologist_id, patient_course_sk, hospital_sk, activity_sk, activity_transaction_sk, procedure_code_sk, patient_sk, activity_category_id, site_sk, source_fact_activity_billing_id, primary_global_charge_amt, primary_technical_charge_amt, primary_professional_charge_amt, other_professional_charge_amt, other_technical_charge_amt, other_global_charge_amt, forecast_charge_amt, actual_charge_amt, activity_cost_amt, activity_billing_code_text, service_start_date_time, service_end_date_time, complete_date_time, export_date_time, mark_complete_date_time, credit_export_date_time, credit_date_time, credit_note_text, modifier_code_text, credit_amt, scheduled_ind, object_status_ind, log_id, run_id, source_system_code, dw_last_update_date_time)
