...
-- #################################################################################
-- bteq << EOF >> $1;
-- SET QUERY_BAND = 'App=EDWCR_ETL; Job=J_CN_Patient_Heme_Clinical_Trial_stg;' FOR SESSION;
...
BEGIN
  ...
  MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_clinical_trial AS mt USING (
    SELECT DISTINCT
        iq.cn_patient_heme_clinical_trial_sid AS cn_patient_heme_clinical_trial_sid,
      ...
      FROM
        (
          SELECT
              stg.patientclinicaltrialfactid AS cn_patient_heme_clinical_trial_sid,
              ...
              'H' AS company_code,
              CASE
                 upper(trim(stg.hemeevaluated_screenedind))
                WHEN 'YES' THEN 'Y'
                WHEN 'NO' THEN 'N'
              ...
              CASE
                 upper(trim(stg.hemepatientenrolledinclinicaltrial))
                WHEN 'YES' THEN 'Y'
                WHEN 'NO' THEN 'N'
                ELSE CAST(NULL as STRING)
              ...
              CASE
                 upper(trim(stg.hemeofferedclinicaltrialind))
                WHEN 'YES' THEN 'Y'
                WHEN 'NO' THEN 'N'
                ELSE CAST(NULL as STRING)
            ...
            FROM
              `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_heme_clinical_trial_stg AS stg
        ) AS iq
      ...
      WHERE trim(cphc.hashbite_ssk) IS NULL
  ) AS ms
  ON mt.cn_patient_heme_clinical_trial_sid = ms.cn_patient_heme_clinical_trial_sid
   ...
   AND mt.dw_last_update_date_time = ms.dw_last_update_date_time
     WHEN NOT MATCHED BY TARGET THEN
      INSERT (cn_patient_heme_clinical_trial_sid, nav_patient_id, tumor_type_id, diagnosis_result_id, nav_diagnosis_id, navigator_id, coid, company_code, clinical_trial_evaluated_ind, clinical_trial_evaluated_date, clinical_trial_enrolled_ind, clinical_trial_enrolled_date, clinical_trial_offered_ind, clinical_trial_offered_date, clinical_trial_not_offered_text, clinical_trial_not_offered_other_text, clinical_trial_name, clinical_trial_other_name, not_screened_reason_text, not_screened_other_reason_text, hashbite_ssk, source_system_code, dw_last_update_date_time)
...
BEGIN
  SET _ERROR_CODE = 0;
  DELETE FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_clinical_trial AS cphc WHERE NOT EXISTS (
    SELECT
        'X'
      FROM
        `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_heme_clinical_trial_stg AS sphc
