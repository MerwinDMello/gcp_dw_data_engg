...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme AS mt USING
  (SELECT DISTINCT stg.patienthemefactid,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_heme_stg AS stg
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme');
