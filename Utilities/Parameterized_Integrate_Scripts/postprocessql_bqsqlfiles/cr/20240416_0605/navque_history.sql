...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.navque_history AS mt USING
  (SELECT DISTINCT stg.navque_history_id,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.navque_history_stg AS stg
   WHERE upper(trim(stg.hashbite_ssk)) NOT IN
       (SELECT upper(trim(navque_history.hashbite_ssk))
        FROM `hca-hin-dev-cur-ops`.edwcr.navque_history) ) AS ms ON mt.navque_history_id = ms.navque_history_id
  ...
  (SELECT count(*)
   FROM
     (SELECT navque_history_id
      FROM `hca-hin-dev-cur-ops`.edwcr.navque_history
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.navque_history');
