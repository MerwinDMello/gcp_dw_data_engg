...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_complication
WHERE upper(cn_patient_complication.hashbite_ssk) NOT IN
    (SELECT upper(cn_patient_complication_stg.hashbite_ssk) AS hashbite_ssk
     FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_complication_stg);
  ...
  (SELECT DISTINCT stg.cn_patient_complication_sid,
   ...
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_complication.hashbite_ssk) AS hashbite_ssk
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_complication) ) AS ms ON mt.cn_patient_complication_sid = ms.cn_patient_complication_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_complication');
