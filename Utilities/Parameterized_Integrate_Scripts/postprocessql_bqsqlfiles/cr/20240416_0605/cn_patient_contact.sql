...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_contact AS mt USING
  (SELECT DISTINCT cpstg.cn_patient_contact_sid,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_contact_stg AS cpstg
   ...
   WHERE upper(cpstg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_contact.hashbite_ssk) AS hashbite_ssk
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_contact) ) AS ms ON mt.cn_patient_contact_sid = ms.cn_patient_contact_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_contact');
