...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_timeline
  ...
  (SELECT DISTINCT cn_patient_timeline_stg.cn_patient_timeline_id,
   ...
   WHERE upper(cn_patient_timeline_stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_timeline.hashbite_ssk) AS hashbite_ssk
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_timeline) ) AS ms ON mt.cn_patient_timeline_id = ms.cn_patient_timeline_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_timeline');
