...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.ref_rad_onc_site AS rr ON rr.source_site_id = dhd.dimsiteid) AS ms ON mt.patient_sk = ms.patient_sk
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT patient_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient
      GROUP BY patient_sk
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.rad_onc_patient');
