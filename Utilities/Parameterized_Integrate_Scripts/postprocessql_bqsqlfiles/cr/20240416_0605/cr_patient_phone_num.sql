...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_phone_num AS mt USING
  (SELECT DISTINCT coalesce(cr_patient_phone_num_stg.patient_id, -99) AS patient_id,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_phone_num_stg) AS ms ON mt.cr_patient_id = ms.patient_id
  ...
  (SELECT count(*)
   FROM
     (SELECT cr_patient_id,
             patient_contact_id,
             phone_num_type_code
      FROM `hca-hin-dev-cur-ops`.edwcr.cr_patient_phone_num
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cr_patient_phone_num');
