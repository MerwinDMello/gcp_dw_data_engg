...
TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr.cr_patient_address;
...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_address AS mt USING
  (SELECT DISTINCT cr_patient_address_wrk.cr_patient_id,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_address_wrk) AS ms ON mt.cr_patient_id = ms.cr_patient_id
  ...
  (SELECT count(*)
   FROM
     (SELECT cr_patient_id
      FROM `hca-hin-dev-cur-ops`.edwcr.cr_patient_address
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cr_patient_address');
