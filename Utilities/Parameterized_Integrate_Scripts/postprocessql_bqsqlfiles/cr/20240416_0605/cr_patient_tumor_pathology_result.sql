...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cr_patient_tumor_pathology_result AS mt USING
  (SELECT DISTINCT stg.patientid AS cr_patient_id,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cr_pat_tumor_pathology_result_stg AS stg
   LEFT OUTER JOIN
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT tumor_id,
             cr_patient_id
      FROM `hca-hin-dev-cur-ops`.edwcr.cr_patient_tumor_pathology_result
      GROUP BY tumor_id,
               cr_patient_id
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cr_patient_tumor_pathology_result');
