   ...
   FROM
     (SELECT CAST(NULL AS INT64) AS physician_sk,
      ...
      UNION DISTINCT SELECT CAST(NULL AS INT64) AS physician_sk,
                            ...
                            'R' AS source_system_code,
                            datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimdoctor) AS dim_sub
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT physician_sk,
             phone_num_type_code
      FROM `hca-hin-dev-cur-ops`.edwcr.rad_onc_junc_physician_phone
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.rad_onc_junc_physician_phone');
