...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_radiation_oncology
WHERE upper(trim(cn_patient_radiation_oncology.hashbite_ssk)) NOT IN
    (SELECT upper(trim(cn_patient_radiation_oncology_stg.hashbite_ssk))
     FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_radiation_oncology_stg);
...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cn_patient_radiation_oncology AS mt USING
  (SELECT DISTINCT stg.cn_patient_rad_oncology_sid,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_patient_radiation_oncology_stg AS stg
   ...
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_radiation_oncology.hashbite_ssk) AS hashbite_ssk
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_radiation_oncology) ) AS ms ON mt.cn_patient_radiation_oncology_sid = ms.cn_patient_rad_oncology_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_radiation_oncology');
