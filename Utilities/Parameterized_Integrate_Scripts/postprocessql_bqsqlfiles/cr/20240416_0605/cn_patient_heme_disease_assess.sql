  ...
  (SELECT DISTINCT CAST(iq.cn_patient_heme_sid AS INT64) AS cn_patient_heme_sid,
   ...
   FROM
     (SELECT phd.patienthemefactid AS cn_patient_heme_sid,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.patient_heme_disease_assessment_stg AS phd
  ...
  (SELECT count(*)
   FROM
     (SELECT cn_patient_heme_sid
      FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_disease_assess
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_disease_assess');
...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_disease_assess AS cphd
WHERE NOT EXISTS
    (SELECT 'X'
     FROM `hca-hin-dev-cur-ops`.edwcr_staging.patient_heme_disease_assessment_stg AS sphd
