...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_diagnosis_detail AS mt USING
  (SELECT DISTINCT row_number() OVER (
     ...
     (SELECT coalesce(max(ref_diagnosis_detail.diagnosis_detail_id), 0) AS id1
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_diagnosis_detail) AS diagnosis_detail_id,
   ...
   FROM
     (SELECT trim(ref_diagnosis_detail_stg.diagnosis_detail_desc) AS diagnosis_detail_desc,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.ref_diagnosis_detail_stg
      WHERE (upper(trim(ref_diagnosis_detail_stg.diagnosis_detail_desc)),
          ...
          (SELECT AS STRUCT upper(trim(ref_diagnosis_detail.diagnosis_detail_desc)),
           ...
           FROM `hca-hin-dev-cur-ops`.edwcr.ref_diagnosis_detail) ) AS type_stg) AS ms ON mt.diagnosis_detail_id = ms.diagnosis_detail_id
  ...
  (SELECT count(*)
   FROM
     (SELECT diagnosis_detail_id
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_diagnosis_detail
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_diagnosis_detail');
