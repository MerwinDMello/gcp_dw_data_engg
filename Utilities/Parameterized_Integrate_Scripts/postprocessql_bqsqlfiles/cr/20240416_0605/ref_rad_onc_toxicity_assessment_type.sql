  ...
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM
     (SELECT CASE
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.stg_factpatienttoxicity AS dhd QUALIFY row_number() OVER (PARTITION BY upper(toxicity_assessment_type_desc)
  ...
  (SELECT count(*)
   FROM
     (SELECT toxicity_assessment_type_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_toxicity_assessment_type
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_toxicity_assessment_type');
