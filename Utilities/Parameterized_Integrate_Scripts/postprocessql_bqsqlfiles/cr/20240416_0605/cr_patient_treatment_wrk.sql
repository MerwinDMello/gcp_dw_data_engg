...
FROM `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_treatment_stg AS trt
...
FROM
  (SELECT stg.treatmentid AS treatment_id,
          ...
          datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cr_patient_treatment_wrk1 AS stg) AS src
...
LEFT OUTER JOIN -- AND SRC.DESCRIPTION=TT.TREATMENT_TYPE_DESC
 /*LEFT JOIN  EDWCR.REF_LOOKUP_CODE RLC
        ...
        AND NM1.LOOKUP_NAME='SURGERY MARGIN RESULT'*/ `hca-hin-dev-cur-ops`.edwcr.ref_treatment_type_group AS tgtp ON upper(rtrim(src.rxtype)) = upper(rtrim(tgtp.treatment_type_group_code))
LEFT OUTER JOIN
