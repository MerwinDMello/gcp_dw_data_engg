...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_lookup AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.stg_dimlookup AS ds
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT lookup_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_lookup
      GROUP BY lookup_sk
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_rad_onc_lookup');
