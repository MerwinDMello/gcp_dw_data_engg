  ...
  (SELECT DISTINCT iq.cn_patient_heme_diagnosis_sid AS cn_patient_heme_diagnosis_sid,
   ...
   FROM
     (SELECT phd.patienthemediagnosisfactid AS cn_patient_heme_diagnosis_sid,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.stg_patienthemediagnosis AS phd
  ...
  (SELECT count(*)
   FROM
     (SELECT cn_patient_heme_diagnosis_sid
      FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_diagnosis
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_diagnosis');
...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_heme_diagnosis AS cphd
WHERE NOT EXISTS
    (SELECT 'X'
     FROM `hca-hin-dev-cur-ops`.edwcr_staging.stg_patienthemediagnosis AS sphd
