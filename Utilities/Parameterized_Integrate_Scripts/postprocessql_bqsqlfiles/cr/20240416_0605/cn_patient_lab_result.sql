  ...
  (SELECT DISTINCT cn_patient_lab_result_stg.nav_patient_lab_result_sid,
   ...
   WHERE upper(cn_patient_lab_result_stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_lab_result.hashbite_ssk) AS hashbite_ssk
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_patient_lab_result) ) AS ms ON mt.nav_patient_lab_result_sid = ms.nav_patient_lab_result_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_patient_lab_result');
