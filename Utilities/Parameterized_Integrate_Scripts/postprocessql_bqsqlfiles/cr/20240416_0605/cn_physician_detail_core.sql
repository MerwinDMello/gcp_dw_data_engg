  ...
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM
     (SELECT DISTINCT trim(cn_physician_detail_stg_wrk.physician_name) AS physician_name,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_physician_detail_stg_wrk
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_physician_detail');
