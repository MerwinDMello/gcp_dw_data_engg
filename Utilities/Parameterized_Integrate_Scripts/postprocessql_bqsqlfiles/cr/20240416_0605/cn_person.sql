...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr.cn_person
WHERE cn_person.nav_patient_id NOT IN
    (SELECT cn_person_stg.nav_patient_id
     FROM `hca-hin-dev-cur-ops`.edwcr_staging.cn_person_stg);
  ...
  (SELECT DISTINCT cn_person_stg.nav_patient_id,
   ...
   WHERE cn_person_stg.nav_patient_id NOT IN
       (SELECT cn_person.nav_patient_id
        FROM `hca-hin-dev-cur-ops`.edwcr.cn_person) ) AS ms ON mt.nav_patient_id = ms.nav_patient_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cn_person');
