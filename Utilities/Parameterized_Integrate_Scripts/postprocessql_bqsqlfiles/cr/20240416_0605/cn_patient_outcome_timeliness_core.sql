...
TRUNCATE TABLE `hca-hin-dev-cur-ops`.edwcr.cancer_patient_outcome_timeliness;
...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cancer_patient_outcome_timeliness AS mt USING
  (SELECT DISTINCT stg.cancer_patient_tumor_driver_sk,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_outcome_timeliness_stg AS stg) AS ms ON mt.cancer_patient_tumor_driver_sk = ms.cancer_patient_tumor_driver_sk
  ...
  (SELECT count(*)
   FROM
     (SELECT cancer_patient_tumor_driver_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.cancer_patient_outcome_timeliness
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cancer_patient_outcome_timeliness');
