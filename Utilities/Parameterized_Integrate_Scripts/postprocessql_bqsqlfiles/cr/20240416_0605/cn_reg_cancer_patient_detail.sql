 ...
 datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
   FROM
     (SELECT DISTINCT cp_0.patient_market_urn_text,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.cr_patient AS cp_0
      INNER JOIN
      ...
      LEFT OUTER JOIN
        (SELECT ref_lookup_code.master_lookup_sid,
         ...
         FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_lookup_code
   ...
   FULL OUTER JOIN
     (SELECT cp_0.nav_patient_id,
             ...
             cpa.zip_code,
             cp_0.source_system_code
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.cn_person AS cp_0
      ...
      LEFT OUTER JOIN `hca-hin-dev-cur-ops`.edwcr_base_views.cn_patient_phone_num AS cpp_0 ON cp_0.nav_patient_id = cpp_0.nav_patient_id QUALIFY row_number() OVER (PARTITION BY coalesce(cpt.patient_market_urn, concat(cpt.medical_record_num, df2.facility_mnemonic_cs), format('%#20.0f', cp_0.nav_patient_id))
   ...
   LEFT OUTER JOIN
     (SELECT cancer_patient_driver.cr_patient_id,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.cancer_patient_driver QUALIFY row_number() OVER (PARTITION BY cancer_patient_driver.cr_patient_id
   ...
   LEFT OUTER JOIN
     (SELECT cancer_patient_driver.cn_patient_id,
             cancer_patient_driver.cancer_patient_driver_sk
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.cancer_patient_driver QUALIFY row_number() OVER (PARTITION BY cancer_patient_driver.cn_patient_id
  ...
  (SELECT count(*)
   FROM
     (SELECT cancer_patient_detail_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.cancer_patient_detail
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cancer_patient_detail');
...
UPDATE `hca-hin-dev-cur-ops`.edwcr.cancer_patient_detail AS cd1
SET cancer_patient_driver_sk = cp.cancer_patient_driver_sk
FROM `hca-hin-dev-cur-ops`.edwcr.cancer_patient_driver AS cp
...
FROM
  (SELECT cpd.cp_patient_id,
   ...
   INNER JOIN `hca-hin-dev-cur-ops`.edwcr.cancer_patient_driver AS cp1 ON cpd.cp_patient_id = cp1.cp_patient_id) AS cp
WHERE upper(rtrim(cd1.medical_record_num)) = upper(rtrim(cp.medical_record_num))
  AND upper(rtrim(cd1.patient_market_urn_text)) = upper(rtrim(cp.patient_market_urn_text));
