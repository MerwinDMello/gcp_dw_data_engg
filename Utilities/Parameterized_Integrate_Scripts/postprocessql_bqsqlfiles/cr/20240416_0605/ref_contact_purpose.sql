...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_contact_purpose AS mt USING
  (SELECT DISTINCT row_number() OVER (
     ...
     (SELECT coalesce(max(ref_contact_purpose.contact_purpose_id), 0) AS id1
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_contact_purpose) AS contact_purpose_id,
   ...
   FROM
     (SELECT contact_purpose_stg.contact_purpose_desc,
      ...
      FROM `hca-hin-dev-cur-ops`.edwcr_staging.contact_purpose_stg
      WHERE upper(trim(contact_purpose_stg.contact_purpose_desc)) NOT IN
          (SELECT upper(trim(ref_contact_purpose.contact_purpose_desc))
           FROM `hca-hin-dev-cur-ops`.edwcr.ref_contact_purpose
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_contact_purpose');
