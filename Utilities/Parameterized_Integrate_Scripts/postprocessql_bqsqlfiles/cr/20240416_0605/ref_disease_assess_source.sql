...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.ref_disease_assess_source AS mt USING
  (SELECT DISTINCT
     (SELECT coalesce(max(ref_disease_assess_source.disease_assess_source_id), 0) AS maxid
      FROM `hca-hin-dev-cur-ops`.edwcr_base_views.ref_disease_assess_source) + row_number() OVER (
  ...
  (SELECT count(*)
   FROM
     (SELECT disease_assess_source_id
      FROM `hca-hin-dev-cur-ops`.edwcr.ref_disease_assess_source
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.ref_disease_assess_source');
