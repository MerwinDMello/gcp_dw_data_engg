...
DELETE
FROM `hca-hin-dev-cur-ops`.edwcr_staging.cdm_synthesys_patientid_results
...
FROM `hca-hin-dev-cur-ops`.edwcr_staging.cdm_synthesys_patientid_results AS stg
...
MERGE INTO `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_output AS mt USING
  (SELECT DISTINCT cancer_patient_id_output_wrk.cancer_patient_id_output_sk,
   ...
   FROM `hca-hin-dev-cur-ops`.edwcr_staging.cancer_patient_id_output_wrk) AS ms ON mt.cancer_patient_id_output_sk = ms.cancer_patient_id_output_sk
  ...
  (SELECT count(*)
   FROM
     (SELECT cancer_patient_id_output_sk
      FROM `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_output
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-ops`.edwcr.cancer_patient_id_output');
