...
FROM
 ...
 CASE 
     ...
     WHEN ep.mt_active_txf_provider_sw = 1 
          AND coalesce(CAST(`hca-hin-dev-cur-pub`.bqutil_fns.cw_td_normalize_number(mds.mdstaff_active_dea_license_sw) AS FLOAT64), CAST(`hca-hin-dev-cur-pub`.bqutil_fns.cw_td_normalize_number(format('%4d', 0)) AS FLOAT64)) = 1 THEN 0 
     ...
     ELSE CASE 
              ...
              WHEN ep.cactus_privileged_provider_sw IS NULL THEN CASE 
                                                                     WHEN ep.mt_active_txf_provider_sw = 1 
                                                                          AND coalesce(CAST(`hca-hin-dev-cur-pub`.bqutil_fns.cw_td_normalize_number(mds.mdstaff_active_dea_license_sw) AS FLOAT64), CAST(`hca-hin-dev-cur-pub`.bqutil_fns.cw_td_normalize_number(format('%4d', 0)) AS FLOAT64)) = 0 THEN 3 
