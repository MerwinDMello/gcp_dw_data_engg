...
*==========================================================================*/ BEGIN TRUNCATE TABLE {{ params.param_im_core_dataset_name }}.epms_provider_activity;

BEGIN TRANSACTION;


INSERT INTO {{ params.param_im_core_dataset_name }}.epms_provider_activity (im_domain_id, epms_user_id, esaf_activity_date, source_system_code, epms_prov_npi, cactus_privileged_provider_sw, cactus_active_dea_license_sw, mt_active_provider_sw, mt_active_txf_provider_sw, dw_last_update_date_time)
...
FROM
 ...
 'Privileges without Membership')
          ...
          CASE
              WHEN upper(rtrim(ep.source_system_code)) = 'E'
                   AND CAST({{ params.param_im_bqutil_fns_dataset_name }}.cw_td_normalize_number(dea.cactus_active_dea_license_sw) AS FLOAT64) = 1 THEN 1
              ELSE 0
 ...
 datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
   FROM
     (SELECT im.im_domain_name AS epms_prac_code,
      ...
      FROM {{ params.param_im_base_views_dataset_name }}.epms_provider_status AS epms
      INNER JOIN {{ params.param_im_base_views_dataset_name }}.ref_im_domain AS im ON epms.im_domain_id = im.im_domain_id
      INNER JOIN
        (SELECT DISTINCT company_master.unit_num,
                         company_master.state_code
         FROM {{ params.param_im_views_dataset_name }}.company_master
   ...
   LEFT OUTER JOIN /*LEFT JOIN ( --CACTUS FACILITY STATUS
                										...
                										SELECT
                													...
                													(
                																	...
                																	LEFT OUTER JOIN  `hca-hin-dev-cur-comp`.{{ params.param_im_auth_base_views_dataset_name }}.Fact_Facility fac ON e1.Coid = fac.Coid AND fac.Company_Code = e1.Company_Code
     ...
     (SELECT CASE
      ...
      FROM
        (SELECT hcp.hcp_npi AS npi_number,
                ...
                f2.asgn_cat_desc AS fac_category,
                e1.hcp_fac_asgn_eff_to_date AS facility_effective_to_date
         FROM {{ params.param_im_base_views_dataset_name }}.hcp
         INNER JOIN {{ params.param_im_base_views_dataset_name }}.hcp_facility_assignment AS e1 ON hcp.hcp_dw_id = e1.hcp_dw_id
         AND upper(rtrim(e1.fac_asgn_level_code)) = 'E'
         LEFT OUTER JOIN --  Level Code E = Entity, A = Dept
 {{ params.param_im_base_views_dataset_name }}.ref_facility_asgn_status AS f1 ON e1.fac_asgn_stts_sid = f1.fac_asgn_stts_sid
         LEFT OUTER JOIN {{ params.param_im_base_views_dataset_name }}.ref_assignment_category AS f2 ON e1.asgn_cat_sid = f2.asgn_cat_sid
         LEFT OUTER JOIN `hca-hin-dev-cur-comp`.{{ params.param_im_auth_base_views_dataset_name }}.fact_facility AS fac ON e1.coid = fac.coid
      ...
      LEFT OUTER JOIN
        (SELECT substr(attribute_text, 1, 5) AS coid,
                substr(attribute_text, 7, 5) AS coid_shared,
                com.unit_num AS shared_unit_number
         FROM {{ params.param_im_base_views_dataset_name }}.ref_attribute_value AS rav
         INNER JOIN {{ params.param_im_views_dataset_name }}.company_master AS com ON rtrim(substr(attribute_text, 7, 5)) = rtrim(com.coid)
         WHERE rav.attribute_id = 14
 ...
 d1.hcp_licn_awarded_date AS license_award_date
         FROM {{ params.param_im_base_views_dataset_name }}.hcp_license AS d1
         INNER JOIN {{ params.param_im_base_views_dataset_name }}.hcp ON hcp.hcp_dw_id = d1.hcp_dw_id
         INNER JOIN {{ params.param_im_base_views_dataset_name }}.ref_license_status AS s1 ON d1.licn_stts_sid = s1.licn_stts_sid
         INNER JOIN {{ params.param_im_base_views_dataset_name }}.ref_license_type AS d2 ON d1.licn_type_sid = d2.licn_type_sid
         INNER JOIN {{ params.param_im_base_views_dataset_name }}.hcp_facility_assignment AS e2 ON d1.hcp_dw_id = e2.hcp_dw_id
         INNER JOIN `hca-hin-dev-cur-comp`.{{ params.param_im_auth_base_views_dataset_name }}.facility_address AS ad ON e2.coid = ad.coid
 ...
 -- MEDITECH
     ...
     (SELECT CAST(/* expression of unknown or erroneous type */ prc.prctnr_ssk_cs AS STRING) AS provider_mnemonic,
             ...
             fclt.unit_number
      FROM {{ params.param_im_base_views_dataset_name }}.prctnr_ent AS prc
      INNER JOIN
        (SELECT CAST(/* expression of unknown or erroneous type */ fac.ntwk_ssk_cs AS STRING) AS ntwk_ssk_cs,
         ...
         FROM {{ params.param_im_base_views_dataset_name }}.prctnr_fclt_ent AS fac
         INNER JOIN {{ params.param_im_views_dataset_name }}.company_master AS comp ON fac.company_code = comp.company_code
 ...
 --  Search By NPI
        ...
        (SELECT prc_0.role_plyr_sk,
                fclt_0.coid,
                ROUND(CAST({{ params.param_im_bqutil_fns_dataset_name }}.cw_td_normalize_number(trim(npi.id_txt)) AS NUMERIC), 0, 'ROUND_HALF_EVEN') AS npi_number
         FROM {{ params.param_im_base_views_dataset_name }}.prctnr_dtl AS prc_0
         INNER JOIN {{ params.param_im_base_views_dataset_name }}.prctnr_fclt_dtl AS fclt_0 ON prc_0.role_plyr_sk = fclt_0.role_plyr_sk
         ...
         INNER JOIN --  NPI
           ...
           (SELECT hcp.role_plyr_sk,
            ...
            FROM {{ params.param_im_base_views_dataset_name }}.prctnr_role_idfn AS hcp
            INNER JOIN `hca-hin-dev-cur-comp`.{{ params.param_im_auth_base_views_dataset_name }}.nppes_provider AS nppes ON trim(hcp.id_txt) = rtrim(CAST(/* expression of unknown or erroneous type */ nppes.npi AS STRING))
  ...
  (SELECT count(*)
   FROM
     (SELECT im_domain_id,
             ...
             esaf_activity_date,
             source_system_code
      FROM `hca-hin-dev-cur-comp`.{{ params.param_im_core_dataset_name }}.epms_provider_activity
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-comp`.{{ params.param_im_core_dataset_name }}.epms_provider_activity');
