...
*
*==========================================================================*/ BEGIN TRUNCATE TABLE {{ params.param_im_core_dataset_name }}.epms_provider_status;

BEGIN TRANSACTION;


INSERT INTO {{ params.param_im_core_dataset_name }}.epms_provider_status (im_domain_id, epms_prov_npi, source_system_code, epms_user_id, epms_prov_last_name, epms_prov_first_name, epms_prov_enabled_date, epms_prov_active_flag, dw_last_update_date_time)
  ...
  (SELECT DISTINCT imd.im_domain_id,
   ...
   FROM -- RCOPIA
     ...
     (SELECT rc1.provider_npi AS epms_prov_npi,
      ...
      FROM {{ params.param_im_stage_dataset_name }}.rcopia_provider_status_stg AS rc1
      INNER JOIN
        (SELECT SPLIT(av_0.attribute_text, ':')[ORDINAL(1)] AS unit_number,
                SPLIT(av_0.attribute_text, ':')[ORDINAL(2)] AS facility_name
         FROM {{ params.param_im_base_views_dataset_name }}.ref_attribute_value AS av_0
   ...
   INNER JOIN {{ params.param_im_base_views_dataset_name }}.ref_im_domain AS imd ON rtrim(rc2.epcs_practice_code) = rtrim(imd.im_domain_name)
   AND upper(rtrim(imd.source_system_code)) = 'R'
   LEFT OUTER JOIN {{ params.param_im_base_views_dataset_name }}.ad_account AS ad ON SAFE_CAST(rc2.epms_prov_npi AS NUMERIC) = ad.ad_account_npi
  ...
  (SELECT DISTINCT imd.im_domain_id,
   ...
   FROM -- EPCS
     ...
     (SELECT epcs.provider_npi AS epms_prov_npi,
             ...
             av.unit_number AS epcs_practice_code
      FROM {{ params.param_im_stage_dataset_name }}.epcs_provider_status_stg AS epcs
      INNER JOIN
        (SELECT SPLIT(av_0.attribute_text, ':')[ORDINAL(1)] AS unit_number,
                SPLIT(av_0.attribute_text, ':')[ORDINAL(2)] AS facility_name
         FROM {{ params.param_im_base_views_dataset_name }}.ref_attribute_value AS av_0
   ...
   INNER JOIN {{ params.param_im_base_views_dataset_name }}.ref_im_domain AS imd ON rtrim(epcs2.epcs_practice_code) = rtrim(imd.im_domain_name)
   AND upper(rtrim(imd.source_system_code)) = 'E'
   LEFT OUTER JOIN {{ params.param_im_base_views_dataset_name }}.ad_account AS ad ON SAFE_CAST(epcs2.epms_prov_npi AS NUMERIC) = ad.ad_account_npi
  ...
  (SELECT count(*)
   FROM
     (SELECT im_domain_id,
             epms_prov_npi,
             source_system_code
      FROM `hca-hin-dev-cur-comp`.{{ params.param_im_core_dataset_name }}.epms_provider_status
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-comp`.{{ params.param_im_core_dataset_name }}.epms_provider_status');
