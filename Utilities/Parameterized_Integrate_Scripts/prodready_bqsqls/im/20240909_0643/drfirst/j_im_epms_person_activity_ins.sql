...
*==========================================================================*/ BEGIN
DELETE
FROM {{ params.param_im_core_dataset_name }}.im_person_activity
...
INSERT INTO {{ params.param_im_core_dataset_name }}.im_person_activity (im_domain_id, im_person_user_id, esaf_activity_date, access_rule_id, im_person_inactivate_sw, source_system_code, dw_last_update_date_time)
...
FROM
 ...
 ep.source_system_code 
      FROM {{ params.param_im_base_views_dataset_name }}.epms_provider_activity AS ep 
      INNER JOIN {{ params.param_im_base_views_dataset_name }}.ref_im_domain AS imd ON ep.im_domain_id = imd.im_domain_id 
        ...
        (SELECT hre.employee_34_login_code, 
         ...
         FROM 
           (SELECT jbcode.job_code_sid, 
            ...
            FROM {{ params.param_im_base_views_dataset_name }}.hr_job_code AS jbcode 
            INNER JOIN {{ params.param_im_base_views_dataset_name }}.hr_job_class AS jclcode ON jbcode.job_class_sid = jclcode.job_class_sid 
         ...
         INNER JOIN 
           (SELECT hr_job_position.position_sid, 
            ...
            FROM {{ params.param_im_base_views_dataset_name }}.hr_job_position 
         ...
         INNER JOIN 
           (SELECT hr_employee_position.employee_sid, 
            ...
            FROM {{ params.param_im_base_views_dataset_name }}.hr_employee_position 
         ...
         INNER JOIN 
           (SELECT hr_employee.employee_34_login_code, 
            ...
            FROM {{ params.param_im_base_views_dataset_name }}.hr_employee 
 ...
 CASE 
     ...
     WHEN ep.mt_active_txf_provider_sw = 1 
          AND coalesce(CAST({{ params.param_im_bqutil_fns_dataset_name }}.cw_td_normalize_number(mds.mdstaff_active_dea_license_sw) AS FLOAT64), CAST({{ params.param_im_bqutil_fns_dataset_name }}.cw_td_normalize_number(format('%4d', 0)) AS FLOAT64)) = 1 THEN 0 
     ...
     ELSE CASE 
              ...
              WHEN ep.cactus_privileged_provider_sw IS NULL THEN CASE 
                                                                     WHEN ep.mt_active_txf_provider_sw = 1 
                                                                          AND coalesce(CAST({{ params.param_im_bqutil_fns_dataset_name }}.cw_td_normalize_number(mds.mdstaff_active_dea_license_sw) AS FLOAT64), CAST({{ params.param_im_bqutil_fns_dataset_name }}.cw_td_normalize_number(format('%4d', 0)) AS FLOAT64)) = 0 THEN 3 
 ...
 ep.source_system_code
      ...
      LEFT OUTER JOIN
        (SELECT fac.im_domain_id,
         ...
         FROM {{ params.param_im_base_views_dataset_name }}.provider_license AS pl
         INNER JOIN {{ params.param_im_base_views_dataset_name }}.provider_detail AS pd ON pd.provider_id = pl.provider_id
         AND pd.source_system_code = pl.source_system_code
         LEFT OUTER JOIN {{ params.param_im_base_views_dataset_name }}.ref_provider_license_status AS rls ON rls.licn_stts_id = pl.licn_status_id
         AND rls.source_system_code = pl.source_system_code
         LEFT OUTER JOIN {{ params.param_im_base_views_dataset_name }}.ref_provider_license_type AS rplt ON rplt.licn_type_id = pl.licn_type_id
         ...
         INNER JOIN
           (SELECT imd_0.im_domain_id,
            ...
            FROM {{ params.param_im_base_views_dataset_name }}.ref_im_domain AS imd_0
            INNER JOIN {{ params.param_im_views_dataset_name }}.company_master AS fac_0 ON upper(trim(coalesce(imd_0.im_domain_name, ''))) = upper(trim(coalesce(fac_0.unit_num, '')))
  ...
  (SELECT count(*)
   FROM
     (SELECT im_domain_id,
             im_person_user_id,
             esaf_activity_date
      FROM `hca-hin-dev-cur-comp`.{{ params.param_im_core_dataset_name }}.im_person_activity
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `hca-hin-dev-cur-comp`.{{ params.param_im_core_dataset_name }}.im_person_activity');
