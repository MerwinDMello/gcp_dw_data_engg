...
SELECT NULL AS cancer_abstraction_sk, /* nvl(ROW_NUMBER() OVER ( ORDER BY unique_message_id || ' - ' ||COALESCE(site_and_associated_model_output_score,'' )||COALESCE(second_primary_site,'') ),0)as Cancer_Abstraction_SK ------Added for CES 17570 */ coalesce(o.cancer_patient_id_output_sk, -1) AS cancer_patient_id_output_sk,
                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                        ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(translate(a.patient_account_number, translate(a.patient_account_number, '0123456789', ''), '')) AS NUMERIC), 0, 'ROUND_HALF_EVEN') AS pat_acct_num,
...
FROM {{ params.param_cr_stage_dataset_name }}.cancer_abstraction_stg AS a
LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_output AS o ON upper(trim(a.unique_message_id)) = upper(trim(o.message_control_id_text))
LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.clinical_acctkeys AS cact ON ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(translate(a.patient_account_number, translate(a.patient_account_number, '0123456789', ''), '')) AS NUMERIC), 0, 'ROUND_HALF_EVEN') = cact.pat_acct_num
...
LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_abstraction AS cr ON upper(trim(coalesce(wr.message_control_id_text, '##'))) = upper(trim(coalesce(cr.message_control_id_text, '##')))
...
FROM {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_abstraction_wrk1 AS stg QUALIFY row_number() OVER (PARTITION BY stg.cancer_abstraction_sk
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cancer_patient_id_abstraction AS mt USING
  (SELECT DISTINCT stg.cancer_abstraction_sk,
   ...
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_abstraction AS tgt ON stg.cancer_abstraction_sk = tgt.cancer_abstraction_sk
