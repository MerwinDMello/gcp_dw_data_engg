  ...
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM
     (SELECT DISTINCT cancer_abstraction_values_stg.abstraction_field
      FROM {{ params.param_cr_stage_dataset_name }}.cancer_abstraction_values_stg) AS src
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_abstraction_measure AS tgt ON upper(trim(tgt.abstraction_measure_name)) = upper(trim(src.abstraction_field))
