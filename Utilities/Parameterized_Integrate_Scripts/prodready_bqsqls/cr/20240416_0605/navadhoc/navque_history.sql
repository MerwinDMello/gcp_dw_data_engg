...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.navque_history;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.navque_history AS mt USING
  (SELECT DISTINCT stg.navque_history_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.navque_history_stg AS stg
   WHERE upper(trim(stg.hashbite_ssk)) NOT IN
       (SELECT upper(trim(navque_history.hashbite_ssk))
        FROM {{ params.param_cr_core_dataset_name }}.navque_history) ) AS ms ON mt.navque_history_id = ms.navque_history_id
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT navque_history_id
      FROM {{ params.param_cr_core_dataset_name }}.navque_history
      GROUP BY navque_history_id
      HAVING count(*) > 1));

IF DUP_COUNT <> 0 THEN
ROLLBACK TRANSACTION;

RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.navque_history');
