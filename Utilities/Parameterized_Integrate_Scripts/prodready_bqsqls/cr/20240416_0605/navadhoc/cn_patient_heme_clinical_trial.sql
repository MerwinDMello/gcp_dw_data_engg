...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_heme_clinical_trial AS mt USING
  (SELECT DISTINCT iq.cn_patient_heme_clinical_trial_sid AS cn_patient_heme_clinical_trial_sid,
   ...
   FROM
     (SELECT stg.patientclinicaltrialfactid AS cn_patient_heme_clinical_trial_sid,
      ...
      FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_heme_clinical_trial_stg AS stg) AS iq
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_patient_heme_clinical_trial AS cphc ON upper(trim(iq.hashbite_ssk)) = upper(trim(cphc.hashbite_ssk))
  ...
  (SELECT count(*)
   FROM
     (SELECT cn_patient_heme_clinical_trial_sid
      FROM {{ params.param_cr_core_dataset_name }}.cn_patient_heme_clinical_trial
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_heme_clinical_trial');
...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_heme_clinical_trial AS cphc
WHERE NOT EXISTS
    (SELECT 'X'
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_heme_clinical_trial_stg AS sphc
