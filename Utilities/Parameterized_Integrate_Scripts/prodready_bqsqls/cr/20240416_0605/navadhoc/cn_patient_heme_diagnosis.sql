...
-- bteq << EOF >> $1;;
 ...
 --Job=J_CN_PATIENT_HEME_DIAGNOSIS;;
 --' FOR SESSION;;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_heme_diagnosis AS mt USING
  (SELECT DISTINCT iq.cn_patient_heme_diagnosis_sid AS cn_patient_heme_diagnosis_sid,
   ...
   FROM
     (SELECT phd.patienthemediagnosisfactid AS cn_patient_heme_diagnosis_sid,
             ...
             CASE upper(trim(phd.therapyrelated))
                 WHEN 'YES' THEN 'Y'
                 WHEN 'NO' THEN 'N'
                 WHEN 'UNKNOWN' THEN 'U'
   ...
   WHERE trim(cphd.hashbite_ssk) IS NULL ) AS ms ON mt.cn_patient_heme_diagnosis_sid = ms.cn_patient_heme_diagnosis_sid
...
INSERT (cn_patient_heme_diagnosis_sid,
        ...
        company_code,
        speciman_date,
        disease_diagnosis_text,
        therapy_related_ind,
        transformed_from_mds_ind,
        mipi_text,
        ipi_text,
        flipi_text,
        aids_related_ind,
        ...
        nhl_type_text,
        other_nhl_type_text,
        transformed_disease_text,
        non_malignant_type_text,
        feature_text,
        risk_category_text,
        mds_disease_risk_text,
        staging_field_1_text,
        staging_field_2_text,
        hashbite_ssk,
        source_system_code,
  ...
  (SELECT count(*)
   FROM
     (SELECT cn_patient_heme_diagnosis_sid
      FROM {{ params.param_cr_core_dataset_name }}.cn_patient_heme_diagnosis
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_heme_diagnosis');
...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_heme_diagnosis AS cphd
WHERE NOT EXISTS
    (SELECT 'X'
     FROM {{ params.param_cr_stage_dataset_name }}.stg_patienthemediagnosis AS sphd
