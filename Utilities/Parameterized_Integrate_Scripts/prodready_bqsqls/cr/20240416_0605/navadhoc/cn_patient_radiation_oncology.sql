...
-- bteq << EOF > $1;;
 ...
 --Job=J_CN_PATIENT_RADIATION_ONCOLOGY;;
 --' FOR SESSION;;
...
WHERE upper(trim(cn_patient_radiation_oncology.hashbite_ssk)) NOT IN
    (SELECT upper(trim(cn_patient_radiation_oncology_stg.hashbite_ssk))
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_radiation_oncology_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_radiation_oncology AS mt USING
  (SELECT DISTINCT stg.cn_patient_rad_oncology_sid,
   ...
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_site_location AS loc ON upper(trim(stg.treatment_site_location_id)) = upper(trim(loc.site_location_desc))
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_lung_lobe_location AS lob_loc ON upper(trim(stg.lung_lobe_location_id)) = upper(trim(lob_loc.lung_lobe_location_desc))
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_facility AS fac ON upper(trim(stg.radiation_oncology_facility_id)) = upper(trim(fac.facility_name))
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_radiation_oncology.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_radiation_oncology) ) AS ms ON mt.cn_patient_radiation_oncology_sid = ms.cn_patient_rad_oncology_sid
...
INSERT (cn_patient_radiation_oncology_sid,
        treatment_site_location_id,
        treatment_type_id,
        lung_lobe_location_id,
        radiation_oncology_facility_id,
        nav_patient_id,
        core_record_type_id,
        ...
        treatment_end_date,
        treatment_fractions_num,
        elapse_ind,
        elapse_start_date,
        elapse_end_date,
        radiation_oncology_reason_text,
        palliative_ind,
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT cn_patient_radiation_oncology_sid
      FROM {{ params.param_cr_core_dataset_name }}.cn_patient_radiation_oncology
      GROUP BY cn_patient_radiation_oncology_sid
      HAVING count(*) > 1));

IF DUP_COUNT <> 0 THEN
ROLLBACK TRANSACTION;

RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_radiation_oncology');
