...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_medical_oncology
WHERE upper(trim(cn_patient_medical_oncology.hashbite_ssk)) NOT IN
    (SELECT upper(trim(cn_patient_medical_oncology_stg.hashbite_ssk))
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_medical_oncology_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_medical_oncology AS mt USING
  (SELECT DISTINCT stg.cn_patient_medical_oncology_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_medical_oncology_stg AS stg
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_facility AS REF ON upper(trim(stg.medical_oncology_facility_id)) = upper(trim(ref.facility_name))
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_medical_oncology.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_medical_oncology) ) AS ms ON mt.cn_patient_medical_oncology_sid = ms.cn_patient_medical_oncology_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_medical_oncology');
