...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_procedure
WHERE upper(trim(cn_patient_procedure.hashbite_ssk)) NOT IN
    (SELECT upper(trim(cn_patient_procedure_stg.hashbite_ssk))
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_procedure_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_procedure AS mt USING
  (SELECT DISTINCT stg.cn_patient_procedure_sid,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_procedure_stg AS stg
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_procedure_type AS refpr ON upper(rtrim(stg.proceduretype)) = upper(rtrim(refpr.procedure_type_desc))
   ...
   WHERE upper(trim(stg.hashbite_ssk)) NOT IN
       (SELECT upper(trim(cn_patient_procedure.hashbite_ssk))
        FROM {{ params.param_cr_base_views_dataset_name }}.cn_patient_procedure) ) AS ms ON mt.cn_patient_procedure_sid = ms.cn_patient_procedure_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_procedure');
