...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_address AS mt USING
  (SELECT DISTINCT stg.nav_patient_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_address_stg AS stg
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_housing_type AS ht ON upper(rtrim(ht.housing_type_name)) = upper(rtrim(stg.housing))
   WHERE stg.nav_patient_id NOT IN
       (SELECT cn_patient_address.nav_patient_id
        FROM {{ params.param_cr_base_views_dataset_name }}.cn_patient_address) ) AS ms ON mt.nav_patient_id = ms.nav_patient_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_address');
