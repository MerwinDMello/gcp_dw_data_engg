...
WHERE upper(cn_patient_heme_risk_factor.hashbite_ssk) NOT IN
    (SELECT upper(patient_heme_risk_factor_stg.hashbite_ssk) AS hashbite_ssk
     FROM {{ params.param_cr_stage_dataset_name }}.patient_heme_risk_factor_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_heme_risk_factor AS mt USING
  (SELECT DISTINCT t1.cn_patient_heme_diagnosis_sid,
   ...
   FROM
     (SELECT stg.patienthemediagnosisfactid AS cn_patient_heme_diagnosis_sid,
      ...
      LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.cn_patient_heme_risk_factor AS tgt ON upper(rtrim(stg.hashbite_ssk)) = upper(rtrim(tgt.hashbite_ssk))
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_heme_risk_factor');
