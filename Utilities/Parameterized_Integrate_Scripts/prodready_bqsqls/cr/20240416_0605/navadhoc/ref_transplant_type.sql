...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_transplant_type AS mt USING
  (SELECT DISTINCT
     (SELECT coalesce(max(ref_transplant_type.transplant_type_id), 0) AS maxid
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_transplant_type) + row_number() OVER (
   ...
   FROM
     (SELECT DISTINCT stg_patienthemetransplant.type_type AS transplant_type_name
      FROM {{ params.param_cr_stage_dataset_name }}.stg_patienthemetransplant
      WHERE stg_patienthemetransplant.type_type IS NOT NULL ) AS ssc
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_transplant_type AS rtt ON upper(rtrim(ssc.transplant_type_name)) = upper(rtrim(rtt.transplant_type_name))
  ...
  (SELECT count(*)
   FROM
     (SELECT transplant_type_id
      FROM {{ params.param_cr_core_dataset_name }}.ref_transplant_type
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_transplant_type');
