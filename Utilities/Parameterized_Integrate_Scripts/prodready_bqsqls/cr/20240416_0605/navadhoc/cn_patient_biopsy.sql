...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_biopsy
WHERE upper(cn_patient_biopsy.hashbite_ssk) NOT IN
    (SELECT upper(cn_patient_biopsy_stg.hashbite_ssk) AS hashbite_ssk
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_biopsy_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_biopsy AS mt USING
  (SELECT DISTINCT stg.cn_patient_biopsy_sid,
   ...
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_facility AS fac ON upper(rtrim(stg.biopsyfacility)) = upper(rtrim(fac.facility_name))
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_site_location AS l ON upper(rtrim(stg.biopsysite)) = upper(rtrim(l.site_location_desc))
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_physician_specialty AS s ON upper(rtrim(stg.biopsyphysiciantype)) = upper(rtrim(s.physician_specialty_desc))
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_biopsy.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_biopsy) ) AS ms ON mt.cn_patient_biopsy_sid = ms.cn_patient_biopsy_sid
...
INSERT (cn_patient_biopsy_sid,
        ...
        biopsy_result_id,
        biopsy_facility_id,
        biopsy_site_location_id,
        biopsy_physician_specialty_id,
        nav_patient_id,
        tumor_type_id,
        diagnosis_result_id,
        nav_diagnosis_id,
  ...
  (SELECT count(*)
   FROM
     (SELECT cn_patient_biopsy_sid
      FROM {{ params.param_cr_core_dataset_name }}.cn_patient_biopsy
      GROUP BY cn_patient_biopsy_sid
      HAVING count(*) > 1));

IF DUP_COUNT <> 0 THEN
ROLLBACK TRANSACTION;

RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_biopsy');
