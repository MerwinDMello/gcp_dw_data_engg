...
WHERE upper(cn_patient_diagnosis.hashbite_ssk) NOT IN
    (SELECT upper(cn_patient_diagnosis_stg.hashbite_ssk) AS hashbite_ssk
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_diagnosis_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_diagnosis AS mt USING
  (SELECT DISTINCT stg.cn_patient_diagnosis_sid,
                   stg.nav_patient_id,
                   stg.tumor_type_id,
   ...
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_side AS rs ON upper(rtrim(stg.diagnosisside)) = upper(rtrim(rs.side_desc))
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_diagnosis_detail AS rdd ON upper(rtrim(coalesce(trim(stg.diagnosismetastatic), 'X'))) = upper(rtrim(coalesce(trim(rdd.diagnosis_detail_desc), 'X')))
   ...
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_diagnosis.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_diagnosis) ) AS ms ON mt.cn_patient_diagnosis_sid = ms.cn_patient_diagnosis_sid
...
INSERT (cn_patient_diagnosis_sid,
        nav_patient_id,
        tumor_type_id,
        diagnosis_result_id,
        nav_diagnosis_id,
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT cn_patient_diagnosis_sid
      FROM {{ params.param_cr_core_dataset_name }}.cn_patient_diagnosis
      GROUP BY cn_patient_diagnosis_sid
      HAVING count(*) > 1));

IF DUP_COUNT <> 0 THEN
ROLLBACK TRANSACTION;

RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_diagnosis');
