...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_pathway_var_reason AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(dis.pathway_var_reason_type_desc),
                                               upper(dis.pathway_var_reason_sub_type_desc)) + coalesce(
                                                                                                         (SELECT max(coalesce(a.pathway_var_reason_id, 0)) AS max_key
                                                                                                          FROM {{ params.param_cr_base_views_dataset_name }}.ref_pathway_var_reason AS a), 0) AS pathway_var_reason_id,
   ...
   WHERE
       (SELECT logical_and(NOT ((upper(trim(rcdc.pathway_var_reason_type_desc)) = upper(trim(dis.pathway_var_reason_type_desc))
        ...
        FROM {{ params.param_cr_base_views_dataset_name }}.ref_pathway_var_reason AS rcdc) ) AS ms ON mt.pathway_var_reason_id = ms.pathway_var_reason_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_pathway_var_reason');
