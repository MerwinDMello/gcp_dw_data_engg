...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_therapy_type AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(trim(type_stg.therapy_type_desc))) +
     (SELECT coalesce(max(ref_therapy_type.therapy_type_id), 0) AS id1
      FROM {{ params.param_cr_core_dataset_name }}.ref_therapy_type) AS therapy_type_id,
   ...
   FROM
     (SELECT therapy_type_stg.therapy_type_desc,
             therapy_type_stg.source_system_code
      FROM {{ params.param_cr_stage_dataset_name }}.therapy_type_stg
      WHERE upper(trim(therapy_type_stg.therapy_type_desc)) NOT IN
          (SELECT upper(trim(ref_therapy_type.therapy_type_desc))
           FROM {{ params.param_cr_core_dataset_name }}.ref_therapy_type) ) AS type_stg) AS ms ON mt.therapy_type_id = ms.therapy_type_id
  ...
  (SELECT count(*)
   FROM
     (SELECT therapy_type_id
      FROM {{ params.param_cr_core_dataset_name }}.ref_therapy_type
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_therapy_type');
