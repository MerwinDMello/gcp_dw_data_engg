...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_side AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(trim(src.side_desc))) +
     (SELECT coalesce(max(ref_side.side_id), 0) AS id1
      FROM {{ params.param_cr_core_dataset_name }}.ref_side) AS side_id,
   ...
   WHERE src.side_desc IS NOT NULL
     AND upper(rtrim(src.side_desc)) <> '' ) AS ms ON mt.side_id = ms.side_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_side');
