...
-- bteq << EOF >> $1;;
 ...
 --Job=J_CN_PATIENT_HEME_DISEASE_ASSESS;;
 --' FOR SESSION;;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_heme_disease_assess AS mt USING
  (SELECT DISTINCT CAST(iq.cn_patient_heme_sid AS INT64) AS cn_patient_heme_sid,
                   iq.test_type_id,
                   iq.sample_type_id,
   ...
   FROM
     (SELECT phd.patienthemefactid AS cn_patient_heme_sid,
             ...
             CASE upper(trim(phd.initialdiagnosis))
                 WHEN 'YES' THEN 'Y'
                 WHEN 'NO' THEN 'N'
                 ELSE 'U'
             END AS initial_diagnosis_ind,
             CASE upper(trim(phd.diseasemonitoring))
                 WHEN 'YES' THEN 'Y'
                 WHEN 'NO' THEN 'N'
                 ELSE 'U'
   ...
   WHERE trim(cphd.hashbite_ssk) IS NULL ) AS ms ON mt.cn_patient_heme_sid = ms.cn_patient_heme_sid
...
INSERT (cn_patient_heme_sid,
        test_type_id,
        sample_type_id,
        disease_assess_source_id,
        disease_assess_facility_id,
        nav_patient_id,
        tumor_type_id,
        diagnosis_result_id,
        nav_diagnosis_id,
  ...
  (SELECT count(*)
   FROM
     (SELECT cn_patient_heme_sid
      FROM {{ params.param_cr_core_dataset_name }}.cn_patient_heme_disease_assess
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_heme_disease_assess');
...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_heme_disease_assess AS cphd
WHERE NOT EXISTS
    (SELECT 'X'
     FROM {{ params.param_cr_stage_dataset_name }}.patient_heme_disease_assessment_stg AS sphd
