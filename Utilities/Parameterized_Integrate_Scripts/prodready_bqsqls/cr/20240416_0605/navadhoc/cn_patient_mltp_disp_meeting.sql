...
WHERE upper(cn_patient_mltp_disciplinary_meeting.hashbite_ssk) NOT IN
    (SELECT upper(cn_patient_mltp_disciplinary_meeting_stg.hashbite_ssk) AS hashbite_ssk
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_mltp_disciplinary_meeting_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_mltp_disciplinary_meeting AS mt USING
  (SELECT DISTINCT stg.cn_patient_mltp_disc_meet_sid,
   ...
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_mltp_disciplinary_meeting.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_mltp_disciplinary_meeting) ) AS ms ON mt.cn_patient_mltp_disciplinary_meet_sid = ms.cn_patient_mltp_disc_meet_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_mltp_disciplinary_meeting');
