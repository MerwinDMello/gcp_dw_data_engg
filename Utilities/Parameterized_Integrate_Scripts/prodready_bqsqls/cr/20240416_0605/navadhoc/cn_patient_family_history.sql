...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_family_history
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_family_history AS mt USING
  (SELECT DISTINCT cn_patient_family_history_stg.patienthistoryfactid AS cn_patient_family_history_sid,
   ...
   WHERE upper(cn_patient_family_history_stg.hbsource) NOT IN
       (SELECT upper(cn_patient_family_history.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_base_views_dataset_name }}.cn_patient_family_history) ) AS ms ON mt.cn_patient_family_history_sid = ms.cn_patient_family_history_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_family_history');
