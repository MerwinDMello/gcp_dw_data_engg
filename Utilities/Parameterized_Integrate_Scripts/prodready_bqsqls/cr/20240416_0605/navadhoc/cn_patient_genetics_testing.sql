...
-- bteq << EOF > $1;;
 ...
 --Job=J_CN_PATIENT_GENETICS_TESTING;;
 --' FOR SESSION;;
...
WHERE upper(cn_patient_genetics_testing.hashbite_ssk) NOT IN
    (SELECT upper(cn_patient_genetics_testing_stg.hbsource) AS hbsource
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_genetics_testing_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_genetics_testing AS mt USING
  (SELECT DISTINCT pgt.geneticstestingfactid,
                   pgt.corerecordid,
   ...
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_breast_cancer_type AS brca ON upper(rtrim(pgt.geneticsbrcatype)) = upper(rtrim(brca.breast_cancer_type_desc))
   WHERE upper(pgt.hbsource) NOT IN
       (SELECT upper(cn_patient_genetics_testing.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_genetics_testing) ) AS ms ON mt.cn_patient_genetics_testing_sid = ms.geneticstestingfactid
...
INSERT (cn_patient_genetics_testing_sid,
        core_record_type_id,
        nav_patient_id,
        tumor_type_id,
        diagnosis_result_id,
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT cn_patient_genetics_testing_sid
      FROM {{ params.param_cr_core_dataset_name }}.cn_patient_genetics_testing
      GROUP BY cn_patient_genetics_testing_sid
      HAVING count(*) > 1));

IF DUP_COUNT <> 0 THEN
ROLLBACK TRANSACTION;

RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_genetics_testing');
