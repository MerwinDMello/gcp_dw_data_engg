...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.cn_person
WHERE cn_person.nav_patient_id NOT IN
    (SELECT cn_person_stg.nav_patient_id
     FROM {{ params.param_cr_stage_dataset_name }}.cn_person_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_person AS mt USING
  (SELECT DISTINCT cn_person_stg.nav_patient_id,
   ...
   WHERE cn_person_stg.nav_patient_id NOT IN
       (SELECT cn_person.nav_patient_id
        FROM {{ params.param_cr_core_dataset_name }}.cn_person) ) AS ms ON mt.nav_patient_id = ms.nav_patient_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_person');
