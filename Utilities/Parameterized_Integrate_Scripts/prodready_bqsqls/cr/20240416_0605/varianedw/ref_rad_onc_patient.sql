...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.rad_onc_patient;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.rad_onc_patient AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY dhd.dimsiteid,
                                               dhd.dimpatientid) AS patient_sk,
                                     ...
                                     CASE upper(trim(dhd.patientinoutstatus))
                                         WHEN 'IN' THEN 'I'
                                         WHEN 'OUT' THEN 'O'
                                         ELSE 'U'
                                     END AS patient_in_out_ind,
                                     CASE upper(trim(dhd.patientdeathstatus))
                                         WHEN 'ALIVE' THEN 'Y'
                                         WHEN 'DEAD' THEN 'N'
                                         ELSE 'U'
                                     ...
                                     CASE upper(trim(dhd.clinicaltrial))
                                         WHEN 'YES' THEN 'Y'
                                         WHEN 'NO' THEN 'N'
                                         ELSE 'U'
                                     ...
                                     CASE upper(trim(dhd.patienttype))
   ...
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.rad_onc_address AS ra ON upper(rtrim(coalesce(trim(dhd.patientaddressline1), '###'))) = upper(rtrim(coalesce(trim(ra.address_line_1_text), '###')))
   ...
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_site AS rr ON rr.source_site_id = dhd.dimsiteid) AS ms ON mt.patient_sk = ms.patient_sk
...
INSERT (patient_sk,
        ...
        patient_death_ind,
        patient_death_date,
        patient_death_reason_text,
        clinical_trial_ind,
        patient_transportation_text,
        patient_global_unique_id_text,
        patient_room_number_text,
        active_ind,
        patient_language_text,
        patient_notes_text,
        log_id,
        run_id,
        history_user_name,
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT patient_sk
      FROM {{ params.param_cr_core_dataset_name }}.rad_onc_patient
      GROUP BY patient_sk
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.rad_onc_patient');
