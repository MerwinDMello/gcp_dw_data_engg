...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.ref_rad_onc_activity_priority;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_rad_onc_activity_priority AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM
     (SELECT DISTINCT CASE
      ...
      FROM {{ params.param_cr_stage_dataset_name }}.stg_dimactivitytransaction AS dhd) AS src
  ...
  (SELECT count(*)
   FROM
     (SELECT activity_priority_sk
      FROM {{ params.param_cr_core_dataset_name }}.ref_rad_onc_activity_priority
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_rad_onc_activity_priority');
