...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.fact_rad_onc_activity_billing;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.fact_rad_onc_activity_billing AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                     ...
                                     ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.creditamount) AS NUMERIC), 6, 'ROUND_HALF_EVEN') AS credit_amt,
   ...
   INNER JOIN
     (SELECT ref_rad_onc_site.source_site_id,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_site) AS rr ON dp.dimsiteid = rr.source_site_id
   LEFT OUTER JOIN
     (SELECT rad_onc_physician.source_physician_id,
             rad_onc_physician.physician_sk,
             rad_onc_physician.site_sk
      FROM {{ params.param_cr_base_views_dataset_name }}.rad_onc_physician) AS rop ON dp.dimdoctorid = rop.source_physician_id
   AND rr.site_sk = rop.site_sk
   LEFT OUTER JOIN
     (SELECT rad_onc_physician.source_physician_id,
             rad_onc_physician.physician_sk,
             rad_onc_physician.site_sk
      FROM {{ params.param_cr_base_views_dataset_name }}.rad_onc_physician) AS rop1 ON dp.dimdoctid_attdoncologist = rop1.source_physician_id
   ...
   LEFT OUTER JOIN
     (SELECT rad_onc_patient_course.source_patient_course_id,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.rad_onc_patient_course) AS pc ON dp.dimcourseid = pc.source_patient_course_id
   ...
   LEFT OUTER JOIN
     (SELECT ref_rad_onc_hospital.source_hospital_id,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_hospital) AS rh ON dp.dimhospitaldepartmentid = rh.source_hospital_id
   ...
   LEFT OUTER JOIN
     (SELECT ref_rad_onc_activity.source_activity_id,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_activity) AS ra ON dp.dimactivityid = ra.source_activity_id
   ...
   LEFT OUTER JOIN
     (SELECT rad_onc_activity_transaction.source_activity_transaction_id,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.rad_onc_activity_transaction) AS rat ON dp.dimactivitytransactionid = rat.source_activity_transaction_id
   ...
   LEFT OUTER JOIN
     (SELECT ref_rad_onc_procedure_code.source_procedure_code_id,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_procedure_code) AS rpp ON dp.dimprocedurecodeid = rpp.source_procedure_code_id
   AND rr.site_sk = rpp.site_sk
   LEFT OUTER JOIN
     (SELECT rad_onc_patient.source_patient_id,
             rad_onc_patient.patient_sk,
             rad_onc_patient.site_sk
      FROM {{ params.param_cr_base_views_dataset_name }}.rad_onc_patient) AS rp ON dp.dimpatientid = rp.source_patient_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.fact_rad_onc_activity_billing');
