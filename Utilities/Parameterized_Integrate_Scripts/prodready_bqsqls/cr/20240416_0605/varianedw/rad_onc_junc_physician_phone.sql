...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.rad_onc_junc_physician_phone;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.rad_onc_junc_physician_phone AS mt USING
  (SELECT DISTINCT pt.physician_sk AS physician_sk,
   ...
   FROM
     (SELECT CAST(NULL AS INT64) AS physician_sk,
             CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(stg_dimdoctor.dimsiteid) AS INT64) AS dimsiteid,
             CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(stg_dimdoctor.dimdoctorid) AS INT64) AS dimdoctorid,
      ...
      UNION DISTINCT SELECT CAST(NULL AS INT64) AS physician_sk,
                            CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(stg_dimdoctor.dimsiteid) AS INT64) AS dimsiteid,
                            CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(stg_dimdoctor.dimdoctorid) AS INT64) AS dimdoctorid,
      ...
      UNION DISTINCT SELECT CAST(NULL AS INT64) AS physician_sk,
                            ...
                            'R' AS source_system_code,
                            datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
      FROM {{ params.param_cr_stage_dataset_name }}.stg_dimdoctor) AS dim_sub
   ...
   AND dim_sub.dimdoctorid = pt.source_physician_id) AS ms ON mt.physician_sk = ms.physician_sk
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT physician_sk,
             phone_num_type_code
      FROM {{ params.param_cr_core_dataset_name }}.rad_onc_junc_physician_phone
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.rad_onc_junc_physician_phone');
