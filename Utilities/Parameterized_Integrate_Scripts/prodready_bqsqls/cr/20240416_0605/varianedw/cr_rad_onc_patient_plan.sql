...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.rad_onc_patient_plan;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.rad_onc_patient_plan AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(dp.dimsiteid),
                                               upper(dp.dimplanid)) AS patient_plan_sk,
                                     rpp.plan_purpose_sk,
                                     rr.site_sk AS site_sk,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimplanid) AS INT64) AS source_patient_plan_id,
                                     rpc.patient_course_sk,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimlookupid_planstatus) AS INT64) AS plan_status_id,
   ...
   INNER JOIN {{ params.param_cr_core_dataset_name }}.ref_rad_onc_site AS rr ON rr.source_site_id = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimsiteid) AS FLOAT64)
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_rad_onc_plan_purpose AS rpp ON upper(rtrim(dp.planintent)) = upper(rtrim(rpp.plan_purpose_name))
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.rad_onc_patient_course AS rpc ON rpc.source_patient_course_id = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimcourseid) AS FLOAT64)
  ...
  (SELECT count(*)
   FROM
     (SELECT patient_plan_sk
      FROM {{ params.param_cr_core_dataset_name }}.rad_onc_patient_plan
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.rad_onc_patient_plan');
