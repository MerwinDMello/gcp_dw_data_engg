...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.fact_rad_onc_patient_toxicity;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.fact_rad_onc_patient_toxicity AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM
     (SELECT stg_factpatienttoxicity.toxicitycomponentname,
      ...
      FROM {{ params.param_cr_stage_dataset_name }}.stg_factpatienttoxicity) AS dp
   INNER JOIN
   ...
   LEFT OUTER JOIN
     (SELECT ref_rad_onc_toxicity_component.toxicity_component_name,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_toxicity_component) AS tc ON upper(rtrim(dp.toxicitycomponentname)) = upper(rtrim(tc.toxicity_component_name))
   LEFT OUTER JOIN
     (SELECT ref_rad_onc_toxicity_assessment_type.toxicity_assessment_type_desc,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_toxicity_assessment_type) AS ta ON upper(rtrim(dp.toxicityassessmenttype)) = upper(rtrim(ta.toxicity_assessment_type_desc))
   LEFT OUTER JOIN
  ...
  (SELECT count(*)
   FROM
     (SELECT fact_patient_toxicity_sk
      FROM {{ params.param_cr_core_dataset_name }}.fact_rad_onc_patient_toxicity
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.fact_rad_onc_patient_toxicity');
