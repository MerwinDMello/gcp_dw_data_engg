...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.ref_rad_onc_site;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_rad_onc_site AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_ro_dimsite_stg AS ds) AS ms ON mt.site_sk = ms.site_sk
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_rad_onc_site');
