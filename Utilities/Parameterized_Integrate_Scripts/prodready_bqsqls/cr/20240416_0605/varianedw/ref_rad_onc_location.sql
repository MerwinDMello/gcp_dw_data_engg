...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.ref_rad_onc_location;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_rad_onc_location AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY rr.site_sk,
                                               CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(ds.dimlocationid) AS INT64)) AS location_sk,
                                     rr.site_sk AS site_sk,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(ds.dimlocationid) AS INT64) AS source_location_id,
                                     substr(CASE
                                     ...
                                     END AS zip_code,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(ds.logid) AS INT64) AS log_id,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(ds.runid) AS INT64) AS run_id,
                                     'R' AS source_system_code,
                                     datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
   FROM {{ params.param_cr_stage_dataset_name }}.stg_dimlocation AS ds
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_site AS rr ON rr.source_site_id = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(ds.dimsiteid) AS INT64)) AS ms ON mt.location_sk = ms.location_sk
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_rad_onc_location');
