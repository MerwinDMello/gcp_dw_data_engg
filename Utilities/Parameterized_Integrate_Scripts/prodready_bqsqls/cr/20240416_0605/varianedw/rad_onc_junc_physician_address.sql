...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.rad_onc_junc_physician_address;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.rad_onc_junc_physician_address AS mt USING
  (SELECT DISTINCT stg.physician_sk,
   ...
   FROM
     (SELECT pt.physician_sk AS physician_sk,
             ...
             CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimdoctorid) AS INT64) AS dimdoctorid,
             CASE CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.isprimarydoctoraddress) AS INT64)
                 WHEN 1 THEN 'Y'
                 WHEN 0 THEN 'N'
                 ELSE CAST(NULL AS STRING)
             END AS primary_address_ind
      FROM {{ params.param_cr_stage_dataset_name }}.stg_dimdoctor AS dhd
      INNER JOIN {{ params.param_cr_base_views_dataset_name }}.rad_onc_address AS ra ON upper(trim(coalesce(dhd.doctorcompleteaddress, '##'))) = upper(trim(coalesce(ra.full_address_text, '##')))
      ...
      INNER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_site AS rr ON rr.source_site_id = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimsiteid) AS INT64)
      INNER JOIN {{ params.param_cr_base_views_dataset_name }}.rad_onc_physician AS pt ON rr.site_sk = pt.site_sk
      AND CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimdoctorid) AS INT64) = pt.source_physician_id) AS stg) AS ms ON mt.physician_sk = ms.physician_sk
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT physician_sk,
             address_type_code
      FROM {{ params.param_cr_core_dataset_name }}.rad_onc_junc_physician_address
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.rad_onc_junc_physician_address');
