...
-- bteq << EOF >> $1;;
 ...
 --Job=J_CR_RO_Rad_Onc_Activity_Transaction;;
 --' FOR SESSION;;
...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.rad_onc_activity_transaction;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.rad_onc_activity_transaction AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                     ...
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimlookupid_appointmentstatus) AS INT64) AS appointment_status_id,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimlookupid_actualresourcetype) AS INT64) AS actual_resource_type_id,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimlookupid_cancelreasontype) AS INT64) AS cancel_reason_type_id,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimlookupid_appointmentrsrcsta) AS INT64) AS appointment_resource_status_id,
                                     rr.site_sk AS site_sk,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimactivitytransactionid) AS INT64) AS source_activity_transaction_id,
                                     ...
                                     CASE upper(rtrim(dp.waitlistedflag))
                                         WHEN '1' THEN 'Y'
                                         WHEN '0' THEN 'N'
                                     END AS waitlist_ind,
                                     substr(CASE
                                     ...
                                     CASE upper(rtrim(dp.appointmentinstanceflag))
                                         WHEN '1' THEN 'Y'
                                         WHEN '0' THEN 'N'
                                     ...
                                     CASE upper(rtrim(dp.activityownerflag))
                                         WHEN '1' THEN 'Y'
                                         WHEN '0' THEN 'N'
                                     END AS activity_owner_ind,
                                     CASE upper(rtrim(dp.isvisittypeopenchart))
                                         WHEN '1' THEN 'Y'
                                         WHEN '0' THEN 'N'
                                     END AS visit_type_open_chart_ind,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.ctrresourceser) AS INT64) AS resource_id,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.logid) AS INT64) AS log_id,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.runid) AS INT64) AS run_id,
   ...
   INNER JOIN
     (SELECT ref_rad_onc_site.source_site_id,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_site) AS rr ON CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimsiteid) AS FLOAT64) = rr.source_site_id
   LEFT OUTER JOIN
     (SELECT ref_rad_onc_activity_priority.activity_priority_sk,
             ref_rad_onc_activity_priority.activity_priority_desc
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_activity_priority) AS ap ON upper(rtrim(dp.activitypriority)) = upper(rtrim(ap.activity_priority_desc))
   LEFT OUTER JOIN
     (SELECT ref_rad_onc_hospital.source_hospital_id,
             ref_rad_onc_hospital.hospital_sk,
             ref_rad_onc_hospital.site_sk
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_hospital) AS rh ON CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimhospitaldepartmentid) AS FLOAT64) = rh.source_hospital_id
   AND rr.site_sk = rh.site_sk
   LEFT OUTER JOIN
     (SELECT rad_onc_patient.source_patient_id,
             rad_onc_patient.patient_sk,
             rad_onc_patient.site_sk
      FROM {{ params.param_cr_base_views_dataset_name }}.rad_onc_patient) AS rp ON CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimpatientid) AS FLOAT64) = rp.source_patient_id
   ...
   LEFT OUTER JOIN
     (SELECT ref_rad_onc_activity.source_activity_id,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_activity) AS ra ON CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dimactivityid) AS FLOAT64) = ra.source_activity_id
   AND rr.site_sk = ra.site_sk) AS ms ON mt.activity_transaction_sk = ms.fact_patient_toxicity_sk
...
INSERT (activity_transaction_sk,
        activity_priority_sk,
        hospital_sk,
        patient_sk,
        activity_sk,
        appointment_status_id,
        actual_resource_type_id,
        cancel_reason_type_id,
        appointment_resource_status_id,
        site_sk,
        source_activity_transaction_id,
        ...
        activity_note_text,
        check_in_ind,
        patient_arrival_date_time,
        waitlist_ind,
        patient_location_text,
        appointment_instance_ind,
        appointment_task_date_time,
        activity_owner_ind,
        visit_type_open_chart_ind,
        resource_id,
        log_id,
        run_id,
        source_system_code,
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT activity_transaction_sk
      FROM {{ params.param_cr_core_dataset_name }}.rad_onc_activity_transaction
      GROUP BY activity_transaction_sk
      HAVING count(*) > 1));

IF DUP_COUNT <> 0 THEN
ROLLBACK TRANSACTION;

RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.rad_onc_activity_transaction');
