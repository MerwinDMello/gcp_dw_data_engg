...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.rad_onc_address_wrk;
...
INSERT INTO {{ params.param_cr_stage_dataset_name }}.rad_onc_address_wrk (address_sk, address_line_1_text, address_line_2_text, full_address_text, address_comment_text)
...
FROM {{ params.param_cr_stage_dataset_name }}.stg_dimhospitaldepartment
...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.rad_onc_address;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.rad_onc_address AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.rad_onc_address_wrk AS ds) AS ms ON mt.address_sk = ms.address_sk
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT address_sk
      FROM {{ params.param_cr_core_dataset_name }}.rad_onc_address
      GROUP BY address_sk
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.rad_onc_address');
