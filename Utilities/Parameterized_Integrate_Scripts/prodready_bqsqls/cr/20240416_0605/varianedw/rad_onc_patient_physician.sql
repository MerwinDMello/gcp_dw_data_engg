...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.rad_onc_patient_physician;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.rad_onc_patient_physician AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.stg_dimpatientdoctor AS dhd
   INNER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_site AS rs ON rs.source_site_id = dhd.dimsiteid
   INNER JOIN {{ params.param_cr_base_views_dataset_name }}.rad_onc_patient AS ra ON dhd.dimpatientid = ra.source_patient_id
  ...
  (SELECT count(*)
   FROM
     (SELECT patient_physician_sk
      FROM {{ params.param_cr_core_dataset_name }}.rad_onc_patient_physician
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.rad_onc_patient_physician');
