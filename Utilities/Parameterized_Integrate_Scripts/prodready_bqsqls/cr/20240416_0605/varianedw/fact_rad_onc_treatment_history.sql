...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.fact_rad_onc_treatment_history;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.fact_rad_onc_treatment_history AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                     ...
                                     ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.coursedosedelivered) AS NUMERIC), 38, 'ROUND_HALF_EVEN') AS course_dose_delivered_amt,
                                     ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.coursedoseplanned) AS NUMERIC), 38, 'ROUND_HALF_EVEN') AS course_dose_planned_amt,
                                     ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.coursedoseremaining) AS NUMERIC), 38, 'ROUND_HALF_EVEN') AS course_dose_remaining_amt,
                                     ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.othercoursedosedelivered) AS NUMERIC), 38, 'ROUND_HALF_EVEN') AS other_course_dose_delivered_amt,
                                     ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dosecorrection) AS NUMERIC), 38, 'ROUND_HALF_EVEN') AS dose_correction_amt,
                                     ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.totaldoselimit) AS NUMERIC), 38, 'ROUND_HALF_EVEN') AS total_dose_limit_amt,
                                     ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dailydoselimit) AS NUMERIC), 38, 'ROUND_HALF_EVEN') AS daily_dose_limit_amt,
                                     ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.sessiondoselimit) AS NUMERIC), 38, 'ROUND_HALF_EVEN') AS session_dose_limit_amt,
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.fact_rad_onc_treatment_history');
