...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.rad_onc_patient_history;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.rad_onc_patient_history AS mt USING
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT patient_sk,
             history_query_id
      FROM {{ params.param_cr_core_dataset_name }}.rad_onc_patient_history
      GROUP BY patient_sk,
               history_query_id
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.rad_onc_patient_history');
