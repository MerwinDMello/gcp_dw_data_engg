...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.fact_cancer_patient_financial;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.fact_cancer_patient_financial AS mt USING
  (SELECT DISTINCT cpe.encounter_id AS encounter_id,
   ...
   FROM
     (SELECT consolidated_patient_encounter.*
      FROM {{ params.param_cr_base_views_dataset_name }}.consolidated_patient_encounter
   ...
   INNER JOIN
     (SELECT consolidated_patient_service.patient_type_code_pos1,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.consolidated_patient_service
   ...
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.consolidated_patient_financial AS cpf ON cpe.encounter_id = cpf.encounter_id
   AND upper(trim(cpe.encounter_source_system_code)) = upper(trim(cpf.encounter_source_system_code))
   INNER JOIN
     (SELECT fact_facility.coid
      FROM {{ params.param_cr_base_views_dataset_name }}.fact_facility
   ...
   LEFT OUTER JOIN
     (SELECT pe.company_code,
             ...
             pe.esl_level_5_code,
             pe.esl_level_5_desc
      FROM {{ params.param_cr_base_views_dataset_name }}.patient_esl AS pe
      LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_esl AS re ON upper(rtrim(re.esl_code)) = upper(rtrim(pe.esl_code))
      LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.diagnosis_related_group AS drg ON upper(rtrim(pe.esl_level_5_code)) = upper(rtrim(drg.reimbursement_group_code))
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT encounter_id,
             encounter_source_system_code
      FROM {{ params.param_cr_core_dataset_name }}.fact_cancer_patient_financial
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.fact_cancer_patient_financial');
