...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.fact_rad_onc_course_diagnosis;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.fact_rad_onc_course_diagnosis AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   LEFT OUTER JOIN
     (SELECT fact_rad_onc_patient_diagnosis.source_fact_patient_diagnosis_id,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.fact_rad_onc_patient_diagnosis) AS ra ON dp.factpatientdiagnosisid = ra.source_fact_patient_diagnosis_id
   ...
   LEFT OUTER JOIN
     (SELECT rad_onc_patient_course.source_patient_course_id,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.rad_onc_patient_course) AS ra1 ON dp.dimcourseid = ra1.source_patient_course_id
   ...
   LEFT OUTER JOIN
     (SELECT ref_rad_onc_diagnosis_code.source_diagnosis_code_id,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_diagnosis_code) AS dc ON dp.dimdiagnosiscodeid = dc.source_diagnosis_code_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.fact_rad_onc_course_diagnosis');
