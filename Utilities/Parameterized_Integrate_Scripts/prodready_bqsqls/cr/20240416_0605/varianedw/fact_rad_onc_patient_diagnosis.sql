...
-- bteq << EOF >> $1;;
 ...
 --Job=J_CR_RO_Fact_Rad_Onc_Patient_Diagnosis;;
 --' FOR SESSION;;
...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.fact_rad_onc_patient_diagnosis;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.fact_rad_onc_patient_diagnosis AS mt USING
  (SELECT DISTINCT dp.fact_patient_sk,
   ...
   AND rp.site_sk = rr.site_sk) AS ms ON mt.fact_patient_diagnosis_sk = ms.fact_patient_sk
...
INSERT (fact_patient_diagnosis_sk,
        diagnosis_code_sk,
        patient_sk,
        diagnosis_status_id,
        cell_category_id,
        ...
        diagnosis_type_id,
        site_sk,
        source_fact_patient_diagnosis_id,
        diagnosis_status_date,
        diagnosis_text,
        clinical_text,
        pathology_comment_text,
        node_num,
        positive_node_num,
        log_id,
        run_id,
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT fact_patient_diagnosis_sk
      FROM {{ params.param_cr_core_dataset_name }}.fact_rad_onc_patient_diagnosis
      GROUP BY fact_patient_diagnosis_sk
      HAVING count(*) > 1));

IF DUP_COUNT <> 0 THEN
ROLLBACK TRANSACTION;

RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.fact_rad_onc_patient_diagnosis');
