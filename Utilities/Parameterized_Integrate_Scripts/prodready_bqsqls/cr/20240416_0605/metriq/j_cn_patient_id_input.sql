...
MERGE INTO {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_input_wrk AS mt USING
  (SELECT DISTINCT substr(a.message_control_id_text, 1, 50) AS message_control_id_text,
                   ...
                   a.patient_dw_id,
                   ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(a.pid_pat_account_num) AS NUMERIC), 0, 'ROUND_HALF_EVEN') AS pat_acct_num,
   ...
   FROM
     (SELECT max(stg.msh_msg_control_id) AS message_control_id_text,
             ...
             'H' AS company_code,
             ca.patient_dw_id,
             nullif(format('%#14.0f', ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(translate(stg.pid_pat_account_num, translate(stg.pid_pat_account_num, '0123456789', ''), '')) AS NUMERIC), 0, 'ROUND_HALF_EVEN')), '') AS pid_pat_account_num,
             ...
             CASE
                 ...
                 WHEN length(stg.txa_origination_date_time) >= 8 THEN parse_datetime('%Y%m%d%H%M%S', concat(substr(trim({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_strtok(stg.txa_origination_date_time, '^', 1)), 1, 14), substr('00000000000000', 1, 14 - length(substr(trim({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_strtok(stg.txa_origination_date_time, '^', 1)), 1, 14)))))
             ...
             CASE
                 ...
                 WHEN length(stg.txa_transcription_date_time) >= 8 THEN parse_datetime('%Y%m%d%H%M%S', concat(substr(trim({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_strtok(stg.txa_transcription_date_time, '^', 1)), 1, 14), substr('00000000000000', 1, 14 - length(substr(trim({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_strtok(stg.txa_transcription_date_time, '^', 1)), 1, 14)))))
                 ELSE CAST(NULL AS DATETIME)
      ...
      FROM {{ params.param_cr_stage_dataset_name }}.scri_patient_id_history AS stg
      LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.clinical_acctkeys AS ca ON ca.pat_acct_num = ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(translate(stg.pid_pat_account_num, translate(stg.pid_pat_account_num, '0123456789', ''), '')) AS NUMERIC), 0, 'ROUND_HALF_EVEN')
      AND upper(rtrim(ca.coid)) = upper(rtrim(CASE
                                                  WHEN length(trim(stg.coid)) = 4
                                                       AND trim(stg.coid) IS NOT NULL THEN concat('0', trim(stg.coid))
                                                  ELSE trim(stg.coid)
                                              END))
      LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_patient_type_status AS pt ON upper(trim(stg.patient_type_status)) = upper(trim(pt.patient_type_status_code))
      WHERE upper(rtrim(stg.message_flag)) IN('PAT',
                                              'RAD',
                                              'RADN')
        AND stg.msh_msg_control_id IS NOT NULL
      GROUP BY upper(stg.msh_msg_control_id),
               2,
               upper(nullif(CASE
                                WHEN length(trim(stg.coid)) = 4
                                     AND trim(stg.coid) IS NOT NULL THEN concat('0', trim(stg.coid))
               ...
               5,
               6,
               upper(nullif(stg.pid_medical_record_num, '')),
               upper(nullif(stg.pid_medical_record_urn, '')),
               upper(nullif(stg.msh_msg_type_message_code, '')),
               upper(nullif(stg.message_flag, '')),
               upper(nullif(stg.msh_msg_type_trigger_event, '')),
               12,
               13,
               14,
               16,
               17) AS a QUALIFY row_number() OVER (PARTITION BY upper(a.message_control_id_text)
                                                   ORDER BY a.etl_insert_date_time DESC) = 1) AS ms ON mt.message_control_id_text = ms.message_control_id_text
