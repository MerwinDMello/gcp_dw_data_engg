...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.ref_icd_oncology
WHERE upper(ref_icd_oncology.icd_oncology_code) NOT IN
    (SELECT upper(ref_icd_oncology_stg.icd_oncology_code) AS icd_oncology_code
     FROM {{ params.param_cr_stage_dataset_name }}.ref_icd_oncology_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_icd_oncology AS mt USING
  (SELECT DISTINCT stg.icd_oncology_code AS icd_oncology_code,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.ref_icd_oncology_stg AS stg
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_icd_oncology AS rio ON upper(rtrim(stg.icd_oncology_code)) = upper(rtrim(rio.icd_oncology_code))
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_icd_oncology');
