...
SELECT row_number() OVER (
                          ORDER BY upper(max(stg.unique_message_id))) AS cancer_patient_id_output_sk,
                         ...
                         'H' AS company_code,
                         ca.patient_dw_id,
                         ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(nullif(format('%#14.0f', ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(translate(stg.patient_account_number, translate(stg.patient_account_number, '0123456789', ''), '')) AS NUMERIC), 0, 'ROUND_HALF_EVEN')), '')) AS NUMERIC), 0, 'ROUND_HALF_EVEN') AS pat_acct_num,
                         ...
                         max(nullif(stg.message_flag, '')), -- CASE WHEN STG.MESSAGE_ORIGIN_OR_REQUESTED_DATE_TIME = '' THEN NULL ELSE CAST(RPAD(STG.MESSAGE_ORIGIN_OR_REQUESTED_DATE_TIME,12,'0') AS TIMESTAMP(0) FORMAT'YYYYMMDDHHMI' ) END AS MESSAGE_ORIGIN_OR_REQUESTED_DATE_TIME,
 CASE
 ...
 END AS message_origin_or_requested_date_time, -- CASE WHEN STG.MESSAGE_SIGNED_OR_OBSERVATION_DATE_TIME= '' THEN NULL ELSE CAST(RPAD(STG.MESSAGE_SIGNED_OR_OBSERVATION_DATE_TIME,12,'0') AS TIMESTAMP(0) FORMAT'YYYYMMDDHHMI' ) END AS MESSAGE_SIGNED_OR_OBSERVATION_DATE_TIME,
 CASE
     ...
     ELSE CAST(NULL AS DATETIME)
 END AS message_signed_or_observation_date_time, -- CASE WHEN STG.INGESTION_DATE_TIME='' THEN NULL ELSE CAST(SUBSTR(STG.INGESTION_DATE_TIME,1,10) || ' ' || SUBSTR(STG.INGESTION_DATE_TIME,12,8) AS TIMESTAMP(0)) END AS INGESTION_DATE_TIME,
 CASE
 ...
 END AS ingestion_date_time, -- CASE WHEN STG.MESSAGE_CREATED_DATETIME= '' THEN NULL ELSE CAST(RPAD(STG.MESSAGE_CREATED_DATETIME,12,'0') AS TIMESTAMP(0) FORMAT'YYYYMMDDHHMI' ) END AS MESSAGE_CREATED_DATETIME,
 CASE
 ...
 substr(max(nullif(stg.document_type_name_of_coding_sys, '')), 1, 20), -- CASE WHEN STG.ETL_FIRSTINSERT_DATETIME='' THEN NULL ELSE CAST(CAST(STG.ETL_FIRSTINSERT_DATETIME AS CHAR(14)) AS TIMESTAMP(0) FORMAT'YYYYMMDDHHMISS' ) END AS ETL_FIRSTINSERT_DATETIME,
 CASE
 ...
 CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(stg.transition_of_care) AS INT64) AS transition_of_care,
 substr(max(nullif(stg.user_action, '')), 1, 20),
 max(nullif(stg.user_action_criticality, '')), -- CASE WHEN STG.USER_ACTION_DATE_TIME='' THEN NULL ELSE CAST(SUBSTR(STG.USER_ACTION_DATE_TIME,1,10) || ' ' || SUBSTR(STG.USER_ACTION_DATE_TIME,12,8) AS TIMESTAMP(0)) END AS USER_ACTION_DATE_TIME,
 CASE
 ...
 max(nullif(stg.usr, '')), -- CASE WHEN STG.REPORT_ASSIGNED_TO_USER_AT='' THEN NULL ELSE CAST(SUBSTR(STG.REPORT_ASSIGNED_TO_USER_AT,1,10) || ' ' || SUBSTR(STG.REPORT_ASSIGNED_TO_USER_AT,12,8) AS TIMESTAMP(0)) END AS REPORT_ASSIGNED_TO_USER_AT,
 CASE
 ...
 max(CASE upper(rtrim(stg.tumor_type_benign_brain))
         WHEN 'TRUE' THEN 'Y'
         WHEN 'FALSE' THEN 'N'
         ELSE CAST(NULL AS STRING)
     END) AS benign_brain_tumor_type_ind,
 max(CASE upper(rtrim(stg.tumor_type_met_to_brain))
         WHEN 'TRUE' THEN 'Y'
         WHEN 'FALSE' THEN 'N'
         ELSE CAST(NULL AS STRING)
     END) AS met_to_brain_tumor_type_ind,
 max(CASE upper(rtrim(stg.tumor_type_na))
         WHEN 'TRUE' THEN 'Y'
         WHEN 'FALSE' THEN 'N'
         ELSE CAST(NULL AS STRING)
...
FROM {{ params.param_cr_stage_dataset_name }}.cdm_synthesys_patientid_results AS stg
LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.clinical_acctkeys AS ca ON ca.pat_acct_num = ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(translate(stg.patient_account_number, translate(stg.patient_account_number, '0123456789', ''), '')) AS NUMERIC), 0, 'ROUND_HALF_EVEN')
...
GROUP BY upper(substr(stg.unique_message_id, 1, 50)),
         ...
         5,
         6,
         upper(nullif(stg.patient_mrn, '')),
         upper(substr(nullif(stg.patient_urn, ''), 1, 30)),
         upper(substr(nullif(stg.patient_last_name, ''), 1, 50)),
         upper(substr(nullif(stg.patient_first_name, ''), 1, 50)),
         11,
         upper(nullif(stg.patient_gender, '')),
         upper(nullif(stg.social_security_number, '')),
         upper(nullif(stg.patient_address_1, '')),
         upper(nullif(stg.patient_address_2, '')),
         upper(substr(nullif(stg.patient_city, ''), 1, 50)),
         upper(nullif(stg.patient_state, '')),
         upper(substr(nullif(stg.patient_zip_code, ''), 1, 10)),
         upper(nullif(stg.patient_type_status, '')),
         upper(nullif(stg.message_type, '')),
         upper(nullif(stg.message_event_type, '')),
         upper(nullif(stg.message_flag, '')),
         23,
         24,
         25,
         26,
         upper(substr(nullif(stg.document_type_identifier, ''), 1, 20)),
         upper(substr(nullif(stg.document_type_text, ''), 1, 50)),
         upper(substr(nullif(stg.document_type_name_of_coding_sys, ''), 1, 20)),
         30,
         upper(nullif(replace(stg.model_predicted_primary_site_icdo3, '.', ''), '')),
         upper(substr(nullif(stg.model_predicted_primary_site, ''), 1, 255)),
         upper(nullif(replace(stg.suggested_primary_site_icdo3, '.', ''), '')),
         upper(substr(nullif(stg.suggested_primary_site, ''), 1, 255)),
         upper(nullif(replace(stg.submitted_primary_site_icdo3, '.', ''), '')),
         upper(substr(nullif(stg.submitted_primary_site, ''), 1, 255)),
         38,
         upper(substr(nullif(stg.user_action, ''), 1, 20)),
         upper(nullif(stg.user_action_criticality, '')),
         41,
         upper(nullif(stg.usr, '')),
         43,
         upper(substr(nullif(trim(stg.attending_physician), ''), 1, 100)),
         upper(substr(nullif(trim(stg.primary_care_physician), ''), 1, 100)),
         upper(substr(nullif(stg.pcp_phone, ''), 1, 15)),
         47,
         48,
         upper(CASE
                   WHEN upper(rtrim(stg.model_predicted_sarcoma)) = 'SARCOMA' THEN 'Y'
         ...
         upper(CASE
                   ...
                   ELSE 'N'
               END),
         upper(CASE upper(rtrim(stg.tumor_type_benign_brain))
                   WHEN 'TRUE' THEN 'Y'
                   WHEN 'FALSE' THEN 'N'
                   ELSE CAST(NULL AS STRING)
               END),
         upper(CASE upper(rtrim(stg.tumor_type_met_to_brain))
                   WHEN 'TRUE' THEN 'Y'
                   WHEN 'FALSE' THEN 'N'
                   ELSE CAST(NULL AS STRING)
               END),
         upper(CASE upper(rtrim(stg.tumor_type_na))
                   WHEN 'TRUE' THEN 'Y'
                   WHEN 'FALSE' THEN 'N'
                   ELSE CAST(NULL AS STRING)
               END);
