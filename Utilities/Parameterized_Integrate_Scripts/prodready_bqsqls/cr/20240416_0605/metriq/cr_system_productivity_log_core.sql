...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cr_system_productivity_log;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cr_system_productivity_log AS mt USING
  (SELECT DISTINCT cr_system_productivity_log_wrk.system_productivity_log_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_system_productivity_log_wrk) AS ms ON mt.system_productivity_log_id = ms.system_productivity_log_id
  ...
  (SELECT count(*)
   FROM
     (SELECT system_productivity_log_id
      FROM {{ params.param_cr_core_dataset_name }}.cr_system_productivity_log
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cr_system_productivity_log');
