     ...
     (SELECT dates.cancer_patient_driver_sk,
      ...
      FROM
        (SELECT t2.cancer_patient_driver_sk,
         ...
         INNER JOIN {{ params.param_cr_base_views_dataset_name }}.cancer_patient_driver AS t2 ON t1.patient_dw_id = t2.cp_patient_id
         ...
         INNER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_patient AS t2 ON t1.cn_patient_id = t2.nav_patient_id
         INNER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_patient_diagnosis AS t3 ON t2.nav_patient_id = t3.nav_patient_id
         ...
         UNION DISTINCT SELECT t1.cancer_patient_driver_sk AS cancer_patient_driver_sk,
                               ...
                               min(diagnosis_date) AS date_driver
         FROM {{ params.param_cr_base_views_dataset_name }}.cancer_patient_driver AS t1
         INNER JOIN {{ params.param_cr_base_views_dataset_name }}.cr_patient_diagnosis_detail AS t2 ON t1.cr_patient_id = t2.cr_patient_id
         ...
         FROM {{ params.param_cr_base_views_dataset_name }}.cancer_patient_driver AS t1
         INNER JOIN {{ params.param_cr_base_views_dataset_name }}.cr_patient_tumor AS t2 ON t1.cr_patient_id = t2.cr_patient_id
...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cr_patient_date_driver;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cr_patient_date_driver AS mt USING
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cr_patient_date_driver');
  ...
  (SELECT DISTINCT b.cancer_patient_driver_sk AS cancer_patient_driver_sk,
   ...
   FROM
     (SELECT fe.patient_dw_id,
             ...
             sc.hospemr_date_driver
      FROM {{ params.param_cr_base_views_dataset_name }}.fact_encounter AS fe
      INNER JOIN {{ params.param_cr_auth_base_views_dataset_name }}.clinical_facility AS cf ON upper(rtrim(fe.coid)) = upper(rtrim(cf.coid))
 ...
 sc ON upper(rtrim(fe.patient_market_urn)) = upper(rtrim(sc.patient_market_urn_text))
   ...
   WHERE (b.patient_dw_id,
       ...
       (SELECT AS STRUCT cr_patient_date_driver.patient_dw_id,
        ...
        FROM {{ params.param_cr_base_views_dataset_name }}.cr_patient_date_driver) ) AS ms ON mt.cancer_patient_driver_sk = ms.cancer_patient_driver_sk
AND mt.patient_dw_id = ms.patient_dw_id
AND (coalesce(mt.cancer_diagnosis_date, DATE '1970-01-01') = coalesce(ms.cancer_diagnosis_date, DATE '1970-01-01')
     AND coalesce(mt.cancer_diagnosis_date, DATE '1970-01-02') = coalesce(ms.cancer_diagnosis_date, DATE '1970-01-02'))
AND (coalesce(mt.cancer_diagnosis_90_day_prior_date, DATE '1970-01-01') = coalesce(ms.cancer_diagnosis_90_day_prior_date, DATE '1970-01-01')
