...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cr_patient_tumor;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cr_patient_tumor AS mt USING
 ...
 coalesce(mltp3.master_lookup_sid,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_tumor_stg AS stg
   LEFT OUTER JOIN -- 0
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT tumor_id
      FROM {{ params.param_cr_core_dataset_name }}.cr_patient_tumor
      GROUP BY tumor_id
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cr_patient_tumor');
...
SELECT max(t3.group_id) AS groupid,
       t3.lookup_id,
       t1.tumorid
FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_tumor_stg AS t1
INNER JOIN {{ params.param_cr_stage_dataset_name }}.ref_lookup_groups_stg AS t3 ON t3.beginhisto <= CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(left(t1.histology, 4)) AS FLOAT64)
AND t3.endhisto >= CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(left(t1.histology, 4)) AS FLOAT64)
AND t3.beginprimarysite <= CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(CASE
                                                                                                         WHEN upper(rtrim(right(t1.primarysite, 3))) = '***' THEN '000'
                                                                                                         ELSE right(t1.primarysite, 3)
                                                                                                     END) AS FLOAT64)
AND t3.endprimarysite >= CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(CASE
                                                                                                       WHEN upper(rtrim(right(t1.primarysite, 3))) = '***' THEN '000'
                                                                                                       ELSE right(t1.primarysite, 3)
                                                                                                   END) AS FLOAT64)
...
UPDATE {{ params.param_cr_core_dataset_name }}.cr_patient_tumor AS tu
...
WHERE tu.tumor_id = stg.tumorid
  ...
  AND vt_cm1.groupid = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(vt_lkp3.lookup_sub_code) AS FLOAT64)
  AND vt_cm1.tumorid = stg.tumorid;
...
WHERE tu.tumor_id = stg.tumorid
  ...
  AND vt_dss.groupid = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(vt_lkp4.lookup_sub_code) AS FLOAT64)
  AND vt_dss.tumorid = stg.tumorid;
...
WHERE tu.tumor_id = stg.tumorid
  ...
  AND vt_hs.groupid = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(vt_lkp5.lookup_sub_code) AS FLOAT64)
  AND vt_hs.tumorid = stg.tumorid;
...
WHERE tu.tumor_id = stg.tumorid
  ...
  AND vt_is1.groupid = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(vt_lkp6.lookup_sub_code) AS FLOAT64)
  AND vt_is1.tumorid = stg.tumorid;
...
WHERE tu.tumor_id = stg.tumorid
  ...
  AND vt_pcs.groupid = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(vt_lkp8.lookup_sub_code) AS FLOAT64)
  AND vt_pcs.tumorid = stg.tumorid;
...
WHERE tu.tumor_id = stg.tumorid
  ...
  AND vt_psss.groupid = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(vt_lkp16.lookup_sub_code) AS FLOAT64)
  AND vt_psss.tumorid = stg.tumorid;
...
WHERE tu.tumor_id = stg.tumorid
  ...
  AND vt_pssh.groupid = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(vt_lkp22.lookup_sub_code) AS FLOAT64)
  AND vt_pssh.tumorid = stg.tumorid;
