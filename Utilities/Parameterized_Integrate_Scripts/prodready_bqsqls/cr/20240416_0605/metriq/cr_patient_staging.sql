...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.cr_patient_staging_wrk;
...
INSERT INTO {{ params.param_cr_stage_dataset_name }}.cr_patient_staging_wrk (cr_patient_staging_sid, cr_patient_id, tumor_id, ajcc_stage_id, cancer_stage_classification_method_code, cancer_stage_type_code, cancer_stage_result_text, source_system_code, dw_last_update_date_time)
...
FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_staging_sample_stage AS stg
LEFT OUTER JOIN --  LEFT JOIN EDWCR_STAGING.REF_TUMOR_STAGE TR
 -- 	ON STG.TUMORID=TR.TUMORID1
 -- LEFT JOIN EDWCR.REF_AJCC_STAGE AJCC
 {{ params.param_cr_base_views_dataset_name }}.ref_ajcc_stage AS ajcc ON upper(rtrim(stg.code)) = upper(rtrim(ajcc.ajcc_stage_code))
AND stg.sub = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(ajcc.ajcc_stage_sub_code) AS FLOAT64)
...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cr_patient_staging;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cr_patient_staging AS mt USING
  (SELECT DISTINCT cr_patient_staging_wrk.cr_patient_staging_sid,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_staging_wrk
   GROUP BY 1,
            2,
            3,
            4,
  ...
  (SELECT count(*)
   FROM
     (SELECT cr_patient_staging_sid
      FROM {{ params.param_cr_core_dataset_name }}.cr_patient_staging
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cr_patient_staging');
