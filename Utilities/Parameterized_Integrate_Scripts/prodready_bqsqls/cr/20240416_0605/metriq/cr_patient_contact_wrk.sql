...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.cr_patient_contact_wrk;
...
INSERT INTO {{ params.param_cr_stage_dataset_name }}.cr_patient_contact_wrk (patient_contact_id, cr_patient_id, contact_relation_id, contact_type_id, contact_num_code, contact_first_name, contact_last_name, contact_middle_name, preferred_contact_method_text, source_system_code, dw_last_update_date_time)
SELECT src.patient_contact_id,
       ...
       coalesce(rlc1.master_lookup_sid,
                  (SELECT rcl1.master_lookup_sid
                   ...
                   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS nm ON rcl1.lookup_id = nm.lookup_sid
...
FROM
  (SELECT stg.patient_contact_id AS patient_contact_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_contact_stg AS stg) AS src
LEFT OUTER JOIN
  (SELECT lkp.lookup_code,
          lkp.master_lookup_sid
   FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS lkp
   INNER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS nm ON lkp.lookup_id = nm.lookup_sid
