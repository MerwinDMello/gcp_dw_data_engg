...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.cr_patient_radiation_oncology_wrk;
...
INSERT INTO {{ params.param_cr_stage_dataset_name }}.cr_patient_radiation_oncology_wrk (radiation_id, treatment_id, radiation_type_id, radiation_hospital_id, radiation_treatment_start_date, radiation_treatment_end_date, source_system_code, dw_last_update_date_time)
...
FROM
  (SELECT stg.radiation_id AS radiation_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_radiation_oncology_stg AS stg) AS src
LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_hospital AS a ON upper(rtrim(src.radiation_hospital_id)) = upper(rtrim(a.hospital_code))
LEFT OUTER JOIN
