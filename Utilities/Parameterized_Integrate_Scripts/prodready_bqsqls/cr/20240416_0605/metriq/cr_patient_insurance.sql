...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cr_patient_insurance;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cr_patient_insurance AS mt USING
  (SELECT DISTINCT stg.tumor_id AS tumor_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_insurance_stg AS stg
   LEFT OUTER JOIN
  ...
  (SELECT count(*)
   FROM
     (SELECT cr_patient_id,
             tumor_id
      FROM {{ params.param_cr_core_dataset_name }}.cr_patient_insurance
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cr_patient_insurance');
