...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cr_patient_diagnosis_detail;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cr_patient_diagnosis_detail AS mt USING
  (SELECT DISTINCT stg.tumor_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_diagnosis_detail_stg AS stg
   LEFT OUTER JOIN
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT tumor_id,
             cr_patient_id
      FROM {{ params.param_cr_core_dataset_name }}.cr_patient_diagnosis_detail
      GROUP BY tumor_id,
               cr_patient_id
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cr_patient_diagnosis_detail');
