...
-- bteq << EOF >> $1;;
 ...
 --Job=J_CN_CANCER_PAT_ID_INPUT_DRIVER;;
 --' FOR SESSION;;
...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cancer_patient_id_input_driver;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cancer_patient_id_input_driver AS mt USING
  (SELECT DISTINCT CAST(row_number() OVER (
                                           ORDER BY sk.cancer_patient_driver_sk) AS NUMERIC) AS cancer_patient_id_input_driver_sk,
   ...
   LEFT OUTER JOIN
     (SELECT a1.cancer_patient_driver_sk,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_output_driver AS a1 QUALIFY row_number() OVER (PARTITION BY upper(a1.message_control_id_text)
                                                                                                                          ORDER BY a1.user_action_date_time DESC) = 1) AS sk ON upper(rtrim(a.message_control_id_text)) = upper(rtrim(sk.message_control_id_text))) AS ms ON mt.cancer_patient_id_input_driver_sk = ms.cancer_patient_id_input_driver_sk
...
INSERT (cancer_patient_id_input_driver_sk,
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT cancer_patient_id_input_driver_sk
      FROM {{ params.param_cr_core_dataset_name }}.cancer_patient_id_input_driver
      GROUP BY cancer_patient_id_input_driver_sk
      HAVING count(*) > 1));

IF DUP_COUNT <> 0 THEN
ROLLBACK TRANSACTION;

RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cancer_patient_id_input_driver');
