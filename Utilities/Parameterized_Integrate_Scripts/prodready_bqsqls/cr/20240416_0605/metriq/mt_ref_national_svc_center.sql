...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.ref_national_service_center;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_national_service_center AS mt USING
  (SELECT DISTINCT ref_national_svc_center_stg.id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.ref_national_svc_center_stg) AS ms ON mt.nsc_id = ms.id
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT nsc_id
      FROM {{ params.param_cr_core_dataset_name }}.ref_national_service_center
      GROUP BY nsc_id
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_national_service_center');
