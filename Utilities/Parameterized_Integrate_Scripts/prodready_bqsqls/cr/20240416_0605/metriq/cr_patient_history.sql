...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cr_patient_history;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cr_patient_history AS mt USING
  (SELECT DISTINCT stg.patientid AS cr_patient_id,
                   ...
                   coalesce(lkp.master_lookup_sid,
                              (SELECT rlc.master_lookup_sid
                               ...
                               WHERE upper(rtrim(rln.lookup_name)) = 'SMOKE HISTORY'
                                 AND upper(rtrim(rlc.lookup_code)) = '-99' )) AS smoked_history_id,
                   coalesce(lkp1.master_lookup_sid,
                              (SELECT rlc.master_lookup_sid
                               FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS rln
                               INNER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
                               WHERE upper(rtrim(rln.lookup_name)) = 'FAMILY HISTORY'
                                 AND upper(rtrim(rlc.lookup_code)) = '-99' )) AS family_cancer_history_type_id,
                   coalesce(lkp2.master_lookup_sid,
                              (SELECT rlc.master_lookup_sid
                               FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS rln
                               INNER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
                               WHERE upper(rtrim(rln.lookup_name)) = 'PATIENT HISTORY'
                                 AND upper(rtrim(rlc.lookup_code)) = '-99' )) AS patient_cancer_history_type_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_history_stg AS stg
   LEFT OUTER JOIN
     (SELECT rln.lookup_sid,
      ...
      FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS rln
      INNER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
      WHERE upper(rtrim(rln.lookup_name)) = 'SMOKE HISTORY' ) AS lkp ON upper(rtrim(stg.smokhist)) = upper(rtrim(lkp.lookup_code))
   LEFT OUTER JOIN
     (SELECT rln.lookup_sid,
             rlc.lookup_code,
             rlc.lookup_sub_code,
      ...
      FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS rln
      INNER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
      WHERE upper(rtrim(rln.lookup_name)) = 'FAMILY HISTORY' ) AS lkp1 ON upper(rtrim(stg.famhxca)) = upper(rtrim(lkp1.lookup_code))
   LEFT OUTER JOIN
     (SELECT rln.lookup_sid,
             rlc.lookup_code,
             rlc.lookup_sub_code,
      ...
      FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS rln
      INNER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
      WHERE upper(rtrim(rln.lookup_name)) = 'PATIENT HISTORY' ) AS lkp2 ON upper(rtrim(stg.pthxca)) = upper(rtrim(lkp2.lookup_code))) AS ms ON mt.cr_patient_id = ms.cr_patient_id
...
INSERT (cr_patient_id,
        tumor_id,
        smoked_history_id,
        family_cancer_history_type_id,
        patient_cancer_history_type_id,
        tobacco_amt_text,
        years_tobacco_used_num_text,
        source_system_code,
  ...
  (SELECT count(*)
   FROM
     (SELECT cr_patient_id,
             tumor_id
      FROM {{ params.param_cr_core_dataset_name }}.cr_patient_history
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cr_patient_history');
