...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.navque_patient_tumor_driver;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.navque_patient_tumor_driver AS mt USING
  (SELECT DISTINCT CAST(row_number() OVER (
   ...
   FROM {{ params.param_cr_base_views_dataset_name }}.navque_history AS a
   LEFT OUTER JOIN
     (SELECT cpio.*
      FROM
        (SELECT DISTINCT a_0.cancer_patient_driver_sk,
         ...
         FROM {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_output_driver AS a_0) AS cpio QUALIFY row_number() OVER (PARTITION BY upper(cpio.message_control_id_text)
   ...
   LEFT OUTER JOIN
     (SELECT cancer_tumor_driver.cp_icd_oncology_code,
      ...
      FROM {{ params.param_cr_base_views_dataset_name }}.cancer_tumor_driver QUALIFY row_number() OVER (PARTITION BY cancer_tumor_driver.cn_navque_tumor_type_id
  ...
  (SELECT count(*)
   FROM
     (SELECT navque_patient_tumor_driver_sk
      FROM {{ params.param_cr_core_dataset_name }}.navque_patient_tumor_driver
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.navque_patient_tumor_driver');
