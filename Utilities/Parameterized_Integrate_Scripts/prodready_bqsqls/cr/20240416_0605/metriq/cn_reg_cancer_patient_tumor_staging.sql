...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cancer_patient_tumor_staging;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cancer_patient_tumor_staging AS mt USING
     ...
     (SELECT DISTINCT cptd.cancer_patient_tumor_driver_sk,
      ...
      LEFT OUTER JOIN
        (SELECT cpt.cr_patient_id,
         ...
         LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_ajcc_stage AS ras ON cps.ajcc_stage_id = ras.ajcc_stage_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cancer_patient_tumor_staging');
