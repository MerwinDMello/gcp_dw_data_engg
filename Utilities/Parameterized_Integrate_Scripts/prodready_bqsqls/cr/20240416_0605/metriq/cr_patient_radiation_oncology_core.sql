...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cr_patient_radiation_oncology;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cr_patient_radiation_oncology AS mt USING
  (SELECT DISTINCT cr_patient_radiation_oncology_wrk.radiation_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_radiation_oncology_wrk) AS ms ON mt.radiation_id = ms.radiation_id
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT radiation_id
      FROM {{ params.param_cr_core_dataset_name }}.cr_patient_radiation_oncology
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cr_patient_radiation_oncology');
