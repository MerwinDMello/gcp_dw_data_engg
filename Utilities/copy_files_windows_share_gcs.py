...
# excel_files_path_folder = r"C:\Merwin\Corrections\Para\Excel Files\Fixed_May06"
mount_drive = "l"
file_server =  "xrdcnpsvmcfs01_cfs_ag1.hca.corpad.net"
file_share = "CCCShare"
shared_drive = r"\\" + f"{file_server}\{file_share}"
file_folder = "CCLogReports"
gcs_bucket = "eim-parallon-cs-datamig-dev-0003"
gcs_parent_directory = "edwradata/srcfiles/govreports"
input_file_extension = "xlsx"

file_patterns_config = r"InputFiles\file_patterns.yaml"
...
mount_drive = "l"
subprocess.call(f'net use {mount_drive}: /del', shell=True)

# Connect to shared drive, use drive letter M
print(shared_drive)
subprocess.call(f'net use {mount_drive}: {shared_drive}', shell=True) 

for sub_directory in file_patterns_json['processing_groups']:
    print(sub_directory)
    file_list = [filename for filename in os.listdir(f"{mount_drive}:\{file_folder}\{sub_directory}") if filename.endswith(input_file_extension)]
    full_file_count = 0
...
#         run_gsutil_command(source_local_path, destination_gcs_path)
        ...
        for filename in file_list:
            ...
            if is_pattern:
                ...
                file_count += 1
        print(file_count)
        full_file_count += file_count
    print(full_file_count)