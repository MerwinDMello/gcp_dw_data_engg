...
CREATE OR REPLACE VIEW {{ params.param_pbs_views_dataset_name }}.smry_ar_cash_collections_lp AS SELECT
    a.rptg_period,
  ...
  FROM
    {{ params.param_pbs_base_views_dataset_name }}.smry_ar_cash_collections_lp AS a
    INNER JOIN {{ params.param_auth_base_views_dataset_name }}.secref_facility AS sf ON upper(a.coid) = upper(sf.co_id)
...
SELECT
  ...
  FROM
    {{ params.param_pbs_base_views_dataset_name }}.sla_metric_adjustment AS a
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.facility_dimension AS fd ON upper(fd.coid) = upper(a.parent_code)
    INNER JOIN {{ params.param_auth_base_views_dataset_name }}.secref_facility AS sf ON upper(a.parent_code) = upper(sf.co_id)
