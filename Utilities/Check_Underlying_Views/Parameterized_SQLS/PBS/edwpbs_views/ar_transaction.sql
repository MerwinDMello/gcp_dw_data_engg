...
CREATE OR REPLACE VIEW {{ params.param_pbs_views_dataset_name }}.ar_transaction AS SELECT
    bv.company_code,
    bv.coid,
    ...
    bv.discharge_date
  FROM
    {{ params.param_pbs_base_views_dataset_name }}.ar_transaction AS bv
    INNER JOIN {{ params.param_auth_base_views_dataset_name }}.secref_facility AS sf ON upper(bv.coid) = upper(sf.co_id)
