...
CREATE OR REPLACE VIEW {{ params.param_pbs_views_dataset_name }}.dim_rcm_organization
  ...
  AS SELECT
      ...
      a.ssc_alias_name,
      CASE
        WHEN upper(a.ssc_coid) IN(
          '08942', '08591'
        )
         AND (upper(a.company_code) = 'H'
         OR upper(a.customer_code) IN(
          'PDM', 'ADV'
        )) THEN 'Nashville SSC'
        WHEN upper(a.ssc_coid) IN(
          '08942', '08591'
      ...
      ROUND(a.discrepancy_threshold_amt, 3, 'ROUND_HALF_EVEN') AS discrepancy_threshold_amt,
      ROUND(a.sma_high_dollar_threshold_amt, 3, 'ROUND_HALF_EVEN') AS sma_high_dollar_threshold_amt,
      a.hsc_member_ind,
    ...
    FROM
      {{ params.param_pbs_base_views_dataset_name }}.dim_rcm_organization AS a
  ;
