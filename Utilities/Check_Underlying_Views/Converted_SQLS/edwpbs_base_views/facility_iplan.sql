...
-- Source: internal_metastore/db_hca-hin-dev-cur-parallon/schema_edwpbs_base_views/facility_iplan.memory
-- Translated from: Teradata
-- Translated to: BigQuery

CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_base_views.facility_iplan AS SELECT
    ROUND(facility_iplan.payor_dw_id, 0, 'ROUND_HALF_EVEN') AS payor_dw_id,
    facility_iplan.coid,
    facility_iplan.company_code,
    facility_iplan.iplan_id,
    facility_iplan.plan_desc,
    facility_iplan.payor_name,
    ROUND(facility_iplan.payor_id, 0, 'ROUND_HALF_EVEN') AS payor_id,
    ...
    ROUND(facility_iplan.iplan_financial_class_code, 0, 'ROUND_HALF_EVEN') AS iplan_financial_class_code,
    ...
    ROUND(facility_iplan.charge_benefit_code, 0, 'ROUND_HALF_EVEN') AS charge_benefit_code,
    ROUND(facility_iplan.op_coinsurance_code, 0, 'ROUND_HALF_EVEN') AS op_coinsurance_code,
    ...
    ROUND(facility_iplan.ancillary_maximum_amt, 3, 'ROUND_HALF_EVEN') AS ancillary_maximum_amt,
    ...
    ROUND(facility_iplan.inpatient_npi, 0, 'ROUND_HALF_EVEN') AS inpatient_npi,
    ROUND(facility_iplan.outpatient_npi, 0, 'ROUND_HALF_EVEN') AS outpatient_npi,
  ...
  FROM
    `hca-hin-dev-cur-parallon`.edwpf_base_views.facility_iplan
;
