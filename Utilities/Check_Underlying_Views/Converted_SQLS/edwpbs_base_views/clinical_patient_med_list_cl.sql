...
-- Source: internal_metastore/db_hca-hin-dev-cur-parallon/schema_edwpbs_base_views/clinical_patient_med_list_cl.memory
-- Translated from: Teradata
-- Translated to: BigQuery

CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_base_views.clinical_patient_med_list_cl AS SELECT
    ROUND(clinical_patient_med_list.patient_dw_id, 0, 'ROUND_HALF_EVEN') AS patient_dw_id,
    ...
    ROUND(clinical_patient_med_list.audit_event_seconds_amt, 0, 'ROUND_HALF_EVEN') AS audit_event_seconds_amt,
    clinical_patient_med_list.company_code,
    clinical_patient_med_list.coid,
    ROUND(clinical_patient_med_list.pat_acct_num, 0, 'ROUND_HALF_EVEN') AS pat_acct_num,
    ...
    clinical_patient_med_list.finalized_date_time,
    clinical_patient_med_list.finalized_user_mnem_cs,
    clinical_patient_med_list.source_system_code,
    clinical_patient_med_list.dw_last_update_date_time
  FROM
    `hca-hin-dev-cur-parallon`.edwcl_base_views.clinical_patient_med_list
;
