...
DECLARE cw_const STRING DEFAULT bqutil.fn.cw_td_normalize_number(substr(format_date('%Y%m', date_add(current_date('US/Central'), interval -1 MONTH)), 1, 6));
CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_views.fact_rcom_ar_patient_lvl_nkey AS SELECT
    ...
    a.payor_prorated_liability_amt - a.prorated_liability_sys_adj_amt - a.payor_payment_amt - a.write_off_amt - a.payor_adjustment_amt - a.payor_contractual_amt AS payor_balance_amt,
    CASE
      WHEN a.iplan_insurance_order_num = 0 THEN a.ar_patient_amt
      ELSE a.ar_insurance_amt
    END AS payor_balance_amt_calc,
    a.payor_bill_cnt,
  ...
  FROM
    ...
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.eis_date_dim AS g ON a.date_sid = g.time_id
     AND a.date_sid = CAST(cw_const as FLOAT64)
    ...
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.secref_facility AS o ON a.coid = o.co_id
     AND a.company_code = o.company_code
