...
CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_views.dim_organization AS SELECT
    ...
    CAST(bqutil.fn.cw_td_normalize_number(a.unit_num) as INT64) AS unit_num_sid,
    CAST(bqutil.fn.cw_td_normalize_number(a.coid) as INT64) AS coid_sid,
    CAST(bqutil.fn.cw_td_normalize_number(substr(b.group_code, 2, 5)) as INT64) AS group_sid,
    CAST(bqutil.fn.cw_td_normalize_number(substr(b.division_code, 2, 5)) as INT64) AS division_sid,
    CAST(bqutil.fn.cw_td_normalize_number(substr(b.market_code, 2, 5)) as INT64) AS market_sid,
    b.ssc_code,
    b.customer_code,
    ...
    b.sma_high_dollar_threshold_amt,
    b.ssc_alias_name,
    b.corporate_name,
    b.customer_name,
    b.discrepancy_threshold_amt,
    ...
    CASE
      WHEN b.ssc_coid IN(
       ...
       AND (b.company_code = 'H'
       OR b.customer_code IN(
        'PDM', 'ADV'
      )) THEN '63333'
      WHEN b.ssc_coid IN(
        '08942', '08591'
    ...
    CASE
      ...
      WHEN b.customer_code = 'HHH' THEN 'Specialty Services'
      ELSE 'Unknown'
  ...
  FROM
    ...
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.facility_dimension AS a ON a.coid = b.coid
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_views.bi_cpc_entity_hierarchy AS c ON c.coid = b.coid
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.facility_dimension_eom AS d ON d.coid = b.coid
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwra_views.ref_cc_org_structure AS e ON e.coid = b.coid
