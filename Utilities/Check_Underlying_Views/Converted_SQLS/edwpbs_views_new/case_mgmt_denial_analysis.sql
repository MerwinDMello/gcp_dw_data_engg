...
CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_views.case_mgmt_denial_analysis AS SELECT
    ...
    fact_patient.financial_class_code AS fp_financial_class_code,
    max(CASE
    ...
    max(CASE
      WHEN cm_encounter.iq_adm_rev_type_ip_ind = 'Y'
       OR cm_encounter.iq_adm_rev_type_obs_ind = 'Y' THEN 'Y'
      ELSE 'N'
    END) AS iq_adm_rev_type_ip_obs_ind,
    cm_encounter.iq_adm_rev_type_ip_mn_met_ind,
    cm_encounter.iq_adm_rev_type_obs_mn_met_ind,
    max(CASE
    ...
    max(cm_encounter_payer.midas_principal_payer_auth_num) AS midas_principal_payer_auth_num,
    ...
    substr(max(format_date('%Y', fact_patient.discharge_date)), 1, 4) AS discharge_year,
  ...
  FROM
    ...
    RIGHT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.fact_patient ON diagnosis_related_group.reimbursement_group_code = fact_patient.drg_code_hcfa
    ...
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.fact_facility ON fact_facility.coid = fact_patient.coid
     AND fact_facility.company_code = fact_patient.company_code
    ...
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.cm_encounter ON fact_patient.patient_dw_id = cm_encounter.patient_dw_id
     AND cm_encounter.active_dw_ind = 'Y'
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.ref_cm_location ON cm_encounter.location_id = ref_cm_location.location_id
     AND ref_cm_location.active_dw_ind = 'Y'
     AND cm_encounter.active_dw_ind = 'Y'
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.cm_encounter_payer ON cm_encounter_payer.midas_encounter_id = cm_encounter.midas_encounter_id
     AND cm_encounter_payer.active_dw_ind = 'Y'
     AND cm_encounter.active_dw_ind = 'Y'
    ...
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.ref_cc_root_cause ON ref_cc_root_cause.cc_root_cause_code = denial_eom.cc_root_cause_id
     AND ref_cc_root_cause.company_code = 'H'
      ...
      SELECT
        ...
        FROM
              ...
              WHERE upper(a.active_dw_ind) = 'Y'
               AND (a.patient_dw_id, coalesce(a.cert_end_date, DATE '1900-01-01')) IN(
                SELECT AS STRUCT
                    ...
                    max(coalesce(cm_encounter_payer_cert_hist.cert_end_date, DATE '1900-01-01'))
                  FROM
                    `hca-hin-dev-cur-parallon`.edwcm_views.cm_encounter_payer_cert_hist
                  WHERE upper(cm_encounter_payer_cert_hist.active_dw_ind) = 'Y'
                  GROUP BY 1
              )
          ...
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.cm_encounter AS cm_encounter_0 ON fact_patient_0.patient_dw_id = cm_encounter_0.patient_dw_id
           AND cm_encounter_0.active_dw_ind = 'Y'
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.cm_encounter_payer AS cm_encounter_payer_0 ON cm_encounter_payer_0.midas_encounter_id = cm_encounter_0.midas_encounter_id
           AND cm_encounter_payer_0.active_dw_ind = 'Y'
           AND cm_encounter_0.active_dw_ind = 'Y'
          LEFT OUTER JOIN (
            SELECT
                a.patient_dw_id,
    ...
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.chois_product_line ON fact_patient.drg_code_hcfa = chois_product_line.drg_code
    ...
    LEFT OUTER JOIN (
      SELECT
          ...
          case_type_order AS case_type_order,
          row_number() OVER (PARTITION BY delay_progression_detail.coid, delay_progression_detail.pat_acct_num ORDER BY case_type_order) AS priority_order
        FROM
        ...
        GROUP BY 1, 2, 3, 4
    ) AS drv_surgical_case_type ON drv_surgical_case_type.coid = fact_patient.coid
  ...
  END), upper(CASE
    WHEN external_pa_referral.patient_dw_id IS NULL THEN 'N'
    ELSE 'Y'
  END), upper(cm_encounter_payer.midas_principal_payer_auth_num), 42, 43, upper(drv_max_cert_end.hcm_status_cause_name), upper(cm_encounter.midas_acct_num), upper(substr(format_date('%Y', fact_patient.discharge_date), 1, 4)), 47, upper(coalesce(fact_patient.drg_desc_hcfa, 'DRG Not Assigned')), 49, 50, 51, 52, 53, 54, 55, upper(ref_cm_location.location_name)
;
