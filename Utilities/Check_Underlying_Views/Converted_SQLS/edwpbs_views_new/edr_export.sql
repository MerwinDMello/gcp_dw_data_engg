...
SELECT
    ...
    cr1py.last_update_date_time AS payer_refund_last_update_date_time,
    ...
    CASE
      WHEN upper(crb.credit_status_alias) = 'TB - TAKE BACK'
       AND fp.company_code = 'H'
       AND upper(gov_sec_tert_iplan) = 'N'
       AND ccap.iplan_order_num = 1
       AND CAST(bqutil.fn.cw_td_normalize_number(fi.major_payor_group_id) as FLOAT64) NOT IN(
        CAST(555 as FLOAT64), CAST(923 as FLOAT64)
      )
       AND upper(org.consolidated_ssc_alias_name) NOT IN(
        'RICHMOND SSC'
      )
       AND date_diff(current_date('US/Central'), CAST(/* expression of unknown or erroneous type */ cr1py.last_update_date_time as DATE), DAY) > 26 THEN 'Y'
      ELSE 'N'
  ...
  FROM
    ...
    INNER JOIN (
      SELECT
          ...
          o.coid
        FROM
          `hca-hin-dev-cur-parallon`.edwra_views.ref_cc_org_structure AS o
        WHERE upper(o.active_ind) = 'Y'
    ) AS f_active ON f_active.coid = fp.coid
     AND f_active.company_code = fp.company_code
    LEFT OUTER JOIN (
      SELECT DISTINCT
          drv_strengthofcase.patient_dw_id,
          max(CASE
        ...
        FROM
          (
            SELECT
                ...
                clm.claim_id,
                rank() OVER (PARTITION BY clm.patient_dw_id ORDER BY clm.peer_to_peer_completed_date_time_utc) AS firststrengthofcaserank,
                rank() OVER (PARTITION BY clm.patient_dw_id ORDER BY clm.peer_to_peer_completed_date_time_utc DESC) AS laststrengthofcaserank
              FROM
                `hca-hin-dev-cur-parallon`.edwcm_views.fact_patient AS fctptnt
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.fact_facility AS ftfacility ON fctptnt.coid = ftfacility.coid
                 AND fctptnt.company_code = ftfacility.company_code
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_dna_views.cm_dna_claim AS clm ON fctptnt.patient_dw_id = clm.patient_dw_id
                 AND clm.eff_to_date_time = '9999-12-31 00:00:00'
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_dna_views.ref_cm_dna_closure_status ON clm.closure_status_id = ref_cm_dna_closure_status.closure_status_id
                 AND upper(ref_cm_dna_closure_status.eff_to_date_time) = '9999-12-31 00:00:00'
              WHERE upper(ftfacility.company_code) = 'H'
               AND upper(ftfacility.coid_status_code) = 'F'
               AND fctptnt.admission_patient_type_code LIKE '%I%'
               AND clm.claim_status_id = 6
               AND clm.peer_to_peer_status_id = 4
               AND clm.closure_status_id IS NOT NULL
          ) AS drv_strengthofcase
        WHERE drv_strengthofcase.firststrengthofcaserank = 1
         OR drv_strengthofcase.laststrengthofcaserank = 1
        GROUP BY 1
    ) AS tmr_ptp ON tmr_ptp.patient_dw_id = fp.patient_dw_id
      ...
      SELECT DISTINCT
        ...
        FROM
          ...
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.facility_iplan AS fi_0 ON fi_0.payor_dw_id = ccap_0.payor_dw_id
           AND fi_0.company_code = ccap_0.company_code
        WHERE ccap_0.iplan_order_num > 1
         AND CAST(fi_0.iplan_financial_class_code as INT64) IN(
          1, 2, 3, 6
        )
    ) AS gov_flag ON gov_flag.patient_dw_id = ccap.patient_dw_id
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.admission AS ad ON ad.patient_dw_id = fp.patient_dw_id
     AND ad.company_code = fp.company_code
    LEFT OUTER JOIN (
      SELECT
    ...
    LEFT OUTER JOIN (
      SELECT
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwcm_views.ur_metric AS u
