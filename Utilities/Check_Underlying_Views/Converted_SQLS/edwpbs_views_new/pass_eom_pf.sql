...
CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_views.pass_eom_pf AS SELECT
    ...
    a.stack_code,
    a.rptg_period,
    date_sub(date_add(date_add(date_sub(parse_date('%Y%m%d', concat(CAST(/* expression of unknown or erroneous type */ a.rptg_period as STRING), '01')), interval extract(DAY from parse_date('%Y%m%d', concat(CAST(/* expression of unknown or erroneous type */ a.rptg_period as STRING), '01'))) DAY), interval 1 DAY), interval 1 MONTH), interval 1 DAY) AS pe_date,
    a.pat_acct_num,
    a.facility_name,
    CASE
    ...
    acct_age_category AS acct_age_category,
    substr(CASE
      WHEN acct_age_category = 0
       AND a.discharge_date IS NULL THEN 'Not Discharged'
      WHEN acct_age_category = 0
       AND a.discharge_date IS NOT NULL THEN 'Next Mo Disch'
      WHEN acct_age_category = 1 THEN '0-30'
      WHEN acct_age_category = 2 THEN '31-60'
      WHEN acct_age_category = 3 THEN '61-90'
      WHEN acct_age_category = 4 THEN '91-120'
      WHEN acct_age_category = 5 THEN '121-150'
      WHEN acct_age_category = 6 THEN '151-180'
      WHEN acct_age_category = 7 THEN '181-210'
      WHEN acct_age_category = 8 THEN '211-240'
      WHEN acct_age_category = 9 THEN '241-270'
      WHEN acct_age_category = 10 THEN '271-300'
      WHEN acct_age_category = 11 THEN '301-330'
      WHEN acct_age_category = 12 THEN '331-360'
      WHEN acct_age_category = 13 THEN '> 360'
    END, 1, 15) AS acct_age_category_desc,
  ...
  FROM
    ...
    CROSS JOIN UNNEST(ARRAY[
      CASE
        ...
        WHEN a.discharge_date >= date_add(CAST(trim(concat(substr(a.rptg_period, 1, 4), '-', substr(a.rptg_period, 5, 2), '-01')) as DATE), interval 1 MONTH) THEN 0
        WHEN abs(EXTRACT(MONTH FROM make_interval(CAST(regexp_extract(CAST(/* expression of unknown or erroneous type */ a.discharge_date as STRING), '([0-9]+)', 1, 1) as INT64), CAST(regexp_extract(CAST(/* expression of unknown or erroneous type */ a.discharge_date as STRING), '([0-9]+)', 1, 2) as INT64), 0, 0, 0, 0) - make_interval(CAST(regexp_extract(substr(format_date('%Y-%m', CAST(trim(concat(substr(a.rptg_period, 1, 4), '-', substr(a.rptg_period, 5, 2), '-01')) as DATE)), 1, 7), '([0-9]+)', 1, 1) as INT64), CAST(regexp_extract(substr(format_date('%Y-%m', CAST(trim(concat(substr(a.rptg_period, 1, 4), '-', substr(a.rptg_period, 5, 2), '-01')) as DATE)), 1, 7), '([0-9]+)', 1, 2) as INT64), 0, 0, 0, 0))) + 1 > 12 THEN 13
        ELSE abs(EXTRACT(MONTH FROM make_interval(CAST(regexp_extract(CAST(/* expression of unknown or erroneous type */ a.discharge_date as STRING), '([0-9]+)', 1, 1) as INT64), CAST(regexp_extract(CAST(/* expression of unknown or erroneous type */ a.discharge_date as STRING), '([0-9]+)', 1, 2) as INT64), 0, 0, 0, 0) - make_interval(CAST(regexp_extract(substr(format_date('%Y-%m', CAST(trim(concat(substr(a.rptg_period, 1, 4), '-', substr(a.rptg_period, 5, 2), '-01')) as DATE)), 1, 7), '([0-9]+)', 1, 1) as INT64), CAST(regexp_extract(substr(format_date('%Y-%m', CAST(trim(concat(substr(a.rptg_period, 1, 4), '-', substr(a.rptg_period, 5, 2), '-01')) as DATE)), 1, 7), '([0-9]+)', 1, 2) as INT64), 0, 0, 0, 0))) + 1
      END
    ]) AS acct_age_category
;
