...
CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_views.payor_current_eom AS WITH a AS (
    ...
    FROM
      (
        SELECT
            ...
            CAST(CASE
              WHEN trim(left(r.parent_payor_name, strpos(r.parent_payor_name, '-'))) BETWEEN '0' AND '99999999999' THEN SAFE_CAST(trim(left(r.parent_payor_name, strpos(r.parent_payor_name, '-') - 1)) AS BIGNUMERIC)
              ELSE CAST(CASE
          ...
          FROM
            ...
            LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.facility_dimension AS fd ON fd.coid = fi.coid
      ) AS a
), a_payor AS (
  SELECT
      a.*,
    ...
    FROM
      a
      INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.payor_eom AS payor ON payor.coid = a.coid
  ...
  SELECT
      ...
      CAST(NULL as DATETIME) AS null_25
    FROM
      a
    WHERE a.rn NOT IN(
      SELECT
...
SELECT
    dense_rank() OVER (ORDER BY xx.month_id, xx.payor_dw_id, xx.coid) AS sid,
    xx.month_id,
    xx.pe_date,
    xx.coid,
    xx.company_code,
    ...
    xx.eff_from_date,
    xx.eff_to_date,
    xx.iplan_financial_class_code,
    xx.calc_rev_cat_financial_class_code,
  ...
  FROM
    (
      SELECT
          CAST(bqutil.fn.cw_td_normalize_number(substr(format_date('%Y%m', a.pe_date), 1, 6)) as INT64) AS month_id,
          a.pe_date,
          a.coid,
          a.company_code,
          a.unit_num,
      ...
      UNION ALL
      SELECT
          CAST(bqutil.fn.cw_td_normalize_number(substr(format_date('%Y%m', x.pe_date), 1, 6)) as INT64) AS month_id,
          x.pe_date,
          x.coid,
          x.company_code,
          x.unit_num,
