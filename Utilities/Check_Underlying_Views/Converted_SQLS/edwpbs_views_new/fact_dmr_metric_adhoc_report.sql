...
CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_views.fact_dmr_metric_adhoc_report AS SELECT
  ...
  WHERE fact_dmr_metric.dmr_metric_code NOT IN(
    '9A', '9B', '29A'
  )
UNION ALL
SELECT
  ...
  FROM
    ...
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric AS b ON upper(a.service_type_name) = upper(b.service_type_name)
     ...
     AND a.dmr_metric_code = '1'
     AND b.dmr_metric_code = '3'
...
SELECT
    ...
    a.dmr_patient_type_code,
    a.dmr_fin_class_code,
    ROUND(a.dmr_metric_value / extract(DAY from a.rptg_date), 3, 'ROUND_HALF_EVEN') AS dmr_metric_value
  FROM
    `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric AS a
  WHERE a.dmr_metric_code = '1'
...
SELECT
  ...
  FROM
    `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric AS a
  WHERE a.dmr_metric_code = '27'
   AND a.dmr_day_month_ind = 'DCMTH'
   AND a.dmr_patient_type_code IN(
    'AP', 'NB'
  )
   AND a.dmr_fin_class_code <> 'NA'
...
SELECT
    ...
    a.dmr_patient_type_code,
    a.dmr_fin_class_code,
    CAST(ROUND(ROUND(a.dmr_metric_value / b.day_of_year, 3, 'ROUND_HALF_EVEN'), 3, 'ROUND_HALF_EVEN') as NUMERIC) AS dmr_metric_value
  FROM
    `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric AS a
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_views.lu_date AS b ON a.dmr_metric_code = '1'
     AND a.rptg_date = b.date_id
     AND a.dmr_day_month_ind = 'DCYTD'
...
SELECT
  ...
  FROM
    ...
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric AS b ON upper(a.service_type_name) = upper(b.service_type_name)
     ...
     AND a.rptg_date = b.rptg_date
     AND a.dmr_day_month_ind = b.dmr_day_month_ind
     AND a.dmr_metric_code = '10'
     AND b.dmr_metric_code = '29A'
     AND a.dmr_code = b.dmr_code
     AND a.dmr_patient_type_code = b.dmr_patient_type_code
     AND a.dmr_fin_class_code = b.dmr_fin_class_code
UNION ALL
  ...
  FROM
    ...
    INNER JOIN (
      SELECT
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric
        WHERE fact_dmr_metric.dmr_metric_code = '27'
        GROUP BY upper(fact_dmr_metric.service_type_name), 2, upper(fact_dmr_metric.parent_code), 4, 5, fact_dmr_metric.dmr_metric_code, 7, 8
    ) AS b ON upper(a.service_type_name) = upper(b.service_type_name)
     AND a.fact_lvl_code = b.fact_lvl_code
     AND upper(a.parent_code) = upper(b.parent_code)
     AND a.rptg_date = b.rptg_date
     AND a.dmr_day_month_ind = b.dmr_day_month_ind
     AND a.dmr_metric_code = '27'
...
SELECT
  ...
  FROM
    `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric
  WHERE fact_dmr_metric.dmr_metric_code = '27'
   AND fact_dmr_metric.dmr_code = 'NA'
   AND fact_dmr_metric.dmr_patient_type_code IN(
    'AP', 'NB'
  )
  GROUP BY upper(fact_dmr_metric.service_type_name), 2, upper(fact_dmr_metric.parent_code), 4, 5, fact_dmr_metric.dmr_metric_code, 7, 8
UNION ALL
  ...
  FROM
    ...
    INNER JOIN (
      SELECT
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric
        WHERE fact_dmr_metric.dmr_metric_code = '16'
         AND fact_dmr_metric.dmr_fin_class_code <> 'NA'
        GROUP BY upper(fact_dmr_metric.service_type_name), 2, upper(fact_dmr_metric.parent_code), 4, 5, fact_dmr_metric.dmr_metric_code, 7, 8
    ) AS b ON upper(a.service_type_name) = upper(b.service_type_name)
     AND a.fact_lvl_code = b.fact_lvl_code
     AND upper(a.parent_code) = upper(b.parent_code)
...
SELECT
    ...
    fact_dmr_metric.rptg_date,
    CASE
      WHEN fact_dmr_metric.dmr_day_month_ind = 'DCMTH' THEN 'DCMTD'
  ...
  FROM
    `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric
  WHERE fact_dmr_metric.dmr_metric_code = '16'
   AND fact_dmr_metric.dmr_patient_type_code = 'O'
  GROUP BY upper(fact_dmr_metric.service_type_name), 2, upper(fact_dmr_metric.parent_code), 4, fact_dmr_metric.dmr_day_month_ind, fact_dmr_metric.dmr_metric_code, 7, 8
UNION ALL
SELECT
    a.service_type_name,
    a.fact_lvl_code,
  ...
  FROM
    (
      SELECT
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric
        WHERE fact_dmr_metric.dmr_metric_code = '10'
        GROUP BY upper(fact_dmr_metric.service_type_name), 2, upper(fact_dmr_metric.parent_code), 4, 5, 6, 7, 8, 9
    ) AS a
    INNER JOIN (
      SELECT
          max(fact_dmr_metric.service_type_name) AS service_type_name,
    ...
    (
      SELECT
        ...
        WHERE fact_dmr_metric.dmr_metric_code = '10'
         ...
         AND fact_dmr_metric.dmr_fin_class_code <> 'NA'
         AND fact_dmr_metric.dmr_code = 'NA'
        GROUP BY upper(fact_dmr_metric.service_type_name), 2, upper(fact_dmr_metric.parent_code), 4, 5, 6, 7, 8
    ) AS a
    INNER JOIN (
      SELECT
          max(fact_dmr_metric.service_type_name) AS service_type_name,
...
SELECT
  ...
  FROM
    `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric
  WHERE fact_dmr_metric.dmr_metric_code IN(
    '10', '28'
  )
   AND fact_dmr_metric.dmr_fin_class_code <> 'NA'
   AND fact_dmr_metric.dmr_patient_type_code IN(
  ...
  GROUP BY upper(fact_dmr_metric.service_type_name), 2, upper(fact_dmr_metric.parent_code), 4, 5, 6, fact_dmr_metric.dmr_code, 8
UNION ALL
SELECT
    max(fact_dmr_metric.service_type_name) AS service_type_name,
    fact_dmr_metric.fact_lvl_code,
...
SELECT
    ...
    fact_dmr_metric.rptg_date,
    CASE
      WHEN fact_dmr_metric.dmr_day_month_ind = 'DCMTD' THEN 'DCMTH'
      ELSE fact_dmr_metric.dmr_day_month_ind
    END AS dmr_day_month_ind,
  ...
  WHERE fact_dmr_metric.dmr_metric_code IN(
   ...
   AND fact_dmr_metric.dmr_fin_class_code = 'NA'
   AND fact_dmr_metric.dmr_code IN(
    'NA', '100'
  )
  GROUP BY upper(fact_dmr_metric.service_type_name), 2, upper(fact_dmr_metric.parent_code), 4, 5, 6, 7, 8, 9
UNION ALL
...
SELECT
  ...
  FROM
    ...
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric AS b ON upper(a.service_type_name) = upper(b.service_type_name)
     ...
     AND a.rptg_date = b.rptg_date
     AND a.dmr_day_month_ind = b.dmr_day_month_ind
     AND a.dmr_metric_code = '33'
     AND b.dmr_metric_code = '34'
     AND a.dmr_code = b.dmr_code
     AND a.dmr_patient_type_code = b.dmr_patient_type_code
     AND a.dmr_fin_class_code = b.dmr_fin_class_code
UNION ALL
SELECT
  ...
  FROM
    ...
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric AS b ON upper(a.service_type_name) = upper(b.service_type_name)
     ...
     AND a.rptg_date = b.rptg_date
     AND a.dmr_day_month_ind = b.dmr_day_month_ind
     AND a.dmr_metric_code = '9A'
     AND b.dmr_metric_code = '9B'
     AND a.dmr_code = b.dmr_code
     AND a.dmr_patient_type_code = b.dmr_patient_type_code
     AND a.dmr_fin_class_code = b.dmr_fin_class_code
UNION ALL
...
SELECT
  ...
  FROM
    ...
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric AS b ON upper(a.service_type_name) = upper(b.service_type_name)
     ...
     AND a.rptg_date = b.rptg_date
     AND a.dmr_day_month_ind = b.dmr_day_month_ind
     AND a.dmr_metric_code = '29A'
     AND b.dmr_metric_code = '29B'
     AND a.dmr_code = b.dmr_code
     AND a.dmr_patient_type_code = b.dmr_patient_type_code
     AND a.dmr_fin_class_code = b.dmr_fin_class_code
UNION ALL
  ...
  FROM
    (
      SELECT
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric
        WHERE fact_dmr_metric.dmr_metric_code = '29A'
        GROUP BY upper(fact_dmr_metric.service_type_name), 2, upper(fact_dmr_metric.parent_code), 4, 5, fact_dmr_metric.dmr_metric_code, 7, 8
    ) AS a
    INNER JOIN (
      SELECT
    ...
    (
      SELECT
        ...
        FROM
          `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric
        WHERE fact_dmr_metric.dmr_metric_code = '27'
         AND fact_dmr_metric.dmr_day_month_ind <> 'DCDAT'
        GROUP BY upper(fact_dmr_metric.service_type_name), 2, upper(fact_dmr_metric.parent_code), 4, 5, fact_dmr_metric.dmr_metric_code, 7, 8
    ) AS a
    INNER JOIN (
      SELECT
...
SELECT
  ...
  FROM
    `hca-hin-dev-cur-parallon`.edwpbs_base_views.fact_dmr_metric
  WHERE fact_dmr_metric.dmr_metric_code = '1'
   AND fact_dmr_metric.dmr_code = 'NA'
   AND fact_dmr_metric.dmr_fin_class_code = 'NA'
;
