  ...
  FROM
    (
      SELECT DISTINCT
          ...
          varpymt AS varpymt,
          lg_mcr_amt AS lg_mcr_amt,
          lg_mcr_amt + varpymt AS underpymtamt
        FROM
          `hca-hin-dev-cur-parallon`.edwpf_views.reimbursement_discrepancy_eom AS rd_0
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.facility_dimension_eom AS fd_0 ON fd_0.coid = rd_0.coid
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.admission_discharge AS ad ON ad.patient_dw_id = rd_0.patient_dw_id
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.eis_dcrp_unit_year_dim AS b ON CAST(bqutil.fn.cw_td_normalize_number(fd_0.unit_num) as INT64) = b.unit_num_sid
          ...
          END
           AND CASE
            WHEN rd_0.ar_bill_thru_date IN(
              '0001-01-01', '1800-01-01'
            ) THEN ad.discharge_date
            ELSE rd_0.ar_bill_thru_date
          END BETWEEN b.year_beg_date AND b.year_end_date
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.eis_dcrp_unit_cost_year_dim AS p ON CAST(bqutil.fn.cw_td_normalize_number(fd_0.unit_num) as INT64) = p.unit_num_sid
          ...
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.explanation_of_reimbursement AS eor ON rd_0.company_code = eor.company_code
           AND rd_0.coid = eor.coid
           ...
           AND rd_0.log_id = eor.log_id
           AND rd_0.log_sequence_num = eor.log_sequence_num
          CROSS JOIN UNNEST(ARRAY[
            CASE
              WHEN rd_0.discrepancy_reason_code_1 IN(
                '801', '713', '712', '712', '713', '801', '865', '866', '867', '868', '869'
              )
               OR rd_0.discrepancy_reason_code_2 IN(
                '801', '712', '713', '801', '865', '866', '867', '868', '869'
              )
               OR rd_0.discrepancy_reason_code_3 IN(
                '801', '712', '713', '801', '865', '866', '867', '868', '869'
              )
               OR rd_0.discrepancy_reason_code_4 IN(
                '801', '712', '713', '801', '865', '866', '867', '868', '869'
          ...
          CROSS JOIN UNNEST(ARRAY[
            CASE
              WHEN rd_0.data_source_code = 125 THEN abs(rd_0.var_insurance_payment_amt)
        ...
        WHERE eor.eff_from_date = (
          SELECT
            ...
            WHERE rd_0.company_code = er.company_code
             AND rd_0.coid = er.coid
             ...
             AND rd_0.log_id = er.log_id
             AND rd_0.log_sequence_num = er.log_sequence_num
             AND er.eff_from_date <= date_sub(CAST(trim(concat(substr(format_date('%Y-%m', current_date('US/Central')), 1, 7), '-01')) as DATE), interval 1 DAY)
        )
         AND upper(rd_0.data_source_code) = '0124'
         AND rd_0.source_system_code IN(
          'N', 'P'
        )
      UNION ALL
      SELECT
        ...
        FROM
          ...
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.facility_dimension_eom AS fd_0 ON fd_0.coid = rd_0.coid
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.admission_discharge AS ad ON ad.patient_dw_id = rd_0.patient_dw_id
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.eis_dcrp_unit_cost_year_dim AS b ON CAST(bqutil.fn.cw_td_normalize_number(fd_0.unit_num) as INT64) = b.unit_num_sid
           AND b.source_sid = CASE
          ...
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.eis_dcrp_unit_year_dim AS p ON CAST(bqutil.fn.cw_td_normalize_number(fd_0.unit_num) as INT64) = p.unit_num_sid
           AND p.source_sid = CASE
            WHEN upper(rd_0.data_source_code) = '0124' THEN 6
            ELSE 7
          END
        ...
        GROUP BY upper(fd_0.pas_current_name), 2, 3, 4, 5, 6, 7, 8, 9
      UNION ALL
      SELECT
        ...
        FROM
          (
            SELECT
                ...
                END) AS varpymt,
                sum(CASE
                  WHEN rd_0.data_source_code = 125 THEN 0
                  ELSE 0
                END) AS lg_mcr_amt,
                sum(CASE
                  WHEN rd_0.data_source_code = 125 THEN 0
    ...
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.facility_dimension_eom AS fd ON fd.coid = rd.coid
  GROUP BY upper(rd.pas_current_name), 2, 3, upper(rd.type_code)
;
