...
-- Source: internal_metastore/db_hca-hin-dev-cur-parallon/schema_edwpbs_views/edr_export.memory
-- Translated from: Teradata
-- Translated to: BigQuery

CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_views.edr_export AS WITH ec1_dy1_dyi1_app1_apps1_e AS (
  SELECT
    ...
    FROM
      ...
      INNER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.hcm_avoid_denied_day AS dy1 ON dy1.midas_encounter_id = ec1.midas_encounter_id
       AND upper(dy1.company_code) = upper(ec1.company_code)
       AND upper(dy1.active_dw_ind) = 'Y'
      ...
      INNER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.hcm_avoid_denied_appeal AS app1 ON app1.hcm_avoid_denied_day_id = dyi1.hcm_avoid_denied_day_id
       ...
       AND upper(app1.active_dw_ind) = 'Y'
...
SELECT
    ...
    ROUND(ccap.pat_acct_num, 0, 'ROUND_HALF_EVEN') AS pat_acct_num,
    ...
    cmd.ptp_performed AS ptp_performed,
    ...
    ROUND(pa.ins1_payor_balance_amt, 3, 'ROUND_HALF_EVEN') AS ins1_payor_balance_amt,
    ROUND(pa.ins2_payor_balance_amt, 3, 'ROUND_HALF_EVEN') AS ins2_payor_balance_amt,
    ROUND(pa.ins3_payor_balance_amt, 3, 'ROUND_HALF_EVEN') AS ins3_payor_balance_amt,
    ...
    coalesce(CASE
      WHEN splitbill.patient_dw_id IS NOT NULL THEN 'Y'
      ELSE 'N'
    END, 'N') AS split_bill_ind,
    ...
    ROUND(cca.account_id, 0, 'ROUND_HALF_EVEN') AS account_id,
    ROUND(ccap.account_payor_id, 0, 'ROUND_HALF_EVEN') AS account_payor_id,
  ...
  FROM
    ...
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwra_views.ref_cc_org_structure AS ccorg ON upper(ccorg.company_code) = upper(ccap.company_code)
     AND upper(ccorg.coid) = upper(ccap.coid)
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.fact_patient AS fp ON fp.patient_dw_id = ccap.patient_dw_id
     AND upper(fp.company_code) = upper(ccap.company_code)
     AND upper(fp.coid) = upper(ccap.coid)
    ...
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.facility_iplan AS fi ON fi.payor_dw_id = ccap.payor_dw_id
     AND upper(fi.company_code) = upper(ccap.company_code)
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.patient_esl_detail AS esl ON esl.patient_dw_id = ccap.patient_dw_id
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_views.dim_organization AS org ON upper(org.coid) = upper(ccap.coid)
     AND upper(org.company_code) = upper(ccap.company_code)
    LEFT OUTER JOIN (
      SELECT
        ...
        WHERE upper(dg.reimbursement_group_name) = 'M'
    ) AS drg_pa ON upper(drg_pa.reimbursement_group_code) = upper(fp.drg_hcfa_icd10_code)
    ...
    LEFT OUTER JOIN (
     ...
     AND upper(apr.coid) = upper(ccap.coid)
     AND upper(apr.company_code) = upper(ccap.company_code)
    LEFT OUTER JOIN (
      SELECT
          patient_condition_code.patient_dw_id,
          max(CASE
            WHEN upper(patient_condition_code.condition_code) IN(
              'XF', 'XG'
            ) THEN 'Y'
            ELSE 'N'
          END) AS cond_code_xf_xg_ind,
          max(CASE
            WHEN upper(patient_condition_code.condition_code) = 'NU' THEN 'Y'
            ELSE 'N'
          END) AS cond_code_nu_ind,
          max(CASE
            WHEN upper(patient_condition_code.condition_code) = 'NE' THEN 'Y'
            ELSE 'N'
          END) AS cond_code_ne_ind,
          max(CASE
            WHEN upper(patient_condition_code.condition_code) = 'NS' THEN 'Y'
            ELSE 'N'
          END) AS cond_code_ns_ind,
          max(CASE
            WHEN upper(patient_condition_code.condition_code) = 'NP' THEN 'Y'
            ELSE 'N'
          END) AS cond_code_np_ind,
          max(CASE
            WHEN upper(patient_condition_code.condition_code) = 'NO' THEN 'Y'
            ELSE 'N'
          END) AS cond_code_no_ind
        FROM
          `hca-hin-dev-cur-parallon`.edwpf_views.patient_condition_code
        WHERE upper(patient_condition_code.condition_code) IN(
    ...
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_views.registration_iplan_pf AS ri ON ri.patient_dw_id = ccap.patient_dw_id
     ...
     AND upper(ri.coid) = upper(ccap.coid)
     AND upper(ri.company_code) = upper(ccap.company_code)
    ...
    LEFT OUTER JOIN (
      SELECT
          ...
          CASE
            WHEN epa.midas_principal_payer_auth_num LIKE '%1%'
             OR epa.midas_principal_payer_auth_num LIKE '%2%'
             OR epa.midas_principal_payer_auth_num LIKE '%3%'
             OR epa.midas_principal_payer_auth_num LIKE '%4%'
             OR epa.midas_principal_payer_auth_num LIKE '%5%'
             OR epa.midas_principal_payer_auth_num LIKE '%6%'
             OR epa.midas_principal_payer_auth_num LIKE '%7%'
             OR epa.midas_principal_payer_auth_num LIKE '%8%'
             OR epa.midas_principal_payer_auth_num LIKE '%9%'
             OR epa.midas_principal_payer_auth_num LIKE '%0%' THEN CASE
              WHEN epa.midas_principal_payer_auth_num IS NULL THEN 'No Auth #'
              WHEN trim(epa.midas_principal_payer_auth_num) = '' THEN 'No Auth #'
              WHEN strpos(epa.midas_principal_payer_auth_num, '/') = 0 THEN 'No "/" in Auth #'
              WHEN substr(epa.midas_principal_payer_auth_num, strpos(epa.midas_principal_payer_auth_num, '/'), 2) = '/I' THEN 'Inpatient (/I)'
              WHEN substr(epa.midas_principal_payer_auth_num, strpos(epa.midas_principal_payer_auth_num, '/'), 2) = '/V' THEN 'Observation (/V)'
              WHEN substr(epa.midas_principal_payer_auth_num, strpos(epa.midas_principal_payer_auth_num, '/'), 2) = '/S' THEN 'IP Skilled Nursing or Swing Bed (/S)'
              WHEN substr(epa.midas_principal_payer_auth_num, strpos(epa.midas_principal_payer_auth_num, '/'), 2) = '/B' THEN 'IP Behavioral Health (/B)'
              WHEN substr(epa.midas_principal_payer_auth_num, strpos(epa.midas_principal_payer_auth_num, '/'), 4) = '/CPT' THEN 'Outpatient (/CPT)'
              ELSE 'Other'
            END
        ...
        FROM
          ...
          INNER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.cm_encounter AS e ON e.patient_dw_id = p.patient_dw_id
           AND upper(e.company_code) = upper(p.company_code)
           AND upper(e.active_dw_ind) = 'Y'
          ...
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.ref_cm_location AS lo ON lo.location_id = e.iq_adm_rev_location_id
           AND upper(lo.active_dw_ind) = 'Y'
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.fact_facility AS ff ON upper(ff.coid) = upper(p.coid)
           AND upper(ff.company_code) = upper(p.company_code)
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.cm_facility AS cmf ON upper(cmf.coid) = upper(e.coid)
           AND upper(cmf.midas_facility_code) = upper(e.midas_facility_code)
           AND upper(cmf.company_code) = upper(e.company_code)
          ...
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.clinical_ed_event_date_time AS cl ON cl.patient_dw_id = e.patient_dw_id
           AND upper(cl.company_code) = upper(e.company_code)
          LEFT OUTER JOIN (
            SELECT
                apt.patient_dw_id,
                apt.eff_from_date,
          ...
          LEFT OUTER JOIN (
            SELECT
              ...
              WHERE upper(ed.active_dw_ind) = 'Y'
          ...
          LEFT OUTER JOIN (
            SELECT DISTINCT
              ...
              WHERE upper(cr.active_dw_ind) = 'Y'
              GROUP BY 1
          ...
          LEFT OUTER JOIN (
            SELECT
              ...
              FROM
                ...
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.ref_hcm_appeal_status AS apps1 ON apps1.hcm_appeal_status_id = app1.hcm_appeal_status_id
                 AND upper(apps1.active_dw_ind) = 'Y'
              WHERE upper(ec1.active_dw_ind) = 'Y'
            ...
            SELECT
              ...
              FROM
                ...
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.ref_hcm_status_cause AS atts1 ON atts1.hcm_status_cause_id = att1.hcm_status_cause_id
                 AND upper(atts1.active_dw_ind) = 'Y'
              WHERE upper(ec1.active_dw_ind) = 'Y'
          ...
          LEFT OUTER JOIN (
            SELECT
              ...
              WHERE upper(ec1_dy1_dyi1_app1_apps1_e_e1_e1.active_dw_ind) = 'Y'
          ...
          LEFT OUTER JOIN (
            SELECT
              ...
              WHERE upper(crr.active_dw_ind) = 'Y'
            ...
            SELECT
              ...
              FROM
                ...
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.ref_hcm_disposition AS hcmd ON hcmd.disposition_id = rcc4.disposition_id
                 AND upper(hcmd.active_dw_ind) = 'Y'
              WHERE upper(crr4.active_dw_ind) = 'Y'
          ...
          LEFT OUTER JOIN (
            SELECT
                ...
                max(CASE
                  WHEN upper(x.xu_billing_condition_code_name) = 'XF' THEN x.eff_from_date
                  ELSE CAST(NULL as DATE)
                END) AS cm_last_xf_code_applied_date,
                max(CASE
                  WHEN upper(x.xu_billing_condition_code_name) = 'XF' THEN 'Y'
                  ELSE 'N'
                END) AS cm_xf_ind,
                max(CASE
                  WHEN upper(x.xu_billing_condition_code_name) = 'XG' THEN x.eff_from_date
                  ELSE CAST(NULL as DATE)
                END) AS cm_last_xg_code_applied_date,
                max(CASE
                  WHEN upper(x.xu_billing_condition_code_name) = 'XG' THEN 'Y'
                  ELSE 'N'
                END) AS cm_xg_ind
              FROM
                `hca-hin-dev-cur-parallon`.edwcm_views.cm_billing_code_comment AS x
              WHERE upper(x.active_dw_ind) = 'Y'
               AND upper(x.xu_billing_condition_code_name) IN(
                'XF', 'XG'
              )
              GROUP BY 1
            ...
            SELECT DISTINCT
              ...
              FROM
                ...
                INNER JOIN `hca-hin-dev-cur-parallon`.edwcm_views.ref_hcm_days_type AS ty ON ty.hcm_days_type_id = ia5.type_of_day_id
                 AND upper(ty.active_dw_ind) = 'Y'
              WHERE upper(ed5.active_dw_ind) = 'Y'
    ...
    LEFT OUTER JOIN (
      SELECT
        ...
        FROM
          ...
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.fact_patient AS fp_0 ON upper(fp_0.company_code) = upper(pduu.company_code)
           AND upper(fp_0.coid) = upper(pduu.coid)
    ...
    LEFT OUTER JOIN (
      SELECT
          ...
          CAST(coalesce(min(trim(CAST(fn.doc_req_medrec_request_date as STRING))), '1900-01-01') as DATE) AS min_doc_req_medrec_request_date,
          CAST(coalesce(max(trim(CAST(fn.doc_req_medrec_request_date as STRING))), '1900-01-01') as DATE) AS max_doc_req_medrec_request_date,
          CAST(coalesce(min(trim(CAST(fn.doc_req_medrec_sent_date as STRING))), '1900-01-01') as DATE) AS min_doc_req_medrec_sent_date,
          CAST(coalesce(max(trim(CAST(fn.doc_req_medrec_sent_date as STRING))), '1900-01-01') as DATE) AS max_doc_req_medrec_sent_date,
          CAST(coalesce(min(trim(CAST(fn.doc_req_ib_request_date as STRING))), '1900-01-01') as DATE) AS min_doc_req_ib_request_date,
          CAST(coalesce(max(trim(CAST(fn.doc_req_ib_request_date as STRING))), '1900-01-01') as DATE) AS max_doc_req_ib_request_date,
          CAST(coalesce(min(trim(CAST(fn.doc_req_ib_sent_date as STRING))), '1900-01-01') as DATE) AS min_doc_req_ib_sent_date,
          CAST(coalesce(max(trim(CAST(fn.doc_req_ib_sent_date as STRING))), '1900-01-01') as DATE) AS max_doc_req_ib_sent_date,
          CAST(coalesce(min(trim(CAST(fn.doc_req_request_date as STRING))), '1900-01-01') as DATE) AS first_doc_request_date,
          CAST(coalesce(max(trim(CAST(fn.doc_req_request_date as STRING))), '1900-01-01') as DATE) AS last_doc_request_date,
          CAST(coalesce(min(trim(CAST(fn.doc_req_sent_date as STRING))), '1900-01-01') as DATE) AS first_doc_sent_date,
          CAST(coalesce(max(trim(CAST(fn.doc_req_sent_date as STRING))), '1900-01-01') as DATE) AS last_doc_sent_date,
          CAST(coalesce(min(trim(CAST(fn.doc_req_received_date as STRING))), '1900-01-01') as DATE) AS first_doc_received_date,
          CAST(coalesce(max(trim(CAST(fn.doc_req_received_date as STRING))), '1900-01-01') as DATE) AS last_doc_received_date,
          CAST(coalesce(min(trim(CAST(fn.doc_req_approve_date as STRING))), '1900-01-01') as DATE) AS first_doc_approved_date,
          CAST(coalesce(max(trim(CAST(fn.doc_req_approve_date as STRING))), '1900-01-01') as DATE) AS last_doc_approved_date,
          CAST(coalesce(min(trim(CAST(fn.doc_req_denied_date as STRING))), '1900-01-01') as DATE) AS first_doc_denied_date,
          CAST(coalesce(max(trim(CAST(fn.doc_req_denied_date as STRING))), '1900-01-01') as DATE) AS last_doc_denied_date
        FROM
          (
            SELECT
                d.patient_dw_id,
                ...
                ROUND(CASE
                  ...
                  WHEN upper(i.major_payor_group_id) = upper(i1.major_payor_group_id) THEN p.payor_dw_id_ins1
                  WHEN upper(i.major_payor_group_id) = upper(i2.major_payor_group_id) THEN p.payor_dw_id_ins2
                  WHEN upper(i.major_payor_group_id) = upper(i3.major_payor_group_id) THEN p.payor_dw_id_ins3
                ...
                CASE
                  ...
                  WHEN upper(i.major_payor_group_id) = upper(i1.major_payor_group_id) THEN p.iplan_id_ins1
                  WHEN upper(i.major_payor_group_id) = upper(i2.major_payor_group_id) THEN p.iplan_id_ins2
                  WHEN upper(i.major_payor_group_id) = upper(i3.major_payor_group_id) THEN p.iplan_id_ins3
                  ELSE 0
                ...
                CASE
                  ...
                  WHEN upper(i.major_payor_group_id) = upper(i1.major_payor_group_id) THEN 1
                  WHEN upper(i.major_payor_group_id) = upper(i2.major_payor_group_id) THEN 2
                  WHEN upper(i.major_payor_group_id) = upper(i3.major_payor_group_id) THEN 3
                  ELSE 0
                ...
                CASE
                  WHEN upper(coalesce(trim(d.cplt_med_rec_ind), 'N')) = 'Y'
                   AND upper(coalesce(trim(d.itm_bill_ind), 'N')) = 'Y' THEN 'IB & MR Request'
                  WHEN upper(coalesce(trim(d.cplt_med_rec_ind), 'N')) = 'Y'
                   AND upper(coalesce(trim(d.itm_bill_ind), 'N')) <> 'Y' THEN 'Medical Record Request'
                  WHEN upper(coalesce(trim(d.cplt_med_rec_ind), 'N')) <> 'Y'
                   AND upper(coalesce(trim(d.itm_bill_ind), 'N')) = 'Y' THEN 'Itemized Bill Request'
                  ELSE 'All Other'
                END AS ib_medrec_doc_req_desc,
                CASE
                  WHEN upper(coalesce(trim(d.cplt_med_rec_aprv_ind), 'N')) = 'Y'
                   AND upper(coalesce(trim(d.itm_bill_aprv_ind), 'N')) = 'Y' THEN 'IB & MR Approved'
                  WHEN upper(coalesce(trim(d.cplt_med_rec_aprv_ind), 'N')) = 'Y'
                   AND upper(coalesce(trim(d.itm_bill_aprv_ind), 'N')) <> 'Y' THEN 'Medical Record Approved'
                  WHEN upper(coalesce(trim(d.cplt_med_rec_aprv_ind), 'N')) <> 'Y'
                   AND upper(coalesce(trim(d.itm_bill_aprv_ind), 'N')) = 'Y' THEN 'Itemized Bill Approved'
                  ELSE 'All Other'
                END AS ib_medrec_doc_req_apr_desc
              FROM
                `hca-hin-dev-cur-parallon`.edwpbs_views.collection_doc_req_dtl AS d
                INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.facility_dimension AS fd ON upper(fd.coid) = upper(d.coid)
                 AND upper(fd.company_code) = upper(d.company_code)
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edw_pub_views.facility_iplan AS i ON upper(i.coid) = upper(d.coid)
                 AND i.iplan_id = d.iplan_id
                 AND upper(i.company_code) = upper(d.company_code)
                INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.fact_patient AS p ON p.patient_dw_id = d.patient_dw_id
                 AND upper(p.company_code) = upper(d.company_code)
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edw_pub_views.facility_iplan AS i1 ON i1.payor_dw_id = p.payor_dw_id_ins1
                 AND upper(i1.coid) = upper(p.coid)
                 AND upper(i1.company_code) = upper(p.company_code)
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edw_pub_views.facility_iplan AS i2 ON i2.payor_dw_id = p.payor_dw_id_ins2
                 AND upper(i2.coid) = upper(p.coid)
                 AND upper(i2.company_code) = upper(p.company_code)
                LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edw_pub_views.facility_iplan AS i3 ON i3.payor_dw_id = p.payor_dw_id_ins3
                 AND upper(i3.coid) = upper(p.coid)
                 AND upper(i3.company_code) = upper(p.company_code)
              WHERE upper(d.company_code) = 'H'
               AND upper(fd.summary_7_member_ind) = 'Y'
               AND upper(coalesce(fd.osg_pas_ind, 'N')) <> 'Y'
               AND upper(coalesce(fd.summary_asd_member_ind, 'N')) <> 'Y'
               AND upper(coalesce(fd.summary_imaging_member_ind, 'N')) <> 'Y'
               AND upper(fd.pas_coid) IN(
                '08591', '08648', '08942', '08945', '08947', '08948', '08949', '08950'
              )
               AND CAST(i.iplan_financial_class_code as INT64) IN(
                5, 7, 8, 9, 11, 12, 13, 14
              )
               AND upper(coalesce(i.payer_type_code, '')) NOT IN(
                'MV', 'VA'
              )
               AND coalesce(i.major_payor_group_id, format('%4d', 0)) NOT IN(
                '930'
              )
               AND upper(coalesce(d.status_code, '')) <> 'CANC  '
               AND d.create_date >= DATE '2020-01-01'
               AND d.reporting_date = (
                SELECT
                    max(collection_doc_req_dtl.reporting_date) AS maxreporting_date
                  FROM
    ...
    LEFT OUTER JOIN (
      SELECT
        ...
        FROM
          ...
          INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_views.fact_patient AS fp_0 ON upper(pd.company_code) = upper(fp_0.company_code)
        ...
        QUALIFY row_number() OVER (PARTITION BY upper(pd.coid), pd.pat_acct_num ORDER BY pd.pa_last_update_date DESC) = 1
    ...
    LEFT OUTER JOIN (
      SELECT
        ...
        WHERE cri.reporting_date = (
          SELECT
              max(payment_compliance_credit_inventory.reporting_date)
            FROM
      ...
      SELECT
        ...
        WHERE ccap_0.iplan_order_num IN(
         ...
         AND ccap_0.dw_last_update_date_time = (
          SELECT
              max(cc_account_payor.dw_last_update_date_time)
            FROM
              `hca-hin-dev-cur-parallon`.edwra_views.cc_account_payor AS ccap1
            WHERE cc_account_payor_0.patient_dw_id = cc_account_payor_1.patient_dw_id
             AND cc_account_payor_2.payor_dw_id = cc_account_payor_3.payor_dw_id
             AND cc_account_payor_4.iplan_order_num = cc_account_payor_5.iplan_order_num
        )
   ...
   AND ccap.dw_last_update_date_time = (
    SELECT
      ...
      FROM
        `hca-hin-dev-cur-parallon`.edwra_views.cc_account_payor AS ccap1
      WHERE cc_account_payor_0.patient_dw_id = cc_account_payor_1.patient_dw_id
       AND cc_account_payor_2.iplan_order_num = cc_account_payor_3.iplan_order_num
  )
   AND upper(cca.archive_state_ind) = 'N'
;
