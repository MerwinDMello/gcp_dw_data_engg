...
-- Source: internal_metastore/db_hca-hin-dev-cur-parallon/schema_edwpbs_views/denial_inventory_analysis_spg.memory
-- Translated from: Teradata
-- Translated to: BigQuery

CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_views.denial_inventory_analysis_spg AS SELECT
    ...
    ROUND(a.total_charge_amt, 3, 'ROUND_HALF_EVEN') AS charge_total,
    ROUND(a.ending_balance_amt, 3, 'ROUND_HALF_EVEN') AS current_appeal_amount,
    date_diff(date_sub(date_add(CAST(concat(substr(CAST(a.date_sid as STRING), 1, 4), '-', substr(CAST(a.date_sid as STRING), 5, 2), '-01') as DATE), interval 1 MONTH), interval 1 DAY), a.appeal_origination_date, DAY) AS denial_age,
    ...
    ROUND(a.denied_charge_amt, 3, 'ROUND_HALF_EVEN') AS denied_charges_amount,
    a.discharge_date,
    ...
    ROUND(a.beginning_appeal_amt, 3, 'ROUND_HALF_EVEN') AS original_appeal_amount,
    ROUND(a.corrections_account_amt, 3, 'ROUND_HALF_EVEN') AS corrections_account_amt,
    ROUND(a.overturned_account_amt, 3, 'ROUND_HALF_EVEN') AS overturned_denied_amt_current,
    ...
    f.payor_gen03,
    f.payor_gen05,
    ROUND(a.account_balance_amt, 3, 'ROUND_HALF_EVEN') AS patient_account_balance,
    ROUND(a.patient_dw_id, 0, 'ROUND_HALF_EVEN') AS patient_dw_id,
    ROUND(a.patient_sid, 0, 'ROUND_HALF_EVEN') AS patient_account_number,
    a.patient_type_sid,
    ...
    ROUND(a.denied_charge_amt - a.overturned_account_amt, 3, 'ROUND_HALF_EVEN') AS remaining_denied_charges,
    ROUND(a.trans_next_party_amt, 3, 'ROUND_HALF_EVEN') AS trans_next_party_amt,
    a.coid,
    ROUND(a.write_off_denial_account_amt + a.corrections_account_amt + a.trans_next_party_amt, 3, 'ROUND_HALF_EVEN') AS upheld_denied_amount,
    ...
    ROUND(a.write_off_denial_account_amt + a.corrections_account_amt + a.trans_next_party_amt + a.not_true_denial_amt + a.overturned_account_amt, 3, 'ROUND_HALF_EVEN') AS resolved_denials_amount,
  ...
  FROM
    ...
    LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.facility_iplan_pf AS fi ON fi.payor_dw_id = a.payor_dw_id
    ...
    LEFT OUTER JOIN (
      SELECT
        ...
        FROM
          ...
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.payor_employer AS r ON upper(q.coid) = upper(r.coid)
           AND q.patient_dw_id = r.patient_dw_id
           AND q.payor_sequence_sid = r.iplan_insurance_order_num
           AND upper(r.employee_code) = 'P'
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.payor_employer AS s ON upper(q.coid) = upper(s.coid)
           AND q.patient_dw_id = s.patient_dw_id
           AND q.payor_sequence_sid = s.iplan_insurance_order_num
           AND upper(s.employee_code) = 'R'
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.payor_employer AS t ON upper(q.coid) = upper(t.coid)
           AND q.patient_dw_id = t.patient_dw_id
           AND q.payor_sequence_sid = t.iplan_insurance_order_num
           AND upper(t.employee_code) = 'S'
          LEFT OUTER JOIN `hca-hin-dev-cur-parallon`.edwpbs_base_views.payor_employer AS u ON upper(q.coid) = upper(u.coid)
           AND q.patient_dw_id = u.patient_dw_id
           AND q.payor_sequence_sid = u.iplan_insurance_order_num
           AND upper(u.employee_code) = 'O'
