...
SELECT
    pf.company_code,
    pf.coid,
    ROUND(pf.patient_dw_id, 0, 'ROUND_HALF_EVEN') AS patient_dw_id,
    pf.ar_transaction_effective_date,
    pf.ar_transaction_enter_date,
    pf.ar_transaction_enter_time,
    pf.iplan_id,
    ROUND(pf.pat_acct_num, 0, 'ROUND_HALF_EVEN') AS pat_acct_num,
    pf.source_system_code,
    pf.eff_from_date,
    pf.bad_debt_reason_code,
    ROUND(pf.payor_dw_id, 0, 'ROUND_HALF_EVEN') AS payor_dw_id,
    pf.credit_gl_sub_account_num,
    pf.system_generated_ind,
    ROUND(pf.ar_transaction_amt, 3, 'ROUND_HALF_EVEN') AS ar_transaction_amt,
    ...
    ROUND(pf.bad_debt_approval_id, 0, 'ROUND_HALF_EVEN') AS bad_debt_approval_id,
    ...
    ROUND(pf.transaction_code, 0, 'ROUND_HALF_EVEN') AS transaction_code,
  ...
  FROM
    ...
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.secref_facility AS sf ON upper(pf.coid) = upper(sf.co_id)
     AND sf.user_id = session_user()
  WHERE upper(pf.source_system_code) = 'P'
;
