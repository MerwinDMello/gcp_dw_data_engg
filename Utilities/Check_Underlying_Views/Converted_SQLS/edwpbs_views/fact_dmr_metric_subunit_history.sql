...
-- Source: internal_metastore/db_hca-hin-dev-cur-parallon/schema_edwpbs_views/fact_dmr_metric_subunit_history.memory
-- Translated from: Teradata
-- Translated to: BigQuery

CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_views.fact_dmr_metric_subunit_history AS SELECT
    a.service_type_name,
    a.fact_lvl_code,
  ...
  FROM
    ...
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.facility_sub_unit AS fsu ON upper(a.parent_code) = upper(fsu.coid)
     ...
     AND a.rptg_date < DATE '2020-09-08'
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.secref_facility AS b ON upper(b.co_id) = upper(fsu.coid)
     AND upper(b.company_code) = upper(fsu.company_code)
