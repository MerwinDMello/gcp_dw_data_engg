...
CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_views.lpnt_premigration_ar AS SELECT
    ...
    ar.unit_num,
    ar.rptg_period,
    ROUND(ar.ar_amt_gt_150, 3, 'ROUND_HALF_EVEN') AS ar_amt_gt_150,
    ROUND(ar.ar_amt, 3, 'ROUND_HALF_EVEN') AS ar_amt
  FROM
    `hca-hin-dev-cur-parallon`.edwpbs_base_views.lpnt_premigration_ar AS ar
    INNER JOIN `hca-hin-dev-cur-parallon`.edwpf_base_views.secref_facility AS sf ON upper(ar.coid) = upper(sf.co_id)
