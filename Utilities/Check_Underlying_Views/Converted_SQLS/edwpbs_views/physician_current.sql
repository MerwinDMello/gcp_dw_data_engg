...
CREATE VIEW IF NOT EXISTS `hca-hin-dev-cur-parallon`.edwpbs_views.physician_current AS SELECT
    ...
    CASE
      WHEN session_user() = zz.userid THEN substr(substr(physician_current.social_security_num, 1, 20), length(substr(physician_current.social_security_num, 1, 20)) - 9, 9)
      ELSE '***'
