CREATE OR REPLACE VIEW `hca-hin-dev-cur-hr.edwhr_ac.bq_query_cost`
AS
SELECT
...
IF (ENDS_WITH(COALESCE(completed.user, running.user), 'gserviceaccount.com'),
    ...
    WHEN 'bobj' THEN 'BobJ'
    WHEN 'td-hr-migration' THEN 'TD Migration'
    WHEN 'td-hr-migrate' THEN 'TD Migration'
...
FROM (
  ...
  FROM
    `hca-hin-dev-cur-hr.auth_base_views.cloudaudit_googleapis_com_data_access` a
  WHERE
    1=1
