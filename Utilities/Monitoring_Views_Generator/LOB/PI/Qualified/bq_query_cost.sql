CREATE OR REPLACE VIEW `hca-hin-dev-cur-ops.edwops_ac.bq_query_cost`
AS
SELECT
...
IF (ENDS_WITH(COALESCE(completed.user, running.user), 'gserviceaccount.com'),
    ...
    WHEN 'bobj' THEN 'BobJ'
    WHEN 'td-ops-migration' THEN 'TD Migration'
    WHEN 'td-ops-migrate' THEN 'TD Migration'
    ...
    CONCAT(
      ...
      WHEN 'hca-at-cao-sa-mgmt' 
        THEN 
          CASE REGEXP_EXTRACT(TRIM(LOWER(SPLIT(COALESCE(completed.user, running.user), '@')[SAFE_OFFSET (0)])), r'.*-(hr|hrg)(?:-.*|$)')
            WHEN 'hr' THEN 'OPS'
            WHEN 'hrg' THEN 'OPS'
            ELSE ''
          END
      ELSE
        CASE REGEXP_EXTRACT(TRIM(LOWER(SPLIT(SPLIT(COALESCE(completed.user, running.user), '@')[SAFE_OFFSET (1)],'.')[SAFE_OFFSET (0)])), r'.*-(hr|hrg)[-]*.*')
            WHEN 'hr' THEN 'OPS'
            WHEN 'hrg' THEN 'OPS'
          ELSE ''
        END
...
FROM (
  ...
  FROM
    `hca-hin-dev-cur-ops.auth_base_views.cloudaudit_googleapis_com_data_access` a
  WHERE
    1=1
