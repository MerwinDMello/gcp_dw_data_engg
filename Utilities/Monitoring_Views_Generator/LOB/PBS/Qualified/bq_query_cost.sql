CREATE OR REPLACE VIEW `hca-hin-dev-cur-clinical.edwpara_ac.bq_query_cost`
AS
SELECT
...
IF (ENDS_WITH(COALESCE(completed.user, running.user), 'gserviceaccount.com'),
    ...
    WHEN 'bobj' THEN 'BobJ'
    WHEN 'td-parallon-migration' THEN 'TD Migration'
    WHEN 'td-parallon-migrate' THEN 'TD Migration'
    ...
    CONCAT(
      ...
      WHEN 'hca-at-cao-sa-mgmt' 
        THEN 
          CASE REGEXP_EXTRACT(TRIM(LOWER(SPLIT(COALESCE(completed.user, running.user), '@')[SAFE_OFFSET (0)])), r'.*-(para)(?:-.*|$)')
            WHEN 'para' THEN 'Parallon'
            ELSE ''
          END
      ELSE
        CASE REGEXP_EXTRACT(TRIM(LOWER(SPLIT(SPLIT(COALESCE(completed.user, running.user), '@')[SAFE_OFFSET (1)],'.')[SAFE_OFFSET (0)])), r'.*-(para)[-]*.*')
            WHEN 'para' THEN 'Parallon'
          ELSE ''
        END
      END,
...
FROM (
  ...
  FROM
    `hca-hin-dev-cur-clinical.auth_base_views.cloudaudit_googleapis_com_data_access` a
  WHERE
    1=1
