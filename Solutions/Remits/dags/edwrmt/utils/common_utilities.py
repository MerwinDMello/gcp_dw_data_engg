...
config = call_config_yaml("edwrmt_config.yaml","hca_rmt_default_vars")

# get project variables from config file
bq_project_id = config['env']['v_curated_project_id']
bq_staging_dataset = config['env']['v_rmt_stage_dataset_name']
stage_dataset = config['env']['v_rmt_stage_dataset_name']
...
def insert_audit_data_files(**kwargs):
    ...
    audit_table = bq_client.dataset(kwargs['config']['env']['v_rmt_audit_dataset_name']).table(kwargs['config']['env']['v_audittablename'].split('.')[-1])
