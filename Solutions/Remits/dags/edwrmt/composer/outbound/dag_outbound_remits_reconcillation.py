...
outbound_config = cu.call_config_yaml(f"{source_system}_outbound_recon_config.yaml", f"{source_system}_outbound_recon")
    ...
    with dag:
            ...
            run_dataflow_job_truncate = [BashOperator(
                    ...
                    # to start parallel tasks at random times within 5 minutes, sleep n seconds
                        bash_command=f"sleep $(shuf -i 1-300 -n 1) ; python /home/airflow/gcs/dags/{lob}/scripts/{truncate_template}"
                                     f" --src_sys_config_file={source_system}_outbound_recon_config.yaml --src_system={source_system}"
        ...
        with TaskGroup(group_id=f'run_dataflow_jobs') as dataflow_group:
            ...
            for idx,(script, target_table) in enumerate(zip(sql_label,tgttablename)):
                ...
                with TaskGroup(group_id=f'TG-df_{target_table}_{sql_name}_recon_writeback') as tablegroup:                
                    run_dataflow_job = [BashOperator(
                        task_id=f"run_{source_system}_{target_table}_{sql_name}_recon_writeback",
                        dag=dag,
                    # to start parallel tasks at random times within 5 minutes, sleep n seconds
                    ...
                    wait_for_python_job_async_done = DataflowJobStatusSensor(
                        task_id=f"wait_for_python_job_async_done_{target_table}_{sql_name}_recon_writeback",
                        job_id=f"{{{{ task_instance.xcom_pull(task_ids= 'run_dataflow_jobs.TG-df_{target_table}_{sql_name}_recon_writeback.run_{source_system}_{target_table}_{sql_name}_recon_writeback')}}}}",
                        expected_statuses={DataflowJobStatus.JOB_STATE_DONE},
                        mode="reschedule",
                        poke_interval=300,
                        location=config['env']['v_region'],
...
for outbound_process in outbound_config[source_system]:
    ...
    if frequency == "None":
        dag_id = f'dag_outbound_{source_system}_sqlserver_adhoc_recon_{dag_name_suffix}'
        tags.extend(['adhoc', lob_abbr])
    else:
        dag_id = f'dag_outbound_{source_system}_sqlserver_{frequency}_recon_{dag_name_suffix}'
