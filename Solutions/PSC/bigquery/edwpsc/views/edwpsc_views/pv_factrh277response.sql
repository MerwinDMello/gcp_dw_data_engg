CREATE OR REPLACE VIEW edwpsc_views.`pv_factrh277response`
AS SELECT
  ...
  FROM
    edwpsc_base_views.`pv_factrh277response`
  INNER JOIN edwpsc_base_views.secref_facility
  ON LPAD(TRIM(secref_facility.co_id, ' '), 5, '0') = LPAD(TRIM(`pv_factrh277response`.coid, ' '), 5, '0')
