CREATE OR REPLACE VIEW edwpsc_views.`ecw_juncclaimpayerfirstlastkeymeasure`
AS SELECT
  ...
  FROM
    edwpsc_base_views.`ecw_juncclaimpayerfirstlastkeymeasure`
  INNER JOIN edwpsc_base_views.secref_facility
  ON LPAD(TRIM(secref_facility.co_id, ' '), 5, '0') = LPAD(TRIM(`ecw_juncclaimpayerfirstlastkeymeasure`.coid, ' '), 5, '0')
