CREATE OR REPLACE VIEW edwpsc_views.`ecw_refsrtforecastestimate`
AS SELECT
  ...
  FROM
    edwpsc_base_views.`ecw_refsrtforecastestimate`
  INNER JOIN edwpsc_base_views.secref_facility
  ON LPAD(TRIM(secref_facility.co_id, ' '), 5, '0') = LPAD(TRIM(CAST(`ecw_refsrtforecastestimate`.coid AS STRING), ' '), 5, '0')
