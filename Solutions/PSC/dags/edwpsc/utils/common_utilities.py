...
def call_config_yaml(filename, variablename):
    ...
    logging.info(default_config)
    return config

config = call_config_yaml("psc_config.yaml","hca_psc_default_vars")


# get project variables from config file
bq_project_id = config['env']['v_curated_project_id']
bq_staging_dataset = config['env']['v_psc_stage_dataset_name']
stage_dataset = config['env']['v_psc_stage_dataset_name']
...
def executevalidationsqls(**kwargs):
        ...
        validationsql_folder = f"dags/{lob}/load_sql/validation_sql/integrate/{source}"
        ...
        if "NONE" not in file_list:
            for filename in file_list:                
                validation_sql_path = f"dags/{lob}/load_sql/validation_sql/integrate/{filename}" 
            ...
            generic_validation_sql = f"dags/{lob}/load_sql/validation_sql/integrate/generic_validation_sql_runtime_capture.sql" 
...
def insert_audit_data_files(**kwargs):
    ...
    audit_table = bq_client.dataset(kwargs['config']['env']['v_psc_audit_dataset_name']).table(kwargs['config']['env']['v_audittablename'].split('.')[-1])
