...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_FactRH277Response AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_FactRH277Response AS source
...
WHEN NOT MATCHED THEN
  INSERT (RH277ResponseKey, ClaimKey, ClaimNumber, Coid, ImportDateKey, RH277ResponseClaimID, RH277ResponsePatientControlNbr, RH277ResponseDateImportedKey, RH277ResponseRespondingPayer, RH277ResponseStatusCtgyCode, RH277ResponseStatusCode, SourcePrimaryKeyValue, DWLastUpdateDateTime, SourceSystemCode, InsertedBy, InsertedDTM, ModifiedBy, ModifiedDTM, FullClaimNumber, RegionKey)
  VALUES (source.RH277ResponseKey, source.ClaimKey, source.ClaimNumber, TRIM(source.Coid), source.ImportDateKey, TRIM(source.RH277ResponseClaimID), TRIM(source.RH277ResponsePatientControlNbr), source.RH277ResponseDateImportedKey, TRIM(source.RH277ResponseRespondingPayer), TRIM(source.RH277ResponseStatusCtgyCode), TRIM(source.RH277ResponseStatusCode), source.SourcePrimaryKeyValue, source.DWLastUpdateDateTime, TRIM(source.SourceSystemCode), TRIM(source.InsertedBy), source.InsertedDTM, TRIM(source.ModifiedBy), source.ModifiedDTM, TRIM(source.FullClaimNumber), source.RegionKey);

SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT RH277ResponseKey
      FROM {{ params.param_psc_core_dataset_name }}.ECW_FactRH277Response
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_FactRH277Response');
ELSE
  COMMIT TRANSACTION;
END IF;
