...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_FactBillVendorInventory AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_FactBillVendorInventory AS source
...
WHEN MATCHED THEN
 ...
 target.OriginalFullClaimNumber = TRIM(source.OriginalFullClaimNumber),
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_FactBillVendorInventory');
ELSE
  COMMIT TRANSACTION;
END IF;
