...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_FactRHInventory AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_FactRHInventory AS source
...
WHEN MATCHED THEN
 ...
 target.ClaimKey = TRIM(source.ClaimKey),
 ...
 target.EcwRegionId = TRIM(source.EcwRegionId),
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT RHInvKey
      FROM {{ params.param_psc_core_dataset_name }}.ECW_FactRHInventory
      GROUP BY RHInvKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_FactRHInventory');
ELSE
  COMMIT TRANSACTION;
END IF;
