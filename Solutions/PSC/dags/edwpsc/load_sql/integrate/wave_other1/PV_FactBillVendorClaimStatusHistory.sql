...
MERGE INTO {{ params.param_psc_core_dataset_name }}.PV_FactBillVendorClaimStatusHistory AS target
...
SET DUP_COUNT = (
  ...
  FROM (
      ...
      FROM {{ params.param_psc_core_dataset_name }}.PV_FactBillVendorClaimStatusHistory
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.PV_FactBillVendorClaimStatusHistory');
ELSE
  COMMIT TRANSACTION;
END IF;
