...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_FactRHClaimHistory AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_FactRHClaimHistory AS source
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_FactRHClaimHistory');
ELSE
  COMMIT TRANSACTION;
END IF;
