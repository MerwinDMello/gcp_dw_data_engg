...
MERGE INTO {{ params.param_psc_core_dataset_name }}.EPIC_FactTransactionsClaimLineFinancialAdjustment AS target
USING {{ params.param_psc_stage_dataset_name }}.EPIC_FactTransactionsClaimLineFinancialAdjustment AS source
...
WHEN MATCHED THEN
 ...
 target.ClaimLineFinancialAdjustmentKey = source.ClaimLineFinancialAdjustmentKey,
...
WHEN NOT MATCHED THEN
  INSERT (TransactionsClaimLineFinancialAdjustmentKey, ClaimKey, ClaimNumber, TransactionNumber, VisitNumber, RegionKey, Coid, CoidConfigurationKey, ServicingProviderKey, ClaimPayer1IplanKey, ClaimLineLiabilityIplanKey, FacilityKey, ClaimLineFinancialAdjustmentKey, AdjustmentCodeKey, TrRefID, TransactionType, TransactionFlag, TransactionAmt, TransactionDateKey, TransactionTime, TransactionByUserKey, TransactionPeriod, SourceAPrimaryKeyValue, SourceARecordLastUpdated, SourceBPrimaryKeyValue, SourceBRecordLastUpdated, DWLastUpdateDateTime, SourceSystemCode, InsertedBy, InsertedDTM, ModifiedBy, ModifiedDTM, DebitCreditFlag, MatchingTrRefId, AccountKey, PatientKey, DistributedByUserKey, UndistributedByUserKey, HistMatchBatch, HistMatchUnbatch, EtrId, MatchingEtrId, ClaimLineChargesKey, AdjustmentCategoryKey, FirstDenialCategory)
  VALUES (source.TransactionsClaimLineFinancialAdjustmentKey, source.ClaimKey, source.ClaimNumber, source.TransactionNumber, source.VisitNumber, source.RegionKey, TRIM(source.Coid), source.CoidConfigurationKey, source.ServicingProviderKey, source.ClaimPayer1IplanKey, source.ClaimLineLiabilityIplanKey, source.FacilityKey, source.ClaimLineFinancialAdjustmentKey, source.AdjustmentCodeKey, source.TrRefID, TRIM(source.TransactionType), TRIM(source.TransactionFlag), source.TransactionAmt, source.TransactionDateKey, source.TransactionTime, source.TransactionByUserKey, TRIM(source.TransactionPeriod), source.SourceAPrimaryKeyValue, source.SourceARecordLastUpdated, TRIM(source.SourceBPrimaryKeyValue), source.SourceBRecordLastUpdated, source.DWLastUpdateDateTime, TRIM(source.SourceSystemCode), TRIM(source.InsertedBy), source.InsertedDTM, TRIM(source.ModifiedBy), source.ModifiedDTM, source.DebitCreditFlag, source.MatchingTrRefId, source.AccountKey, source.PatientKey, TRIM(source.DistributedByUserKey), TRIM(source.UndistributedByUserKey), TRIM(source.HistMatchBatch), TRIM(source.HistMatchUnbatch), source.EtrId, source.MatchingEtrId, source.ClaimLineChargesKey, source.AdjustmentCategoryKey, TRIM(source.FirstDenialCategory));

SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT TransactionsClaimLineFinancialAdjustmentKey
      FROM {{ params.param_psc_core_dataset_name }}.EPIC_FactTransactionsClaimLineFinancialAdjustment
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.EPIC_FactTransactionsClaimLineFinancialAdjustment');
ELSE
  COMMIT TRANSACTION;
END IF;
