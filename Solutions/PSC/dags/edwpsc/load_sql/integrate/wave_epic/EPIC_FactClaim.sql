...
MERGE INTO {{ params.param_psc_core_dataset_name }}.EPIC_FactClaim AS target
USING {{ params.param_psc_stage_dataset_name }}.EPIC_FactClaim AS source
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT ClaimKey
      FROM {{ params.param_psc_core_dataset_name }}.EPIC_FactClaim
      GROUP BY ClaimKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.EPIC_FactClaim');
ELSE
  COMMIT TRANSACTION;
END IF;
