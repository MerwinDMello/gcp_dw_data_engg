...
MERGE INTO {{ params.param_psc_core_dataset_name }}.EPIC_JuncDivisionCoid AS target
USING {{ params.param_psc_stage_dataset_name }}.EPIC_JuncDivisionCoid AS source
...
SET DUP_COUNT = (
  ...
  FROM (
      ...
      FROM {{ params.param_psc_core_dataset_name }}.EPIC_JuncDivisionCoid
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.EPIC_JuncDivisionCoid');
ELSE
  COMMIT TRANSACTION;
END IF;
