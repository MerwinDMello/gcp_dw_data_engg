...
MERGE INTO {{ params.param_psc_core_dataset_name }}.EPIC_RefPatientInsurance AS target
USING {{ params.param_psc_stage_dataset_name }}.EPIC_RefPatientInsurance AS source
ON target.PatientInsuranceKey = source.PatientInsuranceKey
WHEN MATCHED THEN
  UPDATE SET
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.EPIC_RefPatientInsurance');
ELSE
  COMMIT TRANSACTION;
END IF;
