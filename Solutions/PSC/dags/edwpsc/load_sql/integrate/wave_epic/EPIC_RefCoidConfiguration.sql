...
MERGE INTO {{ params.param_psc_core_dataset_name }}.EPIC_RefCoidConfiguration AS target
USING {{ params.param_psc_stage_dataset_name }}.EPIC_RefCoidConfiguration AS source
...
WHEN NOT MATCHED THEN
  INSERT (CoidConfigurationKey, Coid, DeptCode, CoidConfigurationProviderKey, CoidConfigurationFacilityKey, CoidConfigurationPracticeKey, SourcePrimaryKeyValue, SourceRecordLastUpdated, DWLastUpdateDateTime, SourceSystemCode, InsertedBy, InsertedDTM, ModifiedBy, ModifiedDTM, DeleteFlag, RegionKey)
...
SET DUP_COUNT = (
  ...
  FROM (
      ...
      FROM {{ params.param_psc_core_dataset_name }}.EPIC_RefCoidConfiguration
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.EPIC_RefCoidConfiguration');
ELSE
  COMMIT TRANSACTION;
END IF;
