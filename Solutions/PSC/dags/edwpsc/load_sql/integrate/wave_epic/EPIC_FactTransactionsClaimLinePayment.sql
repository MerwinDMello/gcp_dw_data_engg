...
MERGE INTO {{ params.param_psc_core_dataset_name }}.EPIC_FactTransactionsClaimLinePayment AS target
USING {{ params.param_psc_stage_dataset_name }}.EPIC_FactTransactionsClaimLinePayment AS source
ON target.TransactionsClaimLinePaymentKey = source.TransactionsClaimLinePaymentKey
WHEN MATCHED THEN
  UPDATE SET
  target.TransactionsClaimLinePaymentKey = source.TransactionsClaimLinePaymentKey,
...
WHEN NOT MATCHED THEN
  INSERT (TransactionsClaimLinePaymentKey, ClaimKey, ClaimNumber, TransactionNumber, VisitNumber, RegionKey, Coid, CoidConfigurationKey, ServicingProviderKey, ClaimPayer1IplanKey, ClaimLineLiabilityIplanKey, FacilityKey, ClaimLinePaymentKey, TrRefID, TransactionType, TransactionFlag, TransactionAmt, TransactionDateKey, TransactionTime, TransactionByUserKey, TransactionPeriod, SourceAPrimaryKeyValue, SourceARecordLastUpdated, SourceBPrimaryKeyValue, SourceBRecordLastUpdated, DWLastUpdateDateTime, SourceSystemCode, InsertedBy, InsertedDTM, ModifiedBy, ModifiedDTM, MatchingTransactionNumber, PaymentTypeKey, EpicPaymentTypeKey, DebitCreditFlag, MatchingTrRefId, AccountKey, PatientKey, DistributedByUserKey, UndistributedByUserKey, EtrId, MatchingEtrId)
  VALUES (source.TransactionsClaimLinePaymentKey, source.ClaimKey, source.ClaimNumber, source.TransactionNumber, source.VisitNumber, source.RegionKey, TRIM(source.Coid), source.CoidConfigurationKey, source.ServicingProviderKey, source.ClaimPayer1IplanKey, source.ClaimLineLiabilityIplanKey, source.FacilityKey, source.ClaimLinePaymentKey, source.TrRefID, TRIM(source.TransactionType), TRIM(source.TransactionFlag), source.TransactionAmt, source.TransactionDateKey, source.TransactionTime, source.TransactionByUserKey, TRIM(source.TransactionPeriod), source.SourceAPrimaryKeyValue, source.SourceARecordLastUpdated, TRIM(source.SourceBPrimaryKeyValue), source.SourceBRecordLastUpdated, source.DWLastUpdateDateTime, TRIM(source.SourceSystemCode), TRIM(source.InsertedBy), source.InsertedDTM, TRIM(source.ModifiedBy), source.ModifiedDTM, source.MatchingTransactionNumber, source.PaymentTypeKey, source.EpicPaymentTypeKey, source.DebitCreditFlag, source.MatchingTrRefId, source.AccountKey, source.PatientKey, TRIM(source.DistributedByUserKey), TRIM(source.UndistributedByUserKey), source.EtrId, source.MatchingEtrId);

SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT TransactionsClaimLinePaymentKey
      FROM {{ params.param_psc_core_dataset_name }}.EPIC_FactTransactionsClaimLinePayment
      GROUP BY TransactionsClaimLinePaymentKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.EPIC_FactTransactionsClaimLinePayment');
ELSE
  COMMIT TRANSACTION;
END IF;
