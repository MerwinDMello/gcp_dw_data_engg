...
MERGE INTO {{ params.param_psc_core_dataset_name }}.EPIC_JuncFactClaimToPayer AS target
USING {{ params.param_psc_stage_dataset_name }}.EPIC_JuncFactClaimToPayer AS source
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT ClaimKey
      FROM {{ params.param_psc_core_dataset_name }}.EPIC_JuncFactClaimToPayer
      GROUP BY ClaimKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.EPIC_JuncFactClaimToPayer');
ELSE
  COMMIT TRANSACTION;
END IF;
