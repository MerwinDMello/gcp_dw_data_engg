...
MERGE INTO {{ params.param_psc_core_dataset_name }}.EPIC_FactClaimLineFinancialAdjustment AS target
USING {{ params.param_psc_stage_dataset_name }}.EPIC_FactClaimLineFinancialAdjustment AS source
...
WHEN NOT MATCHED THEN
  INSERT (ClaimLineFinancialAdjustmentsKey, ClaimKey, ClaimNumber, TransactionNumber, VisitNumber, RegionKey, Coid, CoidConfigurationKey, ServicingProviderKey, ClaimPayer1IplanKey, ClaimLineLiabilityIplanKey, FacilityKey, ClaimAdjustmentKey, ClaimLineChargesKey, AdjustmentAmt, AdjustmentCodeKey, AdjCode, AdjustmentCategoryKey, CPTID, CPTCode, CPTCodeKey, CPTCharges, ClaimAdjustmentAmt, ClaimAdjustmentDateKey, ClaimAdjustmentDeleteFlag, DeleteFlag, CreatedDateKey, CreatedTime, CreatedByUserKey, CPTDeleteFlag, SourceAPrimaryKeyValue, SourceARecordLastUpdated, SourceBPrimaryKeyValue, DWLastUpdateDateTime, SourceSystemCode, InsertedBy, InsertedDTM, ModifiedBy, ModifiedDTM)
  VALUES (source.ClaimLineFinancialAdjustmentsKey, source.ClaimKey, source.ClaimNumber, source.TransactionNumber, source.VisitNumber, source.RegionKey, TRIM(source.Coid), source.CoidConfigurationKey, source.ServicingProviderKey, source.ClaimPayer1IplanKey, source.ClaimLineLiabilityIplanKey, source.FacilityKey, source.ClaimAdjustmentKey, source.ClaimLineChargesKey, source.AdjustmentAmt, source.AdjustmentCodeKey, TRIM(source.AdjCode), source.AdjustmentCategoryKey, TRIM(source.CPTID), TRIM(source.CPTCode), source.CPTCodeKey, source.CPTCharges, source.ClaimAdjustmentAmt, source.ClaimAdjustmentDateKey, source.ClaimAdjustmentDeleteFlag, source.DeleteFlag, source.CreatedDateKey, source.CreatedTime, source.CreatedByUserKey, source.CPTDeleteFlag, TRIM(source.SourceAPrimaryKeyValue), source.SourceARecordLastUpdated, TRIM(source.SourceBPrimaryKeyValue), source.DWLastUpdateDateTime, TRIM(source.SourceSystemCode), TRIM(source.InsertedBy), source.InsertedDTM, TRIM(source.ModifiedBy), source.ModifiedDTM);

SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT ClaimLineFinancialAdjustmentsKey
      FROM {{ params.param_psc_core_dataset_name }}.EPIC_FactClaimLineFinancialAdjustment
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.EPIC_FactClaimLineFinancialAdjustment');
ELSE
  COMMIT TRANSACTION;
END IF;
