...
MERGE INTO {{ params.param_psc_core_dataset_name }}.EPIC_FactClaim_History AS target
USING {{ params.param_psc_stage_dataset_name }}.EPIC_FactClaim_History AS source
ON target.ClaimKey = source.ClaimKey AND target.SysStartTime = source.SysStartTime AND target.SysEndTime = source.SysEndTime
WHEN MATCHED THEN
  UPDATE SET
 ...
 target.ClaimCreatedByID = TRIM(source.ClaimCreatedByID),
 ...
 target.Payer1EpicFinancialClass = source.Payer1EpicFinancialClass,
 target.Payer1EpicFinancialClassDesc = TRIM(source.Payer1EpicFinancialClassDesc),
 target.SourceCPrimaryKeyValue = TRIM(source.SourceCPrimaryKeyValue),
 target.TotalHoldBalanceAmt = source.TotalHoldBalanceAmt,
 target.PreARFlag = source.PreARFlag,
 target.TotalPreARBalanceAmt = source.TotalPreARBalanceAmt,
 target.SysStartTime = source.SysStartTime,
 target.SysEndTime = source.SysEndTime
WHEN NOT MATCHED THEN
  INSERT (ClaimKey, ClaimNumber, Coid, RegionKey, ClaimDateKey, ServiceDateKey, PatientKey, GuarantorPatientKey, ServicingProviderKey, RenderingProviderKey, PayToProviderKey, SupervisorProviderKey, AddProvider1ProviderKey, AddProvider2ProviderKey, LiabilityDateKey, LiabilityIplanKey, BilledToID, BilledToType, FacilityKey, ClaimStatusKey, EncounterKey, EncounterID, EncounterForm, VisitNumber, AccountId, TotalChargesAmt, TotalBalanceAmt, TotalPatientBalanceAmt, TotalInsuranceBalanceAmt, TotalPatientBalanceMinusPatientIplansAmt, TotalPaymentsAmt, TotalInsurancePaymentsAmt, TotalPatientPaymentsAmt, TotalAdjustmentsAmt, TotalUncoveredAmt, TotalAllowedAmt, TotalPatientResponsibilityAmt, TotalEstimatedPatientBalanceAmt, TotalPaymentsAndAdjustmentsAmt, VoidFlag, DeleteFlag, CoidConfigurationKey, ClaimCreatedByUserKey, ClaimQueueID, ReferringProviderKey, ReferringProviderID, ReferringProviderType, Copay, ARClaimFlag, VoidedClaimReversedClaimNumber, VoidedClaimCopyClaimNumber, VoidedClaimOriginalClaimNumber, SourceAPrimaryKeyValue, SourceARecordLastUpdated, SourceAChangedFlag, DWLastUpdateDateTime, SourceSystemCode, InsertedBy, InsertedDTM, ModifiedBy, ModifiedDTM, ClaimLock, ClaimTypeFlag, AccidentIndFlag, AccidentState, AccidentHour, AccidentRel, SymIndFlag, SymDateKey, EmpRelated, AdmitDateKey, DischargeDateKey, SplPgmId, EPSDTReferral, EPSDTRefSts, CrimeRelatedFlag, DocSentDateKey, PWK06, PregIndctorFlag, RelInfoDateKey, RelInfoInd, LabChargesAmt, LabIndFlag, PatientID, PatientInternalID, ServicingProviderID, RenderingProviderID, PayToProviderID, SupervisorProviderID, AddProvider1ProviderID, AddProvider2ProviderID, FacilityID, PracticeID, ClaimCreatedByID, DeptCode, Payer1IplanKey, Payer1IplanID, Payer1FinancialClassKey, Payer2IplanKey, Payer2IplanID, Payer2FinancialClassKey, Payer3IplanKey, Payer3IplanID, Payer3FinancialClassKey, CollectionStatus, CollectionCodeChangeDate, CollectionCode, CollectionCodeChangeDateDATE, CollectionStatusAging, ServiceAreaKey, BillAreaKey, ClaimBilledFlag, SourceBPrimaryKeyValue, HoldCode, Payer1EpicFinancialClass, Payer1EpicFinancialClassDesc, SourceCPrimaryKeyValue, TotalHoldBalanceAmt, PreARFlag, TotalPreARBalanceAmt, SysStartTime, SysEndTime)
  VALUES (source.ClaimKey, source.ClaimNumber, TRIM(source.Coid), source.RegionKey, source.ClaimDateKey, source.ServiceDateKey, source.PatientKey, source.GuarantorPatientKey, source.ServicingProviderKey, source.RenderingProviderKey, source.PayToProviderKey, source.SupervisorProviderKey, source.AddProvider1ProviderKey, source.AddProvider2ProviderKey, source.LiabilityDateKey, source.LiabilityIplanKey, TRIM(source.BilledToID), source.BilledToType, source.FacilityKey, source.ClaimStatusKey, source.EncounterKey, source.EncounterID, TRIM(source.EncounterForm), source.VisitNumber, source.AccountId, source.TotalChargesAmt, source.TotalBalanceAmt, source.TotalPatientBalanceAmt, source.TotalInsuranceBalanceAmt, source.TotalPatientBalanceMinusPatientIplansAmt, source.TotalPaymentsAmt, source.TotalInsurancePaymentsAmt, source.TotalPatientPaymentsAmt, source.TotalAdjustmentsAmt, source.TotalUncoveredAmt, source.TotalAllowedAmt, source.TotalPatientResponsibilityAmt, source.TotalEstimatedPatientBalanceAmt, source.TotalPaymentsAndAdjustmentsAmt, source.VoidFlag, source.DeleteFlag, source.CoidConfigurationKey, source.ClaimCreatedByUserKey, source.ClaimQueueID, source.ReferringProviderKey, TRIM(source.ReferringProviderID), TRIM(source.ReferringProviderType), source.Copay, source.ARClaimFlag, source.VoidedClaimReversedClaimNumber, source.VoidedClaimCopyClaimNumber, source.VoidedClaimOriginalClaimNumber, TRIM(source.SourceAPrimaryKeyValue), source.SourceARecordLastUpdated, source.SourceAChangedFlag, source.DWLastUpdateDateTime, TRIM(source.SourceSystemCode), TRIM(source.InsertedBy), source.InsertedDTM, TRIM(source.ModifiedBy), source.ModifiedDTM, TRIM(source.ClaimLock), TRIM(source.ClaimTypeFlag), TRIM(source.AccidentIndFlag), TRIM(source.AccidentState), TRIM(source.AccidentHour), TRIM(source.AccidentRel), TRIM(source.SymIndFlag), source.SymDateKey, TRIM(source.EmpRelated), source.AdmitDateKey, source.DischargeDateKey, TRIM(source.SplPgmId), TRIM(source.EPSDTReferral), TRIM(source.EPSDTRefSts), TRIM(source.CrimeRelatedFlag), source.DocSentDateKey, TRIM(source.PWK06), TRIM(source.PregIndctorFlag), source.RelInfoDateKey, TRIM(source.RelInfoInd), source.LabChargesAmt, TRIM(source.LabIndFlag), TRIM(source.PatientID), source.PatientInternalID, TRIM(source.ServicingProviderID), TRIM(source.RenderingProviderID), TRIM(source.PayToProviderID), TRIM(source.SupervisorProviderID), TRIM(source.AddProvider1ProviderID), TRIM(source.AddProvider2ProviderID), source.FacilityID, TRIM(source.PracticeID), TRIM(source.ClaimCreatedByID), TRIM(source.DeptCode), source.Payer1IplanKey, source.Payer1IplanID, source.Payer1FinancialClassKey, source.Payer2IplanKey, source.Payer2IplanID, source.Payer2FinancialClassKey, source.Payer3IplanKey, source.Payer3IplanID, source.Payer3FinancialClassKey, TRIM(source.CollectionStatus), TRIM(source.CollectionCodeChangeDate), TRIM(source.CollectionCode), source.CollectionCodeChangeDateDATE, source.CollectionStatusAging, source.ServiceAreaKey, source.BillAreaKey, source.ClaimBilledFlag, TRIM(source.SourceBPrimaryKeyValue), TRIM(source.HoldCode), source.Payer1EpicFinancialClass, TRIM(source.Payer1EpicFinancialClassDesc), TRIM(source.SourceCPrimaryKeyValue), source.TotalHoldBalanceAmt, source.PreARFlag, source.TotalPreARBalanceAmt, source.SysStartTime, source.SysEndTime);

SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT ClaimKey, SysStartTime, SysEndTime
      FROM {{ params.param_psc_core_dataset_name }}.EPIC_FactClaim_History
      GROUP BY ClaimKey, SysStartTime, SysEndTime
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.EPIC_FactClaim_History');
ELSE
  COMMIT TRANSACTION;
END IF;
