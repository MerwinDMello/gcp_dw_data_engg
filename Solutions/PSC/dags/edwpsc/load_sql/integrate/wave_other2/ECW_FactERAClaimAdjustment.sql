...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_FactERAClaimAdjustment AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_FactERAClaimAdjustment AS source
ON target.ERAClaimAdjustmentKey = source.ERAClaimAdjustmentKey
WHEN MATCHED THEN
  UPDATE SET
  target.ERAClaimAdjustmentKey = source.ERAClaimAdjustmentKey,
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_FactERAClaimAdjustment');
ELSE
  COMMIT TRANSACTION;
END IF;
