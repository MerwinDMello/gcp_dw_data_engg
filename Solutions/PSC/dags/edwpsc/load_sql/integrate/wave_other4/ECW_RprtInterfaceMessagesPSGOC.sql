...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RprtInterfaceMessagesPSGOC AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RprtInterfaceMessagesPSGOC AS source
ON target.SourceAPrimaryKeyValue = source.SourceAPrimaryKeyValue AND target.SourceBPrimaryKeyValue = source.SourceBPrimaryKeyValue AND target.SourceSystemCode = source.SourceSystemCode
WHEN MATCHED THEN
  UPDATE SET
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RprtInterfaceMessagesPSGOC');
ELSE
  COMMIT TRANSACTION;
END IF;
