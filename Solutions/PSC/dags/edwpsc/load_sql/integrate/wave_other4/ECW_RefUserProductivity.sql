...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefUserProductivity AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefUserProductivity AS source
ON target.UserProductivityKey = source.UserProductivityKey
WHEN MATCHED THEN
  UPDATE SET
...
SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT UserProductivityKey
      FROM {{ params.param_psc_core_dataset_name }}.ECW_RefUserProductivity
      GROUP BY UserProductivityKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefUserProductivity');
ELSE
  COMMIT TRANSACTION;
END IF;
