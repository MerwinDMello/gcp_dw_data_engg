...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RprtInterfaceMessagesPSCOC AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RprtInterfaceMessagesPSCOC AS source
ON target.SourceAPrimaryKeyValue = source.SourceAPrimaryKeyValue AND target.SendingApplication = source.SendingApplication
WHEN MATCHED THEN
  UPDATE SET
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RprtInterfaceMessagesPSCOC');
ELSE
  COMMIT TRANSACTION;
END IF;
