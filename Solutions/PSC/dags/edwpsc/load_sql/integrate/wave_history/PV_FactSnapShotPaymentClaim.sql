...
MERGE INTO {{ params.param_psc_core_dataset_name }}.PV_FactSnapShotPaymentClaim AS target
USING {{ params.param_psc_stage_dataset_name }}.PV_FactSnapShotPaymentClaim AS source
...
WHEN NOT MATCHED THEN
  INSERT (PaymentClaimKey, MonthID, SnapShotDate, Coid, RegionKey, PracticeKey, PracticeID, ClaimKey, ClaimNumber, GLDepartment, PatientID, ServicingProviderKey, ServicingProviderID, RenderingProviderKey, RenderingProviderID, FacilityKey, FacilityID, ClaimDateKey, ServiceDateKey, Iplan1IplanKey, Iplan1ID, FinancialClassKey, EncounterID, ContractualAdjustmentAmt, AllowedAmt, DeductibleAmt, CoinsuranceAmt, MemberBalanceAmt, PaymentAmt, WithHeldAmt, CheckAmt, PaymentCreatedDateKey, PaymentModifiedDateKey, PaymentTypeKey, PaymentID, PaymentDetailID, CheckDateKey, CheckNumber, CheckType, PayerID, PostedDateKey, PaymentFromIplanKey, PaymentFromIplanID, PaymentMemo, PaymentLock, PaymentDetailModifiedDateKey, DWLastUpdateDateTime)
  VALUES (source.PaymentClaimKey, source.MonthID, source.SnapShotDate, TRIM(source.Coid), source.RegionKey, source.PracticeKey, TRIM(source.PracticeID), source.ClaimKey, source.ClaimNumber, TRIM(source.GLDepartment), source.PatientID, source.ServicingProviderKey, source.ServicingProviderID, source.RenderingProviderKey, source.RenderingProviderID, source.FacilityKey, source.FacilityID, source.ClaimDateKey, source.ServiceDateKey, source.Iplan1IplanKey, TRIM(source.Iplan1ID), source.FinancialClassKey, source.EncounterID, source.ContractualAdjustmentAmt, source.AllowedAmt, source.DeductibleAmt, source.CoinsuranceAmt, source.MemberBalanceAmt, source.PaymentAmt, source.WithHeldAmt, source.CheckAmt, source.PaymentCreatedDateKey, source.PaymentModifiedDateKey, source.PaymentTypeKey, source.PaymentID, source.PaymentDetailID, source.CheckDateKey, TRIM(source.CheckNumber), TRIM(source.CheckType), TRIM(source.PayerID), source.PostedDateKey, source.PaymentFromIplanKey, TRIM(source.PaymentFromIplanID), TRIM(source.PaymentMemo), source.PaymentLock, source.PaymentDetailModifiedDateKey, source.DWLastUpdateDateTime);

SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT SnapShotDate, PaymentClaimKey
      FROM {{ params.param_psc_core_dataset_name }}.PV_FactSnapShotPaymentClaim
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.PV_FactSnapShotPaymentClaim');
ELSE
  COMMIT TRANSACTION;
END IF;
