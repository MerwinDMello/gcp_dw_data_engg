...
MERGE INTO {{ params.param_psc_core_dataset_name }}.PV_FactSnapShotFinancialAdjustmentLine AS target
USING {{ params.param_psc_stage_dataset_name }}.PV_FactSnapShotFinancialAdjustmentLine AS source
...
WHEN NOT MATCHED THEN
  INSERT (AdjustmentLineKey, MonthID, SnapShotDate, ClaimKey, ClaimNumber, ServicingProviderKey, RenderingProviderKey, FacilityKey, AdjustmentCodeKey, Iplan1IplanKey, Practicekey, PracticeID, CPTCodeKey, GLDepartmentNum, PatientID, ServicingProviderID, RenderingProviderID, FacilityID, ClaimDateKey, ServiceDateKey, Iplan1ID, FinancialClassKey, EncounterID, AdjustmentLineID, AdjustmentID, CPTID, CPTCode, CPTStartServiceDateKey, CPTEndServiceDateKey, CPTOrder, CPTDeleletedLine, CPTModifier1, CPTModifier2, AdjustmentLineCreateDate, AdjustmentLineModifiedDate, AdjustmentClaimCreateDate, AdjustmentClaimModifiedDate, RegionKey, Coid, AdjustmentCode, AdjustmentClaimAmt, AdjustmentClaimUnpostedCPTAmt, AdjustmentClaimUnpostedAmt, CPTUnits, CPTChargesAmt, AdjustmentLineAmt)
  VALUES (source.AdjustmentLineKey, source.MonthID, source.SnapShotDate, source.ClaimKey, source.ClaimNumber, source.ServicingProviderKey, source.RenderingProviderKey, source.FacilityKey, source.AdjustmentCodeKey, source.Iplan1IplanKey, source.Practicekey, TRIM(source.PracticeID), source.CPTCodeKey, TRIM(source.GLDepartmentNum), source.PatientID, source.ServicingProviderID, source.RenderingProviderID, source.FacilityID, source.ClaimDateKey, source.ServiceDateKey, TRIM(source.Iplan1ID), TRIM(source.FinancialClassKey), source.EncounterID, TRIM(source.AdjustmentLineID), TRIM(source.AdjustmentID), TRIM(source.CPTID), TRIM(source.CPTCode), source.CPTStartServiceDateKey, source.CPTEndServiceDateKey, source.CPTOrder, source.CPTDeleletedLine, TRIM(source.CPTModifier1), TRIM(source.CPTModifier2), source.AdjustmentLineCreateDate, source.AdjustmentLineModifiedDate, source.AdjustmentClaimCreateDate, source.AdjustmentClaimModifiedDate, source.RegionKey, TRIM(source.Coid), TRIM(source.AdjustmentCode), source.AdjustmentClaimAmt, source.AdjustmentClaimUnpostedCPTAmt, source.AdjustmentClaimUnpostedAmt, source.CPTUnits, source.CPTChargesAmt, source.AdjustmentLineAmt);

SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT SnapShotDate, AdjustmentLineKey
      FROM {{ params.param_psc_core_dataset_name }}.PV_FactSnapShotFinancialAdjustmentLine
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.PV_FactSnapShotFinancialAdjustmentLine');
ELSE
  COMMIT TRANSACTION;
END IF;
