...
MERGE INTO {{ params.param_psc_core_dataset_name }}.EPIC_FactSnapShotRiskAnalysis AS target
USING {{ params.param_psc_stage_dataset_name }}.EPIC_FactSnapShotRiskAnalysis AS source
...
WHEN MATCHED THEN
 ...
 target.PayerFinancialClass = source.PayerFinancialClass,
 target.PPIPayorFinancialClass = TRIM(source.PPIPayorFinancialClass),
 ...
 target.EpicHoldCodeID = TRIM(source.EpicHoldCodeID),
 target.EpicHoldCodeDescription = TRIM(source.EpicHoldCodeDescription),
 target.EpicHoldCodeType = TRIM(source.EpicHoldCodeType),
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.EPIC_FactSnapShotRiskAnalysis');
ELSE
  COMMIT TRANSACTION;
END IF;
