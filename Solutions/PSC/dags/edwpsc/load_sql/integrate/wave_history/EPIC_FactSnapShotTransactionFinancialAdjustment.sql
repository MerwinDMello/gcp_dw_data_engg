...
MERGE INTO {{ params.param_psc_core_dataset_name }}.EPIC_FactSnapShotTransactionFinancialAdjustment AS target
USING {{ params.param_psc_stage_dataset_name }}.EPIC_FactSnapShotTransactionFinancialAdjustment AS source
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.EPIC_FactSnapShotTransactionFinancialAdjustment');
ELSE
  COMMIT TRANSACTION;
END IF;
