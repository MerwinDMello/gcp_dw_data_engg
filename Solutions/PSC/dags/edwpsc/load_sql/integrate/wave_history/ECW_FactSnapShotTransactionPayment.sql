...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_FactSnapShotTransactionPayment AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_FactSnapShotTransactionPayment AS source
...
WHEN NOT MATCHED THEN
  INSERT (TransactionPaymentKey, MonthId, SnapShotDate, RegionKey, Coid, GLDepartment, Claimkey, ClaimNumber, PatientKey, PatientID, ServicingProviderKey, ServicingProviderID, RenderingProviderKey, RenderingProviderID, FacilityKey, FacilityID, ClaimDateKey, ClaimDateMonthID, ServiceDateKey, ServiceDateMonthID, Iplan1IplanKey, Iplan1ID, FinancialClassKey, PaymentTypeKey, CheckDateKey, CheckType, EncounterID, EncounterKey, PaymentDetailID, PaymentID, CheckAmt, PostedDateKey, TransactionID, TransactionAmt, TransactionDateKey, TransactionDateMonthID, PaymentFromIplanKey, PaymentFromIplanID, CheckNumber, DWLastUpdateDateTime, SourceSystemCode, InsertedBy, InsertedDTM, ModifiedBy, ModifiedDTM, TransactionType, TransactionSameMonthFlag)
  VALUES (source.TransactionPaymentKey, source.MonthId, source.SnapShotDate, source.RegionKey, TRIM(source.Coid), TRIM(source.GLDepartment), source.Claimkey, source.ClaimNumber, source.PatientKey, source.PatientID, source.ServicingProviderKey, source.ServicingProviderID, source.RenderingProviderKey, source.RenderingProviderID, source.FacilityKey, source.FacilityID, source.ClaimDateKey, source.ClaimDateMonthID, source.ServiceDateKey, source.ServiceDateMonthID, source.Iplan1IplanKey, source.Iplan1ID, source.FinancialClassKey, source.PaymentTypeKey, source.CheckDateKey, TRIM(source.CheckType), source.EncounterID, source.EncounterKey, source.PaymentDetailID, source.PaymentID, source.CheckAmt, source.PostedDateKey, source.TransactionID, source.TransactionAmt, source.TransactionDateKey, source.TransactionDateMonthID, source.PaymentFromIplanKey, source.PaymentFromIplanID, TRIM(source.CheckNumber), source.DWLastUpdateDateTime, TRIM(source.SourceSystemCode), TRIM(source.InsertedBy), source.InsertedDTM, TRIM(source.ModifiedBy), source.ModifiedDTM, TRIM(source.TransactionType), TRIM(source.TransactionSameMonthFlag));

SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT SnapShotDate, TransactionPaymentKey
      FROM {{ params.param_psc_core_dataset_name }}.ECW_FactSnapShotTransactionPayment
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_FactSnapShotTransactionPayment');
ELSE
  COMMIT TRANSACTION;
END IF;
