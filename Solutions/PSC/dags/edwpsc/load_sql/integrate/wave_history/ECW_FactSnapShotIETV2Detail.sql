...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_FactSnapShotIETV2Detail AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_FactSnapShotIETV2Detail AS source
ON target.SnapShotDate = source.SnapShotDate AND target.IETV2DetailKey = source.IETV2DetailKey
WHEN MATCHED THEN
  UPDATE SET
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_FactSnapShotIETV2Detail');
ELSE
  COMMIT TRANSACTION;
END IF;
