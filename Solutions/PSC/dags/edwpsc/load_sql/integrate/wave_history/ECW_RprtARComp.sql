...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RprtARComp AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RprtARComp AS source
...
WHEN MATCHED THEN
 ...
 target.GLDepartmentNumber = source.GLDepartmentNumber,
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RprtARComp');
ELSE
  COMMIT TRANSACTION;
END IF;
