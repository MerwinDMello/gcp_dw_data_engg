...
MERGE INTO {{ params.param_psc_core_dataset_name }}.EPIC_FactSnapShotFinancialAdjustmentClaim AS target
USING {{ params.param_psc_stage_dataset_name }}.EPIC_FactSnapShotFinancialAdjustmentClaim AS source
...
WHEN NOT MATCHED THEN
  INSERT (AdjustmentClaimKey, MonthID, SnapShotDate, Coid, RegionKey, ClaimKey, ClaimNumber, VisitNumber, TransactionNumber, GLDepartment, PatientID, ServicingProviderKey, ServicingProviderID, RenderingProviderKey, RenderingProviderID, FacilityKey, FacilityID, ClaimDateKey, ServiceDateKey, Iplan1IplanKey, Iplan1ID, FinancialClassKey, AdjustmentID, AdjustmentCode, AdjustmentCodeKey, AdjustmentAmt, UnpostedCPTAmt, UnpostedClaimAmt, AdjustmentCreateDateKey, AdjustmentModifiedDateKey, DWLastUpdateDateTime)
  VALUES (source.AdjustmentClaimKey, source.MonthID, source.SnapShotDate, TRIM(source.Coid), source.RegionKey, source.ClaimKey, source.ClaimNumber, source.VisitNumber, source.TransactionNumber, TRIM(source.GLDepartment), source.PatientID, source.ServicingProviderKey, TRIM(source.ServicingProviderID), source.RenderingProviderKey, TRIM(source.RenderingProviderID), source.FacilityKey, source.FacilityID, source.ClaimDateKey, source.ServiceDateKey, source.Iplan1IplanKey, TRIM(source.Iplan1ID), source.FinancialClassKey, TRIM(source.AdjustmentID), TRIM(source.AdjustmentCode), source.AdjustmentCodeKey, source.AdjustmentAmt, source.UnpostedCPTAmt, source.UnpostedClaimAmt, source.AdjustmentCreateDateKey, source.AdjustmentModifiedDateKey, source.DWLastUpdateDateTime);

SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT SnapShotDate, AdjustmentClaimKey
      FROM {{ params.param_psc_core_dataset_name }}.EPIC_FactSnapShotFinancialAdjustmentClaim
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.EPIC_FactSnapShotFinancialAdjustmentClaim');
ELSE
  COMMIT TRANSACTION;
END IF;
