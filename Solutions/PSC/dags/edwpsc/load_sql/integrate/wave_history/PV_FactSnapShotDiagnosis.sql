...
MERGE INTO {{ params.param_psc_core_dataset_name }}.PV_FactSnapShotDiagnosis AS target
USING {{ params.param_psc_stage_dataset_name }}.PV_FactSnapShotDiagnosis AS source
...
WHEN MATCHED THEN
 ...
 target.DiagnosisID = TRIM(source.DiagnosisID),
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.PV_FactSnapShotDiagnosis');
ELSE
  COMMIT TRANSACTION;
END IF;
