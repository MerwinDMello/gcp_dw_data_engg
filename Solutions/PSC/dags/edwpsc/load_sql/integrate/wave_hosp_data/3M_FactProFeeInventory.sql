...
MERGE INTO {{ params.param_psc_core_dataset_name }}.3M_FactProFeeInventory AS target
USING {{ params.param_psc_stage_dataset_name }}.3M_FactProFeeInventory AS source
ON target.ProFeeInventory3MKey = source.ProFeeInventory3MKey
WHEN MATCHED THEN
  UPDATE SET
  target.ProFeeInventory3MKey = source.ProFeeInventory3MKey,
 ...
 target.HoldDate = source.HoldDate,
 target.HoldBy = TRIM(source.HoldBy),
 ...
 target.OpenInventoryFlag = source.OpenInventoryFlag,
 target.DWLastUpdateDateTime = source.DWLastUpdateDateTime
WHEN NOT MATCHED THEN
  INSERT (ProFeeInventory3MKey, CacAccountNumber, VisitNumber, VisitType, FacilityId, LOCATION, PatientFirstName, PatientLastName, PatientMiddleName, AdmitDate, DischargeDate, ServiceDateKey, ServiceDateTime, CacWorklistName, LookupKey, CodingStatus, CodingStatusDateTime, COMMENT, DocHoldReason, HoldDate, HoldBy, CareProvider, CareProviderFirstName, CareProviderLastName, CareProviderMiddleName, FileName, FileDate, InsertedBy, InsertedDTM, ModifiedBy, ModifiedDTM, Coid, PracticeId, OpenInventoryFlag, DWLastUpdateDateTime)
  VALUES (source.ProFeeInventory3MKey, TRIM(source.CacAccountNumber), TRIM(source.VisitNumber), TRIM(source.VisitType), TRIM(source.FacilityId), TRIM(source.LOCATION), TRIM(source.PatientFirstName), TRIM(source.PatientLastName), TRIM(source.PatientMiddleName), source.AdmitDate, source.DischargeDate, source.ServiceDateKey, source.ServiceDateTime, TRIM(source.CacWorklistName), TRIM(source.LookupKey), TRIM(source.CodingStatus), source.CodingStatusDateTime, TRIM(source.COMMENT), TRIM(source.DocHoldReason), source.HoldDate, TRIM(source.HoldBy), TRIM(source.CareProvider), TRIM(source.CareProviderFirstName), TRIM(source.CareProviderLastName), TRIM(source.CareProviderMiddleName), TRIM(source.FileName), source.FileDate, TRIM(source.InsertedBy), source.InsertedDTM, TRIM(source.ModifiedBy), source.ModifiedDTM, TRIM(source.Coid), TRIM(source.PracticeId), source.OpenInventoryFlag, source.DWLastUpdateDateTime);

SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT ProFeeInventory3MKey
      FROM {{ params.param_psc_core_dataset_name }}.3M_FactProFeeInventory
      GROUP BY ProFeeInventory3MKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.3M_FactProFeeInventory');
ELSE
  COMMIT TRANSACTION;
END IF;
