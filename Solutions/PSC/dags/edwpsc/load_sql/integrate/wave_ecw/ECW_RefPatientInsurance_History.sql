...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefPatientInsurance_History AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefPatientInsurance_History AS source
ON target.PatientInsuranceKey = source.PatientInsuranceKey AND target.RegionKey = source.RegionKey AND target.SysStartTime = source.SysStartTime AND target.SysEndTime = source.SysEndTime
WHEN MATCHED THEN
  UPDATE SET
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefPatientInsurance_History');
ELSE
  COMMIT TRANSACTION;
END IF;
