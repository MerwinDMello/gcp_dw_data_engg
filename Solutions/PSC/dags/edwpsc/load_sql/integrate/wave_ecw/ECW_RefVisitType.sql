...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefVisitType AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefVisitType AS source
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT VisitTypeKey
      FROM {{ params.param_psc_core_dataset_name }}.ECW_RefVisitType
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefVisitType');
ELSE
  COMMIT TRANSACTION;
END IF;
