...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefIplan_History AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefIplan_History AS source
ON target.IplanKey = source.IplanKey AND target.SysStartTime = source.SysStartTime AND target.SysEndTime = source.SysEndTime
WHEN MATCHED THEN
  UPDATE SET
...
SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT IplanKey, SysStartTime, SysEndTime
      FROM {{ params.param_psc_core_dataset_name }}.ECW_RefIplan_History
      GROUP BY IplanKey, SysStartTime, SysEndTime
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefIplan_History');
ELSE
  COMMIT TRANSACTION;
END IF;
