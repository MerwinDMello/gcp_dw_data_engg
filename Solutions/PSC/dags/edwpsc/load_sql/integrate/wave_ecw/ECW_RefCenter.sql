...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefCenter AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefCenter AS source
ON target.CenterKey = source.CenterKey
WHEN MATCHED THEN
  UPDATE SET
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT CenterKey
      FROM {{ params.param_psc_core_dataset_name }}.ECW_RefCenter
      GROUP BY CenterKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefCenter');
ELSE
  COMMIT TRANSACTION;
END IF;
