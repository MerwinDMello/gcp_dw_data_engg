...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_FactEncounterCPT AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_FactEncounterCPT AS source
...
WHEN NOT MATCHED THEN
  INSERT (EncounterCPTKey, RegionKey, Coid, EncounterKey, EncounterID, CPTCodeKey, CPTCode, CPTUnits, CPTOrder, VisitDate, PrimaryFlag, DeleteFlag, SourcePrimaryKeyValue, SourceRecordLastUpdated, DWLastUpdateDateTime, SourceSystemCode, InsertedBy, InsertedDTM, ModifiedBy, ModifiedDTM, ArchivedRecord)
  VALUES (source.EncounterCPTKey, source.RegionKey, TRIM(source.Coid), source.EncounterKey, source.EncounterID, source.CPTCodeKey, TRIM(source.CPTCode), source.CPTUnits, source.CPTOrder, source.VisitDate, source.PrimaryFlag, source.DeleteFlag, TRIM(source.SourcePrimaryKeyValue), source.SourceRecordLastUpdated, source.DWLastUpdateDateTime, TRIM(source.SourceSystemCode), TRIM(source.InsertedBy), source.InsertedDTM, TRIM(source.ModifiedBy), source.ModifiedDTM, TRIM(source.ArchivedRecord));

SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT EncounterCPTKey
      FROM {{ params.param_psc_core_dataset_name }}.ECW_FactEncounterCPT
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_FactEncounterCPT');
ELSE
  COMMIT TRANSACTION;
END IF;
