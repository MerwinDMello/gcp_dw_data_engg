...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefUser AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefUser AS source
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT UserKey
      FROM {{ params.param_psc_core_dataset_name }}.ECW_RefUser
      GROUP BY UserKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefUser');
ELSE
  COMMIT TRANSACTION;
END IF;
