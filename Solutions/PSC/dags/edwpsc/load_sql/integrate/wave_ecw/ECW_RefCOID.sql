...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefCOID AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefCOID AS source
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT Coid
      FROM {{ params.param_psc_core_dataset_name }}.ECW_RefCOID
      GROUP BY Coid
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefCOID');
ELSE
  COMMIT TRANSACTION;
END IF;
