...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefAdjustmentCategory AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefAdjustmentCategory AS source
...
SET DUP_COUNT = (
  ...
  FROM (
      ...
      FROM {{ params.param_psc_core_dataset_name }}.ECW_RefAdjustmentCategory
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefAdjustmentCategory');
ELSE
  COMMIT TRANSACTION;
END IF;
