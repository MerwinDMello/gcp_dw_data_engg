...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefPaymentBatch AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefPaymentBatch AS source
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT BatchKey
      FROM {{ params.param_psc_core_dataset_name }}.ECW_RefPaymentBatch
      GROUP BY BatchKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefPaymentBatch');
ELSE
  COMMIT TRANSACTION;
END IF;
