...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefPatientInsurance AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefPatientInsurance AS source
ON target.PatientInsuranceKey = source.PatientInsuranceKey AND target.RegionKey = source.RegionKey
WHEN MATCHED THEN
  UPDATE SET
...
SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT PatientInsuranceKey, RegionKey
      FROM {{ params.param_psc_core_dataset_name }}.ECW_RefPatientInsurance
      GROUP BY PatientInsuranceKey, RegionKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefPatientInsurance');
ELSE
  COMMIT TRANSACTION;
END IF;
