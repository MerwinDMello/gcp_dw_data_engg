...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefClaimStatus_History AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefClaimStatus_History AS source
ON target.ClaimStatusKey = source.ClaimStatusKey AND target.SysStartTime = source.SysStartTime AND target.SysEndTime = source.SysEndTime
WHEN MATCHED THEN
  UPDATE SET
...
SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT ClaimStatusKey, SysStartTime, SysEndTime
      FROM {{ params.param_psc_core_dataset_name }}.ECW_RefClaimStatus_History
      GROUP BY ClaimStatusKey, SysStartTime, SysEndTime
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefClaimStatus_History');
ELSE
  COMMIT TRANSACTION;
END IF;
