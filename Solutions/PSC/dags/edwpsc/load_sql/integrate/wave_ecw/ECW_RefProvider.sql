...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefProvider AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefProvider AS source
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT ProviderKey
      FROM {{ params.param_psc_core_dataset_name }}.ECW_RefProvider
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefProvider');
ELSE
  COMMIT TRANSACTION;
END IF;
