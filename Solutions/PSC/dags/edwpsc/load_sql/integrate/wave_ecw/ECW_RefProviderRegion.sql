...
MERGE INTO {{ params.param_psc_core_dataset_name }}.ECW_RefProviderRegion AS target
USING {{ params.param_psc_stage_dataset_name }}.ECW_RefProviderRegion AS source
ON target.ProviderRegionKey = source.ProviderRegionKey
WHEN MATCHED THEN
  UPDATE SET
  target.ProviderRegionKey = source.ProviderRegionKey,
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.ECW_RefProviderRegion');
ELSE
  COMMIT TRANSACTION;
END IF;
