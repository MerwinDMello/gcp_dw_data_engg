...
MERGE INTO {{ params.param_psc_core_dataset_name }}.vw_CCU_FactPVInventoryProductivity AS target
...
SET DUP_COUNT = (
  ...
  FROM (
      ...
      FROM {{ params.param_psc_core_dataset_name }}.vw_CCU_FactPVInventoryProductivity
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.vw_CCU_FactPVInventoryProductivity');
ELSE
  COMMIT TRANSACTION;
END IF;
