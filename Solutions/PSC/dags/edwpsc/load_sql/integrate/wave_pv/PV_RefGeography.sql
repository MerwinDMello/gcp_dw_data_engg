...
MERGE INTO {{ params.param_psc_core_dataset_name }}.PV_RefGeography AS target
USING {{ params.param_psc_stage_dataset_name }}.PV_RefGeography AS source
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT GeographyKey
      FROM {{ params.param_psc_core_dataset_name }}.PV_RefGeography
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.PV_RefGeography');
ELSE
  COMMIT TRANSACTION;
END IF;
