...
MERGE INTO {{ params.param_psc_core_dataset_name }}.PV_RefFacility AS target
USING {{ params.param_psc_stage_dataset_name }}.PV_RefFacility AS source
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT FacilityKey
      FROM {{ params.param_psc_core_dataset_name }}.PV_RefFacility
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.PV_RefFacility');
ELSE
  COMMIT TRANSACTION;
END IF;
