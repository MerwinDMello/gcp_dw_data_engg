...
MERGE INTO {{ params.param_psc_core_dataset_name }}.PV_RefPatient AS target
USING {{ params.param_psc_stage_dataset_name }}.PV_RefPatient AS source
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT PatientKey
      FROM {{ params.param_psc_core_dataset_name }}.PV_RefPatient
      GROUP BY PatientKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.PV_RefPatient');
ELSE
  COMMIT TRANSACTION;
END IF;
