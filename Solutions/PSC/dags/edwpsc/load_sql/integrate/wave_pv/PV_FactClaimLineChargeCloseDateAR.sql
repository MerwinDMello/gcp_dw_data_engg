...
MERGE INTO {{ params.param_psc_core_dataset_name }}.PV_FactClaimLineChargeCloseDateAR AS target
USING {{ params.param_psc_stage_dataset_name }}.PV_FactClaimLineChargeCloseDateAR AS source
ON target.ClaimLineChargeKey = source.ClaimLineChargeKey AND target.ClosingDateKey = source.ClosingDateKey AND target.ARHeaderPK = source.ARHeaderPK
WHEN MATCHED THEN
  UPDATE SET
...
SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT ClaimLineChargeKey, ClosingDateKey, ARHeaderPK
      FROM {{ params.param_psc_core_dataset_name }}.PV_FactClaimLineChargeCloseDateAR
      GROUP BY ClaimLineChargeKey, ClosingDateKey, ARHeaderPK
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.PV_FactClaimLineChargeCloseDateAR');
ELSE
  COMMIT TRANSACTION;
END IF;
