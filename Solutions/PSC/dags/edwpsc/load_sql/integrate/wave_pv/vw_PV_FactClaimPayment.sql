...
MERGE INTO {{ params.param_psc_core_dataset_name }}.vw_PV_FactClaimPayment AS target
...
SET DUP_COUNT = (
  ...
  FROM (
      ...
      FROM {{ params.param_psc_core_dataset_name }}.vw_PV_FactClaimPayment
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.vw_PV_FactClaimPayment');
ELSE
  COMMIT TRANSACTION;
END IF;
