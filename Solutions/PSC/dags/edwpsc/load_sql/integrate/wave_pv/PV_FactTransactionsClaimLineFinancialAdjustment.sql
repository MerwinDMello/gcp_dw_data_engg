...
MERGE INTO {{ params.param_psc_core_dataset_name }}.PV_FactTransactionsClaimLineFinancialAdjustment AS target
USING {{ params.param_psc_stage_dataset_name }}.PV_FactTransactionsClaimLineFinancialAdjustment AS source
ON target.TransctionsClaimLineFinancialAdjustmentKey = source.TransctionsClaimLineFinancialAdjustmentKey
WHEN MATCHED THEN
  UPDATE SET
  target.TransctionsClaimLineFinancialAdjustmentKey = source.TransctionsClaimLineFinancialAdjustmentKey,
...
WHEN NOT MATCHED THEN
  INSERT (TransctionsClaimLineFinancialAdjustmentKey, ClaimKey, ClaimNumber, RegionKey, Coid, CoidConfigurationKey, ServicingProviderKey, ClaimPayer1IplanKey, FacilityKey, ClaimLineFinancialAdjustmentsKey, TransactionType, TransactionFlag, TransactionAmt, TransactionDateKey, TransactionTime, TransactionClosingDateKey, TransactionByUserKey, TrRefID, PracticeKey, PracticeName, SourceAPrimaryKeyValue, SourceARecordLastUpdated, DWLastUpdateDateTime, SourceSystemCode, InsertedBy, InsertedDTM, ModifiedBy, ModifiedDTM, PostedDateKey, PaymentNumber, ClaimLineChargesKey, AdjustmentCodeKey, AdjustmentCategoryKey, FirstDenialCategory)
  VALUES (source.TransctionsClaimLineFinancialAdjustmentKey, source.ClaimKey, source.ClaimNumber, source.RegionKey, TRIM(source.Coid), source.CoidConfigurationKey, source.ServicingProviderKey, source.ClaimPayer1IplanKey, source.FacilityKey, source.ClaimLineFinancialAdjustmentsKey, TRIM(source.TransactionType), TRIM(source.TransactionFlag), source.TransactionAmt, source.TransactionDateKey, source.TransactionTime, source.TransactionClosingDateKey, source.TransactionByUserKey, TRIM(source.TrRefID), source.PracticeKey, TRIM(source.PracticeName), TRIM(source.SourceAPrimaryKeyValue), source.SourceARecordLastUpdated, source.DWLastUpdateDateTime, TRIM(source.SourceSystemCode), TRIM(source.InsertedBy), source.InsertedDTM, TRIM(source.ModifiedBy), source.ModifiedDTM, source.PostedDateKey, source.PaymentNumber, source.ClaimLineChargesKey, source.AdjustmentCodeKey, source.AdjustmentCategoryKey, TRIM(source.FirstDenialCategory));

SET DUP_COUNT = (
  SELECT COUNT(*)
  FROM (
      SELECT TransctionsClaimLineFinancialAdjustmentKey
      FROM {{ params.param_psc_core_dataset_name }}.PV_FactTransactionsClaimLineFinancialAdjustment
      GROUP BY TransctionsClaimLineFinancialAdjustmentKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.PV_FactTransactionsClaimLineFinancialAdjustment');
ELSE
  COMMIT TRANSACTION;
END IF;
