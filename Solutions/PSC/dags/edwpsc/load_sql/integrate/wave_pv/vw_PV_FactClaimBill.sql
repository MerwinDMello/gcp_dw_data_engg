...
MERGE INTO {{ params.param_psc_core_dataset_name }}.vw_PV_FactClaimBill AS target
...
SET DUP_COUNT = (
  ...
  FROM (
      SELECT BillKey
      FROM {{ params.param_psc_core_dataset_name }}.vw_PV_FactClaimBill
      GROUP BY BillKey
      HAVING COUNT(*) > 1
  )
);
...
IF DUP_COUNT <> 0 THEN
  ...
  RAISE USING MESSAGE = CONCAT('Duplicates are not allowed in the table {{ params.param_psc_core_dataset_name }}.vw_PV_FactClaimBill');
ELSE
  COMMIT TRANSACTION;
END IF;
