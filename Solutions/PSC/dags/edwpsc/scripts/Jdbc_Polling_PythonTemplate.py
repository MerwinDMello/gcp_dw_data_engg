...
def qualify_done_file_pattern(polling_config_item, wave_group, server_no):
    done_file_pattern = str(polling_config_item['done_file'])
    logging.info("Done File Pattern : {}".format(done_file_pattern))
            
    pattern_type = str(polling_config_item['mask_type']).lower()
    ...
    pattern_mask_code = str(polling_config_item['mask_code']).upper()
    ...
    formatted_done_file_pattern = formatted_done_file_pattern.replace('WAVE_GROUP', wave_group)
    formatted_done_file_pattern = formatted_done_file_pattern.replace('SERVER_NO', server_no)
...
class jdbctobq(beam.DoFn):
    ...
    def replacevarwithliterals(self, src_table_stmt, src_server_name, src_db_name, process_status, wave_group, curr_date):
        ...
        if wave_group:
            src_table_stmt = src_table_stmt.replace('v_wave_group', wave_group)
    ...
    def readjdbcwritebqtable(self, bqproject_id):
        ...
        try:
            ...
            creds = json.loads(access_secret(config['env']['v_pwd_secrets_url'] + srcsys_config['v_secret_name']))

            logging.info(creds)
            username = creds["user"]
            passwd = creds["password"]
            src_server_name = config['env']['v_server01']
            src_db_name = config['env']['v_polling_database']
            ...
            conn = jaydebeapi.connect(jdbc_class_name, jdbc_url, {'user': username, 'password': passwd}, jdbc_lib_path + jdbc_jar, jdbc_lib_path)

            curr_date = str(pendulum.now(current_timezone).strftime("%Y-%m-%d"))

            src_table_stmt = src_db_select_sql

            src_table_stmt = self.replacevarwithliterals(src_table_stmt, src_server_name, src_db_name, process_status, wave_group, curr_date)
            ...
            match polling_type:
                case "preprocess":
                    logging.info(f"Wave Group {wave_group} expects status as {process_status} for polling type {polling_type}")
                    for attempt in range(1, attempts):
                        ...
                        if source_query_df.empty:
                            ...
                            for index, row in source_query_df.iterrows():
                                if process_status.lower() == row['status'].strip().lower():
                                    logging.info("Wave Group Status matched")
                                    polling_record_found = True
                                    break
                                else:
                                    logging.info("Wave Group Status did not matched")
                    ...
                    if polling_record_found:
                        ...
                        server_no = server_host_no
                        done_file_pattern, formatted_done_file_pattern = qualify_done_file_pattern(polling_config_item, wave_group, server_no)
                ...
                #     if source_query_df.empty:
                #         src_table_stmt = polling_config_item['poll_insert_sql']
                #         src_table_stmt = self.replacevarwithliterals(src_table_stmt, src_server_name, src_db_name, process_status, wave_group, curr_date)
                #         logging.info(f"Wave Group {wave_group} inserts status as {process_status} for polling type {polling_type}")
                #         logging.info(f"SQL Statement {src_table_stmt}")
                #         crsr = conn.cursor()
                #         crsr.execute(src_table_stmt)
                #         conn.commit()
                #         logging.info(f"Wave Group {wave_group} inserted the status as {process_status} for polling type {polling_type}")
                #     else:
                ...
                #         src_table_stmt = polling_config_item['poll_update_sql']
                #         src_table_stmt = self.replacevarwithliterals(src_table_stmt, src_server_name, src_db_name, process_status, wave_group, curr_date)
                #         logging.info(f"Wave Group {wave_group} updates status as {process_status} for polling type {polling_type}")
                #         logging.info(f"SQL Statement {src_table_stmt}")
                #         crsr = conn.cursor()
                #         crsr.execute(src_table_stmt)
                #         conn.commit()
                #         logging.info(f"Wave Group {wave_group} updated the status as {process_status} for polling type {polling_type}")
...
def main(sourcesysnm):
    ...
    jobname = f"{sourcesysnm[:3]}-{polling_type}poll-{wave_group.lower().replace('_', '-')}-{pendulum.now(current_timezone).strftime('%Y%m%d%H%M%S')}-{randomstring}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wave_group", required=True,
                        help=("Source Wave Group"))
    parser.add_argument("--polling_type", required=True,
                        help=("Source Polling Type"))
    parser.add_argument("--server_host_no", required=True,
                        help=("Server Host No"))
    ...
    global wave_group
    wave_group = args.wave_group

    global polling_type
    polling_type = args.polling_type.lower()

    global server_host_no
    server_host_no = args.server_host_no

    global sourcesysnm
    sourcesysnm = srcsys_config['v_sourcesysnm']

    polling_config_item = next(filter(lambda item: item['polling_type'].lower() == polling_type, config['env']['v_polling']),None)

    logging.info(polling_config_item)

    global src_db_select_sql
    src_db_select_sql = polling_config_item['poll_select_sql']
