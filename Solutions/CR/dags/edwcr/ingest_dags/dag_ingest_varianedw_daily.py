...
server_db_config = cu.call_config_yaml(f"{src_system}_server_schema.yaml", f"{src_system}_server_schema")
...
def create_dag(dag_id, schedule, start_date, source_system, db_type, frequency, tags, done_file, tblist, truncatetablelist, has_sensor, sensor_list,trigger_dag_ids):
    ...
    with dag:
                ...
                for list in tblist:
                    with TaskGroup(group_id=f'TG-df_{list}') as tablegroup:
                        ...
                        run_dataflow_job = [BashOperator(
                            ...
                            bash_command=f"sleep $(shuf -i 1-300 -n 1) ; python /home/airflow/gcs/dags/{lob}/scripts/{srcsys_config['v_jdbcbqpytemplate']}"
                            f" --src_sys_config_file={source_system}_ingest_config.yaml --src_tbl_list={list} --src_server_name={server_name}"
                        ...
                        wait_for_python_job_async_done = DataflowJobStatusSensor(
                            ...
                            job_id=f"{{{{ task_instance.xcom_pull(task_ids= 'run_dataflow_jobs_{server_name_formatted}.TG-df_{list}.run_{source_system}_{list}')}}}}",
        ...
        if trigger_dag_ids:
            if (done_file != 'NONE'):
                if has_sensor == 'Yes':
                    start_job >> file_sensor  >> v_from_date_task >> sensor_grp >> truncate_grp >> dataflow_group >> delete_old_done_files>> trigger_dag_grp >> end_job
                else:
                    start_job >> file_sensor  >> v_from_date_task >> truncate_grp >> dataflow_group >> delete_old_done_files>> trigger_dag_grp >> end_job
            else:
                if has_sensor == 'Yes':
                    start_job >> v_from_date_task >> sensor_grp >> truncate_grp >> dataflow_group>> trigger_dag_grp >> end_job
                else:
                    start_job >> v_from_date_task >> truncate_grp >> dataflow_group>> trigger_dag_grp >> end_job
        else:
            if (done_file != 'NONE'):
                if has_sensor == 'Yes':
                    start_job >> file_sensor  >> v_from_date_task >> sensor_grp >> truncate_grp >> dataflow_group >> delete_old_done_files >> end_job
                else:
                    start_job >> file_sensor  >> v_from_date_task >> truncate_grp >> dataflow_group >> delete_old_done_files >> end_job
            else:
                if has_sensor == 'Yes':
                    start_job >> v_from_date_task >> sensor_grp >> truncate_grp >> dataflow_group >> end_job
                else:
...
for schedule in srcsys_config['schedule']:
    ...
    dag_id_suffix = str(schedule["dag_id_suffix"]).zfill(2)
    ...
    if frequency == "None":
        ...
        dag_id = f'dag_ingest_{source_system}_{db_type}_{frequency}_{dag_id_suffix}'
