...
WHERE cn_patient.nav_patient_id NOT IN
    (SELECT cn_patient_stg.nav_patient_id
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient AS mt USING
  (SELECT DISTINCT stg.nav_patient_id,
   ...
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.cn_physician_detail AS pd1 ON upper(rtrim(coalesce(trim(stg.gynecologist), 'X'))) = upper(rtrim(coalesce(trim(pd1.physician_name), 'X')))
   AND upper(rtrim(coalesce(trim(stg.gynecologistphone), 'X'))) = upper(rtrim(coalesce(trim(pd1.physician_phone_num), 'X')))
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.cn_physician_detail AS pd2 ON upper(rtrim(coalesce(trim(stg.primarycarephysician), 'XX'))) = upper(rtrim(coalesce(trim(pd2.physician_name), 'XX')))
 ...
 cn_patient.nav_patient_id
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient) QUALIFY row_number() OVER (PARTITION BY stg.nav_patient_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient');
