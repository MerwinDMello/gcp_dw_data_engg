...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.stg_sc_modalities_wrk;
...
INSERT INTO {{ params.param_cr_stage_dataset_name }}.stg_sc_modalities_wrk (treatment_type_sk, treatment_category_desc, treatment_type_desc, source_system_code, dw_last_update_date_time)
...
FROM {{ params.param_cr_stage_dataset_name }}.stg_sc_modalities;
...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.ref_rad_onc_treatment_type;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_rad_onc_treatment_type AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                     ...
                                     'R' AS source_system_code,
                                     datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
   FROM {{ params.param_cr_stage_dataset_name }}.stg_sc_modalities_wrk AS sc
  ...
  (SELECT count(*)
   FROM
     (SELECT treatment_type_sk
      FROM {{ params.param_cr_core_dataset_name }}.ref_rad_onc_treatment_type
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_rad_onc_treatment_type');
