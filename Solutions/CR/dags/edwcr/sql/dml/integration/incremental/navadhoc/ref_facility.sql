...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_facility AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(trim(type_stg.facility_name))) +
     (SELECT coalesce(max(ref_facility.facility_id), 0) AS id1
      FROM {{ params.param_cr_core_dataset_name }}.ref_facility) AS facility_id,
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_facility');
