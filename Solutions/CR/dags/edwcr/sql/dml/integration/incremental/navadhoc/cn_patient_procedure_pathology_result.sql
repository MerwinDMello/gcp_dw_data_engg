...
WHERE (upper(cn_patient_procedure_pathology_result.hashbite_ssk),
    ...
    (SELECT AS STRUCT upper(cn_patient_procedure_pathology_result_stg.hashbite_ssk) AS hashbite_ssk,
     ...
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_procedure_pathology_result_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_procedure_pathology_result AS mt USING
  (SELECT DISTINCT row_number() OVER (
     ...
     (SELECT coalesce(max(cn_patient_procedure_pathology_result.cn_patient_proc_pathology_result_sid), 0) AS id1
      FROM {{ params.param_cr_core_dataset_name }}.cn_patient_procedure_pathology_result) AS cn_patient_proc_pathology_result_sid,
                                     ...
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(stg.oncotype_diagnosis_score_num) AS INT64) AS oncotype_diagnosis_score_num,
   ...
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_result AS rre ON upper(trim(stg.nav_result_id)) = upper(trim(rre.nav_result_desc))
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_result AS rre1 ON upper(trim(stg.margin_result_id)) = upper(trim(rre1.nav_result_desc))
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_result AS rre2 ON upper(trim(stg.oncotype_diagnosis_result_id)) = upper(trim(rre2.nav_result_desc))
   WHERE (upper(stg.hashbite_ssk),
       ...
       (SELECT AS STRUCT upper(cn_patient_procedure_pathology_result.hashbite_ssk) AS hashbite_ssk,
        ...
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_procedure_pathology_result) ) AS ms ON mt.cn_patient_proc_pathology_result_sid = ms.cn_patient_proc_pathology_result_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_procedure_pathology_result');
