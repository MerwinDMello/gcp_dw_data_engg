...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_site_location AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(trim(src.site_location_desc))) +
     (SELECT coalesce(max(ref_site_location.site_location_id), 0) AS id1
      FROM {{ params.param_cr_core_dataset_name }}.ref_site_location) AS site_location_id,
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_site_location');
