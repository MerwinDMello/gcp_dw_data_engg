...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cr_patient_treatment;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cr_patient_treatment AS mt USING
  (SELECT DISTINCT cr_patient_treatment_wrk.treatment_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_treatment_wrk) AS ms ON mt.treatment_id = ms.treatment_id
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT treatment_id
      FROM {{ params.param_cr_core_dataset_name }}.cr_patient_treatment
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cr_patient_treatment');
