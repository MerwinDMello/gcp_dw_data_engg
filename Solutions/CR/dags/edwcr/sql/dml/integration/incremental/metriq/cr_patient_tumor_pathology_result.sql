...
-- bteq << EOF > $1;;
 ...
 --Job=J_CR_PATIENT_TUMOR_PATHOLOGY_RESULT;;
 --' FOR SESSION;;
...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cr_patient_tumor_pathology_result;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cr_patient_tumor_pathology_result AS mt USING
                   ...
                   coalesce(lkp.master_lookup_sid,
                              (SELECT rlc.master_lookup_sid
                               ...
                               WHERE upper(rtrim(rln.lookup_name)) = 'NODES EXAMINED'
                                 AND upper(rtrim(rlc.lookup_code)) = '-99' )) AS nodes_examined_id,
                   coalesce(lkp1.master_lookup_sid,
                              (SELECT rlc1.master_lookup_sid
                               ...
                               WHERE upper(rtrim(rln1.lookup_name)) = 'NODES POSITIVE'
                                 AND upper(rtrim(rlc1.lookup_code)) = '-99' )) AS positive_node_id,
                   'M' AS source_system_code,
                   datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
   FROM {{ params.param_cr_stage_dataset_name }}.cr_pat_tumor_pathology_result_stg AS stg
   LEFT OUTER JOIN
     (SELECT rlc.lookup_code,
             rlc.master_lookup_sid
      FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS rln
      INNER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
      WHERE upper(rtrim(rln.lookup_name)) = 'NODES EXAMINED' ) AS lkp ON upper(rtrim(stg.nodesexamined)) = upper(rtrim(lkp.lookup_code))
   LEFT OUTER JOIN
     (SELECT rlc.lookup_code,
             rlc.master_lookup_sid
      FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS rln
      INNER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS rlc ON rlc.lookup_id = rln.lookup_sid
      WHERE upper(rtrim(rln.lookup_name)) = 'NODES POSITIVE' ) AS lkp1 ON upper(rtrim(stg.nodespositive)) = upper(rtrim(lkp1.lookup_code))) AS ms ON mt.cr_patient_id = ms.cr_patient_id
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT tumor_id,
             cr_patient_id
      FROM {{ params.param_cr_core_dataset_name }}.cr_patient_tumor_pathology_result
      GROUP BY tumor_id,
               cr_patient_id
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cr_patient_tumor_pathology_result');
