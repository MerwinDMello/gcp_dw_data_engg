...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_contact_method AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(trim(type_stg.contact_method_desc))) +
     (SELECT coalesce(max(ref_contact_method.contact_method_id), 0) AS id1
      FROM {{ params.param_cr_core_dataset_name }}.ref_contact_method) AS contact_method_id,
   ...
   WHERE type_stg.contact_method_desc IS NOT NULL
     AND upper(trim(type_stg.contact_method_desc)) <> '' ) AS ms ON mt.contact_method_id = ms.contact_method_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_contact_method');
