...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.ref_rad_onc_toxicity_component;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_rad_onc_toxicity_component AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM
     (SELECT DISTINCT stg_factpatienttoxicity.toxicitycomponentname AS toxicity_component_name,
      ...
      FROM {{ params.param_cr_stage_dataset_name }}.stg_factpatienttoxicity) AS src
  ...
  (SELECT count(*)
   FROM
     (SELECT toxicity_component_sk
      FROM {{ params.param_cr_core_dataset_name }}.ref_rad_onc_toxicity_component
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_rad_onc_toxicity_component');
