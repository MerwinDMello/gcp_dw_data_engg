...
WHERE upper(cn_patient_imaging.hashbite_ssk) NOT IN
    (SELECT upper(cn_patient_imaging_stg.hashbite_ssk) AS hashbite_ssk
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_imaging_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_imaging AS mt USING
 ...
 datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
   ...
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_imaging.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_base_views_dataset_name }}.cn_patient_imaging) ) AS ms ON mt.cn_patient_imaging_sid = ms.cn_patient_imaging_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_imaging');
