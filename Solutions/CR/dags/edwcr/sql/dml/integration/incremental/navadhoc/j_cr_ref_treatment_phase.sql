...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_treatment_phase AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(ref_treatment_phase)) + coalesce(
                                                                                        (SELECT max(coalesce(a.treatment_phase_id, 0)) AS max_key
                                                                                         FROM {{ params.param_cr_base_views_dataset_name }}.ref_treatment_phase AS a), 0) AS treatment_phase_id,
   ...
   WHERE NOT EXISTS
       (SELECT 1
        FROM {{ params.param_cr_base_views_dataset_name }}.ref_treatment_phase AS rcdc
  ...
  (SELECT count(*)
   FROM
     (SELECT treatment_phase_id
      FROM {{ params.param_cr_core_dataset_name }}.ref_treatment_phase
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_treatment_phase');
