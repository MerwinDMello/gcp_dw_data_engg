...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.cr_system_productivity_log_wrk;
...
INSERT INTO {{ params.param_cr_stage_dataset_name }}.cr_system_productivity_log_wrk (system_productivity_log_id, cr_patient_id, tumor_id, system_user_id_code, system_change_status_date, source_system_code, dw_last_update_date_time)
...
FROM
  (SELECT stg.system_productivity_log_id,
          ...
          datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
   FROM {{ params.param_cr_stage_dataset_name }}.cr_system_productivity_log_stg AS stg) AS x
LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.sys_calendar AS b ON (CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(substr(x.system_change_status_date, 1, 4)) AS FLOAT64) - 1900) * 10000 + CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(substr(x.system_change_status_date, 5, 2)) AS FLOAT64) * 100 + CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(substr(x.system_change_status_date, 7, 2)) AS FLOAT64) = (extract(YEAR
