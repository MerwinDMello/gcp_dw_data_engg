...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_brca_screening_assessment AS mt USING
  (SELECT DISTINCT cn_patient_brca_screening_assessment_stg.brca_screening_assessment_sid,
   ...
   WHERE upper(cn_patient_brca_screening_assessment_stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_brca_screening_assessment.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_brca_screening_assessment) ) AS ms ON mt.brca_screening_assessment_sid = ms.brca_screening_assessment_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_brca_screening_assessment');
