...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_phone_num
WHERE cn_patient_phone_num.nav_patient_id NOT IN
    (SELECT cn_patient_phone_num_stg.nav_patient_id
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_phone_num_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_phone_num AS mt USING
  (SELECT DISTINCT cn_patient_phone_num_stg.nav_patient_id,
   ...
   WHERE cn_patient_phone_num_stg.nav_patient_id NOT IN
       (SELECT cn_patient_phone_num.nav_patient_id
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_phone_num) ) AS ms ON mt.nav_patient_id = ms.nav_patient_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_phone_num');
