...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.ref_lookup_code;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS mt USING
   ...
   FROM
     (SELECT CASE
      ...
      GROUP BY 1,
               ...
               upper(TRIM(ref_lookup_code_stg.lookup_desc)),
  ...
  (SELECT count(*)
   FROM
     (SELECT master_lookup_sid
      FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_code
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_lookup_code');
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS mt USING
  (SELECT DISTINCT row_number() OVER (
     ...
     (SELECT coalesce(max(ref_lookup_code.master_lookup_sid), 0)
      FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_code) AS master_lookup_sid,
                                     41 AS lookup_id,
  ...
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM
     (SELECT 33 AS lookup_id,
      ...
      FROM {{ params.param_cr_stage_dataset_name }}.ref_lookup_groups_stg AS lg
      INNER JOIN {{ params.param_cr_stage_dataset_name }}.ref_lookup_code_stg AS lc ON lg.lookup_id = lc.lookup_id
  ...
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM
     (SELECT DISTINCT ref_lookup_name.lookup_sid
      FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_name) AS stg
   INNER JOIN
  ...
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM
     (SELECT DISTINCT 51 AS lookup_id,
                      ...
                      END AS lookup_desc
      FROM {{ params.param_cr_stage_dataset_name }}.ref_lookup_code_stg AS t1
