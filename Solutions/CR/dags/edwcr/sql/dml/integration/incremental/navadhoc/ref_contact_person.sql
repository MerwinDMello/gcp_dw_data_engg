...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_contact_person AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(trim(type_stg.contact_person_desc))) +
     (SELECT coalesce(max(ref_contact_person.contact_person_id), 0) AS id1
      FROM {{ params.param_cr_core_dataset_name }}.ref_contact_person) AS contact_person_id,
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_contact_person');
