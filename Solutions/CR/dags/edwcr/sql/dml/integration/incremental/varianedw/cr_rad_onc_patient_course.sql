...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.rad_onc_patient_course;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.rad_onc_patient_course AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                     ...
                                     ROUND(CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dp.dosedelivered) AS NUMERIC), 10, 'ROUND_HALF_EVEN') AS dose_delivered_amt,
   ...
   INNER JOIN {{ params.param_cr_core_dataset_name }}.ref_rad_onc_site AS rs ON rs.source_site_id = dp.dimsiteid
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.rad_onc_patient AS ra ON dp.dimpatientid = ra.source_patient_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.rad_onc_patient_course');
