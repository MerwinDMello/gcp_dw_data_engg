...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_abstraction_measure AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(src.abstraction_field)) +
     (SELECT coalesce(max(ref_abstraction_measure.abstraction_measure_sk), 0)
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_abstraction_measure) AS abstraction_measure_sk,
   ...
   FROM
     (SELECT DISTINCT cancer_abstraction_values_stg.abstraction_field
      FROM {{ params.param_cr_auth_base_views_dataset_name }}.cancer_abstraction_values_stg) AS src
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_abstraction_measure AS tgt ON upper(trim(tgt.abstraction_measure_name)) = upper(trim(src.abstraction_field))
  ...
  (SELECT count(*)
   FROM
     (SELECT abstraction_measure_sk
      FROM {{ params.param_cr_core_dataset_name }}.ref_abstraction_measure
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_abstraction_measure');
