...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.cr_patient_address_wrk;
...
INSERT INTO {{ params.param_cr_stage_dataset_name }}.cr_patient_address_wrk (cr_patient_id, state_id, address_line_1_text, address_line_2_text, city_name, zip_code, source_system_code, dw_last_update_date_time)
SELECT src.cr_patient_id,
       coalesce(rlc.master_lookup_sid,
                  (SELECT rlc_0.master_lookup_sid
                   FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS rlc_0
                   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS nm ON rlc_0.lookup_id = nm.lookup_sid
                   WHERE upper(rtrim(nm.lookup_name)) = 'STATE'
                     AND upper(rtrim(rlc_0.lookup_code)) = '-99' )) AS state_id,
...
FROM
  (SELECT stg.cr_patient_id AS cr_patient_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_address_stg AS stg) AS src
LEFT OUTER JOIN
  (SELECT lkp.lookup_code,
   ...
   WHERE upper(rtrim(nm.lookup_name)) = 'STATE' ) AS rlc ON upper(rtrim(src.state_id)) = upper(rtrim(rlc.lookup_code));
