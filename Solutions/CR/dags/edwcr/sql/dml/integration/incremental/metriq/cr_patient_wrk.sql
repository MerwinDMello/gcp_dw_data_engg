...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.cr_patient_wrk;
...
MERGE INTO {{ params.param_cr_stage_dataset_name }}.cr_patient_wrk AS mt USING
  (SELECT DISTINCT CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(x.patientid) AS INT64) AS cr_patient_id,
                   coalesce(rlc.master_lookup_sid,
                              (SELECT rcl.master_lookup_sid
                               FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS rcl
                               LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS nm ON rcl.lookup_id = nm.lookup_sid
                   ...
                   coalesce(rlc1.master_lookup_sid,
                              (SELECT rcl1.master_lookup_sid
                               FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS rcl1
                               LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS nm1 ON rcl1.lookup_id = nm1.lookup_sid
                   ...
                   coalesce(rlc2.master_lookup_sid,
                              (SELECT rcl2.master_lookup_sid
                               FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS rcl2
                               LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS nm2 ON rcl2.lookup_id = nm2.lookup_sid
 ...
 CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(x.patientsystemid) AS INT64) AS patientsystemid,
 x.coid AS coid,
 'H' AS companycode,
 x.birthdate,
 x.datelastcontact,
 ...
 x.dw_last_update_date_time AS dw_last_update_date_time
   FROM
     (SELECT trim(format('%11d', a.patientid)) AS patientid,
      ...
      FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_stg AS a) AS x
   LEFT OUTER JOIN /*LEFT JOIN
     ...
     (SELECT rln.lookup_sid,
      ...
      INNER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS rlc_0 ON rlc_0.lookup_id = rln.lookup_sid
  ...
  (SELECT count(*)
   FROM
     (SELECT cr_patient_id
      FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_wrk
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_stage_dataset_name }}.cr_patient_wrk');
