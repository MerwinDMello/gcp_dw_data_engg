...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.ref_rad_onc_hospital;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_rad_onc_hospital AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimsiteid) AS INT64),
                                               CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimhospitaldepartmentid) AS INT64)) AS hospital_sk,
                                     ra.address_sk AS hospital_address_sk,
                                     rr.site_sk AS site_sk,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimhospitaldepartmentid) AS INT64) AS source_hospital_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.stg_dimhospitaldepartment AS dhd
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.rad_onc_address AS ra ON upper(trim(dhd.hospitalcompleteaddress)) = upper(trim(ra.full_address_text))
   AND ra.address_line_1_text IS NULL
   AND ra.address_line_2_text IS NULL
   AND ra.address_comment_text IS NULL
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_site AS rr ON rr.source_site_id = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimsiteid) AS INT64)
   LEFT OUTER JOIN
     (SELECT DISTINCT stg_sc_factfacility.hospitalname,
                      stg_sc_factfacility.coid
      FROM {{ params.param_cr_stage_dataset_name }}.stg_sc_factfacility
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT hospital_sk
      FROM {{ params.param_cr_core_dataset_name }}.ref_rad_onc_hospital
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_rad_onc_hospital');
