...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.ref_cr_treatment_type;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_cr_treatment_type AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.ref_cr_treatment_type_stg) AS ms ON mt.treatment_type_id = ms.treatment_type_group_id
  ...
  (SELECT count(*)
   FROM
     (SELECT treatment_type_id
      FROM {{ params.param_cr_core_dataset_name }}.ref_cr_treatment_type
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_cr_treatment_type');
