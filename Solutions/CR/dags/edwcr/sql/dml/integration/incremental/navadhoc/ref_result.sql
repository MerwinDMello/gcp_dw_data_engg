...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_result AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(trim(type_stg.nav_result_desc))) +
     (SELECT coalesce(max(ref_result.nav_result_id), 0) AS id1
      FROM {{ params.param_cr_core_dataset_name }}.ref_result) AS nav_result_id,
     ...
     (SELECT trim(ref_result_stg.nav_result_desc) AS nav_result_desc,
      ...
      WHERE upper(trim(ref_result_stg.nav_result_desc)) NOT IN
          (SELECT upper(trim(ref_result.nav_result_desc))
           FROM {{ params.param_cr_core_dataset_name }}.ref_result) ) AS type_stg) AS ms ON mt.nav_result_id = ms.nav_result_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_result');
