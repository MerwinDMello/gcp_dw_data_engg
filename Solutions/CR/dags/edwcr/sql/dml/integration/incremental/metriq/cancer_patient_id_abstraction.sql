...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_abstraction_wrk0;
...
SELECT NULL AS cancer_abstraction_sk, /* nvl(ROW_NUMBER() OVER ( ORDER BY unique_message_id || ' - ' ||COALESCE(site_and_associated_model_output_score,'' )||COALESCE(second_primary_site,'') ),0)as Cancer_Abstraction_SK ------Added for CES 17570 */ coalesce(o.cancer_patient_id_output_sk, -1) AS cancer_patient_id_output_sk,
     ...
     CAST(REGEXP_EXTRACT(a.patient_account_number, r'[0-9]+') AS INT64) AS pat_acct_num,
     ...
     NULLIF(TRIM(a.abstraction_action_user),'') AS abstraction_action_user_3_4_id,
     substr(NULLIF(TRIM(a.abstraction_action),''), 1, 20) AS abstraction_action_desc,
     NULLIF(TRIM(a.submitted_primary_site_icdo3),'') AS primary_icd_oncology_code,
     substr(NULLIF(TRIM(a.submitted_primary_site),''), 1, 255) AS primary_icd_site_desc,
     substr(NULLIF(TRIM(a.site_and_associated_model_output_score),''), 1, 255) AS primary_icd_site_and_model_score_desc,
     NULLIF(TRIM(a.abstraction_changed_primary_site_icdo3),'') AS changed_primary_icd_oncology_code,
     substr(NULLIF(TRIM(a.abstraction_changed_primary_site),''), 1, 255) AS changed_primary_icd_site_desc,
     NULLIF(TRIM(a.abstraction_topography_icdo3),'') AS topography_icd_oncology_code,
     substr(NULLIF(TRIM(a.abstraction_topography),''), 1, 255) AS topography_icd_site_desc,
     NULLIF(TRIM(a.abstraction_laterality_code),'') AS laterality_icd_oncology_code,
     substr(NULLIF(TRIM(a.abstraction_laterality),''), 1, 255) AS laterality_icd_site_desc,
     NULLIF(TRIM(a.second_primary_site_icdo3),'') AS secondary_icd_oncology_code,
     substr(NULLIF(TRIM(a.second_primary_site),''), 1, 255) AS secondary_icd_site_desc,
     'H' AS source_system_code,
     datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
FROM {{ params.param_cr_auth_base_views_dataset_name }}.cancer_abstraction_stg AS a
LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_output AS o 
ON upper(trim(a.unique_message_id)) = upper(trim(o.message_control_id_text))
LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.clinical_acctkeys AS cact 
ON CAST(REGEXP_EXTRACT(a.patient_account_number, r'[0-9]+') AS INT64) = cact.pat_acct_num
...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_abstraction_wrk1;
...
INSERT INTO {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_abstraction_wrk1 (cancer_abstraction_sk, cancer_patient_id_output_sk, message_control_id_text, coid, company_code, abstraction_action_date_time, patient_dw_id, pat_acct_num, abstraction_report_assigned_date_time, abstraction_date_time, abstraction_action_user_3_4_id, abstraction_action_desc, primary_icd_oncology_code, primary_icd_site_desc, primary_icd_site_and_model_score_desc, changed_primary_icd_oncology_code, changed_primary_icd_site_desc, topography_icd_oncology_code, topography_icd_site_desc, laterality_icd_oncology_code, laterality_icd_site_desc, secondary_icd_oncology_code, secondary_icd_site_desc, source_system_code, dw_last_update_date_time)
 ...
 coalesce(row_number() OVER (
          ORDER BY upper(wr.message_control_id_text), upper(wr.secondary_icd_site_desc), upper(wr.primary_icd_site_and_model_score_desc)) +
            (SELECT coalesce(max(cancer_patient_id_abstraction.cancer_abstraction_sk), 0) AS max_val
             FROM {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_abstraction), 0) AS cancer_abstraction_sk,
...
FROM {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_abstraction_wrk0 AS wr
LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_abstraction AS cr 
ON upper(trim(coalesce(wr.message_control_id_text, '##'))) = upper(trim(coalesce(cr.message_control_id_text, '##')))
...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_abstraction_wrk2;
...
INSERT INTO {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_abstraction_wrk2 (cancer_abstraction_sk, cancer_patient_id_output_sk, message_control_id_text, coid, company_code, abstraction_action_date_time, patient_dw_id, pat_acct_num, abstraction_report_assigned_date_time, abstraction_date_time, abstraction_action_user_3_4_id, abstraction_action_desc, primary_icd_oncology_code, primary_icd_site_desc, primary_icd_site_and_model_score_desc, changed_primary_icd_oncology_code, changed_primary_icd_site_desc, topography_icd_oncology_code, topography_icd_site_desc, laterality_icd_oncology_code, laterality_icd_site_desc, secondary_icd_oncology_code, secondary_icd_site_desc, source_system_code, dw_last_update_date_time)
...
FROM {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_abstraction_wrk1 AS stg 
QUALIFY row_number() OVER (PARTITION BY stg.cancer_abstraction_sk
...
UPDATE {{ params.param_cr_core_dataset_name }}.cancer_patient_id_abstraction
...
FROM {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_abstraction_wrk2 AS src
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cancer_patient_id_abstraction AS mt USING
  (SELECT DISTINCT stg.cancer_abstraction_sk,
                   ...
                   stg.dw_last_update_date_time
   FROM {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_abstraction_wrk2 AS stg
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_abstraction AS tgt 
   ON stg.cancer_abstraction_sk = tgt.cancer_abstraction_sk
  ...
  (SELECT count(*)
   FROM
     (SELECT cancer_abstraction_sk
      FROM {{ params.param_cr_core_dataset_name }}.cancer_patient_id_abstraction
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cancer_patient_id_abstraction');
