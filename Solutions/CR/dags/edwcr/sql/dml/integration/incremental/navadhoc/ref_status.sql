...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_status AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY upper(trim(type_stg.status_desc))) +
     (SELECT coalesce(max(ref_status.status_id), 0) AS id1
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_status) AS status_id,
   ...
   FROM
     (SELECT trim(ref_status_stg.status_desc) AS status_desc,
      ...
      WHERE (upper(trim(ref_status_stg.status_desc)),
          ...
          (SELECT AS STRUCT -- --where trim(Status_Desc) not in (sel trim(Status_Desc) from EDWCR.REF_STATUS )   ---20190528--- New coulumn added as per CES-13414--
 upper(trim(ref_status.status_desc)),
 upper(trim(ref_status.status_type_desc))
           FROM {{ params.param_cr_base_views_dataset_name }}.ref_status) ) AS type_stg) AS ms ON mt.status_id = ms.status_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_status');
