...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_breast_cancer_type AS mt USING
  (SELECT DISTINCT row_number() OVER (
     ...
     (SELECT coalesce(max(ref_breast_cancer_type.breast_cancer_type_id), 0) AS id1
      FROM {{ params.param_cr_core_dataset_name }}.ref_breast_cancer_type) AS breast_cancer_type_id,
     ...
     (SELECT ref_breast_cancer_type_stg.breast_cancer_type_desc,
      ...
      WHERE upper(trim(ref_breast_cancer_type_stg.breast_cancer_type_desc)) NOT IN
          (SELECT upper(trim(ref_breast_cancer_type.breast_cancer_type_desc))
           FROM {{ params.param_cr_core_dataset_name }}.ref_breast_cancer_type) ) AS type_stg) AS ms ON mt.breast_cancer_type_id = ms.breast_cancer_type_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_breast_cancer_type');
