...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_consultation AS mt USING
  (SELECT DISTINCT cn_patient_consultation_stg.cn_patient_consultation_sid,
   ...
   WHERE upper(cn_patient_consultation_stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_consultation.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_consultation) ) AS ms ON mt.cn_patient_consultation_sid = ms.cn_patient_consultation_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_consultation');
