...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cr_patient;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cr_patient AS mt USING
  (SELECT DISTINCT cr_patient_wrk.cr_patient_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_wrk) AS ms ON mt.cr_patient_id = ms.cr_patient_id
  ...
  (SELECT count(*)
   FROM
     (SELECT cr_patient_id
      FROM {{ params.param_cr_core_dataset_name }}.cr_patient
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cr_patient');
