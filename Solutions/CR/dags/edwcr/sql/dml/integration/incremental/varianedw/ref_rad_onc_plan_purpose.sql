...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.ref_rad_onc_plan_purpose;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_rad_onc_plan_purpose AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   FROM
     (SELECT DISTINCT CASE
      ...
      FROM {{ params.param_cr_stage_dataset_name }}.stg_dimplan AS dhd) AS src
  ...
  (SELECT count(*)
   FROM
     (SELECT plan_purpose_sk
      FROM {{ params.param_cr_core_dataset_name }}.ref_rad_onc_plan_purpose
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_rad_onc_plan_purpose');
