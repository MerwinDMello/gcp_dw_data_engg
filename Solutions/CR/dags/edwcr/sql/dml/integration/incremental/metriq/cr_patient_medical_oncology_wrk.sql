...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.cr_patient_medical_oncology_wrk;
...
INSERT INTO {{ params.param_cr_stage_dataset_name }}.cr_patient_medical_oncology_wrk (cycle_id, treatment_id, drug_route_id, drug_dose_unit_id, drug_id, drug_days_given_num_text, drug_frequency_num, total_drug_dose_amt, drug_hospital_id, nsc_id, treatment_start_date, treatment_end_date, cycle_num_text, source_system_code, dw_last_update_date_time)
...
FROM {{ params.param_cr_stage_dataset_name }}.cr_patient_medical_oncology_stg AS stg
LEFT OUTER JOIN
  (SELECT refc.master_lookup_sid,
          refc.lookup_code
   FROM {{ params.param_cr_core_dataset_name }}.ref_lookup_code AS refc
   INNER JOIN {{ params.param_cr_core_dataset_name }}.ref_lookup_name AS refn ON refc.lookup_id = refn.lookup_sid
...
LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_hospital AS refh ON upper(rtrim(refh.hospital_code)) = upper(rtrim(stg.drug_hospital_id))
LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_national_service_center AS rnsc ON upper(rtrim(rnsc.nsc_code)) = upper(rtrim(stg.nsc_id))
