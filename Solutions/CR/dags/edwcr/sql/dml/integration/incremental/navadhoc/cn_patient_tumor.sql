...
WHERE upper(cn_patient_tumor.hashbite_ssk) NOT IN
    (SELECT upper(cn_patient_tumor_stg.hashbite_ssk) AS hashbite_ssk
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_tumor_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_tumor AS mt USING
  (SELECT DISTINCT stg.cn_patient_tumor_sid,
   ...
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_facility AS rf1 ON upper(rtrim(stg.tumorreferralsource)) = upper(rtrim(rf1.facility_name))
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_status AS rs ON upper(rtrim(stg.navigationstatus)) = upper(rtrim(rs.status_desc))
   AND upper(rtrim(rs.status_type_desc)) = 'NAVIGATION'
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.cn_physician_detail AS pd ON upper(rtrim(stg.endtreatmentphysician)) = upper(rtrim(pd.physician_name))
   AND pd.physician_phone_num IS NULL
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_facility AS rf2 ON upper(rtrim(stg.endtreatmentlocation)) = upper(rtrim(rf2.facility_name))
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_tumor.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_tumor) QUALIFY row_number() OVER (PARTITION BY stg.cn_patient_tumor_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_tumor');
