...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_heme_treatment_regimen
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_heme_treatment_regimen AS mt USING
  (SELECT DISTINCT CAST(stg.patienthemediagnosisfactid AS INT64) AS cn_patient_heme_diagnosis_sid,
                   ...
                   'H' AS company_code,
                   parse_date('%Y-%m-%d', substr(stg.plannedstartdate, 1, 10)) AS planned_start_date, --substr(stg.plannedstartdate, 1, 19)
                   parse_date('%Y-%m-%d', substr(stg.actualstartdate, 1, 10)) AS actual_start_date, --substr(stg.plannedstartdate, 1, 19)
                   substr(trim(stg.drugs), 1, 100) AS drug_text,
                   stg.cycles AS cycle_num,
                   CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(stg.cyclelength) AS INT64) AS cycle_length_num,
                   substr(trim(stg.cyclelengthinterval), 1, 20) AS cycle_frequency_text,
                   CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(stg.cyclenumber) AS INT64) AS ordinal_cycle_num,
                   CASE
                   ...
                   parse_date('%Y-%m-%d', substr(stg.treatmentplandoccumentdate, 1, 10)) AS treatment_plan_document_date, --substr(stg.plannedstartdate, 1, 19)
                   CASE
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.patient_heme_treatment_regimen_stg AS stg
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_regimen AS rs ON upper(trim(stg.regimen)) = upper(trim(rs.regimen_name))
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_treatment_phase AS tr ON upper(trim(stg.treatmentphase)) = upper(trim(tr.treatment_phase_desc))
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_pathway_var_reason AS pr ON upper(trim(stg.pathwayvariancereason)) = upper(trim(pr.pathway_var_reason_type_desc))
   ...
   WHERE upper(trim(stg.hbsource)) NOT IN
       (SELECT upper(trim(cn_patient_heme_treatment_regimen.hashbite_ssk))
        FROM {{ params.param_cr_base_views_dataset_name }}.cn_patient_heme_treatment_regimen) ) AS ms ON mt.cn_patient_heme_diagnosis_sid = ms.cn_patient_heme_diagnosis_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_heme_treatment_regimen');
