 ...
 datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
   FROM
     (SELECT DISTINCT cp_0.patient_market_urn_text,
                      ...
                      CASE upper(TRIM(gn.lookup_desc))
                          WHEN 'MALE' THEN 'M'
                          WHEN 'FEMALE' THEN 'F'
                          ELSE 'U'
                      END AS gender_code,
      ...
      INNER JOIN
        (SELECT clinical_facility.facility_mnemonic_cs,
         ...
         FROM {{ params.param_cr_auth_base_views_dataset_name }}.clinical_facility QUALIFY row_number() OVER (PARTITION BY upper(clinical_facility.coid)
                                                                                                              ORDER BY upper(clinical_facility.facility_active_ind) DESC) = 1) AS df ON upper(TRIM(cp_0.coid)) = upper(TRIM(df.coid))
     ...
     (SELECT crio.patient_market_urn,
      ...
      INNER JOIN
        (SELECT clinical_facility.facility_mnemonic_cs,
                ...
                clinical_facility.facility_active_ind
         FROM {{ params.param_cr_auth_base_views_dataset_name }}.clinical_facility QUALIFY row_number() OVER (PARTITION BY upper(clinical_facility.coid)
                                                                                                              ORDER BY upper(clinical_facility.facility_active_ind) DESC) = 1) AS df1 ON upper(TRIM(crio.coid)) = upper(TRIM(df1.coid)) QUALIFY row_number() OVER (PARTITION BY coalesce(concat(crio.patient_market_urn, crio.network_mnemonic_cs), concat(crio.medical_record_num, df1.facility_mnemonic_cs), format('%#20.0f', crio.patient_dw_id))
                                                                                                                                                                                                                                                                     ORDER BY crio.report_assigned_date_time DESC) = 1) AS cpid ON TRIM(coalesce(met.patient_market_urn_text, concat(met.medical_record_num, met.facility_mnemonic_cs))) = TRIM(coalesce(cpid.patient_market_urn, concat(cpid.medical_record_num, cpid.facility_mnemonic_cs)))
     ...
     (SELECT cp_0.nav_patient_id,
      ...
      INNER JOIN
        (SELECT clinical_facility.facility_mnemonic_cs,
                ...
                clinical_facility.facility_active_ind
         FROM {{ params.param_cr_auth_base_views_dataset_name }}.clinical_facility QUALIFY row_number() OVER (PARTITION BY upper(clinical_facility.coid)
                                                                                                              ORDER BY upper(clinical_facility.facility_active_ind) DESC) = 1) AS df2 ON upper(TRIM(cpt.coid)) = upper(TRIM(df2.coid))
      ...
      LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_patient_phone_num AS cpp_0 ON cp_0.nav_patient_id = cpp_0.nav_patient_id QUALIFY row_number() OVER (PARTITION BY coalesce(cpt.patient_market_urn, concat(cpt.medical_record_num, df2.facility_mnemonic_cs), format('%#20.0f', cp_0.nav_patient_id))
                                                                                                                                                                           ORDER BY cp_0.nav_patient_id DESC) = 1) AS inav ON TRIM(coalesce(concat(trim(met.network_mnemonic_cs), met.patient_market_urn_text), concat(met.medical_record_num, met.facility_mnemonic_cs))) = TRIM(coalesce(inav.patient_market_urn, concat(inav.medical_record_num, inav.facility_mnemonic_cs)))
...
WHERE upper(TRIM(cd1.medical_record_num)) = upper(TRIM(cp.medical_record_num))
  AND upper(TRIM(cd1.patient_market_urn_text)) = upper(TRIM(cp.patient_market_urn_text))
  AND upper(TRIM(cd1.network_mnemonic_cs)) = upper(TRIM(cp.network_mnemonic_cs))
