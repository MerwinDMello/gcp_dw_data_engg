...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_imaging_mode AS mt USING
  (SELECT DISTINCT CAST(CAST(row_number() OVER (
                                                ORDER BY upper(trim(type_stg.imaging_mode_desc))) AS FLOAT64) + CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(
                                                                                                                                                                                            (SELECT coalesce(max(ref_imaging_mode.imaging_mode_desc), format('%4d', 0)) AS id1
                                                                                                                                                                                             FROM {{ params.param_cr_core_dataset_name }}.ref_imaging_mode)) AS FLOAT64) AS INT64) AS imaging_mode_id,
   ...
   FROM
     (SELECT ref_imaging_mode_stg.imaging_mode_desc,
      ...
      FROM {{ params.param_cr_stage_dataset_name }}.ref_imaging_mode_stg
      WHERE upper(trim(ref_imaging_mode_stg.imaging_mode_desc)) NOT IN
          (SELECT upper(trim(ref_imaging_mode.imaging_mode_desc))
           FROM {{ params.param_cr_core_dataset_name }}.ref_imaging_mode) ) AS type_stg) AS ms ON mt.imaging_mode_id = ms.imaging_mode_id
  ...
  (SELECT count(*)
   FROM
     (SELECT imaging_mode_id
      FROM {{ params.param_cr_core_dataset_name }}.ref_imaging_mode
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_imaging_mode');
