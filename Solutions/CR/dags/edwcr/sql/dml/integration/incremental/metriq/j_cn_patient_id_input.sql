...
-- DELETE
-- FROM {{ params.param_cr_stage_dataset_name }}.scri_patient_id_history
-- WHERE scri_patient_id_history.msh_msg_control_id IS NULL;
...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_input_wrk;
...
MERGE INTO {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_input_wrk AS mt USING
  (SELECT DISTINCT substr(a.message_control_id_text, 1, 50) AS message_control_id_text,
                   a.patient_type_status_sk,
                   a.coid AS coid,
                   a.company_code AS company_code,
                   a.patient_dw_id,
                   a.pid_pat_account_num AS pat_acct_num,
                   ...
                   a.orig_etl_insert_date_time,
                   a.source_system_code AS source_system_code,
                   a.dw_last_update_date_time
   FROM
     (SELECT TRIM(stg.msh_msg_control_id) AS message_control_id_text,
             coalesce(pt.patient_type_status_sk,
                        (SELECT ref_patient_type_status.patient_type_status_sk
                         FROM {{ params.param_cr_base_views_dataset_name }}.ref_patient_type_status
                         WHERE upper(rtrim(ref_patient_type_status.patient_type_status_code)) = 'UNK' )) AS patient_type_status_sk,
             CASE
             ...
             'H' AS company_code,
             ca.patient_dw_id,
             CAST(REGEXP_EXTRACT(pid_pat_account_num, r'[0-9]+') AS INT64) AS pid_pat_account_num,
             nullif(TRIM(stg.pid_medical_record_num), '') AS pid_medical_record_num,
             nullif(TRIM(stg.pid_medical_record_urn), '') AS pid_medical_record_urn,
             nullif(TRIM(stg.msh_msg_type_message_code), '') AS msh_msg_type_message_code,
             nullif(TRIM(stg.message_flag), '') AS message_flag,
             nullif(TRIM(stg.msh_msg_type_trigger_event), '') AS msh_msg_type_trigger_event,
             CASE
                 WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.txa_origination_date_time), r'^[1-2][0-9]+')) < 8 
                 THEN CAST(NULL AS DATETIME)
                 WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.txa_origination_date_time), r'^[1-2][0-9]+')) >= 8 
                 THEN PARSE_DATETIME('%Y%m%d%H%M%S', RPAD(REGEXP_EXTRACT(stg.txa_origination_date_time, r'^[1-2][0-9]+'),14,"0"))
                 ELSE CAST(NULL AS DATETIME)
             END AS txa_origination_date_time,
             CASE
                 WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.txa_transcription_date_time), r'^[1-2][0-9]+')) < 8 
                 THEN CAST(NULL AS DATETIME)
                 WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.txa_transcription_date_time), r'^[1-2][0-9]+')) >= 8 
                 THEN PARSE_DATETIME('%Y%m%d%H%M%S', RPAD(REGEXP_EXTRACT(stg.txa_transcription_date_time, r'^[1-2][0-9]+'),14,"0"))
                 ELSE CAST(NULL AS DATETIME)
             END AS txa_transcription_date_time,
             CASE
                 WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.message_created_date_time), r'^[1-2][0-9]+')) < 8 
                 THEN CAST(NULL AS DATETIME)
                 WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.message_created_date_time), r'^[1-2][0-9]+')) >= 8 
                 THEN PARSE_DATETIME('%Y%m%d%H%M%S', RPAD(REGEXP_EXTRACT(stg.message_created_date_time, r'^[1-2][0-9]+'),14,"0"))
                 ELSE CAST(NULL AS DATETIME)
             END AS message_created_date_time,
             CASE
                 WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.etl_insert_date_time), r'^[1-2][0-9]+')) < 8 
                 THEN CAST(NULL AS DATETIME)
                 WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.etl_insert_date_time), r'^[1-2][0-9]+')) >= 8 
                 THEN PARSE_DATETIME('%Y%m%d%H%M%S', RPAD(REGEXP_EXTRACT(stg.etl_insert_date_time, r'^[1-2][0-9]+'),14,"0"))
                 ELSE CAST(NULL AS DATETIME)
             END AS etl_insert_date_time,
             stg.etl_insert_date_time as orig_etl_insert_date_time,
             'H' AS source_system_code,
             datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
      FROM {{ params.param_cr_auth_base_views_dataset_name }}.scri_patient_id_history AS stg
      LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.clinical_acctkeys AS ca 
      ON ca.pat_acct_num = CAST(REGEXP_EXTRACT(TRIM(stg.pid_pat_account_num), r'[0-9]+') AS INT64)
      AND TRIM(ca.coid) = LPAD(TRIM(stg.coid),5,"0")
      LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_patient_type_status AS pt 
      ON upper(trim(stg.patient_type_status)) = upper(trim(pt.patient_type_status_code))
      WHERE upper(TRIM(stg.message_flag)) IN('PAT',
                                              'RAD',
                                              'RADN')
        AND stg.msh_msg_control_id IS NOT NULL) AS a QUALIFY row_number() OVER (PARTITION BY upper(a.message_control_id_text)
                                                   ORDER BY a.orig_etl_insert_date_time DESC) = 1) AS ms ON mt.message_control_id_text = ms.message_control_id_text
...
INSERT (message_control_id_text,
        patient_type_status_sk,
        coid,
        company_code,
        patient_dw_id,
        pat_acct_num,
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT message_control_id_text
      FROM {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_input_wrk
      GROUP BY message_control_id_text
      HAVING count(*) > 1));

IF DUP_COUNT <> 0 THEN
ROLLBACK TRANSACTION;

RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_input_wrk');
...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cancer_patient_id_input;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cancer_patient_id_input AS mt USING
  (SELECT DISTINCT cancer_patient_id_input_wrk.message_control_id_text,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_input_wrk) AS ms ON mt.message_control_id_text = ms.message_control_id_text
AND (coalesce(mt.patient_type_status_sk, 0) = coalesce(ms.patient_type_status_sk, 0)
     AND coalesce(mt.patient_type_status_sk, 1) = coalesce(ms.patient_type_status_sk, 1))
AND (upper(coalesce(mt.coid, '0')) = upper(coalesce(ms.coid, '0'))
     AND upper(coalesce(mt.coid, '1')) = upper(coalesce(ms.coid, '1')))
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT message_control_id_text
      FROM {{ params.param_cr_core_dataset_name }}.cancer_patient_id_input
      GROUP BY message_control_id_text
      HAVING count(*) > 1));
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cancer_patient_id_input');
