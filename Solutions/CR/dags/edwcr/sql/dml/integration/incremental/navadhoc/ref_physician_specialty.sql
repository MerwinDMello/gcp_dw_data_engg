...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_physician_specialty AS mt USING
  (SELECT DISTINCT row_number() OVER (
     ...
     (SELECT coalesce(max(ref_physician_specialty.physician_specialty_id), 0) AS id1
      FROM {{ params.param_cr_core_dataset_name }}.ref_physician_specialty) AS physician_specialty_id,
     ...
     (SELECT physician_specialty_desc AS physician_specialty_desc,
      ...
      WHERE upper(trim(physician_specialty_desc)) NOT IN
          (SELECT upper(trim(ref_physician_specialty.physician_specialty_desc))
           FROM {{ params.param_cr_core_dataset_name }}.ref_physician_specialty) ) AS type_stg) AS ms ON mt.physician_specialty_id = ms.physician_specialty_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_physician_specialty');
