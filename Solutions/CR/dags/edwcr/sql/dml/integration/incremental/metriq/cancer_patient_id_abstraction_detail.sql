...
SELECT abstrct.cancer_abstraction_sk AS cancer_abstraction_sk,
       ...
       substr(NULLIF(stg.value_predicted,''), 1, 255) AS predicted_value_text,
       substr(NULLIF(stg.value_submitted,''), 1, 255) AS submitted_value_text,
       substr(NULLIF(stg.value_suggested,''), 1, 255) AS suggested_value_text,
       'H' AS source_system_code,
       datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
FROM {{ params.param_cr_auth_base_views_dataset_name }}.cancer_abstraction_values_stg AS stg
INNER JOIN {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_abstraction AS abstrct 
ON upper(trim(stg.unique_message_id)) = upper(trim(abstrct.message_control_id_text))
AND upper(coalesce(NULLIF(TRIM(stg.second_primary_site),''), '##')) = upper(coalesce(NULLIF(TRIM(abstrct.secondary_icd_site_desc),''), '##'))
AND upper(coalesce(NULLIF(TRIM(stg.site_and_associated_model_output_score),''), '##')) = upper(coalesce(NULLIF(TRIM(abstrct.primary_icd_site_and_model_score_desc),''), '##'))
INNER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_abstraction_measure AS measure 
ON upper(trim(stg.abstraction_field)) = upper(trim(measure.abstraction_measure_name)) 
QUALIFY row_number() OVER (PARTITION BY cancer_abstraction_sk, abstraction_measure_sk
ORDER BY upper(stg.unique_message_id) DESC, predicted_value_text DESC, submitted_value_text DESC, suggested_value_text DESC) = 1;
