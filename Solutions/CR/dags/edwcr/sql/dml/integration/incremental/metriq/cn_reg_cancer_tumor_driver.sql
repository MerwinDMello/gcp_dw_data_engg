 ...
 datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
   FROM -- PT ID TumorTypes
     ...
     (SELECT DISTINCT t4_0.*
      FROM {{ params.param_cr_base_views_dataset_name }}.cancer_patient_id_output AS t1_0
      INNER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_icd_oncology AS t4_0 ON upper(rtrim(t4_0.icd_oncology_code)) = upper(rtrim(t1_0.submitted_primary_icd_oncology_code))
      AND upper(rtrim(t4_0.icd_oncology_code)) NOT IN('C421')) AS t3
   FULL OUTER JOIN -- CR TumorTypes
 {{ params.param_cr_base_views_dataset_name }}.ref_lookup_code AS t1 ON upper(rtrim(substr(t1.lookup_code, 1, 3))) = upper(rtrim(substr(t3.icd_oncology_code, 1, 3)))
   LEFT OUTER JOIN -- Navigation Tumor Module  Lookup
     ...
     (SELECT CASE
             ...
             t1_0.*
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_tumor_type AS t1_0
     ...
     (SELECT CASE
      ...
      FROM
        (SELECT DISTINCT substr(t1_0.icd_oncology_code, 1, 3) AS icd_oncology_code,
         ...
         FROM {{ params.param_cr_base_views_dataset_name }}.ref_icd_oncology AS t1_0) AS t2) AS t7 ON upper(rtrim(substr(t1.lookup_code, 1, 3))) = upper(rtrim(substr(t7.icd_oncology_code, 1, 3)))
...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cancer_tumor_driver;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cancer_tumor_driver AS mt USING
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cancer_tumor_driver');
