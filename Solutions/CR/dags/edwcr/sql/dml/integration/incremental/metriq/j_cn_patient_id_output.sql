...
-- DELETE
-- FROM {{ params.param_cr_stage_dataset_name }}.cdm_synthesys_patientid_results
-- WHERE cdm_synthesys_patientid_results.unique_message_id IS NULL;
...
TRUNCATE TABLE {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_output_wrk;
...
INSERT INTO {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_output_wrk (cancer_patient_id_output_sk, message_control_id_text, coid, company_code, patient_dw_id, pat_acct_num, medical_record_num, patient_market_urn, last_name, first_name, birth_date, gender_code, social_security_num, address_line_1_text, address_line_2_text, city_name, state_code, zip_code, patient_type_code, message_type_code, message_event_type_code, message_flag_code, message_origin_requested_date_time, message_signed_observation_date_time, message_ingestion_date_time, message_created_date_time, document_type_id_text, document_type_text, document_type_coding_system_code, first_insert_date_time, icd_oncology_type_code, predicted_primary_icd_oncology_code, predicted_primary_icd_site_desc, suggested_primary_icd_oncology_code, suggested_primary_icd_site_desc, submitted_primary_icd_oncology_code, submitted_primary_icd_site_desc, transition_of_care_num, user_action_desc, user_action_criticality, user_action_date_time, user_3_4_id, report_assigned_date_time, attending_physician_full_name, pcp_full_name, pcp_phone_num, facility_menmonic_cs, network_mnemonic_cs, model_predicted_sarcoma_ind, submitted_sarcoma_ind, suggested_sarcoma_ind, benign_brain_tumor_type_ind, met_to_brain_tumor_type_ind, other_tumor_type_ind, source_system_code, dw_last_update_date_time)
SELECT row_number() OVER (
                         ORDER BY upper(max(TRIM(stg.unique_message_id)))) AS cancer_patient_id_output_sk,
                         substr(max(TRIM(stg.unique_message_id)), 1, 50) AS unique_message_id,
                         MAX(
                         CASE
                         WHEN TRIM(stg.coid) = '' OR TRIM(stg.coid) IS NULL
                         THEN NULL
                         ELSE
                         LPAD(TRIM(stg.coid),5,"0")
                         END
                         )
                         AS coid,
                         'H' AS company_code,
                         ca.patient_dw_id,
                         CAST(REGEXP_EXTRACT(TRIM(stg.patient_account_number), r'[0-9]+') AS INT64) AS pat_acct_num,
                         max(nullif(TRIM(stg.patient_mrn), '')),
                         substr(max(nullif(TRIM(stg.patient_urn), '')), 1, 30),
                         substr(max(nullif(TRIM(stg.patient_last_name), '')), 1, 50),
                         substr(max(nullif(TRIM(stg.patient_first_name), '')), 1, 50),
                         parse_date('%Y%m%d', replace(nullif(SUBSTR(TRIM(stg.patient_dob),1,8), ''), '^', '')),
                         max(nullif(stg.patient_gender, '')),
                         max(nullif(TRIM(stg.social_security_number), '')),
                         max(nullif(TRIM(stg.patient_address_1), '')),
                         max(nullif(TRIM(stg.patient_address_2), '')),
                         substr(max(nullif(TRIM(stg.patient_city), '')), 1, 50),
                         max(nullif(TRIM(stg.patient_state), '')),
                         substr(max(nullif(TRIM(stg.patient_zip_code), '')), 1, 10),
                         max(nullif(TRIM(stg.patient_type_status), '')),
                         max(nullif(TRIM(stg.message_type), '')),
                         max(nullif(TRIM(stg.message_event_type), '')),
                         max(nullif(TRIM(stg.message_flag), '')),
                         CASE
                            WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.message_origin_or_requested_date_time), r'^[1-2][0-9]+')) < 8 
                            THEN CAST(NULL AS DATETIME)
                            WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.message_origin_or_requested_date_time), r'^[1-2][0-9]+')) >= 8 
                            THEN PARSE_DATETIME('%Y%m%d%H%M%S', RPAD(REGEXP_EXTRACT(TRIM(stg.message_origin_or_requested_date_time), r'^[1-2][0-9]+'),14,"0"))
                            ELSE CAST(NULL AS DATETIME)
                        END AS message_origin_or_requested_date_time,
                         CASE
                            WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.message_signed_or_observation_date_time), r'^[1-2][0-9]+')) < 8 
                            THEN CAST(NULL AS DATETIME)
                            WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.message_signed_or_observation_date_time), r'^[1-2][0-9]+')) >= 8 
                            THEN PARSE_DATETIME('%Y%m%d%H%M%S', RPAD(REGEXP_EXTRACT(TRIM(stg.message_signed_or_observation_date_time), r'^[1-2][0-9]+'),14,"0"))
                            ELSE CAST(NULL AS DATETIME)
                        ...
                        CASE
                            WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.message_created_datetime), r'^[1-2][0-9]+')) < 8 
                            THEN CAST(NULL AS DATETIME)
                            WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.message_created_datetime), r'^[1-2][0-9]+')) >= 8 
                            THEN PARSE_DATETIME('%Y%m%d%H%M%S', RPAD(REGEXP_EXTRACT(TRIM(stg.message_created_datetime), r'^[1-2][0-9]+'),14,"0"))
                            ELSE CAST(NULL AS DATETIME)
                        END AS message_created_datetime,
                        substr(max(nullif(TRIM(stg.document_type_identifier), '')), 1, 20),
                        TRIM(substr(max(nullif(TRIM(stg.document_type_text), '')), 1, 50)),
                        TRIM(substr(max(nullif(TRIM(stg.document_type_name_of_coding_sys), '')), 1, 20)),
                        CASE
                            WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.etl_firstinsert_datetime), r'^[1-2][0-9]+')) < 8 
                            THEN CAST(NULL AS DATETIME)
                            WHEN LENGTH(REGEXP_EXTRACT(TRIM(stg.etl_firstinsert_datetime), r'^[1-2][0-9]+')) >= 8 
                            THEN PARSE_DATETIME('%Y%m%d%H%M%S', RPAD(REGEXP_EXTRACT(TRIM(stg.etl_firstinsert_datetime), r'^[1-2][0-9]+'),14,"0"))
                            ELSE CAST(NULL AS DATETIME)
                        END AS etl_firstinsert_datetime,
                        '3' AS icd_oncology_type_code,
                        max(nullif(replace(TRIM(stg.model_predicted_primary_site_icdo3), '.', ''), '')) AS model_predicted_primary_site_icdo3,
                        substr(max(nullif(TRIM(stg.model_predicted_primary_site), '')), 1, 255),
                        max(nullif(replace(TRIM(stg.suggested_primary_site_icdo3), '.', ''), '')) AS suggested_primary_site_icdo3,
                        substr(max(nullif(TRIM(stg.suggested_primary_site), '')), 1, 255),
                        max(nullif(replace(TRIM(stg.submitted_primary_site_icdo3), '.', ''), '')) AS submitted_primary_site_icdo3,
                        substr(max(nullif(TRIM(stg.submitted_primary_site), '')), 1, 255),
                        CAST(`hca-hin-dev-cur-ops.bqutil_fns`.cw_td_normalize_number(stg.transition_of_care) AS INT64) AS transition_of_care,
                        substr(max(nullif(TRIM(stg.user_action), '')), 1, 20),
                        max(nullif(TRIM(stg.user_action_criticality), '')), 
                        CASE
                        ...
                        TRIM(substr(max(nullif(TRIM(stg.pcp_phone), '')), 1, 15)),
                        nullif(TRIM(stg.facility_mnemonic), ''),
                        nullif(TRIM(stg.meditech_network_id), ''),
                         max(CASE
                              WHEN upper(TRIM(stg.model_predicted_sarcoma)) = 'SARCOMA' THEN 'Y'
                              ELSE 'N'
                              END) AS model_predicted_sarcoma_ind,
                         max(CASE
                              WHEN upper(TRIM(stg.submitted_sarcoma)) = 'SARCOMA' THEN 'Y'
                              ELSE 'N'
                              END) AS submitted_sarcoma_ind,
                         max(CASE
                              WHEN upper(TRIM(stg.suggested_sarcoma)) = 'SARCOMA' THEN 'Y'
                              ELSE 'N'
                              END) AS suggested_sarcoma_ind,
                         max(CASE upper(TRIM(stg.tumor_type_benign_brain))
                              WHEN 'TRUE' THEN 'Y'
                              WHEN 'FALSE' THEN 'N'
                              ELSE CAST(NULL AS STRING)
                              END) AS benign_brain_tumor_type_ind,
                         max(CASE upper(TRIM(stg.tumor_type_met_to_brain))
                              WHEN 'TRUE' THEN 'Y'
                              WHEN 'FALSE' THEN 'N'
                              ELSE CAST(NULL AS STRING)
                              END) AS met_to_brain_tumor_type_ind,
                         max(CASE upper(TRIM(stg.tumor_type_na))
                              WHEN 'TRUE' THEN 'Y'
                              WHEN 'FALSE' THEN 'N'
                              ELSE CAST(NULL AS STRING)
...
FROM {{ params.param_cr_auth_base_views_dataset_name }}.cdm_synthesys_patientid_results AS stg
LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.clinical_acctkeys AS ca 
ON ca.pat_acct_num = CAST(REGEXP_EXTRACT(TRIM(stg.patient_account_number), r'[0-9]+') AS INT64)
AND upper(TRIM(ca.coid)) = LPAD(TRIM(stg.coid),5,"0")
WHERE stg.unique_message_id IS NOT NULL
GROUP BY upper(substr(stg.unique_message_id, 1, 50)),
         CASE
          WHEN TRIM(stg.coid) = '' OR TRIM(stg.coid) IS NULL
          THEN NULL
          ELSE
         ...
         5,
         6,
         upper(nullif(TRIM(stg.patient_mrn), '')),
         upper(substr(nullif(stg.patient_urn, ''), 1, 30)),
         upper(substr(nullif(TRIM(stg.patient_last_name), ''), 1, 50)),
         upper(substr(nullif(TRIM(stg.patient_first_name), ''), 1, 50)),
         11,
         upper(nullif(TRIM(stg.patient_gender), '')),
         upper(nullif(TRIM(stg.social_security_number), '')),
         upper(nullif(TRIM(stg.patient_address_1), '')),
         upper(nullif(TRIM(stg.patient_address_2), '')),
         upper(substr(nullif(TRIM(stg.patient_city), ''), 1, 50)),
         upper(nullif(TRIM(stg.patient_state), '')),
         upper(substr(nullif(TRIM(stg.patient_zip_code), ''), 1, 10)),
         upper(nullif(TRIM(stg.patient_type_status), '')),
         upper(nullif(TRIM(stg.message_type), '')),
         upper(nullif(TRIM(stg.message_event_type), '')),
         upper(nullif(TRIM(stg.message_flag), '')),
         23,
         24,
         25,
         26,
         upper(substr(nullif(TRIM(stg.document_type_identifier), ''), 1, 20)),
         upper(substr(nullif(TRIM(stg.document_type_text), ''), 1, 50)),
         upper(substr(nullif(TRIM(stg.document_type_name_of_coding_sys), ''), 1, 20)),
         30,
         upper(nullif(replace(TRIM(stg.model_predicted_primary_site_icdo3), '.', ''), '')),
         upper(substr(nullif(TRIM(stg.model_predicted_primary_site), ''), 1, 255)),
         upper(nullif(replace(TRIM(stg.suggested_primary_site_icdo3), '.', ''), '')),
         upper(substr(nullif(TRIM(stg.suggested_primary_site), ''), 1, 255)),
         upper(nullif(replace(TRIM(stg.submitted_primary_site_icdo3), '.', ''), '')),
         upper(substr(nullif(TRIM(stg.submitted_primary_site), ''), 1, 255)),
         38,
         upper(substr(nullif(TRIM(stg.user_action), ''), 1, 20)),
         upper(nullif(TRIM(stg.user_action_criticality), '')),
         41,
         upper(nullif(TRIM(stg.user), '')),
         43,
         upper(substr(nullif(TRIM(stg.attending_physician), ''), 1, 100)),
         upper(substr(nullif(TRIM(stg.primary_care_physician), ''), 1, 100)),
         upper(substr(nullif(TRIM(stg.pcp_phone), ''), 1, 15)),
         47,
         48,
         upper(CASE
                   WHEN upper(TRIM(stg.model_predicted_sarcoma)) = 'SARCOMA' THEN 'Y'
         ...
         upper(CASE
                   ...
                   ELSE 'N'
               END),
         upper(CASE upper(TRIM(stg.tumor_type_benign_brain))
                   WHEN 'TRUE' THEN 'Y'
                   WHEN 'FALSE' THEN 'N'
                   ELSE CAST(NULL AS STRING)
               END),
         upper(CASE upper(TRIM(stg.tumor_type_met_to_brain))
                   WHEN 'TRUE' THEN 'Y'
                   WHEN 'FALSE' THEN 'N'
                   ELSE CAST(NULL AS STRING)
               END),
         upper(CASE upper(TRIM(stg.tumor_type_na))
                   WHEN 'TRUE' THEN 'Y'
                   WHEN 'FALSE' THEN 'N'
                   ELSE CAST(NULL AS STRING)
               END);
...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cancer_patient_id_output;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cancer_patient_id_output AS mt USING
  (SELECT DISTINCT cancer_patient_id_output_wrk.cancer_patient_id_output_sk,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cancer_patient_id_output_wrk) AS ms ON mt.cancer_patient_id_output_sk = ms.cancer_patient_id_output_sk
...
INSERT (cancer_patient_id_output_sk,
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT cancer_patient_id_output_sk
      FROM {{ params.param_cr_core_dataset_name }}.cancer_patient_id_output
      GROUP BY cancer_patient_id_output_sk
      HAVING count(*) > 1));

IF DUP_COUNT <> 0 THEN
ROLLBACK TRANSACTION;

RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cancer_patient_id_output');
