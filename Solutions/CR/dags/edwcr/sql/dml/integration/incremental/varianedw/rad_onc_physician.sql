...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.rad_onc_physician;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.rad_onc_physician AS mt USING
  (SELECT DISTINCT row_number() OVER (
                                      ORDER BY CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimsiteid) AS INT64),
                                               CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimdoctorid) AS INT64)) AS physician_sk,
                                     rr.site_sk AS site_sk,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimdoctorid) AS INT64) AS source_physician_id,
                                     ra.location_sk AS location_sk,
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimlookupid_resourcetype) AS INT64) AS resource_type_id,
                                     substr(CASE
                                     ...
                                     CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.ctrresourceser) AS INT64) AS resource_id,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.stg_dimdoctor AS dhd
   INNER JOIN
     (SELECT ref_rad_onc_site.source_site_id,
             ref_rad_onc_site.site_sk
      FROM {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_site) AS rr ON rr.source_site_id = CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimsiteid) AS FLOAT64)
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_location AS ra ON CAST({{ params.param_cr_bqutil_fns_dataset_name }}.cw_td_normalize_number(dhd.dimlocationid) AS INT64) = ra.source_location_id
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT physician_sk
      FROM {{ params.param_cr_core_dataset_name }}.rad_onc_physician
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.rad_onc_physician');
