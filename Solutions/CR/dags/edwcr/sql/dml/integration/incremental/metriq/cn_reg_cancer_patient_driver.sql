...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.cancer_patient_driver;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cancer_patient_driver AS mt USING
     ...
     (SELECT a.patient_market_urn,
      ...
      FROM
        (SELECT cr.patient_market_urn,
                ...
                e.empi_text
         FROM {{ params.param_cr_base_views_dataset_name }}.clinical_registration AS cr
         INNER JOIN {{ params.param_cr_base_views_dataset_name }}.empi_encounter_id_xwalk AS e ON cr.patient_dw_id = e.encounter_id
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cancer_patient_driver');
