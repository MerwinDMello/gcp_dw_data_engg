...
-- bteq << EOF >> $1;;
 ...
 --Job=J_CN_PATIENT_SURGERY_RECON;;
 --' FOR SESSION;;
...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_surgery_reconstruction
WHERE upper(cn_patient_surgery_reconstruction.hashbite_ssk) NOT IN
    (SELECT upper(cn_patient_surgery_reconstruction_stg.hashbite_ssk) AS hashbite_ssk
     FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_surgery_reconstruction_stg);
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_surgery_reconstruction AS mt USING
  (SELECT DISTINCT stg.cn_patient_surgery_recstr_sid,
   ...
   FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_surgery_reconstruction_stg AS stg
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_side AS rs ON upper(trim(stg.reconsurgeryside)) = upper(trim(rs.side_desc))
   WHERE upper(stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_surgery_reconstruction.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_surgery_reconstruction) ) AS ms ON mt.cn_patient_surgery_recstr_sid = ms.cn_patient_surgery_recstr_sid
...
INSERT (cn_patient_surgery_recstr_sid,
        core_record_type_id,
        surgery_recstr_side_id,
  ...
  (SELECT count(*)
   FROM
     (SELECT cn_patient_surgery_recstr_sid
      FROM {{ params.param_cr_core_dataset_name }}.cn_patient_surgery_reconstruction
      GROUP BY cn_patient_surgery_recstr_sid
      HAVING count(*) > 1));

IF DUP_COUNT <> 0 THEN
ROLLBACK TRANSACTION;

RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_surgery_reconstruction');
