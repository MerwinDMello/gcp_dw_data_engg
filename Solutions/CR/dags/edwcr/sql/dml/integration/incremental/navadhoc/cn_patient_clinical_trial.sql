...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_clinical_trial AS mt USING
  (SELECT DISTINCT /*,Clinical_Trial_Evaluated_Ind
   ...
   WHERE upper(cn_patient_clinical_trial_stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_clinical_trial.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_clinical_trial) ) AS ms ON mt.cn_patient_clinical_trial_sid = ms.cn_patient_clinical_trial_sid
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_clinical_trial');
