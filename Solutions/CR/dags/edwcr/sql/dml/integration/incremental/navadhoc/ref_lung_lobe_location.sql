...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_lung_lobe_location AS mt USING
  (SELECT DISTINCT row_number() OVER (
     ...
     (SELECT coalesce(max(ref_lung_lobe_location.lung_lobe_location_id), 0) AS id1
      FROM {{ params.param_cr_core_dataset_name }}.ref_lung_lobe_location) AS lung_lobe_location_id,
   ...
   FROM
     (SELECT trim(ref_lung_lobe_location_stg.lung_lobe_location_desc) AS lung_lobe_location_desc,
      ...
      FROM {{ params.param_cr_stage_dataset_name }}.ref_lung_lobe_location_stg
      WHERE upper(trim(ref_lung_lobe_location_stg.lung_lobe_location_desc)) NOT IN
          (SELECT upper(trim(ref_lung_lobe_location.lung_lobe_location_desc))
           FROM {{ params.param_cr_core_dataset_name }}.ref_lung_lobe_location) ) AS type_stg) AS ms ON mt.lung_lobe_location_id = ms.lung_lobe_location_id
  ...
  (SELECT count(*)
   FROM
     (SELECT lung_lobe_location_id
      FROM {{ params.param_cr_core_dataset_name }}.ref_lung_lobe_location
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_lung_lobe_location');
