...
-- bteq << EOF >> $1;;
 ...
 --Job=J_CN_PATIENT_HEME_TRANSPLANT;;
 --' FOR SESSION;;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.cn_patient_heme_transplant AS mt USING
  (SELECT DISTINCT iq.cn_patient_heme_sid AS cn_patient_heme_sid,
   ...
   FROM
     (SELECT phd.patienthemefactid AS cn_patient_heme_sid,
      ...
      FROM {{ params.param_cr_stage_dataset_name }}.stg_patienthemetransplant AS phd
      LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_transplant_type AS rtt ON upper(trim(phd.type_type)) = upper(trim(rtt.transplant_type_name))
      LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_status AS rs ON upper(trim(phd.transplantcandidacystatus)) = upper(trim(rs.status_desc))
      AND upper(rtrim(rs.status_type_desc)) = 'CELLULAR THERAPY') AS iq
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_patient_heme_transplant AS cpht ON upper(trim(iq.hashbite_ssk)) = upper(trim(cpht.hashbite_ssk))
   WHERE trim(cpht.hashbite_ssk) IS NULL ) AS ms ON mt.cn_patient_heme_sid = ms.cn_patient_heme_sid
...
INSERT (cn_patient_heme_sid,
        ...
        coid,
        company_code,
        cellular_therapy_status_date,
        cellular_therapy_comment_text,
        transfer_date,
        transplant_date,
        transplant_comment_text,
        hashbite_ssk,
        source_system_code,
  ...
  (SELECT count(*)
   FROM
     (SELECT cn_patient_heme_sid
      FROM {{ params.param_cr_core_dataset_name }}.cn_patient_heme_transplant
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.cn_patient_heme_transplant');
...
DELETE
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_heme_transplant AS cpht
WHERE NOT EXISTS
    (SELECT 'X'
     FROM {{ params.param_cr_stage_dataset_name }}.stg_patienthemetransplant AS sphd
