...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.fact_rad_onc_patient;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.fact_rad_onc_patient AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_rad_onc_location AS ra ON dhd.dimlocationid = ra.source_location_id
  ...
  (SELECT count(*)
   FROM
     (SELECT fact_patient_sk
      FROM {{ params.param_cr_core_dataset_name }}.fact_rad_onc_patient
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.fact_rad_onc_patient');
