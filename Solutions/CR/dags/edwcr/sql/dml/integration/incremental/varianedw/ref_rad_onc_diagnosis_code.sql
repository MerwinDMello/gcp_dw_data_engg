...
-- bteq << EOF >> $1;;
 ...
 --Job=J_CR_RO_Ref_Rad_Onc_Diagnosis_Code;;
 --' FOR SESSION;;
...
TRUNCATE TABLE {{ params.param_cr_core_dataset_name }}.ref_rad_onc_diagnosis_code;
...
MERGE INTO {{ params.param_cr_core_dataset_name }}.ref_rad_onc_diagnosis_code AS mt USING
  (SELECT DISTINCT row_number() OVER (
   ...
   LEFT OUTER JOIN
     (SELECT DISTINCT trim(cr_sc_diagnosissites_stg.diagnosiscode) AS diagnosiscode,
      ...
      FROM {{ params.param_cr_stage_dataset_name }}.cr_sc_diagnosissites_stg) AS sc ON rtrim(sc.diagnosiscode) = rtrim(dp.diagnosiscode)) AS ms ON mt.diagnosis_code_sk = ms.diagnosis_code_sk
...
INSERT (diagnosis_code_sk,
        site_sk,
        source_diagnosis_code_id,
        diagnosis_code,
        diagnosis_site_text,
        diagnosis_code_class_schema_id,
        diagnosis_clinical_desc,
        diagnosis_long_desc,
        diagnosis_type_code,
        log_id,
        run_id,
...
SET DUP_COUNT =
  (SELECT count(*)
   FROM
     (SELECT diagnosis_code_sk
      FROM {{ params.param_cr_core_dataset_name }}.ref_rad_onc_diagnosis_code
      GROUP BY diagnosis_code_sk
      HAVING count(*) > 1));

IF DUP_COUNT <> 0 THEN
ROLLBACK TRANSACTION;

RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_cr_core_dataset_name }}.ref_rad_onc_diagnosis_code');
