...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'rad_onc_phone_wrk');
...
FROM {{ params.param_cr_stage_dataset_name }}.rad_onc_phone_wrk)
  );
...
WHERE rad_onc_phone.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
