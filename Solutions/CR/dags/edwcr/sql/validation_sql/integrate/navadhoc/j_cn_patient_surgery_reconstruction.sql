...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_surgery_reconstruction_stg');
...
FROM
  (SELECT stg.cn_patient_surgery_recstr_sid,
   ...
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_side AS rs
   ON upper(trim(stg.reconsurgeryside)) = upper(trim(rs.side_desc))
...
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_surgery_reconstruction
WHERE cn_patient_surgery_reconstruction.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
