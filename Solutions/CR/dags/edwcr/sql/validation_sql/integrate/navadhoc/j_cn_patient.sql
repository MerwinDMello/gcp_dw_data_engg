...
FROM
  (SELECT stg.nav_patient_id,
   ...
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_physician_detail AS pd1
   ON upper(rtrim(coalesce(trim(stg.gynecologist), 'X'))) = upper(rtrim(coalesce(trim(pd1.physician_name), 'X')))
   AND upper(rtrim(coalesce(trim(stg.gynecologistphone), 'X'))) = upper(rtrim(coalesce(trim(pd1.physician_phone_num), 'X')))
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_physician_detail AS pd2
   ON upper(rtrim(coalesce(trim(stg.primarycarephysician), 'XX'))) = upper(rtrim(coalesce(trim(pd2.physician_name), 'XX')))
