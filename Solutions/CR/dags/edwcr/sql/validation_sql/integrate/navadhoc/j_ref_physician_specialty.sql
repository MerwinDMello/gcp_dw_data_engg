...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_physician_specialty_stg');
...
FROM
  (SELECT trim(ref_physician_specialty_stg.physician_speciality_desc) AS physician_speciality_desc,
   ...
   WHERE upper(trim(ref_physician_specialty_stg.physician_speciality_desc)) NOT IN
     ...
     AND DATE(ref_physician_specialty_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.ref_physician_specialty
WHERE ref_physician_specialty.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
