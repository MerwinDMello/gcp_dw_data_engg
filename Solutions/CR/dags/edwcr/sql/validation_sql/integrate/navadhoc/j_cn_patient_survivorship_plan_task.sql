...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_survivorship_plan_task_stg');
...
FROM
  (SELECT stg.nav_survivorship_plan_task_sid,
   ...
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_contact_method AS ref1
   ON upper(rtrim(stg.taskmeasnofcontact)) = upper(rtrim(ref1.contact_method_desc))
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_status AS ref2
   ON upper(rtrim(stg.taskstate)) = upper(rtrim(ref2.status_desc))
...
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_survivorship_plan_task
WHERE cn_patient_survivorship_plan_task.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
