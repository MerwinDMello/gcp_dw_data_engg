...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_lab_result_stg');
...
FROM
  (SELECT cn_patient_lab_result_stg.nav_patient_lab_result_sid,
...
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_lab_result
WHERE cn_patient_lab_result.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
