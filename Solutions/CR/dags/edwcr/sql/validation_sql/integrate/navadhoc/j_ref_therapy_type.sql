...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'therapy_type_stg');
...
FROM
  (SELECT therapy_type_stg.therapy_type_desc,
   ...
   WHERE upper(trim(therapy_type_stg.therapy_type_desc)) NOT IN
     ...
     AND DATE(therapy_type_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.ref_therapy_type
WHERE ref_therapy_type.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
