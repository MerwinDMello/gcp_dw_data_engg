...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_consultation_stg');
...
FROM
  (SELECT cn_patient_consultation_stg.cn_patient_consultation_sid,
          cn_patient_consultation_stg.hashbite_ssk
   FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_consultation_stg
   WHERE upper(cn_patient_consultation_stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_consultation.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_consultation
        FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central'))
     AND DATE(cn_patient_consultation_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_base_views_dataset_name }}.cn_patient_consultation
WHERE cn_patient_consultation.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
