...
FROM
  (SELECT cn_physician_role_stg.physician_id,
   ...
   FROM {{ params.param_cr_base_views_dataset_name }}.cn_physician_detail 
   INNER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_patient
   ON cn_physician_detail.physician_id = cn_patient.gynecologist_physician_id
   UNION DISTINCT SELECT cn_physician_detail.physician_id,
                         'PCP' AS physician_role_code
   FROM {{ params.param_cr_base_views_dataset_name }}.cn_physician_detail
   INNER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_patient
   ON cn_physician_detail.physician_id = cn_patient.primary_care_physician_id
   UNION DISTINCT SELECT cn_physician_detail.physician_id,
                         'ETP' AS physician_role_code
   FROM {{ params.param_cr_base_views_dataset_name }}.cn_physician_detail 
   INNER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_patient_tumor
   ON cn_physician_detail.physician_id = cn_patient_tumor.treatment_end_physician_id) AS ab
