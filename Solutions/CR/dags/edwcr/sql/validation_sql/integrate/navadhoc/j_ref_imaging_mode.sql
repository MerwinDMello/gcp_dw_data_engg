...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_imaging_mode_stg');
...
FROM
  (SELECT ref_imaging_mode_stg.imaging_mode_desc,
   ...
   WHERE upper(trim(ref_imaging_mode_stg.imaging_mode_desc)) NOT IN
     ...
     AND DATE(ref_imaging_mode_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.ref_imaging_mode
WHERE ref_imaging_mode.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
