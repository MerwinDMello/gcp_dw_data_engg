...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_breast_cancer_type_stg');
...
FROM
  (SELECT ref_breast_cancer_type_stg.breast_cancer_type_desc,
   ...
   WHERE upper(trim(ref_breast_cancer_type_stg.breast_cancer_type_desc)) NOT IN
     ...
     AND DATE(ref_breast_cancer_type_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.ref_breast_cancer_type
WHERE ref_breast_cancer_type.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
