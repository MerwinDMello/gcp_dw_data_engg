...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_physician_detail_stg_wrk');
...
FROM
  (SELECT row_number() OVER (
     ...
     (SELECT coalesce(max(cn_physician_detail.physician_id), 900000) AS physician_id
      ...
      FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central')) AS physician_id,
                            wrk.physician_name,
   ...
   LEFT OUTER JOIN
     (SELECT cn_physician_detail.physician_id,
      ...
      WHERE
         cn_physician_detail.physician_id > 900000 ) AS tgt ON upper(rtrim(coalesce(trim(wrk.physician_name), 'XX'))) = upper(rtrim(coalesce(trim(tgt.physician_name), 'XX')))
  ...
  FROM (SELECT format('%20d', count(*)) AS source_string
FROM {{ params.param_cr_core_dataset_name }}.cn_physician_detail
WHERE cn_physician_detail.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
