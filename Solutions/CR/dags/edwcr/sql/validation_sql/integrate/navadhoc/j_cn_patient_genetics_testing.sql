...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_genetics_testing_stg');
...
FROM
  (SELECT pgt.geneticstestingfactid,
   ...
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_breast_cancer_type AS brca ON upper(rtrim(pgt.geneticsbrcatype)) = upper(rtrim(brca.breast_cancer_type_desc))
  ...
  FROM (SELECT format('%20d', count(*)) AS source_string
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_genetics_testing
WHERE cn_patient_genetics_testing.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
