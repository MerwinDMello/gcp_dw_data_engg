...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_housing_type_stg');
...
FROM
  (SELECT DISTINCT row_number() OVER (
   ...
   WHERE NOT EXISTS
       (SELECT 1
        FROM {{ params.param_cr_core_dataset_name }}.ref_housing_type AS rcdc
...
FROM {{ params.param_cr_core_dataset_name }}.ref_housing_type
WHERE ref_housing_type.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
