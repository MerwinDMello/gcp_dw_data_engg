...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_staging_stg');
...
FROM
  (SELECT cn_patient_staging_stg.cn_patient_staging_sid,
   ...
   WHERE (upper(cn_patient_staging_stg.hashbite_ssk),
     ...
     AND DATE(cn_patient_staging_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
WHERE cn_patient_staging.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
