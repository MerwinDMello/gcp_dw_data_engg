...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'stg_patienthemediagnosis');
...
FROM
  (SELECT iq.*
   ...
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.cn_patient_heme_diagnosis AS cphd
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(trim(iq.hashbite_ssk)) = upper(trim(cphd.hashbite_ssk))
...
WHERE cn_patient_heme_diagnosis.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
