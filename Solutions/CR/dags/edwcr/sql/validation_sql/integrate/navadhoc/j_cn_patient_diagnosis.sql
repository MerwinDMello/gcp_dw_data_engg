...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_diagnosis_stg');
...
FROM
  (SELECT stg.cn_patient_diagnosis_sid,
   ...
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_side AS rs ON upper(rtrim(stg.diagnosisside)) = upper(rtrim(rs.side_desc))
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_diagnosis_detail AS rdd ON upper(rtrim(coalesce(trim(stg.diagnosismetastatic), 'X'))) = upper(rtrim(coalesce(trim(rdd.diagnosis_detail_desc), 'X')))
  ...
  FROM (SELECT format('%20d', count(*)) AS source_string
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_diagnosis
WHERE cn_patient_diagnosis.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
