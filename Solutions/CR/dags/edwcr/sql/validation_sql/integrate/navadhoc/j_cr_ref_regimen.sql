...
BEGIN
  ...
  (SELECT 'X' AS ccnt
   FROM
     (SELECT DISTINCT substr(trim(regimen), 1, 50) AS regimen_name
      FROM {{ params.param_cr_stage_dataset_name }}.patient_heme_treatment_regimen_stg
      WHERE trim(regimen) IS NOT NULL ) AS stg
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_regimen AS tgt 
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central')
   ON upper(trim(stg.regimen_name)) = upper(trim(tgt.regimen_name))
   WHERE trim(tgt.regimen_name) IS NULL ) AS iq)
  );
