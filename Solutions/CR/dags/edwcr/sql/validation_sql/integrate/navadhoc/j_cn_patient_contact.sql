...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_contact_stg');
...
FROM
  (SELECT cn_patient_contact_stg.cn_patient_contact_sid,
   ...
   WHERE upper(cn_patient_contact_stg.hashbite_ssk) NOT IN
     ...
     AND DATE(cn_patient_contact_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_contact
WHERE cn_patient_contact.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
