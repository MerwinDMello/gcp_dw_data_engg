...
FROM
  (SELECT 'X' AS ccnt
   ...
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_sample_type AS rtt 
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central')
   ON upper(trim(ssc.sample_type_name)) = upper(trim(rtt.sample_type_name))
...
WHERE ref_sample_type.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
