...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_diagnosis_detail_stg');
...
FROM
  (SELECT trim(ref_diagnosis_detail_stg.diagnosis_detail_desc) AS diagnosis_detail_desc,
   ...
   WHERE (upper(trim(ref_diagnosis_detail_stg.diagnosis_detail_desc)),
     ...
     AND DATE(ref_diagnosis_detail_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.ref_diagnosis_detail
WHERE ref_diagnosis_detail.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
