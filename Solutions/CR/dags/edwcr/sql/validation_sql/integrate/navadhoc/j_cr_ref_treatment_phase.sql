...
BEGIN
  ...
  (SELECT 'X' AS ccnt
   FROM
     (SELECT DISTINCT substr(trim(treatmentphase), 1, 50) AS treatment_phase_desc
      FROM {{ params.param_cr_stage_dataset_name }}.patient_heme_treatment_regimen_stg
      WHERE trim(treatmentphase) IS NOT NULL ) AS stg
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_treatment_phase AS tgt 
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central')
   ON upper(trim(stg.treatment_phase_desc)) = upper(trim(tgt.treatment_phase_desc))
   WHERE trim(tgt.treatment_phase_desc) IS NULL ) AS iq)
  );
