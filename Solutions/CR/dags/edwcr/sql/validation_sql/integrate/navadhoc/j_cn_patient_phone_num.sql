...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_phone_num_stg');
...
FROM
  (SELECT cn_patient_phone_num_stg.nav_patient_id
   ...
   WHERE cn_patient_phone_num_stg.nav_patient_id NOT IN
     ...
     AND DATE(cn_patient_phone_num_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_phone_num
WHERE cn_patient_phone_num.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
