...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_test_type_stg');
...
FROM
  (SELECT trim(ref_test_type_stg.test_type_desc) AS test_type_desc,
   ...
   WHERE (upper(trim(ref_test_type_stg.test_type_desc)),
     ...
     AND DATE(ref_test_type_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.ref_test_type
WHERE ref_test_type.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
