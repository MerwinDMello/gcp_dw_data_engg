...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_result_stg');
...
FROM
  (SELECT trim(ref_result_stg.nav_result_desc) AS nav_result_desc,
   ...
   WHERE upper(trim(ref_result_stg.nav_result_desc)) NOT IN
     ...
     AND DATE(ref_result_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.ref_result
WHERE ref_result.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
