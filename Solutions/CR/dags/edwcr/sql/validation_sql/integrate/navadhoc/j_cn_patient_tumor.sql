...
FROM
  (SELECT stg.cn_patient_tumor_sid,
   ...
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_facility AS rf1
   ON upper(rtrim(stg.tumorreferralsource)) = upper(rtrim(rf1.facility_name))
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_status AS rs
   ON upper(rtrim(stg.navigationstatus)) = upper(rtrim(rs.status_desc))
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_physician_detail AS pd
   ON upper(rtrim(stg.endtreatmentphysician)) = upper(rtrim(pd.physician_name))
   AND pd.physician_phone_num IS NULL
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_facility AS rf2
   ON upper(rtrim(stg.endtreatmentlocation)) = upper(rtrim(rf2.facility_name))
