...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_lung_lobe_location_stg');
...
FROM
  (SELECT trim(ref_lung_lobe_location_stg.lung_lobe_location_desc) AS lung_lobe_location_desc,
   ...
   WHERE upper(trim(ref_lung_lobe_location_stg.lung_lobe_location_desc)) NOT IN
     ...
     AND DATE(ref_lung_lobe_location_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.ref_lung_lobe_location
WHERE ref_lung_lobe_location.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
