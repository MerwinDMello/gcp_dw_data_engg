...
FROM
  (SELECT 'X' AS ccnt
   ...
   LEFT OUTER JOIN {{ params.param_cr_core_dataset_name }}.ref_disease_assess_source AS rtt 
   FOR SYSTEM_TIME AS OF TIMESTAMP(tableload_start_time, 'US/Central') ON upper(rtrim(ssc.disease_assess_source_name)) = upper(rtrim(rtt.disease_assess_source_name))
  ...
  SELECT SPLIT(SOURCE_STRING,',') values_list
  FROM (SELECT format('%20d', count(*)) AS source_string
FROM {{ params.param_cr_base_views_dataset_name }}.ref_disease_assess_source
WHERE ref_disease_assess_source.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
