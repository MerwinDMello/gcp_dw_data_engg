...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_heme_stg');
...
FROM
  (SELECT stg.patienthemefactid,
   ...
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_physician_detail AS dtl ON upper(rtrim(dtl.physician_name)) = upper(rtrim(stg.hematologist))
...
WHERE cn_patient_heme.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
