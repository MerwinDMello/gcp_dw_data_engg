...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'contact_person_stg');
...
FROM
  (SELECT contact_person_stg.contact_person_desc,
   ...
   WHERE upper(trim(contact_person_stg.contact_person_desc)) NOT IN
     ...
     AND DATE(contact_person_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.ref_contact_person
WHERE ref_contact_person.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
