...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_address_stg');
...
FROM
  (SELECT cn_patient_address_stg.*
   ...
   WHERE cn_patient_address_stg.nav_patient_id NOT IN
     ...
     AND DATE(cn_patient_address_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_base_views_dataset_name }}.cn_patient_address
WHERE cn_patient_address.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
