...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_surgery_stg');
...
FROM
  (SELECT stg.cn_patient_surgery_sid,
   ...
   ON upper(trim(stg.surgeryside)) = upper(trim(rs.side_desc))
   LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_facility AS rf
   ON upper(trim(stg.surgeryfacility)) = upper(trim(rf.facility_name))
...
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_surgery
WHERE cn_patient_surgery.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
