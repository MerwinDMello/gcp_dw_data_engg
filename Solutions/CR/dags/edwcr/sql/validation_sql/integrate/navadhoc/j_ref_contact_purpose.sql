...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'contact_purpose_stg');
...
FROM
  (SELECT contact_purpose_stg.contact_purpose_desc,
   ...
   WHERE upper(trim(contact_purpose_stg.contact_purpose_desc)) NOT IN
     ...
     AND DATE(contact_purpose_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.ref_contact_purpose
WHERE ref_contact_purpose.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
