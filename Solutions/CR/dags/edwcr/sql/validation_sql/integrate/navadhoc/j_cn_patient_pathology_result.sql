...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_pathology_result_stg');
...
FROM
  (SELECT cn_patient_pathology_result_stg.cn_patient_pathology_res_sid,
          ...
          cn_patient_pathology_result_stg.coid,
          cn_patient_pathology_result_stg.company_code,
          cn_patient_pathology_result_stg.result_value_text,
          cn_patient_pathology_result_stg.hashbite_ssk,
          cn_patient_pathology_result_stg.source_system_code,
          datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time
   FROM {{ params.param_cr_stage_dataset_name }}.cn_patient_pathology_result_stg
   WHERE upper(cn_patient_pathology_result_stg.hashbite_ssk) NOT IN
       (SELECT upper(cn_patient_pathology_result.hashbite_ssk) AS hashbite_ssk
        FROM {{ params.param_cr_core_dataset_name }}.cn_patient_pathology_result
...
FROM {{ params.param_cr_core_dataset_name }}.cn_patient_pathology_result
WHERE cn_patient_pathology_result.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
