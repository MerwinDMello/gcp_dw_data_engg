...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'cn_patient_family_history_stg');
...
FROM
  ...
  SELECT SPLIT(SOURCE_STRING,',') values_list
  FROM (SELECT format('%20d', count(*)) AS source_string
FROM {{ params.param_cr_base_views_dataset_name }}.cn_patient_family_history
WHERE cn_patient_family_history.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
