...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_site_location_stg');
  ...
  (SELECT ref_site_location_stg.site_location_desc,
   ...
   WHERE upper(trim(ref_site_location_stg.site_location_desc)) NOT IN
       (SELECT upper(trim(ref_site_location.site_location_desc))
        ...
        WHERE
           ref_site_location.site_location_desc IS NOT NULL )
     AND DATE(ref_site_location_stg.dw_last_update_date_time) = current_date('US/Central') ) AS a)
  );
...
FROM {{ params.param_cr_core_dataset_name }}.ref_site_location
WHERE ref_site_location.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
