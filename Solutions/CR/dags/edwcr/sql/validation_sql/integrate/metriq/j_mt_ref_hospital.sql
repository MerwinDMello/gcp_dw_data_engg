...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_hospital_stg');
...
FROM {{ params.param_cr_stage_dataset_name }}.ref_hospital_stg)
  );
...
WHERE ref_hospital.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
