...
BEGIN
  ...
  SET srctablename = concat(srcdataset_id, '.', 'ref_national_svc_center_stg');
...
FROM {{ params.param_cr_stage_dataset_name }}.ref_national_svc_center_stg)
  );
...
WHERE ref_national_service_center.dw_last_update_date_time >= tableload_start_time - INTERVAL 1 MINUTE)
  );
