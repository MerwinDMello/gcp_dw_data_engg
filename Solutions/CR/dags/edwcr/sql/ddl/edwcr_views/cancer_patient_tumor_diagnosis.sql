...
-- Source: eim-ops-cs-datamig-dev-0002/sql_conversion/edwcr_migration_source/{{ params.param_cr_views_dataset_name }}/cancer_patient_tumor_diagnosis.sql
-- Translated from: Teradata
-- Translated to: BigQuery

CREATE OR REPLACE VIEW {{ params.param_cr_views_dataset_name }}.cancer_patient_tumor_diagnosis AS SELECT DISTINCT
  ...
  FROM
    ...
    LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cr_patient_diagnosis_detail AS cpdd ON cpt.cr_patient_id = cpdd.cr_patient_id
     AND cpt.tumor_id = cpdd.tumor_id
    LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_lookup_code AS t12 ON cpt.tumor_primary_site_id = t12.master_lookup_sid
     AND t12.lookup_id = 12
    LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_lookup_code AS t11 ON cpdd.diagnosis_name_id = t11.master_lookup_sid
    ...
    LEFT OUTER JOIN (
      SELECT
          ...
          rtt.tumor_type_desc,
          CASE
            WHEN cpd.diagnosis_date = '1900-01-01' THEN 0
            ELSE DIV(DATE_DIFF(cpd.diagnosis_date, cp.birth_date, DAY), 365)
          END AS diagnose_age_num
        FROM
          {{ params.param_cr_base_views_dataset_name }}.cn_person AS cp
          LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_patient_tumor AS cpt_0 ON cp.nav_patient_id = cpt_0.nav_patient_id
          LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.cn_patient_diagnosis AS cpd ON cpt_0.nav_patient_id = cpd.nav_patient_id
           AND cpt_0.tumor_type_id = cpd.tumor_type_id
          LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_diagnosis_detail AS rdd ON cpd.diagnosis_detail_id = rdd.diagnosis_detail_id
          LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_diagnosis_result AS rdr ON cpd.diagnosis_result_id = rdr.diagnosis_result_id
          LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_side AS rs ON cpd.diagnosis_side_id = rs.side_id
          LEFT OUTER JOIN {{ params.param_cr_base_views_dataset_name }}.ref_tumor_type AS rtt ON cpd.tumor_type_id = rtt.tumor_type_id
