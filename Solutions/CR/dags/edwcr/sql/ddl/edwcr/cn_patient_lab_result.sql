CREATE OR REPLACE TABLE {{ params.param_cr_core_dataset_name }}.cn_patient_lab_result
(
  nav_patient_lab_result_sid INT64 NOT NULL OPTIONS(description='A unique identifier for each lab result.'),
  lab_type_id INT64 OPTIONS(description='A unique identifier for each lab type.'),
  ...
  lab_date DATE OPTIONS(description='The date the lab was taken.'),
  lab_result_text STRING OPTIONS(description='A free text field that details the lab result.'),
...
OPTIONS(
  description='Contains the lab results for each patient.'
);
