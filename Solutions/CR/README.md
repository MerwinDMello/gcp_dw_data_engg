...
2. Starting on the dev branch, create a feature branch and make updates to the following key areas:
    - All environment config files (update with correct values for \<domain> and \<subdomain>, define variables)
...
8. If ready to do so, coordinate with the admin of the gcp-hin-operations repo to get secrets depoloyed to GSM (this can come later if not immediately needed)
...
This repository contains the code for ingestion and integration of data for EDWCR, part of the Operations HIN domain. Each major folder contains a README with additional context for its contents.
