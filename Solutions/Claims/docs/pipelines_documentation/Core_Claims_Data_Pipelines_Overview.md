...
%%{init: {'theme': 'base', 'themeVariables': { 'fontSize': '12pt' }}}%%
 graph TD
    ...
    Claim_Source[(Claim Source Tables)]
	Claims_Staging[(Claims Staging)]
	Claims_Core[(Claims Core)]
    Claim_Stats_Writeback[(Fact Claim Stats)]
    DW_Counts_Writeback[(Core Table DW Counts)]

    Pre_Poll == triggers ==> Ingest == triggers ===> Integrate == triggers ===> Fact_Claim_Stats_Writeback == triggers ==> Core_Table_DW_Counts_Writeback == triggers ==> Post_Poll
	Claim_Source --o Ingest --x Claims_Staging --o Integrate --x Claims_Core
	Claims_Staging --o Fact_Claim_Stats_Writeback --x Claim_Stats_Writeback
	Claims_Core --o Core_Table_DW_Counts_Writeback --x DW_Counts_Writeback
    
	
	class Pre_Poll start_dag
	class Post_Poll end_dag
	class Claims_Staging,Claims_Core bigquery
	class Claim_Source,Claim_Stats_Writeback,DW_Counts_Writeback sqlserver
```