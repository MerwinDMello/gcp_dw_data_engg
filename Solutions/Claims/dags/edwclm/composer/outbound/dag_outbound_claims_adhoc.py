...
def create_dag(dag_id, schedule, start_date, source_system, frequency, tags, has_sensor, sensor_list, trigger_dag_ids, tgttablename):
    ...
    with dag:
        ...
        '''def process_where_condition(**kwargs):
            where_value = str(kwargs.get('dag_run', {}).get('conf', {}).get('where_condition', '')).lower().strip() 
            logging.info(f"Where Condition: {where_value}")
            return where_value'''
        where_condition = Variable.get("var_clm_where_condition")
        ...
        with TaskGroup(group_id=f'run_dataflow_jobs') as dataflow_group:
            ...
            for script, target_table in zip(sql_label, tgttablename):
                ...
                #logging.info(f"Where Condition: {where_condition}") # Format the script with the where_condition (passing it as a variable) 
                #formatted_script = script.replace("{where_condition}", where_condition) 
                #logging.info(f"Sql query: {formatted_script} target_table: {target_table}")
