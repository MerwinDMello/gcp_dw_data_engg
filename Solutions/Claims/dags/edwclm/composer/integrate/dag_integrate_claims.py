...
def create_dag(dag_id, schedule, start_date, dependency, source_system, frequency, tags, done_file,trigger_dag_ids):
    dag = DAG(
        ...
        catchup=False, 
        max_active_runs=1, 
        template_searchpath=f'/home/airflow/gcs/dags/{lob}/sql/',
        tags=tags
    )
...
for integrate in integrate_config['integrate']:
    ...
    if frequency == "None":
        dag_id = f'dag_integrate_{source_system}_{type}_adhoc_{dag_name_suffix}'
        tags.extend(['adhoc', lob_abbr])
    else:
        dag_id = f'dag_integrate_{source_system}_{type}_{frequency}_{dag_name_suffix}'
        tags.extend([frequency, lob_abbr])

    globals()[dag_id] = create_dag(dag_id, schedule, start_date, dependency, source_system, frequency, tags, done_file,trigger_dag_ids)