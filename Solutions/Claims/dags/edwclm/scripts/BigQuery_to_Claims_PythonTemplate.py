...
class setuprunnerenv(beam.DoFn):
    def process(self, context):
        jdkfile = claims_config['v_jdkfile']
        gcsjarbucket = config['env']['v_dfjarbucket']
        jdkversion = claims_config['v_jdkversion']
        jdbcjar = claims_config['v_jdbc_jar']
...
class bqtojdbc(beam.DoFn):
    ...
    def readbqwriteclaimstable(self, sql_tgttablename):
        ...
        jdbc_class_name = claims_config['v_jdbc_class_name']
        jdbc_jar = claims_config['v_jdbc_jar']

        creds = json.loads(access_secret(config['env']['v_secrets_url'] + claims_config['v_secret_name']))

        logging.info(creds)
        username = creds["user"]
        passwd = creds["password"]
        src_server_name = creds["server"]
        src_db_name = claims_config['v_database']
        src_db_port = claims_config['v_port']
        src_db_type = claims_config['v_databasetype']
        ...
        tableinfo = claims_config[src_db_sql]
        ...
        if sql_task=="truncate":
            ...
            try:
                ...
                conn.commit()
                logging.info(f"{sql_tgttablename}' truncated in claims")
            except Exception as e:
                logging.error('Could not truncate table')
                exit
            
            ## WRITE TO claims DB IN BATCHES
            #cursor = conn.cursor()
            ...
            for start in range(0, len(df), batch_size):
                ...
                try:
                    ...
                    conn.commit()
                    logging.info(f"{len(batched_data)} rows loaded to claims")
                ...
                logging.info('Data written to claims successfully')
        ...
        if sql_task=="update":
            ...
            try:
                ...
                conn.commit()
                logging.info(f"{sql_tgttablename}' updated in claims")
            except Exception as e:
...
def run():
    pipeline_args = [
        ...
        "--worker_machine_type", claims_config['v_machine_type'],
...
def main(sql_tgttablename):
    ...
    if srctablename :
        jobname ="clm-outbound-load-to-claims" + '-' + srctablename.lower() + '-' + '-' + time.strftime("%Y%m%d%H%M%S") 
    else :
        jobname ="clm-outbound-load-to-claims" + '-' + sql_tgttablename.lower() + '-' + '-' + time.strftime("%Y%m%d%H%M%S") 
...
if __name__ =="__main__" :
    ...
    global claims_config    
    claims_config = call_config_yaml(config_folder,args.src_sys_config_file)
