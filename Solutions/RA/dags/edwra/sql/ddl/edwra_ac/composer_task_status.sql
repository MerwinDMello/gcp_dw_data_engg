CREATE OR REPLACE VIEW `{{ params.param_parallon_cur_project_id }}.edwpara_ac.composer_task_status`
AS
SELECT
 'Parallon' AS domain,
 project_id,
 ...
 user_id,
 CASE WHEN dag_name LIKE '%ra_%oracle%' THEN 'concuity'
WHEN dag_name LIKE '%ra_sqlserver_webtools_denials%' THEN 'webtools_denials'
WHEN dag_name LIKE '%ra_sqlserver_webtools%' THEN 'webtools'
WHEN dag_name LIKE '%ra_sqlserver_path_issue%' THEN 'path_issue'
WHEN dag_name LIKE '%ra_sqlserver_crt%' THEN 'crt'
WHEN dag_name LIKE '%ra_paraedw_%'  THEN 'ra_paraedw'
ELSE
  CASE
...
(
 SELECT
  ...
  task_name,
  execution_date,
  MIN(start_ts) OVER (PARTITION BY project_id, resource_type, user_id, dag_name, task_name,execution_date, table_suffix_dt ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS start_ts,
  CASE job_status
   WHEN 3
   THEN end_ts
  END
  ...
  AS job_status,
  CASE
   WHEN MAX(job_status) OVER (PARTITION BY project_id, resource_type, user_id, dag_name, task_name, execution_date, table_suffix_dt ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) IN (2,3) THEN False
   ELSE True
   END
 ...
 FROM
  ...
  SELECT
   ...
   DATETIME(CASE
    ...
    table_suffix_dt
  FROM
   `{{ params.param_auth_base_views_dataset_name }}.airflow_worker`
  WHERE
