CREATE OR REPLACE VIEW {{ params.param_parallon_ra_views_dataset_name }}.edr_export AS
...
select 
  ...
  fn.iplan_id ,
  fn.To_Dept ,
  fn.unbilled_reason_code ,
  fn.note_date_time
from(
select
  rf.ssi_queue_dept_desc as From_Dept ,
  r.ssi_queue_dept_desc as To_Dept ,
  ep.*
from {{ params.param_parallon_pbs_base_views_dataset_name }}.erequest_productivity_dtl ep
  left join {{ params.param_parallon_pbs_base_views_dataset_name }}.ref_ssi_responsible_dept_pf r on r.ssi_queue_dept_id = ep.to_queue_dept_id
  left join {{ params.param_parallon_pbs_base_views_dataset_name }}.ref_ssi_responsible_dept_pf rf on rf.ssi_queue_dept_id = ep.from_queue_dept_id
where ep.unbilled_reason_code = 'HDUO'
qualify row_number() over(partition by ep.patient_dw_id, ep.iplan_id order by ep.note_date_time desc) = 1 ) fn
where fn.To_Dept = 'Net AR MA Automation Review' ) AS erequest ON erequest.patient_dw_id = ccap.patient_dw_id
AND erequest.iplan_id = ccap.iplan_id
