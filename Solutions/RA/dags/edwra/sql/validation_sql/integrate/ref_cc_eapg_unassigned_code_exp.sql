...
SET srctablename = concat(srcdataset_id, '.','code_def' ); -- This needs to be added
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(eapg_unassigned_code) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_eapg_unassigned_code
       WHERE ref_cc_eapg_unassigned_code.dw_last_update_date_time >=
