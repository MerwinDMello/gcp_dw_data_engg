...
SET srctablename = concat(srcdataset_id, '.','sec_establishment' ); -- This needs to be added
...
select count(*) as row_count
   ...
   WHERE se.level_no = 9
     ...
     AND seo.eff_dt_end >= current_date('US/Central') ) a
);
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(org_id) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_org_structure
        WHERE ref_cc_org_structure.dw_last_update_date_time >=
