...
SET srctablename = concat(srcdataset_id, '.','mon_status_category' ); -- This needs to be added
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(status_category_id) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_status_category
       WHERE ref_cc_status_category.dw_last_update_date_time >=
