...
SET srctablename = concat(srcdataset_id, '.','cc_account_occur_span_code' ); -- This needs to be added
...
select count(*) as row_count
   ...
   WHERE upper(rtrim(upper(pv.group_id))) = 'NONCLINICAL_CODE_CATEGORY'
     AND upper(rtrim(upper(pv.display_text))) = 'OCCURRENCE SPAN CODES') a
);
...
FROM
  (SELECT count(*) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.cc_account_occur_span_code) AS a -- This needs to be added
);
