...
SET srctablename = concat(srcdataset_id, '.','reimbursement_discrepancy_wrk1' ); -- This needs to be added
...
FROM {{ params.param_parallon_ra_stage_dataset_name }}.reimbursement_discrepancy_wrk1) a
);
...
FROM
  (SELECT count(*) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.cc_reimbursement_discrepancy
     WHERE cc_reimbursement_discrepancy.dw_last_update_date_time >=
