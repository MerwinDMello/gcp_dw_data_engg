...
SET srctablename = concat(srcdataset_id, '.','apg_grpr_output' ); -- This needs to be added
...
FROM
  (
select count(*) as row_count FROM ( SELECT DISTINCT patient_dw_id, apg_grouper_num
...
FROM
  (SELECT count(*) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.cc_eor_eapg_grouper
        WHERE cc_eor_eapg_grouper.dw_last_update_date_time >=
