...
SET srctablename = concat(srcdataset_id, '.','mon_transaction_master' ); -- This needs to be added
...
select count(*) as row_count
   ...
   WHERE apt.effective_end_date > current_date('US/Central') ) a
);
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(transaction_code) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_fin_transaction
        WHERE ref_cc_fin_transaction.dw_last_update_date_time >=
