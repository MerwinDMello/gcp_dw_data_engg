...
SET srctablename = concat(srcdataset_id, '.','ce_service' ); -- This needs to be added
...
FROM {{ params.param_parallon_ra_stage_dataset_name }}.ce_service AS ces
   INNER JOIN {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_org_structure AS rccos ON rccos.schema_id = ces.schema_id) a
);
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(ce_service_id) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_ce_service
       WHERE ref_cc_ce_service.dw_last_update_date_time >=
