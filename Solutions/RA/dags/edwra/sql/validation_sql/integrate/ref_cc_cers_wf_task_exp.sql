...
SET srctablename = concat(srcdataset_id, '.','cers_workflow_task' ); -- This needs to be added
...
select count(*) as row_count
   ...
   INNER JOIN {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_org_structure AS rccos ON rccos.schema_id = cwt.schema_id) a
);
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(cers_wf_task_id) AS ROW_COUNT
