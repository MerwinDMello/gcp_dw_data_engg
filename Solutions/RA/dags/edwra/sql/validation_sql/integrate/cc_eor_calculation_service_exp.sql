...
SET srctablename = concat(srcdataset_id, '.','mon_account_payer_calc_latest' ); -- This needs to be added
...
FROM {{ params.param_parallon_ra_stage_dataset_name }}.mon_account_payer_calc_latest AS mapcl
   ...
   AND cdr.cers_term_id = CASE
                          ...
                          END
   AND upper(rtrim(cdr.drg_type)) = upper(rtrim(mapcl.drg_type))
   AND upper(rtrim(cdr.drg)) = upper(rtrim(mapcl.drg))
   LEFT OUTER JOIN
 ...
 mapc_apc.apc_outlier_percent AS apc_outlier_pct
   ...
   AND coin_fs.ce_service_id = mapcsvc.ce_service_id) a
);
...
FROM
  (SELECT count(*) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.cc_eor_calculation_service
     WHERE cc_eor_calculation_service.dw_last_update_date_time >=
