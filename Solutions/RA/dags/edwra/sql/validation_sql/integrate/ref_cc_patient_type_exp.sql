...
SET srctablename = concat(srcdataset_id, '.','mon_patient_type' ); -- This needs to be added
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(patient_type_id) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_patient_type
        WHERE ref_cc_patient_type.dw_last_update_time >=
