...
SET srctablename = concat(srcdataset_id, '.','path_issue' ); -- This needs to be added
...
FROM {{ params.param_parallon_ra_stage_dataset_name }}.path_issue) a
);
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(issue_information_id) AS ROW_COUNT-- count(*) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_path_issue
       WHERE ref_path_issue.dw_last_update_date_time >=
