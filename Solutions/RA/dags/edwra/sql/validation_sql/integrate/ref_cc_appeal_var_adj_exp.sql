...
SET srctablename = concat(srcdataset_id, '.','apl_variance_adj' ); -- This needs to be added
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(company_code) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_appeal_var_adj
        WHERE ref_cc_appeal_var_adj.dw_last_update_date_time >=
