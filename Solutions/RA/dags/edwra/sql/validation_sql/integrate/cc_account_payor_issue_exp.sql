...
SET srctablename = concat(srcdataset_id, '.','acc_pyr_issue' ); -- This needs to be added
...
FROM
  (SELECT count(coid) AS ROW_COUNT-- count(*) AS ROW_COUNT
   ...
   WHERE cc_account_payor_issue.dw_last_update_date_time >=
