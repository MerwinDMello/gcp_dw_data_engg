...
SET srctablename = concat(srcdataset_id, '.','mon_account_transaction' ); -- This needs to be added
...
FROM
  (
    select count(*) as row_count from (
      select count(*)  FROM {{ params.param_parallon_ra_stage_dataset_name }}.mon_account_transaction AS mat
...
mat.snapshot_mon_status_id,
       mat.snapshot_mon_reason_id,
       mat.mon_accounting_period_id,
       mat.snapshot_appeal_no,
       mat.snapshot_appeal_seq_no,
         upper(CASE
...
FROM
  (SELECT count(*) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.cc_financial_transaction
        WHERE cc_financial_transaction.dw_last_update_date_time >=
   (SELECT coalesce(max(audit_control.load_end_time), date_add(timestamp_trunc(current_datetime('US/Central'), SECOND), INTERVAL -1 DAY))
	...
	WHERE upper(audit_control.tgt_tbl_nm) = upper(tgttablename)
