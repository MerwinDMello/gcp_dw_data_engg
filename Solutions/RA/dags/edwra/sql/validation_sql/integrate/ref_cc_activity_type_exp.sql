...
SET srctablename = concat(srcdataset_id, '.','mon_activity_type' ); -- This needs to be added
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(activity_type_id) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_activity_type
        WHERE ref_cc_activity_type.dw_last_update_date_time >=
