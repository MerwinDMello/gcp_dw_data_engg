...
SET srctablename = concat(srcdataset_id, '.','payor_organization' ); -- This needs to be added

SET tgtdataset_id = (select arr[offset(1)] from (select split("{{ params.param_parallon_ra_stage_dataset_name }}" , '.') as arr));
...
FROM {{ params.param_parallon_ra_stage_dataset_name }}.payor_organization AS po
   WHERE EXISTS
       (SELECT 1
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(coid) as ROW_COUNT--count(*) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_stage_dataset_name }}.payor_organization
  --       WHERE payor_organization.dw_last_update_time >=
