...
SET srctablename = concat(srcdataset_id, '.','apl_appeal' ); -- This needs to be added
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(appeal_code_id) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_appeal_code
        WHERE ref_cc_appeal_code.dw_last_update_date_time >=
