...
SET srctablename = concat(srcdataset_id, '.','mon_accounting_period' ); -- This needs to be added
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(financial_period_id) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_fin_acct_period
       WHERE ref_cc_fin_acct_period.dw_last_update_date_time >=
