...
SET srctablename = concat(srcdataset_id, '.','ra_group_header' ); -- This needs to be added

SET tgtdataset_id = (select arr[offset(1)] from (select split("{{ params.param_parallon_ra_core_dataset_name }}" , '.') as arr));

SET tgttablename =concat(tgtdataset_id, '.', 'cc_remittance_advice');

SET tableload_start_time = '2024-03-13 16:11:12.727926';

SET tableload_end_time = '2024-03-13 16:11:47.035106';

SET tableload_run_time = CAST((tableload_end_time - tableload_start_time) AS STRING);

SET audit_job_name = 'TG-table-cc_remittance_advice.run_cc_remittance_advice.sql';
...
FROM
  (
      select count(*) as row_count from (select count(*) FROM {{ params.param_parallon_ra_stage_dataset_name }}.ra_group_header AS ragh
...
FROM
  (SELECT count(*) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.cc_remittance_advice
        WHERE cc_remittance_advice.dw_last_update_date_time >=
