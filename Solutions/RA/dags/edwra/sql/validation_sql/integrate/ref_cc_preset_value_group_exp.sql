...
SET srctablename = concat(srcdataset_id, '.','preset_value' ); -- This needs to be added
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(preset_value_id) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_preset_value_group
       WHERE ref_cc_preset_value_group.dw_last_update_date_time >=
