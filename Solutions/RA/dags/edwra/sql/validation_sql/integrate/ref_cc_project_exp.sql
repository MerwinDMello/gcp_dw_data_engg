...
SET srctablename = concat(srcdataset_id, '.','mon_project' ); -- This needs to be added
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(project_id) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_project
       WHERE ref_cc_project.dw_last_update_date_time >=
