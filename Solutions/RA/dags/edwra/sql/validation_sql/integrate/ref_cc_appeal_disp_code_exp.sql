...
SET srctablename = concat(srcdataset_id, '.','apl_disposition' ); -- This needs to be added
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(apl_disp_code_id) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_appeal_disp_code
        WHERE ref_cc_appeal_disp_code.dw_last_update_date_time >=
