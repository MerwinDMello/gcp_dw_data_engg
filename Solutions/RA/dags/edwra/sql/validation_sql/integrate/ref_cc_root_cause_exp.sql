...
SET srctablename = concat(srcdataset_id, '.','apl_root_cause' ); -- This needs to be added
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(root_cause_id) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_root_cause
       WHERE ref_cc_root_cause.dw_last_update_date_time >=
