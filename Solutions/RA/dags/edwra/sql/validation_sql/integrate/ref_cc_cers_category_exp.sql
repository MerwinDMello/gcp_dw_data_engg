...
SET srctablename = concat(srcdataset_id, '.','ce_rs_category' ); -- This needs to be added
...
select count(*) as row_count
  FROM {{ params.param_parallon_ra_stage_dataset_name }}.ce_rs_category AS crsc
   INNER JOIN {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_org_structure AS rccos ON rccos.schema_id = crsc.schema_id) a
);
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(cers_category_id) AS ROW_COUNT
