...
SET srctablename = concat(srcdataset_id, '.','mon_status' ); -- This needs to be added
...
select count(*) as row_count
  FROM {{ params.param_parallon_ra_stage_dataset_name }}.mon_status
   ...
   WHERE mon_status.mon_stat_category_id = mon_status_category.id ) a
);
...
FROM
  (SELECT count(*) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_claim_status_build
       WHERE ref_claim_status_build.dw_last_update_date_time >=
