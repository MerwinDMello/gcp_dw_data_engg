...
SET srctablename = concat(srcdataset_id, '.','mon_accounting_cost_period' ); -- This needs to be added
...
FROM {{ params.param_parallon_ra_stage_dataset_name }}.mon_accounting_cost_period AS macp
   ...
   AND macp.cost_report_org_id = ros.org_id) a
);
...
SELECT [format('%20d', a.row_count)]
FROM
  (SELECT count(cost_period_id) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.ref_cc_cost_report_period
      WHERE ref_cc_cost_report_period.dw_last_update_date_time >=
