...
SET srctablename = concat(srcdataset_id, '.','mon_account_nonclinical_code' ); -- This needs to be added
...
FROM {{ params.param_parallon_ra_stage_dataset_name }}.mon_account_nonclinical_code AS mancc
   ...
   WHERE upper(rtrim(upper(pv.group_id))) = 'NONCLINICAL_CODE_CATEGORY'
     AND upper(rtrim(upper(pv.display_text))) = 'OCCURRENCE CODES' ) a
);
...
FROM
  (SELECT count(*) AS ROW_COUNT
   FROM {{ params.param_parallon_ra_core_dataset_name }}.cc_account_occur_code
    WHERE cc_account_occur_code.dw_last_update_date_time >=
