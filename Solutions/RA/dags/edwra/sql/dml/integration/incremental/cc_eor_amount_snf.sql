...
-- Source: eim-parallon-cs-datamig-dev-0002/ra_bteqs_bulk_conversion/ehYakm/input/cc_eor_amount_snf.sql
...
*****************************************************************************/ -- CALL dbadmin_procs.SET_QUERY_BAND('App=RA_Group2_ETL;Job=CTDRA147_SNF;');
...
MERGE INTO {{ params.param_parallon_ra_core_dataset_name }}.cc_eor_amount AS x USING
  (SELECT s.company_code,
   ...
   WHERE upper(trim(s.rate_schedule)) = 'MSNF'
   ...
   CROSS JOIN UNNEST(ARRAY[ CAST(ROUND(CASE
                                           WHEN s.cob_amount = 0 THEN s.tot_expected_payment + s.passthrough_amount + s.acct_subterm_amt + s.map_total_exp_payment - (s.mapcl_base_exp_payment + s.mapcl_exclusion_exp_payment + s.map_sequestration_amt)
                                           ELSE s.tot_expected_payment + s.passthrough_amount + s.acct_subterm_amt + s.cob_amount
   ...
   UNION ALL SELECT s.company_code,
                    ...
                    s.eor_pat_acct_num,
                    s.eor_iplan_id,
                    s.calc_fee_schedule_adj AS eor_reimbursement_amt,
                    datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time,
                    'N' AS source_system_code
   FROM {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_calc_service AS s
   WHERE upper(trim(s.rate_schedule)) = 'MSNF'
     AND s.calc_fee_schedule_adj <> 0 ) AS z ON upper(trim(x.company_code)) = upper(trim(z.company_code))
