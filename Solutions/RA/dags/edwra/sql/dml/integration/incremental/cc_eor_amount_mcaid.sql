...
-- Source: eim-parallon-cs-datamig-dev-0002/ra_bteqs_bulk_conversion/ehYakm/input/cc_eor_amount_mcaid.sql
...
*****************************************************************************/ -- CALL dbadmin_procs.SET_QUERY_BAND('App=RA_Group2_ETL;Job=CTDRA147_MCAID;');
...
MERGE INTO {{ params.param_parallon_ra_core_dataset_name }}.cc_eor_amount AS x USING
  (SELECT s.company_code,
          ...
          s.eor_pat_acct_num,
          s.eor_iplan_id,
          s.mapcl_tot_exp_payment AS eor_reimbursement_amt,
          datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time,
          'N' AS source_system_code
   FROM {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_calc_service AS s
   WHERE upper(trim(s.rate_schedule)) IN('MCDIP',
                                          'MCDOP')
     AND s.mapcl_tot_exp_payment <> 0 ) AS z ON upper(trim(x.company_code)) = upper(trim(z.company_code))
