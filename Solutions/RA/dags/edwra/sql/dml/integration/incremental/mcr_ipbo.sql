...
-- Source: gs://eim-parallon-cs-datamig-dev-0002/ra_ddls_bulk_conversion/nm59fE/input/mcr_ipbo.sql
...
---- bteq << EOF > /etl/ST/MC/CC_EDW/TgtFiles/MCR-IPBO.out;;
/*
...
WHERE upper(trim(gr_gl_recn.log_type, ' ')) = 'MCR-IPB'
...
MERGE INTO `{{ params.param_parallon_ra_core_dataset_name }}`.gr_gl_recn AS mt USING
  (SELECT DISTINCT 'H' AS company_code,
                   max(cclog24mcripb.coid) AS coid,
                   parse_date('%m/%d/%y',REGEXP_EXTRACT(SPLIT(cclog24mcripb.reporting_period, " -")[0], r'[0-9]+/[0-9]+/[0-9]+')) AS cost_report_year_begin,
                   parse_date('%m/%d/%y',REGEXP_EXTRACT(SPLIT(cclog24mcripb.reporting_period, " -")[1], r'[0-9]+/[0-9]+/[0-9]+')) AS cost_report_year_end,
                   substr(max(CASE
                                  WHEN upper(cclog24mcripb.rate_schedule_name) LIKE '%GOV IPBO%' THEN 'MCR-IPB'
                   ...
                   TRIM(substr(max(cclog24mcripb.patient_name), 1, 100)) AS pat_last_name,
                   datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time,
                   'N' AS source_system_code,
                   '501502' AS gl_acct_num, -- -Max(cast(cast(Disch_Date as timestamp(0)) as date format 'YYYY-MM-DD'))AS discharge_date,
 max(CASE
         WHEN cclog24mcripb.disch_date LIKE '%-%' THEN parse_date('%Y-%m-%d',REGEXP_EXTRACT(cclog24mcripb.disch_date, r'[0-9]+-[0-9]+-[0-9]+'))
         WHEN cclog24mcripb.disch_date LIKE '%/%' THEN parse_date('%Y/%m/%d',REGEXP_EXTRACT(cclog24mcripb.disch_date, r'[0-9]+/[0-9]+/[0-9]+'))
         ELSE NULL
     END) AS discharge_date,
 CAST(ROUND(sum(cclog24mcripb.expected_contractual), 3, 'ROUND_HALF_EVEN') AS NUMERIC) AS total_ca_log_24_adj_amt,
 max(CASE
         WHEN cclog24mcripb.account_number LIKE '%*%' THEN 'Y'
         ELSE 'N'
     END) AS interim_bill_ind,
 max(CASE
         WHEN upper(rtrim(cclog24mcripb.section_name, ' ')) = '4TH DAY DELAY' THEN 'Y'
         ELSE 'N'
     END) AS fourth_day_delay_ind
   FROM `{{ params.param_parallon_ra_stage_dataset_name }}`.cclog24mcripb
   WHERE cclog24mcripb.expected_contractual <> 0
     AND upper(cclog24mcripb.file_type) LIKE 'CCLOG24MCRIPB_D%'
   GROUP BY upper(cclog24mcripb.coid),
            3,
            4,
            upper(substr(CASE
                             WHEN upper(cclog24mcripb.rate_schedule_name) LIKE '%GOV IPBO%' THEN 'MCR-IPB'
                         END, 1, 10)),
            6,
            upper(substr(cclog24mcripb.patient_name, 1, 100)),
...
MERGE INTO `{{ params.param_parallon_ra_core_dataset_name }}`.gr_gl_recn AS b USING
  (SELECT max(cclog65mcripb.coid) AS coid,
          max(CASE
          ...
          parse_date('%m/%d/%y',REGEXP_EXTRACT(SPLIT(cclog65mcripb.reporting_period, " -")[0], r'[0-9]+/[0-9]+/[0-9]+')) AS costreportyearbegin,
          '501502' AS gl_acct_num,
          'H' AS company_code,
          sum(cclog65mcripb.var_contractual) AS var_contractual
   FROM `{{ params.param_parallon_ra_stage_dataset_name }}`.cclog65mcripb
   WHERE cclog65mcripb.var_contractual <> 0
     AND upper(cclog65mcripb.report_name) LIKE 'CCLOG65%'
   GROUP BY upper(cclog65mcripb.coid),
            upper(CASE
                      WHEN cclog65mcripb.patient_number LIKE '%*%' THEN translate(cclog65mcripb.patient_number, '*', '')
                      ELSE cclog65mcripb.patient_number
                  END),
...
MERGE INTO `{{ params.param_parallon_ra_core_dataset_name }}`.gr_gl_recn AS mt USING
  (SELECT DISTINCT 'H' AS company_code,
                   ...
                   substr(max(CASE
                                  WHEN upper(a.rate_schedule_name) LIKE '%GOV IPBO%' THEN 'MCR-IPB'
                              END), 1, 10) AS log_type,
                   ROUND(CAST(`{{ params.param_bqutil_fns_dataset_name }}`.cw_td_normalize_number(CASE
 ...
 'N' AS fourth_day_delay_ind
   FROM `{{ params.param_parallon_ra_stage_dataset_name }}`.cclog65mcripb AS a
   WHERE a.var_contractual <> 0
     AND upper(a.report_name) LIKE 'CCLOG65%'
     AND NOT EXISTS
       (SELECT 1
        FROM `{{ params.param_parallon_ra_stage_dataset_name }}`.cclog24mcripb AS b
        WHERE b.expected_contractual <> 0
          AND upper(b.file_type) LIKE 'CCLOG24MCRIPB_D%'
          AND upper(rtrim(b.coid, ' ')) = upper(rtrim(a.coid, ' '))
...
MERGE INTO `{{ params.param_parallon_ra_core_dataset_name }}`.gr_gl_recn AS b USING
 ...
 sum(gl.gl_proc_amt) AS total_gl_posted_acct_amt
   ...
   WHERE gl.gl_account = 501502
     ...
     AND upper(rtrim(gr.log_type, ' ')) = 'MCR-IPB'
...
MERGE INTO `{{ params.param_parallon_ra_core_dataset_name }}`.gr_gl_recn AS b USING
 ...
 gl.pat_acct_num,
 max(CASE
         WHEN rtrim(ltrim(CAST(gl.gl_account AS STRING)), ' ') = '110115' THEN '501502'
