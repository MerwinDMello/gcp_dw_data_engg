...
-- Source: eim-parallon-cs-datamig-dev-0002/ra_bteqs_bulk_conversion/ehYakm/input/cc_account_occur_code.sql
...
/***************************************************************************
 Developer: Sean Wilson
      Name: CC_Account_Occur_Code - BTEQ Script.
      Mod1: Creation of script on 8/11/2011. SW.
      Mod2: Modified script for new DDL on 9/6/2011. SW.
      Mod3: Modified script for 9.7 new tables on 3/6/2012 SW.
...
**************************************************************************************************/ -- CALL dbadmin_procs.SET_QUERY_BAND('App=RA_Group2_ETL;Job=CTDRA225;');
...
TRUNCATE TABLE {{ params.param_parallon_ra_core_dataset_name }}.cc_account_occur_code;
...
MERGE INTO {{ params.param_parallon_ra_core_dataset_name }}.cc_account_occur_code AS x USING
  (SELECT a.patient_dw_id,
          CAST(mancc.payer_rank AS INT64) AS occur_code_seq,
          ...
          mancc.code AS occur_code,
          mancc.code_date AS occur_code_date
   ...
   WHERE upper(rtrim(upper(pv.group_id))) = 'NONCLINICAL_CODE_CATEGORY'
     AND upper(rtrim(upper(pv.display_text))) = 'OCCURRENCE CODES' ) AS z ON x.patient_dw_id = z.patient_dw_id
AND x.occur_code_seq = z.occur_code_seq WHEN MATCHED THEN
UPDATE
SET company_code = z.company_cd,
    coid = substr(z.coido, 1, 5),
    unit_num = substr(z.unit_num, 1, 5),
    pat_acct_num = z.pat_acct_nbr,
    occur_code = substr(z.occur_code, 1, 2),
    occur_code_date = z.occur_code_date,
...
INSERT (patient_dw_id,
        ...
        source_system_code)
VALUES (z.patient_dw_id, z.occur_code_seq, z.company_cd, substr(z.coido, 1, 5), substr(z.unit_num, 1, 5), z.pat_acct_nbr, substr(z.occur_code, 1, 2), z.occur_code_date, datetime_trunc(current_datetime('US/Central'), SECOND), 'N');
  ...
  (SELECT count(*)
   FROM
     (SELECT patient_dw_id,
             occur_code_seq
      FROM {{ params.param_parallon_ra_core_dataset_name }}.cc_account_occur_code
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_parallon_ra_core_dataset_name }}.cc_account_occur_code');
...
-- CALL dbadmin_procs.collect_stats_table('edwra','CC_Account_Occur_Code');
 IF _ERROR_CODE <> 0 THEN RAISE USING MESSAGE = concat('Script terminated with return code: ', _ERROR_CODE, ' ', _ERROR_MSG);
