...
-- Source: eim-parallon-cs-datamig-dev-0002/ra_bteqs_bulk_conversion/ehYakm/input/cc_eor_amount_mirf.sql
...
/****************************************************************************
  Developer: Holly Ray
	...
	Mod6:Code for Amount_Category_Code =142 has been removed and added for Amount_Category_Code=144.added logic to pull data for TCRIF as well in this script,
	refer PBI 18374 and PBI 20158 1/4/2019 PT
*****************************************************************************/ -- CALL dbadmin_procs.SET_QUERY_BAND('App=RA_Group2_ETL;Job=CTDRA147_MIRF;');
...
MERGE INTO {{ params.param_parallon_ra_core_dataset_name }}.cc_eor_amount AS x USING
  (SELECT s.company_code,
          ...
          s.eor_pat_acct_num,
          s.eor_iplan_id,
          o.calc_irf_wage_adj AS eor_reimbursement_amt,
   ...
   WHERE (upper(trim(s.rate_schedule)) = 'MIRF'
          OR upper(trim(s.rate_schedule)) = 'TCRIF')
     AND o.calc_irf_wage_adj <> 0
   UNION ALL SELECT s.company_code,
                    ...
                    s.eor_pat_acct_num,
                    s.eor_iplan_id,
                    o.calc_irf_outlier_payment AS eor_reimbursement_amt,
                    datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time,
                    'N' AS source_system_code
   FROM {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_calc_service AS s
   INNER JOIN {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_other_service AS o ON o.mon_account_payer_id = s.mon_account_payer_id
   ...
   WHERE (upper(trim(s.rate_schedule)) = 'MIRF'
          OR upper(trim(s.rate_schedule)) = 'TCRIF')
     AND o.calc_irf_outlier_payment <> 0
   UNION ALL SELECT s.company_code,
                    s.coid,
                    s.patient_dw_id,
                    s.payor_dw_id,
   ...
   UNION ALL SELECT s.company_code,
                    ...
                    s.eor_pat_acct_num,
                    s.eor_iplan_id,
                    o.calc_irf_teaching_adj AS eor_reimbursement_amt,
                    datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time,
                    'N' AS source_system_code
   FROM {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_calc_service AS s
   INNER JOIN {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_other_service AS o ON o.mon_account_payer_id = s.mon_account_payer_id
   ...
   UNION ALL SELECT s.company_code,
                    ...
                    s.eor_pat_acct_num,
                    s.eor_iplan_id,
                    o.calc_irf_lip_adj AS eor_reimbursement_amt,
                    datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time,
                    'N' AS source_system_code
   FROM {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_calc_service AS s
   INNER JOIN {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_other_service AS o ON o.mon_account_payer_id = s.mon_account_payer_id
   ...
   UNION ALL SELECT s.company_code,
                    ...
                    'N' AS source_system_code
   FROM {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_calc_service AS s
   CROSS JOIN UNNEST(ARRAY[ ROUND(s.calc_fee_schedule_adj, 2, 'ROUND_HALF_EVEN') ]) AS eor_reimbursement_amt
   WHERE (upper(trim(s.rate_schedule)) = 'MIRF'
          OR upper(trim(s.rate_schedule)) = 'TCRIF')
