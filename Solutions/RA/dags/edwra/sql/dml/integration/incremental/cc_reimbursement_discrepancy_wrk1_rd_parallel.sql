...
-- Source: eim-parallon-cs-datamig-dev-0002/ra_bteqs_bulk_conversion/ehYakm/input/cc_reimbursement_discrepancy_wrk1_rd_parallel.sql
...
/****************************************************************************************************************************
  Developer: Ray Holly
       ...
       Mod8: PBI 18545: Added a reference to the ETL_LOAD table so that the origination date and dw_last_update_date_time columns are no longer the current_date and current_timestamp(0) values, respectively. 10/9/2018 BC.
****************************************************************************************************************************/ -- SET QUERY_BAND = 'App=RA_Group2_ETL;Job=CTDRA234_Wrk1;' FOR SESSION;
 /*DELETE FROM  Edwra_Staging.Reimbursement_Discrepancy_Wrk1;

INSERT INTO  Edwra_Staging.Reimbursement_Discrepancy_Wrk1*/ BEGIN
SET _ERROR_CODE = 0;

TRUNCATE TABLE  {{ params.param_parallon_ra_stage_dataset_name }}.reimbursement_discrepancy_wrk1_rd_parallel;
...
MERGE INTO  {{ params.param_parallon_ra_stage_dataset_name }}.reimbursement_discrepancy_wrk1_rd_parallel AS mt USING
  (SELECT DISTINCT e1.company_code AS company_code,
                   ...
                   coalesce(rd_date_from_etl_load.rd_date, current_date('US/Central')) AS discrepancy_origination_date, -- PBI 18545: We are pulling this from the rd_date_from_etl_load subquery below.  If a null is returned, the current date is used.
 ...
 coalesce(rd_date_from_etl_load.rd_timestamp, datetime_trunc(current_datetime('US/Central'), SECOND)) AS dw_last_update_date_time, -- PBI 18545: Because the PF team uses the dw_last_update_date_time column to derive the resolved date, we are pulling this from the rd_date_from_etl_load subquery below.  If a null is returned, the current timestamp is used.
 'N' AS source_system_code
   ...
   CROSS JOIN -- PBI 18545: Using the RD origination date from the ETL_LOAD table

     (SELECT max(etl_load.etl_load_reimbursement_discrepancy_date) AS rd_date, -- Max is taken in case multiple rows where is_current = 1 are present
 CASE
 ...
 END AS rd_timestamp
  ...
  {{ params.param_parallon_ra_stage_dataset_name }}.etl_load
      WHERE etl_load.is_current = 1 ) AS rd_date_from_etl_load
  ...
  (SELECT count(*)
   FROM
     (SELECT company_code,
             ...
             log_id,
             log_sequence_num
      FROM  {{ params.param_parallon_ra_stage_dataset_name }}.reimbursement_discrepancy_wrk1_rd_parallel
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table `{{ params.param_parallon_ra_stage_dataset_name }}`.reimbursement_discrepancy_wrk1_rd_parallel');
