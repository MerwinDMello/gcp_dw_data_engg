...
-- Source: eim-parallon-cs-datamig-dev-0002/ra_bteqs_bulk_conversion/ehYakm/input/cc_eor_amount_tricop.sql
...
*****************************************************************************/ -- CALL dbadmin_procs.SET_QUERY_BAND('App=RA_Group2_ETL;Job=CTDRA147_TRICOP;');
...
MERGE INTO {{ params.param_parallon_ra_core_dataset_name }}.cc_eor_amount AS x USING
  (SELECT s.company_code,
   ...
   CROSS JOIN UNNEST(ARRAY[ s.comp_flg_exp_payment + o.tri_apc_exp_payment ]) AS eor_reimbursement_amt
   WHERE upper(trim(s.rate_schedule)) = 'TCROP'
   ...
   INNER JOIN {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_other_service AS o ON o.mon_account_payer_id = s.mon_account_payer_id
   AND o.schema_id = s.schema_id
   CROSS JOIN UNNEST(ARRAY[ s.comp_flg_apc_outlier_amount + o.tri_apc_outlier_payment ]) AS eor_reimbursement_amt
   WHERE upper(trim(s.rate_schedule)) = 'TCROP'
     AND eor_reimbursement_amt <> 0
   UNION ALL SELECT s.company_code,
                    s.coid,
   ...
   UNION ALL SELECT s.company_code,
                    ...
                    'N' AS source_system_code
   FROM {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_calc_service AS s
   CROSS JOIN UNNEST(ARRAY[ ROUND(s.srv_tricmac_exp_payment, 3, 'ROUND_HALF_EVEN') ]) AS eor_reimbursement_amt
   WHERE upper(trim(s.rate_schedule)) = 'TCROP'
     AND eor_reimbursement_amt <> 0
   UNION ALL SELECT s.company_code,
                    s.coid,
   ...
   INNER JOIN {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_other_service AS o ON o.mon_account_payer_id = s.mon_account_payer_id
   AND o.schema_id = s.schema_id
   CROSS JOIN UNNEST(ARRAY[ ROUND(s.tot_expected_payment - s.calc_atp_exp_payment - o.tri_comp_exp_payment - o.tri_apc_exp_payment - s.comp_flg_apc_outlier_amount - s.srv_tricmac_exp_payment, 3, 'ROUND_HALF_EVEN') ]) AS eor_reimbursement_amt
   WHERE upper(trim(s.rate_schedule)) = 'TCROP'
     AND eor_reimbursement_amt <> 0
   UNION ALL SELECT s.company_code,
                    s.coid,
   ...
   INNER JOIN {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_other_service AS o ON o.mon_account_payer_id = s.mon_account_payer_id
   AND o.schema_id = s.schema_id
   CROSS JOIN UNNEST(ARRAY[ ROUND(o.tri_apc_exp_payment + s.comp_flg_exp_payment - s.comp_flg_apc_coinsurance_amt - s.comp_flg_apc_deductible_amt, 3, 'ROUND_HALF_EVEN') ]) AS eor_reimbursement_amt
   WHERE upper(trim(s.rate_schedule)) = 'TCROP'
     AND eor_reimbursement_amt <> 0
   UNION ALL SELECT s.company_code,
                    s.coid,
...
DELETE
FROM {{ params.param_parallon_ra_core_dataset_name }}.cc_eor_amount
WHERE upper(cc_eor_amount.coid) IN
  ...
  AND DATE(cc_eor_amount.dw_last_update_date_time) <>
    (SELECT max(DATE(cc_eor_amount_0.dw_last_update_date_time))
     FROM {{ params.param_parallon_ra_core_dataset_name }}.cc_eor_amount AS cc_eor_amount_0);
