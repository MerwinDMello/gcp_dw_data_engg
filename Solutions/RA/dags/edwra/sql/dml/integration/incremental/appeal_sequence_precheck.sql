...
-- Source: eim-parallon-cs-datamig-dev-0002/ra_bteqs_bulk_conversion/ehYakm/input/appeal_sequence_precheck.sql
...
SET _ROW_COUNT =
  (SELECT count(*)
   FROM
     (SELECT max(cc_appeal_sequence.company_code) AS company_code,
             max(cc_appeal_sequence.coid) AS coid,
