...
/******************************************************************************
 Developer: PT
      Name: CC_Discrepancy_bthr BTEQ Script.
...
MERGE INTO {{ params.param_parallon_ra_core_dataset_name }}.cc_discrepancy_bthr AS x USING
 ...
 dis.last_remit_received_date AS last_remit_received_date 
   FROM --  PBI 24439
 {{ params.param_parallon_ra_stage_dataset_name }}.edw_daily_discrepancy_bthr AS dis 
...
DELETE FROM edwra.cc_discrepancy_bthr WHERE date_sub(current_date(), interval 400 DAY) > CAST( cc_discrepancy_bthr.extract_date_time as DATE)
 AND extract(DAY from CAST(cc_discrepancy_bthr.extract_date_time as TIMESTAMP)) > 6;
 
DELETE FROM edwra.cc_discrepancy_bthr WHERE cc_discrepancy_bthr.extract_date_time < date_add(current_date(), interval -13 MONTH);
  ...
  (SELECT count(*)
   FROM
     (SELECT patient_dw_id,
             ...
             iplan_order_num,
             extract_date_time
      FROM {{ params.param_parallon_ra_core_dataset_name }}.cc_discrepancy_bthr
...
RAISE USING MESSAGE = concat('Duplicates are not allowed in the table {{ params.param_parallon_ra_core_dataset_name }}.cc_discrepancy_bthr');
