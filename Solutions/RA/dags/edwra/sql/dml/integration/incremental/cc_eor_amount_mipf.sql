...
-- Source: eim-parallon-cs-datamig-dev-0002/ra_bteqs_bulk_conversion/ehYakm/input/cc_eor_amount_mipf.sql
...
/****************************************************************************
  Developer: Holly Ray
	...
	Mod6: Removed amt_cat_code =142 , modified logic for amt_cat_code =30 and added amt_cat_code =15,144 7/4/2019 AM
*****************************************************************************/ -- CALL dbadmin_procs.SET_QUERY_BAND('App=RA_Group2_ETL;Job=CTDRA147_MIPF;');
...
MERGE INTO {{ params.param_parallon_ra_core_dataset_name }}.cc_eor_amount AS x USING
  (SELECT s.company_code,
   ...
   WHERE upper(trim(s.rate_schedule)) = 'MIPF'
   ...
   CROSS JOIN UNNEST(ARRAY[ CAST(ROUND(CASE
                                           WHEN s.cob_amount = 0 THEN s.ipf_pps_adj_per_diem - (s.calc_fee_schedule_adj - s.srv_ect_exp_payment) + s.passthrough_amount + s.acct_subterm_amt + s.map_total_exp_payment - (s.mapcl_base_exp_payment + s.mapcl_exclusion_exp_payment + s.map_sequestration_amt)
                                           ELSE s.ipf_pps_adj_per_diem - (s.calc_fee_schedule_adj - s.srv_ect_exp_payment) + s.passthrough_amount + s.acct_subterm_amt + s.cob_amount
   ...
   UNION ALL SELECT s.company_code,
                    ...
                    s.eor_pat_acct_num,
                    s.eor_iplan_id,
                    s.ipf_pps_out_per_diem_add_on AS eor_reimbursement_amt,
                    datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time,
                    'N' AS source_system_code
   FROM {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_calc_service AS s
   WHERE upper(trim(s.rate_schedule)) = 'MIPF'
     AND s.ipf_pps_out_per_diem_add_on <> 0
   UNION ALL SELECT s.company_code,
                    s.coid,
                    s.patient_dw_id,
                    s.payor_dw_id,
   ...
   UNION ALL SELECT s.company_code,
                    ...
                    s.eor_pat_acct_num,
                    s.eor_iplan_id,
                    s.srv_ect_exp_payment AS eor_reimbursement_amt,
                    datetime_trunc(current_datetime('US/Central'), SECOND) AS dw_last_update_date_time,
                    'N' AS source_system_code
   FROM {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_calc_service AS s
   WHERE upper(trim(s.rate_schedule)) = 'MIPF'
     AND s.srv_ect_exp_payment <> 0
   UNION ALL SELECT s.company_code,
                    s.coid,
                    s.patient_dw_id,
                    s.payor_dw_id,
                    ...
                    'N' AS source_system_code
   FROM {{ params.param_parallon_ra_stage_dataset_name }}.eor_amount_stg_calc_service AS s
   CROSS JOIN UNNEST(ARRAY[ ROUND(s.calc_fee_schedule_adj - s.srv_ect_exp_payment, 3, 'ROUND_HALF_EVEN') ]) AS eor_reimbursement_amt
   WHERE upper(trim(s.rate_schedule)) = 'MIPF'
