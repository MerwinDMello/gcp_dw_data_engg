...
-- Source: gs://eim-parallon-cs-datamig-dev-0002/ra_ddls_bulk_conversion/nm59fE/input/mcrip.sql
...
---- bteq << EOF > /etl/ST/MC/CC_EDW/TgtFiles/MCRIP.out;;
/*
...
WHERE upper(trim(gr_gl_recn.log_type, ' ')) = 'MCR-IP';
...
MERGE INTO `{{ params.param_parallon_ra_core_dataset_name }}`.gr_gl_recn AS mt USING
  (SELECT DISTINCT 'H' AS company_code,
                   max(cclog24mcrip.coid) AS coid,
                   parse_date('%m/%d/%y',REGEXP_EXTRACT(SPLIT(cclog24mcrip.reporting_period, " -")[0], r'[0-9]+/[0-9]+/[0-9]+')) AS cost_report_year_begin,
                   parse_date('%m/%d/%y',REGEXP_EXTRACT(SPLIT(cclog24mcrip.reporting_period, " -")[1], r'[0-9]+/[0-9]+/[0-9]+')) AS cost_report_year_end,
                   substr(max(CASE
                                  WHEN upper(cclog24mcrip.rate_schedule_name) LIKE '%GOV MIPS IP%' THEN 'MCR-IP'
                   ...
                   TRIM(substr(max(cclog24mcrip.patient_name), 1, 100)) AS pat_last_name,
 ...
 max(CASE
         WHEN cclog24mcrip.disch_date LIKE '%-%' THEN parse_date('%Y-%m-%d',REGEXP_EXTRACT(cclog24mcrip.disch_date, r'[0-9]+-[0-9]+-[0-9]+'))
         WHEN cclog24mcrip.disch_date LIKE '%/%' THEN parse_date('%Y/%m/%d',REGEXP_EXTRACT(cclog24mcrip.disch_date, r'[0-9]+/[0-9]+/[0-9]+'))
         ELSE NULL
     END) AS discharge_date,
 CAST(ROUND(sum(cclog24mcrip.expected_contractual), 3, 'ROUND_HALF_EVEN') AS NUMERIC) AS total_ca_log_24_adj_amt,
 max(CASE
         WHEN cclog24mcrip.account_number LIKE '%*%' THEN 'Y'
         ELSE 'N'
     END) AS interim_bill_ind,
 max(CASE
         WHEN upper(rtrim(cclog24mcrip.section_name, ' ')) = '4TH DAY DELAY' THEN 'Y'
         ELSE 'N'
     END) AS fourth_day_delay_ind
   FROM `{{ params.param_parallon_ra_stage_dataset_name }}`.cclog24mcrip
   WHERE cclog24mcrip.expected_contractual <> 0
     AND upper(cclog24mcrip.file_type) LIKE 'CCLOG24MCRIP_D%'
   GROUP BY upper(cclog24mcrip.coid),
            3,
            4,
            upper(substr(CASE
                             WHEN upper(cclog24mcrip.rate_schedule_name) LIKE '%GOV MIPS IP%' THEN 'MCR-IP'
                         END, 1, 10)),
            6,
            upper(substr(cclog24mcrip.patient_name, 1, 100)),
...
MERGE INTO `{{ params.param_parallon_ra_core_dataset_name }}`.gr_gl_recn AS b USING
  (SELECT max(cclog65mcrip.coid) AS coid,
          max(CASE
                  WHEN cclog65mcrip.patient_number LIKE '%*%' THEN translate(cclog65mcrip.patient_number, '*', '')
                  ELSE cclog65mcrip.patient_number
              END) AS patient_number,
          parse_date('%m/%d/%y',REGEXP_EXTRACT(SPLIT(cclog65mcrip.reporting_period, " -")[0], r'[0-9]+/[0-9]+/[0-9]+')) AS costreportyearbegin,
          '501000' AS gl_acct_num,
          'H' AS company_code,
          sum(cclog65mcrip.var_contractual) AS var_contractual
   FROM `{{ params.param_parallon_ra_stage_dataset_name }}`.cclog65mcrip
   WHERE cclog65mcrip.var_contractual <> 0
     AND upper(cclog65mcrip.report_name) LIKE 'CCLOG65%'
   GROUP BY upper(cclog65mcrip.coid),
            upper(CASE
                      WHEN cclog65mcrip.patient_number LIKE '%*%' THEN translate(cclog65mcrip.patient_number, '*', '')
                      ELSE cclog65mcrip.patient_number
                  END),
...
MERGE INTO `{{ params.param_parallon_ra_core_dataset_name }}`.gr_gl_recn AS mt USING
  (SELECT DISTINCT 'H' AS company_code,
                   ...
                   substr(max(CASE
                                  WHEN upper(a.rate_schedule_name) LIKE '%GOV MIPS IP%' THEN 'MCR-IP'
                              END), 1, 10) AS log_type,
                   ROUND(CAST(`{{ params.param_bqutil_fns_dataset_name }}`.cw_td_normalize_number(CASE
 ...
 'N' AS fourth_day_delay_ind
   FROM `{{ params.param_parallon_ra_stage_dataset_name }}`.cclog65mcrip AS a
   WHERE a.var_contractual <> 0
     AND upper(a.report_name) LIKE 'CCLOG65%'
     AND NOT EXISTS
       (SELECT 1
        FROM `{{ params.param_parallon_ra_stage_dataset_name }}`.cclog24mcrip AS b
        WHERE b.expected_contractual <> 0
          AND upper(b.file_type) LIKE 'CCLOG24MCRIP_D%'
          AND upper(rtrim(b.coid, ' ')) = upper(rtrim(a.coid, ' '))
...
MERGE INTO `{{ params.param_parallon_ra_core_dataset_name }}`.gr_gl_recn AS b USING
 ...
 sum(gl.gl_proc_amt) AS total_gl_posted_acct_amt
   ...
   WHERE gl.gl_account = 501000
     ...
     AND upper(rtrim(gr.log_type, ' ')) = 'MCR-IP'
