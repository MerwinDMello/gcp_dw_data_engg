...
INNER JOIN {{ params.param_parallon_ra_stage_dataset_name }}.payor_organization AS pyro ON upper(rtrim(pyro.coid)) = upper(rtrim(rcos.coid))
AND upper(rtrim(pyro.company_code)) = upper(rtrim(rcos.company_code))
AND pyro.iplan_id = wrk.iplan_id
WHERE wrk.schema_id = `$schemaid`;
...
FROM
  (SELECT DISTINCT coalesce(eow.corrections_account_amt, CAST(0 AS NUMERIC)) AS z1,
   ...
   AND eow.report_run_date = wrk.rpt_run_date) AS z
