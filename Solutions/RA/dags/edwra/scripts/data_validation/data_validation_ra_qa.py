...
def get_table_row_count_teradata(src, table_name, td_where_clause,  df, lud_tbls):
...
group by 1, 2,3 ) t1"
        ...
        count, bq_min_lud, bq_max_lud  = bq_count_query_job.result().to_dataframe().iloc[0]
...
def generate_except_query(project_id, dataset_id, table_name, ignore_cols, bq_where_clause, mir_where_clause):
    ...
    if 'parallon' in project_id:
        mir_table_schema = f'{dataset_id}_copy'
    # else:
    #     mir_table_schema = f'{dataset_id}_base_views_copy'
...
#     , ', '
#   ORDER BY
#     ordinal_position) || ' from  `' || table_catalog || '`.' || '{mir_table_schema}' || '.' || table_name || ' {mir_where_clause} ' || ') bq_mir union all select  mir_bq.* from ( select '|| STRING_AGG(
#     CASE
#       WHEN data_type = 'DATETIME' THEN 'datetime_trunc('|| column_name || ', SECOND)' || column_name
#       WHEN data_type = 'STRING' THEN 'trim(upper(' || column_name || '))' || column_name
#     ELSE
...
#     , ', '
#   ORDER BY
#     ordinal_position) || ' from  `' || table_catalog || '`.' || '{mir_table_schema}' || '.' || table_name || ' {mir_where_clause} ' || ' except distinct select '|| STRING_AGG(
#     CASE
...
#    '''
  ...
  ORDER BY
    ordinal_position) || ' from  `' || table_catalog || '`.' || table_schema || '.' || table_name || ' {bq_where_clause} ' || ' except distinct  select ' || STRING_AGG(
    CASE
      WHEN data_type = 'DATETIME' THEN 'datetime_trunc( cast('|| column_name || ' as datetime), SECOND)' || column_name
      WHEN data_type = 'STRING' THEN 'trim(upper(' || column_name || '))' || column_name
    ELSE
    column_name
  END
    , ', '
  ORDER BY
    ordinal_position) || ' from  `' || table_catalog || '`.' || '{mir_table_schema}' || '.' || table_name || ' {mir_where_clause} ' || ') bq_mir  ) ) ;' AS except_query
FROM
  `{project_id}`.{dataset_id}.INFORMATION_SCHEMA.COLUMNS
WHERE
  table_name = '{table_name}'
  ...
  table_name;
   '''
        ...
        if except_query:
            ...
            except_query_result = except_query_job.result().to_dataframe().iloc[0]['diff_qry_result']
...
def generate_column_check_query(project_id, dataset_id, table_name, ignore_cols, bq_where_clause, mir_where_clause, td_keycols):
...
WITH
  ...
  SELECT
    ...
    ordinal_position
  FROM
    `{project_id}`.{mir_table_schema}.INFORMATION_SCHEMA.COLUMNS
  WHERE
    table_name = '{table_name}' )
SELECT
  CONCAT('\
    ...
    select  count(*) as total_count, \
        ...
        inner join deduped_landing_src as b \
...
GROUP BY
  ...
  b.full_name ;
        ...
        traceback.print_exc()
        print("Error while running query for {}.{}.{}".format(project_id, dataset_id, table_name))
        print(cc_query)
        return 0, None
...
def datavalidation(table_name,df):

    try:
        parts = table_name.split('~')
        src, in_teradata_schema, in_teradata_table, in_td_where_clause, in_bigquery_dataset, in_bigquery_table, in_bq_where_clause, in_ignore_cols, in_mir_where_clause, in_comments = parts
        ...
        if not 'staging' in in_teradata_schema:
            teradata_schema = in_teradata_schema + '_base_views'
        else:
            teradata_schema = in_teradata_schema
        ...
        if not in_bq_where_clause:
            bq_where_clause = in_td_where_clause
        else:
        ...
        td_count, td_min_lud, td_max_lud, td_keycols = get_table_row_count_teradata(src,teradata_schema + '.' + in_teradata_table, in_td_where_clause,  df, lud_tbls)
        bq_count, bq_min_lud, bq_max_lud = get_table_row_count_bigquery(src,project_id, bigquery_dataset, bigquery_table, bq_where_clause)
        ...
        mirr_dataset = f'{bigquery_dataset}_copy'
        #print(mirr_dataset)
        ...
        diff_qry_result, diff_qry = generate_except_query(
            project_id, bigquery_dataset, bigquery_table, ignore_cols, bq_where_clause, mir_where_clause)
        table['diff_qry_result'] = diff_qry_result

        count_check_query, count_check_query_results = generate_column_check_query(
            project_id, bigquery_dataset, bigquery_table, ignore_cols, bq_where_clause, mir_where_clause, td_keycols)
...
if __name__ == "__main__":
    ...
    input_file='ra_table_list_daily.txt'


    # TODO: Change based on environment that the validation is running in
    gcp_project_id='hca-hin-qa-cur-parallon'
    data_validation_results_table=f'{gcp_project_id}.edwra_ac.data_validation_results'
    email_list=''
    # TODO: add path to local path where these script will run
    basefolder ='C:\\Users\\LWA3758\\OneDrive - HCA Healthcare\\Documents\\validation_script'
    ...
    teradata_db_params['user'] = '<enter_username_here>'
    # td_password='<enter pass here>'
    # teradata_db_params['password'] = td_password
    ...
    td_lud_tbls_file_name = 'td_lud_tbls.csv'
    td_lud_tbls_file = os.path.join(basefolder, td_lud_tbls_file_name)
    lud_tbls = []
    ...
    data_validation_results_table = data_validation_results_table
    ...
    try:
        ...
        print("Tables names are ",table_names)
    ...
    try:
        ...
        # results = pool.map(datavalidation, table_names)
        # pool.close()
        # pool.join()
        results=[]
        for table in table_names:
            results.append(datavalidation(table,df))

        for t in results:
            result_table.append(t)

        print("Result tables is : ",result_table)
        ...
        if result_table:
            ...
            print("Tabulated data has been saved to {}".format(output_file_name))
            ...
            if data_validation_results_table:
                ...
                df = pd.read_csv(os.path.join(basefolder,  'output', output_file_name))
