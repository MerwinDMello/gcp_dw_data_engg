...
def get_execution_date(execution_date, **kwargs):  
    ...
    if schedule=="0 4 * * 1":
        poke_date = execution_date + timedelta(days=6)
        print("The New Execution Date is ",poke_date)
        ...
        for ind, item in enumerate(dependency):
            ...
            with TaskGroup(group_id=f'TG-table-{table}') as table_grp:
                ...
                update_audit_table = PythonOperator(
                                ...
                                python_callable = cu.executevalidationsqls_ext,
...
config_taleo = cu.call_config_yaml("hrg_taleo_integrate_dependency.yaml","hca_hrg_taleo_integrate_dependency")
for integrate in config_taleo['integrate']:
