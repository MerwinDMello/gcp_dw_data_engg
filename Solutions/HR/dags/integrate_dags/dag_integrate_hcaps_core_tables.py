...
default_args = {
    ...
    'params': {
        ...
        "param_auth_base_views_dataset_name": config['env']['v_curated_project_id'] + 
            ...
            with TaskGroup(group_id=f'TG-table-{table}') as table_grp:
                ...
                for idx,sql in enumerate(item["sql"]):
                        ...
                        update_bq_table = BigQueryOperator(
                            ...
                            query_params=[{ 'name': 'frequency', 'parameterType': { 'type': 'STRING' },'parameterValue': { 'value': frequency } }],
...
config_hcaps = cu.call_config_yaml("hrg_hcaps_integrate_dependency.yaml","hca_hrg_hcaps_integrate_dependency")
for integrate in config_hcaps['integrate']:
