...
def get_execution_date(execution_date, **kwargs):  
    ...
    exec_day = int(execution_date.day)
    print(exec_day)
    schedule = kwargs["params"]["schedule"]
    print(schedule)
    schedule_day = schedule.split()[2]
    if schedule_day == '*':
...
def create_dag( dag_id, schedule, dependency):
    ...
    with dag:
        ...
        for ind, item in enumerate(dependency):
            ...
            with TaskGroup(group_id=f'TG-table-{table}') as table_grp:
                ...
                for idx,sql in enumerate(item["sql"]):
                        update_bq_table = BigQueryOperator(
                            ...
                            sql = f'dml/integration/incremental/bi/{sql["name"]}',
            ...
            if has_sensor == 'Yes':
                with TaskGroup(group_id=f'TG-sensor') as sensor_grp:
...
config_bi = cu.call_config_yaml("hrg_bi_integrate_dependency.yaml","hca_hrg_bi_integrate_dependency")
for integrate in config_bi['integrate']:
