...
with DAG(
    ...
    with TaskGroup(group_id=f'TG-copy-tables') as table_grp:
        for var in copy_var:
            ...
            v_target_copy_dataset = var['target_copy_dataset']
        
            copy_data = BigQueryOperator(
                gcp_conn_id=config['env']['v_curated_conn_id'],
                sql = f"CALL `{config['env']['v_curated_project_id']}.{config['env']['v_hr_core_dataset_name']}`.copy_table_data_with_valid_to_date_adjustment('{config['env']['v_curated_project_id']}','{v_db}','{v_table}','{v_date}', '{v_update}', {v_use_copy_dataset}, {v_target_copy_dataset})",
