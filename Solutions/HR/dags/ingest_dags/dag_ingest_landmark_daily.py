...
srcsys_config = cu.call_config_yaml("landmark_config.yaml", "landmark_vars")
...
with DAG(
    ...
    with TaskGroup(group_id='run_dataflow_jobs') as tg1:
        for i in range(1, 3):
            with TaskGroup(group_id=f'TG-landmark_df_tblist-{i}') as tablegroup:
                run_dataflow_job = [BashOperator(
                    task_id="run_landmark_df_tblist{}".format(i),
                    dag = dag,
                    # to start parallel tasks at random time within 2 minutes, sleep n seconds 
                    bash_command='sleep $(shuf -i 1-120 -n 1) ; python /home/airflow/gcs/dags/scripts/{} --src_sys_config_file=landmark_config.yaml --src_sys_airflow_varname=landmark_vars --src_tbl_list=tblist{}'.format(srcsys_config['v_jdbcbqpytemplate'], i)
                    )]
                wait_for_python_job_async_done = DataflowJobStatusSensor(
                    task_id="wait_for_python_job_async_done",
                    job_id=f"{{{{ task_instance.xcom_pull(task_ids= 'run_dataflow_jobs.TG-landmark_df_tblist-{i}.run_landmark_df_tblist{i}')}}}}",
