...
src_config = cu.call_config_yaml("tms_sftp_config.yaml", "tms_sftp_config_vars")
...
gsutil_command = f'gsutil -m cp gs://{src_bucket}/edwhrdata/srcfiles/tms/* gs://"{tgt_bucket}/edwhrdata/srcfiles/tms/"'
...
def create_dag( dag_id, schedule, start_date, dependency,source_system,frequency,sftp_files):
    ...
    with dag:
        ...
        update_audit_data_for_files = PythonOperator(
                    ...
                    ,provide_context=True,
                    op_kwargs={'dependency': dependency,'src_config': config_tms}
        )
            ...
            with TaskGroup(group_id=f'TG-table-{target}') as table_grp:
                ...
                preprocessfile = PythonOperator(
                        ...
                        op_kwargs = {
                            ...
                            #'srcfilename' : '1_EDWHR_COMP_RATING_RPT.csv',
                            'sourcesysname' : source_system,
                            'delimiter': delimiter,
                            'header': header,
                            'quotechar': quotechar,
...
config_tms = cu.call_config_yaml("hrg_tms_ingest_dependency.yaml","hca_hrg_tms_ingest_dependency")
for ingest in config_tms['ingest']:
