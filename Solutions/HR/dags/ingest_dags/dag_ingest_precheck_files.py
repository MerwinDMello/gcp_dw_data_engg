...
gsutil_command = f'gsutil -m cp gs://{src_bucket}/edwhrdata/srcfiles/precheck/* gs://"{tgt_bucket}/edwhrdata/srcfiles/precheck/"'
...
def create_dag( dag_id, schedule, start_date, dependency,source_system,frequency,sftp_files):
    ...
    with dag:
        ...
        update_audit_data_for_files = PythonOperator(
                    ...
                    ,provide_context=True,
                    op_kwargs={'dependency': dependency,'src_config': config_precheck}
        )
...
config_precheck = cu.call_config_yaml("hrg_precheck_ingest_dependency.yaml","hca_hrg_precheck_ingest_dependency")
for ingest in config_precheck['ingest']:
