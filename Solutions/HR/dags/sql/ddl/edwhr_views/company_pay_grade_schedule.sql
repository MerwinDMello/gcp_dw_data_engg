/***************************************************************************************
S E C U R I T Y   V I E W
****************************************************************************************/

  CREATE OR REPLACE VIEW {{ params.param_hr_views_dataset_name }}.company_pay_grade_schedule AS SELECT
      a.company_pay_schedule_sid,
      a.pay_grade_code,
      a.pay_step_num,
      a.eff_from_date,
      a.valid_from_date,
      a.valid_to_date,
      a.pay_schedule_code,
      a.grade_sequence_num,
      a.pay_rate_amt,
      a.lawson_company_num,
      a.process_level_code,
      a.active_dw_ind,
      a.source_system_code,
      a.dw_last_update_date_time
    FROM
      {{ params.param_hr_base_views_dataset_name }}.company_pay_grade_schedule AS a
      INNER JOIN {{ params.param_hr_base_views_dataset_name }}.hr_secref_process_level AS c ON a.process_level_code = c.process_level_code
       AND a.lawson_company_num = c.lawson_company_num
       AND c.user_id = session_user()
  ;

