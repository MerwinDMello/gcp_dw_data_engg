-- No target-dialect support for source-dialect-specific SHOW object
CREATE OR REPLACE VIEW {{ params.param_hr_views_dataset_name }}.factrnmovement_prod AS (
  SELECT DISTINCT
      -- -- From Movement data
      ...
      CASE
        ...
        WHEN tm.analytics_msr_sid = 80200
         AND tm.from_position_sid IS NULL THEN 'Other Hire'
        WHEN tm.analytics_msr_sid = 80300
         AND tm.to_position_sid IS NULL THEN 'Other Term'
        ...
        WHEN upper(jclf.job_class_code) = '103'
         AND upper(jclt.job_class_code) = '103' THEN 'Aux Status Change'
      ...
      CASE
        ...
        ELSE 'SkillMix Chg'
      END AS chg_group,
      -- WHEN CHANGE_Type = 'Other_Term' THEN 'Other_Term'
      -- WHEN CHANGE_Type = 'Other_Hire' THEN 'Other_Hire'
      compdt.week_number,
      concat(CAST(compdt.week_start_dt as STRING), ' to ', CAST(compdt.week_end_dt as STRING)) AS week_range,
      CASE
        WHEN /*AND jclT.Job_Class_Code = '103'*/ upper(jclf.job_class_code) = '103'
         AND upper(staf.status_code) = 'FT' THEN /*AND STATo.Status_Code <> 'FT'*/ -1
        WHEN upper(jclf.job_class_code) = '103'
        ...
        ELSE 0
      END AS msr_value,
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code = '103' AND STATo.Status_Code  = 'FT' AND staf.Status_Code  <> 'FT' THEN 1
      --  WHEN jclf.Job_Class_Code <>'103' AND jclT.Job_Class_Code = '103' AND STATo.Status_Code = 'FT' THEN 1
      -- WHEN jclf.Job_Class_Code ='103' /*AND jclT.Job_Class_Code <> '103'*/  AND staf.Status_Code = 'FT' THEN -1
      /*AND jclT.Job_Class_Code = '103'*/ /*AND STATo.Status_Code <> 'PT'*/
      --  WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code = '103' AND STATo.Status_Code  = 'PT' AND staf.Status_Code  <> 'PT' THEN 1
      -- WHEN jclf.Job_Class_Code <>'103' AND jclT.Job_Class_Code = '103' AND STATo.Status_Code = 'PT' THEN 1
      -- WHEN jclf.Job_Class_Code ='103' /*AND jclT.Job_Class_Code <> '103' */ AND staf.Status_Code = 'PT' THEN -1
      -- WHEN jclf.Job_Class_Code ='103' /*AND jclT.Job_Class_Code = '103'*/ AND staf.Status_Code = 'PRN' AND STATo.Status_Code = 'TEMP' THEN 0
      -- WHEN jclf.Job_Class_Code ='103' /*AND jclT.Job_Class_Code = '103'*/ AND staf.Status_Code = 'TEMP' AND STATo.Status_Code = 'PRN' THEN 0
      /*AND jclT.Job_Class_Code = '103'*/ /* AND STATo.Status_Code <> 'PRN'*/ -- - Added to catch PRN RN Status Changes
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code = '103' AND STATo.Status_Code  = 'PRN' AND staf.Status_Code  <> 'PRN' THEN 1  --- Added to catch PRN RN Status Changes
      -- WHEN jclf.Job_Class_Code <>'103' AND jclT.Job_Class_Code = '103' AND STATo.Status_Code = 'PRN' THEN 1  --- Added to catch PRN RN Status Changes
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code <> '103'  AND staf.Status_Code = 'PRN' THEN -1  --- Added to catch PRN RN Status Changes
      /*AND jclT.Job_Class_Code = '103' */ /*AND STATo.Status_Code <> 'TEMP' */ -- - Added to catch Temp RN Status Changes
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code = '103' AND STATo.Status_Code  = 'TEMP' AND staf.Status_Code  <> 'TEMP' THEN 1  --- Added to catch Temp RN Status Changes
      -- WHEN jclf.Job_Class_Code <>'103' AND jclT.Job_Class_Code = '103' AND stato.Status_Code = 'TEMP' THEN 1  --- Added to catch Temp RN Status Changes
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code <> '103'  AND staf.Status_Code = 'TEMP' THEN -1  --- Added to catch Temp RN Status Changes
      -- WHEN jclf.Job_Class_Code IS NULL AND jclT.Job_Class_Code = '103' THEN 1  --- Removed Aux status check so that PRN and RN Hires into 103 are assisgned a value
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code IS NULL THEN -1  --- Removed Aux status check so that PRN and RN Terms into 103 are assisgned a value
      facf.lob_code,
      facf.sub_lob_code,
      concat(trim(staf.status_code), ' to ', trim(stato.status_code)) AS aux_chg,
      current_date('US/Central') AS date_stamp
    FROM
      ...
      LEFT OUTER JOIN edw_pub_views.fact_facility AS facf ON tm.from_coid = facf.coid
       AND tm.from_company_code = facf.company_code
      LEFT OUTER JOIN edw_pub_views.fact_facility AS facto ON tm.to_coid = facto.coid
      ...
      LEFT OUTER JOIN edw_pub_views.lu_date AS dt ON tm.date_id = dt.date_id
      INNER JOIN (
        SELECT
            dt_0.week_end_date,
          ...
          FROM
            edw_pub_views.lu_date AS dt_0
          WHERE dt_0.date_id >= (
            SELECT
                min(date_id)
              FROM
                edw_pub_views.lu_date
              WHERE date_id > date_sub(current_date(), interval 93 DAY)
       ...
       AND compdt.dtdiff = 6
      LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.hr_dept_rollup AS hrdrf ON tm.from_coid = hrdrf.coid
      ...
      LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.hr_dept_rollup AS hrdrt ON tm.to_coid = hrdrt.coid
    ...
    WHERE tm.date_id >= (
     ...
     AND tm.analytics_msr_sid IN(
      ...
      -- AND (jclf.Job_Class_Code <> jclt.Job_Class_Code OR staf.Status_Code <> STATo.Status_Code OR tm.From_Auxiliary_Status_SID IS NULL OR tm.To_Auxiliary_Status_Sid IS NULL)  --- Added to remove any jobcode dhanges where the Job Class or Aux Status did not change.
      80700, 80300
    )
  UNION ALL
  SELECT
      /*OR (tm.Analytics_Msr_Sid = 80200 AND tm.Action_Code LIKE '%1xfer%')*/
      -- AND tm.employee_Num IN ('100645643')
      ...
      CASE
        ...
        ELSE 0
      END AS msr_value,
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code = '103' AND staf.Status_Code = 'FT' AND STATo.Status_Code <> 'FT' THEN -1
      /*jclf.Job_Class_Code ='103' AND */ /*AND staf.Status_Code  <> 'FT'*/
      --  WHEN /*jclf.Job_Class_Code <>'103' AND */jclT.Job_Class_Code = '103' AND STATo.Status_Code = 'FT' THEN 1
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code <> '103'  AND staf.Status_Code = 'FT' THEN -1
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code = '103' AND staf.Status_Code = 'PT' AND STATo.Status_Code <> 'PT' THEN -1
      /*jclf.Job_Class_Code ='103' AND */ /*AND staf.Status_Code  <> 'PT' */
      -- WHEN jclf.Job_Class_Code <>'103' AND jclT.Job_Class_Code = '103' AND STATo.Status_Code = 'PT' THEN 1
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code <> '103'  AND staf.Status_Code = 'PT' THEN -1
      -- WHEN /*jclf.Job_Class_Code ='103' AND*/ jclT.Job_Class_Code = '103' AND staf.Status_Code = 'PRN' AND STATo.Status_Code = 'TEMP' THEN 0
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code = '103' AND staf.Status_Code = 'TEMP' AND STATo.Status_Code = 'PRN' THEN 0
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code = '103' AND staf.Status_Code = 'PRN' AND STATo.Status_Code <> 'PRN' THEN -1  --- Added to catch PRN RN Status Changes
      /*jclf.Job_Class_Code ='103' AND */ /*AND staf.Status_Code  <> 'PRN' */ -- - Added to catch PRN RN Status Changes
      -- WHEN jclf.Job_Class_Code <>'103' AND jclT.Job_Class_Code = '103' AND STATo.Status_Code = 'PRN' THEN 1  --- Added to catch PRN RN Status Changes
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code <> '103'  AND staf.Status_Code = 'PRN' THEN -1  --- Added to catch PRN RN Status Changes
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code = '103' AND staf.Status_Code = 'TEMP' AND STATo.Status_Code <> 'TEMP' THEN -1  --- Added to catch Temp RN Status Changes
      /* jclf.Job_Class_Code ='103' AND */ /*AND staf.Status_Code  <> 'TEMP' */ -- - Added to catch Temp RN Status Changes
      -- WHEN jclf.Job_Class_Code <>'103' AND jclT.Job_Class_Code = '103' AND stato.Status_Code = 'TEMP' THEN 1  --- Added to catch Temp RN Status Changes
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code <> '103'  AND staf.Status_Code = 'TEMP' THEN -1  --- Added to catch Temp RN Status Changes
      -- WHEN jclf.Job_Class_Code IS NULL AND jclT.Job_Class_Code = '103' THEN 1  --- Removed Aux status check so that PRN and RN Hires into 103 are assisgned a value
      -- WHEN jclf.Job_Class_Code ='103' AND jclT.Job_Class_Code IS NULL THEN -1  --- Removed Aux status check so that PRN and RN Terms into 103 are assisgned a value
          ...
          FROM
            (
              SELECT
                  ...
                  employee_position.eff_from_date AS position_hire_date
                FROM
                  {{ params.param_hr_views_dataset_name }}.employee_position
                WHERE date(employee_position.valid_to_date) = '9999-12-31'
                QUALIFY CASE
                  WHEN employee_position.position_sid <> lag(employee_position.position_sid, 1, 0) OVER (PARTITION BY employee_position.employee_sid ORDER BY position_hire_date) THEN 1
                  ELSE 0
...
UNION DISTINCT
SELECT
    /*OR (tm.Analytics_Msr_Sid = 80300 AND tm.Action_Code LIKE '%1xfer%')*/
  ...
  FROM
    ...
    LEFT OUTER JOIN {{ params.param_hr_base_views_dataset_name }}.status AS staf ON hc.auxiliary_status_sid = staf.status_sid
     AND date(staf.valid_to_date) = '9999-12-31'
    LEFT OUTER JOIN edw_pub_views.fact_facility AS facf ON hc.coid = facf.coid
    ...
    LEFT OUTER JOIN {{ params.param_hr_base_views_dataset_name }}.process_level AS plf ON hc.process_level_code = plf.process_level_code
     ...
     AND date(plf.valid_to_date) = '9999-12-31'
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.hr_dept_rollup AS hrdrt ON hc.coid = hrdrt.coid
  ...
  WHERE hc.date_id = (
   ...
   AND hc.process_level_code NOT IN(
    '18345', '26394', '00637'
  )
  GROUP BY 1, 2, 3, 4, upper(CASE
    ...
    WHEN upper(hc.process_level_code) = '08936' THEN '27150'
    ELSE hc.process_level_code
  END), 7, 8, 9, 10, 11, 12, 13, 14, 18, 19, 21, 22, 23
;
