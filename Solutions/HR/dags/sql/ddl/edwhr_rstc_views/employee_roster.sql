...
-- A.Carpenter - creating version in RSTC views for Talent Management to get Birthdate/Age Num
****************************************************************************************/
CREATE OR REPLACE VIEW {{params.param_hr_rstc_views_dataset_name}}.employee_roster AS SELECT
    a.employee_sid,
    a.position_sid,
    a.date_id,
    a.active_dw_ind,
    ...
    END AS employee_ssn,
    CASE
      WHEN session_user() = hr.userid THEN cast(a.employee_home_phone_num as string)
      ELSE '***'
