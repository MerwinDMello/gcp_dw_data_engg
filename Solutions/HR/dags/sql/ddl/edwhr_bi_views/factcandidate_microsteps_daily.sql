create or replace view {{ params.param_hr_bi_views_dataset_name }}.factcandidate_microsteps_daily as SELECT
    --  =============================================
    --  Author:        Cheryl Costa
    --  Create date:    02/2/2022
    -- -Copied logic from snapshot view to create daily.
    --  =============================================
    cpm.dw_last_update_date_time AS date_id,
    ...
    CASE
      WHEN date_diff(cpm.microstep_end_date_time, cpm.microstep_start_date_time, DAY) > 365 THEN CAST(timestamp_diff(cpm.microstep_end_date_time, cpm.microstep_start_date_time, DAY) as NUMERIC)
  ...
  FROM
   {{ params.param_hr_base_views_dataset_name }}.candidate_pathway_microstep AS cpm 
    LEFT JOIN {{ params.param_hr_base_views_dataset_name }}.candidate_profile AS cp ON cpm.candidate_profile_sid = cp.candidate_profile_sid
     AND DATE(cp.valid_to_date) = DATE('9999-12-31')
    LEFT JOIN {{ params.param_hr_base_views_dataset_name }}.ref_pathway AS p ON cpm.pathway_id = p.pathway_id
    LEFT OUTER JOIN (
      SELECT DISTINCT
    ...
    LEFT JOIN {{ params.param_hr_base_views_dataset_name }}.candidate AS c ON cp.candidate_sid = c.candidate_sid
     AND DATE(c.valid_to_date) = DATE('9999-12-31')
    LEFT JOIN {{ params.param_hr_base_views_dataset_name }}.candidate_person AS person ON c.candidate_sid = person.candidate_sid
    ...
    LEFT JOIN {{ params.param_hr_base_views_dataset_name }}.ref_microstep AS ms ON cpm.microstep_num = ms.microstep_num
    LEFT JOIN {{ params.param_hr_base_views_dataset_name }}.submission AS s ON cp.candidate_profile_sid = s.candidate_profile_sid
