CREATE OR REPLACE VIEW {{ params.param_hr_bi_views_dataset_name }}.factheadcount_daily AS SELECT
    CASE
      WHEN hrmet.date_id > last_day(date_add(current_date('US/Central'), interval -1 MONTH)) THEN hrmet.date_id
    ...
    CASE
      WHEN (hrmet.lawson_company_num) = 300 THEN date_diff(hrmet.date_id , emp.adjusted_hire_date ,month)
      ELSE date_diff(hrmet.date_id , coalesce(anniv_hist.anniversary_date, emp.anniversary_date), month)
    END AS tenure,
  ...
  FROM
    ...
    /*LEFT JOIN {{ params.param_hr_base_views_dataset_name }}.Employee_Position ep -- 7/6/2021 Added by Stephen for Schedule_Work_Code (Shift Code)
    				ON hrmet.employee_sid = ep.Employee_SID
    				AND hrmet.position_sid = ep.position_sid
    				AND ep.Valid_To_Date = '9999-12-31'
  ...
  WHERE (hrmet.analytics_msr_sid) = 80100
   AND hrmet.date_id BETWEEN '2016-01-01' AND current_date('US/Central')
   AND hrmet.date_id = CASE
    WHEN hrmet.date_id > last_day(date_add(current_date('US/Central'), interval -1 MONTH)) THEN hrmet.date_id
    ELSE last_day(DATE(hrmet.date_id))
  END
;
