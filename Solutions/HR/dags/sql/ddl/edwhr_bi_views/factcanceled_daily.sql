CREATE OR REPLACE VIEW {{ params.param_hr_bi_views_dataset_name }}.factcanceled_daily AS SELECT
    --  =============================================
    --  Author:        Cheryl Costa
  ...
  FROM
    ...
    INNER JOIN (
      SELECT
        ...
        WHERE date(rr.valid_to_date) = date'9999-12-31'
    ...
    (
      SELECT
        ...
        WHERE date(requisition_workflow.valid_to_date) = date'9999-12-31'
        AND (requisition_workflow.end_date) <> DATE'9999-12-31'
        GROUP BY 1
    ...
    LEFT OUTER JOIN (
      SELECT
        ...
        WHERE date(req.valid_to_date) = date'9999-12-31'
    ...
    LEFT OUTER JOIN (
      SELECT
        ...
        WHERE recruitment_requisition_history.requisition_status_id = 3
         AND date(recruitment_requisition_history.valid_to_date) = date'9999-12-31'
    ...
    LEFT OUTER JOIN {{ params.param_hr_base_views_dataset_name }}.recruitment_requisition_status AS rrs ON recreq.recruitment_requisition_sid = rrs.recruitment_requisition_sid
     AND date(rrs.valid_to_date) = date'9999-12-31'
    ...
    {{ params.param_hr_base_views_dataset_name }}.recruitment_user AS recuser2 ON recreq.hiring_manager_user_sid = recuser2.recruitment_user_sid
     AND date(recuser2.valid_to_date) = date'9999-12-31'
    ...
    {{ params.param_hr_base_views_dataset_name }}.requisition_status AS rs ON r.requisition_sid = rs.requisition_sid
     AND date(rs.valid_to_date) = date'9999-12-31'
    ...
    INNER JOIN {{ params.param_hr_base_views_dataset_name }}.gl_lawson_dept_crosswalk AS gldc ON jp.gl_company_num = gldc.gl_company_num
     ...
     AND date(gldc.valid_to_date) = date'9999-12-31'
    ...
    {{ params.param_hr_base_views_dataset_name }}.ref_integrated_lob AS mat3 ON upper(trim(df.functional_dept_desc)) = upper(trim(mat3.functional_dept_desc))
    ...
    LEFT OUTER JOIN (
      SELECT DISTINCT
        ...
        FROM
          {{ params.param_hr_views_dataset_name }}.fact_hr_metric
        WHERE (fact_hr_metric.analytics_msr_sid) = 80600
    ...
    LEFT OUTER JOIN (
      SELECT DISTINCT
        ...
        FROM
          {{ params.param_hr_views_dataset_name }}.fact_hr_metric
        WHERE (fact_hr_metric.analytics_msr_sid) = 80500
