...
-- Source: eim-cs-da-gmek-5764-dev/HRG/Bteq_Source_Files/SARANYADEVI/missing_views/EDWHR_BI_Views/rkg_factrequisitionworkflow_rkg.sql
-- Translated from: Teradata
-- Translated to: BigQuery

CREATE OR REPLACE VIEW {{ params.param_hr_bi_views_dataset_name }}.rkg_factrequisitionworkflow_rkg AS SELECT DISTINCT
    --  =============================================
    --  Author: Cheryl Costa
    ...
    r.last_update_date,
    rp.position_sid,
    concat(rp.position_sid, jp.eff_to_date) AS position_key,
    hrdr.coid,
  ...
  FROM
    {{ params.param_hr_views_dataset_name }}.requisition_workflow AS rw
    INNER JOIN {{ params.param_hr_views_dataset_name }}.requisition AS r ON rw.requisition_sid = r.requisition_sid
     AND date(r.valid_to_date) = '9999-12-31'
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.employee AS e ON rw.workflow_user_id_code = e.employee_34_login_code
     AND date(e.valid_to_date) = '9999-12-31'
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.employee_person AS emp ON e.employee_num = emp.employee_num
     AND date(emp.valid_to_date) = '9999-12-31'
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.employee AS e2 ON r.originator_login_3_4_code = e2.employee_34_login_code
     AND date(e2.valid_to_date) = '9999-12-31'
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.employee_person AS emp2 ON e2.employee_num = emp2.employee_num
     AND date(emp2.valid_to_date) = '9999-12-31'
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.employee_position AS ep ON emp2.employee_sid = ep.employee_sid
     AND date(ep.valid_to_date) = '9999-12-31'
     AND r.requisition_origination_date BETWEEN ep.eff_from_date AND ep.eff_to_date
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.job_position AS jpo ON ep.position_sid = jpo.position_sid
     AND date(jpo.valid_to_date) = '9999-12-31'
     AND ep.eff_from_date BETWEEN jpo.eff_from_date AND jpo.eff_to_date
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.requisition_position AS rp ON r.requisition_sid = rp.requisition_sid
     AND r.process_level_code = rp.process_level_code
     AND date(rp.valid_to_date) = '9999-12-31'
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.job_position AS jp ON rp.position_sid = jp.position_sid
     AND date(jp.valid_to_date) = '9999-12-31'
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.requisition_department AS rd ON r.requisition_sid = rd.requisition_sid
     AND date(rd.valid_to_date) = '9999-12-31'
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.department AS d ON rd.dept_sid = d.dept_sid
     AND date(d.valid_to_date) = '9999-12-31'
    LEFT OUTER JOIN {{ params.param_hr_bi_views_dataset_name }}.rkg_hr_dept_rollup_rkg AS hrdr ON r.process_level_code = hrdr.process_level_code
    ...
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.job_code AS jc ON jp.job_code_sid = jc.job_code_sid
     AND date(jc.valid_to_date) = '9999-12-31'
    LEFT OUTER JOIN {{ params.param_hr_views_dataset_name }}.job_class AS jcl ON jc.job_class_sid = jcl.job_class_sid
  ...
  WHERE r.requisition_eff_date BETWEEN date_add(last_day(date_add(current_date('US/Central'), interval -24 MONTH)), interval 1 DAY) AND date_sub(current_date('US/Central'), interval extract(DAY from current_date('US/Central')) DAY)
   AND date(rw.end_date) < '9999-12-31'
  QUALIFY row_number() OVER (PARTITION BY r.requisition_sid, approver_order ORDER BY approver_order, rw.workflow_user_id_code) = 1
;
--  Rolling 24 months
