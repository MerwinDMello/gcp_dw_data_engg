CREATE OR REPLACE PROCEDURE `{{ params.param_hr_core_dataset_name }}.copy_table_data_with_valid_to_date_adjustment_v2`(project_name STRING, src_ds_name STRING, tgt_ds_name STRING, tbl_name STRING, last_update_date DATE, update_flag STRING)
BEGIN
...
EXECUTE IMMEDIATE CONCAT("Select Count(table_id) from `hca-hin-dev-cur-hr`."||src_ds_name||".__TABLES__ Where table_id = '"||tbl_name||"'") INTO cnt;

IF cnt = 0 THEN
  RAISE USING MESSAGE = CONCAT("Source Table / View ",tbl_name," not found in Dataset ",src_ds_name);
END IF;
...
SET final_Query = CONCAT(" INSERT INTO ", tgt_ds_name, ".", tbl_name," (", insert_Query,") Select ",select_query," From `hca-hin-dev-cur-hr`.", src_ds_name,".", tbl_name);
