create table if not exists `{{ params.param_hr_core_dataset_name }}.candidate_pathway_microstep`
(
...
CLUSTER BY candidate_profile_sid, pathway_id, microstep_num
OPTIONS(
  description="This table assigns each onboarding candidate to a pathway and its associated microsteps. This reloads each day with only the current days data."
);