create table if not exists `{{ params.param_hr_core_dataset_name }}.process_level`
(
  process_level_sid INT64 NOT NULL OPTIONS(description="et generated unique value for an process level code"),
  ...
  process_level_name STRING NOT NULL OPTIONS(description="unique name of an hr company facility or process level. this field is spaces name for hr companyrecords"),
  process_level_active_code STRING OPTIONS(description="indicates if the process level is active in lawson"),
...
OPTIONS(
  description="unique process level of an hr company mainatined in this table"
);