...
CREATE OR REPLACE VIEW {{ params.param_hr_stnd_views_dataset_name }}.psg_requisition AS SELECT DISTINCT
  ...
  WHERE date(r.valid_to_date) = '9999-12-31'
   AND date(r.requisition_open_date) >= '2018-01-01'
   AND ff.lob_code IN(
    'PHY', 'UCC', 'LAB', 'ECH', 'NTW', 'HBP'
  )
;
