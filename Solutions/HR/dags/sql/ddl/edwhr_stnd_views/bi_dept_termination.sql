...
CREATE OR REPLACE VIEW {{ params.param_hr_stnd_views_dataset_name }}.bi_dept_termination AS SELECT
    -- -added 9/23/22
    -- -added 9/23/22
    -- -added 9/23/22
    -- added 9/23/22
  ...
  WHERE hrmet.date_id BETWEEN date_add(last_day(date_add(current_date(), interval -25 MONTH)), interval 1 DAY) AND current_date()
