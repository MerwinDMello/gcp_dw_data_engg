...
-- # 04/06/2021   Thomas Jones    Added Lob_Code, Job_Class_Desc, Director_Grouping_Desc #  #
-- # 05/04/2021   Thomas Jones     Added 1XFERINT to the Action_Code Filter    #  #
-- #                                                               #
-- #####################################################################################
CREATE OR REPLACE VIEW {{ params.param_hr_stnd_views_dataset_name }}.bi_dept_hires AS SELECT DISTINCT
    -- -added 9/23/22
    -- -added 9/23/22
  ...
  FROM
    ...
    INNER JOIN {{ params.param_hr_base_views_dataset_name }}.hr_secref_process_level AS c ON fhrm.process_level_code = c.process_level_code
  ...
  WHERE (fhrm.analytics_msr_sid) = 80200
   AND fhrm.date_id BETWEEN date_add(last_day(date_add(current_date(), interval -25 MONTH)), interval 1 DAY) AND current_date()
   AND fhrm.action_code IN(
    -- -- END FTE_Percent join
