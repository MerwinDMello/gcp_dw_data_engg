...
(select count(*) from
(select *
from {{ params.param_hr_base_views_dataset_name }}.respondent_patient_detail
where source_system = 'H' and dw_last_update_date_time >= tableload_start_time  - interval 1 minute
union all
select * from {{ params.param_hr_stage_dataset_name }}.respondent_patient_detail_exception));
