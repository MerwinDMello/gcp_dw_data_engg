...
src_rec_count = 
(select count(*)
FROM {{ params.param_hr_stage_dataset_name }}.fixed_resp_pat_dtl_core_wrk1);
...
(select count(*) from
(select *
from {{ params.param_hr_base_views_dataset_name }}.fixed_respondent_patient_detail
where 
source_system_code = 'H' and
dw_last_update_date_time >= tableload_start_time  - interval 1 minute

union all 

select * FROM 
{{ params.param_hr_stage_dataset_name }}.fixed_resp_pat_dtl_excp
