...
srctablename = Null;
SET
tgttablename = concat('edwhr.','ref_phone_mode_adjustment');
SET
...
SELECT
      ...
      FROM
        ...
        LEFT OUTER JOIN {{ params.param_hr_core_dataset_name }}.ref_phone_mode_adjustment AS tgt  for system_time as of timestamp(tableload_start_time,'US/Central')
        ON tgt.measure_id_text = stg.measure_id_text
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.ref_phone_mode_adjustment
where eff_to_date = DATE '9999-12-31' and dw_last_update_date_time >= tableload_start_time  - interval 1 minute);
