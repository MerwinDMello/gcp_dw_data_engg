...
declare
sourcesysnm,srcdataset_id,srctablename,tgtdataset_id,tgttablename,audit_type,tableload_run_time,job_name,audit_status string;
declare
...
(select arr[offset(1)]
from(
select split("{{ params.param_hr_stage_dataset_name }}" , '.') as arr));
SET
srctablename = concat(srcdataset_id, '.','bi_psat_weekly_rank_report' );
SET
tgtdataset_id = 
(select arr[offset(1)]
from(
select split("{{ params.param_hr_core_dataset_name }}" , '.') as arr));
SET
tgttablename =concat(tgtdataset_id, '.', @p_table);
SET
...
src_rec_count = 
(SELECT count(*)
FROM {{ params.param_hr_stage_dataset_name }}.bi_psat_weekly_rank_report);
