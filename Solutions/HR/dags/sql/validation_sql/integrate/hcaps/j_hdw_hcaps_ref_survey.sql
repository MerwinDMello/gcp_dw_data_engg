...
SELECT DISTINCT
            ...
            where concat(hca_quest_mstr.ptype,hca_quest_mstr.qtype,hca_quest_mstr.category) not in 
            (select concat(tgt.survey_category_num,tgt.survey_category_code,tgt.survey_category_text) 
            from {{ params.param_hr_core_dataset_name }}.ref_survey as tgt  for system_time as of timestamp(tableload_start_time,'US/Central')
             where tgt.eff_to_date ='9999-12-31')
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.ref_survey
where eff_to_date = '9999-12-31'
