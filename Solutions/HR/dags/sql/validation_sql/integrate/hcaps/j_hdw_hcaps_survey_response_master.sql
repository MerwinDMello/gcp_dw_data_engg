...
src_rec_count = 
(select count(*)
from {{ params.param_hr_stage_dataset_name }}.survey_response_master_wrk wrk
left outer join 
(select * from {{ params.param_hr_core_dataset_name }}.survey_response_master 
where dw_last_update_date_time < tableload_start_time  - interval 1 minute
and source_system_code = 'H'
)
tgt
on
wrk.survey_question_sid=tgt.survey_question_sid and
wrk.response_value_text=tgt.response_value_text
where tgt.survey_response_sid is null);


SET
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.survey_response_master
