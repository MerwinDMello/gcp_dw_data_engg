...
srctablename = Null;
SET
tgttablename = 'edwhr.requisition_approval_stage';
SET
...
WHERE
  (
    concat(trim(stg.requisition_approval_type_code),
    stg.requisition_sid,
    ...
    FROM
      {{ params.param_hr_core_dataset_name }}.requisition_approval_stage for system_time as of timestamp(tableload_start_time,'US/Central')
    WHERE
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.requisition_approval_stage
