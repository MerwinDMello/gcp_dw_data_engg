...
from 
(
select distinct action_type from {{ params.param_hr_stage_dataset_name }}.persaction
union distinct
select distinct action_type from {{ params.param_hr_stage_dataset_name }}.persacthst
union distinct
select distinct action_type from {{ params.param_hr_stage_dataset_name }}.histerr
));
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.ref_action_type
