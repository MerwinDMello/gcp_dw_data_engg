...
SELECT
      ...
      left outer join {{ params.param_hr_core_dataset_name }}.ref_hr_sk_xwlk b for system_time as of timestamp(tableload_start_time,'US/Central') 
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.ref_hr_sk_xwlk
where DATE(dw_last_update_date_time) = DATE(current_ts)
) ;
