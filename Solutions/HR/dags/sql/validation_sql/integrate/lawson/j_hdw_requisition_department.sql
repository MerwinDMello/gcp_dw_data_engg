...
FROM
  ...
  LEFT OUTER JOIN {{ params.param_hr_core_dataset_name }}.requisition_department AS tgt for system_time as of timestamp(tableload_start_time,'US/Central') 
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_core_dataset_name }}.requisition_department
where date(valid_to_date) = '9999-12-31' and dw_last_update_date_time >= tableload_start_time  - interval 1 minute AND source_system_code = 'L'
) ;
