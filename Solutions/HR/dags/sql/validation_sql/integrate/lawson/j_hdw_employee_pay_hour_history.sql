...
select  
  *
from
  {{ params.param_hr_stage_dataset_name }}.prtime
));
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.employee_pay_hour_history
where  date(valid_to_date) = '9999-12-31'
 and source_system_code ='L' and   
(concat(employee_num,lawson_company_num,check_id ,pay_summary_group_code ,time_seq_id) ) 
in  ( select concat(employee,company,check_id,pay_sum_grp,time_seq) from {{ params.param_hr_stage_dataset_name }}.prtime )
) ;
