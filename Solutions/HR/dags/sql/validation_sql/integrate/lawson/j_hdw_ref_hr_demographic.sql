...
(
SELECT
        src.demographic_code,
        src.demographic_type_code,
        src.active_flag,
        src.demographic_desc,
        ...
        (
          SELECT
            ...
            FROM
              ...
              where hr.active_flag = 'A' or hr.hrctry_code = 'U'
            GROUP BY 1, 2, 3, 4, 5
        ) AS src
      QUALIFY row_number() OVER (PARTITION BY src.demographic_code, src.demographic_type_code ORDER BY src.active_flag, upper(src.source_system_code) DESC) = 1
));
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.ref_hr_demographic
