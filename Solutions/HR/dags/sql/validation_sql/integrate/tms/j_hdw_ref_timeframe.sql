...
(SELECT count(*)
FROM
  (SELECT stg.flight_risk_timeframe
   FROM {{ params.param_hr_stage_dataset_name }}.employee_info stg
   LEFT JOIN {{ params.param_hr_core_dataset_name }}.ref_timeframe tgt 
   for system_time as of timestamp(tableload_start_time,'US/Central') 
   ON coalesce(tgt.timeframe_desc, '~') =coalesce(stg.flight_risk_timeframe, '~')
   WHERE tgt.timeframe_desc IS NULL
   AND stg.flight_risk_timeframe IS NOT NULL
   GROUP BY 1)stg);
