...
(SELECT count(*)
FROM
  (SELECT future_role_attribute_desc
   FROM {{ params.param_hr_stage_dataset_name }}.ref_future_role_attribute_wrk stg
   WHERE UPPER(stg.future_role_attribute_desc) not in
       (SELECT UPPER(future_role_attribute_desc)
