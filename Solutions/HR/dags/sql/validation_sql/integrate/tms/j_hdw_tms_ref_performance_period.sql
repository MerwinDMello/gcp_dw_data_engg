...
(SELECT count(*)
FROM
  (SELECT review_period_desc
   FROM {{ params.param_hr_stage_dataset_name }}.ref_performance_period_wrk stg
   WHERE UPPER(stg.review_period_desc) not in
       (SELECT UPPER(review_period_desc)
