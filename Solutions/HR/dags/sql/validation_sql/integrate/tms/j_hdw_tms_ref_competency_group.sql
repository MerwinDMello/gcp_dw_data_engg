...
(SELECT count(*)
FROM
  (SELECT competency_group_desc
   FROM {{ params.param_hr_stage_dataset_name }}.ref_competency_group_wrk stg
   WHERE UPPER(stg.competency_group_desc) not in
       (SELECT UPPER(competency_group_desc)
