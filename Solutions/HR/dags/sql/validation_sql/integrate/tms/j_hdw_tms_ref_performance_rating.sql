...
(SELECT count(*)
FROM
  (SELECT performance_rating_desc
   FROM {{ params.param_hr_stage_dataset_name }}.ref_performance_rating_wrk stg
   WHERE UPPER(stg.performance_rating_desc) not in
       (SELECT UPPER(performance_rating_desc)
