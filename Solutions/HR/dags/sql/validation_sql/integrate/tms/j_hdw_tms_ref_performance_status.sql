...
(SELECT count(*)
FROM
  (SELECT performance_status_desc
   FROM {{ params.param_hr_stage_dataset_name }}.ref_performance_status_wrk stg
   WHERE UPPER(REGEXP_REPLACE(stg.performance_status_desc, r'([^\x20-\x7E]+)', ''))  not in
       (SELECT UPPER(REGEXP_REPLACE(performance_status_desc, r'([^\x20-\x7E]+)', ''))
        FROM {{ params.param_hr_core_dataset_name }}.ref_performance_status
        for system_time as of timestamp(tableload_start_time,'US/Central') ) ) src);
