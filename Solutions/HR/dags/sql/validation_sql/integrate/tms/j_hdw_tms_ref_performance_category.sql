...
(SELECT count(*)
FROM
  (SELECT stg.goal_category
   FROM
     (SELECT goal_category,
             row_number() OVER (PARTITION BY goal_category
                                ORDER BY goal_category) AS rownum
      FROM {{ params.param_hr_stage_dataset_name }}.employee_perf_goals) stg
