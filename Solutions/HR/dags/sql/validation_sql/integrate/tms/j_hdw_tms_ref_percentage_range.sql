...
SELECT
  ...
  FROM
    ...
    LEFT OUTER JOIN {{ params.param_hr_core_dataset_name }}.ref_percentage_range AS tgt 
    for system_time as of timestamp(tableload_start_time,'US/Central') 
    ON UPPER(stg.percentage_range_desc) = UPPER(tgt.percentage_range_desc)
  WHERE tgt.percentage_range_desc IS NULL) y));
