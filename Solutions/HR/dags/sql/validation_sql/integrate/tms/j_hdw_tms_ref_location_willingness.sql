...
(SELECT count(*)
FROM
  (SELECT location_willingness_desc
   FROM {{ params.param_hr_stage_dataset_name }}.ref_location_willingness_wrk stg
   WHERE UPPER(stg.location_willingness_desc) not in
       (SELECT UPPER(location_willingness_desc)
