...
FROM
  (SELECT competency_desc
   FROM {{ params.param_hr_stage_dataset_name }}.ref_competency_wrk stg
   WHERE UPPER(REGEXP_REPLACE(stg.competency_desc, r'([^\x20-\x7E]+)', '')) not in
       (SELECT UPPER(REGEXP_REPLACE(competency_desc, r'([^\x20-\x7E]+)', ''))
