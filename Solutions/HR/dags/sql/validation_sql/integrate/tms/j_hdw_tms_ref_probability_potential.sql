...
(SELECT count(*)
FROM
  (SELECT probability_potential_desc
   FROM {{ params.param_hr_stage_dataset_name }}.ref_probability_potential_wrk stg
   WHERE UPPER(stg.probability_potential_desc) not in
       (SELECT UPPER(probability_potential_desc)
