...
src_rec_count = 
(SELECT count(*)
FROM {{ params.param_hr_stage_dataset_name }}.ref_nursing_program_wrk);
...
FROM
  (SELECT *
   FROM {{ params.param_hr_core_dataset_name }}.ref_nursing_program
   WHERE dw_last_update_date_time >= tableload_start_time - interval 1 MINUTE )a);
