...
srctablename = Null;
SET
tgttablename = 'edwhr.ref_posting_board_type';
SET
...
SELECT
      ...
      FROM
        ...
        LEFT OUTER JOIN {{ params.param_hr_core_dataset_name }}.ref_posting_board_type AS tgt for system_time as of timestamp(tableload_start_time,'US/Central')
        ON coalesce(trim(tgt.posting_board_type_code), 'XXX') = coalesce(trim(stgg.posting_board_type_code), 'XXX')
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.ref_posting_board_type
