...
srctablename = Null;
SET
tgttablename = 'edwhr.ref_recruitment_source';
SET
...
SELECT 
    0 AS recruitment_source_id,
    TRIM(source) AS recruitment_source_desc,
    0 AS recruitment_source_type_id,
    'B'AS SOURCE_SYSTEM_CODE,
    current_ts AS DW_LAST_UPDATE_DATE_TIME
FROM {{ params.param_hr_stage_dataset_name }}.ats_cust_jobapplication_stg
WHERE TRIM(source) != ''
GROUP BY 1,2,3,4,5));
