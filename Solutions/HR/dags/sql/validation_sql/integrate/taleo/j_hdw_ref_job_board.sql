...
srctablename = Null;
SET
tgttablename = 'edwhr.ref_job_board';
SET
...
SELECT
      ...
      FROM
        ...
        LEFT OUTER JOIN {{ params.param_hr_core_dataset_name }}.ref_job_board AS tgt for system_time as of timestamp(tableload_start_time,'US/Central')
        ON coalesce(tgt.job_board_id, 99999) = coalesce(stgg.job_board_id , 99999)
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.ref_job_board
