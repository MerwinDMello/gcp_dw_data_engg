...
srctablename = Null;
SET
tgttablename = 'edwhr.submission_state';
SET
...
SELECT
      ...
      WHERE (concat(coalesce(wrk.submission_sid, 0 ), coalesce(wrk.submission_state_id, 0 ))) NOT IN(
        SELECT 
            concat(coalesce(tgt.submission_sid, 0 ),
            coalesce(tgt.submission_state_id, 0))
          FROM
            {{ params.param_hr_core_dataset_name }}.submission_state AS tgt for system_time as of timestamp(tableload_start_time,'US/Central')
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.submission_state
