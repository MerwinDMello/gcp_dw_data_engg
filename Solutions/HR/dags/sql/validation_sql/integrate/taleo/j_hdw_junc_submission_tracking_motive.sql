...
srctablename = Null;
SET
tgttablename = 'edwhr.junc_submission_tracking_motive';
SET
...
SELECT
      ...
      WHERE (concat(trim(CAST(coalesce(wrk.submission_tracking_sid, 0) as STRING)), trim(CAST(coalesce(wrk.tracking_motive_id, 0) as STRING)), wrk.source_system_code)) NOT IN(
        SELECT 
            concat(trim(CAST(coalesce(tgt.submission_tracking_sid, 0) as STRING)),
            trim(CAST(coalesce(tgt.tracking_motive_id, 0) as STRING)),
            tgt.source_system_code)
          FROM
            {{ params.param_hr_core_dataset_name }}.junc_submission_tracking_motive AS tgt for system_time as of timestamp(tableload_start_time,'US/Central')
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.junc_submission_tracking_motive
