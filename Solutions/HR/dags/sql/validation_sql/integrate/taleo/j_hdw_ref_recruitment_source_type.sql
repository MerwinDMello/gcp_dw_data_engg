...
srctablename = Null;
SET
tgttablename = 'edwhr.ref_recruitment_source_type';
SET
...
FROM {{ params.param_hr_stage_dataset_name }}.taleo_recruitmentsourcetype te
GROUP BY 1

UNION ALL

SELECT
    trim(trim(_source)) AS recruitment_source_type_code
FROM {{ params.param_hr_stage_dataset_name }}.ats_cust_v3_jobapplication_stg
