...
srctablename = Null;
SET
tgttablename = 'edwhr.recruitment_position';
SET
...
WHERE
  (concat(wrk.recruitment_position_sid,
  ...
  SELECT
    concat(tgt.recruitment_position_sid,
  ...
  FROM
    {{ params.param_hr_core_dataset_name }}.recruitment_position AS tgt for system_time as of timestamp(tableload_start_time,'US/Central')
  WHERE
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.recruitment_position
