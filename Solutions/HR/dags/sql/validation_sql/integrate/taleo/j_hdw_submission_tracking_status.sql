...
srctablename = Null;
SET
tgttablename = 'edwhr.submission_tracking_status';
SET
...
SELECT
      ...
      WHERE (concat(coalesce(cast(wrk.submission_tracking_sid as int64), 0), coalesce(cast(wrk.submission_status_id as int64), 0))) NOT IN(
        SELECT concat(
            coalesce(tgt.submission_tracking_sid, 0),
            coalesce(tgt.submission_status_id, 0))
          FROM
            {{ params.param_hr_core_dataset_name }}.submission_tracking_status AS tgt for system_time as of timestamp(tableload_start_time,'US/Central')
...
tgt_rec_count =
(select count(*)
from {{ params.param_hr_base_views_dataset_name }}.submission_tracking_status
