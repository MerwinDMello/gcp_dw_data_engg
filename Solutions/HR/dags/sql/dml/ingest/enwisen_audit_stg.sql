...
FROM {{ params.param_hr_stage_dataset_name }}.enwisen_audit
...
OR SAFE_CAST(requisitionnumber AS INT64) IS NULL
;
UPDATE {{ params.param_hr_stage_dataset_name }}.enwisen_audit
SET dw_last_update_date_time = DATETIME_TRUNC(CURRENT_DATETIME('US/Central'), SECOND),
  workflowname = NULLIF(Trim(workflowname),''),
  workflowstatus = NULLIF(Trim(workflowstatus),''),
  workflowapprover = NULLIF(Trim(workflowapprover),'')
WHERE 1 = 1;