...
USING
  ...
  FROM (
    SELECT
      REPLACE(domain_stg.time_period, 'Q', '0') AS time_period_name,
      ff.company_code,
      ff.corporate_code,
      ff.group_code,
      ff.division_code,
      ...
      MAX(CASE
          WHEN LENGTH(domain_stg.time_period) = 6 THEN 'Quarter'
    ...
    FROM
      {{ params.param_hr_stage_dataset_name }}.domn_data_cms_rfq AS domain_stg
    INNER JOIN
