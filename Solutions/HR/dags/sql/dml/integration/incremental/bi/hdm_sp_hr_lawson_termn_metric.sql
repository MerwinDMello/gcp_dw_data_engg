...
FROM (
  ...
  FROM
    {{ params.param_hr_base_views_dataset_name }}.person_action AS paterm
  INNER JOIN
    {{ params.param_hr_base_views_dataset_name }}.employee AS e
  ON
    e.employee_sid = paterm.employee_sid
  ...
  ON
    ep.employee_sid = paterm.employee_sid
    AND DATE(ep.valid_to_date) = '9999-12-31'
    AND paterm.eff_from_date = ep.eff_to_date
  WHERE
    DATE(paterm.valid_to_date) = '9999-12-31'
    AND UPPER(trim(paterm.action_code)) LIKE '%TERM%'
    AND DATE(paterm.eff_from_date) >= '2016-01-01' QUALIFY ROW_NUMBER() OVER (PARTITION BY paterm.employee_num, paterm.lawson_company_num, paterm.eff_from_date ORDER BY ep.position_level_sequence_num, paterm.action_last_update_date DESC) = 1
  UNION DISTINCT
  SELECT
    paxfer.employee_sid,
    paxfer.action_code,
  ...
  INNER JOIN
    {{ params.param_hr_base_views_dataset_name }}.employee AS e
  ON
    e.employee_sid = paxfer.employee_sid
    AND DATE(e.valid_to_date) = '9999-12-31'
  LEFT OUTER JOIN
    {{ params.param_hr_base_views_dataset_name }}.junc_employee_status AS jes
  ON
  ...
  INNER JOIN (
    SELECT
      ...
      ROW_NUMBER() OVER (PARTITION BY a.employee_sid ORDER BY a.position_level_sequence_num, a.eff_from_date) AS row_rank
    FROM (
      SELECT
      ...
      WHERE
        DATE(employee_position.valid_to_date) = '9999-12-31' ) AS a ) AS ep2
  ON
    ep2.employee_sid = paxfer.employee_sid
  ...
  INNER JOIN (
    ...
    FROM (
      ...
      FROM
        {{ params.param_hr_base_views_dataset_name }}.employee_position
      WHERE
        DATE(employee_position.valid_to_date) = '9999-12-31' ) AS a ) AS ep1
  ON
    ep1.employee_sid = paxfer.employee_sid
    AND paxfer.eff_from_date - 1 BETWEEN ep1.eff_from_date
    AND ep1.eff_to_date
  WHERE
    UPPER(trim(paxfer.action_code)) LIKE '%1XFER%'
    AND DATE(paxfer.valid_to_date) = '9999-12-31'
    AND DATE(paxfer.eff_from_date) >= '2016-01-01'
    AND ep1.row_rank + 1 = ep2.row_rank
    AND ep1.process_level_code <> ep2.process_level_code QUALIFY ROW_NUMBER() OVER (PARTITION BY paxfer.employee_num, paxfer.lawson_company_num, paxfer.eff_from_date ORDER BY ep2.position_level_sequence_num, paxfer.action_last_update_date DESC) = 1 ) AS pa
INNER JOIN
  {{ params.param_hr_base_views_dataset_name }}.gl_lawson_dept_crosswalk AS gldc
ON
  pa.gl_company_num = gldc.gl_company_num
...
ON
  pa.position_sid = jp.position_sid
  AND DATE(jp.valid_to_date) = '9999-12-31'
  AND pa.eff_from_date BETWEEN jp.eff_from_date
  AND jp.eff_to_date
INNER JOIN
...
ON
  pa.employee_sid = s.employee_sid
  AND pa.eff_from_date - 1 BETWEEN s.status_from_date
...
ON
  pa.employee_sid = es.employee_sid
  AND pa.eff_from_date - 1 BETWEEN es.status_from_date
...
ON
  UPPER(trim(dm.analytics_msr_name_child)) = 'HR_LAWSON_TERMINATIONS'
LEFT OUTER JOIN
...
ON
  pa.process_level_code = mat4.process_level_code
