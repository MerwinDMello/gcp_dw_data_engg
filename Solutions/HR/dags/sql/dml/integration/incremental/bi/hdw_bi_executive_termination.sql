BEGIN
  ...
  BEGIN TRANSACTION;
    ...
    INNER JOIN (
      ...
      WHERE
        UPPER(trim(transfer_termination_flag)) = 'T'
    ...
    LEFT OUTER JOIN (
      ...
      WHERE
        UPPER(trim(fcd.facility_phone_type_code)) = 'F' QUALIFY ROW_NUMBER() OVER (PARTITION BY fcd.coid, fcd.company_code ORDER BY fcd.facility_phone_type_code DESC, fcd.eff_from_date DESC) = 1 ) AS facnum
    ON
