...
INSERT INTO {{ params.param_hr_core_dataset_name }}.fact_hr_metric_mv (employee_sid, requisition_sid, position_sid, date_id, analytics_msr_sid, dept_sid, job_class_sid, job_code_sid, location_code, coid, company_code, functional_dept_num, sub_functional_dept_num, auxiliary_status_sid, employee_status_sid, key_talent_id, integrated_lob_id, action_code, action_reason_text, lawson_company_num, process_level_code, work_schedule_code, recruiter_owner_user_sid, requisition_approval_date, employee_num, metric_numerator_qty, metric_denominator_qty, source_system_code, dw_last_update_date_time) 
  SELECT
      ...
      FROM
        (
          SELECT
            ...
            FROM
              ...
              INNER JOIN {{ params.param_hr_base_views_dataset_name }}.employee AS e ON e.employee_sid = paterm.employee_sid
              ...
              INNER JOIN {{ params.param_hr_base_views_dataset_name }}.employee_position AS ep ON ep.employee_sid = paterm.employee_sid
            ...
            FROM
              ...
              INNER JOIN (
                SELECT
                    ...
                    a.gl_company_num,
                    a.account_unit_num,
                    row_number() OVER (PARTITION BY a.employee_sid ORDER BY a.eff_from_date) AS row_rank
                  FROM
                    (
                      SELECT
                          ...
                          min(employee_position.eff_from_date) AS eff_from_date,
                          max(employee_position.eff_to_date) AS eff_to_date
                        FROM
        ...
        INNER JOIN {{ params.param_dim_base_views_dataset_name }}.dim_analytics_measure AS dm ON upper(dm.analytics_msr_name_child) = 'HR_LAWSON_HIRES'
      ...
      QUALIFY row_number() OVER (PARTITION BY pa.employee_sid, pa.position_sid, date_id ORDER BY pa.action_last_update_date DESC) = metric_numerator_qty;
