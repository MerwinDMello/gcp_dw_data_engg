...
---volitale table collects position data history to join to the pa subquery in order to return
--- previous position information for any relavent records
...
LEFT OUTER JOIN {{ params.param_hr_base_views_dataset_name }}.junc_employee_status jes
...
left outer join (
				select	company_pay_schedule_sid, 
						...
						pay_step_num,
						eff_from_date, 
						case when min(eff_from_date) over (partition by company_pay_schedule_sid,pay_grade_code order by eff_from_date desc rows between 2 preceding and 1 preceding) is null then cast ('12/31/9999' as date format 'mm/dd/yyyy')
						else min(eff_from_date) over (partition by company_pay_schedule_sid,pay_grade_code order by eff_from_date desc rows between 2 preceding and 1 preceding) end as eff_to_date,
						pay_schedule_code,
						pay_rate_amt, 
						lawson_company_num
...
inner join (select date_id from {{ params.param_pub_views_dataset_name }}.lu_date where date_id < current_date('US/Central')) d
		on d.date_id between ep.eff_from_date and ep.eff_to_date

LEFT OUTER JOIN {{ params.param_hr_stage_dataset_name }}.aux_status s
...
left outer join {{ params.param_hr_base_views_dataset_name }}.person_action pa
		...
		and ep.eff_to_date = pa.eff_from_date
		and pa.valid_to_date = datetime("9999-12-31 23:59:59")
		and upper(trim(pa.action_code)) = '1TERMPEND'
...
---volitale table collects initial position start dates to act as a buffer between the pa subquery and
--- the pos_date volitale table to correct for issue of hire records incorrectly joining to positions the
--- employee held and in some cases continues to hold prior to the hire action
...
select distinct 
		...
		max(ep.eff_to_date) as start_date
from {{ params.param_hr_base_views_dataset_name }}.employee_position ep
where ep.valid_to_date = datetime("9999-12-31 23:59:59")
		and ep.lawson_company_num <> 300
...
where upper(trim(analytics_msr_name_child)) ='HR_LAWSON_HIRES')
...
pa.action_code,
pa.action_reason_text,
80200 as movement_analytics_sid,
case when pos_date.position_sid is null then 'External' else 'Internal' end as movement_type,
case when pos_date.pay_rate_amt > 1  --- movement direction is defined as a 20% increase or decrease in mid pay rate for the position and must have a position change
...
pos_date.position_sid as from_position_sid, 
pos_date.position_level_sequence_num as from_position_level_sequence_num, 
pos_date.fte_percent as from_fte_percent,
pos_date.schedule_work_code as from_schedule_work_code,
pos_date.working_location_code as from_working_location_code, 
pos_date.dept_sid as from_dept_sid, 
pos_date.lawson_company_num as from_lawson_company_num, 
pos_date.process_level_code as from_process_level_code,
pos_date.job_code_sid as from_job_code_sid,
pos_date.job_class_sid as from_job_class_sid,
pos_date.coid as from_coid,
pos_date.company_code as from_company_code,
cast(pos_date.sub_functional_dept_num as int64) as from_sub_functional_dept_num,
cast(pos_date.functional_dept_num as int64) as from_functional_dept_num,
pos_date.auxiliary_status_sid as from_auxiliary_status_sid,
pos_date.employee_status_sid as from_employee_status_sid,
pos_date.pay_rate_amt as from_pay_rate_amt,
pos_date.key_talent_id AS from_key_talent_id,
pos_date.integrated_lob_id AS from_integrated_lob_id,
pa.position_sid as to_position_sid,
pa.position_level_sequence_num as to_position_level_sequence_num,
pa.fte_percent as to_fte_percent,
pa.schedule_work_code as to_schedule_work_code,
pa.working_location_code as to_working_location_code,
pa.dept_sid as to_dept_sid,
pa.lawson_company_num as to_lawson_company_num,
pa.process_level_code as to_process_level_code,
jc.job_code_sid as to_job_code_sid,
jc.job_class_sid as to_job_class_sid,
gldc.coid as to_coid,
gldc.company_code as to_company_code,
cast(sf.sub_functional_dept_num as int64) as to_sub_functional_dept_num,
cast(sf.functional_dept_num as int64) as to_functional_dept_num,
coalesce(s.aux_status_sid, pa.status_sid) as to_aux_status_sid,
es.emp_status_sid as to_emp_status_sid,
pay.pay_rate_amt as to_pay_rate_amt,
...
from  (
	...
	inner join {{ params.param_hr_base_views_dataset_name }}.employee_position ep
			...
			and paterm.eff_from_date between ep.eff_from_date and ep.eff_to_date
	
	where paterm.valid_to_date = datetime("9999-12-31 23:59:59")
			and upper(trim(paterm.action_code)) like '%HIRE%'
			and paterm.eff_from_date >= '2016-01-01'
	
	qualify row_number()over(partition by paterm.employee_num, paterm.lawson_company_num, paterm.eff_from_date order by paterm.action_last_update_date desc) = 1
	...
	ep2.working_location_code,
	ep2.process_level_code,
	ep2.position_sid,
	ep2.dept_sid,
	ep2.gl_company_num,
	ep2.account_unit_num,
	ep2.position_level_sequence_num,
	ep.fte_percent,
	ep.schedule_work_code,
	jes.status_sid
	...
	inner join	(select
					...
					row_number()over(partition by employee_sid, position_level_sequence_num order by eff_from_date) as row_rank --- added position_level_sequence_num to ensure that primary
					from (																										--- and secondary position movement actions on the same day are
																																--- aligning to the correct record
					select
					...
					dept_sid,
					gl_company_num,
					account_unit_num, --- removed fte percent due to excess records casuing records to incorrectly be omitted
					position_level_sequence_num,
					eff_from_date,
					eff_to_date
			...
			and paxfer.eff_from_date = ep2.eff_from_date
	...
	inner join (
			...
			and ep1.eff_to_date = paxfer.eff_from_date - 1
			
	LEFT OUTER JOIN {{ params.param_hr_base_views_dataset_name }}.employee_position ep --- added join to the ep2 subquery to get the fte percent of that position with out causing join issues
			on ep2.employee_sid = ep.employee_sid
			and ep2.position_sid = ep.position_sid
			and ep2.position_level_sequence_num = ep.position_level_sequence_num
			and ep.eff_from_date = ep2.eff_from_date
			and ep.valid_to_date = datetime("9999-12-31 23:59:59")
...
inner join {{ params.param_hr_stage_dataset_name }}.emp_status es
		...
		and pa.eff_from_date between es.status_from_date and es.status_to_date
		and upper(trim(es.emp_status_code)) in ('01','02','03','04','05')
...
--modified by thomas jones 12/16  subquery gets the mid_pay_rate for a position to calculate movement_direction
left outer join (
				select	company_pay_schedule_sid, 
						pay_grade_code, 
						pay_step_num,
...
left outer join min_pos
		...
		and pa.eff_from_date between min_pos.start_date and min_pos.start_date+1

left outer join pos_date
		on min_pos.employee_num = pos_date.employee_num
		and min_pos.start_date = pos_date.eff_to_date
		---and pos_date.eff_to_date = pos_date.date_id
		and min_pos.position_level_sequence_num = pos_date.position_level_sequence_num
		and ((upper(trim(pa.action_code)) like '%HIRE%' and upper(trim(pos_date.action_code)) = '1TERMPEND') 	
			  or (upper(trim(pa.action_code)) like '%1XFER%' and pa.employee_sid = pos_date.employee_sid))	



where pa.lawson_company_num <>300
and pa.eff_from_date >= '2016-01-01' --- added to limited the returned data

qualify row_number()over(partition by pa.employee_sid, pa.position_sid, pa.eff_from_date order by pa.action_last_update_date desc) = 1;
