...
CREATE TEMPORARY TABLE sub
  AS
    SELECT
      ...
      GROUP BY 1, 2, 3, 4, 5, upper(concat(coalesce(hrdr.coid, '00000'), hrdr.process_level_code, coalesce(hrdr.dept_code, '00000'), hrdr.lawson_company_num)), 7, 8, 9, 10, upper(CASE
        WHEN upper(jobcl.job_class_code) = '103' THEN 'RN'
        WHEN jobco.job_code IN(
          ndir.job_code
        ) THEN 'Leadership'
        WHEN upper(trim(jobco.job_code_desc)) = upper(trim(pct.job_code_desc))
         AND upper(trim(jobpos.position_code_desc)) = upper(trim(pct.job_title_text)) THEN 'PCT'
        ELSE 'Non-RN'
      END), 12, 13, 14, 15, 16, 17, 18, 19, upper(CASE
        WHEN (upper(jobpos.position_code_desc) LIKE '%PRN I'
         OR upper(jobpos.position_code_desc) LIKE '%PRN')
         AND upper(jobcl.job_class_code) = '103' THEN 'PRN Tier 1'
        WHEN upper(jobpos.position_code_desc) LIKE '%PRN II'
...
;
    ...
    SET DUP_COUNT = ( 
        ...
        from  {{ params.param_hr_core_dataset_name }}.fact_vacancy_snapshot
    ...
    IF DUP_COUNT <> 0 THEN
      ...
      drop table pos_date;
      drop table min_pos;
      RAISE USING MESSAGE = concat('Duplicates are not allowed in the table:  {{ params.param_hr_core_dataset_name }}.fact_vacancy_snapshot');
    ELSE
      COMMIT TRANSACTION;
    END IF;
