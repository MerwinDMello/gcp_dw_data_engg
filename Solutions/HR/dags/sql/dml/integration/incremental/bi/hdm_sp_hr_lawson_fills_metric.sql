...
CREATE TEMPORARY TABLE approvaldate AS (
  ...
  INNER JOIN (
    ...
    FROM(
      ...
      AND UPPER(rw.workflow_code) NOT LIKE '%ACQ%'
...
BEGIN TRANSACTION;
  INSERT INTO {{ params.param_hr_core_dataset_name }}.fact_hr_metric_mv (
    ...
    SELECT
        ...
        CAST(coalesce(ad.requisition_open_date_tb, reqday.end_date, reqday.requisition_open_date) AS DATE) AS requisition_approval_date,
        0 AS employee_num,
      ...
      FROM
        (
          SELECT
              ...
              rs.status_sid,
              rs.status_code,
              recreq.accept_date AS date_id,
              recreq.accept_date,
              recreq.offer_sid,
              recreq.offer_status_desc,
              recreq.approved_sw,
              rrh1.recruiter_owner_user_sid,
              rw.end_date,
            ...
            FROM
              ...
              INNER JOIN (
                SELECT
                  ...
                  FROM
                    ...
                    LEFT OUTER JOIN {{ params.param_hr_base_views_dataset_name }}.ref_workflow AS rwo ON rr.workflow_id = rwo.workflow_id
                  WHERE rr.valid_to_date = DATETIME("9999-12-31 23:59:59")
                   AND upper(trim(ros.offer_status_desc)) = 'ACCEPTED'
                   AND (rwo.workflow_code IS NULL
                   OR upper(trim(rwo.workflow_code)) NOT LIKE '%ACQ%')
                  QUALIFY row_number() OVER (PARTITION BY rr.lawson_requisition_sid ORDER BY o.last_modified_date desc, o.last_modified_time DESC, rr.approved_sw DESC, rr.recruitment_requisition_sid DESC, o.sequence_num DESC, o.accept_date DESC) = 1
              ...
              INNER JOIN (
                SELECT
                  ...
                  QUALIFY row_number() OVER (PARTITION BY rrh.recruitment_requisition_sid ORDER BY rrh.creation_date_time DESC) = 1
              ) AS rrh1 ON rrh1.recruitment_requisition_sid = recreq.recruitment_requisition_sid
            ...
            WHERE r.valid_to_date = DATETIME("9999-12-31 23:59:59")
             AND recreq.accept_date >= '2016-01-01'
        ) AS reqday
        ...
        INNER JOIN {{ params.param_dim_base_views_dataset_name }}.dim_analytics_measure AS dm ON upper(dm.analytics_msr_name_child) = 'HR_LAWSON_FILLED_POSITIONS'
      ...
      WHERE reqday.lawson_company_num <> 300
  ;
