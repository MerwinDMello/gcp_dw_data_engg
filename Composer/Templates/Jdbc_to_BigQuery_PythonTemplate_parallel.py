...
config = call_config_yaml("im_config.yaml")
...
class jdbctobq(beam.DoFn):
    ...
    def readjdbcwritebqtable(self, tableinfo, bqproject_id):
        ...
        def fetch_data_from_source(partition, query):
            attempt_limit = 3
            for attempt in range(attempt_limit):
                try:
                    ...
                    logging.info(f"Successfully connected to source database for partition {partition}.")
                    if is_partitioned:
                        formatted_clause = f"AND ABS(CAST({partition_column} AS INT)) % {num_partitions} = {partition}"
                        modified_query = f"{query} {formatted_clause}"
                    else:
                        modified_query = query
                    total_rows = 0
                    chunks = pd.read_sql(modified_query, conn, chunksize=v_chunksize)
                    ...
                    # TODO: See what error message is for connection closing
        ...
        def delete_from_temp_bigquery_table(partition_clause=None):
            delete_query = f"DELETE FROM {tgttablename} WHERE TRUE"
            if partition_clause is not None:
                delete_query = f"DELETE FROM {tgttablename} WHERE TRUE {partition_clause}"
        ...
        def write_data_to_bigquery(data, partition):
            ...
            for attempt in range(5):  # Retry up to 5 times
                try:
                    ...
                    pandas_gbq.to_gbq(data, tgttablename, project_id=bq_project_id, if_exists='append', table_schema=tblschema)
        ...
        def process_partition(partition, query):
            total_rows = 0
            chunk_generator = fetch_data_from_source(
                partition,
                query
            )
            ...
            for data_chunk in chunk_generator:
                ...
                data_chunk = process_chunk(data_chunk)
        ...
        try:
            ...
            if src_db_type == 'sqlserver':
                ...
                jdbc_url = f"jdbc:oracle:thin:@//{src_server_name}:{src_db_dict[0]['name'][0]}/{src_db_dict[0]['name'][1]}"
            ...
            tgttablename = tgttablename.replace(
                'v_im_stage_dataset_name', config['env']['v_im_stage_dataset_name'])
            tgttablename = tgttablename.replace(
                'v_im_core_dataset_name', config['env']['v_im_core_dataset_name'])
            ...
            if len(tableinfo.split('~')) > 6:
                is_partitioned = True
            else:
                is_partitioned = False
            
            if is_partitioned:
                num_partitions = int(tableinfo.split('~')[6])
                partition_column = tableinfo.split('~')[7]
            ...
            # conn_pool = pool.QueuePool(conn, num_partitions)
            # count_conn = conn_pool.connect()
            count_conn = jaydebeapi.connect(jdbc_class_name, jdbc_url, {'user': username, 'password': passwd}, jdbc_lib_path + jdbc_jar, jdbc_lib_path)

            src_rec_count = pd.read_sql(srctablecountquery, count_conn)
